...
## <a name="serverconfig"></a> Настройка ядра СУБД — конфигурация сервера
...
## <a name ="datadir"></a> Настройка ядра СУБД — каталоги данных
...
## <a name="a-nametempdba-database-engine-configuration---tempdb"></a><a name="tempdb"><a/> Настройка ядра СУБД — TempDB
...
## <a name="database-engine-configuration---maxdop"></a>Настройка ядра СУБД — MaxDOP
Максимальная степень параллелизма (MaxDOP) определяет максимальное число процессоров, которые может использовать отдельная инструкция. SQL Server 2019 позволяет настроить этот параметр при установке и автоматически определяет рекомендуемый параметр MaxDOP для сервера по числу ядер. Максимальное значение по умолчанию равно 8.  

Вы можете вручную настроить этот параметр здесь, а также изменить его после установки. Дополнительные сведения см. в разделе [Максимальная степень параллелизма](../../database-engine/configure-windows/configure-the-max-degree-of-parallelism-server-configuration-option.md).
::: moniker-end
...
>  Логины, подготавливаемые в ходе установки в качестве членов предопределенной роли сервера **sysadmin** , подготавливаются в качестве администраторов в базе данных образцов. Они являются членами предопределенной роли сервера **sysadmin** на пользовательском экземпляре, если не удалены  
 ...
 Чтобы внести изменения после завершения выбора, щелкните [!INCLUDE[clickOK](../../includes/clickok-md.md)]. Чтобы закрыть средство без внесения изменений, нажмите кнопку **Отмена**.  
