...
Ознакомительная версия для сообщества (CTP) 2.1 является последним общедоступным выпуском [!INCLUDE[sql-server-2019](..\includes\sssqlv15-md.md)]. Ниже приведены функции, которые добавлены или улучшены в [!INCLUDE[sql-server-2019](..\includes\sssqlv15-md.md)] CTP 2.1.

- [Кластеры больших данных](#bigdatacluster)
  - Развертывание приложений Python и R
- [Ядро СУБД](#databaseengine)
  - Интеллектуальная обработка запросов и масштабируемое встраивание определяемых пользователем функций
  - Улучшенное сообщение об ошибке усечения, которое включает имена таблицы и столбца, а также усеченное значение
  - Поддержка параметров сортировки UTF-8 при настройке [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]
  - Использование псевдонимов производной таблицы или представления для графовых запросов MATCH
  - Улучшенные данные диагностики для блокирующих операций получения статистики
...
[!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)] добавляет или расширяет следующие новые возможности ядра СУБД
...
Дополнительные сведения см. в [статье о встраивании скалярных функций, определяемых пользователем](../relational-databases/user-defined-functions/scalar-udf-inlining.md).
...
Новое сообщение об ошибке 2628 содержит больше сведений о проблеме с усечением данных, что упрощает процесс устранения ошибки. В CTP 2.1 это сообщение об ошибке активируется явным образом и требует включения [флага трассировки](../t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql.md) 460.

### <a name="utf-8-support-ctp-21"></a>Поддержка UTF-8 (CTP 2.1)

Добавлена возможность выбора параметра сортировки UTF-8 по умолчанию во время настройки [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)].
...
Маскирование данных — это процесс применения маски к базе данных, чтобы скрыть конфиденциальные сведения либо заменить их новыми или очищенными данными. В продуктах Майкрософт поддерживаются две операции маскирования — статическое и динамическое маскирование данных. Динамическое маскирование данных было добавлено в SQL Server 2017. В следующей таблице сравниваются два решения:
...
### <a name="utf-8-support-ctp-20"></a>Поддержка UTF-8 (CTP 2.0)
...
- [**SQL Server Management Studio (SSMS) 18.0 (предварительная версия)**](../ssms/sql-server-management-studio-ssms.md)
  ...
  - Меньший размер скачиваемых файлов (CTP 2.0).
...
Начиная с CTP 2.1 в [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)] не добавлены новые возможности для следующих служб:
