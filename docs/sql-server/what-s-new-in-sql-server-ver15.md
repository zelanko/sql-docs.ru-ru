...
Ознакомительная версия для сообщества (CTP) 2.0 является первым общедоступным выпуском [!INCLUDE[sql-server-2019](..\includes\sssqlv15-md.md)]. Ниже приведены функции, которые добавлены или улучшены в [!INCLUDE[sql-server-2019](..\includes\sssqlv15-md.md)] версии CTP 2.0.

- [Кластеры больших данных](#bigdatacluster)
  - Развертывание кластера больших данных с использованием контейнеров SQL и Spark на платформе Linux в Kubernetes.
  ...
  - Использование Azure Data Studio для запуска книг запросов, обеспечивающих условия работы с записной книжкой.

- [Ядро СУБД](#databaseengine)
  - Поддержка UTF-8.
  - Возобновляемое создание индекса в подключенном режиме позволяет возобновить процесс создания индекса после сбоя.
  - Сборка и перестройка кластеризованного индекса columnstore в подключенном режиме.
  - Always Encrypted с безопасными анклавами.
  - Интеллектуальная обработка запросов.
  - Расширение программирования языка Java.
  - Функции графа SQL.
  - Параметр конфигурации на уровне базы данных для возобновляемых операций DDL и операций DDL в подключенном режиме.
  - Группы доступности AlwaysOn — перенаправление подключения вторичной реплики.
  - Классификация и обнаружение данных — функции, встроенные в SQL Server.
  - Расширенная поддержка устройств с постоянной памятью.
  - Поддержка статистики columnstore в `DBCC CLONEDATABASE`.
  - Новые возможности, добавленные в хранимую процедуру `sp_estimate_data_compression_savings`.
  - Отказоустойчивые кластеры Служб машинного обучения SQL Server.
  - Упрощенная инфраструктура профилирования запросов включена по умолчанию.
  - Новые соединители Polybase.
  - Новая системная функция `sys.dm_db_page_info` возвращает сведения о странице.

- [SQL Server в Linux](#sqllinux)
  - Поддержка репликации.
  - Поддержка координатора распределенных транзакций Майкрософт (MSDTC).
  - Группа доступности AlwaysOn в контейнерах Docker с Kubernetes.
  - Поддержка OpenLDAP для сторонних поставщиков Active Directory.
  - Поддержка машинного обучения в Linux.
  - Новый реестр контейнеров.
  - Новые образы контейнеров на основе RHEL.
  - Уведомление о недостатке памяти.

- [Службы Master Data Services](#mds)
  - Заменены элементы управления Silverlight.

- [безопасность](#security)
  - Управление сертификатами в диспетчере конфигурации SQL Server.
...
- Выполнение функций расширенной аналитики и машинного обучения с помощью Spark.
- Использование потоковой передачи данных Spark в пулы данных SQL.
- Запуск книг запросов, обеспечивающих условия работы с записной книжкой в [**Azure Data Studio**](../sql-operations-studio/what-is.md).
  
[!INCLUDE [Big Data Clusters preview](../includes/big-data-cluster-preview-note.md)]

## <a id="databaseengine"></a> Ядро СУБД

CTP 2.0 обеспечивает реализацию или оптимизацию следующих новых функций для компонента "[!INCLUDE[ssdeNoVersion](../includes/ssdenoversion_md.md)]".
...
Например, `LATIN1_GENERAL_100_CI_AS_SC` меняется на `LATIN1_GENERAL_100_CI_AS_SC_UTF8`. UTF-8 доступна только для параметров сортировки Windows, которые поддерживают дополнительные символы, представленные в SQL Server 2012. `NCHAR` и `NVARCHAR` допускают только кодирование UTF-16 и остаются неизменными.

Эта функция может обеспечить значительную экономию места в хранилище в зависимости от используемой кодировки. Например, изменение имеющегося типа данных столбца со строками ASCII с `NCHAR(10)` на `CHAR(10)` с использованием параметров сортировки для UTF-8 почти в два раза снижает требования к хранению. Это снижение связано с тем, что `NCHAR(10)` требует для хранения 22 байта, тогда как `CHAR(10)` требует 12 байтов для той же строки Юникод.

### <a name="resumable-online-index-create"></a>Возобновляемое создание индексов в подключенном режиме
...
### <a name="build-and-rebuild-clustered-columnstore-indexes-online"></a>Сборка и перестройка кластеризованных индексов columnstore в Интернете
...
### <a id="ha"></a>Дополнительные синхронные реплики в группах доступности AlwaysOn 
...
### <a name="expanded-support-for-persistent-memory-devices"></a>Расширенная поддержка устройств с постоянной памятью
...
> [!NOTE]
> В этой предварительной версии компонент паравиртуализации файлов в постоянной памяти доступен только для Linux. SQL Server в Windows поддерживает устройства постоянной памяти, начиная с SQL Server 2016.

### <a name="support-for-columnstore-statistics-in-dbcc-clonedatabase"></a>Поддержка статистики columnstore в DBCC CLONEDATABASE
...
### <a name="new-options-added-to-spestimatedatacompressionsavings"></a>К sp_estimate_data_compression_savings добавлены новые параметры

`sp_estimate_data_compression_savings` возвращает текущий размер запрошенного объекта и оценивает размер объекта для запрошенного состояния сжатия.  Сейчас эта процедура поддерживает три параметра: `NONE`, `ROW` и `PAGE`. В SQL Server 2019 представлены два новых параметра: `COLUMNSTORE` и `COLUMNSTORE_ARCHIVE`. Эти новые параметры позволят оценить экономию места при создании индекса columnstore в таблице, для которой применяется стандартное или архивное сжатие данных columnstore.

### <a id="ml"></a>Отказоустойчивые кластеры Служб машинного обучения SQL Server и моделирование на основе разделов

- **Моделирование на основе разделов.** Обработка внешних скриптов на каждый раздел данных с использованием новых параметров, добавленных в `sp_execute_external_script`. Эта функция поддерживает обучение нескольких небольших моделей (одна модель на раздел данных) вместо одной большой.
...
### <a name="lightweight-query-profiling-infrastructure-enabled-by-default"></a>Упрощенная инфраструктура профилирования запросов включена по умолчанию
...
### <a name="new-sysdmdbpageinfo-system-function-returns-page-information"></a>Новая системная функция sys.dm_db_page_info возвращает сведения о странице
...
- **Поддержка репликации.** [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)] поддерживает репликацию SQL Server на платформе Linux. Виртуальная машина Linux с установленным агентом SQL может быть издателем, распространителем или подписчиком. 
...
## <a id="mds"></a>Службы Master Data Services (MDS)
...
- **Управление сертификатами в диспетчере конфигурации SQL Server.** Сертификаты SSL/TLS широко используются для предоставления безопасного доступа к экземплярам SQL Server. Управление сертификатами интегрировано в диспетчер конфигурации SQL Server, что упрощает выполнение таких распространенных задач:
...
- [**Azure Data Studio.**](../azure-data-studio/what-is.md) Ранее выпущенное в режиме предварительного просмотра с названием SQL Operations Studio средство Azure Data Studio представляет собой современный упрощенный кроссплатформенный рабочий стол с открытым исходным кодом для выполнения самых распространенных задач в области разработки и администрирования данных. С помощью Azure Data Studio можно подключиться к SQL Server в локальной среде и в облаке на платформе Windows, macOS и Linux. Azure Data Studio позволяет:

  - Редактировать и выполнять запросы в современной среде разработки с помощью невероятно быстрой функции Intellisense, фрагментов кода и путем интеграции управления исходным кодом.  
  - Быстро визуализировать данные с помощью встроенных диаграмм результирующих наборов.  
  - Создавать настраиваемые панели мониторинга для серверов и баз данных с помощью настраиваемых мини-приложений.  
  - Легко управлять более широкими средами с помощью встроенного терминала.  
  - Анализировать данные, работая с записной книжкой, интегрированной в Jupyter.  
  - Улучшить работу с пользовательскими расширениями и темами.  
  - Изучить ресурсы Azure с помощью встроенных подписки и обозревателя ресурсов.
  - Поддерживать сценарии, в которых используется кластер больших данных SQL Server.
...
- [**SQL Server Management Studio (SSMS) 18.0 (предварительная версия)**](../ssms/sql-server-management-studio-ssms.md)
  ...
  - Поддержка функции Always Encrypted с безопасными анклавами.
  - Более компактный файл для скачивания.
  - Теперь работает на основе изолированной оболочки Visual Studio 2017.
  - Полный список см. в разделе, посвященном [изменениям SSMS](../ssms/sql-server-management-studio-changelog-ssms.md).

## <a name="other-services"></a>Другие службы

В [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)] CTP 2.0 не предоставлены новые компоненты для следующих служб:
