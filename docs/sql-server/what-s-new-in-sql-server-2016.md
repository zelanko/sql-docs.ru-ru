---
title: "Что нового в SQL Server 2016 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "12/15/2016"
ms.prod: "sql-server-2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "server-general"
ms.tgt_pltfrm: ""
ms.topic: "article"
keywords: 
  - "новая версия sql server"
helpviewer_keywords: 
  - "новые возможности [SQL Server]"
  - "Новые возможности SQL Server"
  - "новые возможности SQL Server 2008"
  - "новые возможности [SQL Server]"
ms.assetid: 6a428023-e3cc-4626-a88a-4c13ccbd7db0
caps.latest.revision: 224
author: "craigg-msft"
ms.author: "craigg"
manager: "jhubbard"
caps.handback.revision: 224
---
# Что нового в SQL Server 2016
 [!INCLUDE[ssSQL15_md](../includes/sssql15-md.md)] — самый крупный рывок вперед за всю историю развития платформы управления данными Майкрософт. Обеспечьте оперативную аналитику транзакционных и аналитических данных с помощью масштабируемой платформы баз данных, которая обладает всем необходимым: от беспрецедентной производительности при обработке в памяти, инновационных средств безопасности и высокой доступности до расширенных функций аналитики, расширяющих возможности критически важных приложений.
 
 В этом разделе приводятся ссылки на подробные сведения о новых возможностях [!INCLUDE[ssCurrent_md](../includes/sscurrent-md.md)] на уровне компонентов.   
 
  ![SQL Server 2016](../sql-server/media/sql-server-2016.png) 
 
[Прочтите техническое описание выпусков SQL Server 2016](http://download.microsoft.com/download/8/A/2/8A2BC8C5-BBA0-4A9C-90BC-AC957D3454D9/SQL_Server_2016_Editions_datasheet.pdf)
 
    
 **Оцените SQL Server уже сегодня!**    
 - Получите **бесплатный** [**выпуск SQL Server 2016 Developer Edition!**](https://www.microsoft.com/en-us/cloud-platform/sql-server-editions-developers)
 
 
[![Скачайте со страницы центра оценки](../analysis-services/media/download.png)](https://www.microsoft.com/en-us/evalcenter/evaluate-sql-server-2016) Скачайте SQL Server 2016 со страницы **[центра оценки](https://www.microsoft.com/en-us/evalcenter/evaluate-sql-server-2016)** 
    
[Маленький значок виртуальной машины Azure![Azure Virtual Machine small](../analysis-services/media/azure-virtual-machine-small.png "Azure Virtual Machine small")](https://azure.microsoft.com/en-us/marketplace/partners/microsoft/sqlserver2016sp1standardwindowsserver2016/) Есть ли учетная запись Azure?  Тогда перейдите **[сюда](https://azure.microsoft.com/en-us/marketplace/partners/microsoft/sqlserver2016sp1standardwindowsserver2016/)**, чтобы запустить виртуальную машину с уже установленным SQL Server 2016 с пакетом обновления 1 (SP1).  

 [![Скачайте из центра оценки](../analysis-services/media/download.png)](https://msdn.microsoft.com/library/mt238290.aspx) [**Скачайте последнюю версию SQL Server Management Studio**](https://msdn.microsoft.com/library/mt238290.aspx).   

    
 ## <a name="whats-new-in-sql-server"></a>Новые возможности в SQL Server
Ссылки на отдельные разделы
 
 - [Установка SQL Server](../database-engine/install-windows/installation-for-sql-server-2016.md)   
  
 - [Подсистема служб Analysis Services (SSAS)](../analysis-services/what-s-new-in-analysis-services.md)  
  
 - [Ядро СУБД](../database-engine/configure-windows/what-s-new-in-sql-server-2016-database-engine.md)

 - [New SQL PowerShell module](https://blogs.technet.microsoft.com/dataplatforminsider/2016/06/30/sql-powershell-july-2016-update/) (Новый модуль SQL PowerShell)
 
 - [Службы интеграции](../integration-services/what-s-new-in-integration-services-in-sql-server-2016.md)    
  
 - [Службы Reporting Services](https://msdn.microsoft.com/library/ms170438.aspx)   
  
 - [Службы Master Data Services (MDS)](../master-data-services/what-s-new-in-master-data-services-mds.md)    
       
 - [Службы R SQL Server](../advanced-analytics/r-services/what-s-new-in-sql-server-r-services.md)    
      
  
    
## <a name="see-also"></a>См. также    
 + [![Заметки о выпуске](../analysis-services/instances/install-windows/media/ssrs-fyi-note.png)](../Topic/SQL%20Server%202016%20Release%20Notes.md) [Заметки о выпуске SQL Server 2016](../sql-server/sql-server-2016-release-notes.md). 
+ [Таблица SQL Server 2016](http://download.microsoft.com/download/C/5/3/C53C3AEF-653C-4598-8721-D522E8AC6A3A/SQL_Server_2016_Everything_Built-In_Datasheet_EN_US.pdf)
+ [Возможности, поддерживаемые различными выпусками SQL Server](https://msdn.microsoft.com/library/cc645993.aspx)
 + [Требования к оборудованию и программному обеспечению для установки SQL Server 2016](../sql-server/install/hardware-and-software-requirements-for-installing-sql-server-2016.md)
 + [Установка SQL Server 2016 с помощью мастера установки](../database-engine/install-windows/install-sql-server-2016-from-the-installation-wizard-setup.md)
 [Настройка и обслуживание установки](Setup%20and%20Servicing%20Installation.md)    