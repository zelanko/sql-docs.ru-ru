...
3. Объекты SQLCLR в проекте не поддерживаются версией DacFx для .NET Core. 
3. Задачи (сборка или публикация) не определяются пользователем
3. Публикация целевых объектов, определенных DacFx
3. Интеграция управления исходным кодом и создание нового проекта не генерируют GITIGNORE-файл автоматически 
3. Поддержка среды WSL ограничена 
