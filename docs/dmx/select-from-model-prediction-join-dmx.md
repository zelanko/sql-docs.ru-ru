...
```  
 ...
 Можно указать прогнозирующие функции, которые не возвращают табличное выражение в \< *списке Выбор выражений*> и \<> *выражение условия* .  
...
## <a name="example-3-predicting-associations"></a>Пример 3. Прогнозирование взаимосвязей  
 В следующем примере показано, как создать прогноз с помощью модели, построенной на основе алгоритма [!INCLUDE[msCoName](../includes/msconame-md.md)] взаимосвязей. Прогнозы на основе модели взаимосвязей можно использовать для рекомендации связанных продуктов. Например, следующий запрос возвращает три продукта, которые, скорее всего, будут приобретены вместе:  
