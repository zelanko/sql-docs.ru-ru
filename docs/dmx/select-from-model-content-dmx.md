---
title: SELECT FROM &lt;модели&gt;. СОДЕРЖИМОЕ (РАСШИРЕНИЯ ИНТЕЛЛЕКТУАЛЬНОГО АНАЛИЗА ДАННЫХ) | Документация Майкрософт
ms.date: 06/07/2018
ms.prod: sql
 ...
 **SELECT FROM**  _\<модели >_**. СОДЕРЖИМОЕ** инструкция возвращает содержимое, характерное для каждого алгоритма. Например, можно использовать описания всех правил модели правил взаимосвязей в пользовательском приложении. Можно использовать **SELECT FROM \<модели >. СОДЕРЖИМОГО** инструкция возвращает значения в столбце NODE_RULE модели.  
...
|TM_DecisionTree|0|  
 ...
 В следующем запросе используется **IsDescendant** функцию для возврата непосредственных потомков узла, который был возвращен в предыдущем запросе.  
  
> [!NOTE]  
>  Поскольку значение NODE_NAME является строкой, нельзя использовать инструкцию подзапроса выборки, возвращающего идентификатор NODE_ID в качестве аргумента **IsDescendant** функции.  
