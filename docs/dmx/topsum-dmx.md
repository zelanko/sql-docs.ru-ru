...
```  
 ...
 Первый аргумент **TopSum** функция — это имя столбца таблицы. В этом примере вложенной таблицы возвращается путем вызова функции Predict и использовать аргумент INCLUDE_STATISTICS.  
  
 Второй аргумент **TopSum** функция является столбцом вложенной таблицы, используемый для упорядочения результатов. В этом примере параметр INCLUDE_STATISTICS возвращает столбцы $SUPPORT, $PROBABILTY и $ADJUSTED PROBABILITY. Данный пример использует $PROBABILITY для возврата строк, сумма которых составляет 50% вероятности.  
  
 Третий аргумент **TopSum** функция указывает целевую сумму имеет тип double. Чтобы получить строки для наибольшего произведения, представляющего 50 процентов вероятности, необходимо ввести число 0,5.  
 ...
 **Примечание** этот пример приведен только для иллюстрации использования **TopSum**. В зависимости от размера набора данных выполнение данного запроса может занять значительное время.  
