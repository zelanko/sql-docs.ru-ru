...
![Снимок экрана, показывающий карту построителя отчетов.](../reporting-services/media/report-builder-map-final-map-only.png)
...
6.  В поле "Галерея карт" разверните узел **Штаты по округам** в узле **США** и выберите **Нью-Йорк**.  
    ...
    ![Снимок экрана, показывающий шаг "Выбор источника пространственных данных" мастера создания карт с Нью-Йорком, выделенным в разделе "Галерея карт".](../reporting-services/media/report-builder-map-ny-counties.png)
...
13. Из раскрывающегося списка **Поле данных** выберите **#COUNTYNAME**. На панели «Предварительный просмотр карты» в мастере отображаются следующие элементы:  
    ...
    ![Снимок экрана: шаг выбора цветовой темы и визуализации данных мастера создания карт.](../reporting-services/media/report-builder-map-choose-color-theme.png)
...
17. На вкладке **Главная** нажмите кнопку **Выполнить** для предварительного просмотра отчета.  
    ...
    ![Снимок экрана, показывающий предварительный просмотр созданной карты построителя отчетов.](../reporting-services/media/report-builder-map-first-preview.png)
...
2.  Дважды щелкните карту, чтобы отобразить панель **Слои карты** . На панели инструментов нажмите кнопку **Мастер создания слоя**![rs_IconMapLayerWizard](../reporting-services/media/rs-iconmaplayerwizard.gif "rs_IconMapLayerWizard"). 
    ...
    ![Снимок экрана, показывающий панель "Слои карт" с выделенным значком мастера создания слоя.](../reporting-services/media/report-builder-map-new-layer-wizard-icon.png) 
...
9. На панели инструментов конструктора запросов нажмите кнопку **Выполнить** ( **!** ).  
    ...
    *   **SpatialLocation** : местоположение, представленное в виде широты и долготы. 
    ...
    ![Снимок экрана: шаг "Проектирование запроса" мастера создания слоя карт.](../reporting-services/media/report-builder-map-design-query.png) 
...
21. В области **Слои карты** щелкните стрелку рядом со слоем PointLayer1 и выберите пункт **Свойства точки**.  
    ...
    ![Снимок экрана: панель "Слои карты" с параметром "Свойства точки".](../reporting-services/media/report-builder-map-point-properties.png)
  
22. На вкладке **Шрифт** выберите стиль **Полужирный** и размер **10 пт**.

    ![Снимок экрана, показывающий раздел параметров изменения шрифта, размера и цвета диалогового окна "Свойства точек карты".](../reporting-services/media/report-builder-map-point-properties-font.png)
...
24. Нажмите кнопку **Выполнить** для предварительного просмотра отчета.  
    ...
    ![Снимок экрана с предварительным просмотром карты построителя отчетов, где названия городов, отображаются полужирным шрифтом размера 10.](../reporting-services/media/report-builder-map-city-names.png)
...
14. Нажмите кнопку **Готово**.  
    ...
    ![Снимок экрана, показывающий карту построителя отчетов со слоем линий.](../reporting-services/media/report-builder-map-line.png)
...
15. Разверните раздел **Вид** , а затем узел **Вид** , чтобы получить доступ к свойству **Масштаб** . Присвойте ему значение **125**. 
    ...
    ![Снимок экрана, показывающий, как корректировать увеличение карты построителя отчетов.](../reporting-services/media/report-builder-map-zoom.png)
...
5.  На вкладке **Общие** в поле **Тип** выберите значение **Вид с воздуха**. Воздушное представление не содержит текста.  
    ...
    ![Снимок экрана, показывающий мозаичный слой вида с воздуха Bing Maps, добавленный в карту построителя отчетов.](../reporting-services/media/report-builder-map-bing-aerial.png)
...
6.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
    ...
    ![Снимок экрана, показывающий, как задать прозрачность слоя для карты построителя отчетов.](../reporting-services/media/report-builder-map-transparency.png)
...
8.  В списке **Из набора аналитических данных** выберите [County].  
    ...
    ![Снимок экрана, на котором показана вкладка "Аналитические данные" в диалоговом окне "Свойства слоя многоугольников".](../reporting-services/media/report-builder-map-county-colors.png)
  
9. [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
10. Просмотрите отчет.  

    ![Снимок экрана, показывающий предварительный просмотр карты построителя отчетов, где выделены конкретные округа.](../reporting-services/media/report-builder-map-county-highlight.png)
...
7.  Измените **Конечный цвет** на **Зеленый**.  
    ...
    **Красный** представляет значения низкого уровня продаж, **Желтый** — средние, а **Зеленый** — значения высокого уровня продаж. Обработчик отчета вычисляет цветовой диапазон исходя из этих значений и параметров, выбранных на странице **Распределение** .  
    ...
    ![Снимок экрана, показывающий раздел изменения правил цвета многоугольников диалогового окна "Свойства цветовых правил карты".](../reporting-services/media/report-builder-map-county-color-rules.png)
...
15. Просмотрите отчет.
    ...
    ![Снимок экрана, показывающий предварительный просмотр карты построителя отчетов, где применены конкретные цветовые правила.](../reporting-services/media/report-builder-map-county-color-rule-preview.png)
...
3. Измените значение свойства TextColor на **Белый**.  
    ...
    ![Снимок экрана, демонстрирующий настройку заголовка легенды.](../reporting-services/media/report-builder-map-color-scale-title.png)
...
3.  Щелкните на PolygonLayer1 стрелку вниз и выберите **Свойства многоугольника**. 
     ...
     ![Снимок экрана, показывающий, как выбрать параметр свойств многоугольника.](../reporting-services/media/report-builder-map-polygon-layer-properties.png)
...
5.  Щелкните место на карте, где нужно добавить точку. В этом учебнике необходимо щелкнуть расположение в округе Онайда. Точка, помеченная окружностью, добавляется на слой в месте щелчка. По умолчанию точка будет выбрана.  
    ...
    ![Снимок экрана, показывающий, как добавлять пользовательские точки к карте построителя отчетов.](../reporting-services/media/report-builder-map-custom-point.png)
...
7.  Установите флажок **Переопределять параметры точек для этого слоя**. В диалоговом окне появляются дополнительные страницы. Значения, заданные на этих страницах, имеют более высокий приоритет, чем параметры отображения, заданные для слоя или цветовых правил.  
    ...
    ![Снимок экрана, показывающий раздел параметров изменения параметров внедренной точки диалогового окна "Свойства внедренных точек карты".](../reporting-services/media/report-builder-map-custom-point-general.png)
...
13. На вкладке **Шрифт** измените размер шрифта на **10 пт** , а тип на **Полужирный**.
    ...
    ![Снимок экрана, показывающий раздел параметров изменения шрифта, размера и цвета диалогового окна "Свойства внедренных точек карты".](../reporting-services/media/report-builder-map-custom-point-font.png)
...
![Снимок экрана, показывающий предварительный просмотр карты построителя отчетов с выделенной пользовательской точкой для нового магазина.](../reporting-services/media/report-builder-map-custom-point-new-store.png)
...
![Снимок экрана, показывающий готовую карту построителя отчетов, отображаемую на портале.](../reporting-services/media/report-builder-map-in-portal.png) 
