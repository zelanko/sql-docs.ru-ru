...
| Не удалось выбрать рабочие дни при создании общего Еженедельное расписание | &nbsp; |
| Отчет не содержит символы возврата каретки должным образом в формате Word | &nbsp; |
| Обновление не больше работает с последней SSRS 2017 System Center Operations Manager(SCOM) 2019 г. | &nbsp; |
| Произошла ошибка при вызове модуля авторизации для общего набора данных | &nbsp; |
| Изменено на хранимую процедуру GetAllProperties в SSRS 2017 и PBIRS, который вызывает конечную веб-службы не удалось получить данные для связанного отчета метод ReportingService2010.GetProperties логики | &nbsp; |
| Простой заголовок строки сетки в мобильный отчет не отображается при щелчке элемента в сетке | &nbsp; |
| Не удается использовать поле даты в параметре управляемой данными подписки | &nbsp; |
| Вложенного табликса отображается маленький шрифт или частичного шрифта в SSRS 2016 и более поздних версий | &nbsp; |
| Подписки с помощью параметра DateTime ошибкой после, как пользователь с улучшенным другой языковой стандарт подписки | &nbsp; |
| Создание управляемой данными подписки с Null модуля доставки произошел сбой с «произошла ошибка доставки» | &nbsp; |
| Кодировка URL-адрес неверен, при задании значения в формате Excel\Word | &nbsp; |
