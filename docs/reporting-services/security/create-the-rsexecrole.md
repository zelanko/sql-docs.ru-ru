...
ms.locfileid: "67140474"
---
# <a name="create-the-rsexecrole"></a>Создание роли RSExecRole

  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] используют стандартную роль базы данных с именем **RSExecRole** , чтобы предоставить серверу отчетов разрешения в базе данных сервера отчетов. Роль **RSExecRole** создается автоматически с базой данных сервера отчетов. Как правило, никогда не следует изменять ее или назначать другим пользователям. При этом при перемещении базы данных сервера отчетов на новый или иной компонент [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] вам нужно создать повторно эту роль в системных базах данных master и msdb.  
...
7.  Щелкните правой кнопкой мыши **Роли базы данных**и выберите пункт **Создать роль базы данных**. **Роли базы данных - новый** появится страница.  
  
8.  В поле **Имя роли**введите **RSExecRole**.  
  
9. В поле **Владелец** введите **dbo**.  
  
10. Страница выбора **защищаемые объекты**.  
...
19. В **Execute** в строке **Grant** столбца, установите флажок.  
...
8.  Выберите **защищаемые объекты** страницы.  
...
20. На **защищаемые объекты** щелкните **поиска** еще раз. Отображается диалоговое окно **Добавление объектов** . Параметр **Выбрать объекты** выбран по умолчанию.  
...
6.  Нажмите кнопку **Проверить соединение**. Вы увидите диалоговое окно с сообщением «Тестирования подключения Succeeded».
  
7.  Выберите **ОК** чтобы закрыть диалоговое окно, а затем выберите **Далее**.  
...
15. Щелкните ссылку, чтобы открыть веб-портала. Будут отображены элементы сервера отчетов из базы данных сервера отчетов.  
