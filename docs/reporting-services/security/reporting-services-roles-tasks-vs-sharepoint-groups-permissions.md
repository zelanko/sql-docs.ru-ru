---
title: "Сравнение ролей и задач служб Reporting Services с группами и разрешениями SharePoint | Microsoft Docs"
ms.custom: ""
ms.date: "03/07/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "разрешения [службы Reporting Services], режим интеграции с SharePoint"
  - "безопасность [службы Reporting Services], задачи"
  - "роли [службы Reporting Services], предопределенные"
  - "интеграция с SharePoint [службы Reporting Services], разрешения"
  - "разрешения [службы Reporting Services], собственный режим"
  - "безопасность [службы Reporting Services], предопределенные роли"
...
|Управление заданиями|System|Отсутствует (не поддерживается).|  
|Управление свойствами сервера отчетов|System|Отсутствует (неприменимо). Сервер отчетов не проверяет, имеет ли пользователь разрешение на просмотр параметров интеграции в центре администрирования.|  
|Управление ролями|Система|Управление разрешениями.|  
|Управление общими расписаниями|System|Управление веб-сайтом, открытие.|  
|Управление безопасностью сервера отчетов|System|Отсутствует (неприменимо). Сервер отчетов не использует назначения ролей на системном уровне на сервере, работающем в режиме интеграции с SharePoint.|  
|Просмотр свойств сервера отчетов|System|Отсутствует (неприменимо). Сервер отчетов не проверяет, имеет ли пользователь разрешение на просмотр параметров интеграции в центре администрирования.|  
|Просмотр общих расписаний|System|Открытие элементов.|  
