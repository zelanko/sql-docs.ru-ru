---
title: Синтаксис пути к элементу для XML-данных отчета | Документация Майкрософт
description: Сведения о синтаксисе пути к элементу и соглашениях, используемых для определения пути к данным XML-отчета в конструктор отчетов.
ms.date: 03/01/2017
...
|**ElementNode**|XML-узел в XML-документе. Узлы обозначаются тегами и находятся в иерархической связи с другими узлами. Например, \<Customers> является узлом корневого элемента. \<Customer> является вложенным элементом \<Customers>.|  
...
|**Attribute**|Пара "имя-значение" в **ElementNode**. Например, в узле элемента \<Customer ID="1"> **ID** является атрибутом, а **\@ID(Integer)** возвращает значение 1 как целочисленный тип данных в соответствующем поле данных **ID**.|  
|**Значение**|Значение элемента. **Value** может использоваться для последнего **ElementNode** в пути к элементу. Например, \<Return> является листовым узлом. Если включить его в конце пути к элементу, значением **Return {@}** будет **Chair**.|  
