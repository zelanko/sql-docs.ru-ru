...
description: Узнайте, как управлять общими наборами данных в Reporting Services, чтобы использовать запрос совместно и предоставить согласованный набор данных для нескольких отчетов.
ms.date: 03/01/2017
...
## <a name="viewing-or-changing-the-shared-dataset-definition"></a>Просмотр или изменение определения общего набора данных  
 ...
 Еще один способ просмотра определения общего набора данных в виде XML-кода — использовать синтаксис доступа через URL-адрес на веб-портале. Например, чтобы просмотреть значения по умолчанию для каждого параметра набора данных, можно применить следующую команду доступа через URL-адрес для отображения определения общего набора данных с именем DataSet1 с сервера отчетов:  
...
|Проверка значений по умолчанию для параметров набора данных.|Открытие в построителе отчетов или использование синтаксиса доступа через URL-адрес.|Пример:<br /><br /> `https://localhost/reportserver/?/Datasets/Dataset1&rs:command=GetShareddatasetDefinition`
|Включение кэширования|веб-портал|[Общие наборы данных в кэше (службы SSRS)](../../reporting-services/report-server/cache-shared-datasets-ssrs.md)|  
|Создание или изменение плана обновления кэша|веб-портал|[Кэширование общего набора данных](../../reporting-services/report-server/cache-a-shared-dataset.md)|  
