---
title: "Синтаксис запроса XML для XML-данных отчета (SSRS) | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "пространства имен [службы Reporting Services]"
  - "модули обработки данных [службы Reporting Services], источники данных"
  - "xmldp [службы Reporting Services]"
...
> [!NOTE]  
>  Первый пример веб-службы перечисляет содержимое сервера отчетов, применяющего метод <xref:ReportService2006.ReportingService2006.ListChildren%2A>. Для выполнения этого запроса необходимо создать новый источник данных и задать строку соединения: http://localhost/reportserver/reportservice2006.asmx. Метод <xref:ReportService2006.ReportingService2006.ListChildren%2A> имеет два параметра: **Item** и **Recursive**. Для **Item** установите значение по умолчанию **/**, а для параметра **Recursive** — значение **1**.  
