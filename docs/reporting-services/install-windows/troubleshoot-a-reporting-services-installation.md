...
3.  Перезапустите службу [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] .  
 ...
 Чтобы убедиться, что действия были выполнены успешно, откройте веб-браузер и перейдите на веб-портале, URL-адрес или URL-адрес сервера отчетов. После этого откройте системный монитор, чтобы проверить, работают ли счетчики.  
...
### <a name="WindowsAuthBreaksAfterUpgrade"></a>Ошибка 401 — Нет доступа для проверки подлинности Windows после обновления SQL Server 2005 до SQL Server 2016

 Если вы обновляете [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] для [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]и с встроенной учетной записи для учетной записи службы сервера отчетов используется проверка подлинности NTLM, при доступе к серверу отчетов или веб-портале после обновления могут возникнуть ошибку 401 — Нет.  
...
### <a name="Uninstall32BitBreaks64Bit"></a>Удаление 32-разрядного экземпляра служб SQL Server 2016 Reporting Services в развертывании side-by-side с 64-разрядного экземпляра приводит к нарушению работы 64-разрядного экземпляра
