...
description: Узнайте, как использовать веб-портал для выбора отчета, который будет доставлен получателям, и как изменить свойства источника данных отчета.
ms.date: 05/23/2016
 ...
 **Примечание.** URL-адресом веб-*портала* является Reports, а не URL-адрес *сервера* отчетов (Reportserver).  
