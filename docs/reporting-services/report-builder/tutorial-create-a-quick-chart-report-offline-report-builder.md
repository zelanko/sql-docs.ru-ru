---
title: "Учебник: Создание стандартного отчета с диаграммой сети (построитель отчетов) | Документы Microsoft"
ms.custom: 
ms.date: 05/30/2017
...
### <a name="using-xml-data-for-this-tutorial"></a>Использование XML-данных при работе с этим учебником  
 Можно использовать XML-данные, скопированные из этого раздела и вставленные в мастер. Не нужно подключаться к [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] режим интеграции сервера отчетов в собственном режиме или в SharePoint и не нужен доступ к экземпляру SQL Server.  
...
### <a name="using-a-includetsqlincludestsql-mdmd-query-that-contains-data-for-this-tutorial"></a>Использование запроса [!INCLUDE[tsql](../../includes/tsql-md.md)] , содержащего данные, при работе с этим учебником  
 Можно скопировать запрос с данными, которые содержатся в нем, из этого раздела и вставить в мастер. Необходимо имя экземпляра SQL Server и учетные данные, достаточные для доступа только для чтения к любой базе данных. Запрос набора данных в учебнике используются данные литералов, но запрос должен обрабатываться экземпляром служб SQL Server для возвращения метаданных, которая необходима для набора данных отчета.  
...
8.  Нажмите кнопку **учетные данные** выберите **использовать текущего пользователя Windows. Может потребоваться делегирование Kerberos**, а затем нажмите кнопку **ОК**.  
...
11. (Необязательно) Нажмите кнопку **запуска** кнопки (**!**) для просмотра данных, будет построена диаграмма.  
  
     ![Конструктора запросов построителя отчетов](../../reporting-services/report-builder/media/rb-designquery.png "конструктора запросов построителя отчетов")  
  
12. Нажмите кнопку **Далее**.  
  
13. На странице **Выбор типа диаграммы** выберите **Круговая диаграмма**и нажмите кнопку **Далее**.  
  
14. В **размещение полей диаграммы** дважды щелкните **продажи** в **доступные поля** поле.  
  
     Обратите внимание на то, что оно автоматически перемещается в поле **Значения** , так как это числовое значение.  
  
     ![Мастер создания размещение полей отчета](../../reporting-services/report-builder/media/rb-wizarrangefields.png "Мастер построителя отчетов размещение полей")  
  
15. Перетащите **FullName** из **доступные поля** поле для **категории** поле (или дважды щелкните его; оно переместится в **категории** поле) и нажмите кнопку **Далее**.  
     ...
     ![Построитель новые диаграммы предварительного просмотра отчета](../../reporting-services/report-builder/media/rb-newchartpreview.png "новой диаграммы области предварительного просмотра построителя отчетов")  
  
16. Нажмите кнопку **Готово**.  
  
     Вы увидите новый отчет с круговой диаграммой в конструкторе, все еще с репрезентативными данными.  
  
     ![Построитель новый круговой в режиме конструктора отчетов](../../reporting-services/report-builder/media/rb-newpiedesign.png "круговой новый построитель в режиме конструктора отчетов")  
  
17. Чтобы просмотреть настоящую круговую диаграмму, нажмите кнопку **Выполнить** на вкладке **Главная** ленты.  
  
     ![Отчет построителя новые диаграммы выполнения](../../reporting-services/report-builder/media/rb-newchartrun.png "новый диаграммы запуск построителя отчетов")  
...
11. В **размещение полей диаграммы** дважды щелкните **продажи** в **доступные поля** поле.  
  
     Обратите внимание на то, что оно автоматически перемещается в поле **Значение** , так как это числовое значение.  
  
12. Перетащите **FullName** из **доступные поля** поле для **категории** поле (или дважды щелкните его; оно переместится в **категории** поле) и нажмите кнопку **Далее**.  
...
2. С заголовком, выбранного в панели «Свойства» измените **цвет** для **черный** и **FontSize** для **12 пт**.
  
## <a name="add-percentages"></a>Добавление процентных показателей  
 
1.  Щелкните правой кнопкой мыши круговую диаграмму и выберите **отобразить метки данных**. Метки данных должны появиться в каждом срезе круговой диаграммы.  
  
2.  Щелкните правой кнопкой мыши метки и выберите **свойства метки ряда**. Появится диалоговое окно **Свойства метки ряда** .  
  
3.  В **метки данных** введите **#PERCENT {P0}**.  
  
     Значение **{P0}** дает процент без десятичных разрядов. Если просто ввести **#PERCENT**, числа будут иметь два десятичных разряда. **#PERCENT** является ключевым словом, выполняются вычисления; имеются и многие другие.  
...
##  <a name="WhatsNext"></a> Дальнейшая работа  
 ...
 Для работы с другими учебниками, требуется указать имя экземпляра служб SQL Server и учетные данные, достаточные для доступа только для чтения к любой базе данных. Системный администратор также может предоставить такой доступ.  
