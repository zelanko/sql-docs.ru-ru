---
title: "Настройки сведений об устройстве HTML | Microsoft Docs"
ms.date: "03/16/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "HTML [службы Reporting Services], подготовка к просмотру"
  - "настройки сведений об устройстве [службы Reporting Services], подготовка к просмотру в формате HTML"
...
|**StreamRoot (\*)**|Путь, используемый в качестве префикса значения атрибута **src** в элементе IMG в HTML-отчете, возвращаемом сервером отчетов. По умолчанию сервер отчетов предоставляет путь. В этом параметре можно указать корневой путь для изображений в отчете (например, **http://\<servername>/resources/companyimages**).|  
