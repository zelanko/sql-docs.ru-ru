...
description: Узнайте о различных настройках сведений об устройстве, доступных для подготовки отчетов к просмотру в формате HTML.
ms.date: 03/16/2017
...
|**StreamRoot (\*)**|Путь, используемый в качестве префикса значения атрибута **src** в элементе IMG в HTML-отчете, возвращаемом сервером отчетов. По умолчанию сервер отчетов предоставляет путь. В этом параметре можно указать корневой путь для изображений в отчете (например, **https://\<servername>/resources/companyimages**).|  
