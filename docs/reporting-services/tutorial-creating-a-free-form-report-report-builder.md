---
title: "Учебник. Создание отчета в свободной форме (построитель отчетов) | Microsoft Docs"
ms.custom: ""
ms.date: "09/02/2016"
...
---
# Учебник. Создание отчета в свободной форме (построитель отчетов)
...
4.  Убедитесь в том, что выбран тип соединения Microsoft SQL Server, а затем в поле **Строка подключения** введите **Data Source = \<имя_сервера>**.  
    ...
    Значение **\<имя_сервера>**, например Report001, указывает компьютер, на котором установлен экземпляр компонента SQL Server Database Engine. Так как данные для этого отчета не извлекаются из базы данных SQL Server, не нужно указывать имя базы данных. База данных по умолчанию на указанном сервере используется только для синтаксического анализа запроса.  
...
2.  Выделив обе диаграммы, щелкните правой кнопкой мыши **Макет** > **Одинаковая ширина**.  
