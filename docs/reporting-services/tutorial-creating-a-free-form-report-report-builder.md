---
title: "Учебник: Создание отчета в свободной форме (построитель отчетов) | Документы Microsoft"
ms.custom: 
ms.date: 09/02/2016
...
4.  Убедитесь, что тип соединения Microsoft SQL Server, а затем в **строка подключения** введите: **источник данных = \<servername >**  
  
    **\<имя_сервера >**для примера Report001, указывает компьютер, на котором установлен экземпляр SQL Server Database Engine. Так как данные для этого отчета не извлекаются из базы данных SQL Server, не нужно указывать имя базы данных. База данных по умолчанию на указанном сервере используется только для синтаксического анализа запроса.  
...
1.  На **вставить** вкладка > **областей данных** > **списка**. 
...
6.  Щелкните правой кнопкой мыши в области группы строк **сведения** группы > **удалить группу**.  
...
2. Щелкните в левом верхнем углу списка и перетащите указатель, чтобы создать прямоугольник высотой 7 дюймов и шириной 3,5 дюйма. Чтобы задать точный размер, в области **Свойства** в разделе **Положение**введите значения свойств **Ширина** и **Высота**.
  
2.  Щелкните правой кнопкой мыши прямоугольник > **Свойства прямоугольника**.  
...
2.  На **вставить** вкладка > **текстовое поле**. Щелкните в левом верхнем углу списка внутри ранее добавленного прямоугольника и перетащите указатель, чтобы создать текстовое поле шириной приблизительно 3,45 дюйма и высотой приблизительно 5 дюймов.  
...
6.  На **Главная** вкладка > **шрифта**выберите: 
...
12. На **Главная** вкладка > **шрифта**выберите: 
...
17.  На **Главная** вкладка > **шрифта**выберите: 
...
22. Выберите только что `[Sales]` поле, щелкните правой кнопкой мыши поле > **выражение**.  
...
29. С `[Sum(Sales)]` по-прежнему выбран на **Главная** вкладка > **номер** группы > **валюты**.  
...
1.  На **вставить** вкладка > **областей данных** области > **таблицы** > **мастера таблиц**.  
...
12. Выберите поле [SalesDate]. На **Главная** вкладка > **номер**, изменить **по умолчанию** для **даты**.

13. Выберите поля [Sum(Sales)]. На **Главная** вкладка > **номер**, изменить **по умолчанию** для **валюты**.
...
## <a name="Line"></a>7. Добавление строки в отдельные области отчета (необязательно)  
...
2.  На **вставить** вкладка > **элементы отчета** > **строки.**  
  
3.  Проведите линию ниже текстового поля, добавленного на занятии 4.  
  
4.  Щелкните строку и на **Главная** вкладка > **границы**выберите:
     * Для параметра**Ширина** выберите значение **3 пт** .
     * Для параметра**Цвет** выберите значение **Томатный**.  
  
## <a name="Visualization"></a>8. Добавление представлений сводных данных (необязательно)  
...
2.  На **вставить** вкладка > **элементы отчета** >  **прямоугольник**. Путем перетаскивания создайте в поле со списком справа от таблицы прямоугольник шириной приблизительно 2,25 дюйма и высотой приблизительно 7,9 дюйма.  
...
1.  На **вставить** вкладка > **визуализации данных** > **диаграммы** > **мастер диаграмм**.  
...
12. На **Главная** вкладка > **шрифта**, создать поле заголовка:
    * **Шрифт** **Segoe UI Semibold**.
    * **Размер** **12 пт**.
    * **Цвет** **Черный**.  

13. Щелкните правой кнопкой мыши условные обозначения > **свойства условных обозначений**.
...
12. Выберите оси диаграммы и на *Главная** вкладку > **номер** > **валюты**.
...
2.  Выделив обе диаграммы, щелкните правой кнопкой мыши > **макета** > **установить ту же ширину**.  
...
Дополнительные сведения о конструкторах запросов см. в разделе [конструкторы запросов &#40; Построитель отчетов &#41; ](http://msdn.microsoft.com/library/553f0d4e-8b1d-4148-9321-8b41a1e8e1b9) и [пользовательский интерфейс конструктора запросов на основе текста &#40; Построитель отчетов &#41; ](../reporting-services/report-data/text-based-query-designer-user-interface-report-builder.md).  
