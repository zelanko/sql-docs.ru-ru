---
title: "Включение и отключение «песочницы» для языка определения Отчетов | Документы Microsoft"
ms.custom: 
ms.date: 03/20/2017
...
|**AllowNew**|Логический атрибут **Разрешить** , управляющий новые экземпляры типа, разрешено ли создаваться в выражениях языка определения Отчетов или в язык определения Отчетов  **\<класса >** элемента.<br /><br /> Примечание. Если режим **RDLSandboxing** включен, то новые массивы не могут создаваться в выражениях языка определения отчетов, независимо от значения **AllowNew**.|  
