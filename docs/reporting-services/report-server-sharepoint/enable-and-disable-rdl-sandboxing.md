---
title: "Включение и отключение «песочницы» для языка определения Отчетов для служб Reporting Services в режиме интеграции с SharePoint | Документы Microsoft"
ms.custom: 
ms.date: 09/25/2017
...
Функция «песочницы» для RDL (язык определения отчетов) позволяет обнаруживать и ограничивать использование определенных типов ресурсов отдельными пользователями в среде нескольких клиентов, использующих один веб-фермы серверов отчетов. Примером этого является сценарий услуг хост-сервера, в котором может поддерживаться единственная веб-ферма серверов отчетов, применяемая несколькими пользователями, а также, возможно, другими компаниями. Администратор сервера отчетов может включить эту функцию для выполнения следующих задач.  
...
-   Пользовательский код в  **\<кода >** элемент определения отчета.  
...
|**AllowNew**|Логический атрибут **Разрешить** , управляющий новые экземпляры типа, разрешено ли создаваться в выражениях языка определения Отчетов или в язык определения Отчетов  **\<класса >** элемента.<br /><br /> Когда **RDLSandboxing** включен, новые массивы не могут создаваться в выражениях языка определения Отчетов, независимо от значения **AllowNew**.|  
...
|**Члены**|Список типов, включенных в  **\<типы >** , представляет список имен членов, которые не допускаются в выражениях языка определения Отчетов.|  
|**Запретить**|Имя члена, запрещенного в выражении языка определения отчетов. Это свойство учитывает регистр символов.<br /><br /> Когда **Deny** указан для элемента, все члены с таким именем для всех типов не допускаются.|  
  
## <a name="working-with-expressions-when-rdl-sandboxing-is-enabled"></a>Работа с выражениями, если включен параметр «песочницы» для языка определения Отчетов
...
-   Добавить  **\<Deny >** элемент для имени элемента.  
...
-   добавить этот новый класс к списку разрешений.  
 ...
 Чтобы добавить ключевые слова, определяющие тип [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] .NET Framework, добавьте к списку разрешений соответствующий тип CLR. Например, чтобы использовать [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] ключевое слово .NET Framework **целое**, добавьте следующий фрагмент XML для  **\<RDLSandboxing >** элемента:  
...
#### <a name="maintaining-the-deny-list-of-members"></a>Обслуживание \<Deny > список членов
...
### <a name="working-with-operators-and-new"></a>Работа с операторами и оператором new
 ...
 По умолчанию все операторы языка [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] .NET Framework, кроме **New**, всегда разрешены. **New** оператор управляется **AllowNew** атрибут  **\<разрешить >** элемента. Другие операторы языка, такие как оператор метода доступа коллекции по умолчанию, оператор **!** и макросы приведения [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] .NET Framework, такие как **CInt**, всегда разрешены.  
