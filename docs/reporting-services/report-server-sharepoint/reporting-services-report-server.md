---
title: "Сервером отчетов служб Reporting Services | Документы Microsoft"
ms.custom: 
ms.date: 09/25/2017
...
# <a name="reporting-services-report-server"></a>Сервером отчетов служб Reporting Services

[!INCLUDE[ssrs-appliesto](../../includes/ssrs-appliesto.md)] [!INCLUDE[ssrs-appliesto-2016-and-later](../../includes/ssrs-appliesto-2016-and-later.md)] [!INCLUDE[ssrs-appliesto-sharepoint-2013-2016i](../../includes/ssrs-appliesto-sharepoint-2013-2016.md)] [!INCLUDE[ssrs-appliesto-pbirsi](../../includes/ssrs-appliesto-pbirs.md)]

[!INCLUDE [ssrs-previous-versions](../../includes/ssrs-previous-versions.md)]

Дополнительные сведения о центре часть [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] установки служб Reporting Services. Он состоит из подсистемы обработки, а также расширений функциональности.

> [!NOTE]
> Интеграция служб Reporting Services с SharePoint больше не доступны после SQL Server 2016.

Сервер отчетов служб Reporting Services работают в одном из двух режимов развертывания; Основном режиме или режиме интеграции с SharePoint. В разделе [Сравнение функций режима интеграции с SharePoint и собственного режима](#bkmk_featuresupport) можно найти сравнительную таблицу функций.  
  
 **Установка:** сведения об установке служб Reporting Services см. в разделе [Установка служб Reporting Services](../install-windows/install-reporting-services.md).

## <a name="overview-of-report-server-modes"></a>Общие сведения о режимах сервера отчетов

 Модули обработки (ЦП) — ядро сервера отчетов. Они служат для поддержания целостности системы отчетности и не могут быть изменены или расширены. Модули также представляют собой обработчики, но они выполняют весьма специализированные функции. Службы Reporting Services включают один или несколько стандартных модулей для каждого типа поддерживаемых модулей. К серверу отчетов можно добавить пользовательские модули. Это позволяет расширить возможности сервера отчетов для поддержки компонентов, которые не поддерживаются в стандартной поставке; в качестве примеров специализированных функциональных возможностей можно указать поддержку для технологий однократной регистрации входа, вывод отчета в прикладных форматах, которые не обрабатываются стандартными модулями подготовки отчетов, а также доставку отчета на принтер или в приложение.  
...
-   **Собственный режим**: том числе собственный режим с веб-частями SharePoint, в котором выполняется сервер отчетов, как сервер приложений, обеспечивающий все функции обработки и управления исключительно через компоненты служб Reporting Services. Настройка сервера отчетов в собственном режиме с помощью диспетчера конфигурации служб Reporting Services и SQL Server Management Studio.  
...
|**Отчеты RDL**|Да|Да<br /><br /> . Отчеты RDL можно запускать на службы Reporting Services серверов отчетов в собственном режиме или в режиме интеграции с SharePoint.|  
|**Отчеты RDLX**|Нет|Да<br /><br /> Power View. Отчеты RDLX может выполняться в службы Reporting Services серверов отчетов в режиме интеграции с SharePoint.|  
...
## <a name="native-mode"></a>Собственный режим
 ...
 В примере ниже показан трехуровневая архитектура развертывания в Reporting Services собственном режиме. На схеме показана база данных сервера отчетов и источники данных в уровне данных, компоненты сервера отчетов в среднем уровне и клиентские приложения и встроенные или пользовательские средства в уровне представления. В диаграмме показан поток запросов и данных в серверных компонентах, а также компоненты, отправляющие и получающие содержимое из хранилища данных.  
...
-   Сервер приложений SharePoint с общей службы Reporting Services установлены и по крайней мере одно приложение службы Reporting Services.  
  
 На следующем рисунке показан среде SharePoint режим служб Reporting Services:  
...
||Description|  
|-|-----------------|  
|**(1)**|Веб-серверы или клиентские веб-интерфейсы (WFE). Надстройка служб Reporting Services необходимо установить на каждом веб-сервере, из которого вы хотите использовать возможностями веб-приложений, таких как просмотр отчетов или страницы управления служб Reporting Services для задач, таких как управление источниками данных и подписок.|  
|**(2)**|Надстройка устанавливает URL-адреса и конечные точки SOAP для клиентов для связи с серверами приложений через прокси-сервер службы Reporting Services.|  
|**(3)**|Серверы приложений с общей службы Reporting Services. Масштабное развертывание служб обработки отчетов управляется как часть фермы SharePoint и путем добавления дополнительных серверов приложений службы Reporting Services.|  
|**(4)**|Можно создать более одного приложения службы Reporting Services с разными конфигурациями, включая разрешения, электронной почты, прокси-сервера и подписок.|  
|**(5)**|Отчеты, источники данных и другие элементы хранятся в базах данных содержимого SharePoint.|  
|**(6)**|Службы Reporting Services создают три базы данных для сервера отчетов, временную и для функции предупреждения об изменении данных. Параметры конфигурации, которые применяются ко всем приложениям служб SSRS, хранятся в файле конфигурации **RSReportserver.config** .|  
...
|Установите службы Reporting Services в режиме интеграции с SharePoint.|[Установка Reporting Services в режиме SharePoint для SharePoint 2010](http://msdn.microsoft.com/47efa72e-1735-4387-8485-f8994fb08c8c)|  
