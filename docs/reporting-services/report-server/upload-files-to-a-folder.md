...
description: Узнайте, что происходит при отправке файлов различных типов из файловой системы и их сохранении в качестве управляемых элементов в базе данных сервера отчетов в службах Reporting Services.
ms.date: 06/17/2019
...
-   Передача любого другого файла добавляет его в базу данных сервера отчетов как один двоичный объект. Эти файлы публикуются на сервере отчетов как ресурсы. Ресурсами могут быть файлы любого типа. Если расширение файла совпадает с известным типом MIME, для идентификации типа ресурса используется значок этого типа MIME. В противном случае ресурс показывает универсальный значок файла.  
 ...
 Максимальный размер файла для передаваемых элементов составляет 2 ГБ, и его можно изменить с помощью свойства MaxFileSizeMb в [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)].  
  
 Визуально файлы, передаваемые в базу данных сервера отчетов, представляются в иерархии папок с помощью следующих значков:  
  
  ![Значки загружаемых файлов сервера отчетов](../../reporting-services/report-server/media/upload-files-to-a-folder/report-server-uploadable-file-icons.png)
