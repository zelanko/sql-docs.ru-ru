...
|Файл Web.config для диспетчера отчетов|Включает только те параметры, которые необходимы для ASP.NET, если это применимо для версии служб SSRS.|\<каталог_установки>\Reporting Services\ReportManager|  
