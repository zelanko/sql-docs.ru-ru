...
##  <a name="summary-of-configuration-files-native-mode"></a><a name="bkmk_config_file_Summary_native_mode"></a> Сводка по файлам конфигурации (собственный режим)  
...
|Файл Web.config для веб-службы сервера отчетов|Включает только те параметры, которые необходимы для ASP.NET.|\<Installation directory> \Reporting Services \ReportServer|  
|Файл Web.config для диспетчера отчетов|Содержит только те параметры, которые необходимы для ASP.NET, если это применимо для используемой версии SSRS.|\<Installation directory> \Reporting Services \ReportManager|  
...
##  <a name="summary-of-configuration-files-sharepoint-mode"></a><a name="bkmk_config_file_Summary_sharepoint_mode"></a> Сводка файлов конфигурации (режим SharePoint)  
