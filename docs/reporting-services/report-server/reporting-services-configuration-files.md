...
|Файл Web.config для диспетчера отчетов|Включает только те параметры, которые необходимы для ASP.NET.|\<Каталог установки > \Reporting Services \ReportManager|  
