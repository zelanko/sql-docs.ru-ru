...
description: Настройте построитель отчетов — средство, устанавливаемое вместе с сервером отчетов SQL Server Reporting Services. Оно может работать либо в собственном режиме, либо в режиме интеграции с SharePoint.
...
На клиентском компьютере с SSRS 2016 или 2017 должна быть установлена платформа [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 4.6 или 4.6.1, соответственно. Платформа [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] предоставляет инфраструктуру для работы приложений [!INCLUDE[ndptecclick](../../includes/ndptecclick-md.md)] .  

Необходимо использовать [!INCLUDE[msCoName](../../includes/msconame-md.md)] Internet Explorer 11 или другой современный браузер.  
...
По умолчанию построитель отчетов включен. Администраторы сервера отчетов могут отключить построитель отчетов, присвоив системному свойству **ShowDownloadMenu** сервера отчетов значение **false**. Этот параметр отключает загрузку построителя отчетов, издателя мобильных отчетов и мобильной платформы Power BI для этого сервера отчетов.  
