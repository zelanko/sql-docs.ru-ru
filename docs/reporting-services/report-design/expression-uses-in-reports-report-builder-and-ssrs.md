...
description: Задавайте или вычисляйте с помощью выражений значения для параметров, запросов, фильтров и свойств текстовых полей в построителе отчетов.
ms.date: 03/14/2017
...
|Спрятать текстовое поле в области конструктора, переключая его видимость с помощью параметра логического типа с именем *Show*.|Hiddenproperty для текстового поля. Используйте диалоговое окно **Свойства текстового поля — Видимость**.|`=Not Parameters!` *Показать\<boolean parameter>* `.Value`|  
