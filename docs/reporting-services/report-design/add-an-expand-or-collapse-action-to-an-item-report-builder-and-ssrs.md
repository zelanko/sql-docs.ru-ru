---
title: Добавление действия "Развернуть" или "Свернуть" (построитель отчетов и службы SSRS) | Документы Майкрософт
description: Дайте пользователю возможность развертывать или свертывать элементы, строки и столбцы, связанные с группой для таблицы или матрицы в отчете, в построителе отчетов.
ms.date: 12/19/2019
...
ms.locfileid: "84681303"
---
# <a name="add-an-expand-or-collapse-action-to-an-item-report-builder--ssrs"></a>Добавление действия "Развернуть" или "Свернуть" к элементу (построитель отчетов и службы SSRS)
...
1.  В конструкторе отчетов щелкните таблицу или матрицу, чтобы выбрать ее. На панели группирования будут отображены группы столбцов и строк.  
     ...
     ![Панель группировки](../../reporting-services/report-design/media/groupingpane.png "Панель группировки")  
...
4.  В пункте **Скрытый**выберите один из следующих параметров для установки видимости данного элемента отчета при первом запуске отчета.  
    ...
    -   Выберите пункт **\<Expression>** , чтобы открыть диалоговое окно **Выражение** для создания выражения, вычисляемого во время выполнения для определения видимости.  
...
5.  В раскрывающемся списке **ToggleItem**выберите имя текстового поля, к которому добавляется изображение переключателя.  
     ...
     ![Настройка развертываемой группы строки](../../reporting-services/report-design/media/expandcollapse-confighiddentoggleitemwithnumbers.png "Настройка развертываемой группы строк")  
...
1.  В конструкторе отчетов щелкните правой кнопкой мыши элемент отчета для его отображения или скрытия, а затем выберите пункт *\<report item>*  **Свойства**. Для элемента отчета откроется диалоговое окно *\<report item>* **Свойства**.  
