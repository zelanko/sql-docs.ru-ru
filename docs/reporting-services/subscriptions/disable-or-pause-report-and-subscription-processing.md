...
ms.locfileid: "67313998"
---
# <a name="disable-or-pause-report-and-subscription-processing"></a>Отключение или приостановка обработки отчетов и подписок  
Существует несколько подходов, которые позволяют отключить или приостановить обработку отчетов и подписок [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] . В этой статье описаны различные подходы — от отключения подписки до прерывания подключения к источнику данных. Не все подходы возможны с обоими [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] режимы работы сервера. Следующие таблицы сводка методов и поддерживается [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] режимы сервера:  
...
>Новые возможности в SQL 2016 Reporting Services, *Включение и отключение подписок*. Новые параметры пользовательского интерфейса позволяют быстро включать и отключать подписки. В отключенных подписках сохраняются другие свойства конфигурации, например расписание. Такие подписки можно легко включить снова. Кроме того, можно программным путем включать и отключать подписки, а также проверять, какие именно подписки отключены.  
  ...
  ![Включение и отключение кнопок страницы "подписки" ](../../reporting-services/subscriptions/media/disable-or-pause-report-and-subscription-processing/subscription-enable-and-disable-buttons.png)  
  
На веб-портале, перейдите к подписке либо **Мои подписки** страницы или **подписок** страницы отдельной подписки. Выберите одну или несколько подписок, а затем нажмите кнопку отключения или включения на ленте (см. предыдущую иллюстрацию). В столбце состояния изменится на «Включено» или «Отключено» соответственно.  
  
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] записывают строку в [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] ведения журнала при подписке включен или отключен. Например, в файле журнала сервера отчетов:  
...
-   **Собственный режим:** на веб-портале выберите **параметры** кнопку ![кнопка "Параметры"](media/ssrs-portal-settings-gear.png) в строке меню в верхней части веб-портала экрана и выберите **параметры сайта**в раскрывающемся меню. Выберите **расписания** вкладку, чтобы отобразить страницу «расписания». Установите флажки рядом с расписания, чтобы включить или отключить, а затем выберите **включить** или **отключить** кнопку, соответственно, чтобы выполнить требуемое действие. В столбце состояния изменится на «Отключено» или «Включено» соответствующим образом.  
...
4.  Разверните **ролей** узел и выберите нужную роль.  
  
5.  Щелкните роль правой кнопкой мыши и выберите пункт **Свойства**.  
  
6.  Очистить **управление отдельными подписками** и **управление всеми подписками** задачи.  
...
-   Библиотека SharePoint (доступна только с сайта SharePoint, который интегрирован с сервером отчетов, работающим в режиме интеграции с SharePoint).  
 ...
 Доставку по электронной почте перед использованием необходимо настроить. Если настройка не выполнена, эта функция недоступна. Дополнительные сведения см. в разделе [настройки электронной почты — Reporting Services в собственном режиме (диспетчер конфигурации)](../install-windows/e-mail-settings-reporting-services-native-mode-configuration-manager.md).  
  
 Если требуется отключить конкретные модули, можно удалить записи, относящиеся к этим модулям, из файла **RSReportServer.config** . Дополнительные сведения см. в разделе [файлы конфигурации служб Reporting Services](../../reporting-services/report-server/reporting-services-configuration-files.md) и [настройки электронной почты — Reporting Services в собственном режиме (диспетчер конфигурации)](../install-windows/e-mail-settings-reporting-services-native-mode-configuration-manager.md).  
