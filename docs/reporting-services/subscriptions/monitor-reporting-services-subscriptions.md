...
description: Сведения о том, как использовать файлы пользовательского интерфейса, PowerShell или журналов для отслеживания подписок Reporting Services. Параметры мониторинга зависят от используемого режима сервера отчетов.
ms.date: 06/12/2019
...
:::row:::
    :::column:::
        [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] Основной режим  
    :::column-end:::
    :::column:::
        [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] Режим интеграции с SharePoint  
    :::column-end:::
:::row-end:::
...
-   [Режим интеграции с SharePoint](#bkmk_sharepoint_mode)  
...
|Готово. Обработано \<*number*> из \<*number*>; ошибок: \<*number*>.|Состояние подписки, управляемой данными. Это сообщение обработчика планирования и доставки.|  
|Обработано: \<*number*>|Число уведомлений обработчика планирования и доставки об успешном завершении доставки или о прекращении дальнейших попыток доставки. При завершении управляемой данными доставки число обработанных уведомлений должно равняться общему количеству созданных уведомлений о доставке.|  
|Всего: \<*number*>|Общее количество уведомлений, созданных для последней доставки подписки.|  
|Ошибок: \<*number*>|Число уведомлений обработчика планирования и доставки о невозможности доставки или прекращении повторных попыток.|  
...
|Файл \<*filename*> записан в папку \<path>.|Показывает, что доставка в расположение общей папки завершилась успешно. Это сообщение модуля доставки в общую папку.|  
...
|Сбой при подключении к папке назначения \<path>. Убедитесь в существовании папки назначения или общей папки.|Показывает, что невозможно найти указанную папку. Это сообщение модуля доставки в общую папку.|  
|Не удалось записать файл \<filename> в \<path>. Попытка повтора.|Показывает, что невозможно обновить файл новой версией. Это сообщение модуля доставки в общую папку.|  
|Ошибка при записи файла \<filename>: \<message>|Показывает, что доставка в расположение общей папки завершилась неуспешно. Это сообщение модуля доставки в общую папку.|  
|\<custom status messages>|Сообщения о состоянии (об успешном и неуспешном завершении доставки), формируемые модулями доставки. При использовании сторонних или пользовательских модулей доставки могут выводиться дополнительные сообщения о состоянии.|  
...
### <a name="native-mode-log-files"></a>Файлы журналов (собственный режим)  
