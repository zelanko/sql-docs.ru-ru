...
description: Узнайте, как подготовить отчет к просмотру с помощью моментального снимка журнала отчета. Для этого необходимо указать параметр rs:Snapshot и присвоить ему значение допустимого идентификатора моментального снимка.
ms.date: 03/01/2017
