...
|**-s**  *servername*|Обязательный.|Указывает экземпляр [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] , на котором размещается база данных сервера отчетов.|  
...
|**-p**  *password*|Обязателен, если указан ключ **-u** .|Указывает пароль, используемый с аргументом *имя_пользователя* . Для этого аргумента можно установить пустое значение, если учетная запись не требует пароля. Это значение учитывает регистр для учетных записей домена.|  
