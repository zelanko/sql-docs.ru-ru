---
title: "Устранение неполадок с публикацией и просмотром отчетов на сервере отчетов, работающем в основном режиме | Microsoft Docs"
ms.custom: ""
ms.date: "02/28/2016"
...
---
# Устранение неполадок с публикацией и просмотром отчетов на сервере отчетов, работающем в основном режиме
...
## Почему при публикации отчета выдается приглашение к вводу учетных данных?  
...
Убедитесь, что в качестве URL-адреса сервера отчетов действительно указан адрес сервера отчетов (например, `http://localhost/reportserver`), а не адрес виртуального каталога диспетчера отчетов (например, `http://localhost/reports`). Если для сервера отчетов задан номер порта, отличный от применяемого по умолчанию (80), такой порт необходимо указать в адресе сервера отчетов (например, `http://localhost:81/reportserver`).   
 ...
 ## Ничего не происходит при переключении элементов в опубликованном отчете  
...
## Не загружаются изображения и диаграммы для выполнения отчета режима Run As и браузера  
При выполнении диспетчера отчетов в другом контексте безопасности в отчете могут быть не видны все его элементы.   
  
### Недостаточно разрешений в папках временных файлов Интернета  
