---
title: Добавление SQLRUserGroup в качестве имени входа SQL Server — SQL Server службы машинного обучения
description: Для замыкания соединений с помощью неявной проверки подлинности Добавление SQLRUserGroup в качестве имени входа SQL Server, чтобы с рабочей учетной записью можно войти на сервер, для преобразования идентификаторов для вызывающего пользователя.
ms.prod: sql
...
Ошибка подключения можно обойти, предоставляя **SQLServerRUserGroup** имени входа SQL Server. Дополнительные сведения об удостоверениях и внешних процессов см. в разделе [Общие сведения о безопасности для инфраструктуры расширяемости](../concepts/security.md).

> [!Note]
>  Убедитесь, что **SQLRUserGroup** имеет разрешения «Локальный вход в систему». По умолчанию это право предоставляется всем новым локальным пользователям, но в некоторых организациях более строгие групповые политики может отключить это право.
