...
3.  Чтобы включить внешние средства написания сценариев, поддерживающий Python, выполните следующую инструкцию:
    ...
    Если был включен компонент для языка R, не требуется для запуска перенастроить еще раз для Python. Базовая платформа расширяемости поддерживает оба языка.
