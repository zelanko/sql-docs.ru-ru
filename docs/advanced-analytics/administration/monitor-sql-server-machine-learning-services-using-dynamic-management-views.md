---
title: Мониторинг сервера службы машинного обучения SQL с помощью динамических административных представлений (DMV) | Документация Майкрософт
description: Используйте динамические административные представления (DMV) для мониторинга служб машинного обучения SQL Server.
ms.prod: sql
...
Просмотр сведений о конфигурации максимальный объем памяти в процентах от SQL Server и внешних пулов ресурсов. Если SQL Sever выполняется со значением по умолчанию `max server memory (MB)`, он рассматривается как 100% от памяти операционной системы.
