---
title: "Сохранение и загрузка объектов R из SQL Server с помощью ODBC | Microsoft Docs"
ms.custom: ""
ms.date: "12/16/2016"
...
---
# Сохранение и загрузка объектов R из SQL Server с помощью ODBC
...
Для повышения производительности этого критического этапа пакет **RevoScaleR** теперь содержит новые функции сериализации и десериализации, призванные значительно повысить производительность и обеспечить более компактное хранение объектов. Кроме того, объекты R можно сохранить в SQL Server непосредственно из среды R путем вызова этих новых функций через соединение ODBC с помощью *RxOdbcData*.

## <a name="overview"></a>Обзор

Теперь пакет **RevoScaleR** включает новые функции, которые упрощают сохранение объектов R в SQL Server и последующее их считывание из таблицы SQL Server. Для использования этих функций требуется установить подключение к серверу SQL Server с помощью источника данных *RxOdbcData*.

В общем случае вызовы функций моделируются на базе простого хранилища значений ключей, в котором ключ является именем объекта, а значение, сопоставленное с этим ключом, является объектом R типа varbinary, перемещаемым в таблицу или из нее. 

- По умолчанию любой объект, вызываемый из R для перемещения в SQL Server, сериализуется и сжимается. 
- И наоборот, при загрузке объекта из таблицы SQL Server для использования в коде R объект десериализуется и распаковывается.
- При необходимости можно указать, что объект не сериализуется, и выбрать новый алгоритм сжатия для использования вместо алгоритма по умолчанию.
...
Для получения подробных сведений о синтаксисе каждой функции воспользуйтесь справкой R. Позже подробные сведения можно будет найти в статье [ScaleR reference](https://msdn.microsoft.com/microsoft-r/scaler/scaler) (Справочник по ScaleR) на сайте MSDN.
...
[Функции и задачи служб R Services](../../advanced-analytics/r-services/sql-server-r-services-features-and-tasks.md)
