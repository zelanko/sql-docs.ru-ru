...
Вы используете сочетание кода R, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] данных и пользовательских функций SQL для создания модели классификации, которая указывает вероятность того, что драйвер может получить совет по определенному пути такси. Вы также развертываете модель R в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] и использует данные сервера для создания оценок на основе модели.
...
В этом пошаговом руководстве требуется несколько библиотек R, которые не устанавливаются [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)]по умолчанию в составе. Пакеты необходимо установить как на клиенте, где разрабатывается решение, так и на [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] компьютере, на котором развертывается решение.
...
2. В командной строке R выполните следующие команды R:
  ...
  ```
  В этом примере функция R grep используется для поиска вектора доступных путей и поиска пути, включающего "Program Files". Дополнительные сведения см. в [https://www.rdocumentation.org/packages/base/functions/grep](https://www.rdocumentation.org/packages/base/functions/grep)разделе.
