...
В этом примере показано, как можно создать модель логистической регрессии в SQL Server, с помощью алгоритма из **revoscalepy** пакета.
...
1. Создание переменных Python, таких как `sql_query` и `sql_connection_string`, определяющие источник и данные, которые вы хотите использовать. Передавать эти переменные RxSqlServerData конструктор, чтобы реализовать **объекта источника данных** с именем `data_source`.
...
Можно также использовать функцию **rxsetcomputecontext** для переключения между контексты вычислений, которые уже были определены. 
...
Дополнительные сведения см. в разделе [RxInSqlServer](https://docs.microsoft.com/r-server/r-reference/revoscaler/rxinsqlserver).
