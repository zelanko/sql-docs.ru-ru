---
title: Шаг 2 импорта данных в SQL Server с помощью PowerShell | Документы Microsoft
ms.prod: sql
...
На этом шаге выполните одну из загруженного скриптов создания объектов базы данных, необходимые для данного пошагового руководства. Скрипт также создает несколько хранимых процедур и отправляет образец данных в таблицу в базе данных указанного.
...
По умолчанию в скрипте предполагается имя пользователя базы данных SQL Server и пароль. Если db_owner под учетной записью пользователя Windows, можно использовать для создания объектов удостоверения Windows. Чтобы сделать это, откройте `RunSQL_SQL_Walkthrough.ps1` в редакторе кода для добавления **`-T`** для bcp bulk insert, команда:
...
2. Скрипт запрашивает следующие сведения:
    ...
    - Если не указано имя пользователя и пароль для входа в SQL Server используется удостоверения Windows.
