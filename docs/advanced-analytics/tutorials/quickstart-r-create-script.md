...
1. Передайте полный скрипт R в хранимую процедуру `sp_execute_external_script`.

   Скрипт передается через аргумент `@script`. Все, что находится внутри аргумента `@script`, должно быть допустимым кодом R.
...
1. Вычисляется правильный результат, а функция R `print` возвращает результат в окно **сообщений** .
...
|WITH RESULT SETS | Определяет схему возвращаемой таблицы данных для SQL Server, добавляя "Hello World" в качестве имени столбца, **int** для типа данных. |
...
По умолчанию `sp_execute_external_script` принимает один набор данных в качестве входных, что обычно предоставляется в виде допустимого SQL-запроса. Затем он возвращает один кадр данных R в качестве выходных данных.
...
1. Выполните следующий скрипт R. Он получает данные из таблицы с помощью оператора `SELECT`, передает их через среду выполнения R и возвращает данные в виде кадра данных. Предложение `WITH RESULT SETS` определяет схему возвращаемой таблицы данных для SQL, добавляя имя столбца *невколнаме*.
...
1. Теперь давайте изменим имена входных и выходных переменных. Имена входных и выходных переменных по умолчанию — **InputDataSet** и **OutputDataSet**. Этот сценарий изменяет имена на **SQL_in** и **SQL_out**:
    ...
    Обратите внимание, что в R учитывается регистр. Входные и выходные переменные, используемые в скрипте R (**SQL_out**, **SQL_in**), должны соответствовать именам, определенным в `@input_data_1_name` и `@output_data_1_name`, включая регистр.
...
1. Можно также создавать значения только с помощью скрипта R без входных данных (`@input_data_1` установлен в пустое значение).
...
Выходные данные получены от `installed.packages()` в R и возвращаются в виде результирующего набора.
...
Чтобы узнать, как использовать структуры данных при использовании R в SQL Server Службы машинного обучения, следуйте указаниям в этом кратком руководстве:
