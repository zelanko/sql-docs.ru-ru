...
1. Выполните следующий скрипт, чтобы выполнить процедуру. Конкретная инструкция для исполнения хранимой процедуры находится `EXECUTE` на четвертой строке.
...
Теперь, когда модель создана, обучена и сохранена, переходите к следующему шагу: создание хранимой процедуры, создающей прогнозы. Это делается путем вызова метода `sp_execute_external_script` для запуска скрипта Python, который загружает сериализованную модель и дает ей новые входные данные для оценки.
...
Чтобы узнать об обработке типов данных Python в SQL Server, следуйте указаниям в этом кратком руководстве:
