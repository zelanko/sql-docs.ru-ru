...
В этой статье объясняется, как установить службы машинного обучения SQL Server в Windows. Службы машинного обучения можно использовать для запуска сценариев R или Python в базе данных.

## <a name="bkmk_prereqs"> </a> Контрольный список перед установкой

+ Необходим экземпляр ядра СУБД. Вы не можете установить только функции R или Python, хотя их можно добавить в существующий экземпляр постепенно.
...
+ Установка служб машинного обучения *не поддерживается* в отказоустойчивом кластере SQL Server 2017. Однако она *поддерживается* в SQL Server 2019. 
...
+ Параллельная установка с другими версиями R и Python поддерживается, но не рекомендуется. Он поддерживается, поскольку экземпляр SQL Server использует собственные копии дистрибутивов R и Anaconda с открытым исходным кодом. Но параллельная установка не рекомендуется, так как выполнение кода, использующего R и Python на компьютере SQL Server за пределами SQL Server, может привести к различным проблемам:
...
1. На странице **Выбор компонентов** выберите следующие компоненты:
   ...
   - **Службы машинного обучения (в базе данных)**
     ...
     Этот параметр устанавливает службы баз данных, поддерживающие выполнение сценариев R, Python и Java.
   ...
   - **Java**
     ...
     Установите этот флажок, чтобы установить открытый JRE, входящий в состав SQL, или укажите расположение другой версии JDK или JRE.
   ::: moniker-end
   ...
   ::: moniker range="=sql-server-ver15||=sqlallproducts-allversions"
   ![Параметры функции для R и Python](media/2019setup-features-page-mls-rpy.png "Параметры установки для R, Python и Java")
   ::: moniker-end
...
1. На странице **Расположение установки Java** можно установить версию Open JRE, входящую в состав SQL, или указать расположение собственной установки JDK или JRE. Выберите **Далее**.
...
> [!NOTE]
> Столбцы или заголовки, используемые в сценарии Python, не возвращаются (по проекту). Чтобы добавить имена столбцов для вывода, необходимо указать схему для набора возвращаемых данных. Для этого используйте параметр WITH RESULTS хранимой процедуры, указывая имена столбцов и типы данных SQL.
> 
...
На устройствах, подключенных к Интернету, накопительные обновления обычно применяются с помощью Центра обновления Windows, но вы также можете использовать приведенные ниже шаги для управления контролируемыми обновлениями. При применении обновления для ядра СУБД программа установки извлекает накопительные обновления для всех компонентов R или Python, установленных в том же экземпляре. 
...
::: moniker range=">=sql-server-ver15||=sqlallproducts-allversions"
В SQL Server 2019 в Windows механизм изоляции изменился. Это влияет на **SQLRUserGroup**, правила брандмауэра, разрешение файла и подразумеваемую проверку подлинности. Дополнительные сведения см. в разделе [Изменения в изоляции служб машинного обучения](sql-server-machine-learning-services-2019.md).
::: moniker-end
...
Пакеты, которыми вы хотите пользоваться из SQL Server, должны быть установлены в библиотеке по умолчанию, используемой экземпляром. При наличии отдельной установки Python или R на компьютере, а также если вы установили пакеты в библиотеки пользователей, вы не сможете использовать их из T-SQL.
