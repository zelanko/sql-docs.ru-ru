...
Аналитика в базе данных SQL Server 2016 — предназначенным только для R с помощью только два CAB файлов для пакетов продукта и распространяемого корпорацией Майкрософт R с открытым кодом, соответственно. Начать с установки любого из этих выпусков: RTM, с Пакетом обновления 1, 2 (SP2). После установки на месте, накопительные пакеты обновления могут применяться в качестве следующего шага.
...
Скопируйте установочный носитель SQL Server (ISO-файл или CAB) и файлы в базе данных аналитики CAB-файла на конечном компьютере. Поместить CAB-файлов и установки файла мультимедиа в той же папке на целевом компьютере, например **загружает** или установки % temp * папки.
...
Если сервер не имеет доступа к Интернету, применения обновлений службы, загрузив обновленного сервера SQL установщика и соответствующих версиях CAB-файлов для конкретного языка. 

1. Начать с экземпляром базовых показателей. Интегрированная установка обновления поддерживаются на этих выпусков SQL Server:

  + Первоначальный выпуск SQL Server 2017
  + Первоначальный выпуск SQL Server 2016
  + SQL Server 2016 с пакетом обновления 1
  + SQL Server 2016 SP 2

2. Получите обновленную версию установщика SQL Server для заданного накопительного пакета обновления. Любое обновление для функции обучения (R и Python) компьютера — вместе с накопительным пакетом обновления базового экземпляра компонента database engine.
...
3. Получите соответствующий CAB-файлы для R и Python. Ссылки для загрузки см. в разделе [CAB-файлов загрузки для накопительные пакеты обновления для анализа в базе данных SQL Server экземпляры](sql-ml-cab-downloads.md).

4. Поместите все файлы в той же папке, запустите программу установки. Во время установки будет предложено выбрать расположение папки для обновленных файлов CAB.
