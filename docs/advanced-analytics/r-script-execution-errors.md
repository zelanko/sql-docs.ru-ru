---
title: Устранение неполадок скриптов R
description: В этой статье описываются некоторые ошибки, возникающие при выполнении скриптов R в SQL Server.
ms.prod: sql
...
В этой статье описываются некоторые ошибки, возникающие при выполнении скриптов R в SQL Server. Этот список не является исчерпывающим. Существует множество пакетов, и ошибки в разных версиях одного пакета могут различаться. Мы рекомендуем публиковать сообщения об ошибках скриптов на [форуме Machine Learning Server](https://social.msdn.microsoft.com/Forums/home?category=MicrosoftR), на котором предоставляется поддержка по компонентам машинного обучения, используемым в R Services (в базе данных), Microsoft R Client и Microsoft R Server.
