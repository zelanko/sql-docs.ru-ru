...
Однако такой подход имеет множество недостатков, для которых имеется нарушено широкого внедрения R на предприятии. 
...
### <a name="using-other-r-packages"></a>С помощью других пакетов R

Помимо собственных библиотеки R, включенные с помощью машинного обучения Microsoft, можно использовать почти все пакеты R в решении, включая:
...
Обычно процесс развертывания начинается с очистки скрипта от ненужного в рабочей среде кода. При перемещении вычислений ближе к данным, вы можете найти способы для более эффективного перемещения, суммировать или представления данных, вместо выполнять все действия в R.  Мы рекомендуем, специалист по обработке данных обратитесь к разработчику базы данных, о способах повышения производительности, особенно в том случае, если решение выполняет очистку данных или признаков, которые могут быть более эффективными в SQL. Возможно, потребуется изменить процессы извлечения, преобразования и загрузки, чтобы рабочие процессы для создания или оценки модели выполнялись без сбоев, а входные данные имели нужный формат.
