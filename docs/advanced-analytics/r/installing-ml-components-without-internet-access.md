---
title: "Установка машинного обучения компоненты без доступа к Интернету | Документы Microsoft"
ms.custom: 
ms.date: 04/20/2017
...
---
# <a name="installing-machine-learning-components-without-internet-access"></a>Установка машинного обучения компоненты без доступа к Интернету

Поскольку компоненты R и Python, предоставляемые с SQL Server 2016 или 2017 г. SQL Server открытым исходным кодом, корпорация Майкрософт не устанавливает компоненты R или Python по умолчанию.

Вместо этого мы предоставляют связанные установщики и объединенными пакетов для удобства в центре загрузки Майкрософт и других надежных сайтов. Должен дать согласие соответствующая лицензия, и затем программы установки SQL Server устанавливаются компоненты R или Python для вас.
...
Как правило программа установки компонентов компьютера, используемого в SQL Server 2016 и 2017 г. SQL Server требуется подключение к Интернету. При запуске программы установки SQL Server, при выборе любого параметра learnig компьютера, программа установки проверит Python или R установщики, как хорошо, как и любые другие необходимые компоненты. Если имеется подключение к Интернету, SQL Server установит их автоматически.

> [!IMPORTANT]
> На сервере без доступа к Интернету необходимо загрузить дополнительные установщики перед продолжением установки.

Как минимум необходимо загрузить R или Python установщики, которые поддерживаются для версии или SQL Server, который вы устанавливаете номер сборки.

В зависимости от конфигурации сервера могут понадобиться дополнительные компоненты, такие как .NET Core.  В разделе [дополнительных компонентов](#bkmk_OtherComponents) подробные сведения.
...
Для **R** в SQL Server 2016 и 2017 г. SQL Server, вам потребуется получить два разных установщика. Мастер установки SQL Server обеспечит их установку в правильном порядке.

+ Установщики с **SRO** в имени содержатся компоненты открытым исходным кодом.
+ Insallers с **SRS** в имени содержат компоненты, предоставляемые корпорацией Майкрософт, включая те, для интеграции базы данных.


Для **Python** загрузки одного CAB-файла и всех необходимых компонентов в SQL Server 2017 г.


1. Скачайте установщик на [сайте центра загрузки Майкрософт](#installerlocs) на компьютер, где есть доступ к Интернету, и сохраните его.
2. Скопируйте файлы программы установки (CAB) на компьютер, где выполняется установка компонентов машины обучения.
3. В настоящее время мастер установки установит английского языка по умолчанию. Для установки с помощью другого языка, изменить имена файлов установщика, как описано здесь: [изменения, необходимые для разных языковых стандартов](#modslocales).
4. Загрузите дополнительные компоненты, которые требуются, такие как MPI или .NET Core.
5. При необходимости можно загрузить архивированного исходного кода для компонентов открытым исходным кодом, но это не является обязательным для установки SQL Server и можно выполнить в любое время. Дополнительные сведения см. в разделе [R Server для Windows](https://msdn.microsoft.com/microsoft-r/rserver-install-windows).


> [!NOTE]
> Файлы должны соответствовать версии SQL Server, которую вы собираетесь установить.
> 
> В CTP 2.0 2017 г. SQL Server предоставляется поддержка Python. Более ранние версии, включая SQL Server 2016, не поддерживают Python.

Пошаговое руководство по процессу установки в автономном режиме для служб R в SQL Server 2016, мы рекомендуем статьи по [SQL Server группа консультантов по](https://blogs.msdn.microsoft.com/sqlcat/2016/10/20/do-it-right-deploying-sql-server-r-services-on-computers-without-internet-access/). В нем также описываются сценарии интегрированной установки и установки исправлений.
...
## <a name="installerlocs"></a>Файлы для загрузки
...
Если вы хотите просмотреть исходный код для Microsoft R, его можно загрузить как архив в формате TAR или: [установщиков загрузки R Server](https://msdn.microsoft.com/microsoft-r/rserver-install-windows#download)
...
Если вы скачиваете файлы CAB в процессе установки SQL Server на компьютере с доступом к Интернету, мастер установки определяет местный язык и автоматически меняет язык установщика.
...
Например, если вы устанавливаете японскую версию SQL Server, имя файла SRS_8.0.3.0_**1033**.cab следует изменить на SRS_8.0.3.0_**1041**.cab.
...
**Компоненты R в SQL Server SQL Server 2017 г.**
...
[Установка Microsoft R Server](https://msdn.microsoft.com/microsoft-r/rserver-install-windows)
