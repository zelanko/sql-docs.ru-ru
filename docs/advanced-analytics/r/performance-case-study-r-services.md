...
В этом разделе перечислены сведения о результатах первый пример внедрения. Для второго примера реализации сводку описывает общую результатов проверки. В конце этого раздела вы найдете ссылки на все скрипты и образец данных и ресурсы, используемые авторов оригинала.
...
## <a name="case-study-optimization-for-resume-matching-task"></a>Практический пример: оптимизация для возобновления соответствующих задач

Модель сопоставления resume был разработан корпорацией Майкрософт специалист по анализу данных Ke Huang для тестирования производительности кода R в SQL Server и включения специалистами по анализу данных для поддержки масштабируемого, корпоративные решения.
...
Для модели сопоставления resume использования внешнего скрипта было активно и возникли другие базы данных под управлением службы ядра СУБД. Таким образом ресурсы, выделенные для внешних скриптов был увеличен до 70%, который был оптимальные для производительности сценария.

Эта конфигурация была полученных в процессе экспериментов с разными значениями. Если используется другое оборудование или другого решения оптимальной конфигурации могут быть иными.

> [!IMPORTANT]
> Экспериментируйте с целью поиска оптимальной конфигурации для вашего случая!
