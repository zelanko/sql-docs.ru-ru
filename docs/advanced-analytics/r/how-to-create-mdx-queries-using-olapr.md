...
[OlapR](https://docs.microsoft.com/machine-learning-server/r-reference/olapr/olapr) пакет поддерживает запросы к многомерной модели, размещенные в SQL Server Analysis Services. Построить запрос к существующего куба с помощью многомерных Выражений, просмотра измерений и других объектов куба и вставьте в существующих запросах многомерных Выражений для извлечения данных.

В этой статье описаны два основных областей применения пакета olapR:

+ [Построение запроса из R, с помощью конструкторов, предоставленных в пакете olapR](#buildMDX)
...
+ Создать куб, который используется в этих примерах ниже учебнике Analysinotes служб до занятия 4: [создание куба OLAP](../../analysis-services/multidimensional-modeling-adventure-works-tutorial.md)
...
Элементы в результаты, возвращаемые с префиксом  **->**  представляют потомков предыдущего элемента.
