...
R с открытым исходным кодом — распределение корпорации Майкрософт [Microsoft R Open (MRO)](https://mran.microsoft.com/open). Поддержка языка R содержит основные возможности, такие как **базового**, **stats**, **utils**и др. Базовой установки R также включает многочисленные образцы наборов данных и стандартные средства R, такие как **RGui** (упрощенных интерактивный редактор) и **RTerm** (Командная строка R). MRO добавляет значение базового R, включая дополнительных пакетов с открытым исходным кодом, такие как [библиотека математического ядра Intel](https://en.wikipedia.org/wiki/Math_Kernel_Library).

Частные R в установке в пакетах:

+ [RevoScaleR](https://docs.microsoft.com/r-server/r-reference/revoscaler/revoscaler) для контекстах удаленных вычислений, потоковой передачи, параллельного выполнения по функциям rx для импорта данных и преобразования данных, моделирования, визуализации и анализа. 
+ [MicrosoftML](https://docs.microsoft.com/r-server/r-reference/microsoftml/microsoftml-package) добавляет машинного обучения на R. моделирования
+ [olapR](https://docs.microsoft.com/machine-learning-server/r-reference/olapr/olapr) для написания инструкций многомерных Выражений в R.
+ [sqlrutils](https://docs.microsoft.com/machine-learning-server/r-reference/sqlrutils/sqlrutils) для включения сценария R в хранимые процедуры.

В следующей таблице перечислены версии R, предоставляемые MRO и пакеты Microsoft, установленные с обработчиками конкретных аналитика в базе данных.
...
| [SQL Server 2017 г машинного обучения служб](../install/sql-machine-learning-services-windows-install.md) | 3.3.3. | RevoScaleR, MicrosoftML, olapR, sqlrutil|

Можно обновить компоненты R-пакеты, добавлять новые пакеты R и предварительно установленные модели по *привязки* политику современных жизненного цикла поддержки. Привязка изменяется модели обслуживания. По умолчанию после начальной установки, пакеты R обновляются через пакеты обновления и накопительные обновления. Дополнительные пакеты и обновления полной версии компонентов R, возможны только через обновления продукта (из SQL Server 2016 2017 г. SQL Server) или путем привязки R поддерживает Microsoft Server обучения машины. Дополнительные сведения см. в разделе [обновление R и Python компоненты SQL Server](use-sqlbindr-exe-to-upgrade-an-instance-of-sql-server.md).
...
Библиотека пакета, используемый экземпляром в базе данных физически расположены в папке Program Files своего экземпляра SQL Server. Запись в этом расположении необходимы права администратора. Однако 2017 г. SQL Server предоставляет некоторых дополнительных методов для установки пакета, который дает возможность добавления пакетов без прав администратора.

+ В SQL Server 2016 административного доступа является обязательным для новой установки пакета.
