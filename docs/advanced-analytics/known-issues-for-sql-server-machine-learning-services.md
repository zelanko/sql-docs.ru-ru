---
title: Известные проблемы в службах машинного обучения | Документация Майкрософт
ms.prod: sql
...
ms.locfileid: "51704112"
---
# <a name="known-issues-in-machine-learning-services"></a>Известные проблемы в службах машинного обучения
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

В этой статье описаны известные проблемы и ограничения с машинным обучением компоненты, которые предоставляются в виде параметра в SQL Server 2016 и SQL Server 2017.

Приведенные здесь сведения относятся ко всем следующим образом, если не указано обратное:
...
- Службы для R (в базе данных) машинного обучения
- Службы для Python (в базе данных) машинного обучения
...
### <a name="r-script-runtime-error-sql-server-2017-cu5-cu7-regression"></a>Ошибка выполнения скрипта R (SQL Server 2017 CU5 CU7 Регрессия)
...
### <a name="unable-to-install-sql-server-machine-learning-features-on-a-domain-controller"></a>Не удалось установить функции машинного обучения SQL Server на контроллере домена
...
### <a name="install-the-latest-service-release-to-ensure-compatibility-with-microsoft-r-client"></a>Установка последней версии службы для обеспечения совместимости с Microsoft R Client
...
**Применяется к:** SQL Server 2016 R Services, с помощью R Server используется версия 9.0.0 или более ранней версии
...
### <a name="unable-to-install-python-components-in-offline-installations-of-sql-server-2017-ctp-20-or-later"></a>Не удалось установить компоненты Python в автономной установке SQL Server 2017 CTP 2.0 или более поздней версии
...
**Применяется к:** SQL Server 2017 с помощью Python
...
### <a name="launchpad-services-fails-to-start-if-the-version-is-different-from-the-r-version"></a>Службы панели запуска не запускаются, если версия отличается от версии R
...
### <a name="remote-compute-contexts-are-blocked-by-a-firewall-in-sql-server-instances-that-are-running-on-azure-virtual-machines"></a>Контексты удаленных вычислений блокируются брандмауэром в экземплярах SQL Server, работающие на виртуальных машинах Azure
...
**Применяется к:** SQL Server 2016 служб R SQL Server, экспресс-выпуск

### <a name="performance-limits-when-libraries-used-by-sql-server-are-called-from-other-tools"></a>Ограничение производительности при вызове библиотек, используемый сервером SQL Server из других инструментов
...
### <a name="sql-server-data-tools-does-not-support-permissions-required-by-external-scripts"></a>SQL Server Data Tools не поддерживает разрешения, необходимые для выполнения внешних скриптов
...
### <a name="external-script-execution-is-throttled-due-to-resource-governance-default-values"></a>Выполнение внешнего скрипта регулируется из-за значения по умолчанию для ресурсов управления
...
### <a name="access-denied-warning-when-executing-r-scripts-on-sql-server-in-a-non-default-location"></a>Отказано в доступе предупреждение при выполнении скриптов R на SQL Server в расположении по умолчанию не
...
### <a name="serialization-error-between-old-and-new-versions-of-revoscaler"></a>Ошибка сериализации между старой и новой версиях RevoScaleR
...
### <a name="real-time-scoring-does-not-correctly-handle-the-learningrate-parameter-in-tree-and-forest-models"></a>Оценки в реальном времени неправильно обрабатывает _значение learningRate_ параметр в моделях дерева и леса
...
**Применяется к:** версии SQL Server 2016 R Services RTM
...
### <a name="avoid-clearing-workspaces-when-you-execute-r-code-in-a-includessnoversionincludesssnoversion-mdmd-compute-context"></a>Старайтесь не очищать рабочие области, при выполнении кода R в [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] контекста вычислений
...
### <a name="use-of-strings-as-factors-can-lead-to-performance-degradation"></a>Использование строк как факторов может привести к снижению производительности

Использование типа строковых переменных, как факторы могут значительно увеличить объем памяти, используемый для операций R. Это известная проблема с R в целом, и существует множество статей по этой теме. Например, см. в разделе [факторы не являются привилегированными r, Джон подключения, в R-блоггеры)](https://www.r-bloggers.com/factors-are-not-first-class-citizens-in-r/) или [stringsasfactors значения: несанкционированного текст биографии, Роджер Peng](https://simplystatistics.org/2015/07/24/stringsasfactors-an-unauthorized-biography/). 
...
### <a name="arguments-varstokeep-and-varstodrop-are-not-supported-for-sql-server-data-sources"></a>Аргументы *varsToKeep* и *varsToDrop* не поддерживаются для источников данных SQL Server
...
Параллелизм для `rxExec` в нескольких процессах запланирована в будущих выпусках.
...
### <a name="call-to-pretrained-model-fails-if-path-to-model-is-too-long"></a>Вызов предварительно обученной модели завершается неудачей, если указан слишком длинный путь к модели
...
### <a name="error-when-saving-serialized-model-to-sql-server"></a>Произошла ошибка при сохранении сериализована модель для SQL Server
...
### <a name="telemetry-warning-on-successful-execution-of-python-code"></a>Предупреждение телеметрии при успешном выполнении кода Python
