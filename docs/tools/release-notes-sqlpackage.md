...
| Компонент | Сведения |
| :------ | :------ |
| Развертывание | Добавлена поддержка для развертывания в хранилище данных SQL Azure (GA). | 
| Платформа | SqlPackage .NET Core (общедоступная версия) для macOS, Linux и Windows. | 
| безопасность | Удалите подписывание кода SHA1. |
| Развертывание | Добавлена поддержка новых выпусков базы данных Azure: Женералпурпосе, Бусинесскритикал, Scale |
| Развертывание | Добавьте поддержку Управляемый экземпляр для пользователей и групп AAD. |
| Развертывание | Поддержка параметра/Акцесстокен для SqlPackage в .NET Core. |
| &nbsp; | &nbsp; |
...
| Компонент | Сведения |
| :------ | :------ |
| ScriptDom |  Регрессия анализа ScriptDom была введена в 18.3.1, где "regression" неправильно обрабатывается как маркер верхнего уровня, что приводит к сбою синтаксического анализа. Это будет исправлено в следующем SqlPackage выпуске. | 
| &nbsp; | &nbsp; |
...
| Компонент | Сведения |
| :------ | :------ |
| Импорт |  Для файлов. BACPAC с сжатыми файлами размером до 4 ГБ для выполнения импорта может потребоваться использовать версию .NET Core SqlPackage.  Такое поведение обусловлено тем, как .NET Core создает заголовки ZIP, которые, хотя и являются допустимыми, не могут быть прочитаны с помощью полной версии .NET Framework SqlPackage. | 
| Развертывание | Параметр/p: Storage = File не поддерживается. В .NET Core поддерживается только память. | 
| Always Encrypted | SqlPackage .NET Core не поддерживает Always Encrypted столбцы. | 
| безопасность | SqlPackage .NET Core не поддерживает параметр/УА для многофакторной проверки подлинности. | 
...
| Компонент | Сведения |
| :------ | :------ |
| Развертывание | Добавлена поддержка для развертывания в хранилище данных SQL Azure (Предварительная версия). | 
