...
|**/Деплойскриптпас:**|**/дсп**|{строка}|Указывает необязательный путь для вывода скрипта развертывания. В среде Azure при использовании команд TSQL для создания или изменения базы данных master скрипт будет записан по тому же пути, но с именем выходного файла Filename_Master.sql. |
