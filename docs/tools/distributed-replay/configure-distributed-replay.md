...
|Максимальное время простоя|`<MaxIdleTime>`|Время простоя в секундах.|Целое число больше >= -1.<br /><br /> Значение `-1` указывает, что значение в исходном файле трассировки не менялось.<br /><br /> `0` указывает наличие некоторой активности, которая происходит в любой момент времени.|Нет. Значением по умолчанию является `-1`.|  
