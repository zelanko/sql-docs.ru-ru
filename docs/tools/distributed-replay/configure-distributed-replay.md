...
### <a name="possible-issue-when-running-with-synchronization-sequencing-mode"></a>Возможная ошибка при запуске с режимом последовательности синхронизации
 Вы можете столкнуться с симптомом, в котором функция воспроизведения выглядит как "кабина", или воспроизведение событий очень медленно. Это явление может возникать, если воспроизводимая трассировка зависит от данных и (или) событий, которые не существуют в восстановленной целевой базе данных. 
 
 Одним из примеров является захваченная Рабочая нагрузка, использующая WAITFOR, например инструкция RECEIVE Service Broker. При использовании режима последовательности синхронизации пакеты воспроизводятся последовательно. Если операция вставки происходит в базе данных-источнике после резервного копирования базы данных, но до начала трассировки записи воспроизведения, то инструкция WAITFOR RECEIVE, выданная во время воспроизведения, может заждать всю длительность WAITFOR. События, настроенные для воспроизведения после того, как инструкция WAITFOR RECEIVE будет остановлена. Это может привести к тому, что счетчик монитора производительности "пакетных запросов/с" для конечного объекта базы данных воспроизведения сбрасывается в ноль до тех пор, пока не завершится инструкция WAITFOR. 
...
2.  Разрешить завершение всех ожидающих действий.
