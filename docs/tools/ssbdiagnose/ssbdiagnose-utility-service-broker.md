---
title: "Программа ssbdiagnose (компонент Service Broker) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Service Broker, отчеты времени выполнения"
  - "Service Broker, программы командной строки"
  - "устранение неполадок [Service Broker], диалоги"
  - "устранение неполадок [Service Broker], конфигурации"
  - "программы командной строки [компонент Service Broker]"
  - "Service Broker, устранение неполадок"
  - "Service Broker, отчеты о конфигурации"
  - "Service Broker, инструменты"
  - "устранение неполадок [Service Broker], время выполнения"
  - "диалоги, устранение неполадок [Service Broker]"
  - "устранение неполадок [Service Broker], программа ssbdiagnose"
  - "инструменты [Service Broker], ssbdiagnose"
  - "Service Broker, программа ssbdiagnose"
  - "ssbdiagnose"
...
## Переменные среды sqlcmd  
 Программа **ssbdiagnose** поддерживает переменные среды SQLCMDSERVER, SQLCMDUSER, SQLCMDPASSWORD и SQLCMDLOGINTIMEOUT, которые также используются программой **sqlcmd** . Они могут быть заданы при помощи команды командной строки SET или команды **setvar** в скриптах [!INCLUDE[tsql](../../includes/tsql-md.md)] , которые выполняются программой **sqlcmd**. Дополнительные сведения об использовании **setvar** в **sqlcmd** см. в разделе [Использование программы sqlcmd с переменными скрипта](../../relational-databases/scripting/use-sqlcmd-with-scripting-variables.md).  
