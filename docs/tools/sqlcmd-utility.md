...
ms.locfileid: "68065484"
---
# <a name="sqlcmd-utility"></a>Служебная программа sqlcmd
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

> Дополнительные сведения о SQL Server 2014 и более [низких см](https://docs.microsoft.com/sql/tools/sqlcmd-utility?view=sql-server-2014
> ). в разделе Программа sqlcmd.
> 
> Сведения об использовании программы sqlcmd в Linux см. в [статье Установка sqlcmd и bcp в Linux](../linux/sql-server-linux-setup-tools.md).

 Программа **sqlcmd** позволяет вводить инструкции TRANSACT-SQL, системные процедуры и файлы скриптов с помощью различных доступных режимов.

- В командной строке.
- В **редакторе запросов** в режиме SQLCMD.
- В файле сценария Windows.
- В шаге задания операционной системы (cmd. exe) агент SQL Server задания.

Программа использует ODBC для выполнения пакетов Transact-SQL.

## <a name="download-the-latest-version-of-sqlcmd-utility"></a>Скачайте последнюю версию программы sqlcmd
...
**Требования к системе** Windows 10, Windows 7, Windows 8, Windows 8.1, Windows Server 2008, Windows Server 2008 R2, Windows Server 2008 R2 с пакетом обновления 1 (SP1), Windows Server 2012, Windows Server 2012 R2 для этого компонента требуется как [установщик Windows 4,5](https://www.microsoft.com/download/details.aspx?id=8483) , так и [Microsoft ODBC Driver 17.3.1.1 для SQL Server](https://www.microsoft.com/download/details.aspx?id=56567).
 
Чтобы проверить выполнение `sqlcmd -?` команды sqlcmd Version и убедиться, что используется версия 15.0.1300.359 или более поздняя.



> [!NOTE]
> Для поддержки Always encrypted (`-g`) и проверки подлинности Azure Active Directory (`-G`) требуется версия 13,1 или более поздняя. (На компьютере можно установить несколько версий sqlcmd.exe. Убедитесь, что используется правильная версия. Чтобы определить версию, выполните `sqlcmd -?`.)

Программу sqlcmd можно запустить из Azure Cloud Shell, так как она предустановлена по умолчанию: [ ![Запуск Cloud Shell](https://shell.azure.com/images/launchcloudshell.png "Launch Cloud Shell")](https://shell.azure.com)
 ...
 **Параметры, связанные с именем для входа**  
  **-A**  
 Входит в SQL Server через выделенное административное соединение. Этот вид соединения предназначен для устранения неполадок на сервере. Это подключение работает только с серверными компьютерами, поддерживающими DAC. Если выделенное административное соединение недоступно, **sqlcmd** выдает сообщение об ошибке и завершает работу. Дополнительные сведения о DAC см. в статье [Диагностическое соединение для администраторов баз данных](../database-engine/configure-windows/diagnostic-connection-for-database-administrators.md). Параметр-A не поддерживается параметром-G. При подключении к базе данных SQL с помощью-A необходимо быть администратором SQL Server. DAC недоступен для администратора Azure Active Directory.
...
- **Встроенная проверка подлинности Azure Active Directory** 
   ...
   *Встроенная проверка подлинности AAD в настоящее время не поддерживается в Linux или macOS*.
...
- **Интерактивная проверка подлинности Azure Active Directory**  
   ...
   Для интерактивной работы Azure AD требуется **sqlcmd** [версии 15.0.1000.34](#download-the-latest-version-of-sqlcmd-utility) или более поздней, а также [ODBC версии 17,2 или более поздней](https://www.microsoft.com/download/details.aspx?id=56567).  
   ...
   Если пользователь Azure AD является федеративным пользователем домена с использованием учетной записи Windows, то имя пользователя, необходимое в командной строке, содержит учетную запись домена (например, joe@contoso.com см. ниже):

   ```
   sqlcmd -S testsrv.database.windows.net -d Target_DB_or_DW -G -U joe@contoso.com  
   ```
 
   Если гостевые пользователи существуют в определенной службе Azure AD и являются частью группы, которая существует в базе данных SQL, которая имеет разрешения базы данных для выполнения команды sqlcmd, используется псевдоним гостевого пользователя (например, *keith0@adventureworks.com* ).

  >[!IMPORTANT]
  >Существует известная неполадка, возникающая `-G` при `-U` использовании параметра и с программой `-U` sqlcmd, где `-G` размещение параметра перед параметром может привести к сбою проверки подлинности. Всегда начинайте с `-G` параметра, за которым `-U` следует параметр.
 ...
 **-P** _пароль_  
 Пароль, задаваемый пользователем. В паролях учитывается регистр символов. Если указан параметр -U, а параметр **-P** отсутствует и не задана переменная среды SQLCMDPASSWORD, программа **sqlcmd** запрашивает пароль у пользователя. Не рекомендуется использовать пароль null, но можно указать пароль null, используя пару смежных двойных кавычек для значения параметра:

- **-P ""**

Рекомендуется использовать надежный пароль.
 
#### <a name="use-a-strong-passwordrelational-databasessecuritystrong-passwordsmd"></a>[**Выбирайте надежные пароли!** ](../relational-databases/security/strong-passwords.md)
...
**ПРИМЕЧАНИЕ.**  Переменная среды OSQLPASSWORD была сохранена в целях обратной совместимости. Переменная среды SQLCMDPASSWORD имеет приоритет над переменной среды OSQLPASSWORD. Теперь, когда OSQLPASSWORD больше не предоставляется для общего доступа, программы **sqlcmd** и **osql** можно использовать рядом друг с другом без помех. Старые скрипты будут продолжать работать.  
 ...
 **-i** _input_file_ [ **,** _входные\_file2_...]  
 ...
 **-y** _ширина_отображения_типа_переменной_длины_  
 ...
 **-Y** _ширина_отображения_типа_фиксированной_длины_  
 ...
 **-V** _степень_серьезности_ошибки_  
...
>  Из-за особенностей передачи широковещательных сообщений по сети программа **sqlcmd** не всегда своевременно получает ответ от всех серверов. Поэтому возвращаемый список серверов от вызова к вызову может различаться.  
 ...
 Если указать необязательный параметр **c**, данные будут выводиться без строки заголовка **Servers:** (Серверы), а в строке каждого сервера не будет начальных пробелов. Эта презентация называется «чистым выводом». Чистый выход улучшает производительность обработки языков скриптов.  
 ...
 **:Connect** (Подключиться) _имя_сервера_[ **\\** _имя\_экземпляра_] [-l *время_ожидания*] [-U *имя_пользователя* [-P *пароль*]]  
...
-   Для команд **:Error**, **:Out** и **:Perftrace** должно быть указано отдельное **\<** _имя_файла_ **>** . Одинаковое **\<** _имя_файла_ **>** может привести к смешиванию входных данных команд.  
