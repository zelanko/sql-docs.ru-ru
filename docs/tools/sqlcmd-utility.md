---
title: "Программа sqlcmd | Microsoft Docs"
ms.custom: ""
ms.date: "11/29/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "инструкции [SQL Server], командная строка"
  - "QUIT, команда"
  - "инструкции Transact-SQL, командная строка"
  - "EXIT, команда"
  - "sqlcmd, команды"
  - "ED, команда"
  - "sqlcmd, программа"
  ...
  - "!! command"
  - "хранимые процедуры [SQL Server], командная строка"
  - "системные хранимые процедуры [SQL Server], командная строка"
  - "программа sqlcmd, о программе sqlcmd"
  - "скрипты [SQL Server], командная строка"
  - "RESET, команда"
  - "GO, команда"
...
[!INCLUDE[tsql-appliesto-ss2008-asdb-asdw-pdw-_md](../includes/tsql-appliesto-ss2008-asdb-asdw-pdw-md.md)]
  ...
  Программа **sqlcmd** позволяет вводить инструкции Transact-SQL, системные процедуры и файлы скриптов из командной строки в **редактор запросов** в режиме SQLCMD, в файл скрипта Windows или шаг задания операционной системы (Cmd.exe) задания агента [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]. Эта программа использует технологию ODBC для выполнения пакетов [!INCLUDE[tsql](../includes/tsql-md.md)]. 
  
> [!NOTE]
> Самые последние версии программы sqlcmd доступны в виде веб-выпуска в [Центре загрузки](http://go.microsoft.com/fwlink/?LinkID=825643). Для поддержки Always Encrypted (`-g`) и проверки подлинности Azure Active Directory (`-G`) требуется версия не ниже 13.1. (На компьютере можно установить несколько версий sqlcmd.exe. Убедитесь, что используется правильная версия. Чтобы определить версию, выполните `sqlcmd -?`.)
...
> [!IMPORTANT] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] использует [!INCLUDE[msCoName](../includes/msconame-md.md)][!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)]SqlClient для выполнения в обычном режиме и в режиме SQLCMD в **редакторе запросов**. При вызове программы **sqlcmd** из командной строки **sqlcmd** использует драйвер ODBC. Так как могут применяться разные параметры по умолчанию, выполнение одного и того же запроса в среде [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] в режиме SQLCMD и в программе **sqlcmd** может различаться.  
>   
 ...
 Другие разделы:
- [Запуск программы sqlcmd](../relational-databases/scripting/start-the-sqlcmd-utility.md)   
 ...
 **Параметры, связанные с именем для входа**  
  **-A**  
 Входит на [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] посредством выделенного административного соединения. Этот вид соединения предназначен для устранения неполадок на сервере. Параметр будет работать только с теми серверами, которые поддерживают выделенные административные соединения. Если соединение DAC недоступно, программа **sqlcmd** выдает сообщение об ошибке и завершает работу. Дополнительные сведения о DAC см. в статье [Диагностическое соединение для администраторов баз данных](../database-engine/configure-windows/diagnostic-connection-for-database-administrators.md).  
 ...
 **-S** [*протокол*:]*сервер*[**\\***имя_экземпляра*][**,***порт*]  
 Указывает экземпляр [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], к которому выполняется подключение. Задает переменную скрипта SQLCMDSERVER программы **sqlcmd** .  
 ...
 Если значение *имя_сервера* [ **\\***имя_экземпляра* ] при запуске программы **sqlcmd** не указано, [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] проверяет наличие переменной среды SQLCMDSERVER и, если она задана, использует ее значение.  
...
>  В целях обратной совместимости доступна переменная среды OSQLUSER. Переменная среды SQLCMDUSER имеет больший приоритет, чем OSQLUSER. То есть программы **sqlcmd** и **osql** могут применяться одновременно, не мешая друг другу. Кроме того, это означает, что существующие скрипты **osql** не перестанут работать.  
 ...
 Если не указан ни параметр **-U**, ни параметр **-P**, программа **sqlcmd** пытается подключиться с помощью режима проверки подлинности [!INCLUDE[msCoName](../includes/msconame-md.md)] Windows. При этом используется учетная запись пользователя Windows, который запустил программу **sqlcmd**.  
 ...
 **-R**  
 Указывает **sqlcmd**, что локализация в столбцах числовых и денежных данных, значений даты и времени, полученных из [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], должна выполняться с учетом языка и региональных параметров клиента. По умолчанию эти столбцы отображаются с использованием региональных параметров сервера.  
...
-   **!!** *команда*  
 ...
 Указание параметра **-X** блокирует передачу переменных среды программе **sqlcmd**. Он также препятствует выполнению скрипта запуска, указанного с помощью переменной скрипта SQLCMDINI. Дополнительные сведения об использовании переменных скрипта **sqlcmd** см. в разделе [Использование программы sqlcmd с переменными скрипта](../relational-databases/scripting/use-sqlcmd-with-scripting-variables.md).  
...
## <a name="remarks"></a>Замечания  
 ...
 Для работы с программой **sqlcmd** в интерактивном режиме введите в командной строке **sqlcmd** , указав любой из параметров, описанных ранее в этой статье. Дополнительные сведения см. в статье [Использование программы sqlcmd](../relational-databases/scripting/use-the-sqlcmd-utility.md)  
...
## <a name="sqlcmd-commands"></a>Команды sqlcmd  
 В дополнение к инструкциям [!INCLUDE[tsql](../includes/tsql-md.md)] в программе **sqlcmd**доступны следующие команды.  
 ...
 **GO** [*число*]  
 GO обозначает конец пакетного файла и исполнения любых кэшированных инструкций [!INCLUDE[tsql](../includes/tsql-md.md)]. Пакет выполняется несколько раз в виде отдельных пакетов; в одном пакете переменную можно объявить всего один раз.
  
 **Прочие команды**  
  **:r \<** *имя_файла* **>**  
 Производит синтаксический анализ дополнительных инструкций [!INCLUDE[tsql](../includes/tsql-md.md)] и команд программы **sqlcmd** в файле **\<***имя_файла***>**, загружая их в кэш инструкций.  
 ...
 **:Connect**  *имя_сервера*[**\\***имя_экземпляра*] [-l *время_ожидания*] [-U *имя_пользователя* [-P *пароль*]]  
...
-   Для команд **:Error**, **:Out** и **:Perftrace** должно быть указано отдельное **\<***имя_файла***>**. Одинаковое **\<***имя_файла***>** может привести к смешиванию входных данных команд.  
...
###  <a name="a-nameoutputxmla-xml-output-format"></a><a name="OutputXML"></a> Формат вывода XML  
...
###  <a name="a-nameoutputjsona-json-output-format"></a><a name="OutputJSON"></a> Формат вывода JSON  
