 ...
 **Sqlcmd** служебная программа позволяет вводить инструкции Transact-SQL, системные процедуры и файлы скриптов посредством различных доступных режимов:

- В командной строке.
- В **редактора запросов** в режиме SQLCMD.
- В файле скрипта Windows.
- Шаг задания операционной системы (Cmd.exe) задания агента SQL Server.
...
**[![Скачайте](../ssdt/media/download.png) скачать служебные программы командной строки Microsoft 15.0.x для SQL Server (x64) (2,4 МБ)](https://go.microsoft.com/fwlink/?linkid=2043518)**
<br>**[![Скачайте](../ssdt/media/download.png) скачать служебные программы командной строки Microsoft 15.0.x для SQL Server (x86) (2,2 МБ)](https://go.microsoft.com/fwlink/?linkid=2043622)**
 ...
 **Параметры, связанные с именем для входа**  
  **-A**  
 Входит в SQL Server через выделенное административное соединение. Этот вид соединения предназначен для устранения неполадок на сервере. Это подключение работает только с серверами, которые поддерживают приложения уровня данных. Если выделенное административное соединение недоступно, **sqlcmd** выдает сообщение об ошибке и завершает работу. Дополнительные сведения о DAC см. в статье [Диагностическое соединение для администраторов баз данных](../database-engine/configure-windows/diagnostic-connection-for-database-administrators.md). Параметр - не поддерживается с параметром -G. При подключении к базе данных SQL с параметром - A, необходимо быть администратором сервера SQL. Приложения уровня данных не доступна для администратора Azure Active Directory.
...
- **Имя пользователя и пароль Azure Active Directory** 
    ...
    ``` 
    Параметр -G формирует следующую строку подключения в серверной части. 
...
- **Встроенная проверка подлинности Azure Active Directory** 
   ...
   *Встроенная проверка подлинности AAD сейчас не поддерживается в Linux или macOS*.
...
- **Интерактивная проверка подлинности Azure Active Directory**  
   ...
   Интерактивный Azure AD требует **sqlcmd** [версии 15.0.1000.34](#download-the-latest-version-of-sqlcmd-utility) или более поздней версии, а также [ODBC версии 17.2 или более поздней](https://www.microsoft.com/download/details.aspx?id=56567).  
   ...
   Предыдущая команда формирует следующую строку подключения в серверной части.  

   ```
   SERVER = Target_DB_or_DW.testsrv.database.windows.net;UID=alice@aadtest.onmicrosoft.com; AUTHENTICATION = ActiveDirectoryInteractive   
   ```

   Если пользователем Azure AD является федеративным пользователем домена с помощью учетной записи Windows, в командной строке требуется имя пользователя содержит свою учетную запись домена (например, joe@contoso.com см. ниже):

   ```
   sqlcmd -S testsrv.database.windows.net -d Target_DB_or_DW -G -U joe@contoso.com  
   ```
 
   Если гостевых пользователей существуют в определенной Azure AD и являются частью группы, которая существует в базе данных SQL с разрешениями базы данных для выполнения команды sqlcmd, псевдонима гостевой используется (например, *keith0@adventureworks.com*).

  >[!IMPORTANT]
  >Имеется известная проблема, при использовании `-G` и `-U` параметр с помощью SQLCMD, где размещение `-U` параметр перед `-G` параметр может вызвать сбой проверки подлинности. Всегда начинаются с `-G` параметра `-U` параметр.
 ...
 **-P** *пароль*  
 Пароль, задаваемый пользователем. В паролях учитывается регистр символов. Если указан параметр -U, а параметр **-P** отсутствует и не задана переменная среды SQLCMDPASSWORD, программа **sqlcmd** запрашивает пароль у пользователя. Не рекомендуется использовать пустой пароль, но можно указать пустой пароль с помощью пары смежных двойные кавычки для значения параметра:

- **-P ""**

Мы рекомендуем использовать надежный пароль.
...
**ПРИМЕЧАНИЕ.**  Переменная среды OSQLPASSWORD была сохранена в целях обратной совместимости. Переменная среды SQLCMDPASSWORD имеет приоритет над переменная среды OSQLPASSWORD. Теперь, когда OSQLPASSWORD больше не является общей, служебные программы **sqlcmd** и **osql** используется рядом друг с другом без помех. Старых сценариев будут продолжать работать.  
 ...
 **-S** [*протокола*:]*server*[**\\**_экземпляр\_имя_] [**,**  _порт_]  
 ...
 **Параметры ввода-вывода**  
  **-f** *кодовая_страница* | **i:**_кодовая_страница_[**,o:**_кодовая_страница_] | **o:**_кодовая_страница_[**,i:**_кодовая_страница_]  
 ...
 **-i** *входной_файл*[**,**_входной\_"файл2"_...]  
...
>  Из-за особенностей передачи широковещательных сообщений по сети программа **sqlcmd** не всегда своевременно получает ответ от всех серверов. Поэтому возвращаемый список серверов от вызова к вызову может различаться.  
 ...
 Если указать необязательный параметр **c**, данные будут выводиться без строки заголовка **Servers:** (Серверы), а в строке каждого сервера не будет начальных пробелов. Данная презентация называется чистым выводом. Чистый выход улучшает производительность обработки языков скриптов.  
 ...
 **GO** [*число*]  
 GO обозначает конец пакета и выполнение любых кэшированных инструкций Transact-SQL. Пакет выполняется несколько раз в виде отдельных пакетов. Невозможно объявить переменную более одного раза в одном пакете.
 ...
 **:Connect** (Подключиться) *имя_сервера*[**\\**_имя\_экземпляра_] [-l *время_ожидания*] [-U *имя_пользователя* [-P *пароль*]]  
