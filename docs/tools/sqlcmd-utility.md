 ...
 **-S** [*protocol*:]*server*[**\\***instance_name*][**,***port*]  
 ...
 Если вы не укажете *имя_сервера* [**\\*** имя_экземпляра* ] при запуске **sqlcmd**, SQL Server проверяет и использует среды SQLCMDSERVER переменная.  
 ...
 **Параметры ввода-вывода**  
  **-f** *кодовую страницу* | **i: ***кодовую страницу*[**, o:***кодовая страница *] | **o: ***кодовую страницу*[**, i:***кодовая страница *]  
...
```
 ...
 Этот параметр может использоваться более одного раза: **-i *** входной_файл* **-я *** я входной_файл.*  
 ...
 **-vvar =**  *значение*[ **var =** *значение*...]  
 Создает переменную скрипта **sqlcmd**, которая может быть использована в скрипте **sqlcmd** . Если значение содержит пробелы, заключите его в кавычки. Можно указать несколько ***var***=**»***значения***»** значения. Если в каком-либо из указанных значений обнаружена ошибка, программа **sqlcmd** выдает сообщение и завершает работу.  
 ...
 **Прочие команды**  
  **:r \<** *filename* **>**  
 Выполняет синтаксический анализ дополнительных инструкций Transact-SQL и **sqlcmd** команды из файла, указанного параметром  **\< ***filename***>**в инструкцию кэш.  
