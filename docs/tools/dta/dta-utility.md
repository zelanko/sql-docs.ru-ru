---
title: "dta, программа | Документы Microsoft"
ms.custom: 
ms.date: 01/09/2017
...
---
# <a name="dta-utility"></a>dta, программа
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]**Dta** программа — версия ядра СУБД для командной строки. Программа **dta** предназначена для использования функций помощника по настройке ядра СУБД в приложениях и скриптах.  
...
**-fc**  
 Указывает, что индексы columnstore необходимо рассмотреть новые рекомендации для. ОНА будет рассматривать обоих columnstore кластеризованных и некластеризованных индексов. Дополнительные сведения см. в разделе    
[Рекомендации по индексам ColumnStore в помощник по настройке ядра базы данных (DTA)](../../relational-databases/performance/columnstore-index-recommendations-in-database-engine-tuning-advisor-dta.md).
 ||  
|-|  
...
|ПОЛНОЕ|Полное секционирование (выберите для улучшения производительности)|  
...
**-iq**  
 Указывает, что хранилище запросов использовать в качестве рабочей нагрузки. Анализируется верхняя тысяча событий из хранилища запросов для явно выбранных баз данных. Это значение можно изменить с помощью параметра **–n** .  В разделе [хранилище запросов](../../relational-databases/performance/how-query-store-collects-data.md) и [помощник по настройке базы данных с помощью рабочей нагрузки из хранилища запросов](../../relational-databases/performance/tuning-database-using-workload-from-query-store.md) для получения дополнительной информации.
 ||  
|-|  
|**Область применения**: начиная с [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].|  
 ...
 **-it** *workload_trace_table_name*  
 Задает имя таблицы, содержащей трассировку рабочей нагрузки для настройки. Имя задается в следующем формате: [*database_name*]**.**[*owner_name*]**.***table_name*.  
 ...
 **-I** *time_window_in_hours*   
   Задает интервал времени (в часах) при запросе должна выполняться для того, чтобы оно рассматривалось как DTA для настройки при использовании **-iq** параметр (рабочая нагрузка из хранилища запросов). 
```  
dta -iq -I 48  
```  
В этом случае DTA использовать хранилище запросов в качестве источника рабочей нагрузки и использовать только запросов, выполненных за последние 48 часов.  
  ||  
|-|  
|**Область применения**: начиная с [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].|  
