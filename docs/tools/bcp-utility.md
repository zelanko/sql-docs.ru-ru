...
**-D**<a name="D"></a>  
Значение, передаваемое в параметр `bcp` `-S`, интерпретируется как имя источника данных (DSN). Дополнительные сведения см. в разделе _Поддержка имени DSN в sqlcmd и bcp_ статьи [Соединение с помощью sqlcmd](../connect/odbc/linux-mac/connecting-with-sqlcmd.md).
