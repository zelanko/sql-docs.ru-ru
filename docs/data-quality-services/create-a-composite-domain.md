...
5.  В **сопоставления** таблицы, выберите исходный столбец из раскрывающегося списка для **исходный столбец** столбец пустой строки. Убедитесь, что исходный столбец содержит составной домен, состоящий из двух существующих отдельных доменов. Если соответствующих отдельных доменов не существует, щелкните значок **Создать домен** .  
  
6.  В **сопоставления** таблицы, выберите исходный столбец из раскрывающегося списка для **исходный столбец** столбец пустой строки. Убедитесь, что исходный столбец содержит составной домен, на части которого указывают два существующих отдельных домена. Если соответствующих отдельных доменов не существует, щелкните значок **Создать домен** , чтобы создать эти домены. Дополнительные сведения см. в статье [Create a Domain](../data-quality-services/create-a-domain.md).  
...
3.  Убедитесь, что существуют несколько доменов, необходимых для составного домена. В противном случае щелкните значок **Создать домен** и создайте его. Дополнительные сведения см. в статье [Create a Domain](../data-quality-services/create-a-domain.md).  
...
4.  Выберите одно из следующих значений для параметра **Метод анализа**:  
    ...
    -   **Ссылочные данные**: синтаксический анализ значений поля в соответствии с форматированием данных службы данных ссылок (RDS). Службы Data Quality Services передают значения в составной домен в RDS, а RDS возвращает исправленные и проанализированные данные в соответствии с доменом в составном домене.  
...
5.  Если в качестве метода анализа выбрано **Разделители** , вы можете также установить флажок **Использовать анализ на основе знаний**. Дополнительные сведения см. в статье [Синтаксический анализ на основе знаний](#KnowledgeBaseParsing).  
  
6.  Нажмите кнопку **Готово** , чтобы завершить операцию управления доменами, как описано в разделе [End the Domain Management Activity](../Topic/End%20the%20Domain%20Management%20Activity.md).  
...
##  <a name="KnowledgeBaseParsing"></a> Синтаксический анализ на основе знаний  
