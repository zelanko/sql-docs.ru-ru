---
title: "Настройка степеней серьезности для файлов журнала DQS | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
helpviewer_keywords: 
  - "степени серьезности"
  - "файлы журналов, степени серьезности"
  - "файлы журналов DQS, степени серьезности"
  - "ведение журналов, степени серьезности"
  - "настройка степеней серьезности"
...
-   **Неустранимая**: критические ошибки времени выполнения, может привести к серьезным/непредвиденным результатам.  
...
-   **Отладка**: сведения о событии (verbose).  
  
 С помощью настройки степеней серьезности для различных операций и модулей DQS осуществляется фильтрация сведений, которые необходимо записать в журнал, и запись в файл журнала DQS для соответствующей операции или модуля DQS. Например, если задать уровень серьезности операциями **Предупреждать**, только предупреждения и выше серьезности (ошибки и Неустранимая ошибка), связанный с действием DQS будут регистрироваться.  
...
3.  Затем перейдите на вкладку **Параметры журнала** . Следующие действия DQS перечислены, для которых можно выбрать уровень серьезности: **Управление доменами**, **обнаружения знаний**, **Очистка проекта (например, RDS)**, **соответствующие политики и проекта**, и **служб удаленных рабочих СТОЛОВ**.  
  
4.  Для операции DQS выберите степень серьезности для записи в журнал. Вы можете выбрать один из следующих вариантов: **Неустранимая ошибка**, **Ошибка**, **Предупреждение**, **Информация**и **Отладка**. Например, если требуется только Неустранимая сообщения для записи файлов журнала DQS для обнаружения знаний, выберите **Неустранимая ошибка** в раскрывающемся списке **обнаружения знаний** действия.  
...
2.  В появившейся сетке выберите имя модуля из раскрывающегося списка в **модуль** столбца.  
  
3.  Далее выберите уровень серьезности для модуля из раскрывающегося списка в **серьезности** столбца. Вы можете выбрать один из следующих вариантов: **Неустранимая ошибка**, **Ошибка**, **Предупреждение**, **Информация**и **Отладка**.  
  
     К примеру, для действия управления доменами можно установить другой уровень гранулярности для функциональной возможности определения правила домена, чем операция управления доменами, выбрав модуль **Microsoft.Ssdqs.DomainRules.Define** и другая степень серьезности для записи в журнал. Аналогично, можно задать другой уровень гранулярности для функций междоменного правила, выбрав **Microsoft.Ssdqs.DomainRules.Condition.CrossDomain** модуль и выберите другой журнал уровень серьезности.  
