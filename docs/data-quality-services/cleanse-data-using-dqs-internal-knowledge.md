---
title: Очистка данных с использованием набора знаний служб DQS (внутренних)
ms.date: 03/01/2017
ms.prod: sql
...
##  <a name="create-a-cleansing-data-quality-project"></a><a name="Create"></a> Создание проекта качества очистки данных  
...
##  <a name="mapping-stage"></a><a name="Mapping"></a> Стадия сопоставления  
...
##  <a name="computer-assisted-cleansing-stage"></a><a name="ComputerAssisted"></a> Этап автоматизированной очистки  
...
##  <a name="interactive-cleansing-stage"></a><a name="Interactive"></a> Этап интерактивной очистки  
...
##  <a name="export-stage"></a><a name="Export"></a> Этап экспорта  
...
3.  Далее выберите выходные данные: экспортировать только очищенные данные или экспортировать очищенные данные наряду с информацией очистки.  
    ...
    -   **Сведения о данных и очистке**. Щелкните переключатель, чтобы экспортировать следующие данные для каждого домена:  
        ...
        -   ** \<Domain> _Source**: исходное значение в домене.  
  
        -   ** \<Domain> _Output**: очищенные значения в домене.  
  
        -   ** \<Domain> _Reason**: причина, указанная для исправления значения.  
  
        -   ** \<Domain> _Confidence**: уровень достоверности для всех исправленных условий. Она отображается в виде десятичного значения, эквивалентного соответствующим процентным значениям. Например, степень достоверности 95 % отображается как 0,9500000.  
  
        -   ** \<Domain> _Status**: состояние значения домена после очистки данных. В качестве примера можно назвать **Предложенное**, **Новое**, **Недействительное**, **Исправленное**или **Правильное**.  
  
        -   **Состояние записи**. Помимо поля состояния для каждого сопоставленного домена **( \<DomainName> _Status**), в поле **состояние записи** отображается состояние записи. Если какое-либо состояние домена в записи является *новым* или *правильным*, **состояние записи** будет установлено в значение *верно*. Если какое-либо состояние домена в записи *предложено*, *недопустимо*или *Исправлено*, то **состояние записи** будет равно соответствующему значению. Например, если в записи будет *предложено*какое-либо состояние домена, то для **записи** будет задано значение " *предложено*".  
...
##  <a name="profiler-statistics"></a><a name="Profiler"></a>Статистика профилировщика  
