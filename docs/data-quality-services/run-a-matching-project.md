...
3.  Выберите **Открыть проект служб DQS** , чтобы выполнить сопоставление в существующем проекте служб DQS. Выберите проект и нажмите кнопку **Далее**. (Или щелкнуть проект в **Последний проект качества данных**.) При открытии проекта сопоставления, была закрыта, можно будет перейти к этапу закрытое действие проекта сопоставления в (подтверждается **состояние** столбца в таблице проекта или в области имени проекта **последний проект качества данных**). Если открыть завершенный проект сопоставления, будет выполнен переход к **Экспорт** страницы (и не может вернуться на предыдущие экраны).  
...
4.  Щелкните **плюс (+)** элемента управления, чтобы добавить строку в таблицу сопоставления или **минус (-)** элемента управления, чтобы удалить строку.  
...
6.  Щелкните **Просмотр и выбор составных доменов** для просмотра списка составных доменов, доступных в базе знаний и выбранных для сопоставления.  
...
1.  На **сопоставление**, выберите **перекрывающиеся кластеры** из раскрывающегося списка для отображения сводных записей и следующих записей для всех кластеров при выполнении сопоставления, даже если группы кластеров имеют общие записи. Для отображения кластеров, которые имеют общие записи, в качестве единого кластера при выполнении сопоставления выберите пункт **Неперекрывающиеся кластеры** .  
  
2.  Щелкните **перезагрузить данные из источника** (по умолчанию) для копирования данных из источника данных в промежуточную таблицу и повторной индексации при запуске проекта сопоставления. Щелкните **выполнить на предыдущих данных** для запуска проекта сопоставления без копирования данных в промежуточную таблицу и повторной индексации данных. Пункт**Выполнить на предыдущих данных** отсутствует при первом запуске проекта сопоставления или при изменении сопоставления на странице **Сопоставление** и нажатии кнопки **Да** во всплывающем окне. В обоих этих случаях необходимо произвести повторную индексацию. Нет необходимости в выполнении повторной индексации, если проект сопоставления не был изменен. Выполнение на предыдущих данных может повысить производительность.  
...
7.  При выборе не сводной записи в таблице и нажмите кнопку **Просмотр сведений о** значок (или дважды щелкните запись), DQS будет отображать **сведения о сопоставлении Оценка** всплывающее окно, отображающее двойном щелчке записи и ее Сводная запись (и значения всех полей), оценка между ними и детализация совпадающего оценка вклад каждого поля. Двойной щелчок сводной записи не приводит к отображению всплывающего меню.  
...
10. Чтобы изменить минимальный показатель сопоставления, определяющий уровень сопоставления, для отображения записи, выберите **мин. Показатель сопоставления** значок над правой части таблицы и введите число больше. Минимальный показатель сопоставления по умолчанию составляет 80%. Нажмите кнопку **Обновить** , чтобы изменить содержание таблицы.  
...
3.  Установите флажок для **результаты сопоставления** для экспорта результатов сопоставления (см. выше объяснение) для указанной таблицы в базе данных SQL Server или для указанного CSV-файла или файла Excel. Установите флажок для **Результаты выживания** для экспорта результатов выживания (см. выше объяснение) для указанной таблицы в базе данных SQL Server, либо для указанного CSV-файла или файла Excel.  
...
7.  Выберите правило выживания следующим образом:  
    ...
    -   Выберите **Сводная запись** (по умолчанию) для идентификации выжившие записи в качестве первоначальной сводной записи, выбранной произвольно службами DQS.  
