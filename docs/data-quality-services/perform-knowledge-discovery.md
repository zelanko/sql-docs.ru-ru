...
1.  В **источника данных** выберите **SQL Server** (по умолчанию) или **файл Excel**.  
...
4.  В таблице **Сопоставления** сопоставьте каждый из исходных столбцов, в которых необходимо провести обнаружение наборов знаний, с доменами в базе знаний, как описано ниже.  
    ...
    1.  Создать сопоставление, выбрав исходный столбец из раскрывающегося списка для **исходный столбец** пустую строку, а затем выбрав домен из раскрывающегося списка для столбца **домена** столбец в той же строке, если домен существует. Если домен не существует, нажмите кнопку **Создать домен** или **Создать составной домен** , чтобы создать домен. Дополнительные сведения см. в разделе [Create a Domain Rule](../data-quality-services/create-a-domain-rule.md) или [Create a Composite Domain](../data-quality-services/create-a-composite-domain.md).  
    ...
    3.  Щелкните **Просмотр и выбор составных доменов** для отображения составных доменов, которые были определены. Если составные домены не определены, элемент управления будет недоступен.  
  
    4.  Щелкните **Предварительный просмотр источника данных** для отображения во всплывающем окне все данные в источнике данных, выбранном в **таблицу или представление** или **файл Excel** текстовое поле.  
...
6.  Нажмите кнопку **Далее** , чтобы перейти на страницу **Управление значениями домена** мастера обнаружения набора знаний. На этой странице вы можете изменить знания, добавленные в домены базы знаний. Также вы можете выбрать следующие действия.  
    ...
    -   Нажмите кнопку **Закрыть** , чтобы вернуться на домашнюю страницу DQS с сохранением результатов работы. База знаний будет заблокирована вами, а также состояние базы знаний в таблице баз знаний в **Открытие базы знаний** будет отображено **обнаружение — обнаружение**. После нажатия кнопки **Закрыть**для выполнения действий управления доменами потребуется нажать кнопку **Обнаружение знаний** на экране **Открыть базу знаний** , перейти на экран **Управление базами знаний: управление терминами доменов** , нажать кнопку **Готово**, а затем кнопку **Да** , чтобы опубликовать базу знаний, или **Нет** , чтобы сохранить работу в базе знаний и выйти.  
...
1.  На панели **Список доменов** в левой части экрана выберите домен, для которого следует задать значения домена. Для изменения отображаемых значений вы можете выполнить следующие действия.  
    ...
    -   Щелкните **Показать или скрыть панель журнала изменения значений домена** кнопку, чтобы отобразить всплывающее окно предварительного просмотра в нижней части таблицы значений, которое показывает Недавние изменения в домен значения коллекции.  
...
3.  Выберите в поле **Фильтр** значение **Все значения** и убедитесь, что состояния значений допустимы. Чтобы изменить состояние значения, выберите значение и нажмите кнопку **задать выбранные значения домена как исправленные** кнопки (проверка) **задать выбранные значения домена как ошибки** (с крестиком) или **задать выбранные значения домена как недопустимые** (с треугольником).  
  
4.  Чтобы изменить состояние значения, выполните следующие действия.  
  
    1.  **Задать выбранные значения домена как исправленные**: Чтобы изменить состояние значения с ошибочного или недопустимого на исправленное, выберите значение и нажмите кнопку **задать выбранные значения домена как исправленные** (проверить) из направленную вниз стрелку на панели значков или из раскрывающегося списка тип. Если ошибочное или недопустимое значение сгруппировано с правильным значением, удалите это значение после операции.  
  
    2.  **Задать выбранные значения домена как ошибки**: Чтобы изменить состояние значения с верного или недопустимого на ошибки, выберите значение и нажмите кнопку **задать выбранные значения домена как ошибки** значок (между) из направленную вниз стрелку на панели значков или из раскрывающегося списка тип. Вы можете ввести исправление в столбце **Исправить на** или оставить его пустым.  
  
    3.  **Задать выбранные значения домена как недопустимые**: Чтобы изменить состояние значения с верного или ошибочного на недопустимое, выберите значение и нажмите кнопку **задать выбранные значения домена как недопустимые** значок (треугольник) из направленную вниз стрелку на панели значков или из раскрывающегося списка тип. Вы можете ввести исправление в столбце **Исправить на** или оставить его пустым.  
...
10. **Удалить выбранные значения домена**: удалить один или несколько существующих значений из домена, выбирая значения и выбрав **Удалить выбранные значения домена** кнопки. Удалить элемент DQS_NULL нельзя, поэтому, если удаляется несколько значений, одно из которых — элемент DQS_NULL, операция завершится ошибкой.  
...
-   **Недопустимый.** Это значение не принадлежит к домену и не имеет исправления. Например, значение «12345» в домене «Город» является недопустимым. Службы DQS определяют значение как недопустимое, если оно не соответствует правилу домена.  
 ...
 При выполнении интерактивной очистке данных на **Управление и просмотр результатов** Страница **Очистка** действие, недопустимое и ошибочные значения включены в **недопустимый** на вкладке **Управление и просмотр результатов** страницы.  
  
##  <a name="Display"></a> Как отобразить соответствующие значения  
 Вы можете изменять отображаемые сведения следующим образом.  
  
-   **Фильтр** результаты в таблице, на основе их состояния, выбирая состояние в **фильтра** раскрывающегося списка.  
...
-   В **Предварительная обработка записей** этапе DQS загружает данные и индексирует его. Обработка производится по одной записи или по одному пакету, поэтому ход выполнения может отображаться по количеству записей. Во время выполнения этого шага может формироваться большинство данных профилирования, за исключением значений **Допустимых в домене** .  
