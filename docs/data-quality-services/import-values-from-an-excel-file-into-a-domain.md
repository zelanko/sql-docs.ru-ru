---
title: "Импорт значений из файла Excel в домен | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
###  <a name="Prerequisites"></a> Предварительные требования  
 Для импорта доменов из файла Excel, должна быть установлена программа Excel на компьютере, [!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)] приложение устанавливается на, чтобы импортировать значения домена или весь домен, необходимо созданный файл Excel со значениями домена (см. [принцип работы импорта](#How)), и вам необходимо создать и открыть базу знаний, чтобы импортировать домен.  
...
##  <a name="Import"></a> Импорт значений в домен из файла Excel  
...
7.  В **выберите файл** диалоговое окно, перейдите в папку с файлом Excel, который вы хотите импортировать значения домена из файла (с расширением .xlsx, .xls или .csv) и нажмите кнопку **Open**. Файл должен находиться либо на клиентском компьютере, с которого запускается DQS, либо в общей папке, к которой пользователь имеет доступ.  
  
8.  Из **лист** раскрывающегося списка выберите импортируемый из листа.  
...
##  <a name="FollowUp"></a> Дальнейшие действия. После импорта значений из файла Excel в домен  
 После импорта значений в домен вы можете выполнить другие задачи по управлению доменами для этого домена, провести обнаружение набора знаний для добавления его в домен или добавить в домен политику сопоставления. Дополнительные сведения см. в разделе [Обнаружение знаний](../data-quality-services/perform-knowledge-discovery.md), [Управление доменом](../data-quality-services/managing-a-domain.md), или [Создание политики сопоставления](../data-quality-services/create-a-matching-policy.md).  
...
-   Значения в первом столбце импортируются в домен. Если помимо первого столбца, заполняются один или несколько дополнительных столбцов, то значения в этих столбцах будут добавлены как синонимы (см. [Импорт синонимов](#Synonyms)).  
