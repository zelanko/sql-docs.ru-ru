...
5.  В **сопоставления** таблицы, выберите исходный столбец из раскрывающегося списка для **исходный столбец** столбец пустой строки. Если соответствующий домен не существует, щелкните значок **Создать домен** .  
...
1.  В диалоговом окне **Создание домена** введите имя, уникальное в базе знаний, и описание длиной не более 256 символов.  
    ...
    > [!NOTE]  
    >  Дополнительные сведения о свойствах домена см. в разделе [Задание свойств домена](../data-quality-services/set-domain-properties.md).  
  
2.  Из списка **Тип данных** выберите тип данных для значений в домене. Тип данных может быть **Строка** (по умолчанию), **дату**, **целое**, или **десятичное**.  
...
5.  Из **Формат вывода** раскрывающегося списка, выберите формат, который будет применяться при выводе значений данных в домене. Форматирование зависит от типа данных, выбранного в шаге 2, как показано в следующем списке.  
    ...
    -   Для десятичного значения вы можете указать точность и тип маски формата, которую следует применить.  
     ...
     Выбор **Нет** в **Формат вывода** означает раскрывающегося списка будет применяться ни один из форматов в списке.  
  
6.  Если тип данных является **строки**, в **языка** раскрывающемся списке, выберите язык варианта проверки орфографии необходимо применить при включении проверки орфографии.  
...
10. Нажмите кнопку **Готово** , чтобы завершить операцию управления доменами, как описано в разделе [End the Domain Management Activity](../Topic/End%20the%20Domain%20Management%20Activity.md).  
