...
3.  На экране мониторинга активности отображаются сведения о каждом действии в сетке действий. В сетке действий показаны следующие сведения о каждом действии DQS:  
     ...
     **Имя**: имя базы знаний или проекта качества данных, используемого для этого действия.  
  
     **Активна**: указывает, активно ли данное действие. Этот параметр может принимать перечисленные ниже значения.  
  
    -   **Активный**: активность в данный момент выполняется.  
  
    -   **Закончено**: действие завершено.  
  
    -   **Завершено**: действие было прервано с помощью экрана мониторинга активности АДМИНИСТРАТОРом DQS, или действие было отменено пользователем во время его работы в соответствующей функциональной области в [!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)].  
  
     **Тип**: указывает тип действия. **Подтип** указывает конкретный рабочий процесс, выполняемый для типа действия. Отслеживаются следующие типы действий:  
     ...
     **Текущее состояние** может иметь следующие значения:  
  
    -   **Выполняется**: вычислительный процесс выполняется.  
  
    -   **Успешно**. это состояние задается до выполнения вычислительного процесса и снова после успешного завершения вычислительного процесса.  
  
    -   **Сбой**: не удалось выполнить вычислительный процесс.  
  
    -   **Остановлено**: вычислительный процесс был остановлен.  
  
     **DQKB**: имя базы знаний, используемой для действия.  
  
     **Пользователь**: имя пользователя, инициировавшего действие, или последнего пользователя, который работал над действием (в случае, если они не одинаковы).  
