...
# Создание междоменного правила
 ...
 Помимо информирования о допустимости междоменное правило, определяющим *затем* предложения в правиле междоменной **значение равно**, также исправляет данные во время проведения очистки данных. Дополнительные сведения см. в подразделе [Data Correction using Definitive Cross-Domain Rules](../data-quality-services/cleanse-data-in-a-composite-domain.md#CDCorrection) раздела [Cleanse Data in a Composite Domain](../data-quality-services/cleanse-data-in-a-composite-domain.md).  
...
6.  Выберите **Active** чтобы указать, что правило будет выполняться (по умолчанию) и снимите флажок, чтобы предотвратить выполнение правила.  
...
1.  Проверьте междоменное правило следующим образом.  
    ...
    1.  Щелкните **Выполнить выбранное правило домена к тестовым данным** значок в правом верхнем углу панели составного домена.  
  
    2.  В диалоговом окне **Проверка доменного правила** щелкните значок **Добавить новый тестовый терм для доменного правила** .  
  
    3.  Введите тестовые значения для домена, связанного с предложением If, и для домена, связанного с предложением Then. Тестовые значения, введенные в случае предложения должна соответствовать условиям этого предложения или будет введен знак вопроса в **действия** указывающий, что междоменное правило не относится к тестовым данным.  
