...
7.  Если результат всех правил окажется удовлетворительным, нажмите кнопку **Готово** для завершения процесса политики сопоставления, затем нажмите одну из следующих кнопок.  
    ...
    -   **Нет — сохранить работу в базе знаний и выйти**. Работа будет сохранена, база знаний останется заблокированной, а ее состояние изменится на **В работе**. Будут доступны как операция управления доменами, так и операция обнаружения знаний. Выполняется возврат на домашнюю страницу.  
  
    -   **Отмена — остаться на текущем экране**. Всплывающее окно закрывается, и вы вернетесь на экран управления доменами.  
  
8.  Нажмите кнопку **Закрыть** , чтобы сохранить результаты работы и вернуться на домашнюю страницу DQS. Состояние базы знаний отобразит строку "Политика сопоставления — " и текущее состояние. После нажатия **Закрыть** на экране **Результаты сопоставления** отобразится следующее состояние: «Политика сопоставления — результаты». После нажатия "Закрыть" на экране **Политика сопоставления** отобразится следующее состояние: "Политика сопоставления — политика сопоставления". После нажатия кнопки **Закрыть**для выполнения операции **Обнаружение знаний** придется вернуться к операции **Политика сопоставления** . Нажмите кнопку **Готово**, затем либо кнопку **Да** для публикации базы знаний, либо **Нет** , чтобы сохранить работу в базе знаний и выйти.  
...
-   **Записи**: Общее число записей в базы данных-источника  
