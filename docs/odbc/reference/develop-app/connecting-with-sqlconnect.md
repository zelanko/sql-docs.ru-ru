---
title: Подключение с помощью S'LConnect (ru) Документы Майкрософт
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
Самой простой функцией соединения является **S'LConnect.** Он требует имени источника данных и принимает дополнительный идентификатор пользователя и пароль. Он хорошо работает для приложений, которые жестко код имя источника данных и не требуют идентификатора пользователя или пароля. Он также хорошо работает для приложений, которые хотят контролировать свои собственные "смотреть и чувствовать", или которые не имеют пользовательского интерфейса. Такие приложения могут создавать список источников данных с помощью **S'LDataSources,** подсказывать пользователю источник данных, идентификатор пользователя и пароль, а затем вызывать **S'LConnect.**  
  
 Следующий пример подключается к базе данных Northwind, используя DSN под названием Northwind, и извлекает все поля первой и последней фамилии из всех записей в таблице сотрудников.  
