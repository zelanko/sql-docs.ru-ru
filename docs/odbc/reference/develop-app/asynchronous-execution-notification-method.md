...
```  
 ...
 Если Последнее значение SQL_ATTR_ASYNC_DBC_EVENT, установленное для обработчика соединений ODBC, не равно NULL и асинхронный режим приложения, поддерживающий приложение, задается атрибутом SQL_ATTR_ASYNC_DBC_FUNCTION_ENABLE с SQL_ASYNC_DBC_ENABLE_ON, вызов любой функции подключения ODBC, которая поддерживает асинхронный режим, получит уведомление о завершении. Если Последнее значение SQL_ATTR_ASYNC_DBC_EVENT, установленное для обработчика соединений ODBC, имеет значение NULL, ODBC не будет передавать приложение ни одного уведомления, независимо от того, включен ли асинхронный режим.  
