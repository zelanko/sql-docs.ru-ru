...
При использовании привязки на уровне столбца приложение привязывает один или два или в некоторых случаях три массива к каждому столбцу, для которого возвращаются данные. Первый массив содержит значения данных, а второй массив содержит буферы длины или индикатора. Индикаторы и значения длины могут храниться в отдельных буферах путем установки полей SQL_DESC_INDICATOR_PTR и SQL_DESC_OCTET_LENGTH_PTR дескрипторов в различные значения. в этом случае привязывается Третий массив. Каждый массив содержит столько элементов, сколько строк в наборе строк.  
 ...
 Приложение объявляет, что оно использует привязку на уровне столбца с атрибутом оператора SQL_ATTR_ROW_BIND_TYPE, который определяет тип привязки для буферов набора строк в отличие от буферов набора параметров. Драйвер возвращает данные для каждой строки в последовательных элементах каждого массива. На следующем рисунке показано, как работает привязка на уровне столбца.  
  
 ![&#45;столбца привязку для трех столбцов на уровне столбцов](../../../odbc/reference/develop-app/media/pr21.gif "pr21")  
  
 Например, следующий код привязывает 10-элементные массивы к столбцам OrderID, SalesPerson и Status:  
