 ...
 *дататипептр*  
 Проверки Указатель на буфер, в котором возвращается тип данных SQL параметра. Это значение считывается из поля записи SQL_DESC_CONCISE_TYPE в IPD. Это одно из значений в разделе [типы данных SQL](../../../odbc/reference/appendixes/sql-data-types.md) приложения D: Типы данных или тип данных SQL, зависящий от драйвера.  
  
 В ODBC 3. *x*, SQL_TYPE_DATE, SQL_TYPE_TIME или SQL_TYPE_TIMESTAMP будут возвращены в  *\*дататипептр* для данных даты, времени или отметки времени соответственно; в ODBC 2.будут возвращены x, SQL_DATE, SQL_TIME или SQL_TIMESTAMP. Диспетчер драйверов выполняет необходимые сопоставления при использовании ODBC 2. Приложение *x* работает с ODBC 3. драйвер *x* или при использовании ODBC 3. Приложение *x* работает с ODBC 2. драйвер *x* .  
 ...
 *нуллаблептр*  
 Проверки Указатель на буфер, в котором возвращается значение, указывающее, допускает ли параметр значения NULL. Это значение считывается из поля SQL_DESC_NULLABLE в IPD. Это может быть:  
