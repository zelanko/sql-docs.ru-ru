...
-   {*Списка значений атрибута*} является допустимым для соответствующего перечисления фактических значений *ключевое слово атрибут*. Обратите внимание, что фигурные скобки ({}) не указывают на список вариантов, они возвращаются с помощью драйвера. Например он может быть список имен серверов или список имен баз данных.  
