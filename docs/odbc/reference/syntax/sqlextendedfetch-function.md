 ...
 *фетчоффсет*  
 Входной Номер извлекаемой строки. Это то же самое, что и *фетчоффсет* в **SQLFetchScroll**, за одним исключением. Если *фетчориентатион* имеет значение SQL_FETCH_BOOKMARK, *фетчоффсет* является закладкой фиксированной длины, а не смещением от закладки. Иными словами, **SQLExtendedFetch** извлекает закладку из этого аргумента, а не атрибут инструкции SQL_ATTR_FETCH_BOOKMARK_PTR. Он не поддерживает закладки с переменной длиной и не поддерживает выборку набора строк в смещении (отличном от 0) из закладки.  
  
 *RowCountPtr*  
 Проверки Указатель на буфер, в котором возвращается число фактически выбранных строк. Этот буфер используется так же, как и буфер, заданный атрибутом инструкции SQL_ATTR_ROWS_FETCHED_PTR. Этот буфер используется только **SQLExtendedFetch**. Он не используется **SQLFetch** или **SQLFetchScroll**.  
  
 *ровстатусаррай*  
 Проверки Указатель на массив, в котором возвращается состояние каждой строки. Этот массив используется так же, как массив, заданный атрибутом инструкции SQL_ATTR_ROW_STATUS_PTR.  
...
|01S06|Попытка выборки до того, как результирующий набор вернул первый набор строк|Запрошенный набор строк перекрывает начало результирующего набора, когда текущая строка находится за пределами первой строки, а *фетчориентатион* был SQL_PRIOR или *фетчориентатион* был SQL_RELATIVE с отрицательным *фетчоффсет* , чей абсолютное значение меньше или равно текущему SQL_ROWSET_SIZE. (Функция возвращает SQL_SUCCESS_WITH_INFO.)|  
...
|07009|Недопустимый индекс дескриптора|Столбец 0 был связан с **SQLBindCol**, а атрибуту инструкции SQL_ATTR_USE_BOOKMARKS было задано значение SQL_UB_OFF.|  
...
|22002|Требуемая, но не определенная переменная индикатора|Данные NULL были получены в столбец, *StrLen_or_IndPtr* которого задано значением **SQLBindCol** в качестве пустого указателя.<br /><br /> (Функция возвращает SQL_SUCCESS_WITH_INFO.)|  
...
-   **SQLExtendedFetch** и **SQLFetchScroll** используют разные размеры наборов строк. **SQLExtendedFetch** использует значение атрибута SQL_ROWSET_SIZE Statement, а **SQLFetchScroll** использует значение атрибута инструкции SQL_ATTR_ROW_ARRAY_SIZE.  
