...
|ASC_OR_DESC (ODBC 1.0)|10|char(1)|Сортировку последовательности для столбца: «» для по возрастанию; «D», по убыванию; Если порядок сортировки столбцов не поддерживается источником данных, или если тип является SQL_TABLE_STAT, возвращается значение NULL.|  
