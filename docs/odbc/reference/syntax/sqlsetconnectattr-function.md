...
|HY121|Невозможно одновременно включить библиотеку курсоров и пулы с поддержкой драйверов.|Дополнительные сведения см. в статье [Организация пулов соединений с учетом драйверов](../../../odbc/reference/develop-app/driver-aware-connection-pooling.md).|  
...
|SQL_ATTR_DBC_INFO_TOKEN (ODBC 3,8|Значение склпоинтер, используемое для установки токена сведений о соединении в обработчик DBC, если параметр [склратеконнектион](../../../odbc/reference/syntax/sqlrateconnection-function.md)(\**пратинг*) не равен 100.<br /><br /> SQL_ATTR_DBC_INFO_TOKEN только задано. Невозможно использовать **SQLGetConnectAttr** или **SQLGetConnectOption** для получения этого значения. **SQLSetConnectAttr** диспетчера драйверов не будет принимать SQL_ATTR_DBC_INFO_TOKEN, так как приложение не должно устанавливать этот атрибут.<br /><br /> Если драйвер возвращает SQL_ERROR после установки SQL_ATTR_DBC_INFO_TOKEN, подключение, полученное из пула, будет освобождено. Диспетчер драйверов попытается получить другое подключение из пула. Дополнительные сведения см. [в разделе Разработка поддержки пула подключений в драйвере ODBC](../../../odbc/reference/develop-driver/developing-connection-pool-awareness-in-an-odbc-driver.md) .|  
