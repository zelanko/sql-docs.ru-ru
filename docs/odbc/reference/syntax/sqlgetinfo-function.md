...
-   **SQL_ASYNC_NOTIFICATION_NOT_CAPABLE** Уведомление о асинхронном выполнении не поддерживается драйвером.  
 ...
 SQL_CVT_BIGINT (ODBC 1.0) SQL_CVT_BINARY (ODBC 1.0) SQL_CVT_BIT (ODBC 1,0) SQL_CVT_GUID (ODBC 3.5) SQL_CVT_CHAR (ODBC 1,0) SQL_CVT_DATE (ODBC 1.0) SQL_CVT_DECIMAL SQL_CVT_DOUBLE (ODBC 1.0) SQL_CVT_FLOAT (ODBC 1.0) SQL_CVT_INTEGER (ODBC 1.0) SQL _CVT_INTERVAL_YEAR_MONTH (ODBC 3.0) SQL_CVT_INTERVAL_DAY_TIME (ODBC 3.0) SQL_CVT_LONGVARBINARY (ODBC 1.0) SQL_CVT_LONGVARCHAR (ODBC 1.0) SQL_CVT_NUMERIC (ODBC 1.0) SQL_CVT_REAL ODBC 1.0) SQL_CVT_SMALLINT (ODBC 1.0) SQL_CVT_TIME (ODBC 1.0) SQL_CVT_ ОТМЕТКА ВРЕМЕНИ (ODBC 1.0) SQL_CVT_TINYINT (ODBC 1.0) SQL_CVT_VARCHAR SQL_CVT_VARBINARY (ODBC 1.0) (ODBC 1,0)  
 ...
 Следующие биты определяют возможность создания ограничений столбцов: SQL_CDO_DEFAULT = указание ограничений домена поддерживается (промежуточный уровень) SQL_CDO_CONSTRAINT = указание поддерживаемых значений по умолчанию для домена (промежуточный уровень) SQL_CDO_COLLATION = Указание параметров сортировки домена поддерживается (полный уровень)  
 ...
 Символьная строка. "Y", если для источника данных задан режим "только для чтения", в противном случае — "N".  
 ...
 **SQLGetData** требуется для возврата данных только из несвязанных столбцов, которые происходят после последнего связанного столбца, вызываются в порядке возрастания номера столбца и не находятся в строке блока строк.  
 ...
 SQL_FN_NUM_ABS (ODBC 1.0) SQL_FN_NUM_ACOS (ODBC 1.0) SQL_FN_NUM_ASIN (ODBC 1.0) SQL_FN_NUM_ATAN (ODBC 1.0) SQL_FN_NUM_ATAN2 (ODBC 1.0) SQL_FN_NUM_CEILING (ODBC 1.0) SQL_FN_NUM_COS (ODBC 1.0) SQL_FN_NUM_COT (ODBC 2.0) SQL_FN_NUM_DEGREES FN_NUM_EXP (ODBC 1.0) SQL_FN_NUM_FLOOR (ODBC 1.0) SQL_FN_NUM_LOG (ODBC 1.0) SQL_FN_NUM_LOG10 (ODBC 2.0) SQL_FN_NUM_MOD (ODBC 1.0) SQL_FN_NUM_PI (ODBC 2.0) SQL_FN_NUM_POWER SQL_FN_NUM_RADIANS (ODBC 1.0) SQL_FN_NUM_RAND NUM_ROUND (ODBC 2.0) SQL_FN_NUM_SIGN (ODBC 1.0) SQL_FN_NUM_SIN (ODBC 1.0) SQL_FN_NUM_SQRT (ODBC 1.0) SQL_FN_NUM_TAN (ODBC 2,0)  
 ...
 SQL_SG_DELETE_TABLE (начального уровня) SQL_SG_INSERT_COLUMN (промежуточный уровень) SQL_SG_INSERT_TABLE (начального уровня) SQL_SG_REFERENCES_TABLE (уровень ввода) SQL_SG_REFERENCES_COLUMN SQL_SG_SELECT_TABLE (начального уровня) SQL_SG_UPDATE_COLUMN ( Уровень ввода) SQL_SG_UPDATE_TABLE (уровень ввода) SQL_SG_USAGE_ON_DOMAIN (уровень передачи FIPS) SQL_SG_USAGE_ON_CHARACTER_SET SQL_SG_USAGE_ON_COLLATION (уровень перехода FIPS) SQL_SG_USAGE_ON_TRANSLATION (FIPS Уровень перехода) SQL_SG_WITH_GRANT_OPTION (уровень записи)  
 ...
 SQL_SP_BETWEEN (начального уровня) SQL_SP_COMPARISON (начального уровня) SQL_SP_EXISTS (начального уровня) SQL_SP_IN (начального уровня) SQL_SP_ISNOTNULL (начального уровня) SQL_SP_ISNULL (уровень записи) SQL_SP_LIKE (полный уровень) SQL_SP_MATCH_FULL (Полный уровень) SQL_SP_MATCH_UNIQUE_FULL (полный уровень) SQL_SP_MATCH_UNIQUE_PARTIAL (полный уровень) SQL_SP_OVERLAPS (уровень перехода FIPS) SQL_SP_QUANTIFIED_COMPARISON (уровень ввода) SQL_SP_UNIQUE (начального уровня)  
 ...
 SQL_SRJO_CORRESPONDING_CLAUSE (промежуточный уровень) SQL_SRJO_CROSS_JOIN (полный уровень) SQL_SRJO_EXCEPT_JOIN (промежуточный уровень) SQL_SRJO_FULL_OUTER_JOIN (промежуточный уровень FIPS) SQL_SRJO_INNER_JOIN (Промежуточный уровень) SQL_SRJO_LEFT_OUTER_JOIN (уровень переходов FIPS) SQL_SRJO_NATURAL_JOIN (уровень перехода FIPS) SQL_SRJO_RIGHT_OUTER_JOIN SQL_SRJO_UNION_JOIN (полный уровень)  
 ...
 SQL_SR_CASCADE (уровень перехода FIPS) SQL_SR_DELETE_TABLE (уровень ввода) SQL_SR_GRANT_OPTION_FOR (промежуточный уровень) SQL_SR_INSERT_COLUMN (промежуточный уровень) SQL_SR_INSERT_TABLE (начального уровня) SQL_SR_REFERENCES_COLUMN REFERENCES_TABLE (уровень ввода) SQL_SR_RESTRICT (уровень перехода FIPS) SQL_SR_SELECT_TABLE (уровень ввода) SQL_SR_UPDATE_COLUMN (уровень записи) SQL_SR_UPDATE_TABLE (уровень ввода FIPS) SQL_SR_USAGE_ON_DOMAIN CHARACTER_SET (уровень переходов FIPS) SQL_SR_USAGE_ON_COLLATION SQL_SR_USAGE_ON_TRANSLATION (уровень переходов FIPS)  
 ...
 SQL_FN_TD_CURRENT_DATE ODBC 3.0) SQL_FN_TD_CURRENT_TIME (ODBC 3.0) SQL_FN_TD_CURRENT_TIMESTAMP (ODBC 3.0) SQL_FN_TD_CURDATE (ODBC 1.0) SQL_FN_TD_CURTIME (ODBC 1,0) SQL_FN_TD_DAYNAME (ODBC 2.0) SQL_FN_TD_DAYOFMONTH (SQL_FN_TD_DAYOFWEEK ODBC) ( ODBC 1.0) SQL_FN_TD_DAYOFYEAR (ODBC 1,0) SQL_FN_TD_EXTRACT (ODBC 3.0) SQL_FN_TD_HOUR (ODBC 1.0) SQL_FN_TD_MINUTE (ODBC 1.0) SQL_FN_TD_MONTH (ODBC 1.0) SQL_FN_TD_MONTHNAME (ODBC 1.0) SQL_FN_TD_NOW (ODBC 1,0) SQL_FN_TD_QUARTER ВТОРОЕ (ODBC 1.0) SQL_FN_TD_TIMESTAMPADD (ODBC 2.0) SQL_FN_TD_TIMESTAMPDIFF (ODBC 2.0) SQL_FN_TD_WEEK (ODBC 1.0) SQL_FN_TD_YEAR (ODBC 1,0)  
