...
## <a name="scalar-function-information"></a>Сведения о скалярных функциях  
 Следующие значения *InfoType* аргумент возвращать сведения о скалярных функциях, поддерживаемых источником данных и драйвера. Дополнительные сведения о скалярных функциях см. в разделе [скалярные функции в приложении E:](../../../odbc/reference/appendixes/appendix-e-scalar-functions.md).  
...
## <a name="information-type-descriptions"></a>Описания типов сведений  
 ...
 Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать все эти возможности, поддерживаемой.  
  
 SQL_ALTER_DOMAIN (ODBC 3.0)  
 Битовая маска SQLUINTEGER, перечисление предложения в **ALTER домена** инструкции, как определено в SQL-92, поддерживаемых источником данных. Полный SQL-92 уровень совместимый драйвер всегда будет возвращать все битовые маски. Возвращаемое значение «0» означает, что **ALTER домена** инструкция не поддерживается.  
...
-   **SQL_ASYNC_NOTIFICATION_NOT_CAPABLE** асинхронное выполнение уведомлений не поддерживается драйвером.  
 ...
 Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать SQL_CL_START. Если каталоги не поддерживаются источником данных, возвращается значение 0. Чтобы определить, поддерживается ли каталоги, приложение вызывает **SQLGetInfo** с типом SQL_CATALOG_NAME сведения.  
 ...
 Полный SQL-92 уровень — совместимый драйвер всегда возвращает «Y».  
 ...
 Если каталоги не поддерживаются источником данных, возвращается пустая строка. Чтобы определить, поддерживается ли каталоги, приложение вызывает **SQLGetInfo** с типом SQL_CATALOG_NAME сведения. Всегда будет возвращать полный SQL-92 драйвером уровень — совместимые «.».  
 ...
 Если каталоги не поддерживаются источником данных, возвращается пустая строка. Чтобы определить, поддерживается ли каталоги, приложение вызывает **SQLGetInfo** с типом SQL_CATALOG_NAME сведения. Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать «каталог».  
 ...
 SQL_CU_DML_STATEMENTS = каталоги поддерживаются во всех инструкциях языка обработки данных: **ВЫБЕРИТЕ**, **вставить**, **обновление**, **DELETE**и если поддерживается, **ВЫБЕРИТЕ для обновления** и расположен update и delete.  
 ...
 Если каталоги не поддерживаются источником данных, возвращается значение 0. Чтобы определить, поддерживается ли каталоги, приложение вызывает **SQLGetInfo** с типом SQL_CATALOG_NAME сведения. Битовая маска, со всеми этими установленных битов всегда возвращает драйвером SQL-92 полный уровень совместимую.  
  
 Это *InfoType* был переименован для ODBC 3.0 из ODBC 2.0 *InfoType* SQL_QUALIFIER_USAGE.  
  
 SQL_COLLATION_SEQ (ODBC 3.0)  
 Имя параметров сортировки. Это символьная строка, которая указывает имя параметров сортировки по умолчанию для кодировки по умолчанию для этого сервера (например, "ISO 8859-1" и EBCDIC). Если неизвестно, возвращается пустая строка. Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать непустой строкой.  
  
 SQL_COLUMN_ALIAS (ODBC 2.0)  
 Строка символов: «Y», если источник данных поддерживает псевдонимы столбцов; в противном случае — «N».  
  
 Псевдоним является альтернативным именем, которое можно указать для столбца в списке выбора с помощью предложения AS. SQL-92 начального уровня совместимую драйвер всегда вернет «Y».  
 ...
 SQL-92 начального уровня совместимую драйвер всегда вернет SQL_CB_NULL.  
 ...
 SQL-92 начального уровня совместимую драйвер всегда вернет SQL_CN_ANY.  
 ...
 Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать все эти возможности, поддерживаемой. Возвращаемое значение «0» означает, что **создать УТВЕРЖДЕНИЕ** инструкция не поддерживается.  
 ...
 Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать все эти возможности, поддерживаемой. Возвращаемое значение «0» означает, что **создать НАБОР символов** инструкция не поддерживается.  
 ...
 Драйвером SQL-92 полный уровень совместимую всегда будет возвращать этот параметр, так как поддерживается. Возвращаемое значение «0» означает, что **создать параметры СОРТИРОВКИ** инструкция не поддерживается.  
 ...
 SQL-92 промежуточного драйвера уровень совместимую всегда будет возвращать параметры SQL_CS_CREATE_SCHEMA и SQL_CS_AUTHORIZATION, так как поддерживается. Они также должны поддерживаться, на уровне записи SQL-92, но не обязательно, как инструкции SQL. Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать все эти возможности, поддерживаемой.  
 ...
 Драйвером SQL-92 полный уровень совместимую всегда будет возвращать эти параметры, так как поддерживается. Возвращаемое значение «0» означает, что **создать перевод** инструкция не поддерживается.  
 ...
 Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать параметры SQL_CV_CREATE_VIEW и SQL_CV_CHECK_OPTION, так как поддерживается.  
  
 Полный SQL-92 уровень — совместимый драйвер всегда будет возвращать все эти возможности, поддерживаемой.  
 ...
 Драйвером SQL-92 начального уровня совместимую всегда будет возвращать параметр SQL_UNSPECIFIED, так как поддерживается.  
  
 Драйвером SQL-92 полный уровень совместимую всегда будет возвращать параметр SQL_INSENSITIVE, так как поддерживается.  
...
>  В ODBC 3 *.x*, возвращаемое значение для данного *InfoType* также может быть получен путем вызова **SQLGetConnectAttr** с *атрибут* Аргумент SQL_ATTR_CURRENT_CATALOG.  
 ...
 FIPS Transitional драйвер уровень совместимую всегда возвращает значение «1» в Битовая маска для битов в следующем списке. Значение «0» означает, что литералами даты и времени SQL-92 не поддерживаются.  
 ...
 **Количество затронутых фантомных** 1 транзакция считывает набор строк, отвечающих некоторым критериям поиска. Транзакция 2 создает одну или несколько строк (с помощью операции вставки или обновления), соответствующие критериям поиска. Если транзакция 1 reexecutes инструкцию, которая считывает строки, он получает другой набор строк.  
 ...
 Полный SQL-92 драйвер уровень совместимую обычно вернет «Y», так как он будет поддерживать **входные данные, ОПИСЫВАЮЩИЕ** инструкции. Так как это не соответствует непосредственно базовой поддержки SQL, тем не менее, описывающие параметры могут не поддерживаться, даже в полный SQL-92 уровень — совместимый драйвер.  
 ...
 Драйвером SQL-92 полный уровень совместимую всегда будет возвращать этот параметр, так как поддерживается.  
 ...
 SQL-92 промежуточного драйвера уровень совместимую всегда будет возвращать все эти возможности, поддерживаемой.  
 ...
 Драйвером уровень — соответствующие стандарту FIPS Transitional всегда будет возвращать все эти возможности, поддерживаемой.  
 ...
 SQL_CA1_POSITIONED_UPDATE = UPDATE ГДЕ ТЕКУЩЕГО SQL инструкция поддерживается в том случае, когда курсор наведен курсор dynamic. (Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать этот параметр поддерживается.)  
  
 SQL_CA1_POSITIONED_DELETE = удаление ГДЕ ТЕКУЩЕГО SQL инструкция поддерживается в том случае, когда курсор наведен курсор dynamic. (Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать этот параметр поддерживается.)  
  
 SQL_CA1_SELECT_FOR_UPDATE = SELECT для инструкции UPDATE SQL поддерживается в том случае, когда курсор наведен курсор dynamic. (Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать этот параметр поддерживается.)  
 ...
 SQL-92 промежуточного драйвера уровень совместимую обычно возвращает параметры SQL_CA1_NEXT, SQL_CA1_ABSOLUTE и SQL_CA1_RELATIVE поддерживается, так как он поддерживает прокручиваемые курсоры через внедренный оператор SQL FETCH. Так как это не определить непосредственно базовой поддержки SQL, тем не менее, Прокручиваемые курсоры могут не поддерживаться, даже для SQL-92 промежуточного драйвера уровень совместимую.  
 ...
 **SQLGetData** должна возвращать данные только непривязанные столбцы, которые произошли после последнего привязанного столбца, вызываются в порядке возрастания номеров столбцов, а не в строку в блоке строк.  
 ...
 SQL_GB_NO_RELATION = столбцы в **GROUP BY** предложения и список выбора не связаны. Значение nongrouped, неагрегированные столбцы в списке выбора зависит от источника данных. Например **DEPT BY Выбор DEPT, Зарплата из СОТРУДНИКОВ группы, ВОЗРАСТ**. (ODBC 2.0)  
  
 Драйвером SQL-92 начального уровня совместимую всегда будет возвращать параметр SQL_GB_GROUP_BY_EQUALS_SELECT, так как поддерживается. Драйвером SQL-92 полный уровень совместимую всегда будет возвращать параметр SQL_GB_COLLATE, так как поддерживается. Если ни один из вариантов поддерживается, **GROUP BY** предложение не поддерживается источником данных.  
 ...
 Строка символов, содержащий список с разделителями запятыми всех ключевых слов специфического для источника данных. Этот список не содержит ключевые слова, относящиеся к ODBC или ключевые слова, используемые ODBC и источника данных. Этот список представляет все зарезервированные ключевые слова; взаимодействующие приложения, не следует использовать эти слова в именах объектов.  
  
 Список ключевых слов ODBC, см. в разделе [зарезервированные ключевые слова](../../../odbc/reference/appendixes/reserved-keywords.md) в [грамматике SQL в C: приложение](../../../odbc/reference/appendixes/appendix-c-sql-grammar.md). **#Define** значение SQL_ODBC_KEYWORDS содержит разделенный запятыми список ключевых слов ODBC.  
  
 Приложение В. Грамматика SQL  
  
 SQL_LIKE_ESCAPE_CLAUSE (ODBC 2.0)  
 Строка символов: «Y», если источник данных поддерживает escape-символа для символ процента (%) и символ подчеркивания (_) знака в из **как** предиката и драйвер поддерживает синтаксис ODBC при определении **как** предиката escape-символ; «N» в противном случае.  
 ...
 Возвращает по крайней мере 128 драйвером FIPS полный уровень совместимую.  
 ...
 Драйвером уровень — соответствующие стандарту FIPS запись возвратит меньше 18. Возвращает по крайней мере 128 FIPS промежуточного драйвера уровень совместимую.  
 ...
 Возвращает по крайней мере 6 драйвером уровень — соответствующие стандарту FIPS запись. Возвращает по крайней мере 15 FIPS промежуточного уровня — совместимые драйвера.  
 ...
 Возвращает по крайней мере 100 драйвером уровень — соответствующие стандарту FIPS запись. FIPS промежуточного драйвера уровень совместимую вернет менее 250.  
 ...
 Возвращает по крайней мере 2 000 драйвером уровень — соответствующие стандарту FIPS запись. FIPS промежуточного драйвера уровень совместимую вернет по крайней мере 8 000.  
  
 SQL_MAX_ROW_SIZE_INCLUDES_LONG (ODBC 3.0)  
 Строка символов: «Y», если максимальный размер строки, возвращенный для типа данных SQL_MAX_ROW_SIZE включает длину всех столбцов, SQL_LONGVARCHAR и SQL_LONGVARBINARY в строке; «N» в противном случае.  
 ...
 Возвращает по крайней мере 15 драйвером уровень — соответствующие стандарту FIPS запись. Возвращает по крайней мере 50 FIPS промежуточного драйвера уровень совместимую.  
 ...
 Драйвером SQL-92 начального уровня совместимую вернет SQL_NNC_NON_NULL.  
 ...
 Примечание: Тип данных появилась в ODBC 1.0; Каждая маска помечается версии, в котором он был представлен.  
 ...
 SQL_OIC_LEVEL1: Уровня, включая основных функций уровня соответствия стандартам, а также Прокручиваемые курсоры, закладки, расположенный обновляет и удаляет и т.д.  
  
 SQL_OIC_LEVEL2: Уровень включая функций уровня соответствия стандартам уровня 1, а также дополнительные функции, такие как конфиденциальные курсоры; обновлять, удалять и обновлять с закладки; Поддержка хранимых процедур; функции работы с каталогами для первичных и внешних ключей; Поддержка нескольких каталога; и т. д.  
 ...
 Sql_ic_mixed, имеющим = кавычках чувствительны к регистру идентификаторов в SQL и хранятся в смешанном регистре, в системном каталоге. (В базе данных SQL-92 совместимого заключенные в кавычки идентификаторы, всегда с учетом регистра.)  
 ...
 SQL-92 начального уровня совместимую драйвер всегда вернет sql_ic_mixed, имеющим.  
  
 SQL_ROW_UPDATES (ODBC 1.0)  
 Строка символов: «Y», если курсор управляемых набором ключей или смешанный поддерживает версии строк, или значения для всех выбранных строк и таким образом, можно обнаружить все обновления, внесенные в строку любой пользователь с момента последней выборки. (Это относится только к обновлению, удаления или вставки). Драйвер может возвращать SQL_ROW_UPDATED флаг состояния строк массива, когда **SQLFetchScroll** вызывается. В противном случае — «N».  
 ...
 Драйвером SQL-92 начального уровня совместимую всегда возвращает «schema».  
 ...
 SQL_SU_DML_STATEMENTS = схемы поддерживаются во всех инструкциях языка обработки данных: **ВЫБЕРИТЕ**, **вставить**, **обновление**, **DELETE**и если поддерживается, **ВЫБЕРИТЕ для обновления** и расположен update и delete.  
 ...
 Драйвером SQL-92 начального уровня совместимую всегда будет возвращать параметры SQL_SU_DML_STATEMENTS, SQL_SU_TABLE_DEFINITION и SQL_SU_PRIVILEGE_DEFINITION, так как поддерживается.  
 ...
 Строка символов, с именем сервера специфического для источника данных; полезно, когда имя источника данных используется во время **SQLConnect**, **SQLDriverConnect**, и **SQLBrowseConnect**.  
 ...
 Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать Битовая маска, в котором все биты равны.  
 ...
 Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать «table».  
 ...
 Драйвером уровень — соответствующие стандарту FIPS Transitional всегда будет возвращать Битовая маска, в котором все биты равны.  
 ...
 Примечание: Тип данных появилась в ODBC 1.0; Каждое возвращаемое значение меткой с версией, в которую было включено.  
 ...
 Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать SQL_TXN_SERIALIZABLE, так как поддерживается. Переходные FIPS драйвер уровень совместимую всегда будет возвращать все эти возможности, поддерживаемой.  
 ...
 Драйвером уровень — соответствующие стандарту SQL-92 запись всегда будет возвращать оба этих варианта, поддерживаемой.  
 ...
 SQL_ALTER_DOMAIN (ODBC 3.0)  
 Битовая маска SQLUINTEGER, перечисление предложения в **ALTER домена** инструкции, как определено в SQL-92, поддерживаемых источником данных. Полный SQL-92 уровень совместимый драйвер всегда будет возвращать все битовой маски. Возвращаемое значение «0» означает, что **ALTER домена** инструкция не поддерживается.  
