...
|TABLE_TYPE (ODBC 1.0)|4|Varchar|Имя типа таблицы; одно из следующих: «Таблица», «ПРЕДСТАВЛЕНИЕ», «СИСТЕМНАЯ таблица», «Глобальный ВРЕМЕННЫЙ», «ЛОКАЛЬНЫЙ ВРЕМЕННЫЙ», «ALIAS», «СИНОНИМ» или имя типа зависящие от источника данных.<br /><br /> Значение «ALIAS» и «СИНОНИМ» зависят от драйвера.|  
