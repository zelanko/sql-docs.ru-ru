 ...
 **SQLGetEnvAttr** можно вызывать в любое время между выделением и освобождением обработчика среды. Все атрибуты среды, успешно заданные приложением для среды, сохраняются до тех пор, пока не будет вызван **SQLFreeHandle** в *ЕНВИРОНМЕНСАНДЛЕ* с *параметром handletype* SQL_HANDLE_ENV. В ODBC 3 *. x*можно одновременно выделить более одного маркера среды. При выделении другой среды атрибут среды в одной среде не затрагивается.  
  
> [!NOTE]
>  Атрибут среды SQL_ATTR_OUTPUT_NTS поддерживается приложениями, совместимыми со стандартами. При вызове **SQLGetEnvAttr** диспетчер драйверов ODBC 3 *. x* всегда возвращает SQL_TRUE для этого атрибута. SQL_ATTR_OUTPUT_NTS может иметь значение SQL_TRUE только при вызове **SQLSetEnvAttr**.  
