...
|*После End и Фетчоффсет >= 0*|*После окончания*|  
 ...
 [1] ***SQLFetchScroll*** возвращает тот же набор строк, что и, если он был вызван с фетчориентатион, для которого задано значение SQL_FETCH_ABSOLUTE. Дополнительные сведения см. в разделе "SQL_FETCH_ABSOLUTE".  
  
 [2] **SQLFetchScroll** ВОЗВРАЩАЕТ значение SQLSTATE 01S06 (попытка выборки до того, как результирующий набор вернул первый набор строк) и SQL_SUCCESS_WITH_INFO.  
