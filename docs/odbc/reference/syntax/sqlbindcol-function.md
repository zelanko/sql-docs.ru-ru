 ...
 *TargetType*  
 ...
 Можно также указать расширенного типа данных C. Дополнительные сведения см. в разделе [типы данных C в ODBC](../../../odbc/reference/develop-app/c-data-types-in-odbc.md).  
...
## <a name="binding-columns"></a>Привязка столбцов  
 ...
 Использование этих буферов откладывается; то есть приложение выполняет привязку их в **SQLBindCol** , но драйвер получает к ним доступ из других функций, а именно **SQLBulkOperations**, **SQLFetch**,  **SQLFetchScroll**, или **SQLSetPos**. Приложения должны убедиться, что указатели, указанные в **SQLBindCol** остаются действительными до тех пор, пока привязка остается в силе. Если в приложении имеется эти указатели недействительными — например, он освобождает буфер, и затем вызывает функцию, ожидающую их была допустимой, последствия не определены. Дополнительные сведения см. в разделе [отложенного буферы](../../../odbc/reference/develop-app/deferred-buffers.md).  
