---
title: Пример кода библиотеки Cursor (ru) Документы Майкрософт
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
>  Эта функция будет удалена в будущей версии Windows. Избегайте использования этой функции в новых разработках и планируйте модифицировать приложения, использующие эту функцию в настоящее время. Корпорация Майкрософт рекомендует использовать функцию курсора драйвера.  
 ...
 В следующем примере используется библиотека курсора для получения идентификатора каждого заказа, даты открытия и статуса из таблицы ORDERS. Затем отображается 20 строк данных. Если пользователь обновляет эти данные, код обновляет буферы набора строк и выполняет позиционированное заявление обновления. Наконец, он подсказывает пользователю направление для прокрутки и повторяет процесс.  
