---
title: "Диагностическое соединение для администраторов баз данных | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "10/16/2015"
...
ms.topic: "article"
helpviewer_keywords: 
  - "управление сервером [SQL Server], подключения"
  - "соединения администратора [SQL Server]"
  - "порты [SQL Server], DAC"
  - "DAC"
  - "сетевые подключения [SQL Server], выделенный администратор"
  - "диагностические соединения [SQL Server]"
...
## Соединение с помощью выделенного административного соединения  
 ...
 Выделенное административное соединение доступно и поддерживается через программу командной строки **sqlcmd** со специальным административным параметром (**-A**). Дополнительные сведения об использовании **sqlcmd** см. в разделе [Использование программы sqlcmd с переменными скрипта](../../relational-databases/scripting/use-sqlcmd-with-scripting-variables.md). Можно также подключиться, добавив префикс **admin:** к имени экземпляра следующим образом: **sqlcmd -Sadmin:***\<имя_экземпляра>.* Выделенное административное соединение можно также инициировать через редактор запросов среды [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], подключившись к **admin:**\<*имя_экземпляра*>.  
...
## См. также  
 ...
 [Флаги трассировки (Transact-SQL)](../Topic/Trace%20Flags%20\(Transact-SQL\).md)  
