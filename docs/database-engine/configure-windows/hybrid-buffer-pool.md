...
В традиционной системе без постоянной памяти SQL Server кэширует страницы данных в буферном пуле. Благодаря гибридному буферному пулу SQL Server пропускает копирование страницы в часть буферного пула на основе DRAM и вместо этого обращается к странице напрямую в файле базы данных, хранящемся на PMEM-устройстве. Доступ к файлам данных на PMEM-устройствах для гибридного буферного пула выполняется с использованием режима MMIO. Это также называется *компонентом паравиртуализации* для файлов данных в SQL Server.

На PMEM-устройстве напрямую можно обратиться только к "чистым" страницам. Когда страница помечается как "грязная", она копируется в буферный пул DRAM, а затем записывается обратно на PMEM-устройство и снова помечается как "чистая". Это происходит во время периодических операций контрольной точки.

Функция гибридного буферного пула доступна в Windows и Linux. PMEM-устройства должны быть отформатированы с использованием файловой системы, поддерживающей DAX (DirectAccess). DAX поддерживают файловые системы XFS, EXT4, NTFS и ReFS. SQL Server автоматически определит, что файлы данных находятся на соответствующим образом отформатированном PMEM-устройстве, и выполнит сопоставление памяти в пространстве пользователя во время запуска при подключении, восстановлении либо создании базы данных или при включении функции гибридного буферного пула.

Дополнительные сведения о поддержке PMEM в Windows Server, что также называют памятью класса хранилища (SCM), см. в описании [развертывания постоянной памяти в Windows Server](/windows-server/storage/storage-spaces/deploy-pmem/).
...
По умолчанию гибридный буферный пул отключен на уровне экземпляра.
...
При форматировании PMEM-устройства в Windows используйте максимальный размер единицы размещения, доступный для файловой системы NTFS или ReFS (2 МБ в Windows Server 2019), и отформатируйте устройство для DAX (Direct Access).
