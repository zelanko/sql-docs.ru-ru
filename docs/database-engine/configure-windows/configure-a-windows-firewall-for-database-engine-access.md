...
description: Узнайте, как настроить брандмауэр Windows для доступа клиентских компьютеров к экземпляру ядра СУБД SQL Server.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
4.  В диалоговом окне **Протокол и порты** выберите протокол **TCP**. Выберите пункт **Определенные локальные порты**и введите номер порта для экземпляра компонента [!INCLUDE[ssDE](../../includes/ssde-md.md)], например **1433** для экземпляра по умолчанию. Щелкните **Далее**.  
