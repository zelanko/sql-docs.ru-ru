---
title: "Настройка учетных записей службы Windows и разрешений | Microsoft Docs"
ms.custom: ""
ms.date: "03/13/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "типы запуска служб [SQL Server]"
  - "Программа установки [SQL Server], учетные записи пользователей"
  - "разрешения Windows [SQL Server]"
  - "изменение учетных записей пользователей"
  - "учетные записи по умолчанию"
  - "домены [SQL Server], учетные записи пользователей"
  - "стартовые учетные записи [SQL Server]"
  - "системные учетные записи [SQL Server]"
  - "службы [SQL Server], разрешения"
  - "список управления доступом (ACL)"
  - "локальные системные учетные записи [SQL Server]"
  - "службы, привязанные к экземпляру [SQL Server]"
  - "разрешения [SQL Server], службы"
  - "Служба агента SQL Server, учетные записи пользователей"
  - "разрешения Windows NT [SQL Server]"
  - "учетные записи пользователей [SQL Server]"
  - "идентификация служб, не привязанных к экземпляру [SQL Server]"
  - "Установка SQL Server, учетные записи пользователей"
  - "тип запуска «отключено» [SQL Server]"
  - "учетные записи пользователей [SQL Server], пользователи"
  - "Local Service, учетная запись [SQL Server]"
  - "мастер установки SQL Server"
  - "службы, не привязанные к экземпляру [SQL Server]"
  - "службы [SQL Server], настройка при установке"
  - "учетные записи Windows [SQL Server]"
  - "Службы SQL Server, учетные записи пользователей"
  - "учетные записи пользователей [SQL Server], службы"
  - "MSSQLServer"
  - "идентификация служб, привязанных к экземпляру [SQL Server]"
  - "службы [SQL Server] учетные записи"
  - "списки управления доступом"
  - "дополнительные учетные записи [SQL Server]"
  - "учетные записи служб [SQL Server]"
  - "учетные записи [SQL Server], службы"
  - "встроенные системные учетные записи [SQL Server]"
  - "тип запуска «авто»"
  - "домены [SQL Server]"
  - "тип запуска «вручную» [SQL Server]"
  - "учетные записи [SQL Server], пользователи"
...
##  <a name="a-nameservicedetailsa-services-installed-by-includessnoversiontokenssnoversionmdmd"></a><a name="Service_Details"></a> Службы, устанавливаемые с [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
...
-   **Службы [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Database Services** — служба реляционного компонента [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] в [!INCLUDE[ssDE](../../includes/ssde-md.md)]. Путь к исполняемому файлу: \<MSSQLPATH>\MSSQL\Binn\sqlservr.exe.  
...
-   Службы **[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]** — предоставляют средства аналитической обработки в сети (OLAP) и средства интеллектуального анализа данных для приложений бизнес-аналитики. Путь к исполняемому файлу: \<MSSQLPATH>\OLAP\Bin\msmdsrv.exe.  
  
-   Службы **[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]** — предназначены для создания, выполнения, создания, планирования, доставки отчетов и управления ими. Путь к исполняемому файлу: \<MSSQLPATH>\Reporting Services\ReportServer\Bin\ReportingServicesService.exe.  
  
-   Службы **[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]** — обеспечивают поддержку управления хранением и выполнением пакетов служб [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]. Путь к исполняемому файлу: \<MSSQLPATH>\130\DTS\Binn\MsDtsSrvr.exe  
...
##  <a name="a-nameservpropa-service-properties-and-configuration"></a><a name="Serv_Prop"></a> Свойства и настройка служб  
...
###  <a name="a-namedefaultacctsa-default-service-accounts"></a><a name="Default_Accts"></a> Учетные записи служб по умолчанию  
...
####  <a name="a-namechangingaccountsa-changing-account-properties"></a><a name="Changing_Accounts"></a> Изменение свойств учетной записи  
...
###  <a name="a-namenewaccountsa-managed-service-accounts-group-managed-service-accounts-and-virtual-accounts"></a><a name="New_Accounts"></a> Управляемые учетные записи служб, групповые управляемые учетные записи служб и виртуальные учетные записи  
...
-   <a name="VA_Desc"></a>**Виртуальные учетные записи**  
    ...
    |Служба агента [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] на экземпляре [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] с именем **PAYROLL**|**NT SERVICE\SQLAGENT$PAYROLL**|  
...
###  <a name="a-nameautostarta-automatic-startup"></a><a name="Auto_Start"></a> Автоматический запуск  
...
###  <a name="a-nameconfigureservicesa-configuring-services-during-unattended-installation"></a><a name="Configure_services"></a> Настройка служб во время автоматической установки  
...
###  <a name="a-namefirewalla-firewall-port"></a><a name="Firewall"></a> Порт брандмауэра  
...
##  <a name="a-nameservperma-service-permissions"></a><a name="Serv_Perm"></a> Разрешения службы  
...
###  <a name="a-nameservsida-service-configuration-and-access-control"></a><a name="Serv_SID"></a> Настройка служб и управление доступом  
...
###  <a name="a-namewindowsa-windows-privileges-and-rights"></a><a name="Windows"></a> Права доступа и права Windows  
...
|**[!INCLUDE[ssRS](../../includes/ssrs-md.md)]:**<br /><br /> (Все права предоставляются удостоверению безопасности службы. Экземпляр по умолчанию: **NT SERVICE\ReportServer**. Именованный экземпляр: **NT SERVICE\$\\$***имя_экземпляра*.)|**Вход в систему в качестве службы** (SeServiceLogonRight)|  
...
###  <a name="a-namereviewingaclsa-file-system-permissions-granted-to-sql-server-per-service-sids-or-local-windows-groups"></a><a name="Reviewing_ACLs"></a> Разрешения файловой системы, предоставляемые для SQL Server согласно SID-идентификаторам служб или локальным группам Windows  
...
###  <a name="a-namefilesystemothera-file-system-permissions-granted-to-other-windows-user-accounts-or-groups"></a><a name="File_System_Other"></a> Разрешения файловой системы, предоставляемые другим учетным записям или группам Windows  
...
||Удостоверение пула приложений диспетчера отчетов|*\<папка установки>*\Reporting Services\ReportManager\Pages\\*.\*|Чтение|  
...
||Сетевая служба|*\<установка>*\Reporting Services\ReportServer\ReportService.asmx|Полный|  
...
###  <a name="a-nameunusuallocationsa-file-system-permissions-related-to-unusual-disk-locations"></a><a name="Unusual_Locations"></a> Разрешения файловой системы, связанные с нестандартными дисковыми расположениями  
...
###  <a name="a-namereviewadditionalconsiderationsa-reviewing-additional-considerations"></a><a name="Review_additional_considerations"></a> Обзор дополнительных вопросов  
...
###  <a name="a-nameregistrya-registry-permissions"></a><a name="Registry"></a> Разрешения для реестра  
...
###  <a name="a-namepipesa-named-pipes"></a><a name="Pipes"></a> Именованные каналы  
...
##  <a name="a-nameprovisioninga-provisioning"></a><a name="Provisioning"></a> Подготовка  
...
###  <a name="a-namedeprova-database-engine-provisioning"></a><a name="DE_Prov"></a> Подготовка ядра СУБД к работе  
...
####  <a name="a-namewinprincipalsa-windows-principals"></a><a name="Win_Principals"></a> Участники Windows  
...
####  <a name="a-namesaa-sa-account"></a><a name="sa"></a> Учетная запись sa  
...
####  <a name="a-nameloginsa-sql-server-per-service-sid-login-and-privileges"></a><a name="Logins"></a> Вход на SQL Server и права доступа для удостоверений безопасности служб  
...
####  <a name="a-nameagenta-sql-server-agent-login-and-privileges"></a><a name="Agent"></a> Вход и права доступа агента SQL Server  
...
####  <a name="a-namehadrona-includesshadrctokensshadrcmdmd-and-sql-failover-cluster-instance-and-privileges"></a><a name="Hadron"></a> Экземпляр и права доступа [!INCLUDE[ssHADRc](../../includes/sshadrc-md.md)] и отказоустойчивого кластера SQL  
...
####  <a name="a-namewritera-sql-writer-and-privileges"></a><a name="Writer"></a> Модуль записи SQL и права доступа  
...
####  <a name="a-namesqlwmia-sql-wmi-and-privileges"></a><a name="SQLWMI"></a> SQL WMI и права доступа  
...
###  <a name="a-namessasa-ssas-provisioning"></a><a name="SSAS"></a> Подготовка служб SSAS  
...
###  <a name="a-namessrsa-ssrs-provisioning"></a><a name="SSRS"></a> Подготовка служб SSRS  
...
##  <a name="a-nameupgradea-upgrading-from-previous-versions"></a><a name="Upgrade"></a> Обновление с предыдущих версий  
...
##  <a name="a-nameappendixa-appendix"></a><a name="Appendix"></a> Приложение  
...
###  <a name="a-nameservacctsa-description-of-service-accounts"></a><a name="Serv_Accts"></a> Описание учетных записей служб  
...
####  <a name="a-nameanyosa-accounts-available-with-any-operating-system"></a><a name="Any_OS"></a> Учетные записи, доступные в любой операционной системе  
...
###  <a name="a-nameidentifyinstanceawareandunawarea-identifying-instance-aware-and-instance-unaware-services"></a><a name="Identify_instance_aware_and_unaware"></a> Идентификация служб, не привязанных к экземпляру  
...
###  <a name="a-namelocalizedservicenamesa-localized-service-names"></a><a name="Localized_service_names"></a> Локализованные имена служб  
