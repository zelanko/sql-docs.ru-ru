...
description: Изучите различные способы входа в экземпляр SQL Server. Узнайте, какой формат следует использовать для имени сервера в различных средах.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
|Соединение с экземпляром по умолчанию, который не прослушивает TCP-порт по умолчанию, с указанием используемого порта, в данном случае 2828. (Указывать порт необязательно, если компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] ожидает передачи данных на порте по умолчанию, т. е. 1433.)|APPHOST, 2828|  
|Соединение с именованным экземпляром на назначенный TCP порт, в данном случае 2828. (Указывать порт часто необходимо, если служба браузера [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] не запущена на компьютере узла.)|APPHOST, 2828|  
...
## <a name="related-content"></a>См. также  
 ...
 [Создание имени входа](../../t-sql/lesson-2-configuring-permissions-on-database-objects.md)
