...
Создайте папку (каталог локального аудита), куда функция локального аудита будет записывать журналы. Например, полный путь к каталогу локального аудита для экземпляра ядра СУБД по умолчанию будет иметь следующий вид: *C:\\SQLCEIPAudit\\MSSQLSERVER\\DB\\*. 
