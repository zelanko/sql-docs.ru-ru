...
-  Если говорить о производительности, то, поскольку с каждой новой версией в оптимизатор запросов вводятся усовершенствования, в разных версиях [!INCLUDE[ssde_md](../../includes/ssde_md.md)] можно было бы ожидать различий в планах запросов. Различия в планах запросов при обновлении обычно приводят к возникновению риска, если некоторые изменения могут негативно сказаться на определенном запросе или рабочей нагрузке. В свою очередь, этот риск является побуждающей причиной для повторной сертификации, из-за которой могут откладываться обновления и возникать проблемы с жизненным циклом и поддержкой. 
   Именно в целях устранения рисков, возникающих при обновлении, улучшения оптимизатора запросов ограничиваются уровнем совместимости по умолчанию нового выпуска. Сертификация на совместимость включает в себя **защиту формы плана запроса**: сохранение уровня совместимости базы данных сразу после обновления [!INCLUDE[ssde_md](../../includes/ssde_md.md)] означает, что модель оптимизации запросов, используемая для создания планов запросов в новой версии, остается такой же, как и до обновления. Дополнительные сведения о защите формы плана запроса см. в разделе [Использование уровня совместимости для обеспечения обратной совместимости](../../t-sql/statements/alter-database-transact-sql-compatibility-level.md#using-compatibility-level-for-backward-compatibility).
   
Если приложению не нужно использовать улучшения, доступные только на более высоких уровнях совместимости, допускается обновлять [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] с сохранением прежнего уровня совместимости без необходимости повторно сертифицировать приложение. Дополнительные сведения см. в разделе [Уровни совместимости и обновления ядра СУБД](#compatibility-levels-and-database-engine-upgrades) далее в этой статье.
...
-  **Обновление без изменения кода**. Если при обновлении до новой версии [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] или [!INCLUDE[ssazure_md](../../includes/ssazure_md.md)] сохранить уровень совместимости исходной системы, то изменения кода не потребуются. Кроме того, не нужно будет выполнять повторную сертификацию до тех пор, пока приложению не потребуется использовать усовершенствования, доступные только на более высоких уровнях совместимости базы данных.
