---
title: "Переключение ролей во время сеанса зеркального отображения базы данных (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  ...
  - "зеркально отображение участников [SQL Server]"
  - "отработка отказа [SQL Server]"
  - "партнеры по обеспечению отработки отказа [SQL Server]"
  - "зеркальное отображение базы данных [SQL Server], участники"
  - "участники сеансов зеркального отображения базы данных [SQL Server]"
...
|Принудительное обслуживание|Да|Да|Нет|  
 ...
 При переключении ролей время недоступности зеркального отображения базы данных зависит от типа переключения ролей и типа причины переключения. Дополнительные сведения см. в разделе [Оценка прерывания обслуживания во время переключения ролей (зеркальное отображение базы данных)](../../database-engine/database-mirroring/estimate-the-interruption-of-service-during-role-switching-database-mirroring.md).  
...
6.  Бывший основной сервер берет на себя роль зеркального сервера, а бывшая основная база данных становится зеркальной базой данных. Новый зеркальный сервер проводит быструю повторную синхронизацию новой зеркальной базы данных с новой основной базой данных.  
 ...
 После отработки отказа клиенты должны повторно подключиться к текущей основной базе данных. Дополнительные сведения см. в разделе [Подключение клиентов к сеансу зеркального отображения базы данных (SQL Server)](../../database-engine/database-mirroring/connect-clients-to-a-database-mirroring-session-sql-server.md).  
...
##  <a name="ForcedService"></a> Принудительное обслуживание (с вероятностью потери данных)  
