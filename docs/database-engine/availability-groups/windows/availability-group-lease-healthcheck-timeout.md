...
Аренда в основном является механизмом синхронизации между основным экземпляром и кластером, но она также может создавать условия сбоя тогда, когда переход на другой ресурс по другим причинам не требовался. Например, высокая загрузка ЦП, нехватка памяти, процесс SQL, не реагирующий при создании дампа памяти, зависание всей системы или нагрузка tempdb могут привести к истощению ресурсов рабочего потока аренды, что будет препятствовать продлению аренды в экземпляре SQL и приведет к отработке отказа. 
