---
title: Просмотреть свойства объекта, методы и события | Документация Майкрософт
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- View object [ADOX], members
ms.assetid: 02b5ba88-cacd-4a68-881b-974824ea4a04
author: MightyPen
ms.author: genemi
manager: jroth
ms.openlocfilehash: 7c527c33ac7f8493f7c0d506a10f870975b10aa1
ms.sourcegitcommit: 3026c22b7fba19059a769ea5f367c4f51efaf286
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/15/2019
ms.locfileid: "66697087"
---
# <a name="view-object-properties-methods-and-events"></a>Свойства, методы и события объекта View
## <a name="properties"></a>Свойства  
 [Свойство Command](../../../ado/reference/adox-api/command-property-adox.md)  
  
 [Свойство DateCreated](../../../ado/reference/adox-api/datecreated-property-adox.md)  
  
 [Свойство DateModified](../../../ado/reference/adox-api/datemodified-property-adox.md)  
  
 [Свойство Name](../../../ado/reference/adox-api/name-property-adox.md)  
  
## <a name="methods"></a>Методы  
 Нет.  
  
## <a name="events"></a>События  
 Нет.  
  
## <a name="see-also"></a>См. также  
 [Объект View (ADOX)](../../../ado/reference/adox-api/view-object-adox.md)
