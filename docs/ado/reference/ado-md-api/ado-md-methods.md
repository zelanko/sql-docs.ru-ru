---
title: Методы многомерных Объектов ADO | Документация Майкрософт
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- ADO MD, methods
- methods [ADO MD]
ms.assetid: 78bfa2f0-358b-40bb-be2e-16262752d676
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 669e1bdc70aa0c5bd5510db8c9fbd86e26b22b41
ms.sourcegitcommit: 61381ef939415fe019285def9450d7583df1fed0
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/01/2018
ms.locfileid: "47770852"
---
# <a name="ado-md-methods"></a>Методы многомерных объектов ADO
|||  
|-|-|  
|[Закрыть](../../../ado/reference/ado-md-api/close-method-ado-md.md)|Закрывает откройте набор ячеек.|  
|[GetSchemaObject](../../../ado/reference/ado-md-api/getschemaobject-method-ado-md.md)|Извлекает объект схемы ADO MD (измерения, иерархии, уровня или элемента) по уникальному имени.|  
|[Открытие](../../../ado/reference/ado-md-api/open-method-ado-md.md)|Извлекает результаты многомерного запроса и возвращает результаты в наборе ячеек.|  
  
## <a name="see-also"></a>См. также  
 [Справочник по API ADO MD](../../../ado/reference/ado-md-api/ado-md-api-reference.md)   
 [Примеры кода многомерных Объектов ADO](../../../ado/reference/ado-md-api/ado-md-code-examples.md)   
 [Коллекции многомерных Объектов ADO](../../../ado/reference/ado-md-api/ado-md-collections.md)   
 [Перечисляемые константы многомерных Объектов ADO](../../../ado/reference/ado-md-api/ado-md-enumerated-constants.md)   
 [Объектная модель ADO MD](../../../ado/reference/ado-md-api/ado-md-object-model.md)   
 [Многомерные объекты ADO](../../../ado/reference/ado-md-api/ado-md-objects.md)   
 [Свойства многомерных объектов ADO](../../../ado/reference/ado-md-api/ado-md-properties.md)
