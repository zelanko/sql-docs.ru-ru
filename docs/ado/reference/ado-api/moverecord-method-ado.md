 ...
 *Параметры*  
 Необязательный. Объект [MoveRecordOptionsEnum](../../../ado/reference/ado-api/moverecordoptionsenum.md) значение, значение которого по умолчанию равно **adMoveUnspecified**. Задает поведение этого метода.  
  
 *Async*  
 Необязательный параметр. Объект **логическое** значением, которое при **True**, указывает, эта операция должна выполняться асинхронно.  
