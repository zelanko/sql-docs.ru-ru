---
title: Объект свойства, методы и события | Документация Майкрософт
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- Property object [ADO], members
ms.assetid: 21f67a7d-6273-4648-9abd-2236650efa4c
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: f23c35833c121214c9d51d89f3ba8c2165b346c3
ms.sourcegitcommit: f7fced330b64d6616aeb8766747295807c92dd41
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/23/2019
ms.locfileid: "63027718"
---
# <a name="property-object-properties-methods-and-events"></a>Объект свойства, методы и события
## <a name="properties"></a>Свойства  
 [Атрибуты свойства](../../../ado/reference/ado-api/attributes-property-ado.md)  
  
 [Свойство Name](../../../ado/reference/ado-api/name-property-ado.md)  
  
 [Свойство Type](../../../ado/reference/ado-api/type-property-ado.md)  
  
 [Значение свойства](../../../ado/reference/ado-api/value-property-ado.md)  
  
## <a name="methods"></a>Методы  
 Нет.  
  
## <a name="events"></a>События  
 Нет.  
  
## <a name="see-also"></a>См. также  
 [Объект Property (ADO)](../../../ado/reference/ado-api/property-object-ado.md)
