 ...
 *SkipRows*  
 Необязательный параметр. Объект **Long** значение, значение по умолчанию которого равно нулю, которое указывает смещение строки из текущей строки или *запустить* закладки, чтобы начать поиск. По умолчанию на текущей строке начнется поиск.  
  
 *SearchDirection*  
 Необязательный параметр. Объект [SearchDirectionEnum](../../../ado/reference/ado-api/searchdirectionenum.md) значение, указывающее начало поиска в текущей строке или следующую доступную строку в направлении поиска. Выполняется поиск неудачных останавливается в конце **записей** Если значение равно **adSearchForward**. Останавливает неудачного поиска в начале **записей** Если значение равно **adSearchBackward**.  
