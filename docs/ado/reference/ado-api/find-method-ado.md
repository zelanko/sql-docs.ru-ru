 ...
 *скипровс*  
 Необязательный параметр. Значение **типа Long** , значение по умолчанию которого равно нулю, которое указывает смещение строки от текущей строки или *открывающую* закладку для начала поиска. По умолчанию поиск начнется в текущей строке.  
