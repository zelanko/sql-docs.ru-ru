---
title: Пример (VC ++) модели событий ADO | Документы Microsoft
ms.prod: sql
...
В Visual C++ части [экземпляр события ADO языком](../../../ado/guide/data/ado-event-instantiation-by-language.md) предоставляет общее описание способов создания экземпляра модели событий ADO. Ниже приведен конкретный пример создания экземпляра модели событий в среде, созданные **#import** директивы.  
 ...
 Общее описание использует **adoint.h** как ссылку для сигнатуры метода. Тем не менее, некоторые сведения в описании общие немного измениться в результате использования **#import** директиву:  
  
-   **#Import** разрешает директива **typedef**элемента, типы данных сигнатуру метода и модификаторы в основные формы.  
  
-   Чистые виртуальные методы, которые должны быть перезаписаны начинаются с «**raw_**».  
  
 Некоторые части кода просто отражает стиль написания кода.  
  
-   Указатель на **IUnknown** используемые **Advise** метод явным образом получается с помощью вызова **QueryInterface**.  
  
-   Не нужно явно создавать код деструктор в определения классов.  
  
-   Требуется код более надежной реализации QueryInterface, AddRef и Release.  
  
-   **__Uuidof()** директива широко используется для получения идентификаторов интерфейсов.  
  
 Наконец в примере показаны некоторые рабочего кода.  
  
-   Пример представляет собой консольное приложение.  
  
-   Необходимо вставить код в комментарий, «`// Do some work`».  
  
-   Все события обработчики по умолчанию не выполняет никаких действий и Отмена дополнительные уведомления. Вставьте соответствующий код для приложения и разрешить уведомлений, если это необходимо.  
