...
-   Положительное десятичное число — низкий два байта полного числа в десятичном формате. Это число отображается в Visual Basic по умолчанию появления сообщения об ошибке. Например, ошибка времени выполнения "3707".  
  
-   Отрицательное десятичной — десятичное преобразование полный номер.  
  
-   Шестнадцатеричное — шестнадцатеричное представление числа ошибку переполнения. Код устройства Windows находится в четвертом цифра. Код устройства для номера ошибок ADO — *объект*. Например: 0x800***объект***0E7B.  
