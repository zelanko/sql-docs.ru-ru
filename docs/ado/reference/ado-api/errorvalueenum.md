---
title: ErrorValueEnum | Документация Майкрософт
ms.prod: sql
...
ms.locfileid: "52542602"
---
# <a name="errorvalueenum"></a>ErrorValueEnum
Указывает тип ошибки времени выполнения ADO.  
  
 Перечислены три формы номер ошибки.  
  
-   Положительное десятичной низкой два байта полного числа в десятичном формате. Это число отображается в Visual Basic по умолчанию появления сообщения об ошибке. Например, ошибка времени выполнения "3707".  
  
-   Отрицательное значение десятичной десятичное преобразование числа ошибку переполнения.  
  
-   Шестнадцатеричный шестнадцатеричное представление числа ошибку переполнения. Код устройства Windows находится в четвертом цифра. Код устройства для номера ошибок ADO — *объект*. Пример: 0x800***объект***0E7B.  
  
> [!NOTE]
>  Приложения ADO могут передаваться ошибок OLE DB. Как правило, они могут идентифицироваться по код устройства Windows из *4*. Например, 0x800***4***.  
  
|Константа|Значение|Описание|  
|--------------|-----------|-----------------|  
|**adErrBoundToCommand**|3707-2146824581 0x800A0E7B|Невозможно изменить **ActiveConnection** свойство **записей** объект, имеющий **команда** объект в качестве источника.|  
|**adErrCannotComplete**|3732 -2146824556 0x800A0E94|Серверу не удается выполнить операцию.|  
|**adErrCantChangeConnection**|3748-2146824540 0x800A0EA4|Подключение было запрещено. Новое соединение, которое вы запросили имеет разные характеристики, отличный от уже используется.|  
|**adErrCantChangeProvider**|3220 -2146825068 0X800A0C94|Указанный поставщик отличается от той, которая уже используется.|  
|**adErrCantConvertvalue**|3724-2146824564 0x800A0E8C|Данных значение не может быть преобразовано по причинам, отличным от несоответствия знаков и данные переполнения. Например преобразование будет усечение данных.|  
|**adErrCantCreate**|3725-2146824563 0x800A0E8D|Значение данных нельзя задать или извлечь, потому что тип данных поля неизвестно, или поставщик было недостаточно ресурсов для выполнения операции.|  
|**adErrCatalogNotSet**|3747 -2146824541 0x800A0EA3|Операция требует действительного **ParentCatalog**.|  
|**adErrColumnNotOnThisRow**|3726-2146824562 0x800A0E8E|Запись не содержит этого поля.|  
|**adErrDataConversion**|3421 -2146824867 0x800A0D5D|Приложение использует значение неверного типа для текущей операции.|  
|**adErrDataOverflow**|3721-2146824567 0x800A0E89|Значение данных слишком велик для представления типа данных поля.|  
|**adErrDelResOutOfScope**|3738-2146824550 0x800A0E9A|URL-адрес объекта для удаления выходит за пределы текущей записи.|  
...
|**adErrUnsafeOperation**|3716 -2146824572 0x800A0E84|Параметры безопасности на этом компьютере не, доступ к источнику данных в другом домене.|  
