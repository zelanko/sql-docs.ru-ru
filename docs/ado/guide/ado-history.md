...
## <a name="ado-28"></a>ADO 2.8
 ADO 2.8 был включен в Windows XP и Windows Server 2003, в рамках из Microsoft данных Access Components (MDAC) 2.8. Распространяемая версия MDAC 2.8 также доступна; Обратите внимание на то, что этот распространяемая версия должен быть установлен только в Windows 2000. ADO 2.8 Active Directory решают ряд проблем, связанных с безопасностью.

 *За пределами зоны надежных узлов не разрешен доступ на жестком диске.*
В междоменная работа со скриптами с ненадежных сайтов, будут отключены следующие операции: **Stream.SaveToFile**, **Stream.LoadFromFile**, **Recordset.Save**, и **Recordset.Open**, которое используется в сочетании с **adCmdFile** флаг или с помощью поставщика Microsoft OLE DB сохраняемости (MSPersist).

 **Recordset.Open** *,***Recordset.Save** *,***Stream.SaveToFile** *, и* **Stream.LoadFromFile***оперируют только физические файлы.* 
Эти методы теперь убедитесь, что дескрипторы файлов указывать только физические файлы.

 **Recordset.ActiveCommand***возвращает сообщение об ошибке при вызове с HTML/ASP-страницы.* 
Это предотвращает **команда** объект из неправильное использование.
