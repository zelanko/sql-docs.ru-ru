...
|**Обработчик**|Указывает имя программы настройки на стороне сервера (или обработчика), которая расширяет функциональные возможности [RDSServer.](../../reference/rds-api/datafactory-object-rdsserver.md)данных и всех параметров, используемых обработчиком, разделенных запятыми (","). **Строковое** значение.|
