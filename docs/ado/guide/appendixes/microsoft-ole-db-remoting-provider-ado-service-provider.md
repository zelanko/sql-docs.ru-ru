...
|-------------|-----------------|
|**Источник данных**|Задает имя удаленного источника данных. Он передается поставщику удаленного взаимодействия OLE DB для обработки.<br /><br /> Это ключевое слово эквивалентно [RDS. ](../../reference/rds-api/datacontrol-object-rds.md) Свойство [соединения](../../reference/rds-api/connect-property-rds.md) объекта элемента управления DataObject.|
...
|**Обработчик**|Указывает имя программы настройки на стороне сервера (или обработчика), которая расширяет функциональные возможности [RDSServer.](../../reference/rds-api/datafactory-object-rdsserver.md)данных и всех параметров, используемых обработчиком, разделенных запятыми (","). Значение типа **String**.|
...
|**Удаленный сервер**|Указывает имя сервера и протокол связи, которые будут использоваться этим соединением. Это свойство эквивалентно [RDS. ](../../reference/rds-api/datacontrol-object-rds.md) Свойство [сервера](../../reference/rds-api/server-property-rds.md) объектов датаконтро.|
|**Обновления Transact**|При установке значения **true**это значение указывает, что при выполнении [UpdateBatch](../../reference/ado-api/updatebatch-method.md) на сервере он будет выполняться внутри транзакции. Значение по умолчанию для этого динамического свойства Boolean равно **false**.|
...
## <a name="remarks"></a>Комментарии
 В ADO 2,0 поставщик удаленного взаимодействия OLE DB можно указать только в параметре *ActiveConnection* метода **Open** объекта [Recordset](../../reference/ado-api/recordset-object-ado.md) . Начиная с ADO 2,1, поставщик может быть также указан в параметре *ConnectionString* метода **Open** объекта [Connection](../../reference/ado-api/connection-object-ado.md) .

 Эквивалент **RDS. ** Свойство [SQL](../../reference/rds-api/sql-property.md) объекта элемента управления DataObject недоступно. Вместо этого используется аргумент " *источник* метода **открытия** объекта [набора записей](../../reference/ado-api/recordset-object-ado.md) ".
