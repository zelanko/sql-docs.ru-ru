...
|**Обработчик**|Указывает имя программы Настройка на сервере (или обработчика), которое расширяет функциональность [RDSServer.DataFactory](../../../ado/reference/rds-api/datafactory-object-rdsserver.md)и все параметры, используемые обработчиком *,* разделенные запятыми () ","). Объект **строка** значение.|
