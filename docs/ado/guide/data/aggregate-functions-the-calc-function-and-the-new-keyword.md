...
|COUNT(*chapter-alias*[.*column-name*])|Подсчитывает количество строк в заданный псевдоним. Если указан столбец, при подсчете включаются только строки, для которых этот столбец не является нулевым.|  
