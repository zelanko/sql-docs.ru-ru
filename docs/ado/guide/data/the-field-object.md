...
## <a name="describing-a-field"></a>Описание поля  
 Свойства рассматривается в разделах, которым будет следовать [поле](../../../ado/reference/ado-api/field-object.md) объект, который представляет сведения, описывающие **поле** сам объект — метаданные о поле. Эти сведения позволяют понять слишком много о схеме **записей**. Эти свойства включают в себя **тип**, **DefinedSize** и **ActualSize**, **имя**, и **NumericScale**и **точности**.  
