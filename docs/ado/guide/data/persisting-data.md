...
-   **Во время поездки:** при создании приложения в дороге, нам было крайне важно, чтобы предоставить возможность вносить изменения и добавления новых записей, которые затем можно использовать подключение к базе данных более поздней версии и фиксации.  
  
-   **Обновляемые сравнительно редко уточняющие запросы:** часто в приложении, таблицы используются в качестве уточняющих запросов — например, состояние таблиц налогов. Они обновляются нечасто и доступны только для чтения. Вместо повторного чтения этих данных с сервера при каждом запуске приложения, приложение может просто загрузить данные из локально сохраненного **записей**.  
