...
Ошибки ADO выводятся в программу как ошибки во время выполнения. Перехват ошибок механизм язык программирования позволяет перехватывать и обрабатывать их. Например, в Visual Basic, используйте **On Error** инструкции. В Visual C++ он зависит от метода, используемого для доступа к библиотеки ADO. С помощью #import, использовать **try-catch** блока. В противном случае нужно явно получить объект error, вызвав программисты на C++ **GetErrorInfo**. Приведенная ниже процедура sub в Visual Basic демонстрирует перехват ошибка ADO:
