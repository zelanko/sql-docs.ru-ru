...
```  
 ...
 Эта команда открывает **набор записей** в форме с двумя уровнями. Родительский уровень — это созданный **набор записей** со столбцом со`SUM(rs.population)`статистическими данными (), столбцом,`rs`ссылающимся на дочерний **набор записей** (),`state`и столбцом для группирования дочернего **набора записей** (). Дочерний уровень — это **набор записей** , возвращаемый командой`select * from demographics`запроса ().  
