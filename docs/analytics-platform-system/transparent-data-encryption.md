---
title: Прозрачное шифрование данных - Parallel Data Warehouse | Документы Microsoft
description: Прозрачное шифрование данных (TDE) для хранилища параллельных данных (PDW) выполняет в реальном времени ввода-вывода шифрования и расшифровки данных и файлов журнала транзакций и специальные файлы журналов PDW.»
author: mzaman1
manager: craigg
ms.prod: sql
...
ms.locfileid: "31539154"
---
# <a name="transparent-data-encryption"></a>прозрачное шифрование данных.
Чтобы защитить базу данных, можно принять ряд мер предосторожности, например спроектировать систему безопасности, проводить шифрование конфиденциальных ресурсов и поместить серверы базы данных под защиту брандмауэра. Тем не менее для сценария, украдены физический носитель (например, диск или ленты резервной копии), злоумышленник можно просто восстановить или присоединить базу данных и просматривать данные. Одним из решений может стать шифрование конфиденциальных данных в базе данных и защита ключей, используемых при шифровании, с помощью сертификата. Это не позволит использовать данные ни одному человеку, не имеющему ключей, но такой тип защиты следует планировать заранее.  
  
*Прозрачное шифрование данных* (TDE) выполняет в реальном времени ввода-вывода шифрования и расшифровки данных и файлов журнала транзакций и PDW специальные файлы журналов. При шифровании используется ключ шифрования базы данных (DEK), который хранится в загрузочной записи базы данных, где можно получить к нему доступ при восстановлении. Ключ шифрования базы данных является симметричным ключом, защищенным с помощью сертификата, хранящегося в базе данных master SQL Server PDW. Функция прозрачного шифрования данных защищает "неактивные" данные, то есть файлы данных и журналов. Благодаря ей обеспечивается соответствие требованиям различных законов, постановлений и рекомендаций, действующих в разных отраслях. Эта функция позволяет разработчикам программного обеспечения шифровать данные с помощью алгоритмов шифрования AES и 3DES, не меняя существующие приложения.  
...
**Первый этап: Включить прозрачное шифрование данных на сервере SQL Server PDW.** Это действие необходимо только в один раз.  
...
**Второй: Создание и резервную копию сертификата в базе данных master.** Это действие выполняется только требуется один раз. Может иметь отдельный сертификат для каждой базы данных (рекомендуется), или можно защитить несколько баз данных с одним сертификатом.  
...
**: Создайте ключ шифрования данных и последнего Шифрования базы данных пользователя с помощью инструкции ALTER DATABASE.** Это действие повторяется для каждой базы данных, который защищен службой прозрачного шифрования данных.  
...
Операции шифрования и дешифрования запланированы в фоновых потоках в SQL Server. Можно просмотреть состояние этих операций с помощью представления каталога и динамические административные представления в списке, представленном далее в этой статье.  
...
Состояние шифрования базы данных с помощью можно определить **sys.dm_pdw_nodes_database_encryption_keys** динамическое административное представление. Дополнительные сведения см. в разделе *представления каталогов и динамические административные представления* ранее в этой статье.  
...
Пример действия, чтобы заменить виртуальной машины.  
...
Следующие ссылки содержат общие сведения о том, как SQL Server управляет шифрования. Эти статьи помогут понять шифрование SQL Server, но эти статьи не имеют сведений, относящихся к SQL Server PDW, а также рассматриваются функции, которые не существуют в SQL Server PDW.  
