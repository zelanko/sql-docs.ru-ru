...
Операции шифрования и расшифровки планируются в фоновых потоках SQL Server. Состояние этих операций можно просмотреть с помощью представлений каталога и динамических административных представлений в списке, который приведен далее в этой статье.  
...
Для просмотра метаданных, участвующих в TDE `CONTROL SERVER` , требуется разрешение.  
...
Во время выполнения `CREATE DATABASE ENCRYPTION KEY`инструкций, `ALTER DATABASE ENCRYPTION KEY` `DROP DATABASE ENCRYPTION KEY`, или `ALTER DATABASE...SET ENCRYPTION` не допускаются следующие операции.  
...
Следующие операции или условия не позволяют выполнить инструкции `CREATE DATABASE ENCRYPTION KEY`, `ALTER DATABASE ENCRYPTION KEY` `DROP DATABASE ENCRYPTION KEY`, или `ALTER DATABASE...SET ENCRYPTION` .  
  
-   Выполняются `ALTER DATABASE` команды.  
