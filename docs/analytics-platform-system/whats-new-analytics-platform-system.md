---
title: Новые возможности в системе платформы аналитики — масштабируемое хранилище данных
...
## <a name="aps-cu75"></a>ТД CU 7.5
Дата выпуска — сентябрь 2019

### <a name="alter-external-data-source"></a>Изменение внешнего источника данных
Клиенты смогут изменять определение внешнего источника данных с помощью обновления CU 7.5. Клиенты с узлом имен Hadoop с высоким уровнем доступности теперь могут изменять источник данных для изменения аргументов при отработке отказа. Для ТД можно изменять только расположение, RESOURCE_MANAGER_LOCATION и УЧЕТные данные. Дополнительные сведения см. в разделе [Изменение внешнего источника данных](https://docs.microsoft.com/sql/t-sql/statements/alter-external-data-source-transact-sql?view=sql-server-2017) .
...
| Версия APS | Informatica Поверцентер | Драйвер |
|:---|:---|:---|
...
Мы добавили [параметр функции](appliance-feature-switch.md) , который позволяет выбрать параметры MAXDOP больше 1 для операций вставки. Теперь можно задать для параметра MAXDOP значение 0, 1, 2 или 4. Значение по умолчанию равно 1.
...
Сведения о работоспособности индекса columnstore можно просмотреть с помощью динамического административного представления **dm_pdw_nodes_db_column_store_row_group_physical_stats** . Используйте следующее представление, чтобы определить фрагментацию и решить, когда следует перестроить или реорганизовать индекс columnstore.
...
ТД поддерживает подмножество команд T-SQL [DBCC](https://docs.microsoft.com/sql/t-sql/database-console-commands/dbcc-transact-sql) , таких как [DBCC DROPCLEANBUFFERS](https://docs.microsoft.com/sql/t-sql/database-console-commands/dbcc-dropcleanbuffers-transact-sql). Ранее эти команды использовали [слоты параллелизма](https://docs.microsoft.com/sql/analytics-platform-system/workload-management?view=aps-pdw-2016-au7#concurrency-slots) , уменьшая количество пользовательских нагрузок и запросов, которые могут быть выполнены. Теперь `DBCC` команды выполняются в локальной очереди, которая не использует слоты параллелизма пользователей, что повышает общую производительность выполнения запросов.
...
Теперь @var поддерживается SELECT. Дополнительные сведения см. в разделе [Выбор локальной переменной](/sql/t-sql/language-elements/select-local-variable-transact-sql) . 
...
- Поддержка [Широкие таблицы][] аналогична SQL Server 2016. Предыдущее ограничение в 32 КБ для размера строки больше не существует. 

**Типы данных**

- [Varchar (max)][], [nvarchar (max)][] и [varbinary (max)][]. Эти типы данных LOB имеют максимальный размер, равный 2 ГБ. Чтобы загрузить эти объекты, используйте [Программа bcp][]. Polybase и dwloader в настоящее время не поддерживают эти типы данных. 
- [ИМЕЕТ SYSNAME][]
- [UNIQUEIDENTIFIER][]
- [NUMERIC][] и десятичные типы данных.
...
[ВЫБЕРИТЕ... БИТЬ]:/sql/t-sql/queries/select-into-clause-transact-sql
