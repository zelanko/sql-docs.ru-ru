---
title: "Задачи управления рабочей нагрузки"
author: barbkess
ms.author: barbkess
manager: jhubbard
...
---
# <a name="workload-management-tasks"></a>Задачи управления рабочей нагрузки
...
Выделение ресурсов, перечислены в [управления рабочей нагрузкой](workload-management.md) раздела.  
...
Этот запрос отображает имена входа, которые являются членами членство в роли сервера ресурсов класса. Имеется три класса ресурсов **mediumrc**, **largerc**, и **xlargerc**.  
...
Если нет имен входа, являющиеся членами роли сервера класс ресурсов, результирующая таблица будет пустой. Таким образом Если запрос возвращает имя Чинг входа, затем когда Чинг отправляет запрос, запрос получит системных ресурсов по умолчанию, который меньше системных ресурсов класса ресурсов. Если имя входа является членом более чем одним классом ресурсов, наибольшее класс имеет более высокий приоритет.  
  
Список выделений ресурсов для каждого класса ресурсов, в разделе [управления рабочей нагрузкой](workload-management.md) раздела.  
...
В следующем примере перемещается Чинг mediumrc серверной роли.  Для этого он должен быть удален из xlargerc и largerc ролей сервера и добавляются к роли сервера mediumrc.  
...
Чинг является членом роли сервера mediumrc.  В следующем примере изменяется Чинг иметь системных ресурсов по умолчанию для своих запросов.  
...
Описывает, как изменять параметры распределения ресурсов системы, назначенные имени входа SQL Server PDW суммы по умолчанию. Это влияет на системные ресурсы, которые назначает запросов, отправленных имени входа SQL Server PDW.  
...
Запрос может ожидания слишком долго не выполнен. Один из способов решения этой проблемы является рассмотрим общее число слотов параллелизма, необходимые для запроса.  Следующий пример показывает общее число слотов параллелизма, необходимые для каждого ожидающего запроса.  
