...
Эти инструкции показано, как найти и задайте правильный IP-адрес InfiniBand адреса и маски подсети на сервере подключен InfiniBand. Они также объясняется, как задать сервер, чтобы использовать устройство APS DNS, чтобы подключение разрешается active сети InfiniBand.  
...
1.  Найти InfiniBand IP-адреса APS DNS-серверы (appliance_domain AD01 и appliance_domain *-AD02). Для этого войдите в качестве серверов AD01 и AD02 и получить IP-адреса для каждой сети InfiniBand. InfiniBand IP-адреса на узле AD являются DNS IP-адреса.  
  
2.  Настройте каждый сетевой адаптер для использования доступного IP-адреса в сетях APS InfiniBand.  
  
    1.  Если у вас есть два сетевых адаптера InfiniBand, доступный IP-адрес, в первый сети InfiniBand, который называется TeamIB1, а другой доступные IP-адрес, второй сети InfiniBand, которая называется TeamIB2 настройки одного адаптера. Использовать TeamIB1 appliance_domain AD01 IP-адрес предпочитаемого DNS-сервера и TeamIB1 appliance_domain AD02 IP-адрес альтернативного DNS-сервера для TeamIB1 сетевого адаптера. Использовать TeamIB2 appliance_domain AD01 IP-адрес предпочитаемого DNS-сервера и TeamIB2 appliance_domain AD02 IP-адрес альтернативного DNS-сервера для TeamIB2 сетевого адаптера.  
  
    2.  При наличии только одного сетевого адаптера InfiniBand, настройте адаптер с доступными IP-адрес из одной сети InfiniBand. Затем настройте предпочитаемый и Альтернативный DNS-серверы для этого адаптера с помощью TeamIB1 appliance_domain AD01 и TeamIB1 appliance_domain AD02 или TeamIB2 appliance_domain AD01 и TeamIB2 appliance_domain AD02 наступит на том же сетевого адаптера, настроенного как основной и Альтернативный DNS-серверов соответственно.  
...
3.  Настройки сетевого адаптера InfiniBand использования APS DNS-серверы для разрешения подключения к активной сети InfiniBand.  
    ...
    1.  Чтобы настроить это использовать дополнительные параметры TCP/IP для добавления DNS-суффикс домена устройства в начало списка суффиксов DNS-сервера клиента. Это действие необходимо настроить на один из сетевых адаптеров; Этот параметр применяется к обоим адаптерам.  
  
После настройки сетевых адаптеров InfiniBand, клиентских процессов можно подключиться к узлу элемента управления в сети InfiniBand с помощью `PDW_region-SQLCTL01` для адреса сервера. Сервер добавляет суффикс DNS система платформы аналитики или ввести полный адрес, который является `PDW_region-SQLCTL01.appliance_domain.pdw.local`.  
  
Если имя устройства не MyAPS ваше имя региона PDW — MyPDW, спецификации dwloader сервера для загрузки данных является одним из следующих:  
...
С помощью SQLCTL01, служба DNS платформы Analytics подключается ваш клиент-сервер к узлу с помощью active сети InfiniBand. Это относится только к подключения; Если в сети InfiniBand перестает работать во время загрузки или резервного копирования, необходимо перезапустить процесс.  
...
1.  Войдите в устройство AD01 узла с помощью учетной записи appliance_domain\Administrator.  
...
1.  Войдите в систему как администратор Windows к загрузке, резервного копирования или другой сервер клиента на устройстве сети InfiniBand.  
...
2.  В окне «Свойства»  
    ...
    1.  На вкладке "Общие" задайте IP-адрес в IP-адрес, который проверяется как бесплатно в проверки связи для TeamIB1. Примеры значений, используемые в этой статье можно ввести 172.16.14.254.  
...
3.  В окне «Свойства»  
    ...
    1.  На вкладке "Общие" задайте IP-адрес в IP-адрес, который проверяется как бесплатно в проверки связи для TeamIB2. Примеры значений, используемые в этой статье можно ввести 172.16.18.254.  
...
3.  В окне Дополнительные параметры TCP/IP Дописывать следующие DNS-суффиксы (по порядку) параметр не неактивен, проверьте поле вызывается добавлять следующие DNS-суффиксы (по порядку): выберите суффикс домена устройства и нажмите кнопку Добавить... Суффикс домена устройства `appliance_domain.local`  
...
7.  Теперь, подключением к сети Infiniband устройства с помощью `PDW_region-SQLCTL01.appliance_domain.local`, или просто `appliance_domain-SQLCTL01`. Соединение может быть установлен быстрее, если связь с полное имя и DNS-суффикс.  
    ...
    Примеры для устройства с именем MyAPS и регион MyPDW PDW:  
