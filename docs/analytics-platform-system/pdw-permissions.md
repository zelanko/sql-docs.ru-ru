---
title: "Разрешения PDW (SQL Server PDW)"
author: barbkess
ms.author: barbkess
manager: jhubbard
...
---
# <a name="pdw-permissions"></a>Разрешения PDW
В этом разделе описываются требования и возможности управления для SQL Server PDW разрешения базы данных.  
...
-   При создании пользователя базы данных с помощью **CREATE USER** инструкции, пользователь получает **CONNECT ON DATABASE::***< имя_базы_данных >* разрешение, позволяя Имя входа для подключения к этой базе данных имени пользователя.  
...
-   **Используйте** инструкция не требует разрешений. Всем участникам можно запустить **используйте** инструкции в любой базе данных, однако для доступа к базе данных они должны иметь участника-пользователя в базе данных или пользователь-Гость необходимо включить.  
...
*Явное разрешение* — **GRANT** или **DENY** разрешения для участника, **GRANT** или **DENY**инструкции. Разрешения уровня базы данных, перечислены в [sys.database_permissions](../relational-databases/system-catalog-views/sys-database-permissions-transact-sql.md) представления. Разрешения уровня сервера перечислены в [sys.server_permissions](../relational-databases/system-catalog-views/sys-server-permissions-transact-sql.md) представления.  
...
Члены предопределенной роли сервера **sysadmin** могут выполнять любые действия на сервере. **Sa** входа является единственным членом **sysadmin** предопределенной роли сервера. Не удается добавить дополнительные учетные данные **sysadmin** предопределенной роли сервера. Предоставление **CONTROL SERVER** разрешение аппроксимирует членство в **sysadmin** предопределенной роли сервера. В следующем примере предоставляется **CONTROL SERVER** разрешения для имени входа с именем Fay.  
