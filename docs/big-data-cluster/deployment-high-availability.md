...
- Базы данных, созданные в результате рабочих процессов, отличных от `CREATE DATABASE`, таких как `RESTORE DATABASE` или `CREATE DATABASE FROM SNAPSHOT`, не добавляются в группу доступности автоматически. [Подключитесь к экземпляру](#instance-connect) и вручную добавьте базу данных в группу доступности.
