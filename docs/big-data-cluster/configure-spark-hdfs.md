...
В настоящее время имеются четыре категории конфигураций: 
- `sql` 
- `hdfs` 
- `spark` 
- `gateway` 

В Кластерах больших данных определяется 3 службы: `hdfs`, `spark` и `sql`. По случайному совпадению каждая служба соответствует одноименной категории конфигураций. Все конфигурации шлюзов относятся к категории `gateway`. 
...
Все возможные конфигурации для каждой категории можно найти на соответствующем сайте документации по Apache.
...
Помимо этих конфигураций, мы предлагаем возможность настройки выполнения заданий Spark в пуле носителей. 

Это логическое значение, `includeSpark`, находится в файле конфигурации `bdc.json` в `spec.resources.storage-0.spec.settings.spark`.

## <a name="unsupported-configurations"></a>Неподдерживаемые конфигурации

Перечисленные ниже конфигурации не поддерживаются и не могут быть изменены в контексте кластера больших данных.
...
* Во-вторых, на уровне службы. Назначьте службе несколько ресурсов и укажите конфигурации службы.
   ...
   Ниже приведен пример файла исправления для профиля. 
