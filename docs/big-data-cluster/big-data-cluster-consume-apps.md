...
Существует несколько способов применить приложение в кластере больших данных. Вы можете использовать [команду azdata app run](big-data-cluster-create-apps.md). В этом разделе показано, как использовать общие средства разработчика, такие как Postman, для выполнения приложения. 
