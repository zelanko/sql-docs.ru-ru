...
1. [Создайте контейнер больших двоичных объектов или файловую систему](https://docs.microsoft.com/azure/storage/blobs/storage-quickstart-blobs-portal) в этой учетной записи хранения для внешних данных.
...
1. Перейдите к «Azure Active Directory». Эта служба должна отображаться на левой панели навигации.
1. На панели навигации справа выберите "Регистрация приложений" и создайте новую регистрацию.
1. Создайте "веб-приложение" и следуйте указаниям мастера. **Запомните имя приложения, которое вы создадите здесь**. Его потребуется добавить в учетную запись ADLS в качестве полномочного пользователя. Также обратите внимание на идентификатор клиента приложения в обзоре при выборе приложения.
1. После создания веб-приложения перейдите в раздел "сертификаты & секреты" и создайте **новый секрет клиента** и выберите длительность ключа. **Добавьте** секрет.
1.  Вернитесь на страницу регистрации приложений и щелкните "конечные точки" вверху. **Запишите "конечная точка токена OAuth (v2)** URL-адрес
1. Теперь вы должны располагать следующими компонентами для OAuth:

    - Идентификатор клиента приложения для веб-приложения.
    - Секрет клиента
...
1. Перейдите на портал еще раз и перейдите к своей файловой системе учетной записи хранения ADLS и выберите в меню слева пункт Управление доступом (IAM).
1. Выберите "добавить назначение ролей". 
1. Выбор роли "участник данных BLOB-объектов хранилища"
1. Найдите имя, созданное ранее (Обратите внимание, что оно не отображается в списке, но будет найдено при поиске полного имени).
1. Сохраните роль.
...
В следующем примере выполняется обновление подключения. Это обновление также очистит кэш подключения.
