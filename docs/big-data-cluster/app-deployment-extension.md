...
Если щелкнуть `Links`, вы увидите, что доступ к `swagger.json` развернутого приложения, так что, написать собственные клиенты, которые вызывают приложения:
...
Можно также ссылаться на дополнительные примеры по [примеры развертывания приложений](https://aka.ms/sql-app-deploy) попробовать с расширением.
