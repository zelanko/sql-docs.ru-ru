---
title: Отладка и диагностика приложений Spark в кластерах больших данных SQL Server в сервер журнала Spark
description: Отладка и диагностика приложений Spark в кластерах больших данных SQL Server в сервер журнала Spark
...
Открыть сервер журнала Spark, перейдя к следующему URL-АДРЕСУ, замените `<Ipaddress>` и `<Port>` со сведениями об определенном кластере больших данных. Дополнительные сведения можно обращаться к: [кластера больших данных развертывание SQL Server](quickstart-big-data-cluster-deploy.md)
...
+ Сохранять все данные в виде CSV-файл, нажав кнопку **csv**.
    ...
    ![Данные сохранить](./media/apache-azure-spark-history-server/sparkui-data-save.png)
...
+ Скачивание одного файла, нажав кнопку **частичной загрузки** , поместите справа, то выбранный файл загружается на локальный месте. Если файл больше не существует, откроется новая вкладка для отображения сообщения об ошибках.
    ...
    ![Строки загрузки данных](./media/apache-azure-spark-history-server/sparkui-data-download-row.png)
...
+ Скопируйте полный или относительный путь, выбрав **Копировать полный путь**, **Копировать относительный путь** это развернет окно, в меню загрузки. Для файлов хранилища Озера данных azure **открыть в обозревателе службы хранилища Azure** запустит обозреватель службы хранилища Azure. И найдите папку точное, при входе в систему.
    ...
    ![Путь к копии данных](./media/apache-azure-spark-history-server/sparkui-data-copy-path.png)
...
+ Воспроизведение задания, щелкнув **воспроизведения** кнопку и остановить в любое время, нажав кнопку "Остановить". Отображение задач в цвет для отображения разных состояния, при воспроизведении:
    ...
    + Зеленый, при успешном выполнении: задание завершено успешно.
    + Оранжевый для Повторено: экземпляры задач, которые не удалось, но не влияют на результат задания. Эти задачи было дублирующиеся или повторные экземпляры, которые может завершиться успешно позже.
    + Синий — для выполнения: запущена задача.
    + Белый ожидания или пропущенных: задача ожидает выполнения или пропущен рабочей области.
    + Красный, сбой: не удалось выполнить задачу.
