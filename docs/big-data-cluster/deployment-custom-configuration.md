...
Аналогично изменению параметров службы единый в определенном ресурсе. Например, если вы хотите изменить параметры памяти Spark только для компонента Spark в пуле носителей, вы конструируемой UDPATE ресурс **Storage-0** с разделом **параметров** для службы **Spark** в файле конфигурации **BDC. JSON.** .
```json
"resources":{
    ...
     "storage-0": {
