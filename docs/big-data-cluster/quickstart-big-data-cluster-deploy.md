...
1. При запросе введите следующие сведения:
   ...
   > [!IMPORTANT]
   > Каждое утверждение постоянного тома в кластере требуется подключенный диск. В настоящее время работы с большими данными кластера требуется 21 утверждения постоянного тома. При выборе размера виртуальной машины Azure и количество узлов, убедитесь, что общее число дисков, которые могут присваиваться на узлах, больше или равно 21. Например [Standard_L4s](https://docs.microsoft.com/azure/virtual-machines/windows/sizes-storage#ls-series) размер машины, поддерживает 16 подключенные диски, поэтому три узла означает, что 48 диски могут быть подключены.
