...
|Платформа|Поддерживаемые версии|
|---------|---------|
|`azdata`|Должна быть такая же дополнительная версия, что и у сервера (тот же основной экземпляр SQL Server).<br/><br/>Запустите `azdata –-version`, чтобы проверить версию.<br/><br/>Последнюю версию см. в разделе [История выпусков](#release-history).|
