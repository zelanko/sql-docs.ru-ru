...
- `security.activeDirectory.clusterAdmins`: этот параметр принимает одну группу AD. Область группы AD должна быть универсальной или доменной глобальной группой. Члены этой группы получают в кластере разрешения администратора. Это означает, что у них имеются разрешения `sysadmin` в SQL Server, разрешения суперпользователя в HDFS и разрешения администраторов в контроллере. 
...
- `security.activeDirectory.clusterUsers`: Список групп AD, которые являются обычными пользователями (без прав администратора) в кластере больших данных. Список может включать группы AD, областью действия которых является универсальная или доменная глобальная группа. Они не могут быть локальными доменными группами.
...
- `security.activeDirectory.appOwners` **Необязательный параметр**: список групп AD, имеющих разрешения на создание, удаление и запуск любого приложения. Список может включать группы AD, областью действия которых является универсальная или доменная глобальная группа. Они не могут быть локальными доменными группами.

  >[!IMPORTANT]
  >Создайте эти группы в AD перед началом развертывания. Если область для любой из этих групп AD является локальной доменной, развертывание завершается сбоем.

- `security.activeDirectory.appReaders` **Необязательный параметр**: список групп AD, имеющих разрешения на запуск любого приложения. Список может включать группы AD, областью действия которых является универсальная или доменная глобальная группа. Они не могут быть локальными доменными группами.
