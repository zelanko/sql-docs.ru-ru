...
Для настройки порядка отправки значения java.sql.Time используется свойство **sendTimeAsDatetime**. Если задано значение false, значения времени отправляется как тип времени SQL Server. Если задано значение true, время отправки значение как тип даты и времени. Если в них столбец времени зашифрован, **sendTimeAsDatetime** свойство должно быть задано значение false, так как зашифрованные столбцы не поддерживает преобразование из времени в datetime. Также Обратите внимание, что это свойство по умолчанию true, поэтому при использовании зашифрованных столбцов вы должны присвоить ему значение false. В противном случае драйвер вызовет исключение. Класс SQLServerConnection, начиная с версии 6.0 драйвера, используются два метода для программной настройки значение этого свойства.
