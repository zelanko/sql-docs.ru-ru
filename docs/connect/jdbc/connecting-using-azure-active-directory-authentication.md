...
Дополнительные сведения об установке свойства authentication см. в статье [Setting the connection properties](../../connect/jdbc/setting-the-connection-properties.md) (Установка свойств подключения).  
...
* Пользователь автономной базы данных, представляющий управляемое удостоверение, назначаемое системой, или управляемое удостоверение, назначаемое пользователем, Ресурса Azure или одну из групп, к которым принадлежит ваше удостоверение MSI, должен существовать в целевой базе данных и иметь разрешение CONNECT.
...
### <a name="set-kerberos-ticket-on-windows-linux-and-mac"></a>Настройка билета Kerberos в Windows, Linux и Mac
...
2.  Найдите приведенные ниже строки кода и замените имена сервера и базы данных именами своих сервера и базы данных.
    ...
    ds.setDatabaseName("demo"); // replace with your database name
    ```
3.  Найдите приведенные ниже строки кода и замените имя пользователя именем пользователя Azure Active Directory, от имени которого вы хотите установить подключение.
    ```java
