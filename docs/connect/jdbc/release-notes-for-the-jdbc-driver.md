...
Microsoft JDBC Driver 6.4 для SQL Server полностью соответствует спецификации JDBC 4.1 и 4.2. JAR-файлы, содержащиеся в пакете 6,4 присваиваются в соответствии с совместимость версий Java. Например mssql-jdbc-6.4.0.jre8.jar файл из 6,4 пакета рекомендуется для использования с Java 8. 
...
Поддержка спецификаций Java Database Connectivity API 4.3 в дополнение к версиям 4.1 и 4.2. Методы JDBC 4.3 API добавлены, но еще не реализован. Дополнительные сведения см. [соответствия JDBC 4.3 для JDBC-драйвера](../../connect/jdbc/jdbc-4-3-compliance-for-the-jdbc-driver.md).
...
1. Для значений даты: при использовании возвращающего табличное значение Параметра для заполнения таблицы, содержащей значения datetime, smalldatetime или date, хранящиеся в столбце sql_variant, вызов методов getDateTime()/getSmallDateTime()/getDate() resultset не работает и вызывает следующее исключение:
    ```
...
При использовании возвращающего табличное значение Параметра для заполнения таблицы и отправки значения NULL в столбец типа sql_variant, возникнет исключение, как вставка значения NULL со столбцом типа sql_variant в возвращающий табличное значение параметр сейчас не поддерживается.
...
Microsoft JDBC Driver 6.2 для SQL Server полностью соответствует спецификации JDBC 4.1 и 4.2. JAR-файлы, содержащиеся в пакете 6.0 присваиваются в соответствии с совместимость версий Java. Например mssql-jdbc-6.2.1.jre8.jar файл из 6.2 пакета рекомендуется для использования с Java 8. 
...
* Проверка подлинности между областями, встроенную проверку подлинности Kerberos без явного указания имени участника-службы сервера. Драйвер теперь автоматически вычисляет области даже в том случае, если не предоставлен.
...
Microsoft JDBC Driver 6.1 для SQL Server полностью соответствует спецификации JDBC 4.1 и 4.2. Это представляет собой версию драйвера JDBC начальной открытым исходным кодом и mssql-jdbc-6.1.0.jre8.jar mssql-jdbc-6.1.0.jre7.jar, файлы, которые соответствуют совместимость версий Java. 

## <a name="updates-in-microsoft-jdbc-driver-60-for-sql-server"></a>Обновления в Microsoft JDBC Driver 6.0 для SQL Server

Microsoft JDBC Driver 6.0 для SQL Server полностью соответствует спецификации JDBC 4.1 и 4.2. JAR-файлы, содержащиеся в пакете 6.0 присваиваются в соответствии с их соответствие с версией JDBC API. Например sqljdbc42.jar из пакета 6.0 он совместим с JDBC API 4.2. Аналогичным образом файл sqljdbc41.jar совместима с API JDBC 4.1.
...
Microsoft JDBC Driver 4.2 для SQL Server полностью соответствует спецификации JDBC 4.1 и 4.2. JAR-файлы, содержащиеся в пакете 4.2 присваиваются в соответствии с их соответствие с версией JDBC API. Например sqljdbc42.jar из пакета 4.2 он совместим с JDBC API 4.2. Аналогичным образом файл sqljdbc41.jar совместима с API JDBC 4.1.
 ...
 **Соответствие JDBC 4.1 и 4.2**  
 ...
 Поддержка спецификаций Java Database Connectivity API 4.1 и 4.2 в дополнение к версии 4.0. Дополнительные сведения см. [соответствие JDBC 4.1 для драйвера JDBC](../../connect/jdbc/jdbc-4-1-compliance-for-the-jdbc-driver.md) и [соответствие JDBC 4.2 для драйвера JDBC](../../connect/jdbc/jdbc-4-2-compliance-for-the-jdbc-driver.md).  
 ...
 **Возможность отката транзакций XA**  
 ...
 Добавлены новые параметры времени ожидания для существующего автоматического отката неподготовленных транзакций. Подробности см. в разделе [основные сведения о транзакциях XA](../../connect/jdbc/understanding-xa-transactions.md).  
  
 **Новое свойство соединения участника Kerberos**  
  
 Добавлено новое свойство соединения для улучшения гибкости подключений Kerberos. Подробности см. в статье [Использование встроенной проверки подлинности Kerberos для подключения к SQL Server](../../connect/jdbc/using-kerberos-integrated-authentication-to-connect-to-sql-server.md).  
...
## <a name="itanium-not-supported-for-jdbc-driver-64-60-42-and-41-applications"></a>Itanium не поддерживается в приложениях, использующих драйвер JDBC версии 6.4, 6.0, 4.2 и 4.1.  
 ...
 Драйвер Microsoft JDBC версий 6.4, 6.0, 4.2 и 4.1 для приложений SQL Server не поддерживают выполнение на компьютерах с процессором Itanium.  
