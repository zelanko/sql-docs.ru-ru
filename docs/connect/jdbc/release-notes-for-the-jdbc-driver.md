...
| Совместимо с пакетом SDK для Java (JDK) версии 12,0, 11,0 и 1,8. | Microsoft JDBC Driver версии 7.4 для SQL Server совместим с пакетом Java Development Kit (JDK) версии 12.0 наряду с JDK версии 11.0 и 1.8. |
| &nbsp; | &nbsp; |
...
### <a name="introduces-ntlm-authentication"></a>Введение в проверку подлинности NTLM

| Изменение NTLM | Сведения |
| :--------- | :------ |
| Поддерживает режим проверки подлинности NTLM. | Этот режим проверки подлинности позволяет клиентам Windows и не Windows проходить проверку подлинности для SQL Server с помощью пользователей домена Windows. |
| Дополнительные сведения и пример приложения для использования этого режима проверки подлинности. | См. раздел [подключение с использованием проверки подлинности NTLM](../../connect/jdbc/using-ntlm-authentication-to-connect-to-sql-server.md). |
| &nbsp; | &nbsp; |

### <a name="introduces-querying-parametermetadata-via-_usefmtonly_"></a>Введение в запросы ParameterMetaData через _усефмтонли_

| изменение Усефмтонли | Сведения |
| :---------- | :------ |
| Добавлено свойство соединения **усефмтонли** . | Эта функция позволяет пользователям при необходимости запрашивать ParameterMetaData через `SET FMTONLY ON` устаревший API. Это полезно в сценариях, `sp_describe_undeclared_parameters` где не выполняется должным образом. |
| Дополнительные сведения и ограничения. | См. об [использовании useFmtOnly](../../connect/jdbc/using-usefmtonly.md). |
| &nbsp; | &nbsp; |
...
| При использовании useFmtOnly. | С этой функцией имеются некоторые проблемы, причиной которых являются недостатки в логике синтаксического анализа SQL. Дополнительные сведения и рекомендации по решению см. [в разделе Использование усефмтонли](../../connect/jdbc/using-usefmtonly.md) . |
| &nbsp; | &nbsp; |
