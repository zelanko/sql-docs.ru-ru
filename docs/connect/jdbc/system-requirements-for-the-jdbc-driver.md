...
**Microsoft JDBC Driver 7.2 для SQL Server**  
  ...
  Драйвер JDBC 7.2 содержит две библиотеки классов JAR в каждом пакете установки: **mssql-jdbc-7.2.0.jre8.jar** и **mssql-jdbc-7.2.0.jre11.jar**.

  7.2 драйвер JDBC предназначен для работы с и должны поддерживаться всеми основными виртуальных машин Java, но его проверка только на OpenJDK 8.0, OpenJDK 11.0, Azul Zulu JRE 8.0 и Azul Zulu JRE 11.0.
...
|---------|-----------------------------|----------------------|-----------------|   
|mssql-jdbc-7.2.0.jre8.jar|4.2|8|Требуется среда выполнения Java (JRE) версии 8.0. Использование JRE 7.0 или ниже выдает исключение.<br /><br /> Новые возможности в 7.2: Поддержка JDK 11, проверки подлинности Active Directory управляемого удостоверения службы (MSI), OSGi поддержки, SQLServerError API-интерфейсы. |    
|mssql-jdbc-7.2.0.jre11.jar|4.3|10|Требуется среда выполнения Java (JRE) 11.0. С помощью JRE версии 10.0 или ниже выдает исключение.<br /><br /> Новые возможности в 7.2: Поддержка JDK 11, проверки подлинности Active Directory управляемого удостоверения службы (MSI), OSGi поддержки, SQLServerError API-интерфейсы. |    
...
**Microsoft JDBC Driver 7.0 для SQL Server**  
  ...
  |JAR|Соответствие версии JDBC|Рекомендуемая версия Java|Описание|  
|---------|-----------------------------|----------------------|-----------------|   
|mssql-jdbc-7.0.0.jre8.jar|4.2|8|Требуется среда выполнения Java (JRE) версии 8.0. Использование JRE 7.0 или ниже выдает исключение.<br /><br /> Новые возможности в 7.0: Поддержка JDK 10, уровень совместимости по умолчанию обновлен для спецификации JDBC 4.2, поддержка пространственных типов данных, свойство соединения cancelQueryTimeout, запросить границ методов useBulkCopyForBatchInsert свойство соединения, данные обнаружения и классификации сведения, расширения компонента UTF-8 и поддержки CityHash. |    
|mssql-jdbc-7.0.0.jre10.jar|4.3|10|Требуется среда выполнения Java (JRE) 10.0. С помощью 9.0 с JRE или ниже выдает исключение.<br /><br /> Новые возможности в 7.0: Поддержка JDK 10, уровень совместимости по умолчанию обновлен для спецификации JDBC 4.2, поддержка пространственных типов данных, свойство соединения cancelQueryTimeout, запросить границ методов useBulkCopyForBatchInsert свойство соединения, данные обнаружения и классификации сведения, расширения компонента UTF-8 и поддержки CityHash. |    
...
**Microsoft JDBC Driver 6.4 для SQL Server**  
  ...
  |JAR|Соответствие версии JDBC|Рекомендуемая версия Java|Описание|  
|---------|-----------------------------|----------------------|-----------------|   
|mssql-jdbc-6.4.0.jre7.jar|4.1|7|Требуется среда выполнения Java (JRE) версии 7.0. Использование JRE 6.0 или ниже выдает исключение.<br /><br /> Новые возможности в 6.4: Повторно использовать проверку подлинности Azure AD для Linux, метод субъект и пароль проверки подлинности Kerberos, автоматическое обнаружение области в имени участника-службы для проверки подлинности между доменами, ограниченное делегирование Kerberos, время ожидания запроса, время ожидания сокета и дескриптор подготовленной инструкции. |  
|mssql-jdbc-6.4.0.jre8.jar|4.2|8|Требуется среда выполнения Java (JRE) версии 8.0. Использование JRE 7.0 или ниже выдает исключение.<br /><br /> Новые возможности в 6.4: Повторно использовать проверку подлинности Azure AD для Linux, метод субъект и пароль проверки подлинности Kerberos, автоматическое обнаружение области в имени участника-службы для проверки подлинности между доменами, ограниченное делегирование Kerberos, время ожидания запроса, время ожидания сокета и дескриптор подготовленной инструкции. |    
|mssql-jdbc-6.4.0.jre9.jar|4.3|9|Требуется среда выполнения Java (JRE) версии 9.0. Использование JRE 8.0 или ниже выдает исключение.<br /><br /> Новые возможности в 6.4: Повторно использовать проверку подлинности Azure AD для Linux, метод субъект и пароль проверки подлинности Kerberos, автоматическое обнаружение области в имени участника-службы для проверки подлинности между доменами, ограниченное делегирование Kerberos, время ожидания запроса, время ожидания сокета и дескриптор подготовленной инструкции. |
...
|JAR|Соответствие версии JDBC|Рекомендуемая версия Java|Описание|  
|---------|-----------------------------|----------------------|-----------------|
|mssql-jdbc-6.2.2.jre7.jar|4.1|7|Требуется среда выполнения Java (JRE) версии 7.0. Использование JRE 6.0 или ниже выдает исключение.<br /><br /> Новые возможности в 6.2: Повторно использовать проверку подлинности Azure AD для Linux, метод субъект и пароль проверки подлинности Kerberos, автоматическое обнаружение области в имени участника-службы для проверки подлинности между доменами, ограниченное делегирование Kerberos, время ожидания запроса, время ожидания сокета и дескриптор подготовленной инструкции. |  
|mssql-jdbc-6.2.3.jre8.jar|4.2|8|Требуется среда выполнения Java (JRE) версии 8.0. Использование JRE 7.0 или ниже выдает исключение.<br /><br /> Новые возможности в 6.2: Проверка подлинности Azure AD для Linux, метод субъект и пароль проверки подлинности Kerberos, автоматическое обнаружение области в имени участника-службы для проверки подлинности между доменами, ограниченное делегирование Kerberos, время ожидания запроса, время ожидания сокета и дескриптор подготовленной инструкции повторного использования|    
...
|JAR|Соответствие версии JDBC|Рекомендуемая версия Java|Описание|  
|---------|-----------------------------|----------------------|-----------------|   
|sqljdbc41.jar|4.1|7|Требуется среда выполнения Java (JRE) версии 7.0. Использование JRE 6.0 или ниже выдает исключение.<br /><br /> Новые возможности в пакетах 6.0 и 4.2 включают: JDBC 4.1 массовое копирование и соответствие<br /><br /> Кроме того новые возможности в пакете 6.0: Always Encrypted, возвращающего табличное значение параметров, аутентификации Azure Active Directory, прозрачное подключение к групп доступности AlwaysOn, улучшения получения метаданных параметров подготовленные запросы и международных доменных имен (IDN)|  
|sqljdbc42.jar|4.2|8|Требуется среда выполнения Java (JRE) версии 8.0. Использование JRE 7.0 или ниже выдает исключение.<br /><br /> Новые возможности в пакетах 6.0 и 4.2 включают: JDBC 4.1 соответствия, JDBC 4.2, массовое копирование и соответствие<br /><br /> Кроме того новые возможности в пакете 6.0: Always Encrypted, возвращающего табличное значение параметров, аутентификации Azure Active Directory, прозрачное подключение к групп доступности AlwaysOn, улучшения получения метаданных параметров подготовленные запросы и международных доменных имен (IDN)|  
