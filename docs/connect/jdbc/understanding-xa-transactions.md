---
title: Общие сведения о транзакциях XA | Документация Майкрософт
ms.custom: ''
ms.date: 01/21/2019
ms.prod: sql
...
> [!WARNING]  
> Драйвер Microsoft JDBC Driver 4.2 для SQL (и более поздних версий) включает новые параметры времени ожидания для автоматического отката неподготовленных транзакций существующих компонентов. Дополнительные сведения см. в разделе [Настройка параметров времени ожидания на стороне сервера для автоматического отката неподготовленных транзакций](../../connect/jdbc/understanding-xa-transactions.md#BKMK_ServerSide) далее в этом разделе.  
...
> [!NOTE]  
> Начиная с версии SQL Server 2019 общедоступная Предварительная версия CTP 2,0, компоненты распределенных транзакций XA для JDBC включены в подсистему SQL Server и могут быть включены или отключены с помощью системной хранимой процедуры.
> Чтобы включить необходимые компоненты для выполнения распределенных транзакций XA с помощью драйвера JDBC, выполните следующую хранимую процедуру.
>
> EXEC sp_sqljdbc_xa_install
>
> Чтобы отключить ранее установленные компоненты, выполните следующую хранимую процедуру.
>
...
- **Ксадефаулттимеаут** (в секундах): значение времени ожидания по умолчанию, которое используется, если пользователь не указал время ожидания. Значение по умолчанию равно 0.  
  
- **Ксамакстимеаут** (в секундах): максимальное значение времени ожидания, которое может быть задано пользователем. Значение по умолчанию равно 0.  
...
1. Выгрузите sqljdbc_xa. dll с [!INCLUDE[tsql](../../includes/tsql-md.md)] помощью команды **DBCC sqljdbc_xa (Free)** .  
