...
###  <a name="BKMK_TableSetup"></a> Настройка таблиц  
...
>   
>  Дополнительные сведения см. в разделе [транзакции и массовой операции копирования](../../connect/jdbc/using-bulk-copy-with-the-jdbc-driver.md#BKMK_TransactionBulk)  
...
2.  Разверните базы данных, щелкните правой кнопкой мыши базу AdventureWorks, выберите **Задачи** и **Экспорт данных**…  
...
5.  Установите флажок **Написать запрос, указывающий данные для передачи** и вновь нажмите **Далее**.  Введите свою **SQL-инструкцию** "SELECT ProductID, Name, ProductNumber FROM Production.Product", затем снова нажмите **Далее**  
...
### <a name="bulk-copy-with-always-encrypted-columns"></a>Массовое копирование с помощью постоянного шифрования столбцов  
 Начиная с Microsoft JDBC Driver 6.0 для SQL Server, массовое копирование поддерживается со столбцами постоянного шифрования.  
  
 В зависимости от параметров массового копирования и шифрования тип исходной и целевой таблиц, драйвер JDBC может прозрачно расшифровывать и затем зашифровать данные, или он может отправлять зашифрованные данные, как есть. Например при массовом копировании данных из зашифрованного столбца со столбцом незашифрованные, драйвер прозрачно расшифровывает данные перед отправкой в SQL Server. Аналогичным образом при массовом копировании данных из незашифрованные столбца (или из CSV-файла) для зашифрованного столбца драйвер прозрачно шифрует данные перед отправкой в SQL Server. Если оба источника и назначения шифруется, затем в зависимости от **allowEncryptedValueModifications** параметром массового копирования, драйвер будет отправлять данные, как является или расшифровать данные и зашифровать его повторно перед отправкой в SQL Server.  
  
 Дополнительные сведения см. в разделе **allowEncryptedValueModifications** параметром массового копирования ниже, и [использование функции Always Encrypted с драйвером JDBC](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md).  
  
> [!IMPORTANT]  
>  Ограничение Microsoft JDBC Driver 6.0 для SQL Server, при выполнении массового копирования данных из CSV-файла в зашифрованные столбцы.  
>   
...
|SQLServerBulkCopy (строка connectionURL)|Инициализирует и открывает новый экземпляр SQLServerConnection на основе предоставленного URL-адреса подключения (connectionURL). Конструктор использует SQLServerConnection для инициализации нового экземпляра класса SQLServerBulkCopy.|  
...
|Логическое allowEncryptedValueModifications|Этот параметр доступен с Microsoft JDBC Driver 6.0 (или более поздней версии) для SQL Server.<br /><br /> При указании **allowEncryptedValueModifications** осуществлять массовое копирование зашифрованных данных между таблицами или базами данных, не расшифровывая данные. Как правило приложение нужно выбирать данные из зашифрованных столбцов из одной таблицы без расшифровки данных (приложение должно подключиться к базе данных с ключевым словом параметр шифрования столбца, отключена) и затем использовать этот параметр, чтобы операции массовой вставки данных, который по-прежнему шифруется. Дополнительные сведения: [Использование функции Always Encrypted с драйвером JDBC](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md).<br /><br /> Используйте параметр **allowEncryptedValueModifications** с осторожностью. Драйвер не проверяет, являются ли данные зашифрованными и используется ли при шифровании тот же тип, алгоритм и ключ шифрования, что у целевого столбца. Таким образом, изменение значений может повредить целевую базу.||  
...
|Void setCheckConstraints(Boolean checkConstraints)|Задает ограничения должны проверяться при вставке данных или нет.|  
|Логическое isFireTriggers()|Указывает, если сервер должен срабатывать триггеры вставки для строк, вставляемых в базу данных.|  
|Void setFireTriggers(Boolean fireTriggers)|Задает ли сервер должен быть установлен вызывать срабатывание триггеров для строк, вставляемых в базу данных.|  
|Логическое isKeepIdentity()|Указывает, следует ли сохранять все исходные значения идентификаторов.|  
|Void setKeepIdentity(Boolean keepIdentity)|Задает, следует ли сохранять значения идентификаторов.|  
|Логическое isKeepNulls()|Указывает, следует ли сохранять значения null в целевой таблице независимо от параметров значений по умолчанию, или если они должны заменены значениями по умолчанию (если применимо).|  
|Void setKeepNulls(Boolean keepNulls)|Задает, следует ли сохранять значения null в целевой таблице независимо от параметров значений по умолчанию, или если они должны заменены значениями по умолчанию (если применимо).|  
...
|Void setTableLock(Boolean tableLock)|Задает ли SQLServerBulkCopy получите блокировку массовых обновлений в течение операции массового копирования.|  
|Логическое isUseInternalTransaction()|Указывает, должен ли каждый пакет операции массового копирования выполняться в рамках транзакции.|  
...
|void setAllowEncryptedValueModifications(boolean allowEncryptedValueModifications)|Настраивает этот параметр allowEncryptedValueModifications, который используется для массового копирования со столбцами постоянного шифрования.|  
...
|-----------------------|-----------------|  
|Задайте\<целое число > getColumnOrdinals()|Получение порядковых номеров столбцов, представленных в этой записи данных.|  
|Строка getColumnName(int column)|Получение имени заданного столбца.|  
|Int getColumnType (столбца данных типа int)|Получение типа данных JDBC заданного столбца.|  
|Int getPrecision (столбца данных типа int)|Получение точности для данного столбца.|  
...
|Int getScale (столбца данных типа int)|Получение масштаба для данного столбца.|  
|Логическое isAutoIncrement (столбца данных типа int)|Обозначает, содержит ли столбец идентификаторы.|  
...
|Конструктор|Описание|  
|-----------------|-----------------|  
|SQLServerBulkCSVFileRecord (fileToParse строка, кодировка строки, разделитель строк, логическое firstLineIsColumnNamesSQLServerBulkCSVFileRecord (строка, строка, String, boolean)|Инициализирует новый экземпляр класса SQLServerBulkCSVFileRecord, который будет выполнять синтаксический анализ каждой строки в fileToParse с указанными разделителем и кодировкой. Если firstLineIsColumnNames имеет значение True, первая строка файла будет проанализирована как имена столбцов.  Если кодировка имеет значение NULL, будет использоваться кодировка по умолчанию.|  
|SQLServerBulkCSVFileRecord (строка fileToParse, строку, кодирование, логическое firstLineIsColumnNamesSQLServerBulkCSVFileRecord (String, String, boolean)|Инициализирует новый экземпляр класса SQLServerBulkCSVFileRecord, который будет выполнять синтаксический анализ каждой строки в fileToParse с указанными разделителем-запятой и кодировкой. Если firstLineIsColumnNames имеет значение True, первая строка файла будет проанализирована как имена столбцов.  Если кодировка имеет значение NULL, будет использоваться кодировка по умолчанию.|  
|SQLServerBulkCSVFileRecord (fileToParse строка, логическое firstLineIsColumnNamesSQLServerBulkCSVFileRecord (String, boolean)|Инициализирует новый экземпляр класса SQLServerBulkCSVFileRecord, который будет выполнять синтаксический анализ каждой строки в fileToParse с указанными разделителем-запятой и кодировкой по умолчанию. Если firstLineIsColumnNames имеет значение True (Истина), первая строка файла будет прочитана как содержащая имена столбцов.|  
  
|Метод|Описание|  
|------------|-----------------|  
|Void addColumnMetadata (int positionInFile, строка columnName, int jdbcType, int точности и масштаба int)|Добавляет метаданные для указанного столбца в файле.|  
...
|Void setTimeWithTimezoneFormat (dateTimeFormatter ечная DateTimeForm)|Задает формат для синтаксического анализа данных времени из файла как java.sql.Types.TIME_WITH_TIMEZONE.|  
