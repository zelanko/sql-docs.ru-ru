---
title: Поддержка высокого уровня доступности и аварийного восстановления для драйверов Майкрософт для PHP для SQL Server | Документация Майкрософт
ms.custom: ''
ms.date: 07/31/2018
ms.prod: sql
...
Дополнительные сведения о ключевых словах строки подключения [!INCLUDE[ssDriverPHP](../../includes/ssdriverphp_md.md)]в см. в разделе [Параметры соединения](../../connect/php/connection-options.md).  
...
Разрешение IP-адресов в прозрачной сети (ТНИР) является редакцией существующей функции MultiSubnetFailover. Он влияет на последовательность подключения драйвера, когда первый разрешенный IP-адрес имени узла не отвечает, и существует несколько адресов, связанных с именем узла. Вместе с MultiSubnetFailover они предоставляют следующие четыре последовательности подключения: 

- ТНИР Enabled & MultiSubnetFailover отключено: предпринята одна IP-часть, за которой следуют все адреса IP в параллельном режиме
- Включен & MultiSubnetFailover включено: все IP-адреса попытаются параллельно
- ТНИР Disabled & MultiSubnetFailover отключено: все IP-адреса предпринимались один за другим
- ТНИР Disabled & MultiSubnetFailover включен: все IP-адреса попытаются параллельно

ТНИР включен по умолчанию, а параметр MultiSubnetFailover отключен по умолчанию.

Вот пример включения ТНИР и MultiSubnetFailover с помощью драйвера PDO_SQLSRV:
