...
В следующих примерах демонстрируется фильтрации данных на основе зашифрованных значений и получение данных открытого текста из зашифрованных столбцов, использующих драйверы SQLSRV и PDO_SQLSRV. Обратите внимание на следующие моменты:
 -   Значение, используемое в предложении WHERE для фильтрации по столбцу SSN должен передать с помощью параметра привязки таким образом, чтобы драйвер мог его прозрачно зашифровать перед их отправкой на сервер.
...
Любое значение, предназначенное для зашифрованного столбца должен быть зашифрован перед отправкой на сервер. Попытка вставки, изменения или фильтрации по значению открытого текста в зашифрованном столбце приведет к ошибке. Чтобы избежать таких ошибок, убедитесь в том, что:
