...
Демонстрационная программа разработана так, чтобы временная ошибка (то есть любая ошибка, код которой начинается с префикса "08", как указано в [этом приложении](../../odbc/reference/appendixes/appendix-a-odbc-error-codes.md)) во время попытки подключения вызывала повторную попытку. Тем не менее, в случае временной ошибки во время выполнения команды запроса программа отменяет текущее подключение и создает новое, прежде чем повторять попытку запроса. Этот вариант разработки не рекомендуется, но и не запрещается. Демонстрационная программа иллюстрирует некоторые возможности разработки, доступные пользователю.  
