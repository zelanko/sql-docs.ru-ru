...
Следующая строка кода позволяет выполнить сброс такого поведения по умолчанию (предупреждения обрабатываются как ошибки):  
...
Следующий пример кода показывает, как отключить поведение обработки ошибок по умолчанию. Пример использует команду PRINT языка Transact-SQL для создания предупреждения. Дополнительные сведения о команде PRINT см. в разделе [PRINT (Transact-SQL)](http://go.microsoft.com/fwlink/?linkid=119518).  
