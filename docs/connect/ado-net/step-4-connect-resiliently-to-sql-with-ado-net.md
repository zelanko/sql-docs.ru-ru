...
Классы ADO.NET для подключения к локальному Microsoft SQL Server также могут подключаться к базе данных SQL Azure. Однако сами классы ADO.NET не могут обеспечить устойчивость и надежность, необходимые для использования в рабочей среде. Клиентская программа может столкнуться с временными сбоями, из которых она должна автоматически восстанавливаться и продолжаться.  
