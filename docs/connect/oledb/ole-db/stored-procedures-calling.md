...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  Хранимая процедура может иметь ноль и более параметров. Также она может возвращать значение. При использовании драйвера OLE DB для SQL Server параметры хранимой процедуры могут передаваться следующим образом.  
...
### <a name="odbc-call-escape-sequence"></a>Escape-последовательность ODBC CALL  
 ...
 {[ **? =** ]**вызовите**_процедуру\_с именем_[ **(** [*параметр*] [ **,** [_параметр_]]... **)** ]}  
...
```  
 ...
 Пример приложения, демонстрирующий escape-последовательность RPC, см. в разделе [ &#40;выполнение хранимой процедуры с&#41; использованием синтаксиса RPC и обработка кодов &#40;возврата&#41;и выходных параметров OLE DB](../../oledb/ole-db-how-to/results/execute-stored-procedure-with-rpc-and-process-output.md).  
  
### <a name="transact-sql-execute-statement"></a>Инструкция Transact-SQL EXECUTE  
 Escape-последовательность ODBC CALL и escape-последовательность RPC — это предпочтительные способы вызова хранимых процедур в отличие от инструкции [EXECUTE](../../../t-sql/language-elements/execute-transact-sql.md). Драйвер OLE DB для SQL Server использует механизм [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] RPC для оптимизации обработки команд. Этот протокол RPC повышает производительность, устраняя большую часть обработки параметров и синтаксической проверки инструкций на сервере.  
