...
description: Курсоры наборы строк и SQL Server
ms.custom: ''
ms.date: 03/26/2018
ms.prod: sql
...
|DBPROP_IRowsetLocate или DBPROP_IRowsetScroll и DBPROP_IRowsetChange или DBPROP_IRowsetUpdate|VARIANT_TRUE|Возможно обновление данных [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] через набор строк. Этот набор строк поддерживает прокрутку и выборку в любом направлении. Закладки и абсолютное позиционирование при помощи **IRowsetLocate** поддерживается в наборе строк. Текст команды может содержать предложение ORDER BY.|  
...
## <a name="sql-server-cursor-block-size"></a>Размер блока курсора SQL Server  
 Когда [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] курсор поддерживает драйвер OLE DB для SQL Server набора строк, количество элементов в строке параметре массива дескриптора **IRowset::GetNextRows** или **IRowsetLocate::GetRowsAt** методы определяет размер блока курсора. Строки, указанные дескрипторами в массиве, являются элементами блока курсора.  
