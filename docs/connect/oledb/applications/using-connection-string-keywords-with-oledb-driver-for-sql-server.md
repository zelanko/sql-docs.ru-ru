...
|**Проверка** подлинности <a href="#table1_1"> <sup id="table1_authmode"> **1**</sup></a>|SSPROP_AUTH_MODE|Указывает используемую проверку подлинности SQL или Active Directory. Допустимые значения:<br/><ul><li>`(not set)`: Режим проверки подлинности, определенный другими ключевыми словами.</li><li>`ActiveDirectoryPassword:`Active Directory проверку подлинности с помощью идентификатора входа и пароля.</li><li>`ActiveDirectoryIntegrated:`Встроенная проверка подлинности для Active Directory с использованием учетных данных пользователя Windows, вошедшего в систему.</li><br/>**Примечание.** **Рекомендуется** , чтобы приложения, использующие `Integrated Security` ключевые слова `Trusted_Connection`проверки подлинности (или) или `Authentication` соответствующие свойства, установили для `ActiveDirectoryIntegrated` ключевого слова (или соответствующего свойства) значение, чтобы включить New поведение при проверке шифрования и сертификата.<br/><br/><li>`SqlPassword:`Проверка подлинности с использованием идентификатора входа и пароля.</li><br/>**Примечание.** **Рекомендуется** , чтобы приложения, использующие `SQL Server` проверку подлинности, установили для `Authentication` `SqlPassword` ключевого слова (или соответствующего свойства) значение, чтобы обеспечить новое шифрование и поведение при проверке сертификата.</ul>|
...
|Ключевое слово|Свойство инициализации|Описание|  
|-------------|-----------------------------|-----------------|  
|**Маркер доступа**<a href="#table2_1"><sup id="table2_accesstoken">**1**</sup></a>|SSPROP_AUTH_ACCESS_TOKEN|Маркер доступа, используемый для проверки подлинности в Azure Active Directory. <br/><br/>**Примечание.** Указание этого ключевого слова, а также `UID`ключевых слов строки подключения `Trusted_Connection`, `PWD`, `Authentication` или их соответствующих свойств или ключевых слов является ошибкой.|
...
|Ключевое слово|Свойство инициализации|Описание|  
|-------------|-----------------------------|-----------------|  
|**Маркер доступа**<a href="#table3_1"><sup id="table3_accesstoken">**1**</sup></a>|SSPROP_AUTH_ACCESS_TOKEN|Маркер доступа, используемый для проверки подлинности в Azure Active Directory.<br/><br/>**Примечание.** Указание этого ключевого слова, а также `UID`ключевых слов строки подключения `Trusted_Connection`, `PWD`, `Authentication` или их соответствующих свойств или ключевых слов является ошибкой.|
|**Application Intent**|SSPROP_INIT_APPLICATIONINTENT|Объявляет тип рабочей нагрузки приложения при соединении с сервером. Возможные значения — **ReadOnly** и **ReadWrite**.<br /><br /> Значение по умолчанию — **ReadWrite**. Дополнительные сведения о драйвере OLE DB для поддержки [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)]SQL Server см. в разделе [Драйвер OLE DB для SQL Server поддержки высокого уровня доступности и аварийного восстановления](../../oledb/features/oledb-driver-for-sql-server-support-for-high-availability-disaster-recovery.md).|  
|**Application Name**|SSPROP_INIT_APPNAME|Строка, идентифицирующая приложение.|  
