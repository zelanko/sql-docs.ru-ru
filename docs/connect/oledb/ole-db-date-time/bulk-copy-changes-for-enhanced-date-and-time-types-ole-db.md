...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  В этой статье описываются улучшения даты и времени для поддержки функций копирования в OLE DB драйвер для SQL Server.  
...
## <a name="character-data-files"></a>Файлы символьных данных  
 В файлах символьных данных значения даты и времени представлены, как описано в разделе "форматы данных: строки и литералы" статьи [Поддержка типов данных для OLE DB улучшений даты и времени](../../oledb/ole-db-date-time/data-type-support-for-ole-db-date-and-time-improvements.md) для OLE DB.  
...
## <a name="bcp-types-in-msoledbsqlh"></a>BCP Types in мсоледбскл. h  
 В мсоледбскл. h определены следующие типы. Эти типы передаются с помощью параметра *Еусердататипе* IBCPSession:: BCPColFmt в OLE DB.  
  
|Тип файла хранилища|Тип данных файла|Введите мсоледбскл. h для использования с IBCPSession:: BCPColFmt|Значение|  
|-----------------------|-------------------------|-----------------------------------------------------------|-----------|  
...
|9|Строка проходит синтаксический анализ и преобразуется в значение типа date, datetime, datetimeoffset или time в зависимости от первого встреченного знака препинания и наличия остальных компонентов. Затем строка преобразуется в целевой тип согласно правилам, описанным в таблице, приведенной в конце статьи, для типа исходных данных, который выясняется в процессе анализа. Если передаваемые данные не могут быть проанализированы без ошибок или какая-либо часть компонента находится за пределами допустимого диапазона или если преобразование из типа литерала в целевой тип отсутствует, то будет выдана ошибка. Для параметров DateTime и smalldatetime, если год находится за пределами диапазона, поддерживаемого этими типами, будет отправлена ошибка.<br /><br /> Значение datetimeoffset после преобразования во времени в формате UTC должно находиться в пределах диапазона, даже если преобразование во времени в формате UTC не требуется. Причина этого заключается в том, что поток табличных данных и сервер всегда нормализуют время в значениях datetimeoffset для времени в формате UTC, поэтому клиент должен проверять, что значение времени после преобразования во времени в формате UTC находится в пределах поддерживаемого диапазона. Если значение выходит за пределы поддерживаемого диапазона времени в формате UTC, то отправляется ошибка.|  
|10|При преобразовании «клиент-сервер» при усечении с потерей данных возникает ошибка. Эта ошибка также возникает в том случае, если значение выходит за пределы диапазона, который может быть представлен диапазоном времени в формате UTC, используемым сервером. Если в преобразовании с сервера на клиент происходит усечение секунд или долей секунд, выдается только предупреждение.|  
|11|При преобразовании «клиент-сервер» при усечении с потерей данных возникает ошибка.|
