...
description: Отправка данных BLOB на SQL Server с помощью IROWSETFASTLOAD и ISEQUENTIALSTREAM
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 Образец не использует потоковый интерфейс поставщика для записи данных в поставщик. Вместо этого образец передает указатель на объект потока, который поставщик будет использовать для чтения данных. Как правило поставщики Майкрософт (SQLOLEDB SQLNCLI и MSOLEDBSQL) будут читать данные 1024-байтовыми фрагментами из объекта, пока все данные были обработаны. SQLOLEDB, ни SQLNCLI ни MSOLEDBSQL располагают полными реализациями, чтобы позволить потребителю для записи данных в объект потока поставщика. Только данные нулевой длины могут быть переданы через объект потока поставщика.  
 ...
 Дополнительные сведения см. в разделе [большие двоичные объекты и объекты OLE](../../oledb/ole-db-blobs/blobs-and-ole-objects.md).  
