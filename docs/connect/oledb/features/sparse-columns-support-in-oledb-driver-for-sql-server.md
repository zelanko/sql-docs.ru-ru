...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 Дополнительные сведения о поддержке разреженных столбцов в драйвере OLE DB для SQL Server, [разреженные &#40;столбцы&#41;поддерживают OLE DB](../../oledb/ole-db/sparse-columns-support-ole-db.md).  
...
## <a name="user-scenarios-for-sparse-columns-and-ole-db-driver-for-sql-server"></a>Пользовательские сценарии для разреженных столбцов и драйвера OLE DB для SQL Server  
 В следующей таблице приведены общие пользовательские сценарии для драйвера OLE DB для SQL Server пользователей с разреженными столбцами:  
...
|Получение метаданных для всех столбцов в таблице с помощью набора строк схемы DBSCHEMA_COLUMNS без ограничений по столбцам (OLE DB).|Возвращает строку для всех столбцов, не входящих в **column_set**. Если таблица содержит разреженный **column_set**, для него будет возвращена строка.<br /><br /> Обратите внимание, что при этом не возвращаются метаданные для столбцов, являющихся элементами **column_set**.|  
|Получение данных для всех столбцов, независимо от разреженности или вхождения в **column_set**. В этом случае может вернуться очень большое число строк.|Вызовите метод IDBSchemaRowset:: GetSchema для набора строк схемы DBSCHEMA_COLUMNS_EXTENDED.|  
|Получение метаданных только для столбцов, являющихся элементами **column_set**. В этом случае может вернуться очень большое число строк.|Вызовите метод IDBSchemaRowset:: GetSchema для набора строк схемы DBSCHEMA_SPARSE_COLUMN_SET.|  
...
|Импорт и экспорт разреженных столбцов программой BCP для таблиц без **column_set**.|Не изменилось поведение предыдущих версий драйвера OLE DB для SQL Server.|  
