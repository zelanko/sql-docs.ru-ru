...
description: Использование зеркального отображения базы данных с драйвером OLE DB для SQL Server
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
> [!NOTE]  
>  Следует явно задать базу данных, которая будет использоваться в соединении, если эту возможность нужно использовать в DSN-именах, строке соединения, свойстве или атрибуте соединения. Драйвер OLE DB для SQL Server не будет пытаться выполнить отработку отказа в базу данных-партнер, если это не сделано.  
>   
...
## <a name="ole-db-driver-for-sql-server"></a>Драйвер OLE DB для SQL Server  
 ...
 Дополнительные сведения о драйвере OLE DB для SQL Server поддержки зеркального отображения базы данных см. в разделе [Свойства инициализации и авторизации](../../oledb/ole-db-data-source-objects/initialization-and-authorization-properties.md).  
