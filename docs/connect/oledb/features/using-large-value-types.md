...
> [!NOTE]  
>  Типы данных больших значений могут иметь максимальный размер от 1 KБ до 8 КБ или они могут быть указаны как бесконечное.  
  
 Ранее только [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] типы данных, такие как **текст**, **ntext** и **изображения** могли достигать такой длины. **Max** описатель **varchar**, **nvarchar**, и **varbinary** сделали эти типы данных избыточности. Тем не менее поскольку эти типы данных по-прежнему доступны, большинство интерфейсов для компонентов доступа к данным OLE DB оставаться неизменным. Для обеспечения обратной совместимости с использованием предыдущих версий флаг DBCOLUMNFLAGS_ISLONG в драйвер OLE DB для SQL Server продолжает использоваться. Поставщики и драйверы, предназначенные для [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] и более поздних версий, продолжают использовать эти термины для новых типов при задании неограниченной максимальной длины.  
  
> [!NOTE]  
>  Можно также указать **varchar(max)**, **nvarchar(max)**, и **varbinary(max)** типы возвращаемых значений типов данных, как типы входных и выходных параметров хранимых процедур, функция , или в [CAST и CONVERT](../../../t-sql/functions/cast-and-convert-transact-sql.md) функции.  
  
> [!NOTE]  
>  При репликации данных, может потребоваться настроить [max параметра конфигурации сервера text repl size](../../../database-engine/configure-windows/configure-the-max-text-repl-size-server-configuration-option.md) значение -1.  
...
## <a name="ole-db-driver-for-sql-server"></a>Драйвер OLE DB для SQL Server 
 ...
 Драйвер OLE DB для SQL Server также поддерживает привязку выходных параметров как DBTYPE_IUNKNOWN для типов данных больших значений, чтобы облегчить сценарии, где хранимая процедура возвращает типы больших значений в качестве возвращаемых значений, которые предоставляются как DBTYPE_IUNKNOWN.  
  
 Работать с этими типами приложение может следующими способами.  
  
-   Привязать как тип, имеющий поддерживаемые привязки с базовым типом столбца (например, для nvarchar(max) выполните привязку как тип, который может быть привязан к nvarchar). Если буфер недостаточно большой достаточно будет выполнено усечение — точно, как и для базового типа, хотя и, теперь доступны большие значения.  
...
-   Выполните привязку, указав тип DBTYPE_IUNKNOWN, и используйте потоковую передачу.  
 ...
 Когда сообщает максимальный размер столбца, получит драйвер OLE DB для SQL Server.  
