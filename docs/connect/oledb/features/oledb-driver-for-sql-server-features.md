---
title: Драйвер OLE DB для SQL Server компонентов | Документация Майкрософт
...
Обсуждаются новые методы проверки подлинности, появившиеся в OLE DB драйверов 18.2.1, которые имеют более безопасные параметры по умолчанию и позволяют подключаться к экземпляру базы данных SQL Azure с помощью федеративного удостоверения.
 ...
 Описывает, как драйвер OLE DB для SQL Server поддерживает режим MARS. Режим MARS позволяет выполнять и получать несколько результирующих наборов через одно подключение к базе данных.  
 ...
 Описывает, как драйвер OLE DB для SQL Server поддерживает уведомление потребителя об изменении набора строк.  
 ...
 Описывает, как использовать драйвер OLE DB для SQL Server шифрования данных, отправляемых на сервер, без проверки сертификата.  
 ...
 Описание драйвера OLE DB для SQL Server поддержки возвращающих табличное значение параметров.  
 ...
 Описание драйвера OLE DB для SQL Server поддержки расширенной функции FILESTREAM.  
 ...
 Описание драйвера OLE DB для SQL Server поддержки разреженных столбцов.  
  
 [Улучшения функций даты и времени](../../oledb/features/date-and-time-improvements.md)  
 Описание поддержки, добавленной в драйвер OLE DB для SQL Server типов данных даты и времени.  
 ...
 Обсуждается поддержка кодировки сервера UTF-8 и меры предосторожности при работе с данными в кодировке UTF-8.
 ...
 Описание драйвера OLE DB для SQL Server поддержки функции LocalDB.  
