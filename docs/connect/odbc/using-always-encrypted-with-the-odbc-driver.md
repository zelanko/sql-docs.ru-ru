...
> [!NOTE]
> Драйвер ODBC не поддерживает службы федерации Active Directory (AD FS) для аутентификации Azure Key Vault. Если вы используете проверку подлинности Azure Active Directory для Azure Key Vault и конфигурация Active Directory включает в себя службы федерации, проверка может завершиться ошибкой.
