...
Дополнительные сведения см. в разделе [Always encrypted (ядро СУБД)](../../relational-databases/security/encryption/always-encrypted-database-engine.md) и [Always encrypted с помощью Secure енклавес](../../relational-databases/security/encryption/always-encrypted-enclaves.md).
...
Если сервер и служба аттестации настроены правильно, а также анклава ключей CMK и Цекс для нужных столбцов, то теперь вы сможете выполнять запросы, использующие анклава, например шифрование на месте и расширенные вычисления, в дополнение к существующие функциональные возможности, предоставляемые Always Encrypted. Дополнительные сведения см. [в статье настройка Always encrypted с помощью Secure енклавес](../../relational-databases/security/encryption/configure-always-encrypted-enclaves.md) .
...
> [!NOTE]
> Запросы могут выполнять сравнения на равенство в зашифрованных столбцах только в том случае, если шифрование является детерминированным или включен безопасный анклава. Дополнительные сведения см. в разделе [Выбор детерминированного или случайного шифрования](../../relational-databases/security/encryption/always-encrypted-database-engine.md#selecting--deterministic-or-randomized-encryption).
...
|Имя|Тип|Описание|  
|----------|-------|----------|  
|`SQL_COPT_SS_COLUMN_ENCRYPTION`|Перед подключением|`SQL_COLUMN_ENCRYPTION_DISABLE` (0) — функция Always Encrypted отключена <br>`SQL_COLUMN_ENCRYPTION_ENABLE` (1) — функция Always Encrypted включена<br> Указатель на *тип*, строка*данных* (версия 17,4 и более поздние) включить с защитой анклава|
