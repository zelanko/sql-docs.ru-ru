---
title: Использование постоянного шифрования с драйвером ODBC для SQL Server | Документы Microsoft
ms.custom: ''
ms.date: 10/01/2018
ms.prod: sql
...
- 17 драйвер ODBC для SQL Server

### <a name="introduction"></a>Введение

Этой статье содержатся сведения о том, как разрабатывать приложения ODBC, с использованием [Always Encrypted (ядро СУБД)](../../relational-databases/security/encryption/always-encrypted-database-engine.md) и [драйвер ODBC для SQL Server](../../connect/odbc/microsoft-odbc-driver-for-sql-server.md).

Функция Always Encrypted позволяет шифровать конфиденциальные данные в клиентских приложениях, не раскрывая данные или ключи шифрования для SQL Server или Базы данных SQL Azure. Always Encrypted драйвер с поддержкой, такие как драйвер ODBC для SQL Server, реализует это за счет прозрачного шифрования и расшифровки конфиденциальных данных в клиентском приложении. Драйвер автоматически определяет, какие параметры запроса соответствуют важным столбцам базы данных (защищенным с помощью Always Encrypted), и шифрует значения этих параметров перед передачей данных в SQL Server или Базу данных SQL Azure. Аналогичным образом драйвер прозрачно расшифровывает данные, полученные из зашифрованных столбцов базы в результатах запроса. Дополнительные сведения см. в разделе [Always Encrypted (ядро СУБД)](../../relational-databases/security/encryption/always-encrypted-database-engine.md).
...
В этом разделе описывается оптимизация производительности, встроенные в драйвере ODBC для SQL Server и как можно контролировать влияние двух указанных выше факторов на производительность.
...
Чтобы уменьшить количество вызовов к хранилищу главных ключей столбцов для расшифровки ключей шифрования столбцов, драйвер кэширует столбца открытым текстом в памяти. Кэш CEK является глобальным для драйвера и не связанные с любой одно подключение. После получения ECEK из метаданных базы данных, драйвер сначала пытается найти соответствующий ключ CEK открытого текста зашифрованного значения ключа в кэше. Драйвер вызывает хранилищу ключей, содержащему CMK только в том случае, если не удается найти соответствующий ключ CEK открытого текста в кэше.

> [!NOTE]
> В драйвере ODBC для SQL Server после истечения времени ожидания двухчасовая вытесняются записей в кэше. Это означает, что для данного ECEK, драйвер обращается к хранилищу ключей только один раз в течение времени существования приложения или каждые два часа, какое значение меньше.

Начиная с 17,1 драйвер ODBC для SQL Server, время ожидания кэша CEK может настраиваться с помощью `SQL_COPT_SS_CEKCACHETTL` атрибут соединения, который указывает количество секунд, ключа CEK будет оставаться в кэше. Из-за особенностей глобального кэша этот атрибут может настраиваться из любого дескриптора соединения, допустимый для драйвера. Когда также извлекаются кэша, который TTL уменьшается, существующего столбца, в которых превысит новых истечения срока ЖИЗНИ. Если задано значение 0, без столбца не кэшируются.

### <a name="trusted-key-paths"></a>Доверенных путей ключа

Начиная с 17,1 драйвер ODBC для SQL Server, `SQL_COPT_SS_TRUSTEDCMKPATHS` атрибут соединения позволяет приложению требует постоянного шифрования операций только использования указанного списка ключей CMK, определяется их путей. По умолчанию этот атрибут имеет значение NULL, это означает, что драйвер принимает все пути к ключу. Чтобы использовать эту функцию, установите `SQL_COPT_SS_TRUSTEDCMKPATHS` для указания разделенных нулями, заканчивающаяся нулем строку расширенных символов, который содержит список допустимых путях ключа. Во время шифрования или расшифровки операций с помощью дескриптора соединения, в которой она задана---на котором драйвер будет проверьте пути CMK в соответствии с метаданные сервера регистр не учитывается в данном памяти, на который указывает этот атрибут должен оставаться действительным список. Если пути CMK отсутствует в списке, возникает сбой. Приложение может изменить содержимое памяти, которую этот атрибут указывает, изменить свой список надежных ключей CMK, без установки атрибута еще раз.
...
Драйвер ODBC для SQL Server содержит следующие поставщики встроенного хранилища ключей.

| Название | Описание | Имя поставщика (метаданные) |Доступность|
|:---|:---|:---|:---|
...
Хранилище ключей Azure удобно для хранения главных ключей столбцов для постоянного шифрования, особенно в том случае, если приложения размещены в Azure. Драйвер ODBC для SQL Server в Linux, macOS и Windows включает встроенный столбец поставщик хранилища главных ключей для хранилища ключей Azure. В разделе [хранилище ключей Azure — шаг за шагом](https://blogs.technet.microsoft.com/kv/2015/06/02/azure-key-vault-step-by-step/), [Приступая к работе с хранилищем ключей](https://azure.microsoft.com/documentation/articles/key-vault-get-started/), и [создании главных ключей столбцов в хранилище ключей Azure](https://msdn.microsoft.com/library/mt723359.aspx#Anchor_2) Дополнительные сведения о настройке ключ Azure Хранилище для постоянного шифрования.
...
При использовании **bcp** программы: элемент управления `ColumnEncryption` , используйте параметр -D и укажите имя источника данных, содержащая нужное значение. Чтобы вставить зашифрованных данных, убедитесь, `ALLOW_ENCRYPTED_VALUE_MODIFICATIONS` политика пользователя.

Ниже приводится сводка действий при работе в зашифрованном столбце:

|`ColumnEncryption`|Направление BCP|Описание|
|----------------|-------------|-----------|
|`Disabled`|OUT (клиенту)|Извлекает зашифрованного текста. Наблюдаемое тип данных **varbinary(max)**.|
|`Enabled`|OUT (клиенту)|Извлекает открытый текст. Драйвер будет расшифровывать данные столбца.|
|`Disabled`|IN (для сервера)|Вставляет зашифрованного текста. Этот способ предназначен для неявным образом, перемещение зашифрованных данных без необходимости его для расшифровки. Операция завершится ошибкой, если `ALLOW_ENCRYPTED_VALUE_MODIFICATIONS` параметр не задан для пользователя или BCPMODIFYENCRYPTED не задан для дескриптора соединения. Дополнительные сведения см. ниже.|
|`Enabled`|IN (для сервера)|Вставляет в виде обычного текста. Драйвер будет шифрование данных столбца.|

### <a name="the-bcpmodifyencrypted-option"></a>Параметр BCPMODIFYENCRYPTED

Чтобы предотвратить повреждение данных, сервер обычно не допускает вставку зашифрованных данных непосредственно в зашифрованный столбец, и таким образом сделать это будет невозможно; Однако для массовой загрузки с помощью API BCP, Настройка зашифрованных данных `BCPMODIFYENCRYPTED` [bcp_control](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-control.md) значение TRUE для параметра позволит зашифрованного текста, вставляемый непосредственно и снижает риск повреждения зашифрованные данные через параметр `ALLOW_ENCRYPTED_VALUE_MODIFICATIONS` параметр учетной записи пользователя. Тем не менее ключи должны соответствовать данные и рекомендуется для выполнения некоторых проверок только для чтения, вставляемых данных после массовой вставки и до использования в будущем.

В разделе [перенос конфиденциальных данных с помощью постоянного шифрования](../../relational-databases/security/encryption/migrate-sensitive-data-protected-by-always-encrypted.md) для получения дополнительной информации.
...
|`SQL_COPT_SS_CEKCACHETTL`|После подключения|[Набор символов] Задать срок ЖИЗНИ CEK кэша<br>[Get] Получить текущий кэш CEK TTL|
|`SQL_COPT_SS_TRUSTEDCMKPATHS`|После подключения|[Набор символов] Задать доверенных указатель пути CMK<br>[Get] Получение текущего указателя пути CMK доверенным|
...
|IPD-поле|Размер или тип|Значение по умолчанию|Описание|
|-|-|-|-|  
