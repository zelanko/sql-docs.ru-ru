---
title: Экран мастера источника данных 4 (драйвер ODBC для SQL Server) | Документация Майкрософт
ms.custom: ''
ms.date: 09/27/2017
ms.prod: sql
...
Указывает количество попыток неудачной попытки подключения.

### <a name="connect-retry-interval-seconds"></a>Интервал повтора подключения (в секундах)

Указывает количество секунд между попытками повторного подключения. Дополнительные сведения об этой операции и параметрах **числа повторных попыток подключения** см. [в разделе устойчивость подключений в драйвере Windows ODBC](../../../connect/odbc/windows/connection-resiliency-in-the-windows-odbc-driver.md).
...
Если сведения, указанные на этом экране, завершены, можно нажать кнопку **Готово**. ИМЯ DSN создается с использованием всех атрибутов, указанных на этом и других экранах мастера. Кроме того, вы получили возможность протестировать созданное имя DSN.
