...
- При использовании Azure Active Directory и пулов соединений с учетом драйверов, драйвер также использует значение проверки подлинности для определения членства в пуле соединений.
  
-   Организация пулов соединений с учетом драйвера предотвращает возвращение плохого соединения из пула.  
  
-   Организация пулов соединений с учетом драйвера распознает атрибуты соединения для конкретного драйвера. Таким образом, если соединение использует `SQL_COPT_SS_APPLICATION_INTENT` параметр только для чтения, это соединение получает собственный пул соединений.
-   `SQL_COPT_SS_ACCESS_TOKEN` Установка атрибута приводит к отдельному включению подключения в пул 
