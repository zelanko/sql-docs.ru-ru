...
|Имя поля|Описание|
|:--|:--|
|`Name`|Имя поставщика хранилища ключей. Он не должен быть таким же, как любой другой поставщик хранилища ключей, ранее загружены с помощью драйвера или присутствует в этой библиотеке. Строка расширенных символов*, заканчивающаяся нулем.|
...
|`ctx`|[Вход] Контекст операции.|
|`onError`|[Вход] Функция отчетов об ошибках.|
|`keyPath`|[Вход] Значение [KEY_PATH](../../t-sql/statements/create-column-master-key-transact-sql.md) атрибут метаданных для CMK ссылается заданный ECEK. Строка расширенных символов*, заканчивающаяся нулем. Это предназначено для определения CMK обрабатываемого этим поставщиком.|
|`alg`|[Вход] Значение [АЛГОРИТМ](../../t-sql/statements/create-column-encryption-key-transact-sql.md) атрибут метаданных для данного ECEK. Строка расширенных символов*, заканчивающаяся нулем. Это предназначено для идентификации алгоритм шифрования, используемый для шифрования заданного ECEK.|
