...
Начиная с версии 17,4, драйвер загружает OpenSSL динамически, что позволяет запускать его на системах, имеющих либо версии 1,0, либо 1,1, без отдельных файлов драйверов. При наличии нескольких версий OpenSSL драйвер пытается загрузить последнюю версию. Сейчас драйвер поддерживает OpenSSL 1.0. x и 1.1. x

> [!NOTE]  
> Возможный конфликт может возникнуть, если приложение, использующее драйвер (или один из его компонентов), связано с или динамически загружает другую версию OpenSSL. Если в системе имеется несколько версий OpenSSL и приложение использует ее, настоятельно рекомендуется, чтобы одна из них была тщательной, чтобы убедиться, что версия, загруженная приложением и драйвером, не соответствует, так как ошибки могут повредить память и, таким образом, не обязательно является манифестом очевидным образом или единообразно.
