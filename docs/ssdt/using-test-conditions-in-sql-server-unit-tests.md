---
title: Использование условий теста в модульных тестах SQL Server | Документация Майкрософт
ms.custom:
- SSDT
ms.date: 02/09/2017
ms.prod: sql
...
|----------------------|----------------------------------|  
|Контрольная сумма данных|Завершается сбоем, если контрольная сумма результирующего набора, возвращенного сценарием Transact\-SQL, не совпадает с ожидаемой суммой. Дополнительные сведения см. в разделе [Указание контрольной суммы данных](#SpecifyDataChecksum).<br /><br />**ПРИМЕЧАНИЕ.** Использование этого условия теста не рекомендуется, если каждый запуск теста возвращает разные данные. Например, если результирующий набор содержит формируемые даты, или отметки времени, или столбцы идентификаторов, тесты будут завершаться неудачей, так как при каждом запуске контрольная сумма будет разной.|  
...
Также можно напрямую редактировать код метода теста, что позволяет добавить дополнительную функциональность. Дополнительные сведения см. в разделе [Как открыть модульный тест SQL Server для его изменения](../ssdt/how-to-open-a-sql-server-unit-test-to-edit.md) и [Как написать модульный тест SQL Server, который выполняется в области действия одной транзакции](../ssdt/how-to-write-sql-server-unit-test-that-runs-in-single-transaction-scope.md). Например, функциональность к методу теста можно добавить с помощью инструкций Assert. Дополнительные сведения см. в статье [Использование операторов утверждений Transact-SQL в модульных тестах SQL Server](../ssdt/using-transact-sql-assertions-in-sql-server-unit-tests.md).  
...
5.  Укажите подключение к тестируемой базе данных. Дополнительные сведения см. в разделе [Как создать подключение к базе данных](https://msdn.microsoft.com/library/aa833420(VS.100).aspx).  
