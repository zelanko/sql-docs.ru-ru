...
_SSDT для Visual Studio 2017._
...
| Изменения | Сведения |
| :------- | :------ |
| Добавлен источник Power Query (предварительная версия) для SQL Server Integration Services в ADF 2017. | &nbsp; |
| Возвращена поддержка SQL Server 2012. | &nbsp; |
| Добавлены источник и назначение Oracle для SQL Server 2019. | Источник и назначение Oracle, предназначенные для SQL Server 2019, уже установлены в SSDT.<br/><br/>Чтобы создать пакет, предназначенный для SQL Server 2017 или более ранней версии, скачайте соединитель Oracle соответствующей версии с сайта загрузки корпорации Майкрософт и установите его на компьютере с SSDT. <br/>[Соединитель версии 5.0 для Oracle от Attunity (Microsoft), предназначенный для SQL 2Server 2017](https://www.microsoft.com/en-us/download/details.aspx?id=55179)<br/>[Соединитель версии 4.0 для Oracle от Attunity (Microsoft), предназначенный для SQL 2Server 2016](https://www.microsoft.com/en-us/download/details.aspx?id=52950)<br/>[Соединитель версии 3.0 для Oracle от Attunity (Microsoft), предназначенный для SQL 2Server 2014](https://www.microsoft.com/en-us/download/details.aspx?id=44582)<br/>[Соединитель версии 2.0 для Oracle от Attunity (Microsoft), предназначенный для SQL 2Server 2012](https://www.microsoft.com/en-us/download/details.aspx?id=29283)|
| Устранена проблема с загрузкой задачи или компонента скрипта при миграции с предыдущих версий служб SQL Server Integration Services. | &nbsp; |
| Устранена проблема, из-за которой средство просмотра данных не работало в Windows 7 с пакетом обновления 1 (SP1) и Windows 8.1. | &nbsp; |
| Устранена проблема, из-за которой в некоторых случаях сохранение пакета вызывало аварийное завершение работы Visual Studio. | &nbsp; |
| Устранена проблема, связанная с невозможностью выполнить пакет в некоторых случаях. | Эта проблема возникала, когда выполнялись оба таких условия:<br/><br/>&bull;&nbsp;Уровень защиты задан как EncryptSensitiveWithPassword.<br/>&bull;&nbsp;На целевом сервере используется версия, предшествующая SQL Server 2017. |
| Устранена проблема, из-за которой заметки со шрифтом по умолчанию не отображались в SSDT. | &nbsp; |
| ISDeploymentWizard поддерживает проверку подлинности SQL, встроенную проверку подлинности Azure Active Directory и проверку пароля Azure Active Directory в режиме командной строки. | &nbsp; |
| &nbsp; | &nbsp; |
...
- Формат нового проекта отчета не сохраняет привязку системы управления версиями и выдает ошибку примерно с таким сообщением:
   ...
   *Файл проекта C:\path не связан с системой управления версиями, но соответствующие привязки для него содержатся в решении.*
...
- После перевода проекта на новый формат MSBuild может не работать сохранение. При этом выдается примерно такое сообщение:
   ...
   *"Parameter "unevaluatedValue" cannot be null" (Параметр "unevaluatedValue" не может иметь значение NULL).*
...
_SSDT для Visual Studio 2015._
...
_SSDT для Visual Studio 2015._  
_Поддержка SQL 2Server вплоть до версии 2017._
...
_SSDT для Visual Studio 2015._  
_Поддержка SQL Server вплоть до версии 2016._
...
[Предыдущие выпуски SQL Server Data Tools (SSDT и SSDT-BI)](../ssdt/previous-releases-of-sql-server-data-tools-ssdt-and-ssdt-bi.md)  
[Новые возможности в ядре СУБД](https://msdn.microsoft.com/library/bb510411.aspx)  
