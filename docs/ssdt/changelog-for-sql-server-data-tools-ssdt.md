...
Этот журнал изменений предназначен для [SQL Server Data Tools (SSDT) для Visual Studio 2015](https://msdn.microsoft.com/library/mt204009.aspx).  
  
Подробные сообщения о том, что новые и измененные посетите [блог группы разработки SSDT](https://blogs.msdn.microsoft.com/ssdt/)
...
- Устраните проблемы с базами данных с помощью асимметричного ключа в HSM с помощью поставщика расширенного управления Ключами [элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3132749/sqlpackage-exe-fails-when-extracting-a-database-which-contains-an-asymmetric-key-using-an-ekm-provider)

## <a name="ssdt-170-supports-up-to-sql-server-2017"></a>17.0 SSDT (поддерживает не более 2017 г. SQL Server)
Номер сборки: 14.0.61704.140

### <a name="whats-new"></a>Новые возможности
**Проекты базы данных:**
- Изменение кластеризованного индекса на представлении больше не будет блокировать развертывание
- В строках сравнения схемы, относящихся к шифрования столбца, будут использоваться подходящие имена, а не имена экземпляров.   
- Для программы SqlPackage добавлен новый параметр командной строки — ModelFilePath.  Это позволяет опытным пользователям указать файл для импорта, публикации и операций со скриптами внешних model.xml   
- DacFx API была расширена для поддержки проверки подлинности Azure AD универсальных и многофакторная проверка подлинности (MFA)
...
- Проект служб SSIS теперь поддерживает версии целевого сервера «SQL Server 2017 г.» 
- Поддержка задач управления CDC, разделитель CDC и источник CDC при нацеливании 2017 г. SQL Server. 

**Проекты Analysis Services:**
- PowerQuery интеграции служб Analysis Services (1400 уровня совместимости табличные модели):
    - Режим DirectQuery доступен для SQL Oracle, Teradata и, если пользователь установил драйверы других производителей
    - Добавление столбцов в примере в PowerQuery
    - Параметры в моделях 1400 (уровне модели свойств, используемых подсистемой M), доступ к данным
        - Включение быстрого объединения (по умолчанию — false — если задано значение true, гибридное веб-приложение engine будет игнорировать уровни конфиденциальности источника данных при объединении данных)
        - Включить перенаправляет прежних версий (по умолчанию — false — если задано значение true, модуль гибридного веб-приложения будет следовать перенаправления HTTP, которые потенциально небезопасным.  Например, перенаправление из HTTPS в HTTP URI)  
        - Возвращает значения ошибки как Null (по умолчанию — false — если задано значение true, ошибки уровня строк возвращается как null. Если задано значение false, будет ли вызываться исключение является ячейка содержит ошибку)  
    - Дополнительные источники данных (файловый источник данных) с помощью PowerQuery
        - Excel 
        - Text/CSV 
        - Xml 
        - JSON 
    ...
    - Локализованные PowerQuery пользовательского интерфейса
- Окно инструментов редактора DAX
    - Улучшенная DAX редакторов для меры, вычисляемые столбцы и строки подробностей выражений, доступны через представления, меню другие окна в SSDT
    - Усовершенствования DAX parser\Intellisense
...
- Исправлены некоторые проблемы с усеченные строки пользовательского интерфейса, которые были доступны в некоторых локализованных сборок
- Исправлен ряд проблем с интеграцией PowerQuery в уровне совместимости 1400 как табличные модели
- Устранена проблема с шаблонами стиля мастера отчетов, не отображаются правильно
- Устранена проблема, с помощью мастера отчетов, которая может привести к параметры источника неверные данные, при переходе от SQL к AS
- Устранена проблема, вызов из командной строки (devenv.com\exe) Ошибка построения проекта служб Analysis Services (табличные данные)
- Исправлена проблема со средством синтаксического анализа меры DAX отображать текст выделяется и становится правильный цвет, когда начиная с буквы до: =
- Исправлена проблема с запуском ObjectRefException, если пути получил слишком много времени, попытка Показать все файлы для табличного проекта в режиме интеграции рабочей области
- Устранена проблема с помощью конструктора источников данных для поставщика данных клиента для Compact 4.0 вхождением недоступен
- Исправлена проблема, которая вызвала ошибку при попытке просмотреть как модель интеллектуального анализа данных в VS2017
- Устранена проблема в качестве многомерные модели в VS2017, где останавливает отрисовки после изменения представления диаграммы представления источника данных и затем обнаруживает исключение
- Устранена проблема Предварительный просмотр отчетов с подключением к AS, сбой в VS2017
...
- Устранена проблема систему задачу Hadoop файл TimeoutInMinutes по умолчанию равно 10, после открытия, сохранить пакет служб SSIS и во время выполнения.
...
## <a name="ssdt-165-supports-up-to-sql-server-2016"></a>SSDT 16,5 (поддержка до SQL Server 2016)
