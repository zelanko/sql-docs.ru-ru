---
title: "Системные параметры (службы Master Data Services) | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "службы Master Data Services, системные параметры"
  - "системные параметры [службы Master Data Services]"
...
|**Интервал промежуточного хранения пакетов**|**StagingBatchInterval**|В функциональной области **Управление интеграцией** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] определяется период (в секундах) после выбора параметра **Запуск пакетов**, в течение которого будет обрабатываться пакет. Значение по умолчанию — **60** секунд (1 минута).|  
...
|**Отображать имена в иерархии по умолчанию**|**ShowNamesInHierarchy**|Определяет в функциональной области **Обозреватель** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] параметр по умолчанию, выбираемый при просмотре иерархий.<br /><br /> Значение по умолчанию — **Да** или **1**, что задает отображение имени и кода каждого элемента. Измените значение на **Нет** или **2**, чтобы отображать только код.|  
|**Количество атрибутов на основе домена в списке**|**DBAListRowLimit**|Определяет в функциональной области **Обозреватель** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] количество атрибутов, отображаемых в списке при двойном щелчке в сетке значений атрибутов на основе домена. Значение по умолчанию — **50**. Если имеется более 50 элементов, вместо этого отображается диалоговое окно с возможностью поиска.|  
||**GridFilterDefaultFuzzySimilarityLevel**|Определяет функциональной области **Обозреватель** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] уровень подобия, применяемый при использовании условия фильтра **Совпадает**. Значение по умолчанию — **0,3**. При выборе значения ближе к **1** возвращаются элементы, более соответствующие указанным критериям. Задайте значение **1** для точного сопоставления.|  
...
|Параметр диспетчера конфигурации|Системный параметр|Description|  
|-----------------------------------|--------------------|-----------------|  
|**Шаг приращения для новых бизнес-правил**|**BusinessRuleDefaultPriorityIncrement**|Определяет в функциональной области **Администрирование системы** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] шаг приращения для приоритетов всех новых бизнес-правил. Значение по умолчанию ― **10**.|  
|**Количество элементов, к которым применяются бизнес-правила**|**BusinessRuleRealtimeMemberCount**|Определяет в функциональной области **Обозреватель** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] максимальное количество элементов в сетке, к которым могут применяться бизнес-правила. В [!INCLUDE[ssMDSXLS](../includes/ssmdsxls-md.md)] — максимальное число элементов на активном листе, к которым применяется бизнес-правило. Значение по умолчанию — **10000**.|  
...
|**Регулярное выражение для адреса электронной почты**|**EmailRegExPattern**|Определяет в функциональной области **Разрешения пользователей и групп** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] регулярное выражение, которое служит для проверки адреса электронной почты, введенного на вкладке **Общие**. Дополнительные сведения о регулярных выражениях см. в разделе [Элементы языка регулярных выражений](http://go.microsoft.com/fwlink/?LinkId=164401) библиотеки MSDN.|  
...
|Параметр диспетчера конфигурации|Системный параметр|Description|  
|-----------------------------------|--------------------|-----------------|  
||**SecurityMemberProcessInterval**|Определяет в функциональной области **Разрешения пользователей и групп** [!INCLUDE[ssMDSmdm](../includes/ssmdsmdm-md.md)] частоту (в секундах) применения разрешений пользователей и групп, задаваемых на вкладке **Элементы иерархии**. Значение по умолчанию — **3600** секунд (60 минут).|  
