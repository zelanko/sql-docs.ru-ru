...
2.  Найдите раздел **mdsWsHttpBehavior** в **\<serviceBehaviors>**. Для ** \<сервисаMetadata>** элемент, установить **httpGetEnabled** к **истине**.  
  
    > [!NOTE]  
    >  Если вы хотите включить Web-сервисы по безопасности транспортного уровня (TLS), ранее известный как Безопасные разъемы слоя (SSL), установить **httpsGetEnabled** к **истине** в **разделе mdsWsHttpBehavior** файла web..config. Вам также нужно изменить **mdsWsHTTPBinding** так, чтобы он был настроен для TLS, а также, и прокомментировать не-TLS разделе.  
