...
1. [Установите и настройте отработку отказа Windows Server кластера (WSFC)](#windows-server-failover-cluster-wsfc).

2. [Настройка группы доступности Always on](#sql-server-always-on-availability-group).

3. [Настройка служб MDS для запуска на узле WSFC](#configure-mds-to-run-on-an-wsfc-node).
...
Рисунок 1. Типичная конфигурация группы доступности Always On
...
1. [Установите компонент отказоустойчивого кластера Windows](#install-failover-cluster-feature).
...
1. [Включите SQL Server Always on группу доступности](#enable-sql-server-always-on-availability-groups-on-every-sql-server-instance).
...
- Установите SQL Server на всех узлах. Дополнительные сведения см. в статье [Установка SQL Server 2016](../../database-engine/install-windows/install-sql-server.md).
...
**Синхронная фиксация**. Это гарантирует, что если транзакция фиксируется на первичной реплике базы данных, то транзакция также фиксируется на всех других синхронных репликах. Асинхронная фиксация это не гарантирует, поэтому вторичная реплика может отставать от первичной.
...
**Автоматический переход на другой ресурс:** Когда первичная реплика не работает, Группа доступности автоматически отключается на вторичную реплику, если выбран автоматический переход на другой ресурс. Этот параметр можно включить только для реплик с синхронной фиксацией.

**Вторично доступный для чтения:** По умолчанию пользователи не могут подключаться к вторичным репликам. Этот параметр позволяет пользователям подключиться к вторичной реплике с доступом только для чтения.
...
9. На странице **Выбор синхронизации данных** установите флажок **Полная** и укажите общую сетевую папку, доступную для каждого узла. Для продолжения нажмите кнопку **Далее**. См. рис. 19.
...
10. На странице **Проверка** убедитесь, что все проверки успешно пройдены, и исправьте все ошибки. Для продолжения нажмите кнопку **Далее**.
...
Оказался ли этот документ полезным для Вас? Чтобы оставить отзыв, щелкните ссылку **Комментарии** в верхней части статьи. 
