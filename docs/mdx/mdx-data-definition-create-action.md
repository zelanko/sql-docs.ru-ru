...
> [!NOTE]  
>  Данная инструкция включена для обеспечения обратной совместимости. Действия, не [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]поддерживаемые, например детализация или действия с отчетами, не поддерживаются.  
...
|-----------------|-----------------|  
|**URL-адрес**|Возвращаемая строка действия представляет собой URL-адрес, который можно открыть с помощью интернет-обозревателя.<br /><br /> Примечание. Если это действие не начинается `https://` с или `https://`, действие будет недоступно для браузера, если для **сафетйоптионс** не задано значение **DBPROPVAL_MSMD_SAFETY_OPTIONS_ALLOW_ALL**.|  
...
|**STATEMENT**|Возвращаемая строка действия — это инструкция, которая должна быть выполнена путем установки метода **ICommand:: SetText** объекта Command в строку и вызова метода **ICommand:: Execute**. Если команда завершится неудачно, будет возвращено сообщение об ошибке.|  
