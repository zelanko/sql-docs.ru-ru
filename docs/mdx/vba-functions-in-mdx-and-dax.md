---
title: Функции VBA в DAX и многомерных Выражениях | Документация Майкрософт
ms.date: 06/04/2018
ms.prod: sql
...
ms.locfileid: "63251548"
---
# <a name="vba-functions-in-mdx-and-dax"></a>Функции VBA в DAX и многомерных выражениях


  Этот документ содержит перекрестного ссылки на все функции VBA, доступные в [Visual Basic для приложений-функций](https://msdn.microsoft.com/vba/language-reference-vba/articles/functions-visual-basic-for-applications) , которые поддерживаются в многомерных Выражениях; Кроме того, список включает примечание при функциональной эквивалентности с языком DAX .  
...
|Filter|Не поддерживается|**Предупреждение** многомерных Выражений реализована другая функция с тем же именем; функция FILTER (Set_Expression, Logical_Expression) возвращает набор, получающийся в результате фильтрации заданного набора на основе условия поиска по заданным аргументам<br /><br /> **Предупреждение** DAX реализована другая функция с тем же именем; фильтр (\<таблицы >,\<фильтра >) функция возвращает таблицу, представляющую подмножество другой таблицы или выражения по заданным аргументам|  
...
|Журнал|Только многомерные выражения|**Важные** DAX реализована другая функция с тем же именем; функция LOG (number, base). Возвращает логарифм числа по основанию, указанному в заданных аргументах.|  
