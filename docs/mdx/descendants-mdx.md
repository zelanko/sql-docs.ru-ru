---
title: Descendants (многомерные Выражения) | Документация Майкрософт
ms.date: 06/04/2018
ms.prod: sql
...
## <a name="remarks"></a>Примечания  
 Если уровень указан, **потомков** функция возвращает набор, содержащий потомки заданного элемента или элементы заданного набора на указанном уровне, выборочно изменяемого при помощи флага, определенного в  *Desc_Flag*.  
  
 Если *расстояние* указано, **потомков** функция возвращает набор, содержащий потомки заданного элемента или элементы заданного набора, заданное количество уровней, сейчас в иерархии указанного элемента, выборочно изменяемого при помощи флага, указанного в *Desc_Flag*. Обычно эта функция используется с аргументом Distance для распределения неоднородных иерархий. Если заданное расстояние равно нулю, то функция возвращает набор, состоящий только из заданного элемента или заданного набора.  
  
 Если указано выражение набора, **потомков** функция решается отдельно для каждого элемента набора и набор создается заново. Другими словами, синтаксис, используемый для **потомков** является функциональным эквивалентом многомерных Выражений функция [формирования](../mdx/generate-mdx.md) функции.  
  
 Если указан уровень или расстояние, значение по умолчанию для уровня, используемые функцией определяется путем вызова [уровень](../mdx/level-mdx.md) функция (<\<член >>. Уровня) для указанного элемента (если элемент определен) или путем вызова **уровень** функцию для каждого элемента заданного набора (если набор определен). Если выражение уровня, расстояние и флаги не определены, то функция будет выполнена только в случае использования следующего синтаксиса:  
...
```  
 ...
 В следующем примере возвращается среднее ежедневное значение `Measures.[Gross Profit Margin]` мер, вычисляется по всем дням всех месяцев 2003 финансового года, на основе **Adventure Works** куба. **Потомков** функция возвращает набор дней, определяемый по отношению к текущему элементу из `[Date].[Fiscal]` иерархии.  
