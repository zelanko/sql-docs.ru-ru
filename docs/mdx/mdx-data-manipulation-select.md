...
## <a name="remarks"></a>Примечания  
 ...
 Вычисляемые элементы могут быть включены в \<предложение осей запроса SELECT > каждый раз, когда соединение открыто с помощью параметра строки подключения *вложенные запросы = 1*; см. в разделе [поддерживаемые свойства XMLA &#40; XML для Аналитики&#41; ](https://docs.microsoft.com/bi-reference/xmla/xml-elements-properties/propertylist-element-supported-xmla-properties) и <xref:Microsoft.AnalysisServices.AdomdClient.AdomdConnection.ConnectionString%2A> сведения об использовании параметра. Пример приведен для вычисляемых элементов в подзапросах выборки.  
...
```  
 ...
 Чтобы понять **NON VISUAL** приведенный ниже приведен запрос к [Adventure Works] для получения [Reseller Sales Amount] в таблице, где категории продуктов являются столбцами, а типы торговых посредников являются строками. Обратите внимание, что итоги даны и для продуктов, и для торговых посредников.  
...
|Warehouse|$ 38 726 913,48|$ 331 169,64|$ 932 521,23|  
 ...
 В следующем примере показано применение вычисляемых элементов в подзапросах выборки для фильтрации по ним. Чтобы иметь возможность воспроизвести этот пример, необходимо установить соединение с помощью параметра строки подключения *вложенные запросы = 1*.  
