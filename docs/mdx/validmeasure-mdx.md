 ...
 **ValidMeasure** функция возвращает значение кортежа, игнорируя атрибуты, которые не имеют связи с группой мер данной меры, значение которого возвращает кортеж. Атрибут может быть не связан с мерой по двум следующим причинам.  
...
-   Измерение атрибута не имеет связи с группой мер данной меры, но атрибут гранулярности не является ключевым атрибутом и не имеет прямой связи с атрибутом в кортеже.  
 ...
 Поведение, заданное этой функцией, является поведением по умолчанию на сервере и управляются **IgnoreUnrelatedDimensions** свойство объекта группы мер.  
