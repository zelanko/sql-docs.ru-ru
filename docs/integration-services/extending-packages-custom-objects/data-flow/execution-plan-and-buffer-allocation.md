---
title: "План выполнения и выделения буфера | Документы Microsoft"
ms.custom: 
ms.date: 03/04/2017
...
## <a name="understanding-the-execution-plan"></a>Основные сведения о плане выполнения  
 План выполнения содержит потоки источника и рабочие потоки. Каждый поток содержит списки действий, которые задают списки действий над выходными данными для потоков источника или списки действий над входными и выходными данными для рабочих потоков. Потоки источника в план выполнения представляют компоненты источника в потоке данных и идентифицированы в плане выполнения *SourceThread**n*, где  *n*  с нуля число потоков источника.  
 ...
 Рабочий поток может содержать обоих списков рабочих ввода-вывода и идентифицируется в плане выполнения как *WorkThread**n*, где  *n*  с нуля число рабочих потоков. Эти потоки содержат списки действий над выходными данными, если граф содержит компонент с асинхронными выходами.  
...
> [!NOTE]  
>  План выполнения создается при каждом выполнении пакета и могут собираться при добавлении регистратора к пакету, включив ведение журнала и выбрав **PipelineExecutionPlan** событий.  
...
## <a name="understanding-buffer-allocation"></a>Основные сведения о выделении буферов  
 ...
 Так как буфер, предоставленный компоненту может содержать больше столбцов, чем компонент в его коллекциях входных и выходных столбцов, разработчики компонентов могут вызывать <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSBufferManager100.FindColumnByLineageID%2A> метод, чтобы определить положение столбца в буфер, указав его **LineageID**.  
