...
[!INCLUDE[ssis-appliesto](../../includes/ssis-appliesto-ssvrpluslinux-asdb-asdw-xxx.md)]
 ...
 Во время разработки в среде [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)] можно использовать точки останова выполнения пакета, окно хода выполнения и средства просмотра данных. Точки останова позволяют отслеживать данные по мере их прохождения через поток данных. Однако эти возможности недоступны при выполнении развернутых пакетов. Основными методами устранения неполадок в развернутых пакетах являются следующие.  
...
## <a name="catch-and-handle-package-errors-by-using-event-handlers"></a>Перехват и обработка ошибок пакетов с помощью обработчиков событий  
 Используйте обработчики событий для реагирования на события, вызываемые пакетами и объектами пакетов.  
  
-   **Создайте обработчик события для события OnError**. В обработчике событий можно использовать задачу "Отправить сообщение" для уведомления администратора о сбое. Используйте задачу "Скрипт" и пользовательскую логику, чтобы получить системные сведения для устранения неполадок или очистки временных ресурсов и неполных выходных данных. Дополнительные сведения см. в разделе [Обработчики событий в службах Integration Services (SSIS)](../../integration-services/integration-services-ssis-event-handlers.md).  
...
-   **Добавляйте дополнительные сведения к выводу ошибок**. Чтобы лучше понимать возникшие ошибки, добавьте сообщение об ошибке и имя столбца к двум цифровым идентификаторам, которые по умолчанию предоставляет вывод ошибок. Пример того, как с помощью сценариев добавить эти дополнительные столбцы, есть в статье [Расширение вывода ошибок с помощью компонента скрипта](../../integration-services/extending-packages-scripting-data-flow-script-component-examples/enhancing-an-error-output-with-the-script-component.md).  
...
## <a name="troubleshoot-package-execution-by-using-logging"></a>Устранение неполадок выполнения пакетов с помощью журналов  
 С помощью ведения журналов можно отследить многие события в выполняющихся пакетах. Регистраторы захватывают сведения о конкретных событиях с целью последующего анализа и сохраняют эту информацию в одном из поддерживаемых форматов.  Поддерживаемые форматы регистратора включают таблицу базы данных, неструктурированный файл, XML-файл.  
...
-   **Настройте свойство DelayValidation для известных недопустимых элементов пакета**. Чтобы предотвратить ошибки проверки для элементов пакета с известными проблемами конфигурации, задайте для свойства **DelayValidation** значение **true**. Например, пакет содержит задачу потока данных, использующую целевую таблицу, которая не существует до тех пор, пока задача "Выполнение SQL" не создаст ее во время выполнения. Свойство **DelayValidation** можно включить на уровне пакета или на уровне отдельных задач или контейнеров, содержащихся в пакете.  
...
## <a name="troubleshoot-errors-without-a-description"></a>Устранение неполадок, когда ошибка выдается без описания  
 Если ошибка служб [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] выдается без описания, это описание можно найти в [Справочнике по сообщениям об ошибках служб Integration Services](../../integration-services/integration-services-error-and-message-reference.md). Этот список пока не включает сведения об устранении неполадок.  
