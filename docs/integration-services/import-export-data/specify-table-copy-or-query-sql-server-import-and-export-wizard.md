---
title: "Выбор копирования таблицы или запроса (мастер экспорта и импорта SQL Server) | Документы Microsoft"
ms.custom: 
ms.date: 02/17/2017
...
-   **Скопировать данные из одной или нескольких таблиц или представлений**. Нужно выбрать таблицу или таблицы из списка.
-   **Написать запрос, указывающий данные для передачи**. Необходимо ввести или вставить в текст SQL-запроса.
...
## <a name="screen-shot-of-the-specify-table-copy-or-query-page"></a>Снимок экрана: страница "Выбор копирования таблицы или запроса"    
 ...
 ![Таблица копирования или запроса страницы из мастера импорта и экспорта](../../integration-services/import-export-data/media/table-copy-or-query.png "таблицы копирования или запроса страницы из мастера импорта и экспорта")    
 ...
 **Скопировать данные из одной или нескольких таблиц или представлений**    
 Выберите этот параметр, если требуется скопировать данные из источника без фильтрации и сортировки записей.   
...
Вы можете продолжать использовать **написать запрос** параметр, даже если не надо писать обычно SQL-запросов, при условии, что известно имя таблицы, которую требуется экспортировать. В **определение исходного запроса** dialog box, которое появляется после щелкните **Далее**, введите запрос, в виде `SELECT * FROM <name of table>`. Если имя таблицы содержит пробелы или другие специальные символы, заключите имя в квадратные скобки - `SELECT * FROM [<name of table>]`.

### <a name="more-info"></a>Дополнительные сведения
 Параметр **Скопировать данные из одной или нескольких таблиц или представлений** доступен только для поставщиков, у которых имеется раздел ProviderDescription в файле ProviderDescriptors.xml. (По умолчанию этот файл расположен в \< *диск*>: \Program Files\Microsoft SQL Server\130\DTS\ProviderDescriptors.) Каждый раздел ProviderDescription в этом файле содержит сведения, необходимые для получения метаданных от соответствующего поставщика.    
