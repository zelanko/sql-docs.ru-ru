...
Запуск [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] мастер импорта и экспорта в один из способов, описанных в этом разделе для импорта данных из и экспортировать данные любого поддерживаемого источника данных.

> [!IMPORTANT]
> В этом разделе описывается только **запуск** мастера. Если вам нужны для других целей, см. раздел [задач и содержимого, связанные с](#related).
...
1.  На **запустить** меню найдите и разверните **Microsoft SQL Server 2016**.
...
### <a name="start-the-sql-server-import-and-export-wizard-from-sql-server-management-studio-ssms"></a>Запустить мастер экспорта и импорта SQL Server в SQL Server Management Studio (SSMS)    
