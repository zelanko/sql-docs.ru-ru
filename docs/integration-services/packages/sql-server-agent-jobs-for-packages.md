...
-   Видеоролик [Устранение неполадок. Выполнение пакетов с помощью агента SQL Server (видеоматериал SQL Server)](https://go.microsoft.com/fwlink/?LinkId=141772)в библиотеке MSDN.  
...
7.  В списке **Запуск от имени** выберите **Учетная запись службы агента SQL Server** или выберите учетную запись-посредник с учетными данными, которые будут использоваться шагом задания. Дополнительные сведения о создании учетной записи-посредника см. в разделе [Create a SQL Server Agent Proxy](../../ssms/agent/create-a-sql-server-agent-proxy.md).  
    ...
    > **ПРИМЕЧАНИЕ.** При изменении пароля для учетных данных, используемых учетной записью-посредником, необходимо обновить пароль учетных данных. В противном случае выполнение шага задания завершится ошибкой.  
     ...
     Дополнительные сведения о настройке учетной записи службы агента SQL Server см. в разделе [Назначение стартовой учетной записи службы для агента SQL Server (диспетчер конфигурации SQL Server)](https://msdn.microsoft.com/library/46ffe818-ebb5-43a0-840b-923f219a2472).  
...
-   Видеоролик [Как автоматизировать выполнение пакета с помощью агента SQL Server (видеоматериал SQL Server)](https://go.microsoft.com/fwlink/?LinkId=141771)в библиотеке MSDN.  
