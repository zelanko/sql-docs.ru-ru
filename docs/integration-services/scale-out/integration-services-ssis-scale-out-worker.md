...
Рабочая роль Scale Out запускает службу рабочей роли [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion_md](../../includes/ssisnoversion-md.md)] Scale Out для получения задач выполнения из мастера Scale Out и выполняет пакеты локально с помощью ISServerExec.exe.
...
TaskRequestMaxCPU|Верхний предел ресурсов ЦП для запроса задач рабочей ролью масштабного развертывания. **НЕ ИСПОЛЬЗУЕТСЯ в [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 2017.**|70.0         
TaskRequestMinMemory|Нижний предел объекта памяти в МБ для запроса задач рабочей ролью масштабного развертывания. **НЕ ИСПОЛЬЗУЕТСЯ в [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 2017.**|100.0         
...
ExecutionLogRetryTimeout|Время ожидания повторных попыток при сбое ведения журнала выполнения. Значение ExecutionLogRetryCount игнорируется, если достигнуто значение ExecutionLogRetryTimeout.|7.00:00:00 (7 дней)        
...
Файл журнала службы рабочей роли Scale Out находится в папке \<диск\>:\Users\\*[учетная запись]*\AppData\Local\SSIS\ScaleOut\Agent.

Расположение журнала для каждой отдельной задачи настраивается в файле WorkerSettings.config процессом TasksRootFolder. Если значение не указано, журнал помещается в папку \<диск\>:\Users\\*[учетная запись]*\AppData\Local\SSIS\ScaleOut\Tasks. 

Папка *[учетная запись]* соответствует учетной записи, с помощью которой выполняется служба рабочей роли Scale Out. По умолчанию используется учетная запись SSISScaleOutWorker140.
