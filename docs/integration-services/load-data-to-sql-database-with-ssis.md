---
title: Загрузка данных в Базу данных SQL Azure с помощью SQL Server Integration Services (SSIS) | Документация Майкрософт
description: Демонстрация создания пакета SQL Server Integration Services (SSIS) для перемещения данных из разнообразных источников в Базу данных SQL Azure.
...
ms.locfileid: "40175030"
---
# <a name="load-data-into-azure-sql-database-with-sql-server-integration-services-ssis"></a>Загрузка данных в Базу данных SQL Azure с помощью SQL Server Integration Services (SSIS)

Вы можете создать пакет служб SQL Server Integration Services (SSIS) для загрузки данных в [Базу данных SQL Azure](/azure/sql-database/). При необходимости можно реструктуризировать, преобразовать и очистить данные по мере их прохождения через поток данных SSIS.
...
4. **База данных SQL Azure и разрешения**. В этом руководстве выполняется подключение к экземпляру Базы данных SQL и загрузка в него данных. У вас должны быть разрешения на подключение, создание таблицы и загрузку данных.
5. **Образец данных**. В качестве исходных данных, загружаемых в Базу данных SQL, руководство использует образцы, хранимые в SQL Server в примере базы данных AdventureWorks. [Скачать образцы баз данных AdventureWorks][AdventureWorks 2014 Sample Databases].
6. **Правило брандмауэра**. Перед загрузкой в Базу данных SQL вам нужно создать для нее правило брандмауэра с IP-адресом локального компьютера.
...
4. В списке "Другие назначения" на панели элементов перетащите назначение ADO.NET в область конструктора под источником ADO.NET. Пока адаптер загрузки данных выбран, измените его имя на **SQL DB destination** (Назначение Базы данных SQL) в панели **Свойства**.
...
1. Дважды щелкните адаптер загрузки данных, чтобы открыть **Редактор назначения ADO.NET**.
    ...
    ![][11]
2. На вкладке **Диспетчер соединений** окна **Редактор назначения ADO.NET** нажмите кнопку **Создать** рядом со списком **Диспетчер соединений**, чтобы открыть диалоговое окно **Настройка диспетчера соединений ADO.NET** и создать параметры подключения для Базы данных SQL Azure, куда в этом руководстве будут загружены данные.
3. В диалоговом окне **Настройка диспетчера соединений ADO.NET** нажмите кнопку **Создать**, чтобы открыть диалоговое окно **Диспетчер соединений** и создать подключение к данным.
4. В диалоговом окне **Диспетчер соединений** сделайте следующее:
   1. В поле **Поставщик** выберите поставщик данных SqlClient.
   2. В поле **Имя сервера** введите имя Базы данных SQL.
   ...
   4. В разделе **Соединение с базой данных** выберите существующую Базу данных SQL.
   5. Нажмите кнопку **Проверить соединение**.
   6. В диалоговом окне, сообщающем результаты проверки соединения, нажмите кнопку **ОК**, чтобы вернуться в диалоговое окно **Диспетчер соединений**.
   7. В диалоговом окне **Диспетчер соединений** нажмите кнопку **ОК** для возврата в диалоговое окно **Настройка диспетчера соединений ADO.NET**.
...
Поздравляем! Вы успешно загрузили данные в Базу данных SQL Azure, используя службы SQL Server Integration Services.
