---
title: "Работа с образами с помощью задачи «скрипт» | Документы Microsoft"
ms.custom: 
ms.date: 03/16/2017
...
# <a name="working-with-images-with-the-script-task"></a>Работа с изображениями в задаче «Скрипт»
  База данных продуктов или пользователи часто содержат изображения в дополнение к тексту и числовым данным. **System.Drawing** пространство имен в платформе Microsoft .NET Framework предоставляет классы для управления изображениями.  
  
 [Пример 1: Преобразование изображения в формате JPEG](#example1)  
...
##  <a name="example1"></a>Пример 1. Описание: Преобразование изображений в формат JPEG  
...
3.  В проекте скрипта, необходимо задать ссылку на **System.Drawing** пространства имен.  
  
4.  В коде, используйте **Imports** инструкции, чтобы импортировать **System.Drawing** и **System.IO** пространства имен.  
...
2.  На **коллекции** страница **Редактор циклов по каждому элементу**выберите **файла перечислитель** как перечислитель и укажите путь и маску источника файлы: как «*.bmp».  
  
3.  На **сопоставления переменной** сопоставьте `CurrentImageFile` переменных с индексом 0. Эта переменная передает текущее имя файла в задачу «Скрипт» на каждом проходе перечислителя.  
...
4.  В проекте скрипта, необходимо задать ссылку на **System.Drawing** пространства имен.  
  
5.  В коде, используйте **Imports** инструкции, чтобы импортировать **System.Drawing** и **System.IO** пространства имен.  
  
#### <a name="to-configure-this-script-task-example-for-use-with-multiple-image-files"></a>Настройка этого примера задачи «Скрипт» для работы с несколькими файлами изображения  
  
1.  Поместите задачу «Скрипт» в контейнер «цикл по каждому элементу».  
  
2.  На **коллекции** страница **Редактор циклов по каждому элементу**выберите **файла перечислитель** как **перечислитель**и укажите путь и маску исходных файлов, например «*.jpg».  
