---
title: "Главная роль масштабного развертывания служб Integration Services (SSIS) | Microsoft Docs"
ms.custom: ""
ms.date: "12/16/2016"
...
---
# Главная роль масштабного развертывания служб Integration Services (SSIS)
...
Каталог SSISDB хранит все данные для рабочих ролей масштабного развертывания, пакетов и выполнений. Он обеспечивает интерфейс для использования рабочей роли и выполнения пакетов в масштабном развертывании. Дополнительные сведения см. в статьях [Пошаговое руководство. Настройка масштабного развертывания Integration Services](../integration-services/walkthrough-set-up-integration-services-scale-out.md) и [Выполнение пакетов в службах Integration Services](../integration-services/run-packages-in-integration-services-ssis-scale-out.md).
...
## <a name="scale-out-related-sql-views-and-stored-procdures-in-ssisdb"></a>Связанные с масштабным развертыванием хранимые процедуры и представления SQL в SSISDB
...
InstanceName|Имя экземпляра [!INCLUDE[ssNoVersion_md](../includes/ssnoversion-md.md)], содержащего каталог SSISDB. MSSQLSERVER — имя экземпляра [!INCLUDE[ssNoVersion_md](../includes/ssnoversion-md.md)] по умолчанию. |Имя экземпляра SQL Server, устанавливаемого вместе с главной ролью масштабного развертывания         
...
Файл журнала службы главной роли масштабного развертывания находится в папке \<диск\>:\Users\\*[учетная_запись]*\AppData\Local\SSIS\Cluster\Master. 

Папка *[учетная_запись]* соответствует учетной записи, с помощью которой выполняется служба главной роли масштабного развертывания. По умолчанию этой учетной записью является SSISScaleOutMaster140.