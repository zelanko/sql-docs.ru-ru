...
> [!IMPORTANT]
> Описанная в этой статье процедура, использующая библиотеку `DtsClient`, применима только к пакетам, которые развернуты с помощью модели развертывания пакетов (то есть с параметром `/SQL`, `/DTS` или `/File`). Она не применима к пакетам, которые развернуты с помощью модели развертывания сервера (то есть с параметром `/ISServer`). Для использования выходных данных локального пакета, развернутого с помощью модели развертывания сервера (то есть с параметром `/ISServer`), вместо описанной в этой статье процедуры используйте [назначение потоковой передачи данных](../data-flow/data-streaming-destination.md).
