---
title: "Определение переменной состояния | Документы Microsoft"
ms.custom: 
ms.date: 03/01/2017
...
|---------------|-----------------|  
|**\<Имя состояния >**|Это имя текущего состояния CDC.|  
|**CS**|Это обозначает точку начала текущего диапазона обработки (Current Start).|  
|**\<Номер lsn CS >**|Это последний регистрационный номер транзакции в журнале, обработанный во время предыдущего запуска CDC.|  
...
|**\<Номер lsn CE >**|Это последний номер LSN, который должен быть обработан во время текущего выполнения CDC. Всегда предполагается, что последний последовательный номер, который должен быть обработан, является максимальным (0xFFF…).|  
...
|**\<Short текст ошибки >**|Это краткое описание ошибки.|  
