---
title: "Помощник источника | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
ms.prod: "sql-server-2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "integration-services"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "sql13.dts.designer.sourceassistant.f1"
  - "sql13.dts.designer.addNewSource.f1"
ms.assetid: 5ca9d821-7d61-4727-9133-5f9cb485c7f3
caps.latest.revision: 13
author: "douglaslMS"
ms.author: "douglasl"
manager: "jhubbard"
caps.handback.revision: 12
---
# Помощник источника
  Компонент «Помощник по источнику» позволяет создать компонент источника и диспетчер соединений. Компонент расположен в разделе **Избранное** панели инструментов служб SSIS.  
  
> [!NOTE]  
>  Помощник по источнику заменяет проект соединений служб Integration Services и соответствующий мастер.  
  
## Связанные задачи  
 Сведения о том, как добавить источник и диспетчер соединений с помощью помощника по источнику, см. в разделе [Add a Source using Source Assistant](../Topic/Add%20a%20Source%20using%20Source%20Assistant.md).  
  
  