---
title: "Пользовательские свойства преобразования | Документы Microsoft"
ms.custom: 
ms.date: 03/14/2017
...
---
# <a name="transformation-custom-properties"></a>Пользовательские свойства преобразований
...
|FriendlyExpression|Строковые значения|Выражение, представляющее условие, вычисляемое преобразованием «Условное разбиение». Столбцы представлены своими именами.<br /><br /> Значение этого свойства можно задать с помощью выражения свойства.|  
 ...
 Дополнительные сведения см. в статье [Derived Column Transformation](../../../integration-services/data-flow/transformations/derived-column-transformation.md).  
...
|Свойство|Тип данных|Description|  
|--------------|---------------|-----------------|  
|Delimiters|Строковые значения|Разделители токенов, используемые преобразованием. По умолчанию к разделителям относятся следующие знаки: пробелом (), запятой (,), точку (.), точка с запятой (;), двоеточие (:), дефис (-), двойные прямые кавычки ("«), один прямые кавычки ('), амперсанд (&), косая черта (/), обратная косая черта (\\), знак (@), восклицательный знак (!), вопросительный знак (?), открывающая скобка ((), закрывающая скобка ()), меньше, чем (\<), больше (>), открытие квадратная скобка ([]), закрывающая скобка (]), открывающая фигурная скобка ({}), закрытие фигурная скобка (}), канал (&#124;), знак номера (#), звездочка (*), знак вставки (^) и знак процента (%).|  
...
|Delimiters|Строковые значения|Разделители, используемые преобразованием для разделения значений столбца. По умолчанию к разделителям относятся следующие символы: пробел (), запятая (,), точка (.) запятой (;), двоеточие (:), дефис (-), двойные прямые кавычки ("«), один прямые кавычки ('), амперсанд (&), косая черта (/), обратная косая черта (\\), знак (@), восклицательный знак (!), вопросительный знак (?), открывающая скобка ((), закрывающая скобка ()), меньше, чем (\<), больше (>), открыв квадратная скобка ([]), закрывающая скобка (]), открывающая фигурная скобка ({}), закрытие фигурная скобка (}), канал (&#124;). знак числа (#), звездочка (*), знак вставки (^) и знак процента (%).|  
...
|CopyFromReferenceColumn|Строковые значения|Имя столбца в ссылочной таблице, из которого копируется данный столбец.|  
 ...
 Дополнительные сведения см. в статье [Lookup Transformation](../../../integration-services/data-flow/transformations/lookup-transformation.md).  
...
|SortColumnID|Целочисленный|Идентификатор **LineageID** столбца сортировки.|  
 ...
 Дополнительные сведения см. в статье [Sort Transformation](../../../integration-services/data-flow/transformations/sort-transformation.md).  
...
## <a name="see-also"></a>См. также  
 ...
 [Свойства потока данных, которые могут быть установлены с помощью выражений](http://msdn.microsoft.com/library/cd0e171a-08be-45d6-81dc-ed94f37698b8)  
