---
title: "Источник CDC | Документы Microsoft"
ms.custom: 
ms.date: 03/14/2017
...
-   Имя переменной пакета состояния CDC, на основе которой определен диапазон обработки CDC. Источник CDC не изменяет значение этой переменной.  
 ...
 Данные, возвращенные источником CDC совпадает со значением, возвращенных [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] функции CDC **cdc.fn_cdc_get_all_changes_\<имени экземпляра отслеживания >** или **cdc.fn_cdc_get_net_changes_\<имени экземпляра отслеживания >** (если они доступны). Единственным необязательным добавлением является столбец **__$initial_processing** , который указывает, может ли текущий диапазон обработки перекрывать диапазон начальной загрузки таблицы. Дополнительные сведения о начальной обработке см. в разделе [CDC Control Task](../../integration-services/control-flow/cdc-control-task.md).  
...
-   **Столбцы строки с ошибкой**. Данные записи, которые вызывают ошибку.  
 ...
 В зависимости от настройки поведения в случае ошибки, источник CDC поддерживает возврат ошибок (преобразование данных, усечение), которые обнаруживаются в процессе извлечения в выводе ошибок. Дополнительные сведения см. в разделе [Редактор источника "CDC" (страница "Вывод ошибок")](../../integration-services/data-flow/cdc-source-editor-error-output-page.md).  
...
-   \<CDC включена database-name > — имя [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] базы данных, содержащей таблицы изменений.  
  
-   \<значение from-state-cs > является значением, которое появляется в переменной состояния CDC в виде CS /\<значение from-state-cs > / (CS означает текущий обработки--начало диапазона).  
  
-   \<значение из state-ce > — значение, которое появляется в переменной состояния CDC в виде CE /\<значение from-state-cs > / (CE означает текущий и обработки диапазона конца).  
  
-   \<Режим > режимы обработки CDC. Режимы обработки могут иметь одно из следующих значений: **Все**, **Все со старыми значениями**, **Суммарные**, **Суммарные с маской обновления**, **Суммарные со слиянием**.  
 ...
 **Для процедуры или функции CDC.fn_cdc_get_net_changes указано недостаточное количество аргументов\<... >.**  
...
-   Запись в блоге [Режимы обработки для источника CDC](http://www.mattmasson.com/2012/01/processing-modes-for-the-cdc-source/)на сайте mattmasson.com.  
