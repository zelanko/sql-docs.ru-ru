...
|DetailedTracingFile|String|Путь к файлу журнала, который создается автоматически, когда для параметра *DetailedTracingLevel* не установлено значение *Off*. Это свойство находится в области **Расширенный редактор**.|
...
|SqlCommand|String|Команда SQL, которая должна быть выполнена, если для `AccessMode` задано значение *SQL Command*.|
|TableName|String|Имя таблицы, содержащей данные, которые будут использоваться, если `AccessMode` имеет значение *Table Name*.|
