---
title: "Задача «Выполнение SQL» | Документы Microsoft"
ms.custom:
- ssisdev020617
ms.date: 03/13/2017
...
## <a name="connect-to-a-data-source"></a>Подключения к источнику данных  
...
|SQLMOBILE|[Подключение диспетчер SQL Server Compact Edition](../../integration-services/connection-manager/sql-server-compact-edition-connection-manager.md)|  
...
## <a name="run-parameterized-sql-commands"></a>Запустите параметризованные команды SQL  
...
## <a name="specify-a-result-set-type"></a>Укажите тип результирующего набора  
...
## <a name="general-page---execute-sql-task-editor"></a>Страница «Общие» — выполнение задачи редактора SQL
 ...
 **Соединение**  
 Выберите соединение из списка определенных диспетчеров соединений. Чтобы создать новое соединение, выберите \< **новое подключение...** >.  
 ...
 **См. также:** [Построитель запросов](http://msdn.microsoft.com/library/780752c9-6e3c-4f44-aaff-4f4d5e5a45c5)  
...
## <a name="parameter-mapping-page---execute-sql-task-editor"></a>Страница «сопоставление параметров» - выполнение редактор задачи SQL
 ...
 **Имя переменной**  
 После добавления сопоставления параметров нажатием **добавить**, выберите системную или пользовательскую переменную из списка или нажмите кнопку \< **создать переменную...** > Добавление новой переменной с помощью **Добавление переменной** диалоговое окно.  
 ...
 **Имя параметра**  
 ...
 В зависимости от типа диспетчера соединений, используемого задачей, необходимо использовать числа или имена параметра. Некоторые типы диспетчеров соединений требуют, первым символом имени параметра был знак @, определенного имена наподобие @Param1, или имена столбцов как имена параметров.  
...
## <a name="result-set-page---execute-sql-task-editor"></a>«Результирующий набор» — выполните задачи редактора SQL
 ...
 **Имя переменной**  
 Сопоставления результирующего набора с переменной, выбрав переменную или нажмите кнопку \< **создать переменную...** > Добавление новой переменной с помощью **Добавление переменной** диалоговое окно.  
...
## <a name="parameters-in-the-execute-sql-task"></a>Параметры задачи «Выполнение SQL»
...
-   [С помощью имен и маркеров параметров](#Parameter_names_and_markers)  
...
###  <a name="Parameter_names_and_markers"></a>Имена параметров и маркеров  
 В зависимости от типа соединения, который использует задача «Выполнение SQL», синтаксис команды SQL использует различные маркеры параметров. Например [!INCLUDE[vstecado](../../includes/vstecado-md.md)] тип диспетчера соединений требует, чтобы команда SQL использовала маркер параметра в формате  **@varParameter** , в то время как тип соединения OLE DB требует маркер параметра вопросительный знак (?).  
...
|[!INCLUDE[vstecado](../../includes/vstecado-md.md)]|@\<Имя параметра >|@\<Имя параметра >|ВЫБЕРИТЕ имя, Фамилия, название из Person.Contact ГДЕ ContactID =@parmContactID|  
...
#### <a name="use-parameters-with-adonet-and-ado-connection-managers"></a>Использование параметров с ADO.NET и диспетчерами соединений ADO  
 [!INCLUDE[vstecado](../../includes/vstecado-md.md)]и ADO имеют особые требования к командам SQL, использующих параметры:  
  
-   [!INCLUDE[vstecado](../../includes/vstecado-md.md)]Диспетчеры соединений требуют, чтобы команда SQL использовала имена параметров в качестве маркеров параметров. Это означает, что переменные могут быть прямо сопоставлены с параметрами. Например, переменная `@varName` сопоставляется с параметром по имени `@parName` и предоставляет значение параметру `@parName`.  
...
#### <a name="use-parameters-with-excel-odbc-and-ole-db-connection-managers"></a>Использование параметров с диспетчерами соединений OLE DB, ODBC и EXCEL  
...
#### <a name="use-parameters-with-ole-db-connection-managers"></a>Использование параметров с диспетчерами соединений OLE DB  
...
#### <a name="use-date-and-time-parameters-with-adonet-and-ado-connection-managers"></a>Использование параметров даты и времени с ADO.NET и диспетчерами соединений ADO  
...
#### <a name="use-date-and-time-parameters-with-ole-db-connection-managers"></a>Использование параметров даты и времени с диспетчерами соединений OLE DB  
...
#### <a name="use-date-and-time-parameters-with-odbc-connection-managers"></a>Использование параметров даты и времени с диспетчерами соединений ODBC  
...
-   [!INCLUDE[vstecado](../../includes/vstecado-md.md)] Тип соединения использует имена параметров @parmMinProductID и @parmMaxProductID.  
...
|ADO|Если имеет значение IsQueryStoredProcedure **False**,`EXEC uspGetBillOfMaterials ?, ?`<br /><br /> Если имеет значение IsQueryStoredProcedure **True**,`uspGetBillOfMaterials`|  
|[!INCLUDE[vstecado](../../includes/vstecado-md.md)]|Если имеет значение IsQueryStoredProcedure **False**,`EXEC uspGetBillOfMaterials @StartProductID, @CheckDate`<br /><br /> Если имеет значение IsQueryStoredProcedure **True**,`uspGetBillOfMaterials`|  
...
### <a name="map-query-parameters-to-variables"></a>Сопоставление параметров запроса с переменными
...
|ADO|Если имеет значение IsQueryStoreProcedure **False**,`EXEC ? = myStoredProcedure 1`<br /><br /> Если имеет значение IsQueryStoreProcedure **True**,`myStoredProcedure`|  
...
###  <a name="Result_set_type"></a>Укажите тип результирующего набора  
...
## <a name="troubleshoot-the-execute-sql-task"></a>Устранение неполадок задачей «Выполнение SQL»  
