...
|Файл ADLS по каждому элементу|Укажите каталог ADLS, который содержит файлы для перечисления, а также некоторые фильтры.|
...
|**Перечислитель по каждому элементу ADLS файла**|Перечисляет файлы на ADLS с фильтрами. При выборе этого значения отображаются динамические параметры в данном разделе **файл ADLS перечислитель**.|
 ...
 **Фильтр по имени файла**  
 Укажите фильтр имен, чтобы выбрать файлы с определенным шаблоном имени. Например, MySheet*.xls\* будет включать такие файлы, как MySheet001.xls и MySheetABC.xlsx.  
...
-   **Только имя** — получается только имя файла.  
 ...
 **Перечислитель больших двоичных объектов Azure** позволяет пакету службы SSIS перечислять файлы больших двоичных объектов в указанном расположении этих объектов. Имя перечисленного файла большого двоичного объекта может храниться в переменной и использоваться в задачах в контейнере цикла Foreach.  
...
**FileNamePattern**  
Указывает фильтр по имени файла. Будут перечислены только те файлы, имя которого соответствует указанному шаблону. Подстановочные знаки `*` и `?` поддерживаются. 
