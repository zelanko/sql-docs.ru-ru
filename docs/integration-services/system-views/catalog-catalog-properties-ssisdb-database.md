...
|**IS_SCALEOUT_ENABLED**|Значение `True` свидетельствует о том, что функция SSIS Scale Out включена. Если функция горизонтального увеличения масштаба не включена, это свойство может не отображаться в представлении.|
