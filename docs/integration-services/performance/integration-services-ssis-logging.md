---
title: "Ведение журналов в службах Integration Services (SSIS) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  ...
  - "регистратор событий Windows [службы Integration Services]"
  - "регистратор XML-файлов [службы Integration Services]"
  - "SQL Server Profiler, регистратор"
  - "Пакеты служб SQL Server Integration Services, журналы"
  - "Пакеты служб SSIS, журналы"
  - "Задачи [службы Integration Services], журналы"
  - "Журналы [службы Integration Services], регистраторы"
  - "Пакеты служб Integration Services, журналы"
  - "регистраторы [службы Integration Services]"
  - "Пакеты [службы Integration Services], журналы"
  - "регистратор текстового файла"
  - "регистратор служб SQL Server"
...
# Ведение журналов в службах Integration Services (SSIS)
 ...
 Вы можете настроить область ведения журнала на время выполнения пакета на сервере [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] . Дополнительные сведения см. в разделе [Enable Logging for Package Execution on the SSIS Server](../../integration-services/performance/enable-logging-for-package-execution-on-the-ssis-server.md).  
...
3.  Выберите события и схему данных журнала для этих событий, запись о которых должна содержаться в журнале. Дополнительные сведения см. в разделе [Настройка ведения журналов с помощью сохраненного файла конфигурации](../../integration-services/performance/configure-logging-by-using-a-saved-configuration-file.md).  
...
#### Запись имен столбцов, в которых возникают ошибки  
 При настройке вывода ошибок в потоке данных по умолчанию вывод ошибок отображает только числовой идентификатор столбца, в котором произошла ошибка. Дополнительные сведения см. в разделе [Error Handling in Data](../../integration-services/data-flow/error-handling-in-data.md).  
...
-   [Диалоговое окно «Настройка журналов служб SSIS»](../../integration-services/performance/configure-ssis-logs-dialog-box.md)  
...
-   [Включение ведения журналов при выполнении пакета на сервере служб SSIS](../../integration-services/performance/enable-logging-for-package-execution-on-the-ssis-server.md)  
