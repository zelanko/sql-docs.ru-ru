...
Если в пакете служб SSIS используется проверка подлинности Windows, то в пакете можно использовать только один набор учетных данных. Учетные данные домена, указываемые при выполнении инструкций в этой статье, применяются ко всем операциям (интерактивным или запланированным) в среде выполнения интеграции Azure — SSI, пока эти данные не будут изменены или удалены. Если пакет должен подключаться к нескольким источникам данных и (или) общим файловым ресурсам с разными наборами учетных данных, следует рассмотреть другие описанные выше методы.
