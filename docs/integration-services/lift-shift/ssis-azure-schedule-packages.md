...
Не требуется создавать новую хранимую процедуру для инкапсуляции команд Transact-SQL, необходимые для создания и запустить выполнение пакета служб SSIS. В качестве значения можно предоставить сценарий `stmt` параметр в предыдущем примере JSON. Ниже приведен пример сценария:
