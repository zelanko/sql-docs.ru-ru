...
*value* является целым числом в версиях до [!INCLUDE[ssSQL17](sssql17-md.md)] и числом с плавающей точкой, начиная с [!INCLUDE[sql-server-2019](sssqlv15-md.md)] и в управляемом экземпляре [!INCLUDE[ssSDSfull](sssdsfull-md.md)]. Значение по умолчанию равно 25. Диапазон допустимых значений для *value* — от 1 до 100.
