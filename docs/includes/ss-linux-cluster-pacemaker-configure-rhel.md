...
1. Создайте кластер. Чтобы создать кластер, выполните следующую команду:
   ...
   **RHEL8**
   ...
   >[!NOTE]
   >Если кластер был ранее настроен ранее на тех же узлах, при выполнении команды `--force` используйте параметр `pcs cluster setup`. Этот параметр эквивалентен запуску `pcs cluster destroy`. Чтобы повторно включить Pacemaker, запустите `sudo systemctl enable pacemaker`.
