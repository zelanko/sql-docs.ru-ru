...
## <a name="enable-always-on-availability-groups-and-restart-sqlserver"></a>Включение групп доступности AlwaysOn и перезапук sqlserver
...
>[!NOTE]
>В этом выпуске указать другой IP-адрес для IP-адреса прослушивателя нельзя. Мы работаем над решением этой проблемы, но на данный момент единственное допустимое значения — "0.0.0.0".

Обновление следующий запрос Transact-SQL для своей среды на всех экземплярах SQL Server: 
