---
title: "Олицетворение (табличные службы SSAS) | Microsoft Docs"
ms.custom: ""
ms.date: "03/04/2017"
...
|Параметр|ImpersonationMode*|Description|  
|------------|-------------------------|-----------------|  
|**Имя и пароль определенного пользователя Windows***\*|ImpersonateWindowsUserAccount|Этот параметр указывает, что модель будет использовать учетную запись пользователя Windows для импорта или обработки данных из источника данных. Домен и имя пользовательской учетной записи используют следующий формат: **\<имя домена>\\<имя учетной записи пользователя\>**. При создании новой модели с помощью мастера импорта таблиц это значение установлено по умолчанию.|  
 ...
 *ImpersonationMode указывает значение для свойства [элемент DataSourceImpersonationInfo (ASSL)](../../analysis-services/scripting/properties/datasourceimpersonationinfo-element-assl.md) в источнике данных.  
  
 \*\*Если при использовании этого параметра база данных рабочей области удаляется из памяти (в случае перезагрузки или при задании для свойства **Сохранение рабочей области** значения **Выгрузить из памяти** или **Удалить из рабочей области** и закрытии проекта модели), то в следующем сеансе при попытке обработать данные из таблицы система предложит ввести учетные данные для каждого источника данных. Аналогичным образом, если база данных развернутой модели удаляется из памяти, система попросит указать учетные данные для каждого источника данных.  
...
##  <a name="bkmk_imp_newmodel"></a> Олицетворение при импорте модели  
 В отличие от табличных моделей, где может применяться несколько разных режимов олицетворения для поддержки внепроцессорного сбора данных, [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] использует только один режим — ImpersonateCurrentUser. Так как компонент [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] всегда выполняется в процессе, он подключается к источникам данных, используя учетные данные пользователя, вошедшего в систему. При работе с табличными моделями учетные данные пользователя, вошедшего в систему, используются только с функцией **Просмотр и фильтр** из мастера импорта таблиц, а также в диалоговом окне **Свойства таблицы**. Учетные данные олицетворения используются при импорте и обработке данных в базе данных рабочей области либо при импорте или обработке данных в базе данных развернутой модели.  
  
 При создании новой модели путем импорта существующей книги [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] конструктор моделей по умолчанию настраивает олицетворение для использования учетной записи службы (ImpersonateServiceAccount). Рекомендуется изменить учетные данные олицетворения в моделях, импортированных из [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)], на учетную запись пользователя Windows. После завершения импорта книги [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] и создания новой модели в конструкторе моделей учетные данные можно изменить в диалоговом окне **Существующие подключения**.  
...
##  <a name="bkmk_conf_imp_info"></a> Настройка олицетворения  
 ...
 Сведения об олицетворении для моделей, развернутых на сервере служб Analysis Services, можно настроить в среде SSMS, выбрав **Свойства соединения** > **Сведения об олицетворении**.  
