...
|Текста|String|Строка символьных данных в Юникоде. Может быть строками, числами или датами, представленными в текстовом формате.|  
...
|Н/Д|Пусто|Тип данных с пустыми значениями в DAX представляет и заменяет пустые значения NULL в SQL. Пустое значение создается с помощью функции BLANK, а проверяется с помощью логической функции ISBLANK.|  
 ...
 Ошибка памяти базы данных: "\<Имя столбца >" столбец "\<имя таблицы >" таблица содержит значение "1.7976931348623157E + 308», которое не поддерживается. Операция была отменена.  
