---
title: "Совместимость формул DAX в режиме DirectQuery (SSAS 2016) | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
---
# Совместимость формул DAX в режиме DirectQuery (SSAS 2016)
Некоторые из наиболее значительных улучшений в службах [!INCLUDE[ssASCurrent_md](../../includes/ssascurrent-md.md)] касаются режима DirectQuery для табличных моделей 1200. Многие функциональные ограничения, налагаемые в более ранних версиях, теперь отсутствуют. В частности, для формул DAX:

- Теперь DirectQuery создает более простые запросы, обеспечивая более высокую производительность.
- Теперь безопасность на уровне строк (RLS) поддерживается и для табличных моделей 1200 в режиме DirectQuery.
- Вычисляемые столбцы теперь поддерживаются и для табличных моделей 1200 в режиме DirectQuery.

## Функции DAX в режиме DirectQuery

Проще говоря, для табличных моделей 1200 в режиме DirectQuery поддерживаются все функции DAX. Однако не все функции поддерживаются для всех типов формул, и не все функции оптимизированы для табличных моделей 1200 в режиме DirectQuery. На самом общем уровне функции DAX можно разделить на две группы: оптимизированные и неоптимизированные. Сначала давайте более подробно рассмотрим оптимизированные функции.


### Оптимизированные для DirectQuery
...
### Не оптимизированные для DirectQuery
Эти функции не оптимизированы для работы с DirectQuery в табличных моделях 1200. Они *не* поддерживаются в формулах вычисляемых столбцов и безопасности на уровне строк. Тем не менее эти функции *поддерживаются* в формулах мер и запросов, хотя и с неопределенной производительностью.
...
## Операторы DAX в режиме DirectQuery
Все операторы сравнения и арифметические операторы DAX полностью поддерживаются в табличных моделях 1200 в режиме DirectQuery. Дополнительные сведения см. в статье [Справочник по операторам DAX](https://msdn.microsoft.com/library/ee634237.aspx).


 
## Различия между режимами в памяти и DirectQuery  
...
При преобразовании из строк в нелогические значения режим DirectQuery ведет себя так же, как SQL Server. Дополнительные сведения см. в разделе [Функции CAST и CONVERT (Transact-SQL)](http://msdn.microsoft.com/ru-ru/a87d0850-c670-4720-9ad5-6f5a22343ea8).  
...
### <a name="bkmk_Currency"></a>Измерение валют  
...
## См. также:  
[Режим DirectQuery (табличные службы SSAS)](http://msdn.microsoft.com/ru-ru/45ad2965-05ec-4fb1-a164-d8060b562ea5)  
