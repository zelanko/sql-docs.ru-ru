---
title: "Двунаправленные кросс-фильтры для табличных моделей в службах SQL Server 2016 Analysis Services | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/07/2017"
...
---
# Двунаправленные кросс-фильтры для табличных моделей в службах SQL Server 2016 Analysis Services
 ...
 В SSDT кроссфильтрация настраивается с помощью параметра "Направление фильтрации" в диалоговом окне настройки связей таблиц. Эта функция доступна только для табличных моделей, созданных на уровне совместимости 1200 в [SQL Server Data Tools для Visual Studio 2015 (SSDT)](http://go.microsoft.com/fwlink/p/?LinkID=627574).  
  
 Существует два типа кросс-фильтров: однонаправленные и двунаправленные. Однонаправленный — это традиционный фильтр с направлением от многих к одному в связи между исходной таблицей и таблицей измерения. Двунаправленный — это кросс-фильтр, позволяющий использовать контекст фильтра одной связи в качестве контекста фильтра для другой связи с таблицей, когда одна таблица является общей для обеих связей.  
...
## Пример с пошаговым руководством  
...
4.  Проверьте, установлен ли уровень совместимости модели **SQL Server 2016 RTM (1200)**.  
...
### Просмотр связей таблицы по умолчанию  
...
## См. также  
 ...
 [Практический пример обработки простых связей типа "многие-ко-многим" в табличных моделях Power Pivot и служб SSAS](http://social.technet.microsoft.com/wiki/contents/articles/22202.a-practical-example-of-how-to-handle-simple-many-to-many-relationships-in-power-pivotssas-tabular-models.aspx)   
