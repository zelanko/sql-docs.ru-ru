...
[SELECTEDMEASUREFORMATSTRING](https://docs.microsoft.com/dax/selectedmeasureformatstring-function-dax) — используется выражения для вычисления элементов, извлекаемых в строке формата меру, которая находится в контексте.
...
```
Строковое выражение формата должен возвращать скалярное строковое. Он использует новую [SELECTEDMEASUREFORMATSTRING](https://docs.microsoft.com/dax/selectedmeasureformatstring-function-dax) функции, чтобы вернуться к строке формата базовой мерой, при наличии нескольких валют в контекст фильтра.
