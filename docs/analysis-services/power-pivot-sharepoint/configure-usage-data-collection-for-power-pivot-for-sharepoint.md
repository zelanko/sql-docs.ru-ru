...
##  <a name="qrh"></a> Определение категорий быстрых, средних и медленных откликов на запросы для подготовки отчетов  
 Производительность обработки запросов измеряется по стандартным категориям, которые характеризуют цикл «запрос-ответ» по продолжительности завершения цикла. Стандартные категории запросов включают: Тривиальный, быстрый, ожидаемый, выполняющейся длительное время и превышено. Каждый запрос к серверу [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] попадает в одну из категорий в зависимости от времени его выполнения.  
...
|Параметр|Default Value|Type|Допустимый диапазон|  
|-------------|-------------------|----------|-----------------|  
