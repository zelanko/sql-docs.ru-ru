---
title: Примеры запросов к модели дерева принятия решений | Документы Microsoft
ms.date: 05/01/2018
ms.prod: sql
...
###  <a name="bkmk_Query3"></a> Пример запроса 3. Получение поддеревьев из модели  
 Допустим, что необходимо узнать больше о клиентах, купивших велосипед. Дополнительные сведения о любом из вложенных деревьев можно просматривать с помощью функции [IsDescendant &#40;DMX&#41;](../../dmx/isdescendant-dmx.md) в запросе, как это показано в следующем примере. Запрос возвращает число клиентов, купивших велосипед, возвращая конечные узлы (NODE_TYPE = 4) из дерева, в котором содержатся клиенты, возраст которых превышает 42 года. Запрос ограничивает строки вложенной таблицы теми, в которых Bike Buyer = 1.  
...
###  <a name="bkmk_Query4"></a> Пример запроса 4. Получение прогнозов с оценкой вероятности  
 ...
 В этом запросе используется прогнозирующая функция [PredictHistogram &#40;DMX&#41;](../../dmx/predicthistogram-dmx.md), возвращающая вложенную таблицу с полезными сведениями о вероятностях, обнаруженных моделью. Последнее предложение WHERE в запросе фильтрует результаты, чтобы вернуть только тех клиентов, которые согласно прогнозу купят велосипед с вероятностью больше 0%.  
...
###  <a name="bkmk_Query5"></a> Пример запроса 5. Прогноз сопоставлений на основе модели дерева принятия решений  
 Следующий образец запроса основан на структуре интеллектуального анализа сопоставлений. Для изучения этого примера можно добавить в эту структуру интеллектуального анализа данных новую модель и выбрать алгоритм дерева принятия решений (Майкрософт). Дополнительные сведения о создании структуры интеллектуального анализа сопоставлений см. в разделе [Урок 3. Построение сценария покупательского поведения (учебник по интеллектуальному анализу данных — средний уровень)](http://msdn.microsoft.com/library/651eef38-772e-4d97-af51-075b1b27fc5a)  
