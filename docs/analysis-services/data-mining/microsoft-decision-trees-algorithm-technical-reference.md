...
### <a name="discrete-and-continuous-inputs"></a>Дискретные и непрерывные входные данные  
 ...
 Более подробное объяснение о том, как [!INCLUDE[msCoName](../../includes/msconame-md.md)] работает алгоритм дерева принятия решений с дискретными прогнозируемыми столбцами, см. в разделе [обучаемые Байесовские сети: Сочетание знаний и статистических данных](http://research.microsoft.com/en-us/um/people/heckerman/hgc94uai.pdf). Дополнительные сведения о том, каким образом алгоритм дерева принятия решений [!INCLUDE[msCoName](../../includes/msconame-md.md)] работает с непрерывным прогнозируемым столбцом, см. в приложении к статье [Модели деревьев с авторегрессией для анализа временных рядов](http://go.microsoft.com/fwlink/?LinkId=45966).  
  
### <a name="scoring-methods-and-feature-selection"></a>Методы количественной оценки и выбор компонентов  
 Алгоритм дерева принятия решений Microsoft предусмотрены три формулы для вычисления прироста информации: Энтропии Шеннона, Байесовская сеть с априорной оценкой K2 и Байесовская сеть с однородной априорной оценкой Дирихле. Все три метода широко применяются для интеллектуального анализа данных. Рекомендуется поэкспериментировать с различными параметрами и методами количественной оценки, чтобы понять, какие из них дают наилучшие результаты. Дополнительные сведения об этих методах количественной оценки см. в разделе [Feature Selection](http://msdn.microsoft.com/library/73182088-153b-4634-a060-d14d1fd23b70).  
...
|id|Name|  
|--------|----------|  
|1|**Двоичные:** Указывает, что независимо от фактического числа значений атрибута дерево следует разбить на две ветви.|  
|2|**Выполните:** Указывает, что в дереве можно создавать столько разбиений, сколько существует значений атрибута.|  
|3|**Оба:** Указывает, что службы Analysis Services могут определять, следует ли использовать для получения наилучших результатов разбиения бинарное или полное.|  
...
|-------------------|-----------------|  
|MODEL_EXISTENCE_ONLY|Означает, что столбец будет обрабатываться как имеющий два возможных состояния: **Отсутствует** и **существующие**. NULL означает отсутствие значения.<br /><br /> Применяется к столбцам модели интеллектуального анализа данных.|  
