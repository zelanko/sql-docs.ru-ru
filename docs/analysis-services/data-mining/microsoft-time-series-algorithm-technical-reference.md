...
## <a name="implementation-of-the-microsoft-time-series-algorithm"></a>Реализация алгоритма временных рядов (Майкрософт)  
 [!INCLUDE[msCoName](../../includes/msconame-md.md)]Группа исследований Майкрософт разработала оригинальный алгоритм ARTXP, используемого в SQL Server 2005, создание на основе реализации [!INCLUDE[msCoName](../../includes/msconame-md.md)] алгоритм дерева принятия решений. Таким образом, алгоритм ARTXP можно описать, как модель дерева с авторегрессией для представления данных в периодических временных рядах. Этот алгоритм устанавливает соотношение между переменным количеством предыдущих элементов и каждым текущим элементом, для которого выполняется прогноз. Название ARTXP отражает тот факт, что алгоритм ART (или метод дерева с авторегрессией) применяется ко многим неизвестным первоначальным состояниям. Подробное описание алгоритма ARTXP см. в статье [Модели деревьев с авторегрессией для анализа временных рядов](http://go.microsoft.com/fwlink/?LinkId=45966).  
