 ...
 **MODEL_EXISTENCE_ONLY**  
 Указывает, что столбец должен рассматриваться как имеющий два состояния: **Отсутствует** и **существующие**. Если значение составляет **NULL**, оно рассматривается как недостающее. Флаг MODEL_EXISTENCE_ONLY применяется к прогнозируемому атрибуту и поддерживается большинством алгоритмов.  
  
 Фактически установка для флага MODEL_EXISTENCE_ONLY **True** меняет представление значений так, что существует только два состояния: **Отсутствует** и **существующие**. Все неотсутствующие состояния объединяются в единое значение **Existing** .  
