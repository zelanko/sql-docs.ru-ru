---
title: "Просмотр атрибутов в дереве, списке или сетке в конструкторе измерений | Документы Microsoft"
ms.custom: 
ms.date: 03/04/2017
ms.prod: analysis-services
ms.prod_service: analysis-services
ms.service: 
ms.component: data-mining
ms.reviewer: 
ms.suite: pro-bi
ms.technology: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- Attribute pane formats [Analysis Services]
- attributes [Analysis Services], Dimension Builder Attribute Pane
- formats [Analysis Services], Attribute pane
- modifying Attribute pane format
ms.assetid: ef011559-9ab9-4a19-b5da-265064fea521
caps.latest.revision: "27"
author: Minewiskan
ms.author: owend
manager: kfile
ms.workload: Inactive
ms.openlocfilehash: 5ab0bd75d302d9436f9c875172d8847607f4dc44
ms.sourcegitcommit: f486d12078a45c87b0fcf52270b904ca7b0c7fc8
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 01/08/2018
---
# <a name="view-attributes-in-dimension-designer"></a>Просмотр атрибутов в конструкторе измерений
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]Эта процедура описывает изменение способа отображения атрибутов в конструкторе измерений.  
  
### <a name="to-change-the-format-of-the-attributes-pane-in-dimension-designer"></a>Изменение формата панели «Атрибуты» в построителе измерений  
  
1.  В конструкторе измерений откройте измерение, содержащее атрибуты, с которыми необходимо работать, и перейдите на вкладку «Структура измерения».  
  
2.  Щелкните в любом месте панели **Атрибуты** , чтобы установить фокус на нее **Атрибуты**.  
  
3.  Выберите меню **Измерение**или щелкните правой кнопкой мыши панель **Атрибуты** .  
  
4.  Выберите пункт **Показать атрибуты в**, а затем выберите пункт **Дерево**, **Сетка**или **Список**.  
  
  
