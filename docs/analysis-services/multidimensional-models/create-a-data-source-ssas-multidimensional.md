---
title: "Создание источника данных (многомерные службы SSAS) | Microsoft Docs"
ms.custom: ""
ms.date: "03/16/2017"
...
helpviewer_keywords: 
  - "олицетворение [службы Analysis Services]"
  - "источники данных [службы Analysis Services], создание"
  - "безопасность [службы Analysis Services], подключения к источникам данных"
...
5.  Введите данные, необходимые для выбранного поставщика и подключитесь к соответствующему источнику данных. Если выбран **Собственный поставщик OLE DB \SQL Server Native Client**, введите следующие данные:  
    ...
    1.  **Имя сервера** — это сетевое имя экземпляра Database Engine. Его можно указать как IP-адрес, имя NETBIOS компьютера или в виде полного доменного имени. Если сервер устанавливается в качестве экземпляра с именем, необходимо включить имя экземпляра (например, \<имякомпьютера>\\<имяэкземпляра\>).  
