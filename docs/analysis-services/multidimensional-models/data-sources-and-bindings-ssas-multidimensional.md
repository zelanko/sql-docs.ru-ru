...
## <a name="bindings-for-dimensions"></a>Привязки для измерений  
 Каждый атрибут измерения привязан к столбцу в представлении источника данных. Все атрибуты измерения должны относиться к одному источнику данных. Тем не менее атрибуты могут быть привязаны к столбцам из разных таблиц. Связи между таблицами определены в представлении источника данных. В случае, где существует более чем один набор связей с одной таблицей может потребоваться создать именованный запрос в представлении источника данных, чтобы выступать в роли «псевдонима» таблицы. Выражения и фильтры определяются в представлении источника данных с помощью именованных вычислений и запросов.  
...
### <a name="definition-of-the-out-of-line-binding-type"></a>Определение типа внешней привязки  
 В пределах внешней коллекции **Bindings** язык ASSL позволяет создавать коллекцию привязок для нескольких объектов **Bindings**. Каждый объект **Bindings** имеет расширенную ссылку на объект, которая напоминает простую объектную ссылку, но может ссылаться также на более мелкие объекты (например, атрибуты измерения и атрибуты группы мер). Такой объект принимает плоский вид, типичный для элемента **объект** элемент в **процесс** команд, кроме тех, которые \< *объект* > \< */Object*> отсутствуют теги.  
  
 Каждый объект, для которого задана привязка определяется в XML-элементе в формате \< *объект*> ID (например, **DimensionID**). После определения объекта как можно точнее с формой \< *объект*> ID, находится элемент, для которого задана привязка, которая обычно **источника**. В общем случае элемент **Source** является свойством объекта **DataItem**, представляющего собой вариант привязки к столбцу в атрибуте. В этом случае тег **DataItem** не указывается, вместо этого нужно задать свойство **Source** , как при непосредственной привязке столбца.  
