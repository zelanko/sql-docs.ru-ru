...
|Имя файла или расположение|Type|Используется для|Включено по умолчанию|  
|---------------------------|----------|--------------|-------------------|  
...
|Файлы SQLDmp\<guid > файлы с расширениями mdmp|Сбои и исключения|Устранение серьезных неполадок|Нет|  
 ...
 Мы настоятельно рекомендуем дополнительную информацию, не охваченных в этом разделе по следующей ссылке: [Подсказки к начальному данных коллекции из службы поддержки Майкрософт](http://blogs.msdn.com/b/as_emea/archive/2012/01/02/initial-data-collection-for-troubleshooting-analysis-services-issues.aspx).  
