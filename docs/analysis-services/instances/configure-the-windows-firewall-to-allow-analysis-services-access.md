---
title: "Настройка брандмауэра Windows на разрешение доступа к службам Analysis Services | Microsoft Docs"
ms.custom: ""
ms.date: "03/07/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "порты [службы Analysis Services]"
  - "брандмауэр Windows [службы Analysis Services]"
  - "системы брандмауэров [службы Analysis Services]"
...
-   Для экземпляров служб [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)], выполняющихся на виртуальных машинах Windows Azure, используйте альтернативные инструкции по настройке доступа к серверу. См. раздел [SQL Server Business Intelligence на виртуальных машинах Windows Azure](http://msdn.microsoft.com/library/windowsazure/jj992719.aspx).  
 ...
 Несмотря на то, что экземпляр по умолчанию служб [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] прослушивает TCP-порт 2383, можно настроить сервер на прослушивание другого фиксированного порта при подключении к серверу в следующем формате: \<имя_сервера>:\<номер_порта>.  
...
-   Проверьте порт с использованием среды SQL Server Management Studio и подключитесь к серверу служб Analysis Services в следующем формате: \<IP_адрес>:\<номер_порта>.  
...
8.  Чтобы проверить, разрешены ли удаленные соединения, откройте среду SQL Server Management Studio или Excel на другом компьютере и подключитесь к службам Analysis Services, указав сетевое имя сервера и номер порта в следующем формате: \<имя_сервера>:\<номер_порта>.  
