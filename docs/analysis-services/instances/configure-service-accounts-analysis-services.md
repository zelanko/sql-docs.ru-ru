---
title: "Настройка учетных записей служб (службы Analysis Services) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "безопасность [службы Analysis Services], учетные записи входа"
  - "учетные записи входа [службы Analysis Services]"
  - "учетные записи [службы Analysis Services]"
  - "учетные записи входа [службы Analysis Services], об учетных записях входа"
...
>  Сведения о разрешениях, относящихся к конфигурации IIS и [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)], см. в разделе [Настройка HTTP-доступа к службам Analysis Services в службах Internet Information Services (IIS) 8.0](../../analysis-services/instances/configure http access to analysis services on iis 8.0.md).  
 ...
 Для каждого устанавливаемого вами экземпляра создается одна группа безопасности. Группа безопасности называется по названию экземпляра ─ либо **SQLServerMSASUser$MSSQLSERVER** для экземпляра по умолчанию, либо **SQLServerMSASUser$**\<имя_сервера>$\<имя_экземпляра> для именованного экземпляра. Установка обеспечивает данную группу безопасности разрешениями файла, необходимые для осуществления операций сервера. При проверке прав доступа на каталог \MSAS13.MSSQLSERVER\OLAP\BIN можно увидеть, что группа безопасности (а не учетная запись службы или ее идентификатор безопасности) является держателем разрешения в этом каталоге.  
...
2.  Последовательно выберите **Диспетчер компьютер** | **Локальные пользователи и группы** | **Группы** для проверки членства группы безопасности SQLServerMSASUser$\<имя_сервера>$\<имя_экземпляра>.  
