...
[!INCLUDE[ssas-appliesto-sqlas](../../../includes/ssas-appliesto-sqlas.md)]Содержит отметку времени только для чтения, которая указывает время последней обработки базы данных, содержащей родительский элемент.  
