---
title: "Определение и поиск перспектив | Microsoft Docs"
ms.custom: ""
ms.date: "03/04/2017"
...
manager: "erikre"
---
# Определение и поиск перспектив
...
4.  В следующей строке задайте для свойства DefaultMeasure значение **Продажи через Интернет — объем продаж**.  
    ...
    > [!NOTE]  
    > В окне свойств на вкладке **Структура куба** можно также указать меру по умолчанию для всего куба учебника по службам [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)].  
...
4.  При выполнении команды «Анализ в Excel» выводится запрос выбора перспективы, которая будет использоваться при просмотре модели в Excel, как показано на следующем рисунке.  
    ...
    ![Объекты перспективы «Продажи через Интернет»](../analysis-services/media/l9-perspectives-3.gif "Объекты перспективы «Продажи через Интернет»")  
...
5.  Кроме того, можно запустить Excel из меню «Пуск» Windows, определить подключение к базе данных учебника по службам Analysis Services на узле localhost и выбрать перспективу в мастере подключения к данным, как показано на следующем рисунке.  
    ...
    ![Мастер подключения к данным в Excel](../analysis-services/media/l9-perspectives-3b.gif "Мастер подключения к данным в Excel")  
...
8.  В модели снова выберите Excel. Выберите **Сводка по продажам**.  
    ...
    ![Меры «Продажи через Интернет» и «Продажи через посредников»](../analysis-services/media/l9-perspectives-4.gif "Меры «Продажи через Интернет» и «Продажи через посредников»")  
