...
## <a name="calling-stored-procedures-with-the-call-statement"></a>Вызов хранимых процедур с использованием инструкции Call  
 Хранимые процедуры можно вызывать вне контекста запроса многомерных Выражений, с помощью многомерных Выражений **вызовите** инструкции.  
  
 Этот метод можно использовать либо для создания экземпляра побочных эффектов хранимого запроса, либо для получения приложением результатов хранимого запроса. Обычно используются **вызовите** инструкции можно использовать объекты управления Analysis AMO для выполнения административных функций, у которых нет возвращаемый результат. Например, следующая команда вызывает хранимую процедуру:  
  
```  
Call MyStoredProcedure(a,b,c)  
```  
  
 Единственным поддерживаемым типом, возвращаемым из хранимой процедуры в **вызовите** инструкция является набор строк. Упорядочивание для набора строк определяется XML для аналитики. Если хранимая процедура в **вызовите** инструкция возвращает любого другого типа, оно игнорируется и не возвращается в XML вызывающему приложению. Дополнительные сведения о наборах строк XML для аналитики см. в разделе «Наборы строк схемы XML для аналитики».  
