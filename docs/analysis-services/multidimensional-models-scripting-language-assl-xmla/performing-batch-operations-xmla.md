 ...
 **Транзакционная**  
 Если **транзакции** атрибут **пакета** набора команд значение true, **пакета** команда выполняет все команды, содержащиеся **пакетнойслужбы** команды в одной транзакции — *транзакций* пакетной службы.  
 ...
 **Нетранзакционные**  
 Если **транзакции** атрибут имеет значение false, **пакета** команда выполняет каждую команду, содержащуюся **пакета** команду в отдельной транзакции —  *Нетранзакционные* пакетной службы. Если из команд завершается ошибкой в рамках нетранзакционного пакета **пакета** команда продолжает выполнение команд после команды, которой не удалось. После **пакетной службы** команда пытается выполнить все команды, **пакета** содержит команду, **пакета** команда выводит все возникшие ошибки.  
