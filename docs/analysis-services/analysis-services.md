...
Как правило [документации по службам аналитики Azure](https://docs.microsoft.com/azure/analysis-services/) входит в состав документации Azure. Если вас интересует с табличными моделями в облаке, лучше начать прямо отсюда. 

Это статье и документация в этом разделе является главным образом для SQL Server Analysis Services. Тем не менее по крайней мере для табличных моделей, как создавать и развертывать проекты является так же, независимо от используемой платформы. В этих разделах, чтобы получить дополнительные сведения см.
