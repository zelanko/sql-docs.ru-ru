...
**QueryMemoryLimit**   
Azure Analysis Services. Дополнительное свойство для управления, сколько памяти можно использовать временных результатов во время запроса. Применяется только к меры DAX и запросы. Не учитывает для выделения общей памяти, используемые для запроса. Указан в процентах, значение по умолчанию определяется плана. 
...
Это свойство может быть изменено. Указывается при установке значения 0 означает отсутствие ограничений.
