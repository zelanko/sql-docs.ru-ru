...
- Пользователь, запускающий Деа, может подключиться к исходному серверу базы данных, используя проверку подлинности Windows.
...
- Пользователь, запускающий Деа, может подключиться к целевому серверу базы данных, используя проверку подлинности Windows.
...
- Учетная запись службы, выполняющаяся распределенное воспроизведение клиенты, может подключаться к целевому серверу базы данных с помощью проверки подлинности
...
- Пользователь, запускающий Деа, может подключиться к серверу базы данных анализа с использованием проверки подлинности Windows.
- Пользователь, выполняющий Деа, имеет права системного администратора на исходном сервере базы данных.
...
[Обзор процесса сравнения рабочих нагрузок](database-experimentation-assistant-get-started.md), в котором объясняется процесс сравнения рабочих нагрузок в двух средах.
