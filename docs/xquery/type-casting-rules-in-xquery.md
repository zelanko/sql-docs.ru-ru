...
-   Приведение к списковым типам или списковых типов к другим недопустимо. Сюда входят типы пользовательских списков и встроенные списковые типы такие как **xs: IDREFS**, **xs: Entities**, и **xs: NMTOKENS**.  
  
-   Приведение с клиента или на **xs: QName** не поддерживается.  
...
```  
 ...
 Следующий запрос возвращает статическую ошибку, так как вы не знаете, сколько верхнего уровня <`root`> элементы являются в экземпляре документа.  
...
```  
 ...
 Путем указания одиночный <`root`> элемент в выражении, запрос завершается успешно. Запрос возвращает последовательность значения простого типа, типизированного как xs:string.  
...
```  
 ...
 В следующем примере переменная XML-типа содержит ключевое слово document, указывающее на коллекцию XML-схем. Это слово указывает, что экземпляр XML должен представлять собой документ с одиночным элементом верхнего уровня. При создании двух <`root`> элементов в экземпляре XML, он будет возвращена ошибка.  
...
```  
 ...
 В этом примере два значения, переданные в языка XQuery **min()** функция принадлежат разным типам. Таким образом, выполняется неявное преобразование где **целое число** тип повышается до **двойные** и два **двойные** сравниваются значения.  
  
 Повышение типов, описанное в данном примере, выполняется по следующим правилам.  
  
-   Встроенный производный числовой тип можно повышать до его базового типа. Например **целое число** может быть назначен **десятичное**.  
  
-   Объект **десятичное** может быть назначен **float,** и **float** может быть назначен **двойные**.  
  
 Так как неявное приведение допустимо только для числовых типов, следующие операции недопустимы.  
  
-   Неявное приведение строковых типов недопустимо. Например, если два **строка** ожидаемых значений типа и передать **строка** и **маркера**, неявное приведение выполняться и будет возвращена ошибка.  
