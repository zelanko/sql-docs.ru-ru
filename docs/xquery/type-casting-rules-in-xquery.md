...
```  
 ...
 Если указать в выражении `root` одноэлементный <>, запрос будет выполнен. Запрос возвращает последовательность значения простого типа, типизированного как xs:string.  
...
```  
 ...
 В следующем примере переменная XML-типа содержит ключевое слово document, указывающее на коллекцию XML-схем. Это слово указывает, что экземпляр XML должен представлять собой документ с одиночным элементом верхнего уровня. Если вы создаете в `root` экземпляре XML два элемента <>, он вернет ошибку.  
