...
description: Сведения об указании предикатов на шаге оси выражения пути XQuery для фильтрации последовательности узлов XML.
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.prod_service: sql
...
```  
 ...
 Обратите внимание, что в каждом случае предикат привязан к узлу выражения пути, где он применяется. Например, первое выражение пути выбирает первый `Name` элемент <> в каждом узле/Пеопле/персон и, используя предоставленный экземпляр XML, возвращает следующее:  
  
```  
<Name>John</Name><Name>Goofy</Name><Name>Daffy</Name>  
```  
  
 Однако второе выражение пути выбирает все `Name` элементы <>, которые находятся в первом узле/Пеопле/персон. Таким образом, оно возвращает следующий результат:  
...
```  
 ...
 Условие, заданное этим предикатом, применяется ко всем `Location` дочерним элементам узла <>. Возвращаются только те результаты, у которых значение атрибута LocationID равно 10.  
...
2.  Значения предиката могут быть только xs: integer, xs: Boolean или Node \* . Для Node \* предикат принимает значение true, если имеются узлы, и false для пустой последовательности. При использовании других числовых типов, например double или float, возникает ошибка статической типизации. Значение предиката выражения равно True тогда и только тогда, когда полученное значение типа integer равно значению позиции контекста. Кроме того, только целочисленные литеральные значения и функция **Last ()** уменьшают количество элементов в отфильтрованном выражении шага до 1.  
    ...
    -   Запрос возвращает третий дочерний узел элемента <> элементов дочернего элемента `Features` <`ProductDescription`> элемента в корне документа.  
...
3.  Если значение выражения предиката представляет собой простое значение типа Boolean, значение истинности предиката совпадает с ним.  
     ...
     Например, следующий запрос задается для переменной типа **XML**, которая содержит экземпляр XML, экземпляр XML опроса клиента. Запрос получает только тех клиентов, у которых есть дети. В этом запросе это будет \<HasChildren> 1 \</HasChildren> .  
    ...
    -   Запрос возвращает `Customer` дочерние элементы <>, значение предиката которых равно true, \<Survey> дочерних элементов корневого элемента документа. Результат:  
