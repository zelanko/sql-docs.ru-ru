---
title: Определение предикатов в шаге выражения пути | Документы Microsoft
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.prod_service: sql
...
---
# <a name="path-expressions---specifying-predicates"></a>Выражения пути — Определение предикатов
...
-   Тест узла. Дополнительные сведения см. в разделе [указания проверки узла в шаге выражения пути](../xquery/path-expressions-specifying-node-test.md).  
...
2.  Предикат значения могут быть только xs: Integer, xs: Boolean или узел\*. Для узла\*, предикат принимает значение True, если такие узлы существуют и False в пустую последовательность. При использовании других числовых типов, например double или float, возникает ошибка статической типизации. Значение предиката выражения равно True тогда и только тогда, когда полученное значение типа integer равно значению позиции контекста. Кроме того, только целые литеральные значения и **last()** функции уменьшают количество элементов шага фильтрации выражения до 1.  
...
3.  Если значение выражения предиката представляет собой простое значение типа Boolean, значение истинности предиката совпадает с ним.  
     ...
     Например, следующий запрос адресован **xml**тип переменной, содержащей экземпляр XML в экземпляре XML опроса клиента. Запрос получает только тех клиентов, у которых есть дети. В этом запросе это было бы \<HasChildren > 1\</HasChildren >.  
    ...
    -   Выражение в **для** цикл состоит из двух шагов, а второй шаг задает предикат. Значение предиката имеет тип Boolean. Если это значение равно True, значение истинности предиката также равно True.  
  
    -   Запрос возвращает <`Customer`> дочерние элементы, в которых значение предиката равно True, из \<Survey > дочерний элемент корневого элемента документа. Результат:  
...
4.  Если значение выражения предиката представляет последовательность как минимум из одного узла, значение истинности предиката равно True.  
 ...
 Например, следующий запрос возвращает идентификатор ProductModelID моделей продукции, описания которых в каталоге XML включает по крайней мере один компонент, дочерний элемент элемента <`Features`> элемент из пространства имен, связанный с **wm**префикс.  
...
-   Предложение WHERE определяет [метод exist() (тип данных XML)](../t-sql/xml/exist-method-xml-data-type.md).  
  
-   Выражения пути внутри **exist()** метод задает предикат на втором шаге. Если выражение предиката возвращает последовательность как минимум из одной функции, значение истинности выражения предиката равно True. В этом случае из-за **exist()** метод возвращает значение True, возвращается ProductModelID.  
