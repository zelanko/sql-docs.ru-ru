...
```  
 ...
 Обратите внимание, что в каждом случае предикат привязан к узлу выражения пути, где он применяется. Например, первое выражение пути выбирает первый <`Name`> элемент в узлы/People/Person, а также с помощью заданного экземпляра XML, возвращает следующее:  
  
```  
<Name>John</Name><Name>Goofy</Name><Name>Daffy</Name>  
```  
  
 Однако второе выражение пути выбирает все <`Name`> элементов, которые находятся под первого узла/People/Person. Таким образом, оно возвращает следующий результат:  
...
```  
 ...
 Условие, заданное при помощи предиката, применяется ко всем <`Location`> элементные узлы-потомки. Возвращаются только те результаты, у которых значение атрибута LocationID равно 10.  
...
1.  Если значение выражения предиката представляет пустую последовательность, значение предиката равно False.  
    ...
    ```  
     ...
     В этом запросе выражение пути возвращает только те из них <`Location`> узлов элементов, для которых задан атрибут LotSize. Если предикат возвращает пустую последовательность для конкретного <`Location`>, что участке цеха не возвращается в результате.  
...
2.  Предикат, значения могут быть только в том случае, xs: Integer, xs: Boolean или узел\*. Для узла\*, предикат возвращает значение True, если такие узлы существуют и значение False, если для пустой последовательности. При использовании других числовых типов, например double или float, возникает ошибка статической типизации. Значение предиката выражения равно True тогда и только тогда, когда полученное значение типа integer равно значению позиции контекста. Кроме того, только целые литеральные значения и **last()** функция уменьшают количество элементов шага фильтрации выражения до 1.  
     ...
     К примеру, следующий запрос возвращает третий дочерний узел элемента <`Features`> элемента.  
    ...
    -   Запрос возвращает третий дочерний узел элемента <`Features`> дочерние элементы <`ProductDescription`> дочерние элементы корневого элемента документа.  
...
3.  Если значение выражения предиката представляет собой простое значение типа Boolean, значение истинности предиката совпадает с ним.  
    ...
    -   Запрос возвращает <`Customer`> дочерние элементы, в которых значение предиката равно True, из \<Survey > дочерние элементы корневого элемента документа. Это результат:  
