...
```  
 ...
 Например, следующий запрос проверяет каждое \<расположение> элемента, чтобы определить, имеет ли он атрибут LocationID.  
...
```  
 ...
 Так как LocationID — это обязательный атрибут \<элемента Location>, вы получаете ожидаемый результат:  
