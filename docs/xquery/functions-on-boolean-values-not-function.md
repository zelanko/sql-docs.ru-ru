...
-   Затем запрос конструирует XML, содержащий элемент <`Product`> и его атрибут **ProductModelID** .  
  
-   В предложении WHERE для фильтрации строк используется [метод exist () (тип данных XML)](../t-sql/xml/exist-method-xml-data-type.md) . Метод **exist ()** возвращает значение true, если имеются \<ProductDescription> элементы, не имеющие \<спецификации> дочерних элементов. Обратите внимание на использование функции **NOT ()** .  
