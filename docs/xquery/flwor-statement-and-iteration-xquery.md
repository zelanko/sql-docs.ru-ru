...
-   [Выражение пути](../xquery/path-expressions-xquery.md), `//AWMI:root/AWMI:Location[1]/AWMI:step`, формирует входную последовательность. то есть последовательность из дочерних элементов <`step`> первого узлового элемента <`Location`>.  
