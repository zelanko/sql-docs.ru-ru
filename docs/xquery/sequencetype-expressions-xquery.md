 ...
 **Экземпляр** оператор может использоваться для определения типа динамический или в среде выполнения, значения указанного выражения. Пример:  
...
### <a name="example-d"></a>Пример Г  
 ...
 Следующие коллекции XML-схем определяет простой тип myType и элемент, <`root`>, типа MyType:  
...
-   Задается запрос, который должен быть выполнен для переменной. Выражение запроса получает значение идентификатора первого заказа из атрибута OrderList IDRERS тип первого <`Customer`>. Это значение имеет тип IDREF. Таким образом, оператор `instance of` возвращает True.  
 ...
 **Приведенное** выражение может использоваться для преобразования значения в определенный тип данных. Пример:  
