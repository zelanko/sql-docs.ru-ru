---
title: Выражения сравнения (XQuery) | Документы Microsoft
ms.custom: ''
ms.date: 08/09/2016
ms.prod: sql
ms.prod_service: sql
...
|ge|Больше или равно|  
 ...
 Например, следующий запрос извлекает \<изображение > элементы для модели продукта, где размер рисунка является «small:  
...
-   Отметим, что операторы значений работают только для атомарных значений, **data()** функция неявно используется для получения значения узла. То есть функция `data($P/PD:Size) eq "small"` выдает тот же результат.  
...
-   `>>` : **Операнд 1** выполните **операнда 2** в порядке документа.  
 ...
 Следующий запрос возвращает True, если в описании каталога продуктов \<гарантии > стоит перед элементом \<обслуживания > элемент в порядке документа для определенного продукта.  
