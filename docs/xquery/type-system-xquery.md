---
title: Тип системы (XQuery) | Документы Microsoft
ms.custom: ''
ms.date: 08/10/2016
ms.prod: sql
ms.prod_service: sql
...
-   [Данных функция &#40;XQuery&#41; ](../xquery/data-accessor-functions-data-xquery.md) и [функция string &#40;XQuery&#41;](../xquery/data-accessor-functions-string-xquery.md).  
...
## <a name="types-defined-in-xpath-data-types-namespace"></a>Типы, определенные в пространстве имен типов данных XPath  
 Типы, определенные в **http://www.w3.org/2004/07/xpath-datatypes** пространство имен имеет стандартным префиксом **xdt**. Эти типы обладают следующими свойствами:  
  
-   Они не могут использоваться при создании коллекции XML-схем. Эти типы используются в системе типов XQuery и используются для [XQuery и Статическая типизация](../xquery/xquery-and-static-typing.md). Может быть приведен к атомарным типам, например, **xdt: untypedAtomic**в **xdt** пространства имен.  
  
-   При обращении к нетипизированным XML, является статическим и динамическим типом узлов элементов **xdt: нетипизированный**, и имеет тип значения атрибута **xdt: untypedAtomic**. Результат **query()** метод создает нетипизированный XML. Это означает, что XML-узлы возвращаются в виде **xdt: нетипизированный** и **xdt: untypedAtomic**соответственно.  
...
## <a name="typed-value-vs-string-value"></a>Введенное значение Строковое значение  
 Каждый узел содержит типизированное и строковое значение. Для типизированных XML-данных тип значения определяется коллекцией XML-схем, связанной со столбцом или переменной, к которым происходит обращение. Для нетипизированных XML-данных тип значения — **xdt: untypedAtomic**.  
...
-   [Функция string &#40;XQuery&#41; ](../xquery/data-accessor-functions-string-xquery.md) возвращает строковое значение узла.  
...
```  
 ...
 В следующем примере производится вычисление общей суммы атрибутов `LaborHours`. `data()` Функция получает типизированные значения `LaborHours` атрибуты из всех <`Location`> элементы для модели продукта. Согласно схеме XML, связанные с `Instruction` столбца, `LaborHours` имеет **xs: decimal** типа.  
...
> [!NOTE]  
>  Явное использование **data()** функция в этом примере — только для иллюстрации. Если он не указан, **sum()** неявно применяет **data()** функция для извлечения типизированных значений узлов.  
