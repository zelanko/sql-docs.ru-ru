...
-   Два узла элементов, \<сводные> и \<функции>.  
  
    -   Элемент \<Summary> содержит один дочерний узел текстового узла со значением "часть описания".  
  
    -   Элемент \<> компонентов содержит три дочерних узла, \<> цвета, \<> веса и \<гарантии>. Каждый из этих узлов имеет по одному текстовому дочернему элементу и значения «Red», «25» и «2 years parts and labor» соответственно.  
...
```  
 ...
 Следующий запрос похож на предыдущий. Однако выражение в фигурных скобках указывает функцию **Data ()** для получения атомарного значения элемента <`root`> и присваивает его сконструированному элементу, <`e`>.  
...
```  
 ...
 Обратите внимание, что при построении `b` элемента <> атрибут объявления пространства имен указывается с пустой строкой в качестве значения. Это отменяет объявление пространства имен, произведенное по умолчанию в родительском элементе.  
 ...
 **Примечание** . Пример использования конструктора явных текстовых узлов см. в конкретном примере в статье [insert &#40;XML DML&#41;](../t-sql/xml/insert-xml-dml.md).  
 ...
 В следующем запросе созданный XML-документ содержит элемент, два атрибута, комментарий и инструкцию по обработке. Обратите внимание, что перед> <`FirstLocation` используется запятая, так как создается последовательность.  
