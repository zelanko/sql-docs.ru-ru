---
title: Установка проверки узла в шаге выражения пути | Документация Майкрософт
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.prod_service: sql
...
```  
 ...
 Таким образом, при выполнении этого выражения к XML-документам каталога продуктов в **ProductModel** таблицы, он извлекает все дочерние узлы из \<функции > дочерний узел элемента \< ProductDescription > элемента.  
  
 Выражение пути, `/child::PD:ProductDescription/attribute::ProductModelID`, состоит из двух шагов. В обоих имя узла задается в качестве проверки узла. Кроме того, в каждом шаге используется ось атрибутов. Поэтому в каждом шаге выбираются узлы основного узла оси с именем, заданным в качестве проверки узла. Таким образом, выражение возвращает **ProductModelID** узлом атрибутов \<ProductDescription > элементного узла.  
...
```  
 ...
 Аналогичным образом `/child::ProductDescription/child::Features/child::comment()` получает потомки узла комментариев \<функции > дочерний узел элемента \<ProductDescription > элементного узла.  
...
```  
 ...
 Поскольку `node()` является типом узла, вы будете получать все узлы оси-потомка. Это результат:  
...
### <a name="b-specifying-a-node-name-in-the-node-test"></a>Б. Указание имени узла при проверке узла  
 ...
 Следующий запрос возвращает значение <`Warranty`> элемента из XML-документа каталога продуктов хранится в `Production.ProductModel` таблицы:  
...
-   Необязательный квалификатор шага не задан ни в одном шаге выражения.  
 ...
 Запрос возвращает <`Warranty`> дочерние элементы <`Features`> дочернего элемента <`ProductDescription`> элемента.  
...
```  
 ...
 Этот символ-шаблон задан для имени узла. Таким образом, запрос возвращает все элементные дочерние узлы <`Features`> дочерний узел элемента <`ProductDescription`> элементного узла.  
...
```  
 ...
 Этот запрос возвращает <`Maintenance`> элементные узлы-потомки всех пространств имен из XML-документа каталога продуктов.  
...
-   Выражение возвращает текстовые узлы-потомки элемента <`Features`> дочернего элемента <`ProductDescription`> элементного узла.  
  
 Возвращаются только текстовые узлы. Это результат:  
  
```  
These are the product highlights.   
```  
  
 Следующий запрос возвращает комментарий дочерние узлы <`ProductDescription`> элемент:  
...
-   Таким образом, выражение возвращает комментарий дочерние узлы <`ProductDescription`> узлов элементов.  
