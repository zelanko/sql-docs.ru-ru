...
```  
 ...
 В результате, когда это выражение выполняется для XML-документов каталога продуктов в таблице **ProductModel** , он извлекает все дочерние узлы элементов \<компонентов> дочернего узла элемента> элемента \<ProductDescription.  
  
 Выражение `/child::PD:ProductDescription/attribute::ProductModelID`пути состоит из двух шагов. В обоих имя узла задается в качестве проверки узла. Кроме того, в каждом шаге используется ось атрибутов. Поэтому в каждом шаге выбираются узлы основного узла оси с именем, заданным в качестве проверки узла. Таким результатом, выражение возвращает узел атрибута **ProductModelID** узла элемента \<ProductDescription>.  
...
|---------------|-------------|-------------|  
|`comment()`|Верно для узла комментариев.|
  `following::comment()` выбирает все узлы комментариев, которые появляются после контекстного узла.|  
|`node()`|Верно для всех узлов.|
  `preceding::node()` выбирает все узлы, которые появляются перед контекстным узлом.|  
|`processing-instruction()`|Верно для узла инструкций по обработке.|
  `self::processing instruction()` выбирает все узлы инструкций по обработке в контекстном узле.|  
|`text()`|Верно для текстового узла.|
  `child::text()` выбирает все текстовые узлы, являющиеся потомками контекстного узла.|  
...
```  
 ...
 Аналогичным `/child::ProductDescription/child::Features/child::comment()` образом извлекает дочерние элементы \<узла комментария к компонентам> потомков узла элемента> \<ProductDescription.  
...
### <a name="b-specifying-a-node-name-in-the-node-test"></a>Б. Указание имени узла при проверке узла  
 ...
 Следующее выражение запроса возвращает элемент <`Warranty`> из XML-документа каталога продуктов, хранящегося в `Production.ProductModel` таблице.  
...
-   Необязательный квалификатор шага не задан ни в одном шаге выражения.  
 ...
 Запрос возвращает <`Warranty`> дочерние элементы элемента <`Features`> element элемента <`ProductDescription`>.  
...
```  
 ...
 Следующий запрос похож на предыдущий, за исключением того, что вместе с символом-шаблоном задано пространство имен. В результате возвращаются все элементные узлы-потомки в этом пространстве имен. Обратите внимание, `Features` что элемент> <может содержать элементы из разных пространств имен.  
...
```  
 ...
 Этот запрос возвращает дочерние узлы <`Maintenance`> элементов во всех пространствах имен из XML-документа каталога продуктов.  
