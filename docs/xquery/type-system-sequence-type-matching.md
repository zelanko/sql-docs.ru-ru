...
### <a name="example-determining-whether-a-sequence-is-empty"></a>Пример Определить, является ли последовательность пустой  
 **Empty()** тип последовательности, которая может использоваться в выражении типа последовательности для определения последовательности, возвращаемых в указанном выражении пустую последовательность.  
 ...
 В следующем примере схема XML допускает <`root`> элемент пуст:  
...
```  
 ...
 Теперь, если типизированного экземпляра XML задает значение для <`root`> элемент, `instance of empty()` возвращает значение False.  
...
```  
 ...
 Если <`root`> элемент пуст в экземпляре, его значением является пустая последовательность и `instance of empty()` возвращает значение True.  
...
### <a name="example-cardinality-in-sequence-expressions"></a>Пример Количество элементов выражения последовательности  
 В данном примере показывается влияние количества элементов в выражении последовательности. Следующая схема XML определяет <`root`> элемента, типа byte и который может быть пустым.  
...
```  
 ...
 При внесении <`root`> элемент nil, его значение представляет собой пустую последовательность. Поэтому выражение `/root[1]` возвращает пустую последовательность. Следовательно, выражение `instance of xs:byte` возвращает значение False. Обратите внимание, что количество элементов по умолчанию в данном случае равно 1.  
...
```  
 ...
 Все выражения XQuery в трех следующих запросах, возвращают дочерний узел элемента <`root`> элемента. Поэтому выражение типа последовательности `instance of node()` возвращает значение True, а два других выражения (`instance of text()` и `instance of document-node()`) — значение False.  
...
```  
 ...
 В следующем запросе `instance of document-node()` выражение возвращает значение True, так как родительский <`root`> элемент является узел документа.  
...
### <a name="example-a"></a>Пример A  
 Следующая схема XML определяет `CustomerType` сложный тип где <`firstName`> и <`lastName`> элементы являются необязательными. В указанном XML-экземпляре может понадобиться определить, существует ли запись, содержащая фамилию определенного покупателя.  
...
```  
 ...
 В следующем запросе используется `instance of element (firstName)` выражением, чтобы определить ли первый дочерний элемент элемента <`customer`> является элементом с именем <`firstName`>. Если является, выражение возвращает значение True.  
...
```  
 ...
 При удалении <`firstName`> элемента из экземпляра, запрос будет возвращать значение False.  
...
### <a name="example-b"></a>Пример Б  
 ...
 В следующем примере два <`Customer`> являются элементы в экземпляре XML, к которому происходит запрос к двум разным типам `CustomerType` и `SpecialCustomerType`. Допустим, вам необходимо знать тип <`Customer`> элемента, возвращенного выражением. Следующая коллекция XML-схем определяет типы данных `CustomerType` и `SpecialCustomerType`.  
...
```  
 ...
 Данная коллекция схем XML используется для создания типизированного **xml** переменной. Переменной присваивается экземпляр XML имеет два <`customer`> элементы к двум разным типам. Первый элемент имеет тип данных `CustomerType`, а второй элемент — `SpecialCustomerType`.  
...
```  
 ...
 Если вы измените выражение предыдущего запроса и получить второй <`customer`> элемент (`/x:customer)[2]`), `instance of` возвратит True.  
  
### <a name="example-c"></a>Пример В  
 В данном примере используется проверка атрибута. Следующая XML-схема определяет сложный тип CustomerType с атрибутами CustomerID и Age. Атрибут Age является необязательным. Для конкретного экземпляра XML, может потребоваться определить, присутствует ли атрибут Age в <`customer`> элемента.  
