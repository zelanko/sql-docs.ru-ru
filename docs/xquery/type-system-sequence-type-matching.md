 ...
 **Empty()** тип последовательности можно использовать в выражении типа последовательности для определения последовательности, возвращаемой выражением пустую последовательность.  
...
### <a name="example-querying-against-an-xml-type-column"></a>Пример: Запросы к столбцу типа xml  
 В следующем примере запрос адресован столбцу Instructions типа **xml** введите [!INCLUDE[ssSampleDBobject](../includes/sssampledbobject-md.md)] базы данных. Это типизированный XML-столбец, так как имеется связанная с ним схема. Схема XML определяет целочисленный атрибут `LocationID`. Таким образом, в выражении последовательности `instance of xs:integer?` возвращает значение True.  
...
```  
 ...
 Данная коллекция схем XML используется для создания типизированной **xml** переменной. XML-экземпляр, связанный с данной переменной, содержит два элемента <`customer`> разных типов. Первый элемент имеет тип данных `CustomerType`, а второй элемент — `SpecialCustomerType`.  
...
-   При проверке элементов имя типа должно следовать признака вхождения (**?**).  
  
-   **элемент (ElementName, TypeName)** не поддерживается.  
  
-   **элемент (\*, TypeName)** не поддерживается.  
  
-   **элемент_схемы()** не поддерживается.  
