---
title: Поддержка пространств имен в XQuery | Документы Microsoft
ms.custom: ''
ms.date: 03/07/2017
ms.prod: sql
ms.prod_service: sql
...
```  
 ...
 Обратите внимание, что **имен** ключевое слово используется для определения нового префикса пространства имен, «AWMI:». Этот префикс затем должен применяться ко всем элементам запроса, которые попадают в область действия этого пространства имен.  
...
### <a name="d-construction-using-default-namespaces"></a>Г. Создание с помощью пространств имен по умолчанию  
 Можно также определить пространства имен по умолчанию для использования в создании XML. Например, в следующем запросе показано, как можно указать пространство имен по умолчанию «uri:SomeNamespace»\\для использования по умолчанию для локально именованных элементов, которые формируются, такие как `<Result>` элемента.  
