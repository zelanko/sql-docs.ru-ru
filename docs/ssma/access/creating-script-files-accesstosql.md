...
4.  **Повторное подключение поставщика:** это позволяет пользователю задавать параметры восстановления подключения в случае сбоя подключения. Это можно задать для исходных и целевых серверов.  
...
7.  **Операция остановки:** во время промежуточного операции, если пользователь хочет остановить эту операцию, затем **«Ctrl + C»** можно использовать сочетание клавиш. SSMA для доступа к консоли будет ожидать завершения операции и прекращает выполнение консоли.  
