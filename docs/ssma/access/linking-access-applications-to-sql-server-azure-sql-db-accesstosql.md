...
---
# <a name="linking-access-applications-to-sql-server---azure-sql-db-accesstosql"></a>Связывания приложения Access в SQL Server — базы данных Azure SQL (AccessToSQL)
...
## <a name="linking-access-and-sql-server-tables"></a>Связывание доступа и таблицы SQL Server  
При связывании таблицы Access для [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] или таблице SQL Azure, базы данных Jet хранятся сведения о соединении и метаданных таблицы, но данные хранятся в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] или SQL Azure. Это позволит вашим приложениям доступа работают с доступа к таблицам, даже если фактический таблиц и данных определены в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] или SQL Azure.  
  
> [!NOTE]  
> Если вы используете [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] проверки подлинности, ваш пароль будет сохранен в виде открытого текста в связанных таблицах Access. Рекомендуется использовать проверку подлинности Windows.  
...
После связывания таблицы таблиц в SSMA будут отображаться со значком небольшой ссылки. В режиме доступа таблиц будут отображаться со значком «связанные». Это шар со стрелкой, указывающей на него.  
  
При открытии таблицы в Access данные получаются с помощью курсора набора ключей. Для больших таблиц это означает, что все, что данные не получены за один раз. Однако при просмотре таблицы, доступ будет извлечь дополнительные данные по мере необходимости.  
  
> [!IMPORTANT]  
> Чтобы связать таблицы access с помощью базы данных SQL azure, мы должны собственного Client(SNAC) SQL Server версии 10.5 или более поздней версии.   
...
Если вы связали открывать таблицы в одном экземпляре SQL Server, и вы хотите изменить ссылки на другой экземпляр, необходимо повторно связать таблицы.  
...
Если [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] или изменяются определения таблиц SQL Azure, можно разорвать связь и повторно связывать таблицы в SSMA с помощью приведенных ранее в этом разделе. Также можно обновить таблицы с помощью доступа.  
...
В следующих разделах список проблем, которые могут возникнуть в существующие приложения Access, после миграции баз данных с доступом к [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] или SQL Azure и затем связать таблицы, предоставляя причины и способы их устранения.  
...
-   Приложение зависит от функций, которые не существуют в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] или SQL Azure. В этом случае Jet для таблицы локально для выполнения запроса SELECT.  
  
-   Запросы, которые обновляют или удаляют множество строк отправляются Jet как параметризованный запрос для каждой строки.  
  
**Решение:** преобразование медленного выполнения запросов в передаваемых запросах, хранимых процедур или представлений. Преобразование для запросов к серверу будут связаны следующие проблемы:  
...
### <a name="after-migration-some-queries-will-not-let-the-user-add-a-new-record"></a>После миграции некоторых запросов не позволит пользователю добавить новую запись  
