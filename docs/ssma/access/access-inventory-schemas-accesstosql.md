...
|**IsUpdatable**|**бит**|Указывает, если базы данных могут обновляться. Если значение равно 1, базы данных является обновляемым. Если значение равно 0, базы данных доступен только для чтения.|  
...
|**IsNullable**|**бит**|Указывает, если столбец может содержать значения null. Если значение равно 1, столбец может содержать значения null. Если значение равно 0, столбец не может содержать значения null. Обратите внимание, что правило проверки также может использоваться для предотвращения значения null.|  
|**Тип данных**|**nvarchar(4000)**|Тип доступа к данным столбца, например **текст** или **длинные**.|  
|**IsAutoIncrement**|**бит**|Указывает, если столбец автоматически увеличивается целочисленных значений. Если значение равно 1, целые числа с автоматически увеличивающимися значениями.|  
...
|**IsUnique**|**бит**|Указывает, если каждый элемент в индексе должно быть уникальным. В индексе многостолбцовый сочетание значений должен быть уникальным. Если значение равно 1, то индекс обеспечивающий уникальность значений.|  
|**IsPK**|**бит**|Указывает, если индекс был автоматически создан как часть определения первичного ключа.|  
|**IsClustered**|**бит**|Указывает, если индекс является кластеризованным. Кластеризованный индекс переупорядочивает физического хранилища данных. Таблица может иметь только один кластеризованный индекс.|  
...
|**IsCascadeForUpdate**|**бит**|Указывает, что при обновлении значения первичного ключа все строки, которые ссылаются на значения этого раздела также обновляются.|  
|**IsCascadeForDelete**|**бит**|Указывает, что при удалении значения первичного ключа все строки, которые ссылаются на значения этого раздела также будут удалены.|  
|**IsEnforced**|**бит**|Указывает, соблюдение ограничения внешнего ключа.|  
...
|**IsUpdateable**|**бит**|Указывает, является ли запрос обновляемые или только для чтения.|  
...
[Экспорт инвентаризацию доступа](http://msdn.microsoft.com/en-us/7e1941fb-3d14-4265-aff6-c77a4026d0ed)  
