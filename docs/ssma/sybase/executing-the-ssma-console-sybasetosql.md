...
-   `report-errors-to:`Указывает расположение отчета об ошибках для операции синхронизации (необязательный атрибут). Если указан только путь к папке, то создается файл с именем **таржетсинчронизатионрепорт. XML** .  
...
-   `report-errors-to:`Указывает расположение отчета об ошибках для операции обновления (необязательный атрибут). Если указан только путь к папке, то создается файл с именем **саурцедбрефрешрепорт. XML** .  
...
-   `write-summary-report-to`Указывает путь, по которому будет создан сводный отчет. Если упоминается только путь к папке, то создается файл с именем **конвертсклрепорт. XML** . (необязательный атрибут)  
