...
* Предоставлено исправление для преобразования@ROWCOUNT @ и@ERROR @ в той же инструкции SET.  
...
* Консолидированный продукт "SSMA for Sybase", который [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] поддерживает 2005 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] , 2008 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] , "Denali" и Azure SQL.  
...
* Удалена зависимость от отдельной базы данных с именем **сисдб**, которая содержит функции эмуляции Sybase (устанавливается как часть пакета расширений).
...
* Добавлены улучшения при преобразовании объектов:  
  ...
  * Функция IDENTITY с инструкцией SELECT INTO.  
  * Кластеризованные ограничения и индексы только для данных — заблокированы.  
  * Временные таблицы, созданные функцией SELECT INTO.  
  * Ограничения и индексы для временных таблиц.  
  * Поддерживаются новые [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] типы DateTime 2008.  
  * Sybase 15,0 поддержка подключения и типов.  
...
* Добавлен новый параметр преобразования, позволяющий настроить способ обработки SSMA в следующих ситуациях:  
  * Инструкция CAST или CONVERT, которая содержит двоичную строку.  
  * Проверяет наличие значений NULL в выражениях равенства.  
  * Таблицы-посредники.  
  * Номера ошибок сообщений пользователя для инструкции RAISERROR.  
  * Инструкции UPDATE, содержащие неразрешенные идентификаторы.  
...
* Исправления ошибок, в том числе:
  * SSMA теперь преобразует *таблицу* блокировки таблицы в {Shared | Исключающие операторы режима}, добавляя подсказку TABLOCK или TABLOCKX к последующему запросу SELECT в таблице.  
