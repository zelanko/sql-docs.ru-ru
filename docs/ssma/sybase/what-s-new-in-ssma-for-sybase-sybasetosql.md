...
* Предоставлено исправление для `@@ROWCOUNT` преобразования `@@ERROR` и в той `SET` же инструкции.
...
* Удалена зависимость от отдельной базы данных с `SysDB`именем, которая содержит функции эмуляции Sybase (устанавливается как часть пакета расширений).
...
* Добавлены улучшения при преобразовании объектов:
  ...
  * Временные таблицы, `SELECT INTO`созданные.
  * Ограничения и индексы для временных таблиц.
  * Поддерживаются новые [!INCLUDE [ssSQL10](../../includes/sssql10-md.md)] типы DateTime.
...
* Добавлен новый параметр преобразования, позволяющий настроить способ обработки SSMA в следующих ситуациях:
  * Оператор `CAST` или `CONVERT` , содержащий двоичную строку.
...
* Добавлен новый параметр миграции, позволяющий указать, как SSMA должен поддерживать даты, находящиеся [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] за пределами диапазона дат.
...
* Исправления ошибок, в том числе:
  * SSMA теперь преобразует `LOCK TABLE <table> IN { SHARED | EXCLUSIVE } MODE` инструкции, добавляя `TABLOCK` указание `TABLOCKX` или к последующему `SELECT` запросу в таблице.
