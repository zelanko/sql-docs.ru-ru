---
title: "Сочетания клавиш (Oracle в SQL) | Документы Microsoft"
ms.prod: sql-non-specified
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: 
ms.technology:
- sql-ssma
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: ae7b5af8-eeb5-4c40-9f76-ebbab3d29e5d
caps.latest.revision: 3
author: Shamikg
ms.author: Shamikg
manager: v-thobro
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: 1419847dd47435cef775a2c55c0578ff4406cddc
ms.openlocfilehash: d15ac434efe8b87bf859143b6f16c35e8a302c3f
ms.contentlocale: ru-ru
ms.lasthandoff: 08/02/2017

---
# <a name="shortcut-keysoracle-to-sql"></a>Сочетания клавиш (Oracle To SQL)
Можно использовать сочетания клавиш для перемещения по SSMA приложения. Дополнительные сведения см. в следующих разделах:  
  
-   [Команды меню и другие сочетания клавиш &#40; Oracle или SQL &#41;](../../ssma/oracle/menu-commands-and-other-shortcut-keys-oracle-to-sql.md)  
  
## <a name="see-also"></a>См. также:  
[Начало работы с SSMA для Oracle &#40; OracleToSQL &#41;](../../ssma/oracle/getting-started-with-ssma-for-oracle-oracletosql.md)  
  

