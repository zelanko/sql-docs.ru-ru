...
ms.locfileid: "38983406"
---
# <a name="migrating-oracle-data-into-sql-server-oracletosql"></a>Миграция данных Oracle в SQL Server (OracleToSQL)
После успешной синхронизации преобразованных объектов с [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)], можно перенести данные из Oracle в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)].  
  
> [!IMPORTANT]  
> Если используется обработчик модуль переноса данных на стороне сервера, затем, перед переносом данных, необходимо установить SSMA для Oracle Extension Pack и поставщикам Oracle на компьютере, на котором выполняется SSMA. Также должна запущена служба агента SQL Server. Дополнительные сведения о том, как установить пакет расширений см. в разделе [Установка компонентов сервера (OracleToSQL)](http://msdn.microsoft.com/33070e5f-4e39-4b70-ae81-b8af6e4983c5)  
...
Перенос данных является операцией массовой загрузки, которая перемещает строки данных из таблиц Oracle в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] таблиц в транзакции. Число строк, загруженных в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] при каждой транзакции, настроенный в параметрах проекта.  
...
3.  Чтобы перенести данные, возникают два варианта:  
    ...
    **Перенос данных на стороне сервера:**  
    ...
    -   Прежде чем выполнять перенос данных на стороне сервера, обеспечить:  
        ...
        1.  SSMA для Oracle Extension Pack устанавливается на экземпляр [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)].  
