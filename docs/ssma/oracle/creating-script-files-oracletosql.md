...
4.  **Повторное подключение поставщика:** это позволяет пользователю устанавливать повторное подключение, если параметры ошибки подключения. Это можно задать для исходного и целевого серверов.  
...
7.  **Операция остановки:** при середине операции, если пользователь хочет остановить операцию, затем **«Ctrl + C»** можно использовать сочетание клавиш. SSMA для Oracle консоли будет ожидать завершения операции и завершает выполнение консоли.  
...
Например, перенос данных обычно конкретной таблицы в базе данных Oracle следует иерархию: схемы -&gt; таблицы.  
