...
ms.locfileid: "76762268"
---
# <a name="o2ss0330-unable-to-convert-close-statement-error"></a>O2SS0330: не удалось преобразовать оператор CLOSE (Error)

В этой статье описывается, почему Помощник по миграции SQL Server (SSMA) для Oracle выдает ошибку при `CLOSE` преобразовании инструкции в процедуре, `SYS_REFCURSOR` в которой возвращается параметр.

## <a name="background"></a>Историческая справка

В Oracle `SYS_REFCURSOR` используется для передачи курсоров из хранимой процедуры и в нее.

Всякий раз, когда SSMA встречает `SYS_REFCURSOR` переменную в качестве возвращаемого типа процедуры и использует ту же переменную с `CLOSE` инструкцией, она выдает сообщение об ошибке.
