...
* Улучшено преобразование `COUNT` для функции в таблицах MQ.
...
* Замените использование функции TRIM в запросах обнаружения z/OS `RTRIM` / `LTRIM`с помощью.
...
* Устранение проблемы с загрузкой значений столбцов по `DATE` умолчанию для и `TIMESTAMP` для z/OS.
