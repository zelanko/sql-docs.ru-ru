...
|**Collector**|Метаданные о схеме источника собирает и сохраняет его в проект.|  
|**Converter**|Преобразует структуры объектов базы данных источника, таких как таблицы и хранимые процедуры в соответствующий [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] структуры.|  
|**Migrator данных**|Миграция данных из базы данных-источника в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)].|  
|**Formatter**|Подкомпонента преобразователя, который формирует скрипты для [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] схемы.|  
|**Графический пользовательский интерфейс**|Сообщений, которые появляются при использовании средства SSMA.|  
|**Linker**|Разрешает идентификаторов SQL и предоставляет сведения для других компонентов.|  
...
|**Synchronizer**|Загружает источник объектов базы данных в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)].|  
|**TreeConverter**|Преобразует объекты в исходных метаданных в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] метаданных.|  
|**Tester**|Сообщений, которые появляются при использовании инженер-испытатель SSMA.|  
...
|**Info**|Запись в журнал информационные, предупреждения, ошибки и сообщения о неустранимых ошибках.|  
