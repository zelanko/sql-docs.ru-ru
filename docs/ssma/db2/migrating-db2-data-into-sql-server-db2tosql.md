...
ms.locfileid: "38980616"
---
# <a name="migrating-db2-data-into-sql-server-db2tosql"></a>Миграция данных DB2 в SQL Server (DB2ToSQL)
После успешной синхронизации преобразованных объектов с [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)], можно перенести данные из DB2 в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)].  
...
Перед переносом данных [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)], проверьте параметры проекта миграции в **параметры проекта** диалоговое окно.  
...
Перенос данных является операцией массовой загрузки, которая перемещает строки данных из таблицы DB2 в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] таблиц в транзакции. Число строк, загруженных в [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] при каждой транзакции, настроенный в параметрах проекта.  
...
1.  Проверьте выполнение следующих условий.  
    ...
    -   Вы синхронизировали преобразованных объектов с [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] базы данных.  
...
3.  Чтобы перенести данные, возникают два варианта:  
    ...
    **Перенос данных на стороне сервера:**  
    ...
    -   Прежде чем выполнять перенос данных на стороне сервера, обеспечить:  
        ...
        1.  SSMA для DB2 Extension Pack устанавливается на экземпляр [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)].  
...
5.  В **подключение к DB2** диалоговом окне введите учетные данные для подключения и нажмите кнопку **Connect**. Дополнительные сведения о подключении к DB2, см. в разделе [подключение к базе данных DB2 &#40;DB2ToSQL&#41;](../../ssma/db2/connecting-to-db2-database-db2tosql.md)  
    ...
    Для подключения к целевой базе данных [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)], введите учетные данные для подключения в **подключение к SQL Server** диалоговое окно и нажмите кнопку **Connect**. Дополнительные сведения о подключении к [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)], см. в разделе [подключение к SQL Server](http://msdn.microsoft.com/b59803cb-3cc6-41cc-8553-faf90851410e)  
