...
V7.7 выпуск SSMA для MySQL содержит следующие изменения:
- SSMA для MySQL была дополнена целевых исправлений, которые улучшают метрики качества и преобразования.
- На основе популярных спроса, 32-разрядной версии SSMA для MySQL — обратно. По сравнению с предыдущей реализации (до v7.4), существует два пакета установщика, но они не могут быть установлены параллельно. В результате необходимо выбрать наиболее подходящий на основе компонентов подключения к версии, что у вас есть. Всегда предпочтительнее использовать 64-разрядной версии, если это возможно.
- SSMA для MySQL теперь имеет режим подключения строки подключения ODBC, который позволяет использовать драйверы ODBC сторонних разработчиков, которые совместима с MySQL.
