...
- Скачайте и установите:
    - [Помощник по миграции данных](https://www.microsoft.com/en-us/download/details.aspx?id=53595) v3.6 или более поздней версии.
    ...
    - [SSMS](https://docs.microsoft.com/en-us/sql/ssms/download-sql-server-management-studio-ssms) 17.0 или более поздней версии.
...
2. Откройте [модули PowerShell](https://msdnshared.blob.core.windows.net/media/2018/03/PowerShell-Modules.zip), а затем сохраните их в созданную папку.
      ...
      ![Модули PowerShell](../dma/media//dma-assesssqlonprem/dma-powershell-modules.png)

    Каждая папка содержит связанный psm1-файл, как показано на следующем рисунке:

   ![Файлов psm1 модули PowerShell](../dma/media//dma-assesssqlonprem/dma-powershell-modules-psm1-files.png)
   ...
   ![свойства файлов psm1](../dma/media//dma-assesssqlonprem/dma-psm1-file-properties.png)
...
При использовании в CSV-файл для импорта данных, убедитесь, существует только два столбца данных — **имя экземпляра** и **имя базы данных**, и что у столбцов нет строки заголовка.
 ...
 ![содержимое файла CSV](../dma/media//dma-assesssqlonprem/dma-csv-file-contents.png)
...
- AssessmentFlag
 ...
 ![Содержимое таблицы SQL Server](../dma/media//dma-assesssqlonprem/dma-sql-server-table-contents.png)
...
После загрузки модули PowerShell в каталог modules и проведения инвентаризации, необходимо провести оценку, масштабируемой, откройте PowerShell и выполните функцию dmaDataCollector.
  ...
  ![функция вхождений dmaDataCollector](../dma/media//dma-assesssqlonprem/dma-dmaDataCollector-function-listing.png)
...
|**Имя_сервера** | Имя экземпляра SQL Server при использовании запасов **SqlServer** в **getServerListFrom** параметра. |
...
Если непредвиденная ошибка, то командное окно, которое получает инициировано этот процесс будет завершен.  Просмотрите журнал ошибок, чтобы определить причину сбоя.
  ...
  ![Расположение журнала ошибок](../dma/media//dma-assesssqlonprem/dma-error-log-file-location.png)
...
После завершения оценку, теперь вы готовы для импорта данных в SQL Server для анализа. Чтобы использовать файл JSON для оценки, откройте PowerShell и выполните функцию dmaProcessor.
  ...
  ![Листинг функция dmaProcessor](../dma/media//dma-assesssqlonprem/dma-dmaProcessor-function-listing.png)
...
2. Загрузка хранилища данных.
      ...
      ![Загрузить содержимое LoadWarehouse](../dma/media//dma-assesssqlonprem/dma-LoadWarehouse-loaded.png)
...
Сценарий LoadWarehouse может использоваться для предоставления простых инструкций TSQL для задания владельцы базы данных.
  ...
  ![Владельцы параметр LoadWarehouse](../dma/media//dma-assesssqlonprem/dma-LoadWarehouse-set-owners.png)
...
2. Введите данные сервера, которые указывают на вашей **DMAWarehouse** базы данных, а затем выберите **нагрузки**.
    ...
    > Не нажимайте клавишу ВВОД, чтобы принять значения.
      ...
      ![Загрузить шаблон DMA отчеты Power BI](../dma/media//dma-assesssqlonprem/dma-reports-powerbi-template-loaded.png)

   После отчета обновляются данные из **DMAWarehouse** базы данных, появится сообщение следующего вида.

   ![Представление отчетов DMAWarehouse](../dma/media//dma-assesssqlonprem/dma-DMAWarehouse-report.png)
...
- В локальной оценок
  ...
  ![DMA отчет закладки](../dma/media//dma-assesssqlonprem/dma-report-bookmarks.png)
...
### <a name="readiness-"></a>% Готовности
  ...
  ![Процент готовности DMA](../dma/media//dma-assesssqlonprem/dma-readiness-percentage.png)
...
### <a name="readiness-count"></a>Число готовности
  ...
  ![Число готовности DMA](../dma/media//dma-assesssqlonprem/dma-readiness-count.png)
...
### <a name="readiness-bucket"></a>Готовность контейнеров
  ...
  ![Контейнер готовности DMA](../dma/media//dma-assesssqlonprem/dma-readiness-bucket.png)
...
### <a name="issues-word-cloud"></a>Облако Word проблемы
  ...
  ![Проблемы DMA WordCloud](../dma/media//dma-assesssqlonprem/dma-issues-word-cloud.png)
...
### <a name="database-readiness"></a>Готовность к базе данных
  ...
  ![Отчет о готовности базы данных DMA](../dma/media//dma-assesssqlonprem/dma-database-readiness-report.png)
...
- ImpactedObjectName
 ...
 ![Углубленная детализация DMA готовности базы данных отчетов](../dma/media//dma-assesssqlonprem/dma-database-readiness-report-drilldown.png)
...
Эта задача обеспечивает фильтрацию отчета план исправления к текущему уровню иерархии на основе точки, с которой выбран параметр детализации.
  ...
  ![Отфильтрованные детализации отчета DMA готовности базы данных](../dma/media//dma-assesssqlonprem/dma-database-readiness-report-drilldown-filtered.png)

  ![Отчет плана исправления DMA](../dma/media//dma-assesssqlonprem/dma-remediation-plan-report.png)
...
Можно также воспользоваться отчетом плана исправления для построения пользовательских обновлений плана с помощью фильтров в **фильтры визуализаций** колонке.
  ...
  ![Планирование обновлений DMA параметры фильтра отчета](../dma/media//dma-assesssqlonprem/dma-remediation-plan-report-filter-options.png)
