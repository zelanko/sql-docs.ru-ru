---
title: "Редактор учетной записи-посредника — добавление субъекта | Документация Майкрософт"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- tools-ssms
ms.tgt_pltfrm: 
ms.topic: article
f1_keywords:
- sql13.ag.proxy.editor.f1
ms.assetid: 9d33749f-9653-4691-9a0f-06ed7dd6166f
caps.latest.revision: 3
author: stevestein
ms.author: sstein
manager: jhubbard
ms.workload: Inactive
ms.translationtype: Human Translation
ms.sourcegitcommit: 2edcce51c6822a89151c3c3c76fbaacb5edd54f4
ms.openlocfilehash: 171707ed9dad6a642ed4e1d72dd716689a8ec699
ms.contentlocale: ru-ru
ms.lasthandoff: 06/22/2017

---
# <a name="proxy-editor---add-principal"></a>Редактор учетной записи-посредника — добавление субъекта
При помощи этой страницы можно предоставить участникам сервера доступ к учетной записи-посреднику агента [!INCLUDE[msCoName](../../includes/msconame_md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] .  
  
## <a name="options"></a>Параметры  
**Тип участника**  
Тип участника для отображения.  
  
**Доступные участники**  
Список участников выбранного типа.  
  
**Название**  
Выберите имя участника, которому необходимо предоставить доступ, или **Имя** , чтобы предоставить доступ всем участникам в списке.  
  
## <a name="see-also"></a>См. также:  
[Создание учетной записи-посредника агента SQL Server](../../ssms/agent/create-a-sql-server-agent-proxy.md)  
  

