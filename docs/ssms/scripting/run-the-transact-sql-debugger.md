...
description: Сведения о том, как настроить отладчик Transact-SQL и использовать его для отладки кода Transact-SQL. Отладчик можно запустить в экземпляре ядра СУБД, который находится на другом компьютере.
ms.prod: sql
