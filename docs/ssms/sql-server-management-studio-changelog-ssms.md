...
## <a name="ssms-180-preview-7download-sql-server-management-studio-ssmsmd"></a>[SSMS 18.0 (предварительная версия 7)](download-sql-server-management-studio-ssms.md)

Номер сборки: 15.0.18092.0<br>
Дата выпуска: 1 марта 2019 г.

Предварительная версия 7 — это новейшая общедоступная предварительная версия SSMS 18.0. Чтобы получить последнюю общедоступную версию SSMS, [скачайте и установите SSMS 17.9.1](#ssms-1791-latest-ga-release).

### <a name="whats-new-in-preview-7"></a>Новые возможности в предварительной версии 7

В этом разделе перечислены новые возможности предварительной версии 7 SSMS 18.0. Полный журнал изменений, начиная с версии SSMS 17.9.1, см. в разделе [Предварительная версия SSMS 18.0 — совокупный журнал изменений до предварительной версии 7](#ssms-180-preview---cumulative-changelog-through-preview-7).

- **SSMS**
  - Добавлены функциональные возможности для переноса параметров пользователей SSMS из предыдущей версии SSMS (17.x или более старых предварительных версий 18.0) при запуске SSMS в первый раз.
  - Добавлена поддержка параметров сортировки `UTF8_BIN2`.
  - Добавлена поддержка [пограничных ограничений](../relational-databases/tables/graph-edge-constraints.md) в SMO и SSMS.
  - Добавлен **облачный свидетель** — новый тип кворума и ресурса в SMO и SSMS.
  - Добавлена логика для уведомления пользователя о том, что импорт неструктурированных файлов мог вызвать переименование столбцов.
  - Добавлена поддержка инструкции Showplan для LocalCube RelOp для `DW ROLLUP` и `CUBE`.
  - Добавлена поддержка импорта и экспорта приложения уровня данных с графовой таблицей.
  - Изменен набор правил оценки уязвимостей, которые выполняются в управляемых экземплярах базы данных SQL Azure, чтобы результатом оценки уязвимостей была согласованность базы данных SQL Azure.
  - [Оценка уязвимостей SQL](../relational-databases/security/sql-vulnerability-assessment.md) теперь поддерживает хранилище данных SQL Azure.
  - Добавлена **классификация данных** для SMO. Объект столбца теперь предоставляет новые свойства: `SensitivityLabelName`, `SensitivityLabelId`, `SensitivityInformationTypeName`, `SensitivityInformationTypeId` и `IsClassified` (только для чтения).
    Дополнительные сведения см. в разделе https://docs.microsoft.com/sql/t-sql/statements/add-sensitivity-classification-transact-sql.
  - Добавлен пункт **Классификационный отчет** в меню **Классификация данных**.
  - Добавлены **имена входа AAD** как новый тип входа в SMO и SSMS при подключении к управляемым экземплярам SQL Azure.
  - В Azure Data Studio можно выполнить запрос или создать новую записную книжку при щелчке базы данных в обозревателе объектов правой кнопкой мыши.
  - В средство просмотра XEvent в окне показа планов можно вывести дополнительные XEvents.
  - Маскирование данных:
    - Теперь поддерживаются файлы конфигурации JSON.
    - Формат XML-файла конфигурации был изменен на более гибкий.  Существующие файлы конфигурации необходимо создать заново.
    - Базы данных, содержащие оптимизированные для памяти и темпоральные таблицы, теперь могут быть скрыты.  К оптимизированным для памяти и темпоральным таблицам по-прежнему применяются некоторые ограничения.
    - Операции копирования локальной базы данных теперь используют параметр `COPY_ONLY` для шага `BACKUP DATABASE`.
    - Скрытые базы данных теперь используют модель восстановления `SIMPLE`, пока идет операция маскирования, для сокращения использования журнала, и возвращаются к исходному режиму после завершения.
    - Скрытая база данных теперь называется `<output database name>-MaskInProgress` во время выполнения операции маскирования и переименовывается в `<output database name>` после того, как операция завершилась успешно.
    - Порядок отображения столбцов в пользовательском интерфейсе конфигурации изменен: теперь они стоят по порядковым номерам, а не по алфавиту.
    - Удален параметр массового выбора столбцов в пользовательском интерфейсе конфигурации во избежание непредвиденного поведения.
    - Улучшена внутренняя логика повторных попыток для ошибок подключения и команд.

- **Службы SSIS**
    - При подключении к базе данных SQL Azure или управляемому экземпляру базы данных SQL Azure можно подключиться к `<default>` как к исходной базе данных.
    - Добавлен новый элемент **Попробуйте службы SSIS в фабрике данных Azure** в узле **Каталоги служб Integration Services**, который можно использовать для запуска **мастера создания среды выполнения интеграции** и быстрого создания **среды выполнения интеграции Azure-SSIS**.
    - Добавлена кнопка **Создать среду выполнения интеграции SSIS** в **мастере создания каталога**, которую можно использовать для запуска **мастера создания среды выполнения интеграции** и быстрого создания **среды выполнения интеграции Azure-SSIS**.
    - `ISDeploymentWizard` теперь поддерживает проверку подлинности SQL, встроенную проверку подлинности Azure Active Directory и проверку пароля Azure Active Directory в режиме командной строки.

### <a name="bug-fixes"></a>Исправления ошибок

- **SMO**
    - Исправлены проблемы со снижением производительности в операциях передачи из внешних таблиц.
    - Устранена проблема в потокобезопасности `ServerConnection`, которая вызывала в SMO утечку экземпляров `SqlConnection` при выборе базы данных SQL Azure.
    - Устранена проблема, которая вызывала `StringBuilder.FormatError` при попытке восстановить базу данных, в имени которой использовались фигурные скобки `{}`.

- **Общие ошибки SSMS**
    - Устранена проблема, при которой программа SSMS зависает или дает сбой во время редактирования T-SQL.
    - Устранена проблема, при которой `ApplicationIntent` не передается в подключениях в `Registered Servers`.
    - Устранена проблема, когда форма **пользовательского интерфейса мастера создания сеанса XEvent** неправильно отображалась на мониторах с высоким разрешением.
    - Устранена проблема при импорте файла `.bacpac`.
    - Устранена проблема, когда при попытке отобразить свойства базы данных с `FILEGROWTH > 2048GB` возникала ошибка арифметического переполнения.
    - Устранена проблема, из-за которой службы SSMS не могли открыть SQL-файл, если дважды щелкнуть его.

- **Скрипты объектов**
    - Устранена проблема, которая приводила к тому, что в ходе создания скриптов базы данных из базы данных SQL Azure целевым всегда выбирался локальный экземпляр SQL Server, даже если параметры создания скриптов **обозревателя объектов** сценариев параметры были заданы в согласовании с источником.
    - Устранена проблема, когда при попытке создать скрипт таблицы базы данных в хранилище данных SQL Azure с использованием кластеризованных и некластеризованных индексов создавались неверные инструкции T-SQL.
    - Устранена проблема, когда при попытке создать скрипт таблицы базы данных в хранилище данных SQL Azure с использованием обычных кластеризованных индексов и кластеризованных индексов columnstore создавались неверные (дублирующиеся) инструкции T-SQL.
    - Исправлено создание сценариев секционированных таблиц без значений диапазона (в базах данных хранилища данных SQL Azure).
    - Устранена проблема, которая препятствует созданию скрипта аудита или спецификации аудита `SERVER_PERMISSION_CHANGE_GROUP`.
    - Устранена проблема, которая препятствует созданию скрипта статистики на основе хранилища данных SQL Azure. См. [форумы обратной связи Microsoft Azure](https://feedback.azure.com/forums/908035-sql-server/suggestions/32897296).

- **Управляемый экземпляр базы данных SQL Azure**
    - Устранена проблема, которая вызывала сбой при щелчке базы данных правой кнопкой мыши и выборе команды **Импорт приложения уровня данных**.
    - Устранена проблема, которая вызывала появление ошибок при щелчке `TempDB` правой кнопкой мыши.
    - Устранена проблема при создании скрипта инструкции `ALTER DB ADD FILE` в объектах SMO, из-за которой созданный скрипт T-SQL мог быть пустым.

- **Мастер неструктурированных файлов**
    - Устранена проблема, когда мастеру не удавалось импортировать данные из CSV-файлов с одним столбцом.
    - Устранена проблема, при которой мастер импорта не мог обрабатывать отрицательные десятичные значения.
    
### <a name="known-issues"></a>Известные проблемы

- Недоступно

## <a name="ssms-180-preview---cumulative-changelog-through-preview-7"></a>Предварительная версия SSMS 18.0 — совокупный журнал изменений до предварительной версии 7

Если метка *предварительная версия 5*, *предварительная версия 6* или *предварительная версия 7* отсутствует, значит, данное изменение появилось в первой общедоступной предварительной версии SSMS, которой была *предварительная версия 4* SSMS 18.0.
...
- **SSMS**
  ...
  - Свойства базы данных SLO/Edition/MaxSize теперь принимают пользовательские имена, что облегчает поддержку будущих выпусков баз данных SQL Azure
  ...
  - [Новые возможности в предварительной версии 7] Добавлена поддержка параметров сортировки `UTF8_BIN2`.
...
- **SMO**
  ...
  - [Новые возможности в предварительной версии 7] Добавлен **облачный свидетель** — новый тип кворума и ресурса в SMO и SSMS.
  - [Новые возможности в предварительной версии 7] Добавлена поддержка [пограничных ограничений](../relational-databases/tables/graph-edge-constraints.md) в SMO и SSMS.
...
- **Интеграция с Azure Data Studio**
  ...
  - [Новые возможности в предварительной версии 7] В Azure Data Studio можно выполнить запрос или создать новую записную книжку при щелчке базы данных в обозревателе объектов правой кнопкой мыши.
...
- **Инструкция ShowPlan**
  ...
  - [Новые возможности в предварительной версии 7] Добавлена поддержка инструкции Showplan для LocalCube RelOp для `DW ROLLUP` и `CUBE`.
...
- **Обновление уровня совместимости базы данных**
  - [Новые возможности в предварительной версии 5] Добавлен новый параметр в разделе "<Database name>" -> "Задачи" -> "Обновление базы данных". Он запускает новый помощник по настройке запросов (QTA), который помогает пользователям выполнять следующие действия:
    ...
    Это похоже на процесс обновления базы данных, описанный в разделе [Поддержание стабильной производительности во время обновления до новой версии](../relational-databases/performance/query-store-usage-scenarios.md#CEUpgrade), кроме последнего шага — помощник по обновлению не опирается на последнее известное рабочее состояние при выдаче рекомендаций.
...
- **Маскирование данных**
  - [Новые возможности в предварительной версии 5] Добавлено статическое маскирование данных. Статические маскирование — это средство защиты данных, которое позволяет пользователям создать копию базы SQL и скрыть в ней конфиденциальные данные. Компонент будет полезен тем, кто предоставляет рабочую базу данных сторонним пользователям, например команде по тестированию и разработке или аналитическому отделу. Дополнительные сведения: [Статическое маскирование данных для базы данных SQL Azure и SQL Server](https://azure.microsoft.com/blog/static-data-masking-preview/) (на английском языке).
  - [Новые возможности в предварительной версии 7] Теперь поддерживаются файлы конфигурации JSON.
  - [Новые возможности в предварительной версии 7] Формат XML-файла конфигурации был изменен на более гибкий.  Существующие файлы конфигурации необходимо создать заново.
  - [Новые возможности в предварительной версии 7] Базы данных, содержащие оптимизированные для памяти и темпоральные таблицы, теперь могут быть скрыты.  К оптимизированным для памяти и темпоральным таблицам по-прежнему применяются некоторые ограничения.
  - [Новые возможности в предварительной версии 7] Операции копирования локальной базы данных теперь использует параметр `COPY_ONLY` для шага `BACKUP DATABASE`.
  - [Новые возможности в предварительной версии 7] Скрытые базы данных теперь используют модель восстановления `SIMPLE`, пока идет операция маскирования, для сокращения использования журнала, и возвращаются к исходному режиму после завершения.
  - [Новые возможности в предварительной версии 7] Скрытая база данных теперь называется `<output database name>-MaskInProgress` во время выполнения операции маскирования и переименовывается в `<output database name>` после того, как операция завершилась успешно.
  - [Новые возможности в предварительной версии 7] Порядок отображения столбцов в пользовательском интерфейсе конфигурации изменен: теперь они стоят по порядковым номерам, а не по алфавиту.
  - [Новые возможности в предварительной версии 7] Удален параметр массового выбора столбцов в пользовательском интерфейсе конфигурации во избежание непредвиденного поведения.
  - [Новые возможности в предварительной версии 7] Улучшена внутренняя логика повторных попыток для ошибок подключения и команд.
...
- **Службы SSIS**
  ...
  - [Новые возможности в предварительной версии 7] При подключении к базе данных SQL Azure или управляемому экземпляру можно подключиться к "\<default\>" как к исходной базе данных.
  - [Новые возможности в предварительной версии 7] Добавлен новый элемент "Попробуйте службы SSIS в фабрике данных Azure" в узле "Каталоги служб Integration Services", который можно использовать для запуска "мастера создания среды выполнения интеграции" и быстрого создания "среды выполнения интеграции Azure-SSIS".
  - [Новые возможности в предварительной версии 7] Добавлена кнопка "Создать среду выполнения интеграции SSIS" в "мастере создания каталога", которую можно использовать для запуска "мастера создания среды выполнения интеграции" и быстрого создания "среды выполнения интеграции Azure-SSIS".
  - [Новые возможности в предварительной версии 7] ISDeploymentWizard поддерживает проверку подлинности SQL, встроенную проверку подлинности Azure Active Directory и проверку пароля Azure Active Directory в режиме командной строки.
...
- **Классификация данных**
  ...
  - [Новые возможности в предварительной версии 7] Добавлен новый компонент "Классификация данных" в SMO. Объект столбца теперь предоставляет новые свойства: SensitivityLabelName, SensitivityLabelId, SensitivityInformationTypeName, SensitivityInformationTypeId и IsClassified (только для чтения). Дополнительные сведения см. в разделе: https://docs.microsoft.com/sql/t-sql/statements/add-sensitivity-classification-transact-sql?view=azuresqldb-current 
  - [Новые возможности в предварительной версии 7] Добавлен пункт "Классификационный отчет" в меню "Классификация данных".

- **Оценка уязвимостей**
  - [Новые возможности в предварительной версии 5] Включено меню задач оценки уязвимостей для хранилища данных SQL Azure.
  - [Новые возможности в предварительной версии 7] Изменен набор правил оценки уязвимостей, которые выполняются на серверах управляемых экземплярах SQL Azure, чтобы результатом оценки уязвимостей была согласованность с базой данных SQL Azure. 
  - [Новые возможности в предварительной версии 7] "Оценка уязвимостей SQL" теперь поддерживает хранилище данных SQL Azure.
...
- **Мастер импорта неструктурированных файлов**
  - [Новые возможности в предварительной версии 7] Добавлена логика для уведомления пользователя о том, что импорт неструктурированных файлов мог вызвать переименование столбцов.

- **Мастер приложений уровня данных**
  - [Новые возможности в предварительной версии 7] Добавлена поддержка импорта и экспорта приложения уровня данных с графовой таблицей.

- **Управляемый экземпляр SQL Azure**
  - [Новые возможности в предварительной версии 7] Добавлены **имена входа AAD** как новый тип входа в SMO и SSMS при подключении к управляемым экземплярам SQL Azure.

- **Средство просмотра XEvent**
  - [Новые возможности в предварительной версии 7] В средство просмотра XEvent в окне показа планов можно вывести дополнительные XEvents.
...
- **Аварийное завершение/зависания**
  ...
  - [Новые возможности в предварительной версии 7] Устранена проблема, из-за которой решение SSMS зависает или дает сбой во время редактирования T-SQL. 
...
- **Общие ошибки SSMS**
  - [Новые возможности в предварительной версии 7] Устранена проблема, из-за которой решение SSMS зависает или дает сбой во время редактирования T-SQL.
  - [Новые возможности в предварительной версии 7] Устранена проблема, при которой `ApplicationIntent` не передается в подключениях в `Registered Servers`.
  - [Новые возможности в предварительной версии 7] Устранена проблема, когда форма **пользовательского интерфейса мастера создания сеанса XEvent** неправильно отображалась на мониторах с высоким разрешением.
  - [Новые возможности в предварительной версии 7] Устранена проблема при импорте файла `.bacpac`.
  - [Новые возможности в предварительной версии 7] Устранена проблема, когда при попытке отобразить свойства базы данных с `FILEGROWTH > 2048GB` возникала ошибка арифметического переполнения.
  - [Новые возможности в предварительной версии 7] Устранена проблема, из-за которой службы SSMS не могли открыть SQL-файл, если дважды щелкнуть его.
...
- **Скрипты объектов**
  ...
  - [Новые возможности в предварительной версии 7] Устранена проблема, которая приводила к тому, что в ходе создания скриптов базы данных из базы данных SQL Azure целевым всегда выбирался локальный экземпляр SQL Server, даже если параметры создания скриптов **обозревателя объектов** сценариев параметры были заданы в согласовании с источником.
- [Новые возможности в предварительной версии 7] Устранена проблема, когда при попытке создать скрипт таблицы базы данных в хранилище данных SQL Azure с использованием кластеризованных и некластеризованных индексов создавались неверные инструкции T-SQL.
- [Новые возможности в предварительной версии 7] Устранена проблема, когда при попытке создать скрипт таблицы базы данных в хранилище данных SQL Azure с использованием обычных кластеризованных индексов и кластеризованных индексов columnstore создавались неверные (дублирующиеся) инструкции T-SQL.
- [Новые возможности в предварительной версии 7] Исправлено создание сценариев секционированных таблиц без значений диапазона (в базах данных хранилища данных SQL Azure).
- [Новые возможности в предварительной версии 7] Устранена проблема, которая препятствует созданию скрипта аудита или спецификации аудита `SERVER_PERMISSION_CHANGE_GROUP`.
- [Новые возможности в предварительной версии 7] Устранена проблема, которая препятствует созданию скрипта статистики на основе хранилища данных SQL Azure. См. [форумы обратной связи Microsoft Azure](https://feedback.azure.com/forums/908035-sql-server/suggestions/32897296).
...
- **SMO**
  ...
  - [Новые возможности в предварительной версии 7] Исправлены проблемы со снижением производительности в операциях передачи из внешних таблиц.
  - [Новые возможности в предварительной версии 7] Устранена проблема в потокобезопасности `ServerConnection`, которая вызывала в SMO утечку экземпляров `SqlConnection` при выборе базы данных SQL Azure.
  - [Новые возможности в предварительной версии 7] Устранена проблема, которая вызывала `StringBuilder.FormatError` при попытке восстановить базу данных, в имени которой использовались фигурные скобки `{}`.
...
- **Мастер импорта неструктурированных файлов**
  ...
  - [Новые возможности в предварительной версии 7] Устранена проблема, при которой мастер импорта не мог обрабатывать отрицательные десятичные значения.
  - [Новые возможности в предварительной версии 7] Устранена проблема, когда мастеру не удавалось импортировать данные из CSV-файлов с одним столбцом.
...
- **Поддержка управляемого экземпляра**
  ...
  - [Новые возможности в предварительной версии 7] Устранена проблема, которая вызывала сбой при щелчке базы данных правой кнопкой мыши и выборе команды **Импорт приложения уровня данных**.
  - [Новые возможности в предварительной версии 7] Устранена проблема, которая вызывала появление ошибок при щелчке `TempDB` правой кнопкой мыши.
  - [Новые возможности в предварительной версии 7] Устранена проблема при создании скрипта инструкции `ALTER DB ADD FILE` в объектах SMO, из-за которой созданный скрипт T-SQL мог быть пустым.
...
- **Маскирование данных**
  - [Новые возможности в предварительной версии 7] Устранена ошибка, из-за которой локальные базы данных с несколькими файлами оптимизированных для памяти таблиц журнала и файловых потоков не могли автоматически копироваться.
...
- Сценарии объектов Исправлена проблема, когда пункт "Создать сценарий базы данных" не работал и вызывал ошибку, если пользователь приостанавливал базу данных DW на сервере, но выбирал другую базу данных, отличную от DW, и пытался создавать в ней сценарии.
Исправлена проблема, когда заголовок для включенных в сценарий хранимых процедур не соответствовал параметрам сценария, из-за чего сценарий мог быть непонятным. См. [Connect 3139784](https://connect.microsoft.com/SQLServer/feedback/details/3139784).
...
- Always On:
   - На панель мониторинга Always On и страницу свойств групп доступности добавлен столбец режима заполнения.
...
- Зарегистрированные серверы
   - Реализована поддержка многосерверных запросов для серверов универсальной проверки подлинности (для каждого такого сервера в группе выполняется попытка использовать один и тот же токен).
...
- Мастер сжатия
   - Устранена проблема с мастером управления сжатием, связанная с индексами; исправлена проблема, из-за которой мастеры сжатия данных не работали в SQL 2016 и более ранних версиях.
...
- В списке отчетов хранилища запросов из обозревателя объектов отсутствуют *Регрессионные запросы*.
   - Решение. Щелкните правой кнопкой мыши узел **Хранилище запросов** и выберите **Просмотр регрессионных запросов**.
...
- Изменились параметры скриптов для объектов в обозревателе объектов SQL Server Management Studio.
  ...
  - В SSMS 17.2 добавлен новый параметр: *Использовать параметры сценариев согласно источнику*. Если он имеет значение *Истина*, создаваемый скрипт будет предназначен для той же версии, типа ядра и выпуска ядра, которые используются на сервере, где находится объект, для которого создается скрипт.
...
- Отчет панели мониторинга производительности по серверу теперь доступен как отчет по умолчанию.
  ...
  - Во вложенном отчете по статистике прошлых ожиданий операции ожидания теперь объединяются по категориям. Ожидания в режиме простоя или спящем режиме по умолчанию отфильтровываются.
  - Новый вложенный отчет по прошлым кратковременным блокировкам.
...
- При использовании в Azure AD универсальной проверки подлинности с MFA перечисленные ниже функциональные возможности SSMS не поддерживаются.
  ...
  - **Помощник по настройке ядра СУБД** не поддерживает проверку подлинности Azure AD. Имеется известная проблема, когда пользователь получает непонятное сообщение об ошибке: *Не удалось загрузить файл или сборку Microsoft.IdentityModel.Clients.ActiveDirectory…* вместо правильного сообщения *Помощник по настройке ядра СУБД не поддерживает базу данных SQL Microsoft Azure. (DTAClient)*.
...
- Исправлена проблема, из-за которой при запуске SSMS иногда возникала ошибка "Ссылка на объект не указывает на экземпляр объекта". https://connect.microsoft.com/SQLServer/feedback/details/3134698
...
- Исправлена проблема с производительностью при щелчке правой кнопкой мыши индекса таблицы (при медленном подключении к Интернету). https://connect.microsoft.com/SQLServer/feedback/details/3120783
- Исправлена проблема, из-за которой среде SSMS не удавалось перечислить файлы резервной копии на серверах с параметрами сортировки с учетом регистра. https://connect.microsoft.com/SQLServer/feedback/details/3134787 и https://connect.microsoft.com/SQLServer/feedback/details/3137000
...
- Исправлена проблема, из-за которой в обозревателе объектов не сортировались компилируемые в собственном коде хранимые процедуры, добавленные неправильно. https://connect.microsoft.com/SQLServer/feedback/details/3133365
...
- Исправлена несогласованность имени приложения для подключений SSMS. https://connect.microsoft.com/SQLServer/feedback/details/3135115
...
- Исправлена проблема, из-за которой в SSMS периодически выводилось сообщение об ошибке "Неопределенные данные. Этот метод или свойство не может вызываться для значений, равных Null". при развертывании узла таблицы https://connect.microsoft.com/SQLServer/feedback/details/3136283
...
Общедоступная версия | Номер сборки: 14.0.17119.0
...
- Profiler В меню "Справка" > "О программе" теперь отображается номер версии выпуска (например, 17.1)
...
## <a name="downloadssdtmediadownloadpng-ssms-170httpsgomicrosoftcomfwlinklinkid847722"></a>![скачать](../ssdt/media/download.png) [SSMS 17.0](https://go.microsoft.com/fwlink/?LinkID=847722)
Общедоступная версия | Номер сборки: 14.0.17099.0
...
- Исправлена ошибка, из-за которой попытка создать скрипт удаления базы данных Azure в SSMS приводила к удалению этой базы данных. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3131458/)
...
- Происходит сбой создания SecurityPolicy, если в имени целевого объекта присутствует символ `]`.
...
- Уменьшено количество запросов на изменение механизма в окне запросов. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3113387)
...
- Исправлен сбой в SSMS при запуске пользовательского отчета. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3118856)
- Устранена проблема, из-за которой операция "Сформировать скрипт…" завершалась ошибкой для баз данных SQL Azure.
- Исправлены ошибки в разделах "Создать скрипт как" и "Мастер создания скриптов", которые приводили к добавлению лишних новых строк при создании таких объектов, как хранимые процедуры. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3115850)
- Поставщик SQLAS PowerShell: добавлено свойство LastProcessed в папки Dimension и MeasureGroup. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3111879)
- Статистика активных запросов: исправлена ошибка, при которой отображается только первый запрос в пакете. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3114221)  
...
- Проблемы производительности обозревателя объектов: [элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3114074). Контекстное меню таблиц ненадолго зависает. SSMS замедляет работу при щелчке индекса таблицы правой кнопкой мыши (при работе через удаленное подключение к Интернету). Избегайте выполнения табличных запросов с сортировкой на сервере.
- Мастер развертывания Azure (развертывание базы данных на виртуальной машине Azure) удален из SSMS.
- Исправлена ошибка, при которой отсутствующие индексы не отображались в планах выполнения в SSMS [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3114194).
- Исправлена распространенная ошибка, вызывающая аварийное завершение работы при закрытии SSMS.
- Исправлена ошибка в обозревателе объектов при открытии контекстного меню для узлов PolyBase | Масштабируемая группа [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3115128).
...
- Настройка Always Encrypted для существующей таблицы завершается с ошибками, в которых указаны посторонние объекты. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3103181)
- Настройка Always Encrypted для существующей базы данных с несколькими схемами не работает. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3109591)
- Мастер зашифрованного столбца в Always Encrypted завершается с ошибкой из-за того, что база данных содержит представления, ссылающиеся на системные представления. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3111925)
...
- Добавлен пункт меню "Выполнение в Scale Out..." в узел папки
