...
Номер сборки: 15.0.18098.0<br>
Дата выпуска: 28 марта 2019 г.

Релиз-кандидат 1 (RC1) — это новейшая общедоступная предварительная версия SSMS 18.0. Чтобы получить последнюю общедоступную версию SSMS, [скачайте и установите SSMS 17.9.1](#ssms-1791-latest-ga-release).

### <a name="whats-new-in-rc1"></a>Новые возможности RC1

В этом разделе перечислены новые возможности версии SSMS 18.0 RC1. Полный журнал изменений начиная с версии SSMS 17.9.1 см. в разделе [Предварительная версия SSMS 18.0 — совокупный журнал изменений до версии RC1](#ssms-180-preview---cumulative-changelog-through-rc1).

**Улучшения в SSMS**

- Возможность подключения конечной точки XMLA к наборам данных Power BI. Конечные точки XMLA предоставляют доступ к подсистеме Analysis Services в службе Power BI. Благодаря этому такие средства, как SSMS и SQL Profiler, могут подключаться к наборам данных Power BI для мониторинга, управления, отладки и других задач. Подробные сведения см. в разделе [Подключение конечной точки XMLA](https://go.microsoft.com/fwlink/?linkid=2085204).

- **SMO**
  - Добавлена поддержка каскадного удаления для пограничных ограничений в SMO и SSMS.
  - Добавлена поддержка разрешений на чтение и запись для классификации данных.

- **Файлы аудита**
  - Обновлен список известных действий аудита: добавлены действия FEATURE RESTRICTION ADD/CHANGE GROUP/DROP.

### <a name="bug-fixes"></a>Исправления ошибок

- **Общие ошибки SSMS**
  - Устранена проблема, из-за которой многофакторная проверка подлинности была невозможна, когда идентификаторы пользователей относились к нескольким клиентам.
  - Устранена проблема, из-за которой в отчете панели мониторинга производительности указывались ожидания PAGELATCH и PAGEIOLATCH, которые отсутствовали во вложенных отчетах.

- **Редактор SSMS**
  - Устранена проблема, из-за которой некоторые системные представления и функции с табличными значениями неправильно раскрашивались.

- **Обозреватель объектов**
  - Добавлена поддержка надлежащего экранирования при фильтрации в обозревателе объектов. Подробные сведения см. в [отзыве об Azure с номером 36678803](https://feedback.azure.com/forums/908035/suggestions/36678803).

- **SMO**
  - Устранена проблема, из-за которой метод GetDbComparer неправильно задавал параметры сортировки по умолчанию CI для всех баз данных Azure.

- **Поддержка управляемого экземпляра**
  - Улучшено отображение свойств, относящихся к серверу управляемых экземпляров (поколение оборудования, уровень служб, используемое и зарезервированное хранилище).

- **База данных SQL Azure**
  - Устранена проблема с указанием целей уровня обслуживания (SLO) в коде, которая усложняла поддержку новых целей уровня обслуживания SQL Azure в SSMS. Теперь пользователь может войти в Azure и разрешить SSMS извлечь все соответствующие данные по целям уровня обслуживания (выпуск и максимальный размер).

- **Сетка результатов**
  - Устранена проблема, которая приводила к возникновению исключения "Индекс вне диапазона" при щелчке по сетке.
  - Устранена проблема, из-за которой игнорировался цвет фона для результата в сетке. Подробные сведения см. в [отзыве об Azure с номером 32895916](https://feedback.azure.com/forums/908035/suggestions/32895916).

- **Profiler**
  - Устранена проблема, из-за которой SQL Profiler не запускался в Windows 7 с пакетом обновления 1 (SP1).

- **Инструкция ShowPlan**
  - Добавлены следующие 4 атрибута в RunTimeCountersPerThread действительного плана выполнения XML: HpcRowCount (количество строк, обработанных устройством hpc), HpcKernelElapsedUs (истекшее время ожидания для выполняемого ядра), HpcHostToDeviceBytes (количество байтов, переданных из узла на устройство) и HpcDeviceToHostBytes (количество байтов, переданных с устройства в узел).

- **Мастер приложений уровня данных**
  - Устранена проблема, из-за которой пользователю не удавалось импортировать приложение уровня данных (DACPAC) вследствие ограниченного доступа к серверу (то есть отсутствия доступа ко всем базам данных на сервере).
  - Устранена проблема, из-за которой импорт выполнялся крайне медленно, когда на одном сервере SQL Azure размещалось много баз данных.

### <a name="deprecated-features"></a>Устаревшие возможности

- Удалена функция статического маскирования данных (предварительная версия).

### <a name="known-issues"></a>Известные проблемы

- Недоступно

## <a name="ssms-180-preview---cumulative-changelog-through-rc1"></a>Предварительная версия SSMS 18.0 — совокупный журнал изменений до версии RC1

Если метка *предварительная версия 5*, *предварительная версия 6*, *предварительная версия 7* или *RC1* отсутствует, значит, данное изменение появилось в первой общедоступной предварительной версии SSMS 18.0, которой была *предварительная версия 4* SSMS 18.0.
...
- **SMO**
  ...
  - [Новая возможность в RC1] Добавлена поддержка каскадного удаления для пограничных ограничений в SMO и SSMS.
  - [Новая возможность в RC1] Добавлена поддержка разрешений на чтение и запись для классификации данных.
...
- **Файлы аудита**
  ...
  - [Новая возможность в RC1] Обновлен список известных действий аудита: добавлены действия FEATURE RESTRICTION ADD/CHANGE GROUP/DROP.
...
- **Общие ошибки SSMS**
  ...
  - [Новая возможность в RC1] Устранена проблема, из-за которой многофакторная проверка подлинности была невозможна, когда идентификаторы пользователей относились к нескольким клиентам.
  - [Новая возможность в RC1] Устранена проблема, из-за которой в отчете панели мониторинга производительности указывались ожидания PAGELATCH и PAGEIOLATCH, которые отсутствовали во вложенных отчетах.
...
- **Редактор SSMS**
  ...
  - [Новая возможность в RC1] Устранена проблема, из-за которой некоторые системные представления и функции с табличными значениями неправильно раскрашивались.
...
- **Обозреватель объектов**
  ...
  - [Новая возможность в RC1] Добавлена поддержка надлежащего экранирования при фильтрации в обозревателе объектов. Подробные сведения см. в [отзыве об Azure с номером 36678803](https://feedback.azure.com/forums/908035/suggestions/36678803).
...
- **SMO**
  ...
  - [Новая возможность в RC1] Устранена проблема, из-за которой для всех баз данных Azure в SMO по умолчанию использовались параметры сортировки без учета регистра для всех сравнений строк вместо параметров сортировки, заданных для базы данных.
...
- **Поддержка управляемого экземпляра**
  ...
  - [Новая возможность в RC1] Улучшено отображение свойств, относящихся к серверу управляемых экземпляров (поколение оборудования, уровень служб, используемое и зарезервированное хранилище).
...
- **Сетка результатов**
  ...
  - [Новая возможность в RC1] Устранена проблема, которая приводила к возникновению исключения "Индекс вне диапазона" при щелчке по сетке.
  - [Новая возможность в RC1] Устранена проблема, из-за которой игнорировался цвет фона для результата в сетке. Подробные сведения см. в [отзыве об Azure с номером 32895916](https://feedback.azure.com/forums/908035/suggestions/32895916).
...
- **Profiler**
  ...
  - [Новая возможность в RC1] Устранена проблема, из-за которой SQL Profiler не запускался в Windows 7 с пакетом обновления 1 (SP1).
...
- **Инструкция ShowPlan**
  ...
  - [Новая возможность в RC1] Добавлены следующие 4 атрибута в RunTimeCountersPerThread действительного плана выполнения XML: HpcRowCount (количество строк, обработанных устройством hpc), HpcKernelElapsedUs (истекшее время ожидания для выполняемого ядра), HpcHostToDeviceBytes (количество байтов, переданных из узла на устройство) и HpcDeviceToHostBytes (количество байтов, переданных с устройства в узел).
...
- **Мастер приложений уровня данных**
  - [Новая возможность в RC1] Устранена проблема, из-за которой пользователю не удавалось импортировать приложение уровня данных (DACPAC) вследствие ограниченного доступа к серверу (то есть отсутствия доступа ко всем базам данных на сервере).
  - [Новая возможность в RC1] Устранена проблема, из-за которой импорт выполнялся крайне медленно, когда на одном сервере SQL Azure размещалось много баз данных.
...
- [Новая возможность в RC1] Удалена функция статического маскирования данных (предварительная версия).
