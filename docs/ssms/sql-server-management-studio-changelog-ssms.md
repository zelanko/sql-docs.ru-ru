...
## <a name="ssms-171-release"></a>Выпуск SSMS 17,1
...
## <a name="ssms-170-release"></a>Выпуск SSMS 17.0
Общедоступной | Номер сборки: 14.0.17099.0

### <a name="enhancements"></a>Улучшения 

- Пакет обновления и служб обновления программного обеспечения Windows (WSUS) 
    - Будущие выпуски 17.X включают небольшой пакет накопительного пакета обновления 
  - Пакет обновления также будут опубликованы в каталоге WSUS  
- Обновления значков:
    - Значки были обновлены, чтобы соответствовать значки оболочки VS предоставленный и поддерживать разрешения высокого Разрешения
    - новые значки для профилировщика и SSMS, чтобы различать версии 16.х и 17.х.
- Модуль SQL PowerShell:
  - Модуль SQL Server PowerShell удалена из SSMS и теперь поставляется через коллекцию PowerShell (PowerShell 5.0 теперь требуется для поддержки модуля управления версиями)
  - Прочие улучшения «презентация» (форматирование) некоторых объектов SMO (например базы данных теперь Показать размера и доступное пространство и таблиц Показать строки count и места использования)
  - Добавлены Раскраска при вызове из меню «Запуск PowerShell» в OE командной строке PowerShell
  ...
  - SqlAvailabilityReplicaRoleToSecondary набор командлетов, Grant SqlAvailabilityGroupCreateAnyDatabase и добавлены Revoke-SqlAvailabilityGroupCreateAnyDatabase
  - Добавленный параметр - SeedingMode командлетов Set-SqlAvailabilityReplica и New-SqlAvailabilityReplica
  - Добавленный параметр - ConnectionString для Get-SqlDatabase
- SQL Server в Linux:
    - общие улучшения и исправления для доставки журналов.
  - Добавлена поддержка собственного Linux пути присоединения, восстановления и резервного копирования базы данных
  - Добавлена поддержка собственного Linux пути для папки назначения журнала аудита
- Службы Analysis Services
  - Окно запроса DAX:
    - Круглые скобки, сопоставления, в редакторе
    - Поддержка синтаксиса определения МЕР и ОПРЕДЕЛИТЬ VAR
    - Различные усовершенствования Intellisense
  - Универсальной проверкой подлинности
    - Позволяет пользователям указать имя пользователя и пароль и диалоговое окно входа Azure обрабатывающий соединение
  - Интеграция PQ SSMS: 
    - Выполнять сценарии works источников структурированных данных 
    - Для просмотра и редактирования источников структурированных данных в пользовательском Интерфейсе PQ
...
- Showplan
  ...
  - Новый параметр для «Анализ действительный план выполнения»
  - Основные улучшения для сравнения инструкции showplan
  - Появились функциональные возможности в функции сравнения Showplan для поиска значительные различия в кратности подходящие узлы двух планов запросов и выполнения основных анализа основных причин
...
- SSMS теперь можно создавать базы данных SQL Azure выпуск «PremiumRS»
- Группы доступности AlwaysOn
  - Добавление поддержки для новых типов кластера: ВНЕШНИХ или NONE
    - Добавление поддержки для SQL Server в Linux
    - Добавление автоматического заполнения в качестве альтернативы для начальной синхронизации данных
    - Исправлена некоторые дефекты, например конечной точки URL-адрес обработке обновления базы данных и макета пользовательского интерфейса
    - Удалить реплику Azure связанные функции
  - Усовершенствованная функция IntelliSense для несколько ключевых слов для группы доступности
- Монитор активности
  - Добавлена новая панель «Монитора активности» в окне вывода SSMS
  - Изменить сообщение ошибки или время ожидания подключения в журнал сведения для вывода окна, а не всплывающее сообщение
  - Удалить пустое диаграммы (5-й диаграммы) в разделе общих сведений
  - Добавляемый «(приостановлена)» title Обзор Если приостановить сбор данных монитора активности
  - Расширения Graph в Graph SQL Server - новых значков для узла и границей таблицы graph - узла и границей таблицы будет отображаться в папке Graph таблиц — шаблоны для создания диаграмм узла и границей таблиц, доступных
- Режим представления:
...
- Устранена проблема, где аварийное завершение SSMS при showplan прокручивать через панель surfacebook
- Устранена проблема, где SSMS зависает на длительное время при получении свойств базы данных, в которых выполняется восстановленной или в автономном режиме 
- Устранена проблема, где «Средство просмотра справки» не удалось открыть в версии-КАНДИДАТА построения
- Устранена проблема, где «обслуживания планы задачи «элементов могут отсутствовать в среде SSMS.
- Устранена проблема в SSMS, где пользователь не удалось сжать базу данных, если имя базы данных содержится фигурные скобки. [Элемент Connect](https://connect.microsoft.com/SQLServer/feedback/details/3122618)
- Устранена проблема, из которых пытается выполнить сценарий удаления SSMS из Azure базы данных узнать, вызывает удаление самой базы данных. [Элемент Connect](http://connect.microsoft.com/SQLServer/feedback/details/3131458/)
...
- Постоянное шифрование
  ...
  - Устранена проблема, когда файлы ресурсов может не загружаться корректно, образуя таким образом неточной ошибок
...
- Устранена проблема, где среда SSMS не удается изменить уровень совместимости базы данных Azure значение v140
...
- Устранена проблема, за которой пользователь не сможет изменить уровень совместимости базы данных на 2017 г. SQL Server.
...
- Статистика активных запросов: исправлена ошибка, при которой отображается только первый запрос в пакете. [Элемент подключиться] (http://connect.microsoft.com/SQLServer/feedback/details/3114221)  
...
- Настройка Always Encrypted для существующей базы данных с несколькими схемами не работает. [Элемент подключиться] (http://connect.microsoft.com/SQLServer/feedback/details/3109591)
- Мастер зашифрованного столбца в Always Encrypted завершается с ошибкой из-за того, что база данных содержит представления, ссылающиеся на системные представления. [Элемент подключиться] (http://connect.microsoft.com/SQLServer/feedback/details/3111925)
...
- Исправлена проблема с функцию базы данных SQL Azure брандмауэра белого списка IP-адресов
- Основные проблемы в среде SSMS, которая вызывала ссылку на объект не задан исключение при редактировании источника как многомерные секции
- Основные проблемы в среде SSMS, которая вызывала ссылку на объект не задан исключение при удалении сборки клиента из многомерных AS server
- Устранена проблема, где табличных 1400 базы данных не удалось переименовать
- Исправлена проблема со сценариями уровнем совместимости 1400 как табличный источник данных из диалогового окна свойств соединения
- Удалить допущение, таблицы в качестве 1400 уровня совместимости модели имеют по крайней мере одну секцию
- Сочетание клавиш CTRL-R теперь переключает область результатов в редакторе запросов среды SSMS DAX
