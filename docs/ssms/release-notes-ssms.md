---
title: Заметки о выпуске SSMS | Документация Майкрософт
ms.custom: ''
ms.date: 03/30/2019
ms.prod: sql
...
В этой статье приводятся подробные сведения об обновлениях, улучшениях и исправлениях ошибок в текущей и предыдущих версиях SQL Server Management Studio. Чтобы скачать предыдущие версии SSMS, [щелкните здесь](#previous-ssms-releases).
...
## <a name="180-rc1-march-2019"></a>18.0 (RC1), март 2019 г.

Загрузить: &nbsp; &nbsp; [Скачать SSMS 18.0 (RC1)](download-sql-server-management-studio-ssms.md)<br/>
Номер сборки: &nbsp; &nbsp; 15.0.18098.0<br/>
Дата выпуска: &nbsp; &nbsp; 28 марта 2019 г.

В этом разделе перечислены новые возможности версии SSMS 18.0 RC1.

Релиз-кандидат 1 (RC1) — это общедоступная предварительная версия SSMS 18.0. Версия 17.\* будет считаться последней общедоступной версией, пока не будет выпущена общедоступная версия 18.

> [!NOTE]
> Полный журнал изменений начиная с версии SSMS 17.9.1 см. в разделе [Предварительная версия SSMS 18.0 — совокупный журнал изменений до версии RC1](#180-preview---cumulative-changelog-through-rc1).

### <a name="whats-new-in-rc1"></a>Новые возможности RC1

| Изменения | Сведения |
| :------- | :------ |
| **SSMS:** &nbsp; &nbsp; возможность подключения конечной точки XMLA к наборам данных Power BI. | Конечные точки XML для аналитики (XMLA) предоставляют доступ к подсистеме Analysis Services в службе Power BI. Благодаря этому такие средства, как SSMS и SQL Profiler, могут подключаться к наборам данных Power BI для мониторинга, управления, отладки и других задач.<br/><br/> Дополнительные сведения см. в записи блога о [подключениях конечных точек XMLA](https://go.microsoft.com/fwlink/?linkid=2085204) (28 марта 2019 г.). |
| **SMO:** &nbsp; &nbsp; добавлена поддержка каскадного удаления для _пограничных ограничений_. | Добавление выполнено в SSMS и в управляющих объектах SQL Server (SMO). |
| **SMO:** &nbsp; &nbsp; добавлена поддержка разрешений на _чтение и запись_ для классификации данных. | &nbsp; |
| **Файлы аудита:** &nbsp; &nbsp; обновлен список известных действий аудита. | Список теперь содержит действия FEATURE RESTRICTION ADD, CHANGE GROUP, DROP. |
| &nbsp; | &nbsp; |

### <a name="bug-fixes"></a>Исправления ошибок

| Исправление ошибок | Сведения |
| :------ | :------ |
| **Общие ошибки SSMS:** &nbsp; &nbsp; устранена проблема, которая приводила к блокировке многофакторной проверки подлинности, если идентификаторы пользователя принадлежали нескольким клиентам. | &nbsp; |
...
| **Обозреватель объектов:** Добавлена поддержка надлежащего экранирования при фильтрации в обозревателе объектов. | См. [отзыв об Azure с номером 36678803](https://feedback.azure.com/forums/908035/suggestions/36678803). |
| **SMO:** &nbsp; &nbsp; устранена проблема, из-за которой `GetDbComparer` некорректно задавал CI для параметров сортировки по умолчанию для всех баз данных Azure. | &nbsp; |
| **Поддержка управляемого экземпляра:** &nbsp; &nbsp; улучшено отображение отдельных свойств сервера Управляемых экземпляров. | Такие свойства включают поколение оборудования, уровень служб, а также используемый и зарезервированный объем хранилища. |
| **База данных SQL Azure:** &nbsp; &nbsp; устранена проблема с указанием целей уровня обслуживания (SLO) в коде, которая усложняла поддержку новых целей уровня обслуживания SQL Azure в SSMS. | Теперь пользователи могут войти в Azure и разрешить SSMS извлечь все соответствующие данные о целях уровня обслуживания.<br/>(Применимые элементы данных о целях уровня обслуживания: выпуск и максимальный размер). |
| **Сетка результатов:** &nbsp; &nbsp; устранена проблема, которая приводила к возникновению исключения _Индекс вне диапазона_ при щелчке по сетке. | &nbsp; |
| **Сетка результатов:** &nbsp; &nbsp; устранена проблема, из-за которой игнорировался цвет фона для результата в сетке. | См. [отзыв об Azure с номером 32895916](https://feedback.azure.com/forums/908035/suggestions/32895916). |
| **Профилировщик:** &nbsp; &nbsp; устранена проблема, из-за которой SQL Profiler не запускался в Windows 7 с пакетом обновления 1 (SP1). | &nbsp; |
| **ShowPlan:** &nbsp; &nbsp; добавлено четыре атрибута в `RunTimeCountersPerThread` о фактическом плане выполнения XML. | &bull; &nbsp; &nbsp; `HpcRowCount` — число строк, обрабатываемых HPC-устройствами.<br/><br/>&bull; &nbsp; &nbsp; `HpcKernelElapsedUs` — затраченное на ожидание время для выполнения используемого ядра.<br/><br/>&bull; &nbsp; &nbsp; `HpcHostToDeviceBytes` — число байтов, переданных от узла к устройству.<br/><br/>&bull; &nbsp; &nbsp; `HpcDeviceToHostBytes` — число байтов, переданных от устройства к узлу. |
| **Мастер приложений уровня данных:** &nbsp; &nbsp; устранена проблема, из-за которой пользователю не удавалось импортировать приложение уровня данных (DACPAC) из-за ограниченного доступа к серверу. | Например, пользователям не удавалось импортировать файл DACPAC, если у них не было доступа ко всем базам данных на одном сервере. |
...
| Устаревшая функция | Сведения |
| :----------------- | :------ |
| Удалена функция "Статическое маскирование данных" (предварительная версия). | &nbsp; |
| &nbsp; | &nbsp; |
...
## <a name="180-preview---cumulative-changelog-through-rc1"></a>Предварительная версия SSMS 18.0 — совокупный журнал изменений до версии RC1
...
- **Повышение уровня совместимости базы данных**
...
- **Резервное копирование, восстановление, присоединение и отсоединение базы данных**
...
## <a name="1791-latest-ga-release"></a>17.9.1 (последний общедоступный выпуск)
...
*(Примечание. В настоящее время команда PresentOff недоступна.  Используйте RestoreDefaultFonts, чтобы отключить режим представления.)*
