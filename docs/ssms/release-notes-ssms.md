...
Скачать: [скачать SSMS 18.2](download-sql-server-management-studio-ssms.md)  
Номер сборки: 15.0.18142.0  
Дата выпуска: 25 июля 2019 г.

SSMS 18.2 является последним общедоступным выпуском SSMS. Если вам нужна предыдущая версия SSMS, см. список в разделе [Предыдущие выпуски SSMS](release-notes-ssms.md#previous-ssms-releases).
...
| Общие ошибки SSMS | Исправлена проблема, из-за которой некоторые параметры цвета *(Текстовый редактор > вкладка редактора и строка состояния)* не сохранялись. См. раздел [UserVoice](https://feedback.azure.com/forums/908035/suggestions/37924165).
| Общие ошибки SSMS | В диалоговом окне соединения пункт *Active Directory — универсальная с поддержкой MFA* заменен на *Azure Active Directory — универсальная с MFA* (функциональность аналогична, но, надеемся, суть будет яснее). |
...
| обозревателе объектов | Исправлена проблема, из-за которой некоторые пункты меню (например, *мастер импорта и экспорта* SQL Server) отсутствуют или отключены при подключении к SQL Express. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/37500016). |
| обозревателе объектов | Исправлена проблема, из-за которой в среде SSMS происходил сбой при перетаскивании объекта из обозревателя объектов в редактор. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/37887988). |
...
| обозревателе объектов | Исправлена старая проблема, из-за которой *попытка развернуть узел таблицы* в обозревателе объектов для базы данных, для которой настроено использование параметров сортировки, не поддерживаемых Windows, вызывала ошибку (и пользователь не мог развернуть свои таблицы). Примером таких параметров сортировки может быть Korean_Wansung_Unicode_CI_AS. |
...
| Агент SQL Server | Исправлена проблема, из-за которой при добавлении, вставке, изменении или удалении шагов задания фокус переносился в первую строку, а не в активную. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/38070892). |
...
| Написание скриптов и SMO | Исправлена проблема, при которой *расширенные свойства* столбцов не вносились в скрипт при выборе базы данных SQL Azure. Дополнительные сведения см. на сайте [StackOverflow](https://stackoverflow.com/questions/56952337/how-can-i-script-the-descriptions-of-columns-in-ms-sql-server-management-studio). |
...
- При переключении между несколькими окнами запросов в SSMS 18.0 возникают проблемы перерисовки. См. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/37474042) обходной путь, который позволяет отключить аппаратное ускорение в меню "Сервис" | "Параметры".
...
SSMS 18.1 является последним общедоступным выпуском SSMS. Если вам нужна предыдущая версия SSMS, см. список в разделе [Предыдущие выпуски SSMS](release-notes-ssms.md#previous-ssms-releases).
...
| Специальные возможности | Улучшены специальные возможности на странице "Монитор Stretch": добавлено доступное имя для кнопки *Автообновление*, а также интеллектуальное доступное имя, которое поможет пользователям не только узнать, какая кнопка выбрана, но и определить последствия ее нажатия. |
...
| Общие ошибки SSMS | Фиксированный размер отображения денежного столбца на странице свойств индекса. |
...
| Общие ошибки SSMS | Устранена проблема, при которой SSMS не учитывает параметры справки (автономный режим и Интернет). |
...
| Импорт неструктурированного файла | Исправлена ошибка, из-за которой строки могут быть незаметно потеряны во время импорта.|
...
| Средство просмотра журнала | Устранена проблема, когда в SSMS всегда открывается журнал текущего сервера (или агента), даже в том случае, если дважды щелкнуть старый архив в обозревателе объектов. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/37633648). |
...
| Отчеты | Исправление отчетов фактически выводит GrantedQueryMemory в КБ (отчет панели мониторинга производительности SQL). Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/37167289). |
...
| Написание скриптов и SMO | Исправлена ошибка, из-за которой команда "Создать скрипт" в графовой таблице вызывает сбой при создании данных. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035-sql-server/suggestions/32898466). |
| Написание скриптов и SMO | Устранена проблема, когда метод EnumObjects() не получал имя схемы для синонима. |
...
|Поддержка Azure SQL| Добавлена поддержка для недавно добавленных номеров SKU виртуальных ядер ("Общего назначения" и "Критически важный для бизнеса"): Gen4_24 и все Gen5.|
...
|Инструкция ShowPlan|Добавлено фактическое истекшее время, фактическое и предполагаемое количество строк в узле оператора ShowPlan, если они доступны. Благодаря этому фактический план будет согласован с планом динамической статистики запросов.|
...
|Общие ошибки SSMS|Устранена проблема, когда форма пользовательского интерфейса мастера создания сеанса XEvent неправильно отображалась на мониторах с высоким разрешением.|
...
|Резервное копирование, восстановление, присоединение и отсоединение базы данных|Устранена проблема, при которой SSMS не удавалось находить допустимый план восстановления или удавалось находить неоптимальный. Дополнительные сведения см. в разделе [https://feedback.azure.com/forums/908035-sql-server/suggestions/32897752](https://feedback.azure.com/forums/908035-sql-server/suggestions/32897752). |
...
|обозревателе объектов|Исправлена проблема, при которой пользователь нажимал правой кнопкой мыши на узлах в обозревателе объектов (например, "Таблицы") и хотел выполнить действие, например отфильтровать таблицы в меню "Фильтр" > "Параметры фильтра", а форма параметров фильтра могла появиться на другом экране, а не на том, где активна среда SSMS. Дополнительные сведения см. в разделе [https://feedback.azure.com/forums/908035-sql-server/suggestions/34284106](https://feedback.azure.com/forums/908035-sql-server/suggestions/34284106). |
...
Always On: 
- Устранена проблема, из-за которой панель мониторинга задержки Always On не работала с SQL Server 2012.
...
- Устранена проблема, из-за которой при выполнении команды *Запустить PowerShell* не обнаруживался модуль SQL Server в случае, если определенный пользователем профиль PowerShell создавал выходные данные.
...
RestoreDefaultFonts — восстановление настроек по умолчанию. В настоящее время команда PresentOff недоступна. Используйте RestoreDefaultFonts, чтобы отключить режим представления*
...
- Исправлена ошибка с функцией списка разрешенных IP-адресов в брандмауэре баз данных SQL.
