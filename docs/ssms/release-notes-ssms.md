...
Скачать: [Скачать SSMS 18.4](download-sql-server-management-studio-ssms.md)  
Номер сборки: 15.0.18206.0  
Дата выпуска: 4 ноября 2019 г.

SSMS 18.4 является последним общедоступным выпуском SSMS. Если вам нужна предыдущая версия SSMS, см. список в разделе [Предыдущие выпуски SSMS](release-notes-ssms.md#previous-ssms-releases).

Версия 18.4 представляет собой обновление для версии 18.3.1 со следующими добавлениями и исправлениями ошибок.

## <a name="whats-new-in-184"></a>Новые возможности версии 18.4
...
| Always Encrypted | Устранена проблема, из-за которой при *проверке утверждения* не *учитывался регистр*. |
| Always Encrypted | Устранена проблема, из-за которой отчеты об ошибках и предупреждения не работали должным образом. |
| мастер копирования базы данных | Устранены различные проблемы усечения и разметки при отображении этого диалогового окна. |
...
| обозревателе объектов | Устранена проблема, из-за которой при удалении базы данных SQL Azure в обозревателе объектов отображалось неверное сообщение. |
| Результаты запроса | Устранена возникшая в SSMS 18.3.1 ошибка, из-за которой сетки создавались слишком узкими и в конце самой длинной строки в каждом столбце отображалось многоточие ( *...* ). |
| Средства репликации | Устранена проблема, из-за которой приложение вызвало ошибку ("Не удалось загрузить файл или сборку...") при попытке изменить задания агента SQL Server. |
| Написание скриптов и SMO | Устранена проблема, из-за которой команда *Создать скрипт для таблицы...* для базы данных SQL, в которой использовались параметры сортировки Japanese_BIN2, не работала.|
| Написание скриптов и SMO | Устранена проблема, из-за которой команда ScriptAlter() завершала выполнение инструкций на сервере.|
| Агент SQL Server | Устранена проблема, из-за которой пользовательский интерфейс оператора агента не обновлял имя оператора, когда его изменяли в пользовательском интерфейсе, и создать для него скрипт было невозможно. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/32897647).|
...
* При переключении между несколькими окнами запросов возникают проблемы перерисовки. Дополнительные сведения см. на сайте UserVoice. Для решения этой проблемы можно отключить аппаратное ускорение в разделе *"Сервис" > "Параметры"* .
...
## <a name="downloadssdtmediadownloadpng-ssms-1831httpsgomicrosoftcomfwlinklinkid2105412"></a>![Скачать](../ssdt/media/download.png) [SSMS 18.3.1](https://go.microsoft.com/fwlink/?linkid=2105412)

Номер выпуска: 18.3.1  
Номер сборки: 15.0.18183.0  
Дата выпуска: 2 октября 2019 г.

SSMS 18.3.1 является последним общедоступным выпуском SSMS. Если вам нужна предыдущая версия SSMS, см. список в разделе [Предыдущие выпуски SSMS](release-notes-ssms.md#previous-ssms-releases).

Версия 18.3.1 представляет собой обновление для версии 18.2 со следующими добавлениями и исправлениями ошибок.
...
| Написание скриптов и SMO | Устранена проблема, из-за которой не удавалось создавать скрипты для *внешнего источника данных* в SQL 2019. Дополнительные сведения см. на сайте [UserVoice](https://feedback.azure.com/forums/908035/suggestions/34295080). |
...
|Оценка уязвимостей|Устранена проблема, из-за которой результаты проверки загружались некорректно.|
...
- Стандартные политики DMF
  - Эти политики больше не устанавливаются вместе со средой SSMS. Они будут перенесены в Git. Пользователи смогут добавлять и загружать или устанавливать их, если захотят.
