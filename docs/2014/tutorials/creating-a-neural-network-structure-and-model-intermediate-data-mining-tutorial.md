---
title: Создание нейронной сети структуры и модели (учебник по интеллектуальному анализу данных — средний) | Документы Microsoft
ms.custom: ''
ms.date: 12/29/2017
...
4.  На **создать структуру интеллектуального анализа данных** убедитесь, что параметр **создать структуру и модель интеллектуального анализа данных** выбран.  
...
7.  В разделе **доступные представления источника данных**выберите `Call Center`и нажмите кнопку **Далее**.  
  
8.  На **определение типов таблиц** выберите **случай** флажок рядом с **FactCallCenter** таблицы. Не выбирайте никакие параметры для **DimDate**. Нажмите кнопку **Далее**.  
...
12. На **содержимого и типа данных столбцов указать** , убедитесь, что в сетке содержатся столбцы, типы содержимого и типы данных, как показано в следующей таблице и выберите **Далее**.  
...
13. На **создать тестирования задайте** страницы, очистите текстовое поле для параметра **процент проверочных данных**. Нажмите кнопку **Далее**.  
  
14. На **завершение работы мастера** страницы, для **имя структуры интеллектуального анализа данных**, тип `Call Center`.  
  
15. Для **имя модели интеллектуального анализа данных**, тип `Call Center Default NN`, а затем нажмите кнопку **Готово**.  
  
     **Разрешить детализацию** отключено, так как нельзя выполнять детализацию данных в моделях нейронных сетей.  
  
16. В обозревателе решений щелкните правой кнопкой мыши имя структуры интеллектуального анализа данных только что создана и выберите **процесс**.  
...
> [!NOTE]  
>  Эту статистику можно получить из узла граничной статистики модели после обработки всех данных. Дополнительные сведения об узле граничной статистики см. в разделе [Mining Model Content для модели нейронной сети &#40;службы Analysis Services — Интеллектуальный анализ данных&#41;](../../2014/analysis-services/data-mining/mining-model-content-for-neural-network-models-analysis-services-data-mining.md).  
...
-   **Сегментирование на равные области**  
 ...
 Кроме того, вместо использования числовых значений, можно добавить отдельный производный столбец, который классифицирует уровни обслуживания по стандартным целевым диапазонам, например **наиболее** (ServiceGrade \<= 0,05),  **Допустимые** (0,10 > ServiceGrade > 0,05), и **низкой** (ServiceGrade > = 0,10).  
  
###  <a name="bkmk_newColumn"></a> Создание копии столбца и измените метод дискретизации  
 Предстоит создать копию столбца интеллектуального анализа данных, который содержит целевой атрибут ServiceGrade и изменить способ группировки чисел. Можно создать несколько копий любого столбца в структуре интеллектуального анализа данных, включая прогнозируемый атрибут.  
...
3.  В **выберите столбец** диалоговое окно, выберите ServiceGrade из списка в **исходный столбец**, нажмите кнопку **ОК**.  
...
-   Создании новой модели интеллектуального анализа данных Call Center Binned NN, использующий в качестве результатов значения столбца ServiceGrade, распределенные по четырем контейнерам одинакового размера.  
...
1.  В обозревателе решений щелкните правой кнопкой мыши структуру интеллектуального анализа данных, только что созданное и выберите **откройте**.  
...
4.  В **новой модели интеллектуального анализа данных** диалоговое окно, для **имя_модели**, тип `Call Center Binned NN`. В **имя алгоритма** раскрывающегося списка выберите **алгоритм нейронной сети Майкрософт**.  
...
1.  В **модели интеллектуального анализа данных** в разделе **структуры**, выберите столбец ServiceGrade Binned.  
  
     Обратите внимание, что **свойства** выводятся свойства объекта — столбца ScalarMiningStructure.  
...
4.  Найдите **описание** свойство и введите **временный псевдоним столбца**.  
    ...
    |**Флаги моделирования**||  
    |**Название**|Service Grade|  
    |**Идентификатор SourceColumn**|Service Grade 1|  
    |**Usage**|Predict|  
  
5.  Щелкните в любом месте **модель интеллектуального анализа данных** вкладки.  
  
     Обновление таблицы, чтобы показать новый временный псевдоним столбца, `ServiceGrade`, рядом с использование столбца. Сетка, содержащая структуру интеллектуального анализа данных и две модели интеллектуального анализа данных, должна иметь следующий вид:  
...
###  <a name="bkmk_SeedProcess"></a> Чтобы задать начальное значение и обработка моделей  
