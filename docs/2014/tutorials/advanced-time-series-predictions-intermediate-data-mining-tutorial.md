...
2.  Вставьте новые значения с помощью следующего скрипта.  
    ...
    > [!WARNING]  
    >  Кавычки используются для значений валют, чтобы не возникало проблем с разделителем в виде запятой и символом валюты. Можно также передавать в значениях денежных сумм в следующем формате: `130170.22`  
    >   
...
3.  На **Выбор источника данных** выберите [!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)], а затем нажмите кнопку **Далее**.  
...
4.  Щелкните правой кнопкой мыши пустая область конструктора для **T1000 Pacific Region.dsv**, а затем выберите **новым именованным запросом**.  
     ...
     **Описание**: **фильтра`vTimeSeries`по региону и модели**  
