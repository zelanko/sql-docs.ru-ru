...
## <a name="singleton-query"></a>Одноэлементный запрос  
 Первым шагом является использование [SELECT FROM &#60;модели&#62; PREDICTION JOIN &#40;расширений интеллектуального анализа данных&#41; ](/sql/dmx/select-from-model-cases-dmx) в Одноэлементный прогнозирующий запрос. В следующем фрагменте показан общий пример одноэлементной инструкции:  
...
3.  Вместо  
    ...
    ```  
     ...
     Инструкция AS используется для создания псевдонимов столбцов, которые возвращает запрос. [PredictHistogram](/sql/dmx/predicthistogram-dmx) функция возвращает статистические данные прогноза, включая вероятность и поддержку. Дополнительные сведения о функциях, которые могут использоваться в инструкции прогноза см. в разделе [функции &#40;расширений интеллектуального анализа данных&#41;](/sql/dmx/functions-dmx).  
...
7.  В **Сохранить как** диалоговом окне перейдите к соответствующей папке и присвойте файлу имя `Singleton_Query.dmx`.  
...
## <a name="batch-query"></a>Пакетный запрос  
 Следующим шагом является использование [SELECT FROM &#60;модели&#62; PREDICTION JOIN &#40;расширений интеллектуального анализа данных&#41; ](/sql/dmx/select-from-model-cases-dmx) в пакетном прогнозирующем запросе. Ниже представлен общий пример пакетной инструкции:  
...
```  
 ...
 Как и в одноэлементном запросе, первые две строки кода определяют столбцы из модели интеллектуального анализа, которые возвращает запрос, а также имя модели интеллектуального анализа данных, которая используется для создания прогноза. ВЕРХНЕЙ \<номер > инструкция указывает, что запрос будет возвращать только в том случае, число или результатов, заданное параметром \<номер >.  
...
```  
 ...
 Использовать ORDER BY в сочетании с ВЕРХНЕЙ \<номер > инструкции для фильтрации результатов, возвращаемых. Например, в этом прогнозе возвращаются первые десять покупателей велосипедов, упорядоченных по степени вероятности реализации прогноза. Для управления порядком вывода результатов можно использовать синтаксис [DESC|ASC].  
...
8.  В **Сохранить как** диалоговом окне перейдите к соответствующей папке и присвойте файлу имя `Batch_Prediction.dmx`.  
...
9. На панели инструментов нажмите кнопку **Выполнить** .  
 ...
 Сведения об использовании расширений интеллектуального анализа данных в сценарии потребительской корзины, см. в разделе [руководстве расширений интеллектуального анализа данных Market Basket](../../2014/tutorials/market-basket-dmx-tutorial.md).  
