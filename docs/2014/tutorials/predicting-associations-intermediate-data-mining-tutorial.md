---
title: Прогноз взаимосвязей (учебник по интеллектуальному анализу данных — средний) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="predicting-associations-intermediate-data-mining-tutorial"></a>Прогноз взаимосвязей (учебник по интеллектуальному анализу данных — средний уровень)
  После обработки моделей для создания прогнозов можно использовать сведения о взаимосвязях, хранящиеся в модели. В последней задаче этого занятия предстоит изучить, как создавать прогнозирующие запросы к созданным моделям взаимосвязей. В этом занятии предполагается, что вы знакомы с работой построителя прогнозирующих запросов и хотите научиться создавать прогнозирующие запросы к моделям взаимосвязей. Дополнительные сведения том, как использовать построитель прогнозирующих запросов см. в разделе [интерфейсы запросов интеллектуального анализа данных](../../2014/analysis-services/data-mining/data-mining-query-tools.md).  
...
1.  Нажмите кнопку **модель интеллектуального анализа данных** вкладки конструктора интеллектуального анализа данных.  
  
2.  В **модель интеллектуального анализа данных** области, нажмите кнопку **Выбор модели**. (Если нужная модель уже выбрана, этот и следующий шаги можно пропустить.)  
  
3.  В **Выбор модели интеллектуального анализа данных** диалогового окна разверните узел, представляющий структуру интеллектуального анализа данных **ассоциации**и выберите модель **ассоциации**. Нажмите кнопку **ОК**.  
  
     На данном этапе можно не учитывать входную панель.  
  
4.  В сетке щелкните пустую ячейку под **источника** и выберите **Прогнозирующая функция.** В ячейке столбца **поле**выберите `PredictAssociation`.  
  
     Можно также использовать **Predict** для прогноза взаимосвязей. Если в таком случае обязательно выберите версию **Predict** функцию, которая принимает в качестве аргумента столбец таблицы.  
  
5.  В **модель интеллектуального анализа данных** области, выберите вложенную таблицу `vAssocSeqLineItems`и перетащите ее в сетку, в **критерий или аргумент** поле для `PredictAssociation` функции.  
...
6.  Нажмите кнопку **критерий или аргумент** и введите после имени таблицы следующий текст: `,3`  
  
     Полный текст **критерий или аргумент** окно должно выглядеть следующим образом:  
  
     `[Association].[v Assoc Seq Line Items],3`  
  
7.  Нажмите кнопку **результатов** кнопка в верхнем углу построителя прогнозирующих запросов.  
  
 Ожидаемые результаты содержат один столбец с заголовком **выражение**. **Выражение** столбец содержит вложенную таблицу с один столбец, а также следующие три строки. Поскольку входное значение не было указано, эти прогнозы представляют наиболее вероятные взаимосвязи продуктов для модели в целом.  
...
|Touring-3000|  
 ...
 Далее будет использоваться **одноэлементного запроса** панели, чтобы указать продукт в качестве входных данных для запроса и просмотреть продукты, которые, скорее всего, связанный с этим элементом.  
...
1.  Нажмите кнопку **разработки** в углу построителя прогнозирующих запросов, чтобы вернуться в сетку построения запросов.  
  
2.  На **модель интеллектуального анализа данных** последовательно выберите пункты **одноэлементный запрос**.  
  
3.  В **модель интеллектуального анализа данных** выберите **ассоциации** модели.  
  
4.  В сетке щелкните пустую ячейку под **источника** и выберите **Прогнозирующая функция.** В ячейке столбца **поле**выберите `PredictAssociation`.  
  
5.  В **модель интеллектуального анализа данных** области, выберите вложенную таблицу `vAssocSeqLineItems`и перетащите ее в сетку, в **критерий или аргумент** поле для `PredictAssociation` функции. Тип `,3` после имени вложенной таблицы, как и в предыдущей процедуре.  
  
6.  В **одноэлементного запроса** диалоговое окно, нажмите кнопку **значение** рядом с **vAssoc Seq Line Items**, а затем нажмите кнопку **(...)**  кнопки.  
  
7.  В **Вход вложенной таблицы** выберите `Touring Tire` в **ключевой столбец** , а затем щелкните **добавить**.  
  
8.  Нажмите кнопку **результатов** кнопки.  
...
2.  В **одноэлементного запроса** диалоговое окно, нажмите кнопку **значение** рядом с **vAssoc Seq Line Items**, а затем нажмите кнопку **(...)**  кнопки.  
  
3.  В **ключевой столбец** выберите `Touring Tire`, а затем нажмите кнопку **добавить**.  
  
4.  В сетке щелкните пустую ячейку под **источника** и выберите **Прогнозирующая функция.** В ячейке столбца **поле**выберите `PredictAssociation`.  
  
5.  В **модель интеллектуального анализа данных** области, выберите вложенную таблицу `vAssocSeqLineItems`и перетащите ее в сетку, в **критерий или аргумент** поле для `PredictAssociation` функции. Тип `,3` после имени вложенной таблицы, как и в предыдущей процедуре.  
  
6.  В **Вход вложенной таблицы** выберите `Touring Tire Tube` в **ключевой столбец** , а затем щелкните **добавить**.  
  
7.  В сетке на строке `PredictAssociation` функционировать, нажмите кнопку **критерий или аргумент** поле и измените аргументы таким образом, чтобы добавить аргумент INCLUDE_STATISTICS.  
...
8.  Нажмите кнопку **результатов** кнопки.  
 ...
 Результаты во вложенной таблице меняются, отображая прогнозы вместе с поддержкой и вероятностью. Дополнительные сведения об интерпретации этих значений см. в разделе [Mining Model Content для моделей взаимосвязей &#40;службы Analysis Services — Интеллектуальный анализ данных&#41;](../../2014/analysis-services/data-mining/mining-model-content-for-association-models-analysis-services-data-mining.md).  
...
3.  Нажмите кнопку **результатов** кнопка в верхнем углу построителя прогнозирующих запросов.  
...
1.  В **модель интеллектуального анализа данных** последовательно выберите пункты **одноэлементный запрос** еще раз, снимите флажок.  
...
2.  В **Выбор входных таблиц** диалоговое окно, нажмите кнопку **выбрать таблицу вариантов**.  
  
3.  В **Выбор таблицы** диалоговом для **источника данных**, выберите AdventureWorksDW2008. В **имя таблицы или представления** выберите vAssocSeqOrders, а затем нажмите кнопку **ОК**.  
  
     Таблица vAssocSeqOrders добавляется на панель.  
  
4.  В **Выбор входных таблиц** диалоговое окно, нажмите кнопку **выбрать вложенную таблицу**.  
  
5.  В **Выбор таблицы** диалоговом для **источника данных**, выберите AdventureWorksDW2008. В **имя таблицы или представления** выберите vAssocSeqLineItems, а затем нажмите кнопку **ОК**.  
  
     Таблица vAssocSeqLineItems добавляется на панель.  
  
6.  В **Указание вложенного соединения** диалоговое окно, перетащите поле OrderNumber из таблицы вариантов и поместите его в поле OrderNumber вложенной таблицы.  
  
     Можно также щелкнуть **добавить отношение** и создать связь путем выбора из списка столбцов.  
  
7.  В **укажите отношение** диалогового окна убедитесь, что правильно сопоставлены полей OrderNumber и нажмите кнопку **ОК**.  
  
8.  Нажмите кнопку **ОК** закрыть **Указание вложенного соединения** диалоговое окно.  
  
     Таблица вариантов и вложенная таблица обновляются на панели конструирования для отображения соединений между столбцами внешних данных и столбцами модели. Если связи неправильные, можно правой кнопкой мыши соединительную линию и выберите **изменить соединения** изменение столбца сопоставление, или можно правой кнопкой мыши соединительную линию и выберите **удаление** для удаления связь полностью.  
...
12. Перетащите vAssocSeqLineItems в **критерий или аргумент** поле `PredictAssociation` строки. Нажмите кнопку в конце **критерий или аргумент** и введите следующий текст: `INCLUDE_STATISTICS,3`  
...
13. Нажмите кнопку **результат** кнопку, чтобы просмотреть прогнозы для каждого клиента.  
 ...
 Можно попытаться создать подобный прогнозирующий запрос для нескольких моделей и узнать, изменятся ли результаты прогнозов после фильтрации. Дополнительные сведения о создании прогнозов и других типов запросов см. в разделе [примеры запросов к модели взаимосвязей](../../2014/analysis-services/data-mining/association-model-query-examples.md).  
