---
title: Создание прогнозов для моделей Call Center (учебник по интеллектуальному анализу данных — средний) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
3.  В **модель интеллектуального анализа данных** области, нажмите кнопку **Выбор модели**.  
...
6.  В меню **Модель интеллектуального анализа данных** выберите пункт **Одноэлементный запрос**.  
     ...
     **Одноэлементного запроса** диалоговое окно, в котором столбцы сопоставлены со столбцами в модели интеллектуального анализа данных.  
...
9. В нижней половине **модель интеллектуального анализа данных** щелкните первую строку в сетке.  
  
10. В **источника** столбец, щелкните стрелку вниз и выберите **Прогнозирующая функция**. В **поле** выберите **PredictHistogram**.  
...
12. Нажмите кнопку **коммутатора для запроса представления результатов**, в верхнем углу построителя прогнозирующих запросов.  
...
## <a name="creating-predictions-by-using-a-logistic-regression-model"></a>Создание прогнозов с помощью модели логистической регрессии  
 Если уже известны атрибуты, относящиеся к бизнес-задаче, можно использовать модель логистической регрессии для прогнозирования эффекта изменения определенных независимых переменных. Логистическая Регрессия является статистическим методом, который широко применяется для создания прогнозов на основе изменений в независимых переменных: например, он используется в сценариях финансовой оценки, чтобы прогнозировать поведение покупателей на основании их демографических данных.  
...
### <a name="generating-data-used-for-bulk-prediction"></a>Создание данных, используемых для массового прогноза  
 Существует много способов предоставить входные данные: например, можно импортировать кадровые показатели из электронной таблицы и запуск этих данных с помощью модели, чтобы спрогнозировать качество обслуживания на следующий месяц.  
...
2.  В мастере представлений источников данных выберите [!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)] как источник данных, а затем нажмите кнопку **Далее**.  
...
6.  В **Создание именованного запроса** диалоговом для **имя**, тип `Shifts for Call Center`.  
...
9. Щелкните правой кнопкой мыши вкладку **Shifts.dsv (конструктор)** и нажмите кнопку **Сохранить** сохранить новое определение представления источника данных.  
...
2.  В **модель интеллектуального анализа данных** области, нажмите кнопку **Выбор модели**и выберите Call Center - LR из списка доступных моделей.  
...
5.  В **Выбор таблицы** диалоговое окно, selectShifts из списка источников данных. В **имя таблицы или представления** выберите Shifts for Call Center (он может определяться автоматически), а затем щелкните **ОК.**  
  
     **Модель интеллектуального анализа данных** рабочей области конструктора обновляется для отображения сопоставления, созданные на основании имена и типы данных столбцов во входных данных и модели.  
...
8.  Щелкните пустую ячейку рядом с Calls и выберите **Shifts for Call Center.AvgCalls**. затем **ОК**.  
...
4.  Щелкните следующую пустую строку в сетке. В **источника** выберите **Прогнозирующая функция**. В **поле** выберите **Predict**.  
...
6.  Щелкните следующую пустую строку в сетке. В **источника** выберите **Прогнозирующая функция**. В **поле** выберите **PredictProbability**.  
...
8.  Нажмите кнопку **переключитесь в режим просмотра результата запроса** Чтобы просмотреть прогнозы.  
...
|Смена, ставка и день|Прогнозируемое качество обслуживания с текущим средним временем ответа.|Прогнозируемое качество обслуживания с 90 процентов уменьшение времени ответа|Прогнозируемое качество обслуживания при уменьшении времени ответа до 80 процентов от текущего.|  
|--------------------------|------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|  
...
-   [SQL Server 2005 данных надстройки интеллектуального анализа для Office 2007](http://www.microsoft.com/sql/technologies/dm/addins.mspx): поиска решения и что делать, если анализ сценария  
  
-   [SQL Server 2008 данных надстройки интеллектуального анализа для Office 2007](http://go.microsoft.com/fwlink/?LinkID=117790): что делать, если анализ сценария поиска решения анализ сценария и калькулятор прогнозов  
