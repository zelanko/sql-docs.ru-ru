 ...
 **Сравнения кластеров** вкладка предназначена для сравнения двух кластеров, чтобы определить, какие атрибуты, отличающих из другого кластера. Вкладка содержит четыре столбца: **Переменные**, **значения**, **кластер 1**, и **кластера 2**.  Вы можете выбрать любой кластер для использования в качестве **кластер 1** и **кластера 2**.  
...
1.  В **сравнения кластеров** вкладке для **кластер 1**выберите `Pacific Cluster`.  
     ...
     По умолчанию выбор для **кластера 2** примет **дополнением Тихоокеанский кластер**.  
...
8.  Щелкните узел шины для туристического велосипеда и переместите ползунок обратно в среднее положение.  
     ...
     На первый, существует множество линий переходов, соединяющих шина для туристического велосипеда для других продуктов, но при повышении порога вероятности, менее вероятные переходы будут удалены из графа, оставляя только один переход, но для туристического велосипеда > камера шины для велосипеда. Данный переход означает, что если клиент помещает в свою корзину покупок шину для туристического велосипеда, то существует большая вероятность того, что следующим товаром станет камера шины для туристического велосипеда.  
