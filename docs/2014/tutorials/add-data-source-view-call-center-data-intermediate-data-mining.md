---
title: Добавление данных представления источника для данных Call Center (учебник по интеллектуальному анализу данных — средний) | Документы Microsoft
ms.custom: ''
ms.date: 12/29/2017
...
3.  На **выбрать источник данных** в разделе **реляционные источники данных**выберите [!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)] источника данных. Если у вас этого источника данных, см. раздел [Basic Data Mining Tutorial](../../2014/tutorials/basic-data-mining-tutorial.md). Нажмите кнопку **Далее**.  
...
|WageType|Показывает, является день рабочим, выходным или праздничным.<br /><br /> Это возможно, что существует различие в качество обслуживания клиентов в выходные и рабочие дни, этот столбец будет использоваться в качестве входных данных.|  
...
|ServiceGrade|Показатель, отражающий общее качество обслуживания, измеряемое как *прекращенных* за всю смену. Чем выше показатель звонков, прекращенных абонентом во время ожидания, тем более вероятно, что клиенты остаются недовольными, а компания теряет потенциальные заказы.|  
 ...
 Тем не менее, мы не будем использовать `DateKey` в модели, так как он содержит слишком много уникальных значений. Нет нет прямой связи между `Shift` и **DayOfWeek**, и `WageType` и **DayOfWeek** связаны только частично. Если есть озабоченность по поводу коллинеарности, можно создать структуру, используя все доступные столбцы, затем проигнорировать другие столбцы в каждой модели и протестировать результаты.  
