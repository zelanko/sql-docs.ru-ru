---
title: Общее представление о требованиях для временного ряда модели (учебник по интеллектуальному анализу данных — средний) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
2.  На **таблицы** просмотрите данные, используемые в столбцах TimeIndex и Reporting Date.  
...
3.  В области конструктора представления источника данных выберите столбец Reporting Date и **свойства**. Далее щелкните столбец TimeIndex и выберите **свойства**.  
     ...
     Поле TimeIndex имеет тип данных System.Int32, в то время как поле Reporting Date имеет тип данных System.DateTime. Во многих хранилищах данных значения даты и времени преобразуются в целые числа, и целочисленный столбец служит ключом, что повышает производительность индексирования. Однако если использовать такой столбец, то алгоритм временных рядов (Майкрософт) будет составлять прогнозы, используя значения из будущего: 201014, 201014 и т. д. Поскольку необходимо представить данные о продажах прогноз с использованием календарных дат, в качестве уникального идентификатора последовательности будет применяться столбец Reporting Date.  
