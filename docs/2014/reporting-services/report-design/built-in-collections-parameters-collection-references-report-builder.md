...
|Пример|Описание|  
|-------------|-----------------|  
|`=Parameters!` *\<Имя_параметра>* `.IsMultiValue`|Возвращает `False`.<br /><br /> Проверяет, является ли параметр многозначным. Если `True`, то параметр является многозначным и представляет коллекцию объектов. Если возвращено значение `False`, значит, параметр является однозначным и представляет собой один объект.|  
...
|`=Join(Parameters!` *\<Имя_параметра_с_несколькими_значениями>* `.Value,", ")`|Возвращает `String` , включающая в себя список с разделителями запятыми значений из многозначных параметров. Во втором параметре можно использовать любой разделитель для соединения.|  
