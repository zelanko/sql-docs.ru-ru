...
|Пример|Описание|  
|-------------|-----------------|  
|`=Parameters!` *\<Имя_параметра>* `.IsMultiValue`|Возвращает `False`.<br /><br /> Проверяет, является ли параметр многозначным. Если `True`, то параметр является многозначным, и он является коллекцией объектов. Если возвращено значение `False`, значит, параметр является однозначным и представляет собой один объект.|  
...
|`=Join(Parameters!` *\<Имя_параметра_с_несколькими_значениями>* `.Value,", ")`|Возвращает `String` , состоящий из разделенный запятыми список значений из многозначных параметров. Во втором параметре можно использовать любой разделитель для соединения.|  
