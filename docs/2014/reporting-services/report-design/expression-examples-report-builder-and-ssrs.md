...
-   [Часто используемые фильтры &#40;отчетов построителя отчетов и службы SSRS&#41;](commonly-used-filters-report-builder-and-ssrs.md)  
...
-   `Round` Функция полезна для округления чисел до ближайшего целого числа. В следующем выражении значение 1.3 округляется до 1:  
  
    ```  
    = Round(1.3)  
    ```  
  
     Можно также написать выражение, значение до нескольких, округляющее, аналогично `MRound` функции в Excel. Умножьте значение на фактор, образующий целое число, округлите число, а затем разделите его на тот же фактор. Например, для округления числа 1,3 до ближайшего числа, кратного 2(1.4), воспользуйтесь следующим выражением:  
...
-   `Today` Функция возвращает текущую дату. Следующее выражение можно указать в текстовом поле для вывода даты формирования отчета или в качестве параметра фильтрации данных на основе текущей даты.  
...
-   `Year` Функция отображает год для определенной даты. Это можно использовать для группирования по датам или для вывода года в качестве метки для набора дат. Выражение возвращает год для заданной группы дат заказов на продажу. `Month` Функции и других функций можно также использовать для работы с датами. Дополнительные сведения см. в документации по [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)].  
...
-   Функция `CDate` преобразует это значение в дату. `Now` Функция возвращает значение типа date, содержащее текущую дату и время по часам компьютера. `DateDiff` возвращает значение типа Long, указывающее число интервалов времени между двумя значениями типа Date.  
...
-   `DatePart` Функция возвращает целочисленное значение, содержащее указанный компонент данного значения типа Date. Следующее выражение возвращает год первого значения SellStartDate в DataSet1. Область набора данных заданы из-за наличия нескольких наборов данных в отчете.  
...
-   `DateSerial` Функция возвращает значение типа Date, представляющее указанный год, месяц и день, сведения о времени, равным полуночи. В следующем примере показана дата окончания прошлого месяца с учетом текущего месяца.  
...
-   Функция `Format` позволяет отформатировать даты и числа в виде строки. Следующее выражение отображает значения параметров *StartDate* и *EndDate* в длинном формате даты.  
    ...
    ```  
     ...
     Если текстовое поле содержит только дату или число, следует использовать свойство форматирования текстового поля, чтобы применить форматирование, а не `Format` функция внутри текстового поля.  
  
-   `Right`, `Len`, И `InStr` функции полезны для извлечения подстроки, например, trimming *домена*\\*username* к имени пользователя. Следующее выражение возвращает часть строки параметра\\User *, расположенную справа от символа обратной косой черты (*).  
...
-   Отображает выбранные значения из многозначного параметра. В следующем примере используется `Join` объединяет выбранные значения параметра *MySelection* в одну строку, можно задать в качестве выражения для значения текстового поля в элементе отчета:  
...
-   Используйте несколько `IIF` (также называется «вложенные IIFs») функции, возвращающие одно из трех значений в зависимости от значения `PctComplete`. Следующее выражение может быть помещено в свойство цвета заливки текстового поля, чтобы изменять цвета фона в зависимости от значения текстового поля.  
...
-   Другой способ получить те же функциональные возможности использует `Switch` функции. Функция `Switch` полезна при наличии трех и более проверяемых условий. Функция `Switch` возвращает значение, связанное с первым выражением ряда, которое оценивается как true:  
...
-   Для проверки значения `PhoneNumber` поля и возвращает «No Value», если `null` (`Nothing` в [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]); в противном случае возвращает значение телефонного номера. Это выражение позволяет управлять содержимым текстового поля в элементе отчета.  
  
    ```  
    =IIF(Fields!PhoneNumber.Value Is Nothing,"No Value",Fields!PhoneNumber.Value)  
    ```  
  
-   Для проверки значения `Department` поля и возвращает имя вложенного отчета или `null` (`Nothing` в [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]). Это выражение может использоваться для условной детализации вложенных отчетов.  
...
-   `Sum` Функция суммирует значения в области данных или группы. Она может быть полезна в колонтитулах группы. Следующее выражение отображает сумму данных в группе или области данных Order.  
  
    ```  
    =Sum(Fields!LineTotal.Value, "Order")  
    ```  
  
-   Можно также использовать `Sum` функции для вычислений условных статистических функций. Например, если набор данных содержит поле State с возможными значениями «Not Started», «Started», «Finished», следующее значение, помещенное в верхний колонтитул группы, вычисляет статистическую сумму только для значения «Finished».  
...
-   Если функция `RowNumber` используется в текстовом поле в области данных, то она отображает номер строки для каждого экземпляра текстового поля, в котором содержится это выражение. Эта функция может оказаться полезной для нумерации строк в таблице. Также она может быть полезна для более сложных задач, например для разбивки на страницы по определенному числу строк. Дополнительные сведения см. в подразделе [Разрывы страниц](#PageBreaks) далее в этом разделе.  
     ...
     Область, указанная для элементов управления `RowNumber`, когда начинается перенумерация. `Nothing` Ключевое слово указывает, что функция начнет нумерацию строк с первой строки самой внешней области данных. Чтобы начать подсчет внутри вложенной области данных, используйте имя области данных. Чтобы начать подсчет внутри группы, используйте имя группы.  
...
-   Следующее выражение, если значение в `Hidden` свойство столбца, показывает столбец, только если поле существует в наборе данных отчета после извлечения данных из источника данных:  
...
### <a name="using-group-variables-for-custom-aggregation"></a>Использование групповых переменных для нестандартного статистического выражения  
 ...
 Дополнительные сведения о переменных см. в разделе [отчета и ссылки на коллекции переменных группы &#40;построитель отчетов и службы SSRS&#41;](built-in-collections-report-and-group-variables-references-report-builder.md).  
