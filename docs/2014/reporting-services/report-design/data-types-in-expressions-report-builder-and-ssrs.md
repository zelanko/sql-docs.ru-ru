...
### <a name="converting-a-string-data-type-to-a-clr-datetime-data-type"></a>Преобразование строкового типа данных в тип данных DateTime среды CLR  
 ...
 В этом примере показана дата (1 июля 2008), за которой следует время с 7-значной точностью (6:05:07.9999999 утра) и смещение часового пояса времени в формате UTC в часах и минутах (+ 8 часов 0 минут). В следующих примерах это значение помещается в `String` поле с именем `MyDateTime.Value`.  
