...
> [!NOTE]  
>  Свойство IsMissing обнаруживает изменения в схеме набора данных, произошедшие между временем разработки и временем выполнения, для всех типов источников данных. IsMissing не может использоваться для определения пустых элементов в многомерном кубе и не имеет отношения к концепции языка запросов многомерных Выражений `EMPTY` и `NON EMPTY`.  
...
### <a name="using-extended-field-properties"></a>Использование расширенных свойств поля  
 ...
 Если указать свойство, которое не поддерживается для этого поля, то выражение принимает значение `null` (`Nothing` в [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]). Если поставщик данных не поддерживает расширенные свойства полей или поле не найдено при выполнении запроса, значением свойства является `null` (`Nothing` в [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]) для свойств типа `String` и `Object`, и ноль (0) для свойств типа `Integer`. Модуль обработки данных может использовать преимущества стандартных свойств, оптимизируя запросы, включающие следующий синтаксис.  
