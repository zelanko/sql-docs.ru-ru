...
### <a name="using-extended-field-properties"></a>Использование расширенных свойств поля  
 ...
 Если указать свойство, которое не поддерживается для этого поля, выражение принимает значение `null` (`Nothing` в [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]). Если поставщик данных не поддерживает расширенные свойства полей, или если поле не найдено при выполнении запроса, значением свойства является `null` (`Nothing` в [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]) для свойств типа `String` и `Object`, и ноль (0) для свойств типа `Integer`. Модуль обработки данных может использовать преимущества стандартных свойств, оптимизируя запросы, включающие следующий синтаксис.  
