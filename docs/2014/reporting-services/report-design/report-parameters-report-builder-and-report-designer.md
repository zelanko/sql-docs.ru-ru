...
[!INCLUDE[applies](../../includes/applies-md.md)] Режиме интеграции с SharePoint и собственного режима
  
 Демонстрацию по добавлению параметра в отчет см. в разделе [учебника: Добавление параметров для отчетов (SSRS)](http://technet.microsoft.com/library/aa337432\(v=SQL.105\).aspx)  
...
##  <a name="UserInterface"></a> Вкладка параметров  
 При просмотре отчета на панели инструментов средства просмотра отчетов отображаются все параметры, что обеспечивает возможность интерактивного задания значений пользователем. На следующем рисунке показан область параметров для отчета с параметрами @StartDate, @EndDate, @Subcategory, и @ShowAllRows.  
...
2.  **@StartDate и @EndDate параметры** параметр @StartDate имеет тип данных `DateTime`. Рядом с текстовым полем отображается запрос на ввод начальной даты (Start Date). Чтобы изменить дату, введите новую дату в текстовом поле или используйте элемент управления «календарь».  
  
     Параметр @EndDate рядом @StartDate.  
  
3.  **@Subcategory параметр** параметр @Subcategory имеет тип данных `Text`. Поскольку @Subcategory имеет список доступных значений, допустимые значения отображаются в раскрывающемся списке. Необходимо выбрать значения в этом списке. Поскольку @Subcategory является многозначным, **выделить все** параметр отображается, дает возможность очистить все и выберите все значения в списке.  
  
4.  **@ShowAllRows параметр** параметр @ShowAllRows имеет тип данных `Boolean`. С помощью переключателей укажите `True` или `False`.  
...
7.  **Кнопка просмотра отчетов** на панели инструментов средства просмотра отчетов, нажмите кнопку **просмотреть отчет** запустить отчет после ввода значений параметров. Если для всех параметров заданы значения по умолчанию, то отчет запускается автоматически при первом просмотре.  
...
-   Добавить запрос набора данных, который содержит переменные или хранимую процедуру набора данных, которая содержит параметры ввода.  Параметр набора данных создается для каждой переменной или параметра ввода, а параметр отчета создается для каждого параметра набора данных.  
     ...
     ![Диалоговое окно «Свойства набора данных» и вкладка отчет](../media/datasetquery-parameters.png "диалоговое окно «Свойства набора данных» и вкладка отчет")  
...
-   Выберите значение из раскрывающегося списка для одного параметра, которое определяет доступные значения для другого параметра в раскрывающемся списке. Это каскадные параметры. Каскадные параметры позволяют вам быстро фильтровать значения параметров из тысяч значений в управляемое количество.  
     ...
     Дополнительные сведения см. в разделе [Добавление каскадных параметров в отчет &#40;построитель отчетов и службы SSRS&#41;](add-cascading-parameters-to-a-report-report-builder-and-ssrs.md).  
...
|Доступные значения|При наличии у параметра заданных доступных значений, допустимые значения всегда отображаются в виде раскрывающегося списка. Например, если предоставить доступные значения для `DateTime` параметр, раскрывающегося списка для дат, который появляется в области параметров вместо элемента управления calendar. Чтобы обеспечить согласование списка значений в отчете и вложенных отчетах, можно настроить параметр источника данных для использования одной транзакции во всех запросах в наборах данных, связанных с источником данных.<br /><br /> **\*\* Примечание по безопасности \* \***  в любом отчете, содержащем параметры типа данных `Text`, используйте список доступных значений (также называемый списком допустимых значений) и убедитесь, что любой пользователь, запускающий отчет имеет только разрешения на просмотр данных в отчете. Дополнительные сведения см. в разделах [Security &#40;Report Builder&#41;](../report-builder/security-report-builder.md).|  
...
-   [Добавить, изменить или удалить значения по умолчанию для параметра отчета &#40;отчетов построителя отчетов и службы SSRS&#41;](add-change-or-delete-default-values-for-a-report-parameter.md)  
...
-   [Добавить вложенный отчет и параметры &#40;отчетов построителя отчетов и службы SSRS&#41;](add-a-subreport-and-parameters-report-builder-and-ssrs.md)  
...
##  <a name="bkmk_Related_Topics"></a> См. также  
 ...
 [Интерактивная сортировка, схемы документов и ссылки &#40;отчетов построителя отчетов и службы SSRS&#41;](interactive-sort-document-maps-and-links-report-builder-and-ssrs.md)  
