...
## <a name="using-the-globals-collection"></a>Использование коллекции Globals  
 `Globals` Коллекция содержит глобальные переменные отчета. В области конструктора эти переменные отображаются с префиксом & (амперсандом), например `[&ReportName]`. В следующей таблице описаны элементы `Globals` коллекции.  
  
|**Член**|**Тип**|**Описание**|  
|----------------|--------------|---------------------|  
|ExecutionTime|`DateTime`|Дата и время начала выполнения отчета.|  
|PageNumber|`Integer`|Номер текущей страницы относительно разрывов страниц, сбрасывающих номера страниц. В начале обработки отчета исходное значение устанавливается равным 1. Номер страницы наращивается для каждой страницы, подготовленной к просмотру.<br /><br /> Чтобы нумеровались в составе разрывов страниц для прямоугольника, области данных, группы областей данных или карты, для которого задано значение свойства, присвойте свойству ResetPageNumber значение `True`. Не поддерживается для иерархии групп столбцов табликса.<br /><br /> Свойство PageNumber может использоваться только в выражении в верхнем колонтитуле страницы или в нижнем колонтитуле страницы.|  
