---
title: Учебник. Создание детализированных и главных отчетов (построитель отчетов) | Документация Майкрософт
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "68892387"
---
# <a name="tutorial-creating-drillthrough-and-main-reports-report-builder"></a>Учебник. Создание детализированных и основных отчетов (построитель отчетов)
 ...
 **В детализированном отчете вы узнаете, как:**  
  
1.  [Создание отчета матрицы детализации и набора данных с помощью мастера таблицы или матрицы](#DMatrixAndDataset)  
  
    1.  [Укажите подключение к данным](#DConnection)  
  
    2.  [Создание запроса многомерных выражений](#DMDXQuery)  
  
    3.  [Настройка типа данных в группы](#DLayout)  
...
4.  [Добавление заголовка отчета с названием категории продуктов](#DReportTitle)  
  
5.  [Обновить свойства параметров](#DParameter)  
  
6.  [Сохранение отчета в библиотеке SharePoint](#DSave)  
  
 **В основном отчете вы узнаете, как выполнять следующие задачи:**  
  
1.  [Создание основного матричного отчета и набора данных с помощью мастера таблицы или матрицы](#MMatrixAndDataset)  
  
    1.  [Укажите подключение к данным](#MConnection)  
...
2.  [Удаление строки общего итога](#MGrandTotal)  
  
3.  [Действие "настроить текстовое поле" для детализации](#MDrillthrough)  
  
4.  [Замена числовых значений индикаторами](#MIndicators)  
  
5.  [Обновить свойства параметров](#MParameter)  
...
1.  Нажмите **кнопку Пуск**, укажите **пункт программы** [!INCLUDE[ssCurrentUI](../includes/sscurrentui-md.md)] , выберите пункт **Построитель отчетов**, а затем щелкните **Построитель отчетов**.  
...
##  <a name="DConnection"></a>SR1a. Задание подключения к данным  
...
##  <a name="DMDXQuery"></a>1b. Создание запроса многомерных выражений  
...
5.  В списке критериев фильтра разверните узел **Весь канал**, выберите **Online**, затем **Reseller**, после чего нажмите кнопку **ОК**.  
     ...
     Теперь запрос содержит фильтр, включающий только следующие каналы: В Интернете и у торгового посредника.  
...
##  <a name="DTotals"></a>1д. Добавление подытогов и итогов  
...
##  <a name="MConnection"></a>SR1a. Задание подключения к данным  
...
##  <a name="MMDXQuery"></a>1b. Создание запроса многомерных выражений  
...
##  <a name="MTotals"></a>1д. Добавление подытогов и итогов  
...
1.  На странице **Выбор макета** в области **Параметры**убедитесь в том, что выбран параметр **Показать подытоги и общие итоги** .  
     ...
     На панели просмотра в мастере отображается матрица с четырьмя строками.  При запуске отчета каждая строка отобразится следующим образом: Первая строка является группой столбцов, вторая строка содержит заголовки столбцов, третья строка содержит данные о категории продуктов (`[Sum(Net_ QTY)]` и `[Sum(Net_Sales)]`, а четвертая строка содержит итоговые значения).  
...
2.  Введите **2009 Категория продукта продажи: категории Online и Reseller**.  
