...
|`Cache Hits/Sec`|Количество запросов в секунду для кэшированных отчетов. Это запросы для повторно подготовленных к просмотру отчетов, а не для отчетов, обрабатываемых непосредственно из кэша. (См. в разделе `Total Cache Hits` далее в этом разделе.)|  
...
|`Reports Executed/Sec`|Число успешных выполнений отчетов в секунду. Этот счетчик предоставляет статистику объема отчетов. Используйте этот счетчик с `Request/Sec` Чтобы сравнить показатели выполнения отчетов и запросов, которые могут возвращаться из кэша.|  
...
|`Events/Sec`|Количество событий, обработанных за секунду. События, которые отслеживаются, включают `SnapshotUpdated` и `TimedSubscription`.|  
