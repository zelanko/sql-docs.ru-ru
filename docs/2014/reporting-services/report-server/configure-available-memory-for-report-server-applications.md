...
-   `MemorySafetyMargin` и `MemoryThreshold` указывают границы низкий, средний и высокий уровни нехватки свободной памяти. Для каждого состояния [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] предпринимают действие по исправлению, чтобы отчетов обработки и другие запросы обрабатываются надлежащим образом к общему объему памяти, доступной на компьютере. Можно указать параметры конфигурации, которые определяют разграничение между низким, средним и высоким уровнями нехватки свободной памяти.  
