...
## <a name="memory-management-policies"></a>Политики управления памятью  
 [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]реагирует на ограничения системных ресурсов, настраивая объем памяти, выделяемой конкретным приложениям и типам запросов на обработку. Приложения, которые выполняются в службе сервера отчетов и участвуют в управлении памятью.  
