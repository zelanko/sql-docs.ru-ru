...
##  <a name="bkmk_add_custom"></a> Добавление настроек пользовательской конфигурации для указания местоположения файла дампа  
 Можно добавить пользовательскую настройку для задания местоположения, которое программа «Доктор Средство "Доктор Ватсон" для Windows использует для хранения файла дампа. Пользовательское значение равно `Directory`. Следующий пример иллюстрирует, как эта настройка конфигурации указывается в разделе `RStrace`:  
