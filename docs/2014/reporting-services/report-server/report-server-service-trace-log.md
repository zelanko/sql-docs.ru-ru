...
##  <a name="bkmk_trace_configuration_settings"></a> Параметры конфигурации трассировки  
 ...
 Следующий пример иллюстрирует XML-структуру параметров `RStrace`. Значение для `DefaultTraceSwitch` определяет тип информации, добавляемой в журнал. За исключением `Components` атрибут, значения для `RStrace` одинаковы для всех файлов конфигурации.  
