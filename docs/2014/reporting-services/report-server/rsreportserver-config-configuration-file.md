...
-   [Extensions (файл RSReportServer.config) в основном режиме](#bkmk_extensions)  
    ...
    -   [Общая конфигурация модулей доставки](#bkmk_extensionsgeneral)  
        ...
        -   [Параметры конфигурации модуля доставки общей папки файла](#bkmk_fileshare_extension)  
...
>  Для режима интеграции с SharePoint резервирование URL-адресов настраивается в центре администрирования SharePoint. См. дополнительные сведения о [настройке альтернативного сопоставления доступа (http://technet.microsoft.com/library/cc263208(office.12).aspx)](http://technet.microsoft.com/library/cc263208\(office.12\).aspx).  
 ...
 `URLReservations` является необязательным элементом. Если он отсутствует в файле RSReportServer.config, возможно, сервер не настроен. Если он указан, должны присутствовать все дочерние элементы, за исключением `AccountName`.  
...
|**LogonMethod**|Это значение задает тип входа для `RSWindowsBasic`. Если указать `RSWindowsBasic`, это значение является обязательным. Допустимыми значениями являются 2 или 3, где каждое значение обозначает следующее.<br /><br /> `2` — Сетевой вход в систему высокопроизводительных серверов для проверки подлинности паролей с неформатированным текстом<br /><br /> `3` = Вход в систему открытым текстом, который сохраняет учетные данные для входа в пакете проверки подлинности, который отправляется с каждым запросом HTTP, что позволяет серверу олицетворять пользователя при соединении с другими серверами в сети.<br /><br /> Примечание. Значения 0 (для интерактивного входа) и 1 (для пакетного входа) в службах [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]не поддерживаются.|Нет|  
...
|**ReportBuilderTrustLevel**|Не изменяйте это значение, оно не настраивается. В [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] и более поздних версиях построитель отчетов запускается только в `FullTrust`. Дополнительные сведения см. в разделе [Настройка доступа к построителю отчетов](configure-report-builder-access.md) . Дополнительные сведения о прекращении поддержки режима частичного уровня доверия см. в разделе [неподдерживаемые возможности в SQL Server Reporting Services в SQL Server 2014](../discontinued-functionality-to-sql-server-reporting-services-in-sql-server.md).|N,S|  
...
|**SendEmailToUserAlias**|Это значение работает вместе с параметром **DefaultHostName**.<br /><br /> Когда **SendEmailToUserAlias** присваивается `True`, пользователи, определяющие частные подписки автоматически задаются в виде получателями отчета. Поле **Кому** скрыто. Если это значение равно `False`, **для** поле является видимым. Это значение равно `True` если хотите получить максимальный контроль над распределением отчетов. Допустимые значения.<br /><br /> `True`= Используется адрес электронной почты создающего подписку пользователя. Это значение по умолчанию.<br /><br /> `False` = можно задать любой адрес.|  
...
-   [Настройки сведений об устройстве HTML...] /.. / html устройств сведения settings.md)  
...
##  <a name="bkmk_MapTileServer"></a> MapTileServerConfiguration (файл RSReportServer.config)  
 `MapTileServerConfiguration` Определяет параметры конфигурации для [!INCLUDE[msCoName](../../includes/msconame-md.md)] веб-служб Bing Maps, обеспечивающих мозаичный фон для карты элемента отчета в отчете, публикуемом на сервере отчетов. Необходимы все дочерние элементы.  
