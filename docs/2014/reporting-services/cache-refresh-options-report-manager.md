...
-   **Имя** Указывает имя параметра запроса.  
  
-   **Тип** Указывает тип данных параметра запроса. Этот тип данных остается неизвестным до обработки поставщиком данных запроса к набору данных, поэтому проверка типа данных не может произойти, пока не будет выполнена обработка общего набора данных.  
  
-   **Допускает значения NULL** Указывает, является ли значение NULL допустимым.  
  
-   **Только для чтения** Указывает, помечен ли этот параметр как доступный только для чтения в определении общего набора данных. Параметры, доступные только для чтения, не появляются в списке параметров для параметров обновления кэша и должны иметь значение по умолчанию, указанное в составе определения общего набора данных.  
  
-   **DefaultValues** Значения по умолчанию, указанные в определении общего набора данных. Параметры запроса могут быть многозначными. Чтобы переопределить значения по умолчанию, введите новые значения в текстовых полях областей приглашения.  
