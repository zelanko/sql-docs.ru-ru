...
##  <a name="Percentages"></a>3. Отображение процентных значений в каждом срезе  
...
5.  Используемых Чтобы указать, сколько десятичных разрядов отображается в метке, введите `#PERCENT{Pn}` , где *n* — число десятичных разрядов для отображения. Например, чтобы не отображать десятичные разряды, `#PERCENT{P0}`введите.  
  
    > [!NOTE]  
    >  **Формат чисел** в диалоговом окне **Свойства метки ряда** не действует при форматировании процентов. Это форматирует метки в виде процентов, но не вычисляет процент, который представляет каждый срез круговой диаграммы.  
...
##  <a name="CombineSlices"></a>4. Объединение маленьких срезов в один срез  
...
5.  Присвойте свойству **присвойте свойству collectedstyle** значение **значение singleslice**.  
...
##  <a name="DrawingEffect"></a>5. Настройка эффектов рисования  
