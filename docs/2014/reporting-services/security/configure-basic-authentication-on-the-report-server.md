...
4.  Вставьте его поверх существующих элементов <`Authentication`>.  
     ...
     При использовании нескольких типов проверки подлинности добавьте только `RSWindowsBasic` элемент, но не удаляйте элементы для `RSWindowsNegotiate`, `RSWindowsNTLM`, или `RSWindowsKerberos`.  
  
     Если планируется поддержка браузера Safari, то настраивать сервер отчетов на использование нескольких типов проверки подлинности нельзя. Необходимо указать только `RSWindowsBasic` и удалив все остальные элементы.  
...
|-------------|--------------|------------------|  
|LogonMethod|Да<br /><br /> Если значение не указано, то будет использовано значение 3.|`2` = Сетевой вход в систему для высокопроизводительных серверов для проверки подлинности паролей в виде обычного текста.<br /><br /> `3` = Вход с открытым текстом, сохранив при этом учетные данные для входа в пакете проверки подлинности, которая отправляется при каждом запросе HTTP, что позволяет серверу олицетворять пользователя при соединении с другими серверами в сети. (по умолчанию)<br /><br /> Примечание. Значения 0 (для интерактивного входа) и 1 (для пакетного входа) в службах [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]не поддерживаются.|  
...
4.  Можно также создать файл Web.config для обработки запросов построителя отчетов с анонимной учетной записью.  
    ...
    ```  
     ...
     Режим проверки подлинности должно быть присвоено `Windows` при включении файла Web.config.  
  
     `Identity impersonate` может быть `True` или `False`.  
  
    -   Задайте для него значение `False` Если необходимо считывание токена безопасности приложений ASP.NET. Запрос будет выполняться в контексте безопасности службы сервера отчетов.  
  
    -   Задайте для него значение `True` Если необходимо считывание токена безопасности из уровня узла ASP.NET. При указании значения `True` необходимо также определить анонимную учетную запись, задав элементы `userName` и `password`. Указанные учетные данные определяют контекст безопасности, в котором формируются запросы.  
  
5.  Сохраните файл Web.config в папке ReportBuilder\bin.  
  
6.  Откройте файл RSReportServer.config, в разделе Services найдите `IsReportManagerEnabled` и добавьте следующий параметр ниже:  
