...
4.  Вставьте его на место существующих элементов для <`Authentication`>.  
     ...
     Если вы используете несколько типов проверки подлинности, добавьте только что `RSWindowsBasic` элемент, но не удаляйте элементы для `RSWindowsNegotiate`, `RSWindowsNTLM`, или `RSWindowsKerberos`.  
...
4.  Можно также создать файл Web.config для обработки запросов построителя отчетов с анонимной учетной записью.  
    ...
    ```  
     ...
     Режим проверки подлинности должно быть присвоено `Windows` Если включен файл Web.config.  
  
     `Identity impersonate` может быть `True` или `False`.  
  
    -   Задайте для него значение `False` Если вам не нужно считывание токена безопасности приложений ASP.NET. Запрос будет выполняться в контексте безопасности службы сервера отчетов.  
  
    -   Задайте для него значение `True` Если необходимо считывание токена безопасности на уровне узла ASP.NET. При указании значения `True` необходимо также определить анонимную учетную запись, задав элементы `userName` и `password`. Указанные учетные данные определяют контекст безопасности, в котором формируются запросы.  
