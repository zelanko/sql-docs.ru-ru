...
-   [!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)] Помощник по обновлению не убедитесь, что операционной системе или текущей установке [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] включенной поддержкой расширенной защиты.  
...
### <a name="what-reporting-services-extended-protection-does-not-cover"></a>Какие службы Reporting Services не охватываются расширенной защитой  
 Следующие области и сценарии не поддерживаются [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] функция расширенной защиты:  
  
-   Авторы [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] настраиваемые модули безопасности должны добавлять поддержку расширенной защиты для их настраиваемый модуль безопасности.  
  
-   Сторонние компоненты, добавляемое или используемые [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] установки должны обновляться по сторонних поставщиков, для поддержки расширенной защиты. Обратитесь к сторонним поставщикам за дополнительными сведениями.  
...
|--------------|----------------------|-------------------|  
|Прямая связь по SSL.<br /><br /> Сервер отчетов принудит клиента использовать привязку канала сервера отчетов.|![RS_ExtendedProtection_DirectSSL](../media/rs-extendedprotection-directssl.gif "RS_ExtendedProtection_DirectSSL")<br /><br /> 1) Клиентское приложение<br /><br /> 2) Сервер отчетов|Привязка службы не требуется, поскольку для привязки канала будет использоваться канал SSL.<br /><br /> Задайте `RSWindowsExtendedProtectionLevel` для `Allow` или `Require`.<br /><br /> Задайте `RSWindowsExtendedProtectionScenario` для `Direct`.|  
|Прямая связь по HTTP. Сервер отчетов принудит клиента использовать привязку службы сервера отчетов.|![RS_ExtendedProtection_Direct](../media/rs-extendedprotection-direct.gif "RS_ExtendedProtection_Direct")<br /><br /> 1) Клиентское приложение<br /><br /> 2) Сервер отчетов|Канал SSL отсутствует, поэтому введение в действие привязки канала невозможно.<br /><br /> Привязку службы можно проверить, однако такая защита привязки канала является неполной; одна привязка службы защищает только от простых угроз.<br /><br /> Задайте `RSWindowsExtendedProtectionLevel` для `Allow` или `Require`.<br /><br /> Задайте `RSWindowsExtendedProtectionScenario` для `Any`.|  
...
|Сценарий|Диаграмма сценария|Инструкции по защите|  
|--------------|----------------------|-------------------|  
|Непрямая связь по HTTP.<br /><br /> Шлюз принудит клиента использовать привязку канала шлюза. На пути к привязке службы сервера отчетов имеется шлюз.|![RS_ExtendedProtection_Indirect_SSL](../media/rs-extendedprotection-indirect-ssl.gif "RS_ExtendedProtection_Indirect_SSL")<br /><br /> 1) Клиентское приложение<br /><br /> 2) Сервер отчетов<br /><br /> 3) Шлюзовое устройство|Привязка канала от клиента к серверу отчетов невозможна, поскольку шлюз олицетворяет контекст, т. е. создает новый токен NTLM.<br /><br /> На промежутке от шлюза до сервера отчетов нет SSL, поэтому ввести в действие привязку канала невозможно.<br /><br /> Привязку службы ввести в действие можно.<br /><br /> Задайте `RSWindowsExtendedProtectionLevel` для `Allow` или `Require`.<br /><br /> Задайте `RSWindowsExtendedProtectionScenario` для `Any`.<br /><br /> Администратор должен настроить шлюзовое устройство на введение в действие привязки канала.|  
|Непрямая связь по HTTPS с защищенным шлюзом. Шлюз принудит клиента использовать привязку канала шлюза, а сервер отчетов принудит шлюз использовать привязку канала сервера отчетов.|![RS_ExtendedProtection_IndirectSSLandHTTPS](../media/rs-extendedprotection-indirectsslandhttps.gif "RS_ExtendedProtection_IndirectSSLandHTTPS")<br /><br /> 1) Клиентское приложение<br /><br /> 2) Сервер отчетов<br /><br /> 3) Шлюзовое устройство|Привязка канала от клиента к серверу отчетов невозможна, поскольку шлюз олицетворяет контекст, т. е. создает новый токен NTLM.<br /><br /> Наличие SSL на промежутке от шлюза до сервера отчетов означает, что можно ввести в действие привязку канала.<br /><br /> Привязка службы не требуется.<br /><br /> Задайте `RSWindowsExtendedProtectionLevel` для `Allow` или `Require`.<br /><br /> Задайте `RSWindowsExtendedProtectionScenario` для `Direct`.<br /><br /> Администратор должен настроить шлюзовое устройство на введение в действие привязки канала.|  
...
## <a name="service-binding-and-included-spns"></a>Привязка службы и включенные SPN  
 Привязка службы использует для проверки пункта назначения токенов проверки подлинности имена участников-служб (SPN). [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] использует сведения о существующем резервировании URL-адрес для построения списка SPN, которые считаются допустимыми. Благодаря использованию сведений резервирования URL-адреса для проверки и SPN и резервирования URL-адреса системные администраторы могут управлять обеими проверками из одного места.  
