...
3.  Чтобы назначить учетные записи пользователей и групп для конкретных ролей, используйте диспетчер отчетов. Дополнительные сведения смдолжно быть включено весьма небольшое число пользователей. в разделе Grant User Access to a Report Server &#40;Report Manager&#41;должно быть включено весьма небольшое число пользователей.  
