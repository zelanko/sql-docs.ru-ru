...
3.  Чтобы назначить учетные записи пользователей и групп для конкретных ролей, используйте диспетчер отчетов. Дополнительные сведения см. в разделе [предоставление пользователям доступа к серверу отчетов &#40;диспетчер отчетов&#41;](grant-user-access-to-a-report-server.md).  
  
##  <a name="bkmk_rolelist"></a>Стандартные определения ролей  
...
|---------------------|-----------|-----------------|  
|[Роль диспетчера содержимого](#bkmk_content)|Элемент|Содержит все задачи на уровне элементов. Пользователи, которым назначена эта роль, получают полный доступ к управлению содержимым сервера отчетов, в том числе права на предоставление разрешений другим пользователям, определение структуры папок для хранения отчетов и других элементов.|  
|[Роль издателя](#bkmk_publisher)|Элемент|Пользователи, которым назначена эта роль, имеют право добавлять элементы на сервер отчетов, в том числе право создавать папки, содержащие эти элементы, и управлять ими.|  
|[Роль браузера](#bkmk_browser)|Элемент|Пользователи, которым назначена эта роль, имеют право выполнять отчеты, подписываться на отчеты и перемещаться по структуре папок.|  
...
|[Роль системного администратора](#bkmk_systemadministrator)|Система|Пользователи, которым назначена эта роль, имеют право включать функции, задавать значения по умолчанию, определять безопасность в пределах веб-сайта, создавать определения ролей в среде Management Studio и управлять заданиями.|  
|[Роль "Системный пользователь"](#bkmk_systemuser)|Система|Пользователи, которым назначена эта роль, имеют право просматривать основные сведения о сервере отчетов, например сведения из общего расписания.|  
  
##  <a name="bkmk_content"></a>Роль диспетчера содержимого  
...
### <a name="content-manager-tasks"></a>Задачи диспетчера содержимого  
 В следующей таблице перечислены задачи, которые включены в роль **диспетчера содержимого** .  
...
|Модели (управление)|Создание, просмотр и удаление моделей, а также просмотр и изменение свойств моделей.|  
...
|Установка политики безопасности для элементов|Определение политики безопасности для отчетов, связанных отчетов, папок, ресурсов и источников данных. Дополнительные сведения см. в разделе [Защищаемые элементы](securable-items.md).|  
...
### <a name="customizing-the-content-manager-role"></a>Настройка роли «Диспетчер содержимого»  
 ...
 Роль **Диспетчер содержимого** используется в настройках безопасности по умолчанию. Дополнительные сведения см. в разделе [Using Default Security](role-definitions-predefined-roles.md).  
  
##  <a name="bkmk_publisher"></a>Роль издателя  
...
### <a name="publisher-tasks"></a>Задачи роли «Издатель»  
 В следующей таблице перечислены задачи, которые включены в роль **издателя** .  
...
##  <a name="bkmk_browser"></a>Роль браузера  
...
##  <a name="bkmk_myreports"></a>Роль Мои отчеты  
 ...
 Хотя можно избрать другую роль для использования возможности «Мои отчеты», рекомендуется выбирать ту, которая будет использоваться только для безопасности «Моих отчетов». Дополнительные сведения см. в разделе [Обеспечение безопасности "Моих отчетов"](secure-my-reports.md).  
  
### <a name="my-reports-tasks"></a>Задачи «Мои отчеты»  
 В следующей таблице перечислены задачи, которые включены в роль **Мои отчеты** .  
...
##  <a name="bkmk_systemadministrator"></a>Роль системного администратора  
...
### <a name="system-administrator-tasks"></a>Задачи системного администратора  
 В следующей таблице перечислены задачи, которые включены в роль **системного администратора** .  
  
|Задача|Description|  
|----------|-----------------|  
|Выполнение определений отчета|Запуск выполнения определения отчета без публикации на сервере отчетов.|  
|Управление заданиями|Просмотр и отмена работающих заданий. Дополнительные сведения см. в разделе [Управление запущенным процессом](../subscriptions/manage-a-running-process.md).|  
