---
title: Урок 8. Создание фильтра данных | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
6.  Нажмите кнопку выражения рядом с полем **значение** щелкните **параметры** в **категории** области, а затем дважды щелкните пункт `productid` в  **Значения** области. Поле **Задание выражения для Значение** теперь должно содержать выражение, аналогичное **= параметры! productid. Значение**.  
...
2.  Создайте новую функцию, которая принимает параметр `productid` типа Integer, возвращает объект `datatable` и выполняет следующие действия.  
    ...
    2.  Создание подключения к базе данных SqlServer для выполнения запроса, который определен в разделе **Занятие 4. Определение данных подключения и таблицы данных для дочернего отчета**.  
...
2.  Создайте новую функцию, которая принимает параметр `productid` типа Integer, возвращает объект `datatable` и выполняет следующие действия.  
    ...
    2.  Создание подключения к базе данных SqlServer для выполнения запроса, определенного в разделе **Занятие 4. Определение данных подключения и таблицы данных для дочернего отчета**.  
