---
title: Включение и отключение "песочницы" языка определения Отчетов | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
|`AllowNew`|Логический атрибут элемента **Allow**, указывающий, разрешено ли создание новых экземпляров этого типа в выражениях языка определения отчетов или в элементах языка определения отчетов **\<Class>**.<br /><br /> Примечание. Когда `RDLSandboxing` включено, новые массивы не могут создаваться в выражениях языка определения Отчетов, независимо от значения `AllowNew`.|  
...
|**Запретить**|Имя члена, запрещенного в выражении языка определения отчетов. Это свойство учитывает регистр символов.<br /><br /> Примечание. Если для члена указано **Deny**, то все члены всех типов с этим именем будут запрещены.|  
...
-   добавить этот новый класс к списку разрешений.  
 ...
 Чтобы добавить ключевые слова, определяющие тип [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, добавьте к списку разрешений соответствующий тип CLR. Например, чтобы использовать [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ключевое слово .NET Framework `Integer`, добавьте следующий фрагмент XML  **\<RDLSandboxing >** элемент:  
...
### <a name="working-with-operators-and-new"></a>Работа с операторами и оператором New  
 По умолчанию все операторы языка [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, кроме `New`, всегда разрешены. `New` Оператор управляется `AllowNew` атрибут  **\<разрешить >** элемент. Другие операторы языка, такие как оператор метода доступа коллекции по умолчанию `!` и [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] макросы приведения такие как .NET Framework `CInt`, всегда разрешены.  
