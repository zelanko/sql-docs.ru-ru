...
|`AllowNew`|Логический атрибут элемента **Allow**, указывающий, разрешено ли создание новых экземпляров этого типа в выражениях языка определения отчетов или в элементах языка определения отчетов **\<Class>**.<br /><br /> Примечание: Если `RDLSandboxing` включен, новые массивы не могут создаваться в выражениях языка определения Отчетов, независимо от значения `AllowNew`.|  
...
-   добавить этот новый класс к списку разрешений.  
 ...
 Чтобы добавить [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] функции платформы .NET Framework к списку разрешений, добавьте соответствующие типы из пространства имен Microsoft.VisualBasic к списку разрешений.  
  
 Чтобы добавить ключевые слова, определяющие тип [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, добавьте к списку разрешений соответствующий тип CLR. Например, чтобы использовать [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ключевое слово .NET Framework `Integer`, добавьте следующий фрагмент XML для  **\<RDLSandboxing >** элемента:  
...
### <a name="working-with-operators-and-new"></a>Работа с операторами и оператором New  
 По умолчанию все операторы языка [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, кроме `New`, всегда разрешены. `New` Оператор управляется `AllowNew` атрибут  **\<разрешить >** элемента. Другие операторы языка, например оператор доступа к коллекции по умолчанию `!` и [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework приведение макросов, таких как `CInt`, всегда разрешены.  
