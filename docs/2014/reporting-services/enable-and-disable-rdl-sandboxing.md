...
|`AllowNew`|Логический атрибут элемента **Allow**, указывающий, разрешено ли создание новых экземпляров этого типа в выражениях языка определения отчетов или в элементах языка определения отчетов **\<Class>**.<br /><br /> Примечание: При `RDLSandboxing` включено, новые массивы не могут создаваться в выражениях языка определения Отчетов, независимо от значения `AllowNew`.|  
...
-   добавить этот новый класс к списку разрешений.  
 ...
 Чтобы добавить [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] функции .NET Framework к списку разрешений, добавьте соответствующие типы из пространства имен Microsoft.VisualBasic к списку разрешений.  
