...
## <a name="remarks"></a>Примечания  
 ...
 Если *IsWindowsUser* равно `true`, *UserName* должно быть в формате \<домена >\\< имя пользователя\>.  
  
 При *IsWindowsUser* задано значение `true`, то созданный скрипт предоставляет права входа в систему пользователю для [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], параметр базы данных сервера отчетов в качестве базы данных по умолчанию и предоставляет **RSExec** роли в базе данных сервера отчетов, временная база данных сервера отчетов, базы данных master и системной базы данных MSDB.  
  
 Когда *IsWindowsUser* равен `true`, метод принимает стандартные идентификаторы безопасности Windows в качестве входного. Если предоставлен стандартный идентификатор безопасности Windows или имя учетной записи службы, то они переводятся в строку имени пользователя. Если база данных локальная, то учетная запись переводится в соответствии с представлением текущего языкового стандарта учетной записи. Если база данных удаленная, то учетная запись представляется как учетная запись компьютера.  
...
|NT AUTHORITY\LocalService|Локальная служба.|Ошибка — см. ниже.|  
 ...
 Если `LocalService` указана встроенная учетная запись и базы данных сервера отчетов является удаленной, возвращается сообщение об ошибке.  
...
|\<IP-адрес>|180.012.345,678|  
 ...
 Когда *IsWindowsUser* задано значение `true`, поставщик WMI вызывает LookupAccountName для получения идентификатора безопасности для учетной записи, а затем вызывает LookupAccountSID, чтобы получить имя для вставки в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] сценария. Это гарантирует, что имя учетной записи успешно пройдет проверку [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .  
  
 Когда *IsWindowsUser* равно `false`, созданный скрипт предоставляет **RSExec** роли в базе данных сервера отчетов, временная база данных сервера отчетов и базы данных MSDB.  
  
 Когда *IsWindowsUser* задано значение `false`, пользователь SQL Server должен уже существовать на [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] для успешного выполнения сценария.  
