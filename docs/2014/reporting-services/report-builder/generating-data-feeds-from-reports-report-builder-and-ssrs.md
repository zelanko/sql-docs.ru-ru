...
# <a name="generating-data-feeds-from-reports-report-builder-and-ssrs"></a>Формирование веб-каналов данных из отчетов (построитель отчетов и службы SSRS)
  [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] Модуль подготовки отчетов Atom создает Сервисный документ Atom, в котором перечислены потоки данных, доступные из отчета и данных веб-каналы данных областей в отчете. Этот модуль используется для формирования совместимых с Atom потоков данных, которые допускают чтение и обмен данными с приложениями, которые потребляют потоки данных, формируемые отчетами. Модуль подготовки отчетов Atom можно применить, например, для формирования веб-каналов данных, которые будут использоваться клиентом [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] .  
  
 Сервисный документ Atom содержит сведения по меньшей мере об одном потоке данных для каждой области данных отчета. В зависимости от типа области данных и данных, область данных отображает [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] может создавать несколько потоков данных из области данных. Несколько потоков данных может породить, к примеру, матрица или диаграмма. Когда модуль подготовки отчетов Atom создает сервисный документ Atom, для каждого потока данных создается уникальный идентификатор, который упоминается в URL-адресе для доступа к содержимому потока данных.  
...
##  <a name="DataFeeds"></a> Потоки данных  
 Поток данных представляет собой XML-файл в согласованном табличном формате, который не меняется со временем, и содержит переменные данные, которые могут изменяться при каждом выполнении отчета. Потоки данных, формируемые [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] находятся в том же формате, созданные этой службы ADO.NET Data Services.  
...
### <a name="data-section"></a>Раздел данных  
 ...
 Следующий XML-КОДЕ показано <`entry`> из этого отчета в поток данных. Обратите внимание, что <`entry`> включает итоговые значения по продажам и заказам для группы и итоговые значения по продажам и заказам для всех групп. Элемент <`entry`> включает все значения отчета.  
...
### <a name="working-with-data-feeds"></a>Работа с потоками данных  
 Все потоки данных, формируемые отчетом, включают элементы отчета, находящиеся в области видимости родительского элемента области данных, породившей потоки данных. , и делает это по-другому. Представьте, что отчет содержит несколько таблиц и одну диаграмму. Текстовые поля в теле отчета содержат описания каждой области данных. Каждая запись в каждом потоке данных, формируемом отчетом, включает значение такого текстового поля. Например, если задан текст «Диаграмма отображает средние ежемесячные продажи по региону продаж», то этот текст будет включен в каждую строку всех трех потоков данных.  
