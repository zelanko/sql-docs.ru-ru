...
ms.locfileid: "66107608"
---
# <a name="security-report-builder"></a>Безопасность (построитель отчетов)
  Построитель отчетов — это клиентское приложение для создания отчетов, предназначенное для работы [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] с сервером отчетов. Сервер отчетов может быть настроен для работы в собственном режиме как автономный сервер или для работы в режиме интеграции с SharePoint для поддержки отчетов на сайте SharePoint.  
...
-   **Сервер отчетов или сайт SharePoint, на котором публикуются отчеты** Управление этими функциями осуществляется администратором сервера отчетов или администратором сайта SharePoint.  
  
-   **Опубликованные отчеты и связанные с отчетами элементы** Элементы, связанные с отчетами, включают внедренные и общие источники данных, а также их учетные данные, общие наборы, параметры, элементы отчетов и модели отчетов. Средства безопасности, применимые к данным элементам, управляются создателем отчета. Создатель отчета должен иметь достаточные разрешения от администратора сервера отчетов или сайта SharePoint для публикации и совместного использования элементов.  
  
-   **Внешние источники данных, используемые отчетом** Управление этими функциями осуществляется владельцем внешнего источника данных.  
  
-   **Модели отчетов, основанные на внешних источниках данных** Управление этими функциями осуществляется конструктором моделей.  
  
-   **Интерактивные функции отчетов, такие как параметры** Эти функции управляются автором отчета.  
...
##  <a name="Reports"></a>Общие сведения о безопасности опубликованных отчетов и связанных с отчетами элементов  
...
### <a name="update-notifications-for-report-parts"></a>Уведомления об обновлении для элементов отчетов  
 ...
 Дополнительные сведения см. в разделе [Report parts &#40;построитель отчетов and SSRS&#41;](../report-parts-report-builder-and-ssrs.md).  
...
##  <a name="Models"></a>Основные сведения о моделях и фильтрах безопасности  
...
##  <a name="Interactive"></a>Общие сведения о безопасности при создании отчетов для интерактивных функций  
