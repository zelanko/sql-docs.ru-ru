...
18. Чтобы просмотреть настоящую круговую диаграмму, нажмите кнопку **Выполнить** на вкладке **Главная** ленты.
 ...
 ![Значок стрелки, используемый со ссылкой "К началу"](../../2014-toc/media/uparrow16x16.gif "Значок стрелки, используемый со ссылкой В начало") [К началу](#TwoWays)

##  <a name="CreatePieQueryData"></a>Создание круговой диаграммы с помощью [!INCLUDE[tsql](../../../includes/tsql-md.md)] запроса
...
3.  Введите `#PERCENT{P0}` для параметра **данных метки** .
     ...
     Значение `{P0}` дает процент без десятичных разрядов. Если просто ввести `#PERCENT`, числа будут иметь два десятичных разряда. 
  `#PERCENT` является ключевым словом, с помощью которого выполняются вычисления; имеются и многие другие ключевые слова.
