...
##  <a name="CreateTable"></a>1. Создание отчета с таблицей  
...
8.  В поле **Строка подключения**введите следующий текст:  
     ...
     **Источник данных =\<ServerName>**  
     ...
     Выражение \<ServerName>, например Report001, указывает компьютер, на котором установлен экземпляр ядро СУБД SQL Server. Поскольку данные отчета не извлекаются из базы данных SQL Server, не нужно указывать имя базы данных. Для синтаксического анализа запроса используется база данных по умолчанию на указанном сервере.  
...
4.  На панели инструментов конструктора запросов нажмите кнопку выполнить (**!**).  
     ...
     Запрос выполняется и отображает результирующий набор для полей **SalesDate**, **Подкатегория**, **продукт**, **продажи**и **количество**.  
  
5.  Щелкните **Далее**.  
  
6.  На странице **Размещение полей** перетащите поле **Sales** в область **Значения**.  
  
     Данные о **продажах** суммируются функцией Sum. Значение равно [Sum(Sales)].  
...
4.  Нажмите кнопку **Выполнить** для предварительного просмотра отчета.  
 ...
 Сводные значения для **Sales** отображаются в виде валюты.  
  
##  <a name="FormatDates"></a>6. (необязательно) форматирование данных в виде дат  
...
##  <a name="Width"></a>7. изменение ширины столбцов (необязательно)  
...
3.  Установите указатель на линии раздела между маркерами столбцов, чтобы курсор принял вид двойной стрелки. Перетаскиванием установите нужный размер столбцов. Например, разверните столбец **Product** , чтобы название продукта отображалось в одной строке.  
