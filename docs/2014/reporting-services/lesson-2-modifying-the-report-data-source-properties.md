...
7.  Введите имя пользователя (в виде *домен\имя*) и пароль. Если у вас нет разрешения на доступ к [!INCLUDE[ssSampleDBobject](../includes/sssampledbobject-md.md)] базы данных, указать имя входа, который выполняет.  
...
11. Просмотрите отчет, чтобы убедиться, что он выполняется с указанными учетными данными. Чтобы просмотреть отчет, щелкните вкладку **Просмотр** . Обратите внимание, что после открытия отчета необходимо выбрать имя сотрудника и нажмите кнопку **отчет** кнопку, чтобы просмотреть отчет.  
