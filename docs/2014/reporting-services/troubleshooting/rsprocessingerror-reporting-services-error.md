...
-   `Previous`или `RunningValue` `RowNumber` агрегатные функции, используемые в ячейках * \<данных типа элемента отчета>* "*\<имя элемента отчета>*" ссылаются на группирование областей как в столбцах, так и в строках * \<типа элемента отчета>*. `Previous`Параметры области действия All `RunningValue` и `RowNumber` агрегатные функции в * \<типе элемента отчета>* могут ссылаться на группирование строк или Группирование столбцов данных, но не в обе.  
