...
||  
|-|  
|[!INCLUDE[applies](../includes/applies-md.md)]<br /><br /> 
  [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] Режим SharePoint | SharePoint 2010 и SharePoint 2013<br /><br /> [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]Собственный режим|  
  
 **В этом разделе:**  
  
-   [Сравнение средств и терминологии разрешений](#bkmk_compare_tools_terms)  
...
##  <a name="bkmk_compare_tools_terms"></a>Сравнение средств и терминологии разрешений  
 **Основной режим:** Объекты [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] разрешений в собственном режиме (роли и задачи) создаются в [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] и настраиваются для отдельных пользователей в диспетчер отчетов.  
  
 **Режим** [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] SharePoint. в режиме интеграции с SharePoint используются функции разрешений SharePoint. Управление группами и разрешениями SharePoint осуществляется из следующей страницы **Параметры сайта** .  
...
|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]Собственный режим|SharePoint|  
|---------------------------------------------|----------------|  
|**Роль:** Например, "Диспетчер содержимого".|**Группа:** Например, Группа "зрители" по умолчанию.|  
|---|**Группа уровня разрешений:** Например, "только просмотр" для группы "Читатели".|  
|**Задачи:** например "Управление отчетами".|**Разрешения:** Например, в группе "только просмотр" перечислены разрешения, связанные с элементами представления, версиями представления и страницами приложения.|  
 ...
 **Примечание**. Доступные группы SharePoint по умолчанию зависят от шаблона сайта, используемого для создания сайта SharePoint.  
  
|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]Role|Группы SharePoint|  
|--------------------------------------|-----------------------|  
...
|**Мои отчеты**|Эквивалентной группы нет. **Мои отчеты** не поддерживается для сервера отчетов, работающего в режиме интеграции с SharePoint. Если необходима эквивалентная функциональность, можно воспользоваться функциями «Мой сайт» служб [!INCLUDE[winSPServ](../includes/winspserv-md.md)] .|  
...
|-|Используйте группу **Наблюдатели** для предоставления разрешений на просмотр отчетов, готовых для просмотра. Группа **Наблюдатели** не может загружать или просматривать содержимое элементов отчетов.<br /><br /> **Примечание.** Начиная с SQL Server 2012 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)], Группа " **читатели** " не имеет разрешений на создание подписок.|  
|**Системный пользователь** и **системный администратор**|Эти роли не обязательны для сервера отчетов, работающего в режиме интеграции с SharePoint. **Системный пользователь** и **системный администратор** соответствуют разрешениям фермы SharePoint или уровня веб-приложения. Сервер отчетов не предоставляет никакой функциональности, требующей авторизации на этом уровне.|  
...
|Управление всеми подписками|Элемент|Управление оповещениями.|  
...
|Модели (управление)|Элемент|Добавление, изменение, удаление и просмотр элементов.|  
