...
## <a name="initial-configuration"></a>Первоначальная конфигурация  
 Задание учетной записи службы сервера отчетов производится во время установки. Можно запустить службу в учетную запись пользователя домена или встроенную, такие как `NetworkService` учетной записи. Нет учетной записи по умолчанию; независимо от учетной записи, указанной в [конфигурация сервера — учетные записи служб](../../sql-server/install/server-configuration-service-accounts.md) страница мастера установки становится начальной учетной записью службы сервера отчетов.  
  
> [!IMPORTANT]  
>  Несмотря на то, что сервер веб-службы отчетов и диспетчер отчетов являются [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] приложения, они не выполняются под [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] учетной записи. Единая архитектура служб производит запуск обоих приложений ASP.NET с удостоверением процесса сервера отчетов. В этом заключается важное отличие от предыдущих версий, где и веб-служба сервера отчетов, и диспетчер отчетов запускались с удостоверением рабочего процесса [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)], заданного для служб IIS.  
...
-   Изменение [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] учетной записи (например, переключение с NetworkService на учетную запись пользователя домена) службы.  
