...
|Установите SharePoint Server 2010 с пакетом обновления 1 (SP1).|Если на существующей ферме SharePoint есть SharePoint 2010 SP1 установлены Загрузите и установите SharePoint 2010 SP1 из:[http://support.microsoft.com/kb/2460045](http://go.microsoft.com/fwlink/p/?linkID=219697).<br /><br /> Дополнительные сведения о SharePoint 2010 с пакетом обновления 1 (SP1) см. в разделе [Известные проблемы при установке Office 2010 с пакетом обновления 1 (SP1) и SharePoint 2010 с пакетом обновления 1 (SP1)](http://support.microsoft.com/kb/2532126).|  
|Запустите мастер настройки продуктов SharePoint, чтобы добавить сервер к ферме.|(1) в **продукты Microsoft SharePoint 2010** группы программ, нажмите кнопку **мастер настройки продуктов Microsoft SharePoint 2010**.<br /><br /> 2) на **подключиться к ферме серверов** выберите **подключиться к существующей ферме** и нажмите кнопку **Далее**.<br /><br /> 3) на **параметры конфигурации базы данных** введите имя сервера базы данных в существующей ферме и имя базы данных конфигурации. Нажмите кнопку **Далее**.<br />**\*\* Важные \* \***  Если вы видите следующее сообщение об ошибке и убедившись в наличии разрешений, затем протоколы проверьте, включены ли сетевая конфигурация SQL Server в **Sql Server Configuration Manager**: «не удалось подключиться к серверу базы данных. Убедитесь в базе данных существует, Sql Server и наличие разрешения на доступ к серверу».<br />**\*\* Важные \* \***  Если отображается страница **состояние сервера фермы продуктов и обновлений**, необходимо будет проверить сведения на странице и обновить на сервере необходимые файлы, прежде чем продолжить с помощью присоединения сервера к ферме.<br /><br /> 4) на **задание параметров безопасности фермы** введите парольную фразу фермы и нажмите кнопку **Далее**. Нажмите кнопку **Далее** на странице подтверждения, чтобы запустить мастер.<br /><br /> 5) нажмите кнопку **Далее** для запуска **мастер настройки фермы**.|  
|Проверьте добавление сервера к ферме SharePoint.|1) В Центре администрирования SharePoint в группе **Параметры системы** выберите пункт **Управление серверами в этой ферме** .<br /><br /> 2) Убедитесь, что новый сервер добавлен и имеет правильное состояние.<br /><br /> (3) Примечание, служба не отображается **службы SQL Server Reporting Services** под управлением. Она будет установлена на следующем шаге.<br /><br /> (4) чтобы исключить сервер из роли WFE, щелкните **управление службами на сервере** и остановить службу **веб-приложение Microsoft SharePoint Foundation**.|  
|Установите и настройте службы Reporting Services в режиме интеграции с SharePoint.|Запустите установку [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] . Дополнительные сведения по установке [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] режиме интеграции с SharePoint в разделе [установить службы Reporting Services в режиме SharePoint для SharePoint 2010](../../../2014/sql-server/install/install-reporting-services-sharepoint-mode-for-sharepoint-2010.md) Если сервер будет использоваться только как сервер приложений и сервер не будет использоваться как WFE, необязательно для выбора **надстройки служб Reporting Services для продуктов SharePoint** на:<br /><br /> **роль установки** выберите **Установка компонентов SQL Server**<br /><br /> **Выбор компонентов** выберите **Reporting Services — SharePoint**<br /><br /> -или-<br /><br /> **настройки служб Reporting Services** проверки страниц **только установка** выбран параметр **Reporting Services в режиме SharePoint**.|  
...
##  <a name="bkmk_additional"></a> Дополнительная настройка  
 ...
 Веб-службы Reporting Services не включены. Настройте хотя бы один экземпляр служб Reporting Services SharePoint иметь \<IsWebServiceEnable > задано значение true. Дополнительные сведения см. в статье [Изменение файла конфигурации служб Reporting Services (RSreportserver.config)](../report-server/modify-a-reporting-services-configuration-file-rsreportserver-config.md)  
  
## <a name="see-also"></a>См. также  
 [Добавление web или сервера приложений к фермам в SharePoint 2013](http://technet.microsoft.com/library/cc261752.aspx)   
