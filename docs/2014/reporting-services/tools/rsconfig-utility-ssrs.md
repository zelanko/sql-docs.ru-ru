...
|`-m`  *Имя компьютера*|Обязателен, если настраивается удаленный экземпляр сервера отчетов.|Указывает имя компьютера, на котором размещается сервер отчетов. Если этот аргумент указан, значение по умолчанию — `localhost`.|  
