...
-   SOURCE_URL и TARGET_URL должны быть URL-адреса отчетов серверов, которые указывают на исходном и целевом [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] сервера отчетов. В собственном режиме URL-адрес сервера отчетов выглядит следующим образом:  
