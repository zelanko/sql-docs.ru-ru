...
1.  [Создание табличного отчета и набор данных из мастера таблицы или матрицы](#Table)  
...
##  <a name="Table"></a> 1. Создание табличного отчета и набора данных с помощью мастера таблицы или матрицы  
 Из **Приступая к работе** диалоговое окно, выберите общий источник данных, создайте внедренный набор данных и отображение данных в виде таблицы.  
...
1.  В таблице, щелкните правой кнопкой мыши две ячейки вниз от `[Sum(Sales)]` ячейки (промежуточных итогов, отображаются продажи для подкатегории), а затем щелкните **свойства текстового поля**.  
  
2.  В **заполнения**, нажмите кнопку **fx** рядом с **цвет заливки** и введите следующее выражение в **задать выражение для: BackgroundColor** поля:  
...
2.  В таблице, щелкните правой кнопкой мыши столбец обработчик для ячейки, измененной в предыдущей процедуре, пункты **вставить столбец**и нажмите кнопку **право**. В таблицу добавится новый столбец.  
  
3.  Тип **ключевого показателя Эффективности** в заголовке столбца.  
  
4.  На **вставить** вкладке **областей данных** щелкните **датчика**и затем щелкните область конструктора вне таблицы. Откроется диалоговое окно **Выбор типа датчика** .  
...
10. В **тип указателя**выберите **панель**. Указатель будет изменен с маркера на черту, это сделает его более видимым, когда в таблицу добавится датчик.  
  
11. Нажмите кнопку **заливка указателя**. В **вторичный цвет** выбора **желтый**. Шаблон градиента заливки изменится с белого на желтый.  
  
12. Щелкните правой кнопкой мыши шкалу датчика и выберите пункт **Свойства шкалы**.  
  
13. Задать **максимальное** значение 25 000.  
...
16. (Дополнительно) Добавьте штифт-ограничитель для управления переполнением, чтобы любое значение, превосходящее максимальное значение шкалы, всегда указывало на штифт-ограничитель.  
    ...
    3.  В **ограничители шкалы** категории, разверните **MaximumPin** узла.  
  
    4.  Задать **включить** свойства `True`. Ограничитель появляется за максимальным значением на шкале.  
  
    5.  Задать **BorderColor** для `Lime`.  
...
5.  На **вставить** вкладке **областей данных** последовательно дважды щелкните **индикатора.**  
  
     Откроется диалоговое окно **Выбор стиля индикатора** .  
  
6.  Нажмите кнопку **фигуры**. Первый тип фигуры **3 значка светофора (без Ободка)** выбран.  
...
10. Выберите в раскрывающемся списке значение **[Sum(Sales)]**, но не изменяйте другие параметры.  
...
2.  Тип **ключевой показатель Эффективности продаж продукта**, а затем щелкните вне текстового поля.  
