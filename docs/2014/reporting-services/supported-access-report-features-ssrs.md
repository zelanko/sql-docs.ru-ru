---
title: Поддерживаемые функции отчетов Access (SSRS) | Документы Microsoft
ms.custom: ''
ms.date: 03/07/2017
...
## <a name="importing-access-reports"></a>Импорт отчетов Access  
 ...
 Процесс импорта неправильно передаются точка с запятой (;) и символы разметки XML (\<, >, т. д.) в строке подключения. Если строка соединения содержит такой символ, то необходимо вручную задать пароль в новом отчете после импорта.  
...
## <a name="reports-with-custom-modules"></a>Отчеты с пользовательскими модулями  
 Если имеется пользовательский [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] кода, содержащегося в модулях, он не преобразуется. Если во время импорта конструктор отчетов обнаружит такой код, предупреждение создается и отображается в **список задач** окна.  
