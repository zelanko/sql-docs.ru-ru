...
##  <a name="Query"></a>3. Добавление параметра запроса для создания параметра отчета  
...
3.  Добавьте следующее [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` предложение в качестве последней строки запроса:  
    ...
    ```  
     ...
     `WHERE` Предложение ограничивает извлеченные данные идентификатором магазина, указанным в параметре *@StoreID*запроса.  
...
8.  В области данных отчета разверните папку **Параметры** .  
 ...
 Обратите внимание, что теперь имеется параметр отчета *@StoreID*с именем. По умолчанию он имеет тип данных **Text**. Поскольку идентификатор магазина является целым числом, в следующей процедуре тип данных будет изменен на Integer.  
  
##  <a name="ChangeDefaultProperties"></a>4. изменение типа данных по умолчанию и других свойств для параметра отчета  
...
6.  Просмотрите отчет. В средстве просмотра отчетов отобразится *@StoreID*запрос.  
...
9. Просмотрите отчет.  
     ...
     В панели инструментов средства просмотра отчетов текстовое поле параметра теперь содержит раскрывающийся список ** \<выберите значение>**.  
...
5.  Убедитесь ****, что `equals` в списке Оператор выбрано значение (=).  
  
6.  В разделе **Значение** введите `[@StoreID]`. 
...
##  <a name="Boolean"></a>8. Добавление логического параметра для условной видимости  
...
3.  Выберите или введите имя сервера отчетов, для которого существует разрешение на сохранение отчетов.  
     ...
     Появится сообщение, **подключающегося к серверу отчетов**. После того как соединение установлено, пользователю представляется содержимое папки, заданной администратором сервера отчетов как место по умолчанию для отчетов.  
