...
1.  [Создание матричного отчета и набор данных из мастера таблицы или матрицы](#Setup)  
  
2.  [Организация данных, Выбор макета и стиля в мастере таблицы или матрицы](#CompleteWizard)  
  
3.  [Добавить параметр запроса, чтобы создать параметр отчета](#Query)  
...
4.  [Изменение типа данных по умолчанию и другие свойства параметра отчета](#ChangeDefaultProperties)  
    ...
    4.  [Найти значение из набора данных, который содержит пары имя-значение](#NameValue)  
  
5.  [Отображение значения выбранного параметра в отчет](#Expression)  
...
11. На панели инструментов конструктора запросов нажмите кнопку **Выполнить** (**!**). Результирующий набор содержит 11 строк данных, отображающих число продаж для каждой подкатегории для четырех магазинов, а также содержит следующие столбцы: StoreID, Subcategory и Quantity.  
...
3.  Добавьте следующие [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` предложение в последней строке в запросе:  
  
    ```  
    WHERE StoreID = (@StoreID)  
    ```  
  
     `WHERE` Предложение WHERE ограничивает запрашиваемые данные идентификатором магазина, указанного в параметре запроса *@StoreID*.  
...
8.  В области данных отчета разверните папку **Параметры** .  
 ...
 Обратите внимание, что появился параметр отчета с именем *@StoreID*. По умолчанию он имеет тип данных **Text**. Поскольку идентификатор магазина является целым числом, в следующей процедуре тип данных будет изменен на Integer.  
...
1.  В области данных отчета в разделе **параметры** узла, щелкните правой кнопкой мыши *@StoreID*, а затем нажмите кнопку **свойства параметра**.  
...
##  <a name="AvailableValues"></a> 4б. Задание доступных значений для создания раскрывающегося списка значений  
...
9. Просмотрите отчет.  
     ...
     В панели инструментов средства просмотра отчетов, текстовым полем параметра является теперь отображаются в списке  **\<выберите значение >**.  
...
##  <a name="DefaultValues"></a> 4-х ядер. Задание значений по умолчанию для автоматического выполнения отчета  
...
6.  Изменение `equals` (=) для `IN` в [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` предложения в последней строке запроса:  
...
8.  На панели инструментов средства просмотра отчетов рядом с **Показать выбранные**, нажмите кнопку `True`.  
