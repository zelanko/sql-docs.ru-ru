---
title: Страница «Создание управляемой данными подписки» (диспетчер отчетов) | Документы Microsoft
ms.custom: ''
ms.date: 05/24/2017
 ...
 **Путь**  
 Задает общую папку, доступную через сетевое подключение. Чтобы убедиться, что папка доступна, нажмите кнопку **запуска** меню «Пуск» и введите путь к папке в следующем формате: \\ \\< имя_компьютера\>\\< имя_общей_папки\>.  
 ...
 **Имя пользователя**  
 Введите учетную запись пользователя домена, имеющую разрешение на добавление файлов в общей папке в следующем формате: \<домена >\\< имя_пользователя\>.  
...
|**Использовать значения по умолчанию**|В некоторых отчетах для всех или некоторых параметров содержатся значения по умолчанию. Если для параметра отчета имеется значение по умолчанию, установите этот флажок, чтобы использовать его.|  
