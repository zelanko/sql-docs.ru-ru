---
title: Настройка веб-части средства просмотра отчетов | Документация Майкрософт
ms.custom: ''
ms.date: 06/13/2017
...
|Расположение панели инструментов|Определяет расположение панели инструментов в средстве просмотра отчетов. Допустимые значения: `Top` и `Bottom`.|  
|Область приглашений|Допустимые значения: `Displayed`, `Collapsed`, и `Hidden`. `Displayed` Отображает область параметров для отчетов с параметризованными значениями, которые пользователь должен ввести до выполнения отчета. Используйте `Hidden` Если все параметры отчета заданы и область параметров должен отображаться для пользователей не требуется.|  
