...
|`UniqueName`|`String`|Возвращает полное имя уровня. Например `UniqueName` значение для сотрудника может быть *[Сотрудник]. [ Сотрудник отдела]. [Отдел]. & [Sales]. & [североамериканский менеджер по продажам]. & [272]*.|  
...
|`FormattedValue`|`String`|Возвращает форматированное значение для меры или ключевую цифру. Например `FormattedValue` свойство **Sales Amount Quota** Возвращает формат денежной единицы, такие как 1 124 400,00 р.|  
...
|FONT_FLAGS|(null)|  
 ...
 Если с помощью этого запроса создать набор данных для отчета и привязать его к таблице, то будет доступно свойство поля по умолчанию VALUE, например `=Fields!Month_of_Year!Value`. Если задать это выражение в качестве выражения сортировки для таблицы, результаты будут для сортировки таблицы в алфавитном порядке по месяцам, поскольку поле Value имеет `String` тип данных. Для правильной сортировки таблицы по месяцам с января по декабрь воспользуйтесь следующим выражением.  
