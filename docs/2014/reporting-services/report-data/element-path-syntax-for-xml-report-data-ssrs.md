...
|`Value`|Значение элемента. `Value` может использоваться для последнего `ElementNode` в пути к элементу. Например, поскольку \<Return> является конечным узлом, если он включен в конец пути к элементу, значение `Return {@}` равно. `Chair`|  
