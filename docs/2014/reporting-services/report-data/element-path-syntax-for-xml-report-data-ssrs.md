...
> [!NOTE]  
>  Синтаксис пути к элементу не зависит от пространства имен. Для указания пространств имен в пути к элементу, используйте синтаксис запроса XML, который содержит XML- `ElementPath` описанному [синтаксис запроса XML для XML-данных отчета &#40;SSRS&#41;](report-data-ssrs.md).  
...
|`Value`|Значение элемента. `Value` может использоваться для последнего `ElementNode` в пути к элементу. Например так как \<возвращают > является конечным узлом, в том случае, если вы включите его в конце пути к элементу, значение `Return {@}` является `Chair`.|  
...
|`NamespacePrefix`|`NamespacePrefix` определяется в элементе XML-запроса. Если элемента XML-запроса не существует, пространства имен в XML- `ElementPath` игнорируются. Если элемент XML-запроса существует, XML `ElementPath` имеет необязательный атрибут `IgnoreNamespaces`. Если IgnoreNamespaces имеет `true`, пространства имен в XML- `ElementPath` и XML-документ учитываются. Дополнительные сведения см. в разделе [Синтаксис запроса XML для XML-данных отчета &#40;SSRS&#41;](report-data-ssrs.md).|  
