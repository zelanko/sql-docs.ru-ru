...
|`Attribute`|Пара «имя-значение» в `ElementNode`. Например, в узле элемента \<Customer ID = «1» >, `ID` является атрибутом и `@ID(Integer)` возвращает «1» как целочисленный тип в соответствующем поле данных `ID`.|  
