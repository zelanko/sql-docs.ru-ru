...
-   Если выбран параметр для многозначных параметров, необходимо вручную изменить запрос для проверки на равенство значений в наборе с помощью `IN` оператор, например, `WHERE EmployeeID IN (@EmpID)`.  
