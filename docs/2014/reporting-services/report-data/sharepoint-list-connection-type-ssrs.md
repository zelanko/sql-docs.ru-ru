 ...
 **Таблица 1** предназначена для отчетов, развернутых на устаревшем сайте Windows SharePoint. Устаревший сайт Windows поддерживает только проверку подлинности Kerberos, NTLM и проверку подлинности на основе форм (FBA). **Таблица 2** предназначена для отчетов, развернутых на сайте SharePoint на основе утверждений.  
 ...
 **Учетные данные не требуются (не используйте учетные данные):**  
...
-   **Скрыто** Показывает, отображается ли поле в представлении списка SharePoint.  
...
-   **Примененные фильтры**  Ограничивает данные, возвращаемые из списка SharePoint, перед возвратом данных в отчет. Выберите имя поля, оператор и значение, которые будут использованы для ограничения данных, извлекаемых из списка. Список доступных операторов зависит от выбранного типа данных значения.  
...
-   **Результаты запроса**  Отображает примеры строк, которые возвращаются при выполнении запроса. Если значения списка SharePoint часто изменяются на сайте SharePoint, то значения на панели результатов запросов могут отличаться от значений, выводимых в отчете.  
