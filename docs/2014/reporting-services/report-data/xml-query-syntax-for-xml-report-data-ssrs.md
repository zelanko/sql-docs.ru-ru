...
|---------------------|-------------------|  
|Веб-службы XML данных из метода ListChildren.|`<Query>`<br /><br /> `<Method Name="ListChildren" Namespace="http://schemas.microsoft.com/sqlserver/2005/06/30/reporting/reportingservices" />`<br /><br /> `</Query>`|  
...
> [!NOTE]  
>  Первый пример веб-службы перечисляет содержимое сервера отчетов, применяющего метод <xref:ReportService2006.ReportingService2006.ListChildren%2A> . Для выполнения этого запроса необходимо создать новый источник данных и задать строку подключения: http://localhost/reportserver/reportservice2006.asmx. <xref:ReportService2006.ReportingService2006.ListChildren%2A> Метод принимает два параметра: `Item` и `Recursive`. Значение по умолчанию для `Item` для `/` и `Recursive` для `1`.  
  
## <a name="specifying-namespaces"></a>Указание пространств имен  
 Используйте код XML `Query` элемент для указания пространства имен, которые используются в XML-данные из источника данных. Следующий XML-запрос использует пространство имен `sales`. Узлы XML `ElementPath` для элементов `sales:LineItems` и `sales:LineItem` используют пространство имен `sales`.  
...
```  
 ...
 Чтобы указать пространство имен поставщика данных, так что пространство имен по умолчанию остается пустой, используйте `xmldp`. Это показано в следующем примере.  
