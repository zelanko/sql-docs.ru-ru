...
2.  Откройте RSReportServer.config. Можно открыть файл конфигурации, содержащий [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)] или воспользоваться простым текстовым редактором, например в блокноте.  
...
4.  Добавьте элемент для поставщика данных.  
    ...
    |attribute|Описание|  
    |---------------|-----------------|  
    |`Name`|Уникальное имя поставщика данных, например **MyNETDataProvider**. Длина атрибута `Name` не должна превышать 255 символов. Имя должно быть уникальным среди всех записей в элементе `Extension` элемент файла конфигурации. Указанное здесь значение отображается в раскрывающемся списке типов источников данных при создании нового источника данных.|  
    |`Type`|Список с разделителями-запятыми, включающий полное имя пространства имен для класса, реализующего интерфейс <xref:System.Data.IDbConnection> , а затем имя сборки поставщика [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] (без расширения DLL).|  
...
6.  Добавьте следующую запись в файл RSReportDesigner.config в `Designer` элемент. Вам нужно только заменить `Name` атрибут с именем, которое указано в предыдущих записях.  
...
2.  Откройте файл RSPreviewPolicy.config с [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)] или воспользоваться простым текстовым редактором, например в блокноте.  
  
3.  Найдите в файле RSPreviewPolicy.config элемент `CodeGroup`.  
  
4.  Добавьте группу кода для [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] сборки поставщика данных, предоставляющую `FullTrust` разрешение. как показано ниже.  
