...
2.  Откройте RSReportServer.config. Можно открыть файл конфигурации с [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)] или простого текстового редактора, например «Блокнот».  
  
3.  Найдите `Data` в файле RSReportServer.config. Элемент для поставщика данных [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] необходимо создать в месте, указанном ниже.  
...
4.  Добавьте группу кода для сборки поставщика данных, которая предоставляет `FullTrust` разрешение. как показано ниже.  
...
4.  Добавьте элемент для поставщика данных.  
    ...
    |attribute|Описание|  
    |---------------|-----------------|  
    |`Name`|Уникальное имя поставщика данных, например **MyNETDataProvider**. Длина атрибута `Name` не должна превышать 255 символов. Имя должно быть уникальным среди всех элементов в пределах `Extension` элемента файла конфигурации. Указанное здесь значение отображается в раскрывающемся списке типов источников данных при создании нового источника данных.|  
    |`Type`|Список с разделителями-запятыми, включающий полное имя пространства имен для класса, реализующего интерфейс <xref:System.Data.IDbConnection> , а затем имя сборки поставщика [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] (без расширения DLL).|  
...
6.  Добавьте следующую запись в файл RSReportDesigner.config `Designer` элемента. Необходимо заменить только `Name` атрибут с именем, которое указывалось в предыдущих элементах.  
...
2.  Откройте файл RSPreviewPolicy.config с помощью [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)] или простого текстового редактора, например «Блокнот».  
  
3.  Найдите в файле RSPreviewPolicy.config элемент `CodeGroup`.  
  
4.  Добавьте группу кода для [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] сборки поставщика данных, которая предоставляет `FullTrust` разрешение. как показано ниже.  
