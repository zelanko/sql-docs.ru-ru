...
> [!NOTE]  
>  При **редактор преобразования «скрипт»** первого выхода, редактор задает `SynchronousInputID` свойство вывода `ID` входа компонента. Однако при создании последующих выходов редактор задает для свойства `SynchronousInputID` этих выходов значение равное нулю.  
>   
...
```  
 ...
 Дополнительные сведения см. в разделе [подключение к источникам данных в Component]((connecting-to-data-sources-in-the-script-component.md) сценария.  
