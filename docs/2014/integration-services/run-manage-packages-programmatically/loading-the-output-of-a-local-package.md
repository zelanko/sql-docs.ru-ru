...
4.  Измените значение `dtexecArgs` переменной в соответствии с требованиями, чтобы она содержала параметры командной строки, необходимые программе **dtexec. exe** для запуска пакета. В образце кода пакет загружается из файловой системы.
...
6.  Поместите в форму кнопку и текстовое поле. В примере кода используется `btnRun` в качестве имени кнопки, а `txtResults` в качестве имени текстового поля.
