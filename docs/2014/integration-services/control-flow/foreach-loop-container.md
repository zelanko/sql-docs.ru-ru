...
-   Перечислитель с циклом по каждому файлу ADLS для перечисления файлов в каталоге ADLS.
 ...
 Для обновления свойства объекта в пакете, соответствующего значению коллекции перечислителя, можно использовать сочетание переменных и выражений для свойств. Вначале необходимо сопоставить значение коллекции с пользовательской переменной, а затем задать выражение для свойства, которое использует указанную переменную. Например, со значением коллекции в перечислителе с циклом по каждому файлу сопоставляется переменной с именем `MyFile` и переменная используется в выражении для свойства задачи «Отправка почты» темы. Во время выполнения пакета свойству Subject присваивается имя файла на каждой итерации цикла. Дополнительные сведения см. в разделе [Использование выражений свойств в пакетах](../expressions/use-property-expressions-in-packages.md).  
...
|Большой двоичный объект Azure Foreach|Укажите контейнер больших двоичных объектов Azure, который содержит большие двоичные объекты для перечисления.|  
|Файл ADLS по циклу Foreach|Укажите каталог ADLS, который содержит файлы для перечисления, а также некоторые фильтры.|
