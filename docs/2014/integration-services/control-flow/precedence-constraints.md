...
## <a name="evaluation-operations"></a>Операции вычисления  
 Службы [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] предоставляют следующие операции вычисления:  
...
> [!NOTE]  
>  Только управления очередностью, являющиеся членами одной и той же `Precedence Constraint` коллекции могут быть сгруппированы в логическом условии. Например, нельзя объединить элементы управления очередностью из двух контейнеров «цикл по каждому элементу».  
