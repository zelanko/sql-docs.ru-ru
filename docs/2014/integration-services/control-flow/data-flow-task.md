...
|`OnPipelinePostEndOfRowset`|Обозначает, что компонент получил сигнал конец набора строк, который устанавливается при последнем вызове `ProcessInput` метод. Запись делается для каждого компонента в потоке данных, который обрабатывает ввод. Запись включает имя компонента.|  
...
|`OnPipelinePreEndOfRowset`|Указывает, что компонент уже готов получить сигнал конец набора строк, который устанавливается при последнем вызове `ProcessInput` метод. Запись делается для каждого компонента в потоке данных, который обрабатывает ввод. Запись включает имя компонента.|  
