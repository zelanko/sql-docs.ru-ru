...
## <a name="log-entries"></a>Записи журнала  
 Службы [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] предоставляют несколько событий журнала, которые доступны всем задачам. [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] также предоставляют пользовательские записи журнала для многих задач. Дополнительные сведения см. в разделах [Ведение журналов в службах Integration Services (SSIS)](../performance/integration-services-ssis-logging.md) и [Пользовательские сообщения для ведения журнала](../custom-messages-for-logging.md). Задача потока данных содержит следующие пользовательские записи журнала.  
...
|`OnPipelinePostEndOfRowset`|Обозначает, что компонент получил сигнал конец набора строк, который устанавливается при последнем вызове `ProcessInput` метода. Запись делается для каждого компонента в потоке данных, который обрабатывает ввод. Запись включает имя компонента.|  
|`OnPipelinePostPrimeOutput`|Указывает, что компонент завершил последний вызов `PrimeOutput` метод. В зависимости от потока данных, возможно формирование нескольких записей в журнале. Если компонент является источником, эта запись журнала означает, что компонент завершил обработку строк.|  
|`OnPipelinePreEndOfRowset`|Указывает, что компонент уже готов получить сигнал конец набора строк, который устанавливается при последнем вызове `ProcessInput` метода. Запись делается для каждого компонента в потоке данных, который обрабатывает ввод. Запись включает имя компонента.|  
...
|------------|-----------------|-----------|  
|**PathID**|Значение из `ID` свойства пути между источником OLE DB и преобразование «Сортировка».|1185|  
|**PathName**|Значение из `Name` свойства пути.|Выход данных источника «OLE DB»|  
|**ComponentID**|Значение `ID` свойства преобразования «Сортировка».|1180|  
