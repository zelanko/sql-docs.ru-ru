...
|Свойство|Описание|  
|--------------|-----------------|  
|`DelayValidation`|Логическое значение, указывающее, откладывается ли проверка контейнера до времени выполнения. Значение по умолчанию для этого свойства — `False`.<br /><br /> Дополнительные сведения см. в разделе <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.DelayValidation%2A>.|  
...
|`Disable`|Логическое значение, указывающее, запущен ли контейнер. Значение по умолчанию для этого свойства — `False`.<br /><br /> Дополнительные сведения см. в разделе <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.Disable%2A>.|  
