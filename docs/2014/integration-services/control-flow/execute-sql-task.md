...
|OLE DB|[Диспетчер подключений OLE DB](../connection-manager/ole-db-connection-manager.md)|  
...
## <a name="running-parameterized-sql-commands"></a>Выполнение параметризованных команд SQL  
 В инструкциях SQL и хранимых процедурах часто используются входные параметры, выходные параметры и коды возврата. Задача «Выполнение SQL» поддерживает `Input`, `Output`, и `ReturnValue` типы параметров. Вы используете `Input` для входных параметров `Output` для выходных параметров и `ReturnValue` для кодов возврата.  
...
## <a name="troubleshooting-the-execute-sql-task"></a>Устранение неполадок, связанных с задачей «Выполнение SQL»  
 ...
 Иногда команда SQL или хранимая процедура возвращает несколько результирующих наборов. Эти результирующие наборы включают не только наборы строк, которые являются результатом `SELECT` запросы, но отдельные значения, являющиеся результатом ошибок `RAISERROR` или `PRINT` инструкции. От типа используемого диспетчера соединений зависит, пропускает ли задача ошибки в результирующих наборах, следующих за первым.  
...
-   [Выполнение задач редактор SQL &#40;страница «Общие»&#41;](../execute-sql-task-editor-general-page.md)  
  
-   [Выполнение задач редактор SQL &#40;страница «сопоставление параметров»&#41;](../execute-sql-task-editor-parameter-mapping-page.md)  
  
-   [Выполнение задач редактор SQL &#40;«результирующий набор»&#41;](../execute-sql-task-editor-result-set-page.md)  
