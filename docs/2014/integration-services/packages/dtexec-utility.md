...
-   **/L[ogger]** _classid_orprogid;строка_конфигурации_:  
    ...
    -   [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)], перечислены ниже.  
  
        -   идентификатор ProgID: DTS.LogProviderSQLProfiler.1;  
  
        -   ClassID: {5C0B8D21-E9AA-462E-BA34-30FF5F7A42A1}  
  
    -   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], перечислены ниже.  
...
-   **/Set** [$Sensitive::]*путь_свойства;значение*: Необязательный. Переопределяет конфигурацию параметра, переменной, свойства, контейнера, регистратора, перечислителя Foreach или соединения в пакете. При использовании этот параметра **/Set** заменяет значение аргумента *propertyPath* на указанное значение. Можно указать несколько параметров **/Set** .  
...
-   **/VerifyB[uild]** _основная_[*;дополнительная*[*;сборка*]]: Необязательный. Проверяет соответствие номера сборки пакета номерам сборки, указанным на стадии проверки в аргументах *major*, *minor*и *build* . В случае несоответствия пакет выполняться не будет.  
