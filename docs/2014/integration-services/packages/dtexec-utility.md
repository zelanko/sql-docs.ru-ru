...
-   **/De [crypt]**_пароль_:   Необязательный. Задает пароль для расшифровки, используемый при загрузке пакета с шифрованием пароля.  
...
-   **/ Rep [orting]** _уровень_ [*; event_guid_or_name*[*; event_guid_or_name*[...]]: Необязательный. Определяет, какие типы сообщений включаются в отчет. Ниже приводятся доступные параметры отчета для *level* .  
...
-   **/ VerifyB [uild]** _основных_[*; minor*[*; построения*]]: Необязательный. Проверяет соответствие номера сборки пакета номерам сборки, указанным на стадии проверки в аргументах *major*, *minor*и *build* . В случае несоответствия пакет выполняться не будет.  
