...
|SaveCheckpoints|Показывает, сохраняет ли пакет контрольные точки. Для перезапуска пакета с момента сбоя это свойство должно иметь значение True.|  
 ...
 Кроме того, необходимо установить свойство FailPackageOnFailure в `true` значение для всех контейнеров пакета, которые необходимо определить как точки перезапуска.  
...
> [!NOTE]  
>  Параметр **/checkpointing on команды on** программы dtexec эквивалентен присвоению `SaveCheckpoints` свойству пакета значения `True`, а `CheckpointUsage` свойству — значение Always. Дополнительные сведения см. в статье [dtexec Utility](dtexec-utility.md).  
