...
|`Propagate`|Логическое значение|Указывает, будет ли событие передаваться обработчику события верхнего уровня.<br /><br /> Примечание. Значение переменной `Propagate` не учитывается при проверке пакета.<br /><br /> Если в дочернем пакете свойству `Propagate` присвоено значение `False`, это не препятствует распространению события в родительский пакет.|Все обработчики событий|  
