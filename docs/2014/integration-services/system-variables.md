...
|**InteractiveMode**|Логическое значение|Указывает, выполняется ли пакет в интерактивном режиме. Если пакет выполняется в конструкторе служб [!INCLUDE[ssIS](../includes/ssis-md.md)], это свойство равно `True`. Если пакет выполняется с помощью **DTExec** служебной программы командной строки, свойство имеет значение `False`.|  
...
|`Propagate`|Логическое значение|Указывает, будет ли событие передаваться обработчику события верхнего уровня.<br /><br /> Примечание: Значение `Propagate` переменной не учитывается при проверке пакета.<br /><br /> Если в дочернем пакете свойству `Propagate` присвоено значение `False`, это не препятствует распространению события в родительский пакет.|Все обработчики событий|  
