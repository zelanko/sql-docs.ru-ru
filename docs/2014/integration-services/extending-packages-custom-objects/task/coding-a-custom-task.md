...
### <a name="execute-method"></a>Метод Execute  
 Задачи, содержащиеся в пакете, выполняются, когда среда выполнения служб [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] вызывает их метод `Execute`. Задачи реализуют их основную бизнес-логику и функциональность в этом методе и предоставляют результаты выполнения путем отправки сообщений, возвращая значение из <xref:Microsoft.SqlServer.Dts.Runtime.DTSExecResult> перечисления и переопределяя свойство `get` из `ExecutionValue` свойство.  
