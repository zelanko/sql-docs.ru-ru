...
##  <a name="top"></a>Шаги по созданию, развертыванию и отладке пользовательского объекта для Integration Services  
...
1.  [Подпишите](#signing) сборку, которая будет создана строгим именем.  
  
2.  [Создайте](#building) сборку.  
  
3.  [Разверните](#deploying) сборку, переместив или скопировав ее в соответствующую [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] папку.  
  
4.  [Установите](#installing) сборку в глобальный кэш сборок (GAC).  
  
     Объект автоматически добавляется в область элементов.  
  
5.  При необходимости [устраните неполадки](#troubleshooting) развертывания.  
  
6.  [Тестирование](#testing) и отладка кода.  
...
##  <a name="building"></a>Сборка сборки  
...
##  <a name="testing"></a>Тестирование и отладка кода  
 ...
 Если вы хотите отладить методы времени разработки компонента, такие как `Validate` метод, откройте пакет, который использует этот компонент во втором экземпляре [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)], и присоедините его к процессу **devenv. exe** .  
...
2.  На вкладке **Отладка** в **свойствах проекта**выберите **Запуск внешней программы** в качестве **действия при запуске**и укажите **файл dtexec. exe**, установленный по умолчанию в папке C:\Program Files\Microsoft SQL Server\120\DTS\Binn.  
