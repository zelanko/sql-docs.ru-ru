...
-   **Сбой компонента**, который означает, что преобразование завершается неудачей и входные данные не вставляются в базу данных служб Data Quality Services. Это значение по умолчанию.  
  
-   **Перенаправление строки**, которое указывает, что входные данные не вставляются в базу данных служб Data Quality Services и перенаправляются на вывод ошибок.  
