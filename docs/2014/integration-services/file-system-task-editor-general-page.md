---
title: Редактор задачи файловой системы (страница «Общие») | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
> [!NOTE]  
>  Чтобы переопределить переменные, выбранные для `SourceConnection` и `DestinationConnection` , введите выражение для **источника** и **назначения** свойства. Эти выражения вводятся на странице **Выражения** редактора задачи **Файловая система**. Например, чтобы задать путь к файлам, которые задача будет использовать в качестве назначения, может потребоваться в одном случае использовать переменную A, а в другом — переменную B.  
...
|**Переместить файл**|Переместите файл. При выборе этого значения отображаются динамические параметры для источника и места назначения.<br /><br /> Примечание: При перемещении файла не включает имя файла в пути к каталогу, который предоставляется в качестве цели.|  
|**Переименовать файл**|Переименуйте файл. При выборе этого значения отображаются динамические параметры для источника и места назначения.<br /><br /> Примечание: При переименовании файла, включает имя нового файла в пути к каталогу, указываемое для назначения.|  
