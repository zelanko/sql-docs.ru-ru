...
-   **Изменения для таблицы x** — это источник CDC, который считывает изменения, внесенные в таблицу x, которая выполняется в диапазоне обработки CDC, определенном в родительском потоке управления.  
  
-   **Разделитель CDC X** используется для разделения изменений на вставки, удаления и обновления. В этом сценарии предполагается, что источник CDC настроен для составления суммарных изменений, чтобы различные типы изменений можно было обрабатывать параллельно.  
...
|7-ERROR|Группа CDC находится в состоянии ERROR.|  
 ...
 ![Схема состояния](../media/statediagram.gif "Диаграмма состояния")  
...
-   Видеоролик [CDC для баз данных Oracle с помощью служб SQL Server Integration Services 2012 (видео по SQL Server)](https://technet.microsoft.com/sqlserver/jj218898)на сайте technet.microsoft.com.  
