...
-   значение 2 указывает, что столбец передан на выход и также был использован в уточняющем запросе.  
 ...
 Преобразование «Уточняющий запрос термина» добавляет два столбца в выход преобразования, с именем по умолчанию `Term` и `Frequency`. Столбец `Term` содержит термин из таблицы подстановки, а столбец `Frequency` — количество вхождений термина из ссылочной таблицы во входном наборе данных. Эти столбцы не содержат свойство CustomLineageID.  
...
-   [Редактор преобразования Уточняющий запрос термина &#40;ссылаться вкладка «таблица»&#41;](../../term-lookup-transformation-editor-reference-table-tab.md)  
