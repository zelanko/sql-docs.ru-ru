...
-   **_Similarity**, столбец, описывающий сходство значений во входных и ссылочных столбцах.  
  
-   **_Confidence**— столбец, описывающий качество совпадения.  
...
|**женератеандперсистневиндекс**|Создает и сохраняет новый индекс, но не поддерживает его.|  
|**женератеневиндекс**|Создает новый индекс, но не сохраняет его.|  
|**реусиксистингиндекс**|Повторно использует уже существующий индекс.|  
...
## <a name="row-comparison"></a>Сравнение строк  
 При настройке преобразования «Нечеткий уточняющий запрос» можно определить алгоритм сравнения, который преобразование применяет для поиска записей в ссылочной таблице. Если для `True`Свойства Exhaustive задано значение, то преобразование сравнивает каждую строку во входных данных с каждой строкой в ссылочной таблице. Этот алгоритм сравнения может дать более точные результаты, но, вероятнее всего, данное преобразование будет выполняться довольно долго в том случае, если число строк в ссылочной таблице велико. Если свойство Exhaustive имеет значение `True`, вся ссылочная таблица загружается в память. Чтобы избежать проблем с производительностью, рекомендуется задать для свойства Exhaustive значение только `True` во время разработки пакета.  
  
 Если свойство Exhaustive имеет значение `False`, преобразование «Нечеткий уточняющий запрос» возвращает только те совпадения, которые имеют по крайней мере один индексированный токен или подстроку (подстрока называется *q-граммами*), общие с входной записью. Для повышения эффективности уточняющих запросов в инвертированном индексе, используемом преобразованием «Нечеткий уточняющий запрос» для поиска совпадений, индексируется только подмножество токенов в каждой строке таблицы. Если входной набор данных невелик, можно задать для него полное значение `True` , чтобы избежать пропущенных совпадений, для которых в таблице индексов не существует общих маркеров.  
  
## <a name="caching-of-indexes-and-reference-tables"></a>Кэширование индексов и ссылочных таблиц  
 При настройке преобразования «Нечеткий уточняющий запрос» можно определить, будет ли преобразование производить частичное кэширование индексов и ссылочных таблиц перед началом работы. Если для `True`свойства WarmCaches значения задано значение, индекс и ссылочная таблица загружаются в память. Если входное значение содержит много строк, установка свойства WarmCaches значения в `True` значение может повысить производительность преобразования. Если количество входных строк мало, установка свойства WarmCaches значения в `False` значение позволяет ускорить повторное использование большого индекса.  
