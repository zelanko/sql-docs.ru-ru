...
|OpenRowset|String|Если AccessMode имеет `OpenRowset`значение, то имя таблицы или представления, к которому обращается целевой объект OLE DB.|  
|OpenRowsetVariable|String|Если AccessMode имеет `OpenRowset from Variable`значение, имя переменной, содержащей имя таблицы или представления, к которому обращается целевой объект OLE DB.|  
|SqlCommand|String|Если AccessMode имеет `SQL Command`значение, то инструкция TRANSACT-SQL, используемая назначением OLE DB для указания целевых столбцов данных.|  
