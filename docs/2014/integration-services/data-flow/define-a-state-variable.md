...
|TFSTART|Это состояние при запуске пакета обновления тонкого канала, не начальный после `GetProcessingRange` вызова операции задачи «Управление CDC».<br /><br /> Это означает, что регулярное выполнение CDC запущен, но еще не завершено или имеет не завершено верно (`MarkProcessedRange`).|  
