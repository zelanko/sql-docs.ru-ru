---
title: Задание свойств элементов управления очередностью | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
6.  В `Value` раскрывающегося списка выберите результат выполнения приоритетного исполняемого объекта.  
  
7.  При выполнении операции оценки используется выражение, в `Expression` введите выражение и нажмите кнопку **тест** для вычисления выражения.  
  
    > [!NOTE]  
    >  В именах переменных учитывается регистр букв.  
  
8.  При подключении задач или контейнеров к исполняемому объекту с ограничением, выберите **логические и** для указания, что результаты выполнения все предшествующие исполняемые модули должны возвращать `true`. Выберите **логического или** для указания, что результат выполнения только один должны возвращать `true`.  
...
4.  В окне **Свойства** установите следующие свойства элементов управления очередностью, доступные для чтения-записи.  
    ...
    |LogicalAnd|Задать `LogicalAnd` для указания ли вычислять управление очередностью совместно с другими ограничениями очередностью, когда несколько исполняемых объектов предшествуют и связаны с исполняемый объект с ограничением|  
