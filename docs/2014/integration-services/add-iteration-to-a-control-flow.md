...
ms.locfileid: "66061981"
---
# <a name="add-iteration-to-a-control-flow"></a>Добавление итерации к потоку управления
  [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)]включает контейнер «цикл по элементам», элемент потока управления, который позволяет легко включить циклы, которые условно повторяет поток управления в пакете. Дополнительные сведения см. в статье [For Loop Container](control-flow/for-loop-container.md).  
