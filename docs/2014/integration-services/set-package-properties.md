...
|**Отключено**|Указывает, отключен ли пакет. Значение по умолчанию этого свойства равно `False`.|  
...
|`ProtectionLevel`|Уровень защиты пакета. Значения: `DontSaveSensitive` ,, `EncryptSensitiveWithUserKey` , `EncryptSensitiveWithPassword` `EncryptAllWithPassword` и **ServerStorage**. Значение по умолчанию этого свойства равно `EncryptSensitiveWithUserKey`. Для получения дополнительной информации см. <xref:Microsoft.SqlServer.Dts.Runtime.DTSProtectionLevel>.|  
