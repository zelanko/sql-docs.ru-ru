...
###  <a name="forced-execution-value"></a><a name="ForcedExecutionValue"></a>Значение принудительного выполнения  
...
###  <a name="identification"></a><a name="Identification"></a>Идентификатор  
...
|`Name`|Имя пакета.|  
|`PackageType`|Тип пакета. Допустимые значения — `Default`, `DTSDesigner`, `DTSDesigner100`, `DTSWizard`, `SQLDBMaint` и `SQLReplication`. Значение по умолчанию этого свойства равно `Default`. Для получения дополнительной информации см. <xref:Microsoft.SqlServer.Dts.Runtime.DTSPackageType>.|  
  
###  <a name="misc"></a><a name="Misc"></a>Информации  
...
|`LoggingMode`|Значение, определяющее для пакета режим записи в журнал. Допустимые значения — `Disabled`, `Enabled` и `UseParentSetting`. Значение по умолчанию этого свойства равно `UseParentSetting`. Для получения дополнительной информации см. <xref:Microsoft.SqlServer.Dts.Runtime.DTSLoggingMode>.|  
