...
-   **Контейнер цикла foreach:** Перечисляемое значение контейнера будет сопоставлен переменной пакета, определяемые пользователем. Контейнер будет использовать определенные пользователем переменные, чтобы динамически изменять свойства `ConnectionString` диспетчера соединений с неструктурированными файлами и последовательно соединяться с каждым файлом в папке.  
