...
-   **Диспетчер соединений с неструктурированными файлами:** предстоит изменить диспетчер соединений, который был создан на занятии 1, используя определенные пользователем переменные для заполнения данными диспетчера соединений `ConnectionString` свойство.  
...
4.  В **Редактор циклов по каждому элементу** диалогового **Общие** страницы, для **имя**, введите `Foreach File in Folder`. Нажмите кнопку **ОК**.  
  
5.  Щелкните правой кнопкой мыши контейнер цикла Foreach, щелкните **свойства**и в окне «Свойства» убедитесь, что `LocaleID` свойству **английский (США)**.  
...
3.  В **добавить переменную** диалоговом для **имя**, тип `varFileName`.  
...
-   Перетащите **извлечь данные валют образца** задачи потока данных в контейнере цикла Foreach переименованы и теперь `Foreach File in Folder`.  
...
## <a name="see-also"></a>См. также  
 [Настройке контейнера «цикл по каждому элементу»](control-flow/foreach-loop-container.md)   
