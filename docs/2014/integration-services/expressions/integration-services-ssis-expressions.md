...
-   Управление очередностью может использовать выражения для указания условий, определяющих, будет ли выполняться задача с ограничением или контейнер пакета. Выражения, используемые в ограничении очередностью должны оцениваться как `true` или `false`. Например, выражение \@A > \@B сравнивает две переменные, определяемые пользователем, чтобы определить, выполняется ли задача с ограничением.  
  
-   Контейнер «цикл по элементам» может использовать выражения для создания инструкций инициализации, оценки выражения и приращения счетчика, используемые структурами цикла. Например, выражение \@счетчик = 1 инициализирует счетчик циклов.  
