...
-   Последующие символы могут следовать буквы или цифры по определению стандарта Юникод 2.0, символ подчеркивания (_), \@, $, а знаки #.  
...
## <a name="variables-in-expressions"></a>Переменные в выражениях  
 При использовании переменных в выражениях, необходимо включить \@ префикс. Например **счетчика** переменной указывается с помощью \@счетчика. \@ Символ не является частью имени переменной; он только указывает средству оценки выражений. Если выражение построено в диалоговом окне полей, [!INCLUDE[ssIS](../../includes/ssis-md.md)] конструктор предоставляет, \@ символ автоматически добавляется к имени переменной. Это не должно быть пробелов между \@ символ и имя переменной.  
...
-   Последующие символы могут следовать буквы или цифры по определению стандарта Юникод 2.0, символ подчеркивания (_), \@, $, а знаки #.  
 ...
 Если имя переменной содержит символ, не разрешенный этими правилами, переменная должна быть заключена в квадратные скобки. Например, имя переменной, содержащее пробел, должно быть заключено в квадратные скобки. Перед открывающей скобкой \@ символ. Например **My Name** переменная будет упоминаться как \@[My Name]. Между символом @ и открывающей скобкой не должно быть пробелов.  
...
>  Сочетание пространства имен и уточненного имени переменной необходимо заключить в квадратные скобки, чтобы средство оценки выражений смогло распознать переменную.  
 ...
 Если значение **число** в **пользователя** пространство имен равно 10, а значение **число** в **MyNamespace** равно 2, выражение принимает значение `true` так, как средство оценки выражений правильно распознает две различные переменные.  
  
 Если имена переменных неуникальны, ошибки не возникнет. Вместо этого средство оценки выражений будет использовать только один экземпляр переменной для вычисления выражения, и результат выражения будет неверным. Например, следующее выражение предназначалось для сравнения значений (10 и 2) для двух разных **число** переменные, но выражение принимает значение `false` так, как средство оценки выражений использует один и тот же экземпляр  **Число** дважды.  
