...
## <a name="identifiers"></a>Идентификаторы  
 ...
 Выражения могут содержать системные переменные, которые предоставляют службы [!INCLUDE[ssIS](../../includes/ssis-md.md)] , а также пользовательские переменные. При использовании переменных в выражении, необходимо включить \@ префикс. Например `Counter` переменной указывается с помощью \@счетчика. \@ Символ не является частью имени переменной; он только указывает средству оценки выражений что идентификатор является переменной. Дополнительные сведения см. в разделе [Идентификаторы (службы SSIS)](identifiers-ssis.md).  
