...
ms.locfileid: "36189884"
---
# <a name="enumerated-constants-in-property-expressions"></a>Констант-перечислителей в выражениях свойств
  Если выражения свойств включают в себя значения из списка элементов-перечислителей, эти выражения должны использовать числовое значение элементов-перечислителей вместо понятного имени элемента. Например, если выражение устанавливает `LoggingMode` свойств необходимо использовать числовое значение 2 вместо понятного имени отключена.  
...
|Overwrite|1|  
 ...
 `BackupAction` свойство — устанавливается с помощью значений из `BackupTaskType` перечисления. Это свойство работает с `BackupIsIncremental` свойство, чтобы определить тип резервного копирования, выполняемой задачей.  
