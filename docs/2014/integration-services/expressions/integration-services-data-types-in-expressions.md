...
## <a name="implicit-data-conversion"></a>Неявные преобразования данных  
 Неявное преобразование типа данных происходит, когда средство оценки выражений автоматически преобразует данные из одного типа в другой. Например если `smallint` сравнивается с `int`, `smallint` неявно преобразуется в `int` перед сравнением.  
...
>   
>  Например, функции преобразования языка Visual Basic преобразуют `True` в значение -1, однако метод `System.Convert.ToInt32` в среде .NET Framework преобразует `True` в значение +1. [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] Язык выражений преобразует `True` значение -1.  
>   
