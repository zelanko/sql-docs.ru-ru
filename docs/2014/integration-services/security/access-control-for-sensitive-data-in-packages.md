...
## <a name="protection-level-setting-and-the-ssisdb-catalog"></a>Установка уровня защиты и каталог SSISDB  
 ...
 При экспорте проекта (ispac-файл) из [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] server в файловой системе, система автоматически изменяет уровень защиты для `EncryptSensitiveWithUserKey`. При импорте проекта с помощью **мастер импорта проекта служб Integration Services** в [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)], **ProtectionLevel** свойство в **свойства** окна Показывает значение `EncryptSensitiveWithUserKey`.  
...
1.  Во время разработки, оставьте уровень защиты пакетов, задано значение по умолчанию `EncryptSensitiveWithUserKey`. Этот параметр позволит гарантировать просмотр важных значений в пакете только разработчиком. Можно подумать об использовании `EncryptAllWithUserKey` или `DontSaveSensitive`.  
