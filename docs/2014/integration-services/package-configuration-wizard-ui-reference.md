...
|**Запись реестра**|Храните конфигурацию в реестре. При выборе этого значения отображаются динамические параметры в данном разделе, **Тип конфигурации**.|  
...
|Значение|Описание:|  
|-----------|-----------------|  
|**Запись реестра**|Введите раздел реестра, который содержит сведения о конфигурации. Формат — \<registry key>.<br /><br /> Этот раздел реестра должен уже существовать в кусте HKEY_CURRENT_USER и иметь значение с именем Value. Значение может иметь строковый тип или тип DWORD.<br /><br /> Если вы хотите использовать раздел реестра, расположенный не в корне HKEY_CURRENT_USER, используйте формат, \<Registry key\registry key\\...> чтобы указать ключ.|  
