...
|Когда задача или контейнер получает `OnVariableValueChanged` событий.|Вызывается средой во время выполнения служб [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] , когда изменяется значение переменной. Свойства RaiseChangeEvent переменной должно быть присвоено `true` для порождения этого события.<br /><br /> **\*\* Предупреждение. \*\*** Переменная, связанная с этой точкой останова, должна быть определена в области **контейнера** . Если переменная определена в области пакета, точка останова не достигается.|  
...
## <a name="progress-reporting"></a>Отчет о состоянии  
 [!INCLUDE[ssIS](../../../includes/ssis-md.md)] Конструктор включает два типа отчетов о состоянии: выделение цветом в области конструктора **поток управления** вкладка и сообщения о состоянии на **ход выполнения** вкладки.  
