...
-   **Состояние**: Значок, указывающий текущее состояние экземпляра CDC. Состояния описаны в следующей таблице.  
    ...
    |||  
    |-|-|  
    |![Ошибка](../media/error.gif "Ошибка")|**Ошибка**. Экземпляр Oracle CDC не был запущен, так как произошла ошибка, не дающая возможности повторить операцию. Доступны следующие подтипы состояний:<br /><br /> **Не настроено**: Произошла ошибка конфигурации, требуется ручное вмешательство.<br /><br /> **Требуется пароль**: Пароль не задан для экземпляра CDC Oracle или пароль является недопустимым.<br /><br /> **Непредвиденная**. Все остальные неустранимые ошибки.|  
    |![ОК](../media/okay.gif "ОК")|**Выполняется**. Экземпляр CDC выполняется и обрабатывает записи об изменениях. Доступны следующие подтипы состояний:<br /><br /> **Бездействие**: Все записи об изменениях, которые были обработаны и сохранены в целевых таблицах изменений. Активных транзакций больше нет.<br /><br /> **Обработка**: Существуют записи об изменениях процесса, которые еще не записаны в таблицы изменений.|  
    ...
    |![Приостановлено](../media/paused.gif "Приостановлено")|**Приостановлено**: Экземпляр CDC запущен, но обработка приостановлена из-за повторяемой ошибки. Доступны следующие подтипы состояний:<br /><br /> **Отключено**: Не удалось установить подключение к исходной базе данных Oracle. Обработка возобновится после восстановления связи.<br /><br /> **Служба хранилища**: Хранилище заполнено. Обработка возобновится, когда станет доступным дополнительный объем хранения данных.<br /><br /> **Средство ведения журнала**: Средство ведения журнала соединено с Oracle, но не может прочитать журналы транзакций Oracle из-за временных проблем, например, журнал обязательных транзакций недоступен.|  
  
-   **Подробности состояния**: Текущий подтип состояния.  
  
-   **Сообщение о состоянии**: Дополнительные сведения о текущем состоянии.  
  
-   **Метка времени**: Время в формате UTC при последнего чтения состояния CDC из таблицы состояний.  
  
-   **Обрабатывается в настоящий момент**: Вы можете отслеживать следующие сведения в этом разделе.  
    ...
    -   **Последний номер изменения (CN) в журнале транзакций**: Последние номер изменения (CN), считанное из журнала транзакций Oracle.  
    ...
    -   **Текущий номер изменения (CN)**: Последний номер изменения (SCN) в базе данных Oracle источника (не в журнале транзакций).  
...
-   **Счетчики**: Вы можете отслеживать следующие сведения в этом разделе.  
  
    -   **Завершенные транзакции**: Количество транзакций, выполненных с момента последнего сброса экземпляра CDC. Сюда не входят транзакции, которые не содержат нужных таблиц.  
  
    -   **Записанные изменения**: Таблицы изменений количество изменений, записанных в SQL Server.  
