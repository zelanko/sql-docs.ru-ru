...
-   **Вычисление конечной даты и времени**. Если для добавочной загрузки используется фиксированный интервал, текущая конечная дата и время вычисляются как смещение относительно начальной даты и времени. Опять же, это значение можно вычислить с `datetime` помощью функций TRANSACT-SQL или языка [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] выражений.  
