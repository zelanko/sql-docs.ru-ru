 ...
 **Имя сервера**: Выберите целевой объект [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] подключением к экземпляру (например,  **\\ \\< имя_компьютера >\\< имя_экземпляра >**). По умолчанию выводится экземпляр сервера, к которому подключение выполнялось в последний раз.  
