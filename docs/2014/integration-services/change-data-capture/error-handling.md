...
|RUNNING|1|0|Экземпляр CDC выполняется и обрабатывает записи об изменениях. Состояние RUNNING содержит следующие коды подсостояния:<br /><br /> IDLE. Все записи об изменениях были обработаны и сохранены в целевых контрольных (**_CT**) таблицы. В контрольных таблицах не существует активных транзакций.<br /><br /> PROCESSING. Существуют записи об изменениях, которые еще не записаны в элемент управления (**_CT**) таблицы.|  
