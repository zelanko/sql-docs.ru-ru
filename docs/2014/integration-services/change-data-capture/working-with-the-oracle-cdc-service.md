...
###  <a name="dboxcbcdc_add_servicesvcnamesqlusr"></a><a name="BKMK_dboxcbcdc_add_service"></a> dbo.xcbcdc_add_service(svcname,sqlusr)  
 ...
 Чтобы использовать процедуру **dbo. xcbcdc_add_service \<service name, username> ** , пользователь должен быть членом роли базы данных **db_owner** для базы данных экземпляра CDC с именем или членом предопределенной роли сервера **sysadmin** или **serveradmin** .  
