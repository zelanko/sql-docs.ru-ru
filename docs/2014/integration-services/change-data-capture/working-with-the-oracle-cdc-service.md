...
-   [База данных MSXDBCDC](#BKMK_MSXDBCDC)  
...
-   [Использование интерфейса командной строки для настройки службы CDC Service](#BKMK_CommandConfigCDC)  
...
|включено|Указывает, активен (1) или отключен (0) экземпляр Oracle CDC. При запуске службы Oracle CDC Service запускаются только экземпляры, отмеченные «включить» (1).<br /><br /> **Примечание.** Экземпляр Oracle CDC может быть отключена из-за ошибки, который не предусматривает возможность повтора. В этом случае экземпляр следует перезапустить вручную после устранения ошибки.|  
...
-   [dbo.xcbcdc_reset_db(имя базы данных)](#BKMK_dboxcbcdc_reset_db)  
  
-   [dbo.xdbcdc_disable_db(имя_базы_данных)](#BKMK_dboxdbcdc_disable_db)  
  
-   [dbo.xcbcdc_add_service(svcname,sqlusr)](#BKMK_dboxcbcdc_add_service)  
  
-   [dbo.xdbcdc_start(имя_базы_данных)](#BKMK_dboxdbcdc_start)  
  
-   [dbo.xdbcdc_stop(имя_базы_данных)](#BKMK_dboxdbcdc_stop)  
 ...
 **Примечание.** Любой параметр, который содержит пробелы или кавычки должны быть заключены в двойные кавычки ("«). Встроенные двойные кавычки необходимо удвоить (например, для использования **"A#B" D** в качестве пароля введите **""A#B"" D"**).  
