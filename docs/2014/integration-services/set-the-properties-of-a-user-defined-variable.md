---
title: Установка свойств определяемой пользователем переменной | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
-   Значение переменной можно задать с помощью свойства Value или Expression. По умолчанию, равным свойство EvaluateAsExpression `False` и значение переменной задается свойством значение. Чтобы использовать выражения для задания значения, необходимо сначала задать EvaluateAsExpression `True`, а затем введите выражение в свойство Expression. Свойство Value автоматически устанавливается в значение результата выражения.  
  
-   Свойство ValueType содержит тип данных значения свойства Value. Если свойство Value задается с помощью выражения, свойство ValueType автоматически обновляется типом данных, совместимым с результатом вычисления выражения. Например, если значение 0 и свойство ValueType содержится **Int32** и затем задать выражение для GETDATE(), значение содержит текущую дату и время и ValueType равно `DateTime`.  
...
-   Чтобы использовать выражения для задания значения переменной, сначала убедитесь, что тип данных переменной совместим с результатом вычисления выражения, а затем укажите выражение в `Expression` столбец **переменных** окна. Свойство EvaluateAsExpression в **свойства** окно автоматически устанавливается значение `True`.  
...
-   Окно **Переменные** для переменной предоставляет доступ к диалоговому окну **Построитель выражений** . Это средство можно использовать для построения, проверки и вычисления выражений. Дополнительные сведения см. в разделах [Построитель выражений](expressions/expression-builder.md) и [Выражения служб Integration Services (SSIS)](expressions/integration-services-ssis-expressions.md).  
 ...
 В обоих **переменных** и **свойства** окно, при присвоении выражения для переменной, и `EvaluateAsExpression` равно `True`, невозможно изменить тип данных переменной.  
  
 **Задание свойств имя и пространство имен**  
...
5.  Выберите переменную из списка, а затем обновите значения в `Name`, **тип данных**, `Value`, `Namespace`, **создать событие изменения**, **описание,** и `Expression` столбцов.  
