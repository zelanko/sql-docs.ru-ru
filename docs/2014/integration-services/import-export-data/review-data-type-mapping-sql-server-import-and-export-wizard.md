...
### <a name="table-list"></a>Список «Таблица»  
 Верхняя часть страницы **Просмотр проблем с типами данных** — **это список таблиц** , в котором перечисляются таблицы, которые должны быть переданы из источника в место назначения. В следующей таблице описываются столбцы этого списка.  
...
|**Источник**|Отображает имя исходной таблицы.|  
...
|**Назначение**|Отображает имя целевой таблицы.|  
 ...
 Чтобы просмотреть сведения о преобразовании отдельной таблицы, выберите таблицу в этой сетке **таблицы** . Сведения о преобразовании для выбранной таблицы появятся в столбцах сетки **Сопоставление типов данных** в нижней части страницы.  
  
### <a name="data-type-mapping-list"></a>Список «Сопоставление типов данных»  
 В нижней части страницы **Просмотр проблем с типами данных** находится список **Сопоставление типов данных** . Здесь отображаются подробные сведения о преобразовании столбцов таблицы, выбранной в списке **Таблица** . В следующей таблице описываются столбцы этого списка.  
...
|**Тип источника**|Отображает тип данных исходного столбца.|  
|**Целевой столбец**|Отображает имя целевого столбца.|  
|**Тип назначения**|Отображает тип данных целевого столбца.|  
|**Функция**|Указывает, следует ли продолжать планируемое преобразование.<br /><br /> Чтобы продолжить планируемое преобразование, установите этот флажок.<br /><br /> Чтобы отменить преобразование типа данных, снимите флажок.|  
|**При ошибке**|Укажите порядок обработки ошибок мастером:<br /><br /> Используйте параметр **On Error (Global)** .<br /><br /> завершить с ошибкой и прекратить процесс импорта или экспорта;<br /><br /> пропустить ошибку.|  
|**В случае усечения**|Укажите порядок обработки усечений мастером:<br /><br /> Используйте параметр **On Truncate (Global)** .<br /><br /> завершить работу с ошибкой и прекратить процесс импорта или экспорта;<br /><br /> пропустить усечение.|  
