...
8.  На **входы и выходы** страницы, добавьте новый, второй выход и задать его `SynchronousInputID` значение идентификатора входа, которое также является значением из `SynchronousInputID` свойство выхода по умолчанию. Присвойте свойству `ExclusionGroup` обоих выходов одно и то же ненулевое значение (например, 1), чтобы указать, что каждая строка должна быть направлена только в один из двух выходов. Дайте новому выводу ошибок понятное описательное имя, например MyErrorOutput.  
