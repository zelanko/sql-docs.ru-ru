---
title: Неподдерживаемые функции служб Master Data Services в SQL Server 2014 | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
-   Теперь разрешения на производную иерархию основываются на модели. Например, если требуется, чтобы пользователь должен иметь разрешение на производную иерархию, необходимо назначить **обновление** на объект модели. Затем можно присвоить **Deny** доступ к любой сущности требуется запретить пользователю предоставляется доступ.  
  
-   Разрешения для явных иерархий теперь основываются на сущности. Например, если у пользователя есть **обновление** разрешения для учетной записи сущности все явные иерархии для сущности будет обновляться.  
  
-   Разрешения группы атрибутов больше не могут быть назначены в **разрешения пользователей и групп** функциональной области. Вместо этого в **системное администрирование** функциональной области, где создаются группы атрибутов, пользователей и групп можно предоставить **обновление** разрешение для группы атрибутов. **Только для чтения** разрешение для группы атрибутов больше не доступен.  
...
### <a name="code-generation-business-rules"></a>Бизнес-правила создания кодов  
 Теперь бизнес-правила, которые автоматически формируют значения атрибута Code, администрируются по-разному. Ранее для формирования значений для атрибута Code, использовать **атрибут по умолчанию для него сформированное значение** действия в **системное администрирование** функциональной области **бизнес-правила** . Теперь в **системное администрирование**, необходимо изменить сущность для включения автоматически формируемых значений кода. Дополнительные сведения см. в разделе [Автоматическое создание кодов (службы Master Data Services)](automatic-code-creation-master-data-services.md).  
