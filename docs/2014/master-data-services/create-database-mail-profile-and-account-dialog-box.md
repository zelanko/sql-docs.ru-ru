---
title: Создание профиля компонента Database Mail и диалоговое окно «учетная запись» (диспетчер конфигурации служб Master Data Services) | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
|**SMTP-сервер**|Введите имя или IP-адрес SMTP-сервера, который учетная запись будет использовать для отправки электронной почты. Пример SMTP формата для сервера — `smtp.` *< имя_организации >*`.com`. Обратитесь за помощью к администратору электронной почты.|  
