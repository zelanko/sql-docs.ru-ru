...
## <a name="enable-change-data-capture-for-a-database"></a>Включение системы отслеживания измененных данных для базы данных  
 ...
 Если базы данных включается отслеживание измененных данных, `cdc` схемы, `cdc` пользователя, таблицы метаданных и другие системные объекты создаются для базы данных. `cdc` Схема содержит таблицы метаданных системы отслеживания измененных данных, и после включения исходных таблиц, измененных данных, отдельные таблицы изменений, служащие репозиторием информации об изменениях. `cdc` Схема также содержит связанные системные функции, используемые для запроса информации об изменениях.  
...
## <a name="disable-change-data-capture-for-a-table"></a>Отключение системы отслеживания измененных данных для таблицы  
 Членами `db_owner` экземпляр отслеживания для отдельных исходных таблиц можно удалить в предопределенной роли базы данных с помощью хранимой процедуры `sys.sp_cdc_disable_table`. Чтобы определить, включена ли в настоящее время для исходной таблицы система отслеживания измененных данных, рассмотрите столбец `is_tracked_by_cdc` в представлении каталога `sys.tables`. Если после того, как имело место отключение, отсутствуют какие-либо таблицы, включенные для базы данных, также происходит удаление заданий отслеживания измененных данных.  
