...
## <a name="enable-change-data-capture-for-a-database"></a>Включение системы отслеживания измененных данных для базы данных  
 ...
 При включении базы данных для отслеживания измененных данных `cdc` схемы, `cdc` пользователя, таблицы метаданных и другие системные объекты создаются для базы данных. `cdc` Схема содержит таблицы метаданных системы отслеживания измененных данных, и после включения исходных таблиц для отслеживания измененных данных отдельные таблицы изменений, служащие репозиторием информации об изменениях. `cdc` Схема также содержит связанные системные функции, используемые для запроса информации об изменениях.  
...
## <a name="disable-change-data-capture-for-a-database"></a>Отключение системы отслеживания измененных данных для базы данных  
 Член `sysadmin` предопределенной роли сервера можно выполнить хранимую процедуру [sys.sp_cdc_disable_db &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sys-sp-cdc-disable-db-transact-sql) в контексте базы данных, чтобы отключить отслеживание изменений в данных для базы данных. Нет необходимости отключать отдельные таблицы, прежде чем будет выполнено отключение базы данных. Отключение базы данных удаляет все связанные метаданные отслеживания измененных данных, включая `cdc` пользователя и схему, а также об изменениях записи заданий. Но любые шлюзовые роли, созданные системой отслеживания измененных данных, не будут удалены автоматически, поэтому необходимо выполнить их явное удаление. Чтобы определить, включена ли база данных, выполните запрос к столбцу `is_cdc_enabled` в представлении каталога sys.databases.  
...
## <a name="enable-change-data-capture-for-a-table"></a>Включение отслеживания измененных данных для таблицы  
 После включения базы данных для отслеживания измененных данных, члены `db_owner` предопределенной роли базы данных можно создать экземпляр отслеживания для отдельных исходных таблиц с помощью хранимой процедуры `sys.sp_cdc_enable_table`. Чтобы определить, была ли включена исходная таблица для отслеживания измененных данных, запросите значение столбца is_tracked_by_cdc в представлении каталога `sys.tables`.  
...
## <a name="disable-change-data-capture-for-a-table"></a>Отключение системы отслеживания измененных данных для таблицы  
 Члены `db_owner` экземпляр отслеживания для отдельных исходных таблиц предопределенной роли базы данных можно удалить с помощью хранимой процедуры `sys.sp_cdc_disable_table`. Чтобы определить, включена ли в настоящее время для исходной таблицы система отслеживания измененных данных, рассмотрите столбец `is_tracked_by_cdc` в представлении каталога `sys.tables`. Если после того, как имело место отключение, отсутствуют какие-либо таблицы, включенные для базы данных, также происходит удаление заданий отслеживания измененных данных.  
