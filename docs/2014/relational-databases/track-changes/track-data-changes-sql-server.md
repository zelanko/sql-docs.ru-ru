...
-   Отслеживание изменений основано на зафиксированных транзакциях. Порядок изменений основан на времени фиксации транзакции. Это позволяет получить надежные результаты при наличии долго выполняемых и перекрывающихся транзакций. Пользовательские решения, использующие `timestamp` значения должны проектироваться специально для обработки таких ситуаций.  
 ...
 **Конфигурация и администрирование**  
 Чтобы включить или отключить изменений системы отслеживания измененных данных для базы данных, вызывающий [sys.sp_cdc_enable_db &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sys-sp-cdc-enable-db-transact-sql) или [sys.sp_cdc_disable_db &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sys-sp-cdc-disable-db-transact-sql)необходимо быть членом фиксированной серверной `sysadmin` роли. Включение и отключение отслеживания измененных данных на уровне таблиц пользователь, вызывающий процедуры [sys.sp_cdc_enable_table &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sys-sp-cdc-enable-table-transact-sql) и [sys.sp_cdc_disable_table &#40;Transact-SQL&#41; ](/sql/relational-databases/system-stored-procedures/sys-sp-cdc-disable-table-transact-sql) должен быть членом роли sysadmin или членом базе `database db_owner` роли.  
  
 Использование хранимых процедур для администрирования заданий отслеживания измененных данных ограничено членами серверной `sysadmin` роли и члены `database db_owner` роли.  
