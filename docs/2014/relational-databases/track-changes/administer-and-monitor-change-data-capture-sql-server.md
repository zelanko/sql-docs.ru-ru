...
#### <a name="maxtrans-parameter"></a>Параметр maxtrans  
 Параметр *maxtrans* указывает максимальное количество транзакций, которое может быть обработано в одном цикле просмотра журнала. Если, во время проверки, количество обрабатываемых транзакций достигает этого предела, дополнительные транзакции не будут включены в текущий результат сканирования. После завершения цикла просмотра число обработанных транзакций всегда будет меньше или равным значению параметра *maxtrans*.  
