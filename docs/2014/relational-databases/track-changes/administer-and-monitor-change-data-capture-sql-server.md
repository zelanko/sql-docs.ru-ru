 ...
 *Постоянный* параметр определяет, будет ли `sp_cdc_scan` элемент управления переключаться после очистки журнала или выполнения максимального числа циклов просмотра (один режим однократного создания). Данный параметр также управляет тем, продолжает ли выполняться хранимая процедура `sp_cdc_scan` после того, как она была явно остановлена (непрерывный режим).  
