...
-   Значения типа `sql_variant` сохраняются в файле в символьном формате без метаданных. Каждое значение данных преобразуется в `char` формате, в соответствии с правилами неявного преобразования данных. В столбец типа `sql_variant` данные импортируются как тип `char`. При импорте в столбец с типом данных, отличного от `sql_variant`, данные преобразуются из `char` с помощью неявного преобразования. Дополнительные сведения о преобразовании данных см. в статье [Преобразование типов данных (ядро СУБД)](/sql/t-sql/data-types/data-type-conversion-database-engine).  
  
-   **Bcp** экспортирует `money` значения как файлы данных символьного формата с четырьмя знаками после запятой и без группах символы как запятые-разделители. Например: для столбца типа `money`, содержащего значение 1 234 567,123456, будет выполнен массовый экспорт в файл данных в виде символьной строки «1234567,1235».  
