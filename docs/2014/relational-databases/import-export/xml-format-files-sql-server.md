...
|---------------------|-----------------|------------------------------|  
|ИДЕНТИФИКАТОР **=»*`fieldID`*»**|Задает логическое имя поля в файле данных. Идентификатор поля является ключом, используемым для обращения к полю.<br /><br /> < идентификатор поля **=»*`fieldID`*»**/ > сопоставляется < ИСТОЧНИК СТОЛБЦА **=»*`fieldID`*»**/>|Обязательно|  
...
|**NCharTerm**|`TERMINATOR`|MAX_LENGTH, COLLATION|  
 ...
 Дополнительные сведения о типах значений [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] см. в разделе [Типы данных (Transact-SQL)](/sql/t-sql/data-types/data-types-transact-sql).  
...
|----------------------|-----------------|------------------------------|  
|ИСТОЧНИК **=»*`fieldID`*»**|Задает идентификатор поля, сопоставляемого со столбцом.<br /><br /> < ИСТОЧНИК СТОЛБЦА **=»*`fieldID`*»**/ > сопоставляется < идентификатор поля **=»*`fieldID`*»**/>|Обязательно|  
