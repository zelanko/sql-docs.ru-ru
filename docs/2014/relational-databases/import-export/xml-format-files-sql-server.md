...
> [!NOTE]  
>  Для массового экспорта и импорта поддерживаются два типа файлов форматирования: *файлы форматирования, отличные от XML* , и *XML-файлы форматирования*. Они более гибкие и мощные по сравнению с файлом форматирования в формате, отличном от XML. Сведения о файлах форматирования в формате, отличном от XML, см. в разделе [Файлы формата, отличные от XML (SQL Server)](xml-format-files-sql-server.md).  
...
##  <a name="structure-of-xml-format-files"></a><a name="StructureOfXmlFFs"></a> Структура XML-файлов форматирования  
 ...
 XML-файл форматирования имеет два основных компонента \<RECORD> \<ROW> :  
  
-   \<RECORD>описывает данные, которые хранятся в файле данных.  
  
     Каждый \<RECORD> элемент содержит набор из одного или нескольких \<FIELD> элементов. Эти элементы соответствуют полям в файле данных. Базовый синтаксис:  
...
-   \<ROW>Описывает, как создавать строки данных из файла данных, когда данные из файла импортируются в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] таблицу.  
  
     \<ROW>Элемент содержит набор \<COLUMN> элементов. Эти элементы соответствуют столбцам таблицы. Базовый синтаксис:  
     ...
     Каждый \<COLUMN> элемент может быть сопоставлен только с одним полем в файле данных. Порядок \<COLUMN> элементов в \<ROW> элементе определяет порядок, в котором они возвращаются при выполнении групповой операции. XML-файл форматирования присваивает каждому \<COLUMN> элементу локальное имя, которое не имеет связи со столбцом в целевой таблице операции полного импорта.  
  
##  <a name="schema-syntax-for-xml-format-files"></a><a name="SchemaSyntax"></a> Синтаксис схемы для XML-файлов форматирования  
 Этот раздел содержит список элементов и атрибутов схемы XML для XML-файлов форматирования. Синтаксис файла форматирования не зависит от направления операции; для операций массового импорта и массового экспорта синтаксис одинаков. В этом разделе также рассматривается использование \<ROW> элементов и при импорте \<COLUMN> значения xsi: Type элемента в набор данных.  
...
###  <a name="basic-syntax-of-the-xml-schema"></a><a name="BasicSyntax"></a> Основной синтаксис схемы XML  
 В этих инструкциях синтаксиса показаны только элементы ( \<BCPFORMAT> , \<RECORD> , \<FIELD> , \<ROW> и \<COLUMN> ) и их основные атрибуты.  
...
> [!NOTE]  
>  Дополнительные атрибуты, связанные со значением xsi: Type в \<FIELD> \<COLUMN> элементе или, описаны далее в этом разделе.  
...
####  <a name="schema-elements"></a><a name="SchemaElements"></a> Элементы схемы  
 ...
 Атрибуты этого элемента обсуждаются в [атрибутах \<FIELD> элемента](#AttrOfFieldElement)далее в этом разделе.  
  
 \<ROW .../>  
 Определяет сложный элемент, содержащий один или несколько \<COLUMN> элементов. Порядок \<COLUMN> элементов не зависит от порядка \<FIELD> элементов в определении записи. Вместо этого порядок \<COLUMN> элементов в файле форматирования определяет порядок столбцов результирующего набора строк. Поля данных загружаются в том порядке, в котором соответствующие \<COLUMN> элементы объявляются в \<COLUMN> элементе.  
  
 Дополнительные сведения см. в подразделе [о том, как в ходе импорта используется \<ROW> элемент](#HowUsesROW)далее в этой статье.  
  
 \<COLUMN>  
 Определяет столбец как элемент ( \<COLUMN> ). Каждый \<COLUMN> элемент соответствует \<FIELD> элементу (идентификатор которого указан в атрибуте source \<COLUMN> элемента).  
  
 Атрибуты этого элемента обсуждаются в [атрибутах \<COLUMN> элемента](#AttrOfColumnElement)далее в этом разделе. См. также раздел Использование [ \<COLUMN> элемента массовым импортом](#HowUsesColumn)далее в этом разделе.  
  
 \</BCPFORMAT>  
 Требуется в конце файла форматирования.  
  
####  <a name="attributes-of-the-field-element"></a><a name="AttrOfFieldElement"></a>Атрибуты \<FIELD> элемента  
 В этом разделе описываются атрибуты \<FIELD> элемента, которые обобщены в следующем синтаксисе схемы:  
 ...
 Каждый \<FIELD> элемент не зависит от других. Поле описывается на основе следующих атрибутов:  
  
|Атрибут FIELD|Описание|Необязательный или<br /><br /> Обязательно|  
|---------------------|-----------------|------------------------------|  
|ID **= " *`fieldID`* "**|Задает логическое имя поля в файле данных. Идентификатор поля является ключом, используемым для обращения к полю.<br /><br /> <поле ID **= " *`fieldID`* "**/> сопоставляется с <столбца source **= " *`fieldID`* "**/>|Обязательно|  
...
#####  <a name="xsitype-values-of-the-field-element"></a><a name="XsiTypeValuesOfFIELD"></a>Значения xsi: Type \<FIELD> элемента  
...
|\<FIELD>значения xsi: Type|Обязательные XML-атрибуты<br /><br /> для типа данных|Необязательные XML-атрибуты<br /><br /> для типа данных|  
|-------------------------------|---------------------------------------------------|---------------------------------------------------|  
...
|**NCharTerm**|`TERMINATOR`|MAX_LENGTH, COLLATION|  
 ...
 Дополнительные сведения о типах данных [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] см. в разделе [Типы данных (Transact-SQL)](/sql/t-sql/data-types/data-types-transact-sql).  
  
####  <a name="attributes-of-the-column-element"></a><a name="AttrOfColumnElement"></a>Атрибуты \<COLUMN> элемента  
 В этом разделе описываются атрибуты \<COLUMN> элемента, которые обобщены в следующем синтаксисе схемы:  
...
|----------------------|-----------------|------------------------------|  
|SOURCE **= " *`fieldID`* "**|Задает идентификатор поля, сопоставляемого со столбцом.<br /><br /> <Column source **= " *`fieldID`* "**/> сопоставляется с <поля с идентификатором **= " *`fieldID`* "**/>|Обязательно|  
...
#####  <a name="xsitype-values-of-the-column-element"></a><a name="XsiTypeValuesOfCOLUMN"></a>Значения xsi: Type \<COLUMN> элемента  
 Значение xsi:type — это конструкция XML, используемая как атрибут и определяющая тип данных экземпляра элемента. Сведения по применению этой конструкции см. в пункте «Размещение значения xsi:type в наборе данных» ниже в этом разделе.  
  
 \<COLUMN>Элемент поддерживает собственные типы данных SQL, как показано ниже.  
...
###  <a name="how-bulk-import-uses-the-row-element"></a><a name="HowUsesROW"></a>Использование элемента массовым импортом \<ROW>  
 \<ROW>Элемент не учитывается в некоторых контекстах. Влияет ли \<ROW> элемент на операцию выполнения операций с массовым импортом, зависит от того, как выполняется операция.  
  
-   команда **bcp**  
  
     Когда данные загружаются в целевую таблицу, программа **bcp** игнорирует \<ROW> компонент. Вместо этого команда **bcp** загружает данные с учетом типов столбцов целевой таблицы.  
  
-   [!INCLUDE[tsql](../../../includes/tsql-md.md)] инструкции (команда BULK INSERT и поставщик массового набора строк OPENROWSET)  
  
     При выполнении операций с массовым импортом данных в таблицу [!INCLUDE[tsql](../../../includes/tsql-md.md)] инструкции используют \<ROW> компонент для создания входного набора строк. Кроме того, [!INCLUDE[tsql](../../../includes/tsql-md.md)] инструкции выполняют соответствующие преобразования типов на основе типов столбцов, указанных в столбце \<ROW> , и соответствующего столбца в целевой таблице. При несовпадении типов столбцов в файле форматирования и в целевой таблице производится дополнительное преобразование типов, Это дополнительное преобразование типа может привести к некоторым различиям (т. е. к потере точности) в работе команды BULK INSERT или поставщика массового набора строк OPENROWSET по сравнению с командой **bcp**.  
  
     Сведения в \<ROW> элементе позволяют сформировать строку без дополнительных сведений. По этой причине набор строк можно сформировать при помощи инструкции SELECT (SELECT \* FROM OPENROWSET(BULK *datafile* FORMATFILE=*xmlformatfile*).  
...
###  <a name="how-bulk-import-uses-the-column-element"></a><a name="HowUsesColumn"></a>Использование элемента массовым импортом \<COLUMN>  
 Для выполнения операций с массовым импортом данных в таблицу \<COLUMN> элементы в файле форматирования сопоставляют поле файла данных со столбцами таблицы, указывая:  
...
-   тип столбца, используемый для преобразования типа данных поля в необходимый тип данных столбца.  
 ...
 Аналогично, для выполнения операций с массовым экспортом данных из таблицы каждый \<COLUMN> в файле форматирования сопоставляет столбец из строки входной таблицы с соответствующим полем в выходном файле данных.  
...
> [!NOTE]  
>  Сведения о создании файлов форматирования см. в разделе [Создание файла форматирования (SQL Server)](create-a-format-file-sql-server.md).  
 ...
 **Файл данных (запись):** \<tab>Имя Age \<tab> Фамилия\<return>  
...
> [!NOTE]  
>  Эквивалентный пример [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] см. в разделе [Создание файла форматирования (SQL Server)](create-a-format-file-sql-server.md).  
...
> [!NOTE]  
>  Эквивалентный пример [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] см. в разделе [Использование файла форматирования для сопоставления столбцов таблицы с полями файла данных (SQL Server)](use-a-format-file-to-map-table-columns-to-data-file-fields-sql-server.md).  
  
### <a name="c-omitting-a-data-field"></a>В. пропуск поля данных;  
 ...
 **Файл данных (запись):** Возраст \<tab> EmployeeID \<tab> FirstName \<tab> LastName\<return>  
...
> [!NOTE]  
>  Эквивалентный пример [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] см. в разделе [Использование файла форматирования для пропуска поля данных (SQL Server)](use-a-format-file-to-skip-a-data-field-sql-server.md).  
  
###  <a name="d-mapping-field-xsitype-to-column-xsitype"></a><a name="MapXSItype"></a> Г. Сопоставление \<FIELD> xsi: Type с \<COLUMN> xsi: Type  
