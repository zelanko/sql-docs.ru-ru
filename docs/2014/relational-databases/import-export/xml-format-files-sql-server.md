...
##  <a name="BenefitsOfXmlFFs"></a>Преимущества XML-файлов форматирования  
...
-   
  \<RECORD> описывает способ хранения данных в файле данных.  
     ...
     Поле в файле данных может иметь фиксированную или переменную длину или завершаться определенным символом. 
...
###  <a name="BasicSyntax"></a>Базовый синтаксис схемы XML  
...
####  <a name="AttrOfFieldElement"></a>Атрибуты элемента> \<поля  
...
|-------------------------------|---------------------------------------------------|---------------------------------------------------|  
|**нативефиксед**|`LENGTH`|Нет.|  
|**нативепрефикс**|`PREFIX_LENGTH`|MAX_LENGTH|  
|**чарфиксед**|`LENGTH`|COLLATION|  
|**нчарфиксед**|`LENGTH`|COLLATION|  
|**чарпрефикс**|`PREFIX_LENGTH`|MAX_LENGTH, COLLATION|  
|**нчарпрефикс**|`PREFIX_LENGTH`|MAX_LENGTH, COLLATION|  
|**чартерм**|`TERMINATOR`|MAX_LENGTH, COLLATION|  
|**нчартерм**|`TERMINATOR`|MAX_LENGTH, COLLATION|  
  
 Дополнительные сведения о [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] типах данных см. в разделе [типы данных &#40;&#41;Transact-SQL ](/sql/t-sql/data-types/data-types-transact-sql).  
  
####  <a name="AttrOfColumnElement"></a>Атрибуты элемента> \<столбца  
...
|-------------------|---------------------------|---------------------------------------------------|---------------------------------------------------|  
|исправление|
  `SQLBIT`, `SQLTINYINT`, `SQLSMALLINT`, `SQLINT`, `SQLBIGINT`, `SQLFLT4`, `SQLFLT8`, `SQLDATETIME`, `SQLDATETIM4`, `SQLDATETIM8`, `SQLMONEY`, `SQLMONEY4`, `SQLVARIANT` и `SQLUNIQUEID`|Нет.|NULLABLE|  
|Переменное число|`SQLDECIMAL`перетаскивани`SQLNUMERIC`|Нет.|NULLABLE, PRECISION, SCALE|  
|Большой объект (LOB)|
  `SQLIMAGE`, `CharLOB`, `SQLTEXT` и `SQLUDT`|Нет.|NULLABLE|  
|Большой символьный объект (CLOB)|`SQLNTEXT`|Нет.|NULLABLE|  
|Двоичная строка|`SQLBINARY`перетаскивани`SQLVARYBIN`|Нет.|NULLABLE, LENGTH|  
|Строка символов|
  `SQLCHAR`, `SQLVARYCHAR`, `SQLNCHAR` и `SQLNVARCHAR`|Нет.|NULLABLE, LENGTH|  
...
>  Для массового экспорта или импорта данных SQLXML используйте один из следующих типов данных в файле форматирования: SQLCHAR или SQLVARYCHAR (данные отправляются в кодовой странице клиента или в кодовой странице, предполагаемой параметрами сортировки), SQLNCHAR или SQLNVARCHAR (данные отправляются в формате Юникод) и SQLBINARY или SQLVARYBIN (данные отправляются без преобразования).  
 ...
 Дополнительные сведения о [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] типах данных см. в разделе [типы данных &#40;&#41;Transact-SQL ](/sql/t-sql/data-types/data-types-transact-sql).  
  
###  <a name="HowUsesROW"></a>Как в \<ходе импорта используется элемент Row>  
...
###  <a name="HowUsesColumn"></a>Как в \<ходе импорта используется элемент Column>  
...
##  <a name="SampleXmlFFs"></a>Примеры XML-файлов форматирования  
...
> [!NOTE]  
>  Сведения о создании файлов форматирования см. в разделе [Create a Format File &#40;SQL Server&#41;](create-a-format-file-sql-server.md).  
  
###  <a name="OrderCharFieldsSameAsCols"></a>Конкретного. упорядочивание полей с символьными данными по столбцам таблицы;  
 ...
 **Таблица (строка):** Person (возраст int, FirstName varchar (20), LastName varchar (30))  
  
 **Файл данных (запись):** Вкладка\<"возраст"\<>вкладка "\<имя">"LastName Return>  
...
###  <a name="OrderFieldsAndColsDifferently"></a>&. упорядочивание полей данных в порядке, отличающемся от порядка столбцов таблицы;  
 ...
 **Файл данных** (запись): вкладка\<«Age»\<>«Lastname\<»>«FirstName Return>  
...
> [!NOTE]  
>  Эквивалентный пример [!INCLUDE[ssSampleDBobject](../../../includes/sssampledbobject-md.md)] см. в разделе [Use a Format File to Map Table Columns to Data-File Fields &#40;SQL Server&#41;](use-a-format-file-to-map-table-columns-to-data-file-fields-sql-server.md).  
 ...
 **Файл данных (запись):** Вкладка\<"возраст"\<>вкладке "\<КодСотрудника">\<вкладку "FirstName">"LastName Return>  
...
###  <a name="MapXSItype"></a>Четырехмерного. Сопоставление типа данных xsi:type элемента \<FIELD> с типом данных xsi:type элемента \<COLUMN>  
...
###  <a name="MapXMLDataToTbl"></a>&. сопоставление XML-данных с таблицей;  
...
###  <a name="ImportFixedFields"></a>Ж. импорт полей фиксированной длины и полей фиксированной ширины.  
