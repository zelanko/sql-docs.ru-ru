...
|UDT (определяемый пользователем тип данных)|SQLUDT|  
 ...
 <sup>2</sup> невозможно массово импортировать данные в `text`, `ntext`, и `image` столбцы, имеющие значения по умолчанию.  
...
-   При вводе типа файлового хранилища, представляющий недопустимое неявное преобразование, **bcp** сбое; например, хотя можно указать `int` для `smallint` данных, при указании `smallint` для `int` данных, Ошибка переполнения.  
