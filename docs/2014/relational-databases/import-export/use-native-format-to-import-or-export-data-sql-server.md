...
-   Данные типа `char` или `varchar`.  
     ...
     В начале каждого `char` или `varchar` поля, **bcp** добавляет длину префикса.  
...
-   `sql_variant` Данные  
     ...
     Если `sql_variant` данные сохраняются как SQLVARIANT в файле данных собственного формата, то все характеристики данных сохраняются. Метаданные, в которых записан тип данных каждой величины, записываются вместе со значениями данных. Эти метаданные используются для повторного создания значений данных с тем же типом данных в место назначения `sql_variant` столбца.  
...
|-------------|------------|-----------------|  
|**bcp**|**-n**|Вызывает **bcp** программы для использования собственных типов данных.<sup> 1</sup>|  
