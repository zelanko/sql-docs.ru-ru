...
# <a name="register-a-database-as-a-dac"></a>Регистрация базы данных в качестве приложения уровня данных
  Используйте **Мастер регистрации приложения уровня данных** или сценарий Windows PowerShell, чтобы создать определение приложения уровня данных (DAC), которое описывает объекты в существующей базе данных, и зарегистрируйте определение DAC в системной базе данных `msdb` ( **главный** в [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]).  
 ...
 **Описание.** — необязательно. Текст, поясняющий назначение DAC. При развертывании приложения уровня данных описание хранится в базе данных `msdb` и затем может быть просмотрено в узле **приложения на уровне данные** в [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)].  
  
 **\< Previous** — возврат на страницу **Введение** .  
 ...
 **\< Previous** — возврат к странице **Установка свойств** для изменения записей.  
