 ...
 **Версия (в формате xx.xx.xx.xx, где x — цифра)** — числовое значение, которое указывает версию приложения уровня данных. Версия DAC используется в среде Visual Studio для определения версии DAC, над которой работают разработчики. При развертывании DAC версия хранится в базе данных `msdb` и затем может быть просмотрена в узле **приложения уровня данных** в [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)].  
  
 **Описание:** — необязательно. Описывает DAC. При развертывании приложения уровня данных описание хранится в базе данных `msdb` и затем может быть просмотрено в узле **приложения на уровне данные** в [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)].  
