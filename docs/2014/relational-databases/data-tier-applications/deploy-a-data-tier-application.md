...
-   **Развертывание DAC с помощью:**  [Мастер развертывания приложения уровня данных](#UsingDeployDACWizard), [PowerShell](#DeployDACPowerShell)  
...
###  <a name="DBOptSettings"></a>Параметры и параметры базы данных  
 ...
 **Следующая >** — переход на страницу **Выбор пакета приложения уровня** данных.  
 ...
 **Пакет DAC** — укажите путь и имя файла пакета DAC, содержащего приложение уровня данных для развертывания. Чтобы указать расположение пакета DAC, можно нажать кнопку **Обзор** в правой части окна.  
  
 **Имя приложения** — доступное только для чтения поле, в котором отображается имя DAC, присвоенное при создании или извлечении DAC из базы данных.  
  
 **Версия** — доступное только для чтения поле, в котором отображается версия, назначенная при создании или извлечении DAC из базы данных.  
  
 **Описание** — доступное только для чтения поле, в котором отображается описание, записанное при создании или извлечении DAC из базы данных.  
  
 Назад — возврат на страницу **Введение** . ** \< **  
  
 **Следующий >** — отображает индикатор выполнения, когда мастер подтверждает, что выбранный файл является допустимым пакетом DAC.  
 ...
 **Проверка содержимого DAC** — индикатор выполнения, который сообщает о текущем состоянии процесса проверки.  
  
 Назад — возврат к начальному состоянию страницы **Выбор пакета** . ** \< **  
  
 **Следующий >** — переход к окончательной версии страницы **Выбор пакета** .  
  
 **Отмена** — работа мастера завершается без развертывания приложения уровня данных.  
  
##  <a name="Review_policy"></a>Страница «Проверка политики»  
 ...
 **Результаты оценки условий политики** — отчет только для чтения, показывающий, были ли условия политики развертывания DAC выполнены. Результаты проверки каждого из условий выводятся отдельной строкой.  
 ...
 **Игнорировать нарушения политики** — используйте этот флажок, чтобы продолжить развертывание, если не удалось выполнить одно или несколько условий политики. Этот параметр следует выбирать только при уверенности, что ни одно из невыполненных условий не будет препятствовать успешной работе DAC.  
  
 Назад — возврат на страницу **Выбор пакета** . ** \< **  
  
 **Следующая >** — переход на страницу **настройки обновления** .  
 ...
 **Использовать расположение базы данных по умолчанию** — выберите этот параметр, чтобы создать файлы данных и журналов базы данных в расположении по умолчанию [!INCLUDE[ssDE](../../includes/ssde-md.md)]для экземпляра. Имена файлов будут сформированы с использованием имени базы данных.  
  
 **Указание файлов базы данных** — выберите этот параметр, чтобы указать другое расположение или имя для файлов данных и журналов.  
  
 **Путь и имя файла данных:-** Укажите полный путь и имя файла данных. В этом поле содержится путь и имя файла по умолчанию. Измените строку в этом поле, чтобы изменить значение по умолчанию, либо с помощью кнопки «Обзор» перейдите в папку, в которую следует поместить файл данных.  
 ...
 **Развертывание DAC** — сообщает об успешном или неуспешном завершении каждого действия, предпринятого для развертывания DAC. Просмотрите эти сведения, чтобы выяснить результаты каждого действия. Для каждого действия, в котором обнаружена ошибка, предусмотрена ссылка в столбце **Результат** . Выберите эту ссылку, чтобы просмотреть отчет об ошибках, относящихся данному действию.  
  
 **Сохранить отчет** — нажмите эту кнопку, чтобы сохранить отчет о развертывании в HTML-файл. В этом файле содержится отчет о состоянии каждого из действий, в том числе все выданные сообщения об ошибках. По умолчанию используется папка «SQL Server Management Studio\DAC Packages», вложенная в папку Documents рабочего каталога учетной записи пользователя Windows.  
