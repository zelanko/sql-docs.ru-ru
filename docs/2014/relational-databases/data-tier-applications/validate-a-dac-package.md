...
4.  Используйте `GetDatabaseChanges()` метод для извлечения `ChangeResults` объекта и канал объекта в текстовый файл, чтобы сформировать простой отчет о новых, удаленных и измененных объектах.  
...
5.  Используйте `GetIncrementalUpgradeScript()` метод для получения списка инструкций Transact-SQL, обновление будет запустить и передать список в текстовый файл.  
