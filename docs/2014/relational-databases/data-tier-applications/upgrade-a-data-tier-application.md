...
###  <a name="choosing-dac-upgrade-options"></a><a name="ChoseDACUpgOptions"></a> выбор параметров обновления DAC  
...
-   **Блокировать при изменении** . Если `True` значение равно, то обновление будет завершено, если схема базы данных отличается от схемы, определенной в предыдущем приложении DAC. Если этот параметр равен `False`, обновление продолжится даже в случае обнаружения изменений. Значение по умолчанию — `False`.  
  
-   **Откат в случае сбоя** — если `True` Обновление заключается в транзакцию, а при возникновении ошибок будет предпринята попытка отката. Если этот параметр равен `False`, все изменения фиксируются по мере внесения, а в случае сбоя придется восстанавливать предыдущую версию базы данных с резервной копии. Значение по умолчанию — `False`.  
  
-   **Пропустить проверку политики** — `True` значение, если политика выбора сервера DAC не оценивается. Если этот параметр равен `False`, проверка политики производится, а в случае обнаружения несоответствия обновление прекращается. Значение по умолчанию — `False`.  
 ...
 ** \< Назад** — возврат на страницу **Проверка плана обновления** .  
