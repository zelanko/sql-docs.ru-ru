...
-   **Пропускать потерю данных** — Если `True`, обновление продолжится, даже если некоторые операции приведут к потере данных. если этот параметр равен `False`, обновление прекращается при обнаружении таких операций. Например, если таблицы в текущей базе данных отсутствует в схеме нового приложения уровня данных, таблица будет удалена `True` указано. Значение по умолчанию — `True`.  
  
-   **Блокировать изменения** — Если `True`, обновление прекращается, если схема базы данных отличается от, определенные в предыдущей версии DAC. Если `False`, обновление продолжится даже в случае обнаружения изменений. Значение по умолчанию — `False`.  
  
-   **Откат при сбое** — Если `True`, обновления заключается в транзакции, и при обнаружении ошибки будет попытка отката изменений. Если этот параметр равен `False`, все изменения фиксируются по мере внесения, а в случае сбоя придется восстанавливать предыдущую версию базы данных с резервной копии. Значение по умолчанию — `False`.  
  
-   **Пропустить проверку политики** — Если `True`, политики выбора сервера DAC не вычисляется. Если `False`, применяется политика, и обновление завершается, если ошибка проверки. Значение по умолчанию — `False`.  
 ...
 **\< Предыдущие** -возвращается в исходное состояние **Выбор пакета** страницы.  
 ...
 **\< Предыдущие** -возвращает **Выбор пакета** страницы.  
 ...
 **\< Предыдущие** -возвращает **Выбор пакета DAC** страницы.  
 ...
 **\< Предыдущие** -возвращает **обнаружение изменений** страницы.  
 ...
 **\< Предыдущие** — возврат на **проверить план обновления** страницы.  
...
4.  Используйте `add_DacActionStarted` и `add_DacActionFinished` подписаться на события обновления приложения уровня данных.  
  
5.  Задать `DacUpgradeOptions`.  
  
6.  Используйте `IncrementalUpgrade` метод для обновления приложения уровня данных.  
