...
-   **Блокировать при изменении** . Если `True`значение равно, то обновление будет завершено, если схема базы данных отличается от схемы, определенной в предыдущем приложении DAC. Если этот параметр равен `False`, обновление продолжится даже в случае обнаружения изменений. Значение по умолчанию равно `False`.  
...
-   **Пропустить проверку политики** — значение `True`, если политика выбора сервера DAC не оценивается. Если этот параметр равен `False`, проверка политики производится, а в случае обнаружения несоответствия обновление прекращается. Значение по умолчанию равно `False`.  
