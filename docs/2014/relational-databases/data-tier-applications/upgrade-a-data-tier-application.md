...
-   **Откат в случае сбоя** — `True`если обновление заключается в транзакцию, а при возникновении ошибок будет предпринята попытка отката. Если этот параметр равен `False`, все изменения фиксируются по мере внесения, а в случае сбоя придется восстанавливать предыдущую версию базы данных с резервной копии. Значение по умолчанию — `False`.  
  
-   **Пропустить проверку политики** — значение `True`, если политика выбора сервера DAC не оценивается. Если этот параметр равен `False`, проверка политики производится, а в случае обнаружения несоответствия обновление прекращается. Значение по умолчанию — `False`.  
