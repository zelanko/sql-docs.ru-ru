...
-   **Пропускать потерю данных** — Если `True`, обновление продолжится, даже если некоторые операции приведут к потере данных. если этот параметр равен `False`, обновление прекращается при обнаружении таких операций. Например, если таблицы в текущей базе данных отсутствует в схеме нового приложения уровня данных, таблица будет удалена `True` указан. Значение по умолчанию — `True`.  
  
-   **Блокировать при изменениях** — Если `True`, обновление прекращается в том случае, если схема базы данных отличается от значения, определенные в предыдущей версии DAC. Если `False`, обновление продолжится даже в случае обнаружения изменений. Значение по умолчанию — `False`.  
...
-   **Пропустить проверку политики** — Если `True`, политики выбора сервера DAC не вычисляется. Если `False`, применяется политика и обнаружения несоответствия обновление прекращается при возникновении ошибки проверки. Значение по умолчанию — `False`.  
