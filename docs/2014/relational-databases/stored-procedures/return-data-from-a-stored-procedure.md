...
### <a name="using-the-cursor-data-type-in-output-parameters"></a>Использование типа данных Cursor в выходных параметрах  
 [!INCLUDE[tsql](../../../includes/tsql-md.md)] можно использовать процедуры `cursor` типа данных только для параметров OUTPUT. Если `cursor` указан тип данных для параметра, для этого параметра в определении процедуры необходимо указать ключевые слова VARYING и OUTPUT. Параметр может быть указан только как ВЫХОДНОЙ, однако если в объявлении параметра указано ключевое слово VARYING, тип данных должен быть `cursor` и также должно быть указано ключевое слово OUTPUT.  
  
> [!NOTE]  
>  `cursor` Тип данных не может связываться с переменными приложения через API-интерфейсы, такие как OLE DB, ODBC, ADO и DB-Library базы данных. Поскольку ВЫХОДНЫЕ параметры должны быть привязаны прежде, чем приложение может выполнить процедуру, хранимые процедуры с `cursor` ВЫХОДНЫЕ параметры не может вызываться из API базы данных. Эти процедуры могут быть вызваны из [!INCLUDE[tsql](../../../includes/tsql-md.md)] пакетов и процедур или триггеров, только если `cursor` ВЫХОДНОЙ переменной присваивается [!INCLUDE[tsql](../../../includes/tsql-md.md)] локального `cursor` переменной.  
...
### <a name="examples-of-cursor-output-parameters"></a>Примеры выходных параметров курсора  
 В следующем примере создается процедура, указывает выходной параметр, `@currency`_`cursor` с помощью `cursor` тип данных. Процедура затем будет вызвана из пакета.  
