...
> [!NOTE]
>  Если в форме ** \@параметр =**_значение_указано одно значение параметра, все последующие параметры должны быть заданы таким образом. Если значения параметров не передаются в формате ** \@параметр =**_значение_, значения должны быть указаны в одинаковом порядке (слева направо), так как параметры перечислены в инструкции CREATE PROCEDURE.  
> 
> [!WARNING]
>  Любой параметр, переданный в ** \@параметре формы =**_value_ с параметром с ошибкой, приведет [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] к формированию ошибки и предотвращению выполнения процедуры.  
...
```  
 ...
 Процедура `usp_GetList` возвращает из базы данных [!INCLUDE[ssSampleDBCoShort](../../includes/sssampledbcoshort-md.md)] список товаров (велосипедов) стоимостью менее $ 700. Выходные параметры ** \@Cost** и ** \@compareprices используются совместно** используются с языком управления потоком для возврата сообщения в окне **сообщения** .  
  
> [!NOTE]  
>  Переменная OUTPUT должна быть определена во время создания процедуры, а также в ходе использования переменной. Имена параметра и переменной не должны совпадать. Однако тип данных и положение параметра должны совпадать (если __ ** \@** не используется параметр ListPrice = variable).  
