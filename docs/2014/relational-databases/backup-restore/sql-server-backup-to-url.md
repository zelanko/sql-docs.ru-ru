...
-   [Использование задачи резервного копирования в SQL Server Management Studio](sql-server-backup-to-url.md#BackupTaskSSMS)  
  
-   [SQL Server резервного копирования на URL-адрес с помощью мастера планов обслуживания](sql-server-backup-to-url.md#MaintenanceWiz)  
...
-   При создании контейнера для службы хранилища BLOB-объектов Azure рекомендуется установить доступ к **частному**. Установка закрытых прав доступа означает, что доступ получают только те пользователи и учетные записи, которые могут предоставить необходимую информацию для проверки подлинности в учетной записи Azure.  
    ...
    > [!IMPORTANT]  
    >  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]требует, чтобы имя учетной записи Azure и проверка подлинности [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ключей доступа хранились в учетных данных. Эти сведения используются для проверки подлинности учетной записи Azure при выполнении операций резервного копирования или восстановления.  
 ...
 **Контейнер:** Контейнер предоставляет Группирование набора больших двоичных объектов и может хранить неограниченное количество больших двоичных объектов. Чтобы создать [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] резервную копию в службе BLOB-объектов Azure, необходимо создать по крайней мере корневой контейнер.  
...
###  <a name="Support"></a>Поддержка инструкций Backup/Restore  
...
2.  На странице Общие параметр **URL-адрес** используется для создания резервной копии в службе хранилища Azure. Если выбран этот параметр, на странице будут показаны другие параметры.  
    ...
    2.  **Учетные данные SQL:** Можно либо указать существующие учетные данные SQL Server, либо создать новый, щелкнув **создать** рядом с полем учетные данные SQL.  
    ...
    4.  **Префикс URL-адреса:** Эта процедура создается автоматически с использованием сведений, указанных в полях, описанных в предыдущих шагах. При изменении этого значения вручную убедитесь в том, что оно соответствует прочим данным, указанным ранее. Например, при изменении URL-адреса хранения убедитесь, что учетные данные SQL заданы для проверки подлинности в той же самой учетной записи хранения.  
 ...
 [Страница "резервное копирование базы данных &#40;параметров носителя"&#41;](back-up-database-media-options-page.md)  
...
##  <a name="MaintenanceWiz"></a>SQL Server резервного копирования на URL-адрес с помощью мастера планов обслуживания  
...
-   [Резервное копирование всей базы данных](#complete)  
...
###  <a name="PITR"></a>Восстановление на момент времени с помощью предложения STOPAT  
