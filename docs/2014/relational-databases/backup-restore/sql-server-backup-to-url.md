...
-   При создании контейнера для службы хранилища BLOB-объектов Azure рекомендуется установить доступ к **частному**. Установка закрытых прав доступа означает, что доступ получают только те пользователи и учетные записи, которые могут предоставить необходимую информацию для проверки подлинности в учетной записи Azure.  
    ...
    > [!IMPORTANT]  
    >  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]требует, чтобы имя учетной записи Azure и проверка подлинности ключей доступа хранились в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] учетных данных. Эти сведения используются для проверки подлинности учетной записи Azure при выполнении операций резервного копирования или восстановления.  
...
###  <a name="azure-blob-storage-service"></a><a name="Blob"></a>Служба хранилища BLOB-объектов Azure  
...
>  Если вы решили скопировать и передать файл резервной копии в службу хранилища больших двоичных объектов Azure, используйте страничный BLOB-объект в качестве хранилища. Восстановление из блочных больших двоичных объектов не поддерживается. Попытка выполнить RESTORE из большого двоичного объекта блочного типа приводит к ошибке.  
 ...
 Ниже приведен пример значения URL-адреса: HTTP [s]://Аккаунтнаме.блоб.коре.Виндовс.нет/ \<CONTAINER> / \<FILENAME.bak> . Указывать HTTPS необязательно, но рекомендуется.  
...
##  <a name="using-backup-task-in-sql-server-management-studio"></a><a name="BackupTaskSSMS"></a>Использование задачи резервного копирования в SQL Server Management Studio  
...
##  <a name="sql-server-backup-to-url-using-maintenance-plan-wizard"></a><a name="MaintenanceWiz"></a>SQL Server резервного копирования на URL-адрес с помощью мастера планов обслуживания  
...
##  <a name="restoring-from-azure-storage-using-sql-server-management-studio"></a><a name="RestoreSSMS"></a>Восстановление из хранилища Azure с помощью SQL Server Management Studio  
...
###  <a name="backing-up-a-complete-database"></a><a name="complete"></a>Резервное копирование всей базы данных  
...
###  <a name="backing-up-the-database-and-log"></a><a name="databaselog"></a>Резервное копирование базы данных и журнала  
...
###  <a name="creating-a-full-file-backup-of-the-primary-filegroup"></a><a name="filebackup"></a>Создание полной резервной копии файла первичной файловой группы  
...
###  <a name="creating-a-differential-file-backup-of-the-primary-filegroup"></a><a name="differential"></a>Создание разностной резервной копии файлов первичной файловой группы  
...
###  <a name="restore-a-database-and-move-files"></a><a name="restoredbwithmove"></a>Восстановление базы данных и перемещение файлов  
