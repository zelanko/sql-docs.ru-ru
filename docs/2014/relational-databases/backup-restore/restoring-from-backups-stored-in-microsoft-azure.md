...
### <a name="optimizing-restores"></a>Оптимизация восстановления  
 ...
 Также при выполнении восстановления можно включить флаг трассировки 3051, что приведет к формированию подробного журнала. Этот файл журнала помещается в каталог журнала и имеет имя в формате: BackupToUrl -\<имя_экземпляра >-\<dbname > - action -\<PID >. log. В файле журнала сохраняются сведения о каждом обмене данными с хранилищем Windows Azure, включая время операции, что может быть полезно при диагностике проблемы.  
