...
-   В настоящее время для [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] экземпляров, выполняющихся на виртуальной машине Azure, резервное копирование в службы хранилища BLOB-объектов Azure можно выполнить, создав подключенные диски. Однако существует ограничение на количество дисков, которые можно подключить к виртуальной машине Azure. Оно равно 16 для очень крупного экземпляра и меньше этого значения для экземпляров меньшего размера. Включив прямую архивацию в хранилище BLOB-объектов Azure, можно обойти ограничение в 16 дисков.  
...
-   Удобная оплата. Оплата только тех услуг, которые используются. Может быть экономичным вариантом внешнего архива и архива резервных копий. Дополнительные сведения и ссылки см. в разделе [вопросы выставления счетов в Azure](#Billing) .  
 ...
 **Хранилище:** Плата взимается по используемому пространству и вычисляется на основе градиентного масштабирования и уровня избыточности. Дополнительные сведения и обновленную информацию см. в разделе **Управление данными** статьи [Расчет цен](https://go.microsoft.com/fwlink/?LinkId=277059) .  
