...
> [!NOTE]  
>  Дополнительные сведения о резервном копировании SQL Server в службе хранилища больших двоичных объектов Windows Azure см, [SQL Server Backup and Restore with Windows Azure Blob Storage Service](sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md).  
...
>  Для предотвращения потери данных в случае повреждения носителя (ленты или файла на диске), наборы носителей можно сделать зеркальными. Дополнительные сведения см. в подразделе [Зеркальные наборы носителей резервных копий (SQL Server)](mirrored-backup-media-sets-sql-server.md).  
 ...
 [!INCLUDE[ssEnterpriseEd10](../../includes/sskatmai-md.md)] или более поздней версии, могут считывать сжатые резервные копии. Дополнительные сведения см. в разделе [Сжатие резервных копий (SQL Server)](backup-compression-sql-server.md).  
...
-   Содержит ли описание этого носителя метку MTF или описание носителя.  
    ...
    > [!NOTE]  
    >  Во всех носителях, использующихся для операций резервного копирования или восстановления, применяется стандартный формат резервных копий вызывается [!INCLUDE[msCoName](../../includes/ssnoversion-md.md)] сохраняет любую метку носителя MTF, записанную другим приложением, но не записывает метки носителей MTF.  
...
### <a name="backup-sets"></a>Резервные наборы данных  
 ...
 В следующем примере показан [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкцию, которая создает набор носителей с именем `MyAdvWorks_MediaSet_1` для [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] базы данных, используя три ленточных накопителей в качестве устройства резервного копирования:  
...
####  <a name="Appending"></a> Присоединение к существующим резервным наборам  
...
> [!IMPORTANT]  
>  Начиная с версии [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)], параметр MEDIAPASSWORD не поддерживается при создании резервных копий. Но все еще можно восстанавливать резервные копии, созданные с паролями.  
 ...
 **Чтобы создать новый носитель набор**  
 ...
 **Присоединение новой резервной копии к существующему носителю**  
 ...
 **Для просмотра номеров последовательности последовательности и семейства носителей**  
 ...
 **Считывание заголовка носителя носителя на устройстве резервного копирования**  
