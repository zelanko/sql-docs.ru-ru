...
> [!NOTE]  
>  Если задача восстановления выполняется в среде [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], можно создать соответствующий сценарий [!INCLUDE[tsql](../../includes/tsql-md.md)][RESTORE](/sql/t-sql/statements/restore-statements-transact-sql), нажав **Скрипт** и выбрав место назначения для скрипта.  
