...
1.  **Определение больших двоичных объектов с арендой:** Если у вас есть сценарий или процесс, выполняющий процессы резервного копирования, вы можете записать ошибку в сценарий или процесс и использовать его для очистки больших двоичных объектов.   Можно также использовать свойства LeaseStats и LeastState, чтобы выделить большие двоичные объекты с арендой. После определения больших двоичных объектов рекомендуется просмотреть список, проверить работоспособность файла резервной копии перед удалением большого двоичного объекта.  
  
2.  **Прерывание аренды:** Полномочный запрос может нарушить аренду без предоставления идентификатора аренды. Дополнительные сведения см. [здесь](https://go.microsoft.com/fwlink/?LinkID=275664) .  
...
3.  **Удаление большого двоичного объекта:** Чтобы удалить большой двоичный объект с активной арендой, необходимо сначала разорвать аренду.  
  
###  <a name="Code_Example"></a>Пример сценария PowerShell  
...
2.  При отсутствии больших двоичных объектов с заблокированными арендами появится следующее сообщение:  
     ...
     **Нет BLOB-объектов с заблокированным состоянием аренды**  
  
     При наличии больших двоичных объектов с заблокированными арендами появится следующее сообщение:  
  
     **Прерывание аренды**  
  
     **Аренда \<URL-адреса> большого двоичного объекта — это аренда восстановления. это сообщение появляется только в том случае, если у вас есть большой двоичный объект с арендой восстановления, которая еще активна.**  
  
     **Аренда \<URL-адреса> большого двоичного объекта не является арендой восстановления, которая \<применяет аренду на url-адресе> Боба.**  
