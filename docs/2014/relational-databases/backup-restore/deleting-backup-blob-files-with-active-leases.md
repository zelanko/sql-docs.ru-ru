 ...
 **\*\* Важные \* \***  при запуске PowerShell 2.0 могут возникнуть проблемы при загрузке сборки Microsoft WindowsAzure.Storage.dll. Рекомендуется обновить Powershell до версии 3.0, чтобы решить проблему. Можно также использовать следующее решение для PowerShell 2.0:  
