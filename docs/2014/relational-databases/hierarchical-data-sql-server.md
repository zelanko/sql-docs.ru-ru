...
-   Приложение для управления параллелизмом, создания и присвоения само `hierarchyid` значения. Нет никакой гарантии, что значения `hierarchyid` уникальны, если приложение не использует ограничение уникального ключа или не обеспечивает уникальность своей логикой.  
...
-   Перемещение неконечных узлов выполняется медленнее с `hierarchyid`.  
...
-   Размер ключа очень важен. Для одинаковое количество узлов `hierarchyid` значение, равное или больше, чем из целочисленных типов (`smallint`, `int`, `bigint`) значение. Это обусловлено причине можно использовать родители-потомки в редких случаях `hierarchyid` имеет значительно лучше, Населенный пункт сложности ввода-вывода и ЦП, чем обобщенные табличные выражения, необходимые при использовании структуры родители-потомки.  
...
-   Структура неконечных поддеревьев часто меняется, и очень важна производительность. В иерархической структуре «родители-потомки» изменение местоположения строки иерархии касается только одной строки. Изменение местоположения строки в `hierarchyid` влияет *n* строк, где *n* количество узлов в перемещаемом поддереве перемещается.  
...
### <a name="xml"></a>XML  
 XML-документ является деревом, поэтому один экземпляр типа данных XML может представлять всю структуру. В [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] при создании XML-индекса, `hierarchyid` values are used вternally для представления положения в иерархии.  
