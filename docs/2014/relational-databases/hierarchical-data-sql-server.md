...
ms.locfileid: "36087608"
---
# <a name="hierarchical-data-sql-server"></a>Иерархические данные (SQL Server)
  Встроенная `hierarchyid` тип данных упрощает хранение и запрос иерархических данных. `hierarchyid` оптимизирован для представления деревьев, которые являются наиболее распространенным типом иерархических данных.  
...
##  <a name="keyprops"></a> Основные свойства типа hierarchyid  
 Значение `hierarchyid` тип данных представляет позицию в древовидной иерархии. Значения `hierarchyid` обладают следующими свойствами.  
...
-   Сравнение проводится в порядке приоритета глубины  
     ...
     Заданы два `hierarchyid` значения **a** и **b**, **< b** означает появляется раньше значения b в глубину обхода дерева. Индексы для типов данных `hierarchyid` располагаются в порядке приоритета глубины, и узлы, встречающиеся рядом при проходе по дереву с приоритетным направлением глубины, хранятся рядом друг с другом. Например, потомки некоторой записи хранятся рядом с этой записью.  
...
-   Это приложение для управления параллелизмом, создания и присвоения `hierarchyid` значения. Нет никакой гарантии, что значения `hierarchyid` уникальны, если приложение не использует ограничение уникального ключа или не обеспечивает уникальность своей логикой.  
  
-   Иерархические связи, представленные `hierarchyid` значения не применяются как связи по внешнему ключу. Можно, а иногда и удобно иметь иерархическую связь, в которой у A есть потомок B и когда A удаляется, у B остается связь с несуществующей записью. Если это неприемлемо, приложение должно запросить потомков, прежде чем удалять родителей.  
...
-   Запросы по поддеревьям выполняются значительно быстрее `hierarchyid`.  
  
-   Запросы по прямым потомкам обрабатываются немного медленнее, если используется тип `hierarchyid`.  
  
-   Перемещение неконечных узлов, выполняются медленнее `hierarchyid`.  
...
-   Размер ключа очень важен. Для того же количества узлов `hierarchyid` значение, равное или больше, чем из целочисленных типов (`smallint`, `int`, `bigint`) значение. Это причине можно использовать родители-потомки в редких случаях, поскольку `hierarchyid` имеет значительно более оптимальному расположению ввода-вывода и ЦП сложности чем обобщенных табличных выражений, необходимые при использовании структуры родители-потомки.  
...
-   Структура неконечных поддеревьев часто меняется, и очень важна производительность. В иерархической структуре «родители-потомки» изменение местоположения строки иерархии касается только одной строки. Изменение местоположения строки в `hierarchyid` влияет *n* строк, где *n* количество узлов в перемещаемом перемещается.  
...
### <a name="xml"></a>XML  
 XML-документ является деревом, поэтому один экземпляр типа данных XML может представлять всю структуру. В [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] при создании XML-индекса `hierarchyid` значения используются для представления положения в иерархии.  
...
###  <a name="migrating"></a> Переход со структуры «родители-потомки» на тип hierarchyid  
 Большинство деревьев представлены методом «родители-потомки». Самый простой способ перехода со структуры родители-потомки в таблице с помощью `hierarchyid` заключается в использовании временный столбец или временную таблицу для отслеживания количества узлов на каждом уровне иерархии. Пример преобразования таблицы с организацией "родители-потомки" см. в статье [Учебник. Использование типа данных hierarchyid](../relational-databases/tables/tutorial-using-the-hierarchyid-data-type.md)(урок 1).  
  
  
###  <a name="BKMK_ManagingTrees"></a> Управление древовидной структурой с помощью hierarchyid  
 Несмотря на то что `hierarchyid` столбца не обязательно представляет дерево, приложение легко может обеспечить это.  
