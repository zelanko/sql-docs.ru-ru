...
##  <a name="limits"></a> Ограничения типа данных hierarchyid  
 Тип `hierarchyid` данных имеет следующие ограничения.  
...
###  <a name="findclr"></a> Поиск предков с помощью среды CLR  
 Одной из частых операций, в которых участвуют два узла из иерархии, является нахождение ближайшего общего предка. Это может быть написано либо [!INCLUDE[tsql](../includes/tsql-md.md)] в, либо в среде CLR `hierarchyid` , так как тип доступен в обеих средах. Рекомендуется использовать среду CLR, поскольку она обеспечивает большую производительность.  
