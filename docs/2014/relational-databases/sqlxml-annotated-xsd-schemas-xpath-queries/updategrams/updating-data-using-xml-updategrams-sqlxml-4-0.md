...
-   Для ключевых полей в диаграмме обновления указано одно или несколько значений.  
 ...
 Если либо выполняется, то диаграмма обновления использует ключевые столбцы, указанные в `sql:key-fields` для сопоставления элементов в  **\<перед >** и  **\<после >** блоков.  
 ...
 Записи, которые определены в  **\<перед >** и  **\<после >** блоки не должны обязательно находиться в том же порядке. `updg:id` Атрибут приводит связь между элементами, которые указаны в  **\<перед >** и  **\<после >** блоков.  
  
 Если указать один элемент в  **\<перед >** блок и только один соответствующий элемент в  **\<после >** блока с помощью `updg:id` не требуется. Во избежание недоразумений рекомендуется указывать `updg:id` в любом случае.  
...
```  
 ...
 Обратите внимание как `updg:id` пар «атрибут», первый экземпляр \<HumanResources.Shift > элемент в  **\<перед >** блок с второй экземпляр \< HumanResources.Shift > элемент в  **\<после >** блока.  
