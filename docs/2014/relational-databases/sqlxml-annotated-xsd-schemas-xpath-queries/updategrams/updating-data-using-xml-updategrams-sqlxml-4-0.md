...
## <a name="using-the-updgid-attribute"></a>Использование атрибута updg:id  
 При указании нескольких элементов в блоках ** \< Before>** и ** \< After>** используйте `updg:id` атрибут для пометки строк в блоках ** \< Before>** и ** \< After>** . Логика обработки использует эти сведения, чтобы определить, какая запись в парах " ** \< до>** блока" с какой записью в блоке ** \< After>** .  
...
-   Для ключевых полей в диаграмме обновления указано одно или несколько значений.  
 ...
 Если это так, диаграмма обновления использует ключевые столбцы, заданные в, `sql:key-fields` для связывания элементов в блоках ** \< Before>** и ** \< After>** .  
 ...
 Записи, определенные в блоках ** \< Before>** и ** \< After>** , не обязательно должны находиться в том же порядке. `updg:id`Атрибут принудительно вызывает связь между элементами, указанными в блоках ** \< Before>** и ** \< After>** .  
  
 Если указать один элемент в блоке ** \< Before>** и только один соответствующий элемент в блоке ** \< After>** , использование `updg:id` не требуется. Во избежание недоразумений рекомендуется указывать `updg:id` в любом случае.  
...
-   Также добавляется имя новой смены, «Late Morning», которая начинается в 10:00.  
 ...
 В диаграмма обновления `updg:id` атрибут создает связи между элементами в блоках ** \< Before>** и ** \< After>** .  
...
```  
 ...
 Обратите внимание, что `updg:id` атрибут связывает первый экземпляр \< элемента HumanResources. Shift> в блоке ** \< Before>** со вторым экземпляром \< элемента> HumanResources. Shift в блоке ** \< After>** .  
