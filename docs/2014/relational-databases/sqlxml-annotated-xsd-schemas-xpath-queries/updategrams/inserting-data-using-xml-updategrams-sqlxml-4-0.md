 ...
 **\<before>** Блок можно опустить для операции вставки. Если необязательный `mapping-schema` атрибут не указан, то объект **\<ElementName>** , указанный в диаграмма обновления, сопоставляется с таблицей базы данных, а дочерние элементы или атрибуты сопоставляются со столбцами в таблице.  
...
## <a name="updgguid-attribute"></a>Атрибут updg:guid  
 Атрибут `updg:guid` является необязательным атрибутом, формирующим идентификатор GUID. Это значение остается в области для всего **\<sync>** блока, в котором он указан. Это значение можно использовать в любом месте **\<sync>** блока. Атрибут вызывает `NEWGUID()` [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] функцию для создания уникального идентификатора.  
...
```  
 ...
 Диаграмма обновления указывает на `xsi:nil` **\<fname>** элемент в **\<after>** блоке. Поэтому при выполнении этой диаграммы обновления значение NULL вставляется для столбца first_name таблицы.  
