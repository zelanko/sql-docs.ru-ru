...
```  
 ...
 Можно опустить тег ** \<After>** , если диаграмма обновления выполняет только операцию удаления. Если необязательный `mapping-schema` атрибут не указан, то ** \<>ElementName** , указанный в диаграмма обновления, сопоставляется с таблицей базы данных, а дочерние элементы или атрибуты сопоставляются со столбцами в таблице.  
...
### <a name="a-deleting-a-record-by-using-an-updategram"></a>А) Удаление записи с помощью диаграммы обновления  
