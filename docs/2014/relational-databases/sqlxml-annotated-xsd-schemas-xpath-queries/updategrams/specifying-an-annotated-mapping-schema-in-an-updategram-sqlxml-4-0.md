...
```  
 ...
 Чтобы указать логике диаграмма обновления на обратную связь "родители-потомки" во время операции `inverse` обновления, заметка задается для элемента ** \<связи>** . В итоге записи будут сначала добавляться в таблицу Cust, а затем в таблицу Ord, и операция завершится успешно.  
