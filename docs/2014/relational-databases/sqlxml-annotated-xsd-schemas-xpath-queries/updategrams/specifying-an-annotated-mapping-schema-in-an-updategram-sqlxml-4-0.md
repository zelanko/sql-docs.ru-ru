...
# <a name="specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-40"></a>Определение схемы с заметками сопоставления в диаграмме обновления (SQLXML 4.0)
 ...
 Чтобы указать схему сопоставления, используйте `mapping-schema` атрибут элемента ** \< Sync>** . В следующих примерах показаны две диаграммы обновления: в одной используется простая схема сопоставления, а в другой — более сложная схема сопоставления.  
...
```  
 ...
 Следующая диаграмма обновления использует эту схему XSD для добавления новой записи сведений о заказе (элемент ** \< OD>** в блоке ** \< after>** ) для заказа 43860. Атрибут `mapping-schema` служит для указания схемы сопоставления, которая будет использоваться диаграммой обновления.  
...
```  
 ...
 Чтобы указать логике диаграмма обновления на обратную связь "родители-потомки" во время операции обновления, `inverse` заметка задается для элемента ** \< связи>** . В итоге записи будут сначала добавляться в таблицу Cust, а затем в таблицу Ord, и операция завершится успешно.  
