...
# <a name="specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-40"></a>Определение схемы с заметками сопоставления в диаграмме обновления (SQLXML 4.0)
 ...
 Чтобы указать схему сопоставления, используйте `mapping-schema` атрибут  **\<синхронизации >** элемент. В следующих примерах показаны две диаграммы обновления: в одной используется простая схема сопоставления, а в другой — более сложная схема сопоставления.  
...
## <a name="dealing-with-data-types"></a>Работа с типами данных  
 Если схема указывает `image`, `binary`, или `varbinary` [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] тип данных (с помощью `sql:datatype`) и не соответствует типу данных XML, диаграмма обновления предполагает, что тип данных XML `binary base 64`. Если данные имеют тип `bin.base`, следует явно указать этот тип (`dt:type=bin.base` или `type="xsd:hexBinary"`).  
...
```  
 ...
 Следующая диаграмма обновления использует эту схему XSD для добавления новой записи сведения о порядке (  **\<OD >** элемент в  **\<после >** блок) для заказа 43860. Атрибут `mapping-schema` служит для указания схемы сопоставления, которая будет использоваться диаграммой обновления.  
...
### <a name="c-inserting-a-record-by-using-the-parent-child-relationship-and-inverse-annotation-specified-in-the-xsd-schema"></a>В. Вставка записи с помощью связи «родители-потомки» и заметки INVERSE, указанных в схеме XSD  
 В этом примере иллюстрируется использование связи «родители-потомки», указанной в XSD, в логике диаграммы обновления для обработки обновлений, а также использование заметки `inverse`. Дополнительные сведения о `inverse` заметки, см. в разделе [указания значения атрибута SQL: inverse для SQL: Relationship &#40;SQLXML 4.0&#41;](../../sqlxml-annotated-xsd-schemas-using/specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md).  
...
```  
 ...
 Чтобы указать логике диаграммы обновления для отмены «родитель потомок» во время операции обновления, `inverse` задана заметка для  **\<связь >** элемент. В итоге записи будут сначала добавляться в таблицу Cust, а затем в таблицу Ord, и операция завершится успешно.  
