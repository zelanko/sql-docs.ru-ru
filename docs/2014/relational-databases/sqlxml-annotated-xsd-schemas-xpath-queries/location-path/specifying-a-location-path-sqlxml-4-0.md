...
-   **Проверка узла**  
     ...
     Проверка узла задает тип узла, выбранного на шаге доступа. Каждая ось (`child`, `parent`, `attribute` и `self`) имеет основной тип узла. Для `attribute` оси типом основного узла является ** \< атрибут>**. Для `parent` осей, `child` и `self` Тип основного узла — ** \< элемент>**.  
  
     Например, если путь к расположению определяет **дочерний элемент:: Customer**, выделены дочерние элементы ** \< Customer>** узла контекста. Так как на `child` оси есть ** \< элемент,>** в качестве типа основного узла, проверка узла, Customer, принимает значение true, если Customer является ** \< элементом>** узле.  
