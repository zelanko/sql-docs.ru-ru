...
```  
 ...
 В запросе `child` является осью и `Contact` является тестом узла (значение true `Contact` , если является ** \< элементом>** узлом, поскольку \< элемент> является типом первичного узла, связанным с `child` осью).  
...
> [!NOTE]  
>  Запрос XPath `/Order[../@CustomerID="1"]` возвратит ошибку, так как отсутствует родительский элемент ** \< Order>**. Хотя в схеме сопоставления могут содержаться элементы, которые содержат ** \<>порядок **, XPath не начался ни в одном из них; следовательно, ** \< порядок>** считается типом элемента верхнего уровня в документе.  
...
```  
 ...
 В предикате `attribute::CustomerID` `attribute` является осью и `CustomerID` является тестом узла (если `CustomerID` является атрибутом, проверяемым узлом является true, так как ** \< атрибут>** node является основным узлом для `attribute` оси).  
