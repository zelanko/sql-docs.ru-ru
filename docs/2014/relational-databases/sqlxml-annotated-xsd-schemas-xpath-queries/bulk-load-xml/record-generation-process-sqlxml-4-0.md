...
## <a name="exceptions-to-the-record-generation-rule"></a>Исключения из правила создания записей  
 ...
 Например, рассмотрим следующую схему XSD, которая описывает  **\<клиента >** и  **\<порядок >** элементов. **\<Клиента >** элемент включает в себя **OrderList** атрибут типа IDREFS. Тег `<sql:relationship>` задает связь «один ко многим» между заказчиком и списком заказов.  
