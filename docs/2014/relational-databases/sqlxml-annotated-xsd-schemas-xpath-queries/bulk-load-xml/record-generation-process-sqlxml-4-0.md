...
```  
 ...
 Схема указывает элемент ** \<>клиента** с атрибутами **CustomerID** и **CompanyName** . `sql:relation` Заметка сопоставляет элемент ** \<>клиента** с таблицей Customers.  
...
## <a name="exceptions-to-the-record-generation-rule"></a>Исключения из правила создания записей  
 ...
 Например, рассмотрим следующую схему XSD, которая описывает ** \<>клиента** и ** \<порядок>** элементов. Элемент ** \<>клиента** включает атрибут **OrderList** типа IDREFS. Тег `<sql:relationship>` задает связь «один ко многим» между заказчиком и списком заказов.  
