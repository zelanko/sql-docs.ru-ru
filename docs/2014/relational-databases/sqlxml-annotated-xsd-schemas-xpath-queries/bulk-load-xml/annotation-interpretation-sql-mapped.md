...
```  
 ...
 Поскольку атрибут **HomePhone** указывает, что при `sql:mapped="false"` XML-загрузке не выполняется привязка этого атрибута к соответствующему столбцу. Схема XSD определяет столбец переполнения (**оверфловколумн**), в котором при выполнении групповой загрузки XML эти невостребованные данные сохраняются.  
