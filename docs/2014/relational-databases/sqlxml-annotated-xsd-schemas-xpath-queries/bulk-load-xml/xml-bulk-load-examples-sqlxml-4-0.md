...
## <a name="g-bulk-loading-in-overflow-columns"></a>Ж. Массовая загрузка в столбцы переполнения  
 Если в схеме сопоставления при помощи заметки `sql:overflow-field` задан столбец переполнения, массовая загрузка XML скопирует все невостребованные данные из исходного документа в этот столбец.  
...
5.  Выполните код VBScript.  
     ...
     Схема должна указывать соответствующий `sql:datatype` для **CustomerID** атрибут значение **CustomerID** указывается в виде идентификатора GUID, включающий фигурные скобки ({и}), такие как:  
    ...
    ```  
     ...
     Когда `sql:datatype` указан, определяющий тип столбца как `uniqueidentifier`, операции массовой загрузки удаляет фигурные скобки ({и}) из **CustomerID** перед вставкой в столбце.  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>К. Массовая загрузка XML-данных в столбцы типа данных xml  
 Если схема сопоставления задает [тип данных xml](/sql/t-sql/xml/xml-transact-sql) столбца с помощью `sql:datatype="xml"` заметки, Массовая загрузка XML может копировать дочерние элементы XML сопоставленного поля из исходного документа в этот столбец.  
  
 Рассмотрим следующую схему XSD, которая сопоставляет представление таблицы Production.ProductModel в образце базы данных AdventureWorks. В этой таблице поле CatalogDescription `xml` сопоставляется с типом данных  **\<Desc >** элемента с помощью `sql:field` и `sql:datatype="xml"` заметок.  
