...
```  
 ...
 Следующая схема XML определяет XML-представление этих таблиц. Схема указывает связь «родители-потомки» между ** \<клиентом>** и ** \<порядком>** элементов.  
...
```  
 ...
 При выполнении операции XML-загрузки используется связь «первичный ключ-внешний ключ», указанная выше между элементами ** \<Cust>** и ** \<CustOrder>** , чтобы выполнить их в обе таблицы.  
...
```  
 ...
 Схема задает элемент ** \<Order>** с дочерним элементом ** \<>продукта** . Элемент ** \<Order>** сопоставляется с таблицей, а элемент ** \<>Product** сопоставляется с таблицей Product в базе данных. Отношение цепочки, указанное в элементе ** \<>продукта** , определяет связь M:N, представленную таблицей OrderDetail. (в заказ может входить множество продуктов, а продукт может входить во множество заказов).  
...
```  
 ...
 Схема определяет столбец переполнения (OverflowColumn) для таблицы Cust. В результате в этот столбец добавляется все неиспользованные XML-данные для каждого ** \<элемента>Customer** .  
...
5.  Выполните код VBScript.  
     ...
     Схема должна указывать соответствующий `sql:datatype` атрибуту **CustomerID** , если значение для **CustomerID** указано как идентификатор GUID, включающий фигурные скобки ({и}), например:  
    ...
    ```  
     ...
     Если `sql:datatype` указан параметр, идентифицирующий тип столбца `uniqueidentifier`как, операция групповой загрузки удаляет фигурные скобки ({и}) из значения **CustomerID** перед вставкой в столбец.  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>К. Массовая загрузка XML-данных в столбцы типа данных xml  
 Если схема сопоставления задает столбец [типа данных XML](/sql/t-sql/xml/xml-transact-sql) с помощью аннотации, то `sql:datatype="xml"` при выполнении XML-загрузки можно скопировать дочерние элементы XML для сопоставленного поля из исходного документа в этот столбец.  
  
 Рассмотрим следующую схему XSD, которая сопоставляет представление таблицы Production.ProductModel в образце базы данных AdventureWorks. В этой `xml` таблице поле CatalogDescription типа данных сопоставляется с элементом ** \<DESC>** с помощью заметок `sql:field` и `sql:datatype="xml"` .  
