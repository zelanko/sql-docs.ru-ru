...
5.  Выполните код VBScript.  
     ...
     Схема должна указывать соответствующий `sql:datatype` для **CustomerID** когда значение для **CustomerID** указывается как идентификатор GUID, который включает фигурные скобки ({и}), таких как:  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>К. Массовая загрузка XML-данных в столбцы типа данных xml  
 Если схема сопоставления указывает [тип данных xml](/sql/t-sql/xml/xml-transact-sql) столбца с помощью `sql:datatype="xml"` заметки, Массовая загрузка XML можно скопировать дочерние элементы XML сопоставленного поля из исходного документа в этом столбце.  
  
 Рассмотрим следующую схему XSD, которая сопоставляет представление таблицы Production.ProductModel в образце базы данных AdventureWorks. В этой таблице поле CatalogDescription `xml` тип данных сопоставляется с  **\<Desc >** элемента с помощью `sql:field` и `sql:datatype="xml"` заметок.  
