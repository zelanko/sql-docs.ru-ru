---
title: Отправка данных в виде и возвращающие табличные значения параметров, с использованием данных времени выполнения (ODBC) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="sending-data-as-a-table-valued-parameter-using-data-at-execution-odbc"></a>Отправка данных в виде возвращающего табличное значение параметра с использованием данных времени выполнения (ODBC)
 ...
 Еще один пример, демонстрирующий табличное значение параметров см. в разделе [использование возвращающих табличные значения параметров &#40;ODBC&#41;](table-valued-parameters-odbc.md).  
  
 В этом примере при вызове SQLExecute или SQLExecDirect, драйвер возвращает SQL_NEED_DATA. Затем приложение вызывает SQLParamData несколько раз, пока драйвер возвращает значение, отличное от SQL_NEED_DATA. Драйвер возвращает *ParameterValuePtr* для информирования какого параметра требуются данные для приложения. Приложение вызывает SQLPutData для предоставления данных параметра до следующего вызова SQLParamData. Вызов SQLPutData для возвращающего табличное значение параметра указывает, сколько строк она подготовила для драйвера (в данном примере всегда 1). Когда все строки табличного значения переданы драйверу, SQLPutData вызывается для указания, что доступно 0 строк.  
  
 Можно использовать значения данных времени выполнения в строках табличного значения. Значение, возвращаемое SQLParamData информирует приложение требует драйвер нужное значение. С помощью значения параметра регулярных SQLPutData можно вызывать один или несколько раз для табличного значения символьные или двоичные значения столбца. Это позволяет приложению передавать большие значения по частям.  
...
2.  Выполните привязку параметров. *ColumnSize* -1, что означает, что по крайней мере строки передается одновременно.  
...
6.  Ввод данных времени выполнения для параметров. При возврате SQLParamData *ParameterValuePtr* для возвращающего табличное значение параметра приложение должно подготовить столбцы для следующей строки или строк табличного значения. Затем приложение вызывает SQLPutData с *DataPtr* задать число доступных строк (в данном примере 1) и *StrLen_or_IndPtr* присвоено значение 0.  
...
### <a name="description"></a>Описание  
 В этом примере показано, которые можно использовать потоковую передачу строк, одну строку для каждого вызова SQLPutData с ODBC возвращающего табличное значение Параметра, аналогично как программа BCP.exe используется для загрузки данных в базу данных.  
...
### <a name="description"></a>Описание  
 В этом примере показано, которые можно использовать потоковую передачу строк, несколько строк для одного вызова SQLPutData с ODBC возвращающего табличное значение Параметра, аналогично как программа BCP.exe используется для загрузки данных в базу данных.  
