---
title: Отправка данных в виде табличное значение параметра с использованием данных времени выполнения (ODBC) | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="sending-data-as-a-table-valued-parameter-using-data-at-execution-odbc"></a>Отправка данных в виде возвращающего табличное значение параметра с использованием данных времени выполнения (ODBC)
 ...
 Другой пример, демонстрирующий возвращающих табличные значения параметров, см. в разделе [параметров, возвращающих &#40;ODBC&#41;](table-valued-parameters-odbc.md).  
  
 В этом примере при вызове SQLExecute или SQLExecDirect, драйвер возвращает SQL_NEED_DATA. Затем приложение вызывает SQLParamData несколько раз, пока драйвер возвращает значение, отличное от SQL_NEED_DATA. Драйвер возвращает *ParameterValuePtr* о том, какой параметр требуются данные для приложения. Приложение вызывает SQLPutData для предоставления данных параметра до следующего вызова SQLParamData. Вызов SQLPutData для возвращающих табличные значения параметра, указывает, сколько строк она подготовила для драйвера (в данном примере всегда 1). Когда все строки табличного значения переданы драйверу, SQLPutData вызывается для указания, что доступно 0 строк.  
  
 Можно использовать значения данных времени выполнения в строках табличного значения. Значение, возвращенное SQLParamData информирует о том, какое значение драйвер требует приложение. Как и в случае значения обычному параметру SQLPutData может вызываться один или несколько раз для табличного значения символьные или двоичные значения столбца. Это позволяет приложению передавать большие значения по частям.  
  
 При вызове функции SQLPutData для табличного значения, *DataPtr* используется для числа доступных строк (в данном примере всегда 1). *StrLen_or_IndPtr* всегда должен быть 0. Когда все строки табличного значения переданы, SQLPutData вызывается с *DataPtr* значение 0.  
...
2.  Выполните привязку параметров. *ColumnSize* -1, это означает, что не более одного передается строки за раз.  
...
5.  Вызов процедуры. SQLExecDirect вернет значение SQL_NEED_DATA, так как табличное значение параметра является параметром данных во время выполнения.  
...
6.  Ввод данных времени выполнения для параметров. При возвращении SQLParamData *ParameterValuePtr* для возвращающих табличные значения параметра, приложение должно подготовить столбцы для следующей строки или строк табличного значения. Затем приложение вызывает SQLPutData с *DataPtr* число доступных строк (в данном примере 1) и *StrLen_or_IndPtr* присвоено значение 0.  
...
### <a name="description"></a>Описание  
 В этом примере показано, что можно использовать потоковую передачу строк, одну строку для каждого вызова SQLPutData с ODBC возвращающего табличное значение Параметра, аналогично тому, как используется BCP.exe для загрузки данных в базу данных.  
...
### <a name="description"></a>Описание  
 В этом примере показано, что можно использовать потоковую передачу строк, несколько строк за каждый вызов SQLPutData с ODBC возвращающего табличное значение Параметра, аналогично тому, как используется BCP.exe для загрузки данных в базу данных.  
