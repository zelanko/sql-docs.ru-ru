...
|**EventSubClass**|**nvarchar**|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> **Local**. Выбранный маршрут имеет адрес LOCAL.<br /><br /> **Remote**. Выбранный маршрут имеет адрес, отличный от LOCAL.<br /><br /> **Delayed**. Сообщение отложено либо из-за того, что пересылка выключена, либо из-за отсутствия подходящего маршрута.|21|Да|  
