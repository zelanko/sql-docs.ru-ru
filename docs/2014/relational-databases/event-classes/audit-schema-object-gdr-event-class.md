...
|**Разрешений**|**int**|Указывает, было ли установлено разрешение на доступ к столбцу. Выполните синтаксический анализ текста инструкции, чтобы определить, какие именно разрешения были применены к тем или иным столбцам. 1 = да, 0 = нет.|44|Да|  
...
|**ИмяОбъекта**|**nvarchar**|Имя объекта, являющегося предметом разрешения, отмены разрешения или запрета разрешения.|34|Да|  
...
|**таржетусернаме**|**nvarchar**|Для действий с пользователем базы данных (например, для предоставления разрешения пользователю) — имя этого пользователя.|11,9|Да|  
