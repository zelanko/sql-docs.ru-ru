...
|EventSubClass|`nvarchar`|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> Подписка зарегистрирована: указывает подписка на уведомления о запросах успешно зарегистрирована в базе данных.<br /><br /> Повторная подписка: Указывает, когда [!INCLUDE[ssDE](../../includes/ssde-md.md)] получает запрос на подписку, полностью совпадающий с уже существующей подпиской. В таком случае компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] устанавливает значение тайм-аута для существующей подписки, совпадающее со значением тайм-аута, указанным в новом запросе на подписку.<br /><br /> Подписка запущена: Указывает, когда подписка на уведомление создает сообщение уведомления.<br /><br /> Ошибка компонента service broker не удалось произвести подписку: Указывает, что сообщение уведомления не из-за [!INCLUDE[ssSB](../../includes/sssb-md.md)] ошибки.<br /><br /> Не удалось произвести подписку без ошибки компонента service broker: Указывает, что сообщение уведомления, доставлено, но не из-за [!INCLUDE[ssSB](../../includes/sssb-md.md)] ошибки.<br /><br /> Перехвачена ошибка компонента Service Broker: Указывает, что [!INCLUDE[ssSB](../../includes/sssb-md.md)] передал ошибку в диалоге, который использует уведомление о запросе.<br /><br /> Попытка удаления подписки: Указывает, что [!INCLUDE[ssDE](../../includes/ssde-md.md)] пытался удалить подписку с истекшим сроком действия для высвобождения ресурсов.<br /><br /> Не удалось удалить подписку: указывает на Сбой попытки удалить подписку с истекшим сроком действия. Для высвобождения ресурсов компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] автоматически повторно запланирует удаление этой подписки.<br /><br /> Подписка уничтожена: Указывает, что [!INCLUDE[ssDE](../../includes/ssde-md.md)] удалил подписку с истекшим сроком действия|21|Да|  
