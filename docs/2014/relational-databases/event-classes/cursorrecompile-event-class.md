...
# <a name="cursorrecompile-event-class"></a>CursorRecompile, класс событий
  Класс событий **CursorRecompile** описывает события перекомпилирования курсора, происходящие в курсорах API. События перекомпиляции курсора происходят, [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] когда перекомпилирует курсор Transact-SQL из-за изменения схемы.  
...
|**евентсекуенце**|**int**|Порядковый номер класса событий **CursorRecompile** в пакете.|51|нет|  
