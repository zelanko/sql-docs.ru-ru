...
|EventSubClass|`nvarchar`|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> Созданный шаблон: Указывает, что в базе данных был создан шаблон уведомления запроса.<br /><br /> Шаблон сопоставления: Указывает, когда шаблон уведомления запроса используется повторно.<br /><br /> Шаблон удаления: Указывает, когда шаблон уведомления запроса удаляется из базы данных.|21|Да|  
