...
|EventSubClass|`nvarchar`|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> Выполнение запущено часов: Указывает, что фоновый поток в [!INCLUDE[ssDE](../../includes/ssde-md.md)] что расписания с истекшими сроками действия таблиц параметров началась очистка.<br /><br /> Запустить готовое часов: Указывает, что фоновый поток в [!INCLUDE[ssDE](../../includes/ssde-md.md)] что расписания с истекшими сроками действия таблиц параметров завершения очистки.<br /><br /> Начата задача очистки master: Показывает, когда начинается Очистка (сборка мусора) для удаления данных подписки уведомлений запросов с истекшим сроком действия.<br /><br /> Задача очистки master завершена: Указывает, когда завершается Очистка (сборка мусора) для удаления данных подписки уведомлений запросов с истекшим сроком действия.<br /><br /> Master cleanup задача пропущена. Указывает, что [!INCLUDE[ssDE](../../includes/ssde-md.md)] не выполнил очистку (сборка мусора) для удаления данных подписки уведомлений запросов с истекшим сроком действия.|21|Да|  
