...
|EventSubClass|`nvarchar`|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> Часы выполнения работы: Указывает, что фоновый поток в [!INCLUDE[ssDE](../../includes/ssde-md.md)] что расписания истек срок действия таблицы параметров для начала очистки.<br /><br /> Часы по завершении выполнения: Указывает, что фоновый поток в [!INCLUDE[ssDE](../../includes/ssde-md.md)] , расписания с истекшими сроками действия таблицы параметров завершения очистки.<br /><br /> Начата задача очистки master: Определяет, когда запущена очистка (сборка мусора), удаляющая данные подписки на уведомления о запросах с истекшим сроком давности.<br /><br /> Задача очистки master завершена: указывает на завершение завершена очистка (сборка мусора), удаляющая данные подписки на уведомления о запросах с истекшим сроком.<br /><br /> Задача master очистки пропущено: Указывает, что [!INCLUDE[ssDE](../../includes/ssde-md.md)] не выполнил очистку (сборка мусора) для удаления данных подписки на уведомления о запросах с истекшим сроком.|21|Да|  
