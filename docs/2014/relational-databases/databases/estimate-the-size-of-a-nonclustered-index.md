...
-   Значения LOB  
     ...
     Алгоритм определения точности объем места, используемого для хранения типов данных LOB `varchar(max)`, `varbinary(max)`, `nvarchar(max)`, `text`, `ntext`, `xml`, и `image` сложен. Достаточно только сложить средние размеры ожидаемых значений больших объектов, умножить их на ***Num_Rows***и добавить его к общему размеру некластеризованного индекса.  
