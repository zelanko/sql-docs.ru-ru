...
2.  Укажите количество столбцов с фиксированной и изменяемой длиной, а также рассчитайте необходимый размер места для их хранения.  
     ...
     ***Num_Cols*** = общее число столбцов (с фиксированной и переменной длиной)  
...
4.  Расчет размера данных с переменной длиной:  
    ...
    > [!NOTE]  
    >  Можно сочетать столбцы `varchar`, `nvarchar`, `varbinary` или `sql_variant`, в результате чего общая ширина определенной таблицы превысит 8060 байт. Длина каждого из этих столбцов по-прежнему должна находиться в пределах 8 000 байт для столбца `varchar`, `nvarchar,``varbinary`или. `sql_variant` Тем не менее их общая ширина в таблице может превышать предел в 8 060 байт.  
...
-   Значения LOB  
     ...
     Алгоритм для определения `varchar(max)`того, `varbinary(max)` `nvarchar(max)` `text`какой объем пространства будет использоваться для хранения типов данных LOB,,,, **нтекстксмл**, и `image` является сложным. Достаточно просто прибавить ожидаемую среднюю величину значений LOB к общему размеру кучи.  
