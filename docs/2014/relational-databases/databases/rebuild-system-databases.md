...
##  <a name="CreateMSDB"></a> Создание новой базы данных msdb  
 Если `msdb` база данных повреждена и нет резервной копии `msdb` базы данных, можно создать новый `msdb` с помощью **instmsdb** сценария.  
  
> [!WARNING]  
>  Перестроение `msdb` базы данных с помощью **instmsdb** сценарий исключит все данные, хранящиеся в `msdb` например заданий, предупреждения, операторы, планы обслуживания, журнал резервного копирования, параметры управления на основе политик , Database Mail, хранилище данных о производительности и т. д.  
...
3.  В другом окне командной строки, отключите `msdb` базы данных, выполнив следующую команду, заменив  *\<имя_сервера >* с экземпляром [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]: `SQLCMD -E -S<servername> -dmaster -Q"EXEC sp_detach_db msdb"`  
