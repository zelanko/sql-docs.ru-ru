 ...
 **Включены рекурсивные триггеры**  
 ...
 По умолчанию все пользовательские базы данных и все системные базы данных (за исключением **msdb**) имеют для этого свойства значение `False` . Значение не может быть изменено для баз данных **model** и **tempdb** .  
