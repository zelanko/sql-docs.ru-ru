...
|------------------------|-----------------------------|--------------------|-----------------------|------------|  
|**Окна и точки останова**|Недоступно|**Точки останова**|CTRL + ALT + B|Отображает окно **Точки останова** , в котором можно просматривать и управлять точками останова.|  
|**Windows/Watch/контрольные значения**|Недоступно|**Точки останова/Просмотр значений/контрольные значения**|CTRL+ALT+W, 1|Отображает окно **Просмотр значений1** .|  
|**Окна/Контрольные значения/Просмотр значений2**|Недоступно|**Точки останова/Просмотр значений/Просмотр значений2**|CTRL+ALT+W, 2|Отображает окно **Просмотр значений2** .|  
|**Окна/Контрольные значения/Просмотр значений3**|Недоступно|**Точки останова/Просмотр значений/Просмотр значений3**|CTRL+ALT+W, 3|Отображает окно **Просмотр значений3** .|  
|**Окна/Контрольные значения/Просмотр значений4**|Недоступно|**Точки останова/Просмотр значений/Просмотр значений4**|CTRL + ALT + W, 4|Отображает окно **Просмотр значений4** .|  
|**Windows/локальные переменные**|Недоступно|**Точки останова/локальные переменные**|CTRL + ALT + V, L|Отображает окно **локальные** .|  
|**Окна/Стек вызовов**|Недоступно|**Точки останова/Стек вызовов**|CTRL + ALT + C|Отображение окна **Стек вызовов** .|  
|**Окна и потоки**|Недоступно|**Точки останова/потоки**|CTRL + ALT + H|Отображение окна **потоки** .|  
|**Продолжения**|Недоступно|**Продолжения**|ALT + F5|Выполнить код до следующей точки останова. Параметр **продолжить** неактивен, пока вы не Помещаетесь в окно редактора запросов, которое находится в режиме отладки.|  
...
|**Команда "Прервать все"**|Недоступно|**Команда "Прервать все"**|CTRL + ALT + BREAK|Эта функция не используется отладчиком [!INCLUDE[tsql](../../includes/tsql-md.md)] .|  
|**Отключить отладку**|Недоступно|**Отключить отладку**|SHIFT+F5|Выводит окно редактора запросов из режима отладки и возвращает его в обычный режим.|  
...
|Недоступно|**Выполнить до** Набора|Недоступно|CTRL + F10|Выполнение всего кода от последнего места остановки до текущего места расположения курсора без остановки в точках останова.|  
|**Быстрой**|**Быстрой**|Недоступно|CTRL + ALT + Q|Отображает окно **Быстрая проверка** .|  
|**Точка останова**|**Точка останова и вставка точки останова**|Недоступно|F9|Размещает точку останова на текущей или выделенной инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)] .|  
|Недоступно|**Точка останова/удаление точки останова**|Недоступно|Недоступно|Удаляет точку останова из выделенной строки.|  
|Недоступно|**Точка останова/отключение точки останова**|Недоступно|Недоступно|Отключает точку останова в выделенной строке. Точка останова остается в строке кода, но до ее повторного включения она не будет останавливать выполнение кода.|  
|Недоступно|**Точка останова/Включение точки останова**|Недоступно|Недоступно|Включает точку останова в выделенной строке.|  
|**Удалить все точки останова**|Недоступно|Недоступно|CTRL+SHIFT+F9|Удаляет все точки останова.|  
|**Отключение всех точек останова**|Недоступно|Недоступно|Недоступно|Отключает все точки останова.|  
