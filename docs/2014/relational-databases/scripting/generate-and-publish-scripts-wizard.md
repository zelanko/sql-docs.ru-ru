...
-   **Продолжить выполнение скрипта при ошибке** . Если задано **значение true**, скрипт останавливается при возникновении ошибки. Если значение равно **False**, то выполнение скрипта продолжается. По умолчанию **False**.  
...
-   **Использовать базу данных в скрипте** — добавляет в скрипт `USE DATABASE` инструкцию. В сценарий будет включена инструкция `USE DATABASE`, чтобы гарантировать создание объектов в нужной базе данных. Если предполагается, что скрипт будет использоваться в другой базе данных, выберите **значение false** , чтобы `USE DATABASE` пропустить инструкцию. Значение по умолчанию равно **True**. Дополнительные сведения см. в статье [USE (Transact-SQL)](/sql/t-sql/language-elements/use-transact-sql).  
...
9. **Опубликовать параметры vardecimal** — включает формат `vardecimal` таблицы в целевой таблице базы данных, если она включена в исходной таблице базы данных. Значение по умолчанию равно **True**.  
