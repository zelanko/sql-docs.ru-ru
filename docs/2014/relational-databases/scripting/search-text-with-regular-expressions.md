...
1.  Чтобы включить использование регулярных выражений в **найти** поле во время **быстрый поиск**, **найти в файлах**, **Быстрая замена**, или  **Замена в файлах** операций, выберите **используйте** под заголовком **параметры поиска** и выберите **регулярные выражения**.  
...
|Повтор n раз|^n|Совпадает с числом n вхождений предыдущего выражения. Например, [0-9] ^ 4 соответствует любой последовательности из четырех цифр.|  
