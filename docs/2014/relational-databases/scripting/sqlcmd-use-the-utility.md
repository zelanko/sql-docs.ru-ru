...
-   «Сервер» (**-S**), определяющий экземпляр [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] к которому `sqlcmd` подключается.  
  
-   Параметры проверки подлинности (**-E**, **- U**, и **-P**), укажите учетные данные, `sqlcmd` использует для подключения к экземпляру [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
  
    > [!NOTE]  
    >  Параметр **-E** используется по умолчанию, и нет необходимости его указывать.  
  
-   Параметры входа (**-Q**, **- q**, и **-i**), которые задают расположение входных данных для `sqlcmd`.  
  
-   Параметр output (**-o**), указывает файл, в котором `sqlcmd` является помещает выходные данные.  
...
-   Соединение с экземпляром по умолчанию с использованием проверки подлинности Windows для запуска инструкций [!INCLUDE[tsql](../../includes/tsql-md.md)] в интерактивном режиме:  
    ...
    > [!NOTE]  
    >  В предыдущем примере **-E** не задается, поскольку он используется по умолчанию и `sqlcmd` подключается к экземпляру по умолчанию с использованием проверки подлинности Windows.  
...
## <a name="running-transact-sql-statements-interactively-by-using-sqlcmd"></a>Интерактивный запуск инструкций Transact-SQL с помощью программы sqlcmd  
 ...
 В приглашении `sqlcmd` можно вводить как инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)], так и команды `sqlcmd`, например `GO` и `EXIT`. Каждая инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] помещается в буфер, называемый кэш инструкций. Эти инструкции отправляются [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] после ввода `GO` команду и нажмите клавишу ВВОД. Чтобы выйти из `sqlcmd`, тип `EXIT` или `QUIT` в начале строки.  
  
 Чтобы очистить кэш инструкций, введите `:RESET`. Введя `^C` вызывает `sqlcmd` для выхода. Кроме того, с помощью команды `^C` можно останавливать выполнение кэша инструкций после запуска команды `GO`.  
  
 [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкции, введенные во время интерактивного сеанса, можно изменить, введя **: ED** команды и `sqlcmd` строки. Откроется редактор, и после изменения инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)] и закрытия редактора измененная инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] появится в окне командной строки. Введите `GO` для запуска измененной [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкции.  
...
## <a name="interactive-sqlcmd-example"></a>Пример интерактивной команды sqlcmd  
 ...
 Тип `sqlcmd` для подключения к экземпляру по умолчанию [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] на локальном компьютере, а содержимое из командной строки будет выглядеть:  
...
### <a name="f-using-sqlcmd-in-a-windows-script-file"></a>Е. Использование программы sqlcmd в файлах скриптов Windows  
 Объект `sqlcmd`команды, такие как `sqlcmd -i C:\InputFile.txt -o C:\OutputFile.txt,` может выполняться в BAT-файле вместе со сценарием VBScript. В этом случае интерактивные параметры не используются. Программа `sqlcmd` должна быть установлена на компьютере, на котором выполняется файл BAT.  
...
### <a name="g-using-sqlcmd-to-set-encryption-on-windows-azure-sql-database"></a>Ж. Использование программы sqlcmd для включения шифрования в базе данных SQL Windows Azure  
 Объект `sqlcmd`могут быть выполнены, подключение к [!INCLUDE[ssSDS](../../includes/sssds-md.md)] данных на для определения шифрования и отношения доверия сертификата. Два "sqlcmd '" доступны параметры:  
