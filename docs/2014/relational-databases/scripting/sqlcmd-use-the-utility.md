...
-   Параметры ввода ( **-q**, **-q**и **-i**), которые указывают `sqlcmd`расположение входных данных.  
...
## <a name="running-transact-sql-statements-interactively-by-using-sqlcmd"></a>Интерактивный запуск инструкций Transact-SQL с помощью программы sqlcmd  
 С помощью программы `sqlcmd` можно интерактивно запускать инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)] из окна командной строки. Для [!INCLUDE[tsql](../../includes/tsql-md.md)] интерактивного выполнения инструкций с помощью `sqlcmd`запустите программу, не используя параметры **-q**, **-q**, **-Z**или **-i** , чтобы указать входные файлы или запросы. Пример:  
...
### <a name="f-using-sqlcmd-in-a-windows-script-file"></a>Е. Использование программы sqlcmd в файлах скриптов Windows  
 Команда, например, `sqlcmd -i C:\InputFile.txt -o C:\OutputFile.txt,` может быть выполнена в BAT-файле вместе с VBScript. `sqlcmd` В этом случае интерактивные параметры не используются. Программа `sqlcmd` должна быть установлена на компьютере, на котором выполняется файл BAT.  
...
### <a name="g-using-sqlcmd-to-set-encryption-on-azure-sql-database"></a>Ж. Настройка шифрования в базе данных SQL Azure с помощью программы sqlcmd  
 Можно выполнить для подключения к [!INCLUDE[ssSDS](../../includes/sssds-md.md)] данным в, чтобы указать шифрование и доверие сертификатов. `sqlcmd` Доступны два параметра "sqlcmd" ".  
