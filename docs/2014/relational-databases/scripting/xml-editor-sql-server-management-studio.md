...
## <a name="data-view"></a>Представление данных  
 ...
 В представлении данных предусмотрено две отдельные области: **Таблицы данных** и **данных**. Область **Таблица данных** — это список взаимоотношений, определенных в XML-файле в порядке вложенности (от самых внешних к самым внутренним). Область **Данные** — это сетка данных, отображающая данные на основании выбора в области «Таблица данных».  
  
> [!NOTE]  
>  Новые файлы XML не содержат данных и не могут отображаться в представлении данных. Есть также некоторые XML-документы, для которых не может быть применено представление данных. Хотя XML считается форматом, если это не структурированных данных попытке переключения на данных представления создаст следующее сообщение: «Несмотря на то, что этот документ имеет правильный формат, он содержит структуру, которую невозможно отобразить в представлении данных.»  
