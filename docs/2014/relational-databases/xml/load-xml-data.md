...
-   Чтобы использовать явно задать кодировку, используйте `varbinary()` тип, поскольку он не работает с кодовыми страницами, или используйте символьный тип для соответствующей кодовой страницы. После этого назначьте данные XML-столбцу, переменной или параметру.  
  
### <a name="example-explicitly-specifying-an-encoding"></a>Пример. Явное указание кодировки  
 Предположим, что есть XML-документ vcdoc, хранящийся в типе данных `varchar(max)`, который не объявлен явно как XML. Следующая инструкция добавляет объявление XML с кодировкой «iso8859-1», присоединяет XML-документ, приводит результат к `varbinary(max)` , чтобы двоичное представление сохраняется и, наконец, приводит его в формат XML. Это позволяет процессору XML выполнять синтаксический анализ данных в соответствии с указанной кодировкой «iso8859-1» и создавать для строковых значений соответствующее представление UTF-16.  
