...
## <a name="type-casting-string-and-binary-instances"></a>Строка приведения типа и двоичные экземпляры  
 Можно выполнить синтаксический анализ всех [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] строковые типы данных, такие как [**n**] [**var**]**char**, **[n] text**,  **varbinary**, и **изображения**, в `xml` приведение (CAST) или преобразования (CONVERT) строки в тип данных `xml` типа данных. Проверка нетипизированного XML подтверждает, что он сформирован правильно. Если есть схема, связанная с `xml` типа, проверка также выполняется. Дополнительные сведения см. в статье [Сравнение типизированного и нетипизированного XML](compare-typed-xml-to-untyped-xml.md).  
...
```  
 ...
 Следующая операция вставки неявно преобразовывает строку в `xml` типа:  
  
```  
INSERT INTO T VALUES (3, @s)   
```  
  
 Можно явно cast() строку `xml` типа:  
...
### <a name="example-convert-a-string-to-typed-xml-and-assign-it-to-a-variable"></a>Пример: преобразование строкового значения в типизированное значение xml и присваивание его переменной  
 В следующем примере строка преобразовывается в `xml` введите и присваивается переменной типа `xml` тип данных:  
...
```  
 ...
 Инструкция SELECT возвращает текстовый XML-фрагмент, который затем анализируется в процессе присвоения `xml` переменной типа данных.  
  
 Можно также использовать [директивы TYPE](type-directive-in-for-xml-queries.md) в предложении FOR XML, которая непосредственно возвращает результат запроса как FOR XML `xml` типа:  
...
```  
 ...
 Предыдущий пример неявно преобразовывает строку в `xml` тип данных и присваивает его `xml` переменной типа.  
  
 Следующий пример вставляет строковую константу в `xml` столбец типа:  
...
## <a name="using-bulk-load"></a>Использование массовой загрузки  
 Улучшенная функциональность [OPENROWSET (Transact-SQL)](/sql/t-sql/functions/openrowset-transact-sql) позволяет произвести массовую загрузку XML-документов в базу данных. Можно выполнить массовую загрузку XML-экземпляров из файлов в `xml` столбцов типа в базе данных. Дополнительные сведения см. в статье [Примеры массового импорта и экспорта XML-документов (SQL Server)](../import-export/examples-of-bulk-import-and-export-of-xml-documents-sql-server.md). Дополнительные сведения о загрузке XML-документов см. в статье [Загрузка XML-данных](load-xml-data.md).  
  
## <a name="in-this-section"></a>в этом разделе  
  
|Раздел|Описание|  
|-----------|-----------------|  
|[Получение и запрос XML-данных](retrieve-and-query-xml-data.md)|Описывает компоненты экземпляров XML, не фиксируемых при сохранении экземпляров в базах данных.|  
  
## <a name="see-also"></a>См. также  
 [Сравнение типизированного и нетипизированного XML](compare-typed-xml-to-untyped-xml.md)   
 [Методы типа данных XML](/sql/t-sql/xml/xml-data-type-methods)   
