...
```  
 ...
 В этом запросе значения CustomerID сравниваются от одной строки к следующей при создании \<Cust> элементов, поскольку CustomerID является первичным ключом таблицы. Если CustomerID не идентифицирован для таблицы как первичный ключ, во всех столбцах (CustomerID и CustomerType в этом запросе) проводится сравнение каждой строки со следующей строкой. Если значения различаются, в \<Cust> XML добавляется новый элемент.  
