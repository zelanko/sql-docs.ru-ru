...
|**\@MP:prev**|Предыдущий элемент того же уровня. Благодаря этому можно получить сведения о порядке элементов документа.<br /><br /> **\@MP:prev** содержит идентификатор XML предыдущего соседнего с тем же родительским элементом. Если элемент находится в передней части списке  **\@mp:prev** имеет значение NULL.|  
...
-   **Идентификатор** столбец сопоставлен с  **\@mp:id** атрибут метасвойства и указывает, что столбец содержит созданный системой уникальный идентификатор XML элемента.  
  
-   **Родительского** столбец сопоставлен с  **\@mp:parentid** и указывает, что столбец содержит идентификатор XML родительского элемента.  
  
-   **ParentLocalName** столбец сопоставлен с  **\@mp:parentlocalname** и указывает, что столбец содержит локальное имя родительского элемента.  
 ...
 **Комментарий** столбца определяется в качестве столбца переполнения путем привязки к  **\@mp:xmltext** метасвойств. Для параметра *flags* установлено значение **9** (XML_ATTRIBUTE and XML_NOCOPY). Оно соответствует **атрибутивному** сопоставлению и указывает, что в столбец переполнения необходимо скопировать только невостребованные данные.  
  
 Затем инструкция SELECT возвращает набор строк, предоставленных OPENXML.  
  
 В этом примере  **\@mp:parentlocalname** задано для столбца, **ParentLocalName**, набора строк, сформированного инструкцией OPENXML. В результате этот столбец содержит локальное имя родительского элемента.  
  
 Кроме него, набор строк содержит два столбца: **parent** и **comment**. **Родительского** столбец сопоставлен с  **\@mp:parentid** и указывает, что столбец содержит идентификатор XML родительского элемента элемента. Столбец comment указан в качестве столбца переполнения путем привязки к  **\@mp:xmltext** метасвойств.  
