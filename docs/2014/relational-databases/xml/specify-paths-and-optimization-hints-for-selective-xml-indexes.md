...
##  <a name="untyped"></a> Основные сведения о типах Xquery и SQL Server в нетипизированном XML  
 Селективные XML-индексы поддерживают два типа систем: Типы XQuery и [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] типов. Индексированный путь можно использовать для сопоставления с выражением Xquery или с возвращаемым типом метода value() типа данных XML.  
...
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] типы поддерживают одно указание оптимизации: ЕДИНСТВЕННЫЙ ЭКЗЕМПЛЯР.  
...
1.  **Принцип 1**. Чтобы оценить заданное выражение XQuery, необходимо проиндексируйте все узлы, которые следует просмотреть.  
...
2.  **Принцип 2**. Для наилучшей производительности проиндексируйте все узлы, необходимые для вычисления заданного выражения XQuery. Если индексируются только некоторые узлы, селективный XML-индекс улучшает оценку вложенных выражений, содержащих только индексированные узлы.  
...
### <a name="singleton-optimization-hint"></a>Указание по оптимизации SINGLETON  
 Применимо для следующих объектов: XQuery или [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] типы данных  
