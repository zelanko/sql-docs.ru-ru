...
|**\<xsd:choice>**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]отклоняет схемы с **\<xsd:choice>** примитивом без дочерних элементов, если только часть не определена с нулевым значением атрибута **minOccurs** .|  
|**\<xsd:include>**|В настоящее время [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] не поддерживает этот элемент. XML-схемы, содержащие данный элемент, отклоняются сервером.<br /><br /> В качестве решения схемы XML, включающие **\<xsd:include>** директиву, можно предварительно обработать для копирования и слияния содержимого всех включенных схем в единую схему для отправки на сервер. Дополнительные сведения см. в разделе [Предварительная обработка схемы для слияния включаемых схем](preprocess-a-schema-to-merge-included-schemas.md).|  
...
|**xs: QName**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]не поддерживает типы, производные от **xs: QName** , которые используют элемент ограничения схемы XML.<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]не поддерживает типы Union с **xs: QName** в качестве элемента Member.<br /><br /> Дополнительные сведения см. в разделе [Тип xs:QName](the-xs-qname-type.md).|  
