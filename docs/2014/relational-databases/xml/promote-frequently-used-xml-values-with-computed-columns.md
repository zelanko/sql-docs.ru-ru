...
-   Создать триггеры для XML-столбца с целью обслуживания таблиц свойств. В триггерах сделайте что-либо одно из следующего перечисленного ниже.  
    ...
    -   Используйте `xml` методы, такие как тип данных **nodes()** и **value()**, для вставки и удаления строк в таблицы свойств.  
...
3.  определение с помощью пользовательской функции триггеров вставки, обновления и удаления для обслуживания таблиц свойств.  
 ...
 Чтобы реализовать это решение, создайте сначала потоковую функцию CLR. `xml` Тип данных, предоставляемых в ADO.NET как управляемый класс SqlXml и поддерживает **CreateReader()** метода, возвращающего объект класса XmlReader.  
