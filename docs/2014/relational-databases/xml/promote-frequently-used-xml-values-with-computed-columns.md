...
## <a name="computed-column-based-on-the-xml-data-type"></a>Вычисляемый столбец, основанный на типе данных xml  
 Вычисляемый столбец можно создать с помощью определяемой пользователем функции, которая вызывает методы типа `xml` данных. Вычисляемый столбец может иметь любой тип SQL, в том числе XML. Это продемонстрировано в следующем примере.  
...
```  
 ...
 Можно создать определяемую пользователем функцию для возврата типа `xml` данных и вычисляемого столбца с помощью определяемой пользователем функции. Однако создать XML-индекс для вычисляемого XML-столбца нельзя.  
...
-   Создать триггеры для XML-столбца с целью обслуживания таблиц свойств. В триггерах сделайте что-либо одно из следующего перечисленного ниже.  
    ...
    -   Для `xml` вставки и удаления строк таблиц свойств используются методы типа данных, такие как **nodes ()** и **value ()**.  
...
3.  определение с помощью пользовательской функции триггеров вставки, обновления и удаления для обслуживания таблиц свойств.  
 ...
 Чтобы реализовать это решение, создайте сначала потоковую функцию CLR. Тип `xml` данных предоставляется как управляемый класс SqlXml в ADO.NET и поддерживает метод **CreateReader ()** , возвращающий XmlReader.  
