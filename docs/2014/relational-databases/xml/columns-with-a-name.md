...
## <a name="column-name-starts-with-an-at-sign-"></a>Имя столбца начинается с символа \@  
 Если имя столбца начинается с символа (\@) и не содержит косой черты (/), то создается атрибут элемента <`row`>, имеющего соответствующее значение столбца. Например, следующий запрос возвращает набор строк с двумя столбцами (\@PmId, Name). В итоговом XML-документе атрибут **PmId** добавляется к соответствующему элементу <`row`>, и ему присваивается значение столбца ProductModelID.  
...
```  
 ...
 Имена столбцов используются в качестве пути при построении XML-документа в режиме PATH. Имя столбца, содержащего значения идентификатора сотрудника, начинается с "\@". Таким образом, атрибут **EmpID**добавляется к элементу> <`row` . Имена всех других столбцов содержат символ косой черты (/), являющийся признаком иерархии. В итоговом XML-документе будет присутствовать дочерний элемент <`EmpName`> внутри элемента <`row`>, а элемент <`EmpName`> будет в свою очередь содержать дочерние элементы <`First`>, <`Middle`> и <`Last`>.  
