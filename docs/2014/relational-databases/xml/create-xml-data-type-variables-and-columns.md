...
-   RULE  
 ...
 Вместо использования ограничения является создание программы-оболочки, определяемая пользователем функция программы-оболочки для `xml` метода типа данных и указать определяемую пользователем функцию в проверочном ограничении, как показано в следующем примере.  
...
## <a name="same-or-different-table"></a>Та же таблица или другая  
 `xml` Столбец типа можно создать в таблице, содержащей другие реляционные столбцы, или в таблице, связанной посредством внешнего ключа с главной таблицей.  
...
-   Для построения XML-индекса `xml` столбец типа данных, но первичный ключ главной таблицы отличается от ее ключа кластеризации, главная таблица не имеет первичного ключа или главная таблица является кучей (нет ключа кластеризации). Это может иметь место, если главная таблица уже существует.  
