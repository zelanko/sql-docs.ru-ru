...
#### <a name="example-modeling-xml-data-using-the-xml-data-type"></a>Пример Моделирование XML-данных с помощью типа данных xml  
...
#### <a name="example-retaining-exact-copies-of-xml-data"></a>Пример Сохранение точных копий XML-данных  
 ...
 XML-столбец может быть столбцом, вычисляемым на основе столбца `[n]varchar(max)`. Однако невозможно создать XML-индекс для вычисляемого XML-столбца, и не может быть построен XML-индекса в `[n]varchar(max)` или `varbinary(max)` столбцов.  
