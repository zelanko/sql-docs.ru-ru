...
# <a name="xml-data-type-and-columns-sql-server"></a>Столбцы и типы XML-данных (SQL Server)
  В этом разделе обсуждаются преимущества и ограничения типа `xml` данных в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]и помогает выбрать способ хранения XML-данных.  
...
-   Хранение больших объектов, `[n]varchar(max)` и `varbinary(max)`  
 ...
 Обычно используется сочетание этих подходов. Например, XML-данные можно сохранить в столбце типа `xml`, производя продвижение его свойств до уровня реляционных столбцов. Или вы можете использовать технологию сопоставления для хранения нерекурсивных частей в столбцах, отличных от XML, и только рекурсивных частей `xml` в столбцах типа данных.  
...
#### <a name="example-retaining-exact-copies-of-xml-data"></a>Пример: сохранение точных копий XML-данных  
 ...
 XML-столбец может быть столбцом, вычисляемым на основе столбца `[n]varchar(max)`. Однако нельзя создать XML-индекс для вычисляемого XML-столбца, а также построить XML-индекс на `[n]varchar(max)` основе столбцов или. `varbinary(max)`  
