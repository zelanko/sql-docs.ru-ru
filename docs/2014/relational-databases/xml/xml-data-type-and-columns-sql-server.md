...
# <a name="xml-data-type-and-columns-sql-server"></a>Столбцы и типы XML-данных (SQL Server)
  В этом разделе описаны преимущества и ограничения `xml` в тип данных [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]и помогает выбрать способ хранения XML-данных.  
...
-   требуется запрашивать данные или обновлять их фрагменты на основе их структуры.  
 ...
 Если ни одно из этих условий не выполняется, следует выбрать реляционную модель данных. Например, если данные представлены в формате XML, но приложение пользуется только базы данных для хранения и извлечения данных `[n]varchar(max)` вполне требуется. Хранение данных в XML-столбце обеспечивает дополнительные преимущества. В их число входят проверка структуры и правильности данных, а также поддержка детализированных запросов и обновлений XML-данных.  
...
-   Естественное хранение в виде типа `xml`  
     ...
     Для типизированного `xml` тип данных `xml` тип данных, связанный с XML-схемами, после проверки схемы набора сведений схемы (PSVI) добавляет в информационный набор данные и кодирует их во внутреннее представление. Это значительно ускоряет синтаксический анализ. Дополнительные сведения см. в спецификациях XML-схемы W3C на страницах [http://www.w3.org/TR/xmlschema-1](http://go.microsoft.com/fwlink/?LinkId=48881) и [http://www.w3.org/TR/xmlschema-2](http://go.microsoft.com/fwlink/?LinkId=4871).  
...
-   Хранение больших объектов, `[n]varchar(max)` и `varbinary(max)`  
 ...
 Обычно используется сочетание этих подходов. Например, может потребоваться сохранить данные XML в `xml` столбец типа данных и выполнить продвижение его свойств до реляционных столбцов. Или можно использовать технологию сопоставления для хранения нерекурсивных фрагментов в столбцах, отличных от XML, а только рекурсивные фрагменты в `xml` столбцы с типом данных.  
...
### <a name="native-xml-storage"></a>Хранение XML-данных в собственном формате  
 Можно хранить XML-данные в `xml` столбец типа данных на сервере. Этот вариант уместен, если выполняются следующие условия:  
...
#### <a name="example-retaining-exact-copies-of-xml-data"></a>Пример: сохранение точных копий XML-данных  
 Предположим, что правительственные законы требуют, чтобы хранились точные текстовые копии XML-документов. К этой категории могут относиться подписанные документы, юридические постановления и отчеты о биржевых операциях. Необходимо хранить в `[n]varchar(max)` столбца.  
 ...
 XML-столбец может быть столбцом, вычисляемым на основе `[n]varchar(max)` столбца. Однако не удается создать XML-индекс для вычисляемого XML-столбца, и не может быть построен XML-индекса в `[n]varchar(max)` или `varbinary(max)` столбцов.  
...
-   Приведение или преобразование к типам `text` или `ntext` не поддерживается. Используйте `varchar(max)` или `nvarchar(max)` вместо него.  
