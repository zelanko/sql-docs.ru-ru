...
-   Вторичные XML-индексы.  
 ...
 Первым индексом, создаваемым для столбца типа данных `xml`, должен быть первичный XML-индекс. Первичного XML-индекса поддерживаются вторичные индексы следующих типов: PATH, значение и СВОЙСТВО. Эти вторичные индексы могут способствовать повышению производительности выполнения разных типов запросов.  
...
### <a name="property-secondary-index"></a>Вторичный индекс PROPERTY  
 Производительность запросов, извлекающих одно или несколько значений из отдельных экземпляров XML, может повыситься при использовании индекса PROPERTY. Это происходит при извлечении свойств объекта с помощью **value()** метод `xml` типа, когда известно значение первичного ключа объекта.  
...
```  
 ...
 За исключением отличий, описанных далее в этом разделе, создание XML-индекса для`xml` столбец типа похоже на создание индекса не в`xml` столбец типа. Для создания XML-индексов и управления ими могут использоваться следующие DDL-инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)] :  
