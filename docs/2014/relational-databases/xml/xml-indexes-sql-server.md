...
-   Если запрашиваются значения экземпляров XML, не зная имен элементов или атрибутов, содержащих эти значения, следует подумать о создании индекса VALUE. Обычно это происходит при поиске по осям потомков, например//author [Last-Name = "Говард"], где \<author> элементы могут встречаться на любом уровне иерархии. Это также происходит в запросах с подстановочными знаками, например/Book [@ * = "роман"], где запрос ищет \<book> элементы, имеющие некоторый атрибут со значением "роман".  
