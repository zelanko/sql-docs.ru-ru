...
```  
 ...
 XML-декларация, например `<?xml version='1.0'?>`, не сохраняется при сохранении XML-данных в экземпляре типа `xml`. Это сделано намеренно. Декларация (XML) и ее атрибуты (версия/кодировка/stand-alone) будут утеряны, после данных преобразуется в тип `xml`. XML-декларация обрабатывается как директива для синтаксического анализатора XML. Все данные XML внутренне хранятся в кодировке UCS-2. Все другие инструкции по обработке (PI) в экземпляре XML сохраняются.  
...
##  <a name="query"></a> Задание обязательных параметров запроса  
 При запросе `xml` столбцов или переменных с помощью типа `xml` необходимо задать следующие параметры методов типа данных, как показано.  
...
|QUOTED_IDENTIFIER|ON|  
 ...
 Если эти параметры не установлены, как указано, запросы и изменения на `xml` методов типа данных завершится ошибкой.  
