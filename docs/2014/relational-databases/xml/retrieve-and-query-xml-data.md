...
```  
 ...
 XML-декларация, например `<?xml version='1.0'?>`, не сохраняется при сохранении XML-данных в экземпляре типа `xml`. Это сделано намеренно. После преобразования данных в тип `xml`XML-декларация () и его атрибуты (версия/кодировка/автономные) теряются. XML-декларация обрабатывается как директива для синтаксического анализатора XML. Все данные XML внутренне хранятся в кодировке UCS-2. Все другие инструкции по обработке (PI) в экземпляре XML сохраняются.  
