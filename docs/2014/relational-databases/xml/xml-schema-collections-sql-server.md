...
-   Используйте встроенную функцию **XML_SCHEMA_NAMESPACE()** . К выходным `xml` данным этой функции можно применить методы типа данных. Однако изменять базовые XML-схемы нельзя.  
