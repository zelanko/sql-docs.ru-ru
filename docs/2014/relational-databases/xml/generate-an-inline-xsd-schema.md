...
-   Если во вложенном запросе FOR XML определена директива TYPE, результат запроса будет иметь `xml` типа и этот результат рассматривается как экземпляр нетипизированных XML-данных. Дополнительные сведения см в разделе [Данные XML (SQL Server)](xml-data-sql-server.md).  
...
-   Другой документ схемы, описывающий форму результата запроса FOR XML.  
 ...
 Кроме того Если какие-либо типизированные `xml` типы данных включены в результат запроса, схемы, связанные с этими типами `xml` типы данных включены.  
