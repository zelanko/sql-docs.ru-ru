...
## <a name="creating-typed-xml"></a>Создание типизированного XML  
 Перед созданием типизированных `xml` переменных, параметров или столбцов, сначала необходимо зарегистрировать коллекцию XML-схем с помощью [CREATE XML SCHEMA COLLECTION &#40;Transact-SQL&#41;](/sql/t-sql/statements/create-xml-schema-collection-transact-sql). Затем можно связать коллекцию XML-схем с переменными, параметрами или столбцами `xml` тип данных.  
...
### <a name="example-specifying-a-schema-for-an-xml-type-column"></a>Пример. Указание схемы для столбца типа xml  
 В следующем примере создается таблица с `xml` столбцом типа и указывает схему для столбца:  
...
### <a name="example-passing-an-xml-type-parameter-to-a-stored-procedure"></a>Пример. Передача параметра типа xml в хранимую процедуру  
 В следующем примере передается `xml` параметр_типа хранимой процедуре и указывается схема для переменной:  
...
-   При приведении строки к типизированному `xml` тип данных, анализ также выполняет проверку и типизация на основании пространства имен схемы XML в указанной коллекции.  
  
-   Данные можно приводить из типизированного `xml` в нетипизированный `xml` и наоборот.  
  
 Дополнительные сведения о других способах формирования XML в SQL Server см. в разделе [Создание экземпляров XML-данных](create-instances-of-xml-data.md). После формирования XML, его может быть связан с `xml` тип данных переменной или сохранен в `xml` введите столбцы для дополнительной обработки.  
  
 В иерархии типов данных `xml` отображаются ниже `sql_variant` и определяемых пользователем типов, но выше всех встроенных типов.  
...
### <a name="example-specifying-facets-to-constrain-a-typed-xml-column"></a>Пример. Указание аспектов для ограничения типизированного XML-столбца  
 Для типизированного `xml` столбцы, можно ограничить столбца, чтобы разрешить только отдельные элементы высшего уровня для каждого экземпляра, сохраненного в них. , для указания дополнительного аспекта `DOCUMENT` при создании таблицы, как показано в следующем примере:  
...
```  
 ...
 По умолчанию экземпляры, хранимые в типизированном `xml` столбца хранятся в виде XML-содержимое, а не XML-документов. Это позволяет использовать:  
...
```  
 ...
 Обратите внимание, что дополнительные аспекты DOCUMENT/CONTENT можно указать везде, где определен тип `xml` (типизированный XML). Например, при создании типизированной `xml` переменной, можно добавить аспект DOCUMENT/CONTENT, как показано ниже:  
