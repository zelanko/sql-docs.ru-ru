...
-   Он использует формат хранения переменной длины для числовых типов (например `integer`, `decimal`, и `float`) и, основанные на числовых типов (например `datetime` и `money`).  
...
|`float`|Да|Наименее значащие байты, содержащие нули, не сохраняются. `float` для недробных значений в мантиссе применяется сжатие.|  
|`real`|Да|Наименее значащие байты, содержащие нули, не сохраняются. `real` для недробных значений в мантиссе применяется сжатие.|  
