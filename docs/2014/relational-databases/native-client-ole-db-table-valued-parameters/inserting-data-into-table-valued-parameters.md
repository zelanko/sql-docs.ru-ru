...
ms.locfileid: "36187986"
---
# <a name="inserting-data-into-table-valued-parameters"></a>Вставка данных в параметры, возвращающие табличные значения
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Поставщиком Native Client OLE DB поддерживает две модели задания данных для строк табличных параметров потребителем: Принудительная и по запросу. Пример, демонстрирующий модели по запросу доступен; в разделе [SQL Server Образцы программирования](http://msftdpprodsamples.codeplex.com/).  
...
## <a name="push-model-loads-all-table-valued-paremeter-data-in-memory"></a>Принудительная модель (загружает все данные табличного параметра в память)  
 ...
 После [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] поставщиком Native Client OLE DB начнет выполнять команду, значения параметров, возвращающих табличные значения будут извлечены из этого объекта набора строк возвращающего табличное значение параметра и отправляются на сервер.  
