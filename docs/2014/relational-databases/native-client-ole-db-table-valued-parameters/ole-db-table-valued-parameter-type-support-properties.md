...
## <a name="properties"></a>Свойства  
 Далее приводится список свойств, к которым можно получить доступ, применив метод IRowsetInfo::GetProperties к объектам наборов строк для табличного параметра. Следует заметить, что все свойства объектов наборов строк для возвращающего табличное значение параметра неизменяемы. Таким образом, попытка задать любое из свойств с помощью методов IOpenRowset:: OpenRowset или Итабледефинитионвисконстраинтс:: Креатетаблевисконстраинтс для их значений, не являющихся значениями по умолчанию, приведет к ошибке, и объект не будет создан.  
...
### <a name="dbpropset_sqlservercolumn"></a>DBPROPSET_SQLSERVERCOLUMN  
 Это свойство используется потребителем в процессе создания объекта набора строк возвращающего табличное значение параметра с помощью Итабледефинитионвисконстраинтс:: Креатетаблевисконстраинтс для каждого столбца с помощью структуры DBCOLUMNDESC, если это необходимо.  
