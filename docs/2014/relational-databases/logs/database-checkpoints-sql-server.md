...
> [!NOTE]  
>  `-k` [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Расширенный параметр настройки позволяет администратору базы данных регулировать поведение ввода-вывода, в зависимости от пропускной способности подсистемы ввода-вывода для некоторых типов контрольных точек контрольной точки. `-k` Опция применяется к автоматическим контрольным точкам и любым другим регулируемым ручным и внутренним контрольным точкам.  
...
###  <a name="InteractionBwnSettings"></a> Взаимодействие параметров TARGET_RECOVERY_TIME и «recovery interval»  
 Следующая таблица описывает связь между параметром сервера **sp_configure "`recovery interval`"** параметр и конкретной базы данных ALTER DATABASE... Параметр TARGET_RECOVERY_TIME.  
