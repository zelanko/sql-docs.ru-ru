...
##  <a name="Overview"></a> Обзор контрольных точек  
...
> [!NOTE]  
>  `-k` [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Расширенный параметр настройки позволяет администратору базы данных регулировать поведение ввода-вывода, в зависимости от пропускной способности подсистемы ввода-вывода для некоторых типов контрольных точек контрольной точки. `-k` Установки параметр применяется к автоматическим контрольным точкам и любым другим регулируемым ручным и внутренним контрольным точкам.  
...
###  <a name="InteractionBwnSettings"></a> Взаимодействие параметров TARGET_RECOVERY_TIME и «recovery interval»  
 В следующей таблице перечислены взаимодействия между параметром сервера **sp_configure'`recovery interval`"** параметр и настройки базы данных ALTER DATABASE... Параметр TARGET_RECOVERY_TIME.  
...
####  <a name="PerformanceImpact"></a> Влияние интервала восстановления на производительность восстановления  
 Для оперативной обработки (OLTP) с помощью коротких транзакций транзакций `recovery interval` является основным фактором, определяющим время восстановления. Однако `recovery interval` параметр не влияет на время, необходимое для отмены длительной транзакции. Восстановление базы данных с длительной транзакцией может занять гораздо больше, чем указано в `recovery interval` параметр. Например, если длительной транзакции потребовалось бы два часа для проведения обновлений до экземпляр сервера станет недоступным, действительному восстановлению потребуется значительно больше времени, чем `recovery interval` , на восстановление этой длительной транзакции. Дополнительные сведения о влиянии длительных транзакций на время восстановления см. в статье [Журнал транзакций (SQL Server)](the-transaction-log-sql-server.md).  
...
-   Если обнаружено, что из-за более частого выполнения контрольных точек производительность базы данных снижается.  
 ...
 Если принято решение увеличить параметр `recovery interval`, то рекомендуется увеличивать его постепенно с небольшими приращениями и оценивать влияние каждого приращения на производительность восстановления. Этот подход важен поскольку как `recovery interval` задание увеличивается, восстановление базы данных происходит, гораздо больше времени для завершения. Например, если изменить `recovery interval` 10, восстановления увеличится приблизительно в 10 раз времени, чем при `recovery interval` присваивается нулевое значение.  
