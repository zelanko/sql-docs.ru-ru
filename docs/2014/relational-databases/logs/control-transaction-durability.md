...
```  
 ...
 `ALLOWED`  
 При использовании этого параметра устойчивость каждой транзакции определяется на уровне транзакций — DELAYED_DURABILITY = { *Off* | ON}. Дополнительные сведения см. в разделе [Управление уровнями блока Atomic — скомпилированные в собственном код хранимые процедуры](#atomic-block-level-control---natively-compiled-stored-procedures) и [Управление уровнями фиксации — Transact-SQL](#commit-level-control---t-sql) .  
...
### <a name="atomic-block-level-control---natively-compiled-stored-procedures"></a>Управление на уровне блока Atomic — скомпилированные в собственном режиме хранимые процедуры  
