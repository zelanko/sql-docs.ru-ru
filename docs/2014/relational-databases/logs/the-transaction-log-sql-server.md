...
|16|XTP_CHECKPOINT|Если база данных содержит оптимизированную для памяти файловую группу, журнал транзакций может не усекаться до автоматического [!INCLUDE[hek_2](../../includes/hek-2-md.md)] контрольная точка срабатывает (что происходит на каждые 512 МБ рост журнала).<br /><br /> Примечание: Для усечения журнала транзакций до 512 МБ размер, запускать команду Checkpoint вручную в базе данных в вопросе.|  
