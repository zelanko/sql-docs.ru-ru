...
-   [Операции, которые можно выполнять с минимальным протоколированием](#MinimallyLogged)  
...
|16|XTP_CHECKPOINT|Если база данных содержит оптимизированную для памяти файловую группу, журнал транзакций может не усекаться до автоматического [!INCLUDE[hek_2](../../includes/hek-2-md.md)] контрольная точка срабатывает (что происходит при росте размера журнала каждые 512 МБ).<br /><br /> Примечание: Для усечения журнала транзакций перед размером 512 МБ, выполните команду Checkpoint вручную в нужной базе.|  
  
##  <a name="MinimallyLogged"></a> Операции, которые можно выполнять с минимальным протоколированием  
...
-   [Инструкция WRITETEXT](/sql/t-sql/queries/writetext-transact-sql) и [UPDATETEXT](/sql/t-sql/queries/updatetext-transact-sql) при вставке или добавлении новых данных в `text`, `ntext`, и `image` столбцы с типом данных. Обратите внимание, что минимальное протоколирование не используется при обновлении существующих значений.  
...
-   Если в базе данных используется простая модель восстановления или модель восстановления с неполным протоколированием, некоторые DDL-операции с индексом протоколируются в минимальном объеме при их выполнении как режиме «вне сети», так и в режиме «в сети». Минимально протоколируются следующие операции с индексами.  
    ...
    -   Перестроение новой кучи DROP INDEX (если применимо).  
        ...
        > [!NOTE]  
        >  Освобождение страниц во время индекса [DROP INDEX](/sql/t-sql/statements/drop-index-transact-sql) операции всегда полностью протоколируются.  
