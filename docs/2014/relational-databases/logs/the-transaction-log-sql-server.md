...
> [!NOTE]  
>  Известные рабочие точки, от которых следует начинать применение журналов транзакций при восстановлении базы данных, создаются контрольными точками. Дополнительные сведения см. в разделе [Database Checkpoints &#40;SQL Server&#41;](database-checkpoints-sql-server.md).  
...
##  <a name="FactorsThatDelayTruncation"></a>Факторы, которые могут вызвать задержку усечения журнала  
...
> [!IMPORTANT]  
>  Дополнительные сведения о том, что нужно делать при переполнении журнала транзакций, см. в разделе [Устранение неполадок при переполнении журнала транзакций (ошибка SQL Server 9002)](troubleshoot-a-full-transaction-log-sql-server-error-9002.md).  
...
-   Если в базе данных используется простая модель восстановления или модель восстановления с неполным протоколированием, некоторые DDL-операции с индексом протоколируются в минимальном объеме при их выполнении как режиме «вне сети», так и в режиме «в сети». Минимально протоколируются следующие операции с индексами.  
    ...
    -   [Создание операций с индексами](/sql/t-sql/statements/create-index-transact-sql) (включая индексированные представления).  
  
    -   [ALTER INDEX](/sql/t-sql/statements/alter-index-transact-sql) Операции REBUILD или DBCC DBREINDEX.  
    ...
    -   Перестроение новой кучи DROP INDEX (если применимо).  
        ...
        > [!NOTE]  
        >  Освобождение страницы индекса во время операции [DROP INDEX](/sql/t-sql/statements/drop-index-transact-sql) всегда полностью протоколируется.  
...
## <a name="see-also"></a>См. также:  
 ...
 [Предварительные требования для минимального ведения журнала при выполнении массового импорта](../import-export/prerequisites-for-minimal-logging-in-bulk-import.md)   
