...
-   [Класс SqlErrorLogEvent](../wmi-provider-configuration-classes/sqlerrorlogevent-class.md) (в этом разделе показано, как получить значения регистрируемых событий в указанном файле журнала.)  
  
-   [Класс SqlErrorLogFile](../wmi-provider-configuration-classes/sqlerrorlogfile-class.md) (в этом разделе показано, как получить сведения обо [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] всех файлах журнала на указанном экземпляре [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].)  
...
5.  При подключении к локальному экземпляру соединение устанавливается с использованием учетных данных текущего пользователя.  
    ...
    -   Чтобы подключиться от имени другого пользователя, установите флажок **Подключиться от имени другого пользователя** и нажмите кнопку **Задать пользователя**. При появлении запроса введите учетные данные пользователя (с именем пользователя в формате *domain_name*\\*user_name*), нажмите кнопку **ОК**, а затем еще раз нажмите кнопку **ОК** для подключения.  
