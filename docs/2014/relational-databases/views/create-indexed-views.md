...
###  <a name="Restrictions"></a> Обязательные параметры SET для индексированных представлений  
 ...
 Для правильной поддержки представлений и получения согласованных результатов некоторые параметры SET индексированных представлений должны иметь определенные значения. Параметрам SET из следующей таблицы должно быть присвоено значений, приведенных в **RequiredValue** столбца при каждом выполнении следующих условий:  
...
-   Инструкция SELECT в определении представления не должна содержать следующие элементы языка Transact-SQL:  
    ...
    |CHECKSUM_AGG|||  
     ...
     \*Индексированное представление может содержать `float` столбцов; Однако такие столбцы не могут включаться в ключ кластеризованного индекса.  
