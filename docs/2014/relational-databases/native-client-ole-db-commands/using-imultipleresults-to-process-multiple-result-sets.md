---
title: Обработка нескольких результирующих наборов при помощи интерфейса IMultipleResults | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="using-imultipleresults-to-process-multiple-result-sets"></a>Обработка нескольких результирующих наборов при помощи интерфейса IMultipleResults
 ...
 Следующие [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкция формирует несколько наборов строк, некоторые содержат данные строк из **OrderDetails** таблицы, а некоторые – результаты предложения COMPUTE BY:  
...
```  
 ...
 Потребитель может отменить команду с помощью [ISSAbort::Abort](../native-client-ole-db-interfaces/issabort-abort-ole-db.md) или освободив все удерживаемые ссылки на объект команды и производный набор строк.  
  
 С помощью **IMultipleResults** во всех экземплярах позволяет потребителю получить все наборы строк, сформированного при выполнении команды, а соответствующим образом определить, когда необходимо отменить выполнение команды и освободить объект сеанса для использования другие команды.  
  
> [!NOTE]  
>  При использовании курсоров [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] выполнение команды создает курсор [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] возвращает успех или ошибку создания курсора, поэтому обмен данными с экземпляром [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] завершается по выполнении команды. Каждый **GetNextRows** вызов становится круговом пути. Таким образом, могут существовать несколько активных объектов команд, каждая из которых обрабатывает набор строк, являющийся результатом выборки из серверного курсора. Дополнительные сведения см. в разделе [наборы строк и курсоры SQL Server](../native-client-ole-db-rowsets/rowsets-and-sql-server-cursors.md).  
