...
|Текст сообщения|В SQL Server обнаружено% d запросов ввода-вывода, для выполнения которых в файле [% ls] в базе данных`[%ls] (%d)`требуется больше% d секунд.  Дескриптором файла ОС является 0x%p.  Смещение последней длительной операции ввода-вывода: %#016I64x.|  
