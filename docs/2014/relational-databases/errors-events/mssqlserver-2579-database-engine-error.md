...
|Текст сообщения|Ошибка таблицы: Экстент P_ID в объекте с Идентификатором O_ID, Идентификатором индекса I_ID, Идентификатором секции PN_ID, Идентификатором единицы распределения A_ID (тип TYPE) выходит за пределы диапазона этой базы данных.|  
