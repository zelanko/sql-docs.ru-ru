...
|Текст сообщения|Ошибка таблицы: Страница P_ID, заголовок которой указывает, что она выделена для идентификатора объекта O_ID, Идентификатором индекса I_ID, Идентификатором секции PN_ID, Идентификатором единицы распределения A_ID (тип TYPE), выделена другим объектом.|  
