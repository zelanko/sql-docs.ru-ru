...
|Текст сообщения|Ошибка таблицы: Страница P_ID, выделенная для идентификатора объекта O_ID, Идентификатором индекса I_ID, Идентификатором секции PN_ID, Идентификатором единицы распределения A_ID (тип TYPE), не обнаружена. Возможно, указана недопустимая страница, либо идентификатор единицы распределения в ее заголовке неверен.|  
