...
# <a name="view-and-work-with-the-output-from-the-database-engine-tuning-advisor"></a>Просмотр и работа с выходными данными помощника по настройке ядра СУБД
  Когда помощник по настройке ядра СУБД настраивает базы данных, он создает сводки, рекомендации, отчеты и журналы настройки. Выходные данные журналов настройки можно использовать, чтобы устранять неполадки в сеансах помощника по настройке ядра СУБД. Вы можете использовать сводки, рекомендации и отчеты, чтобы определить, следует ли реализовать рекомендации по настройке или продолжить настройку до тех пор, пока не будут достигнуты улучшения производительности запросов, необходимые [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] для установки. Информация об использовании помощника по настройке базы создания рабочих нагрузок и настройки базы данных см. в разделе [Запуск и использование помощника по настройке ядра СУБД](database-engine-tuning-advisor.md).  
  
##  <a name="View"></a>Просмотр выходных данных настройки  
...
3.  После завершения нового сеанса настройки или после того, как средство загрузило существующий сеанс, перейдите на вкладку **выполнение** . На панели **Журнал настройки** отображается содержимое журнала. Журнал содержит сведения о событиях рабочей нагрузки, которые помощника по настройке ядра СУБД не смог проанализировать.  
...
##  <a name="Analysis"></a>Выполнение исследовательского анализа  
...
3.  Измените элементы `TuningOptions` и `Configuration` в образце входного файла XML-данных. В элементе `TuningOptions` укажите, какие структуры физического проектирования необходимо рассмотреть помощнику по настройке ядра СУБД во время сеанса настройки. В элементе `Configuration` укажите физические структуры, соответствующие предположительной конфигурации физических структур баз данных, которые необходимо проанализировать помощнику по настройке ядра СУБД. Сведения о том, какие атрибуты и дочерние элементы можно использовать `TuningOptions` с `Configuration` родительскими элементами и, см. в разделе [Справочник по входным XML-файлам &#40;помощник по настройке ядра СУБД&#41;](../../tools/dta/xml-input-file-reference-database-engine-tuning-advisor.md).  
  
4.  Сохраните входной файл с расширением **XML** .  
  
5.  Проверьте входной XML-файл, сохраненный на этапе 4, на соответствие XML-схеме помощника по настройке ядра СУБД. Эта схема устанавливается в следующее расположение при установке [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]:  
  
    ```  
    C:\Program Files\Microsoft SQL Server\100\Tools\Binn\schemas\sqlserver\2004\07\dta\dtaschema.xsd  
    ```  
  
     Помощник по настройке ядра СУБД схема XML также доступна в Интернете по адресу [https://schemas.microsoft.com/sqlserver/2004/07/dta](https://schemas.microsoft.com/sqlserver/2004/07/dta).  
...
##  <a name="ReviewEvaluateClone"></a>Проверка, оценка и клонирование сеансов настройки  
...
3.  Если вы хотите дополнительно проверить результаты настройки этого сеанса, перейдите на вкладку **отчеты** . В нем можно просмотреть сводку по настройке или выбрать отчет о настройке из списка **Выбор отчета** .  
...
##  <a name="UI"></a>Описания пользовательского интерфейса  
  
### <a name="sessions-monitor"></a>Монитор сеансов  
 **Монитор сеансов** отображает сведения о сеансах, открытых в помощник по настройке ядра СУБД. Чтобы просмотреть сведения о сеансе в окне свойств, выберите имя сеанса в **мониторе сеансов**.  
 ...
 **Запланировать для последующего использования**  
 ...
 **Число событий в рабочей нагрузке**  
 ...
 **Число индексов в представлениях, рекомендуемых для [создания | удаления]**  
 ...
 **Рекомендуемое количество создаваемых статистик**  
