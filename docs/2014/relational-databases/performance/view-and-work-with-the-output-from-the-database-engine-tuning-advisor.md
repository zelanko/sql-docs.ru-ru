...
3.  Откройте XML-файл, созданный на шаге 2, в любом редакторе XML, текстовом редакторе или в среде [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]. Прокрутите вниз до `Configuration` элемента. Скопируйте и вставьте `Configuration` раздел элемента в XML входного файла шаблона после `TuningOptions` элемента. Сохраните этот входной XML-файл.  
...
3.  Изменить `TuningOptions` и `Configuration` элементы в образце входного файла XML. В `TuningOptions` элемент, укажите, какие структуры физического проектирования необходимо рассмотреть во время сеанса настройки помощник по настройке ядра СУБД. В элементе `Configuration` укажите физические структуры, соответствующие предположительной конфигурации физических структур баз данных, которые необходимо проанализировать помощнику по настройке ядра СУБД. Сведения о какие атрибуты и дочерние элементы можно использовать с `TuningOptions` и `Configuration` родительских элементов см. в разделе [Справочник по входным файлам XML &#40;помощник по настройке ядра СУБД&#41;](../../tools/dta/xml-input-file-reference-database-engine-tuning-advisor.md).  
...
6.  После создания рабочей нагрузки и входного XML-файла все готово для передачи входного файла в программу командной строки **dta** для анализа. Убедитесь, что имя выходного XML-файла для аргумента программы **-ox** указано. Это создает выходные данные в XML-файл с рекомендуемой конфигурацией, указанной в `Configuration` элемент. Если вы хотите запустить помощник по настройке ядра СУБД еще раз для проверки другой предположительной конфигурации, основываясь на данных, можно скопировать и вставить `Configuration` содержимое элемента из выходного файла в новый или в первоначальный входной файл XML. Сведения о том, как работать с входным XML-файлом в программе командной строки **dta** , см. в разделе о настройке базы данных с помощью программы командной строки dta статьи [Запуск и использование помощника по настройке ядра СУБД](database-engine-tuning-advisor.md).  
