...
-   Логический оператор `Inner Join` реализован в форме физического оператора `Merge Join`. Остальные физические типы соединений — это `Nested Loops` и `Hash Join`. `Merge Join` Оператор принимает то обстоятельство, что оба индекса отсортированы по столбцу соединения CustomerID.  
...
план запроса для хэш-соединений дисковых таблиц.  
 ...
 В этом запросе строки из таблицы заказов получаются с помощью кластеризованного индекса. `Hash Match` Физический оператор теперь используется для `Inner Join`. Кластеризованный индекс в таблице Order не отсортирован по столбцу CustomerID, поэтому `Merge Join` потребуется оператор сортировки, который может повлиять на производительность. Обратите внимание на относительную стоимость оператора `Hash Match` (75%) по сравнению с затратами оператора `Merge Join` в предыдущем примере (46%). Оптимизатор рассматривался `Hash Match` оператор также в предыдущем примере, но оказалось, что `Merge Join` оператор обеспечивает лучшую производительность.  
...
-   Этот план содержит оператор `Hash Match`, а не `Merge Join`. Индексы в таблицах Order и Customer представляют собой хэш-индексы и, следовательно, не упорядочены. Объект `Merge Join` потребует операторов сортировки, которые бы привести к снижению производительности.  
...
1.  Пользователь выполняет `CREATE PROCEDURE` инструкции [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].  
...
2.  Средство синтаксического анализа извлекает имя и параметры хранимой процедуры.  
     ...
     Если инструкция была подготовлена, например с помощью `sp_prep_exec`, средство синтаксического анализа не требуется извлекать имя процедуры и параметры во время выполнения.  
 ...
 **Пробное сохранение параметров**  
 ...
 Пробное сохранение параметров не используется для компиляции хранимых процедур, скомпилированных в собственном коде. Предполагается, что у всех параметров хранимой процедуры значения UNKNOWN (неизвестны). Как и интерпретируемые хранимые процедуры, скомпилированные в собственном коде хранимые процедуры также поддержка `OPTIMIZE FOR` подсказку. Дополнительные сведения см. в разделе [Указания запросов (Transact-SQL)](/sql/t-sql/queries/hints-transact-sql-query).  
