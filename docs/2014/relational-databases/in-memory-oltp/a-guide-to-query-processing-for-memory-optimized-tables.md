...
-   Логический оператор `Inner Join` реализован в форме физического оператора `Merge Join`. Остальные физические типы соединений — это `Nested Loops` и `Hash Join`. `Merge Join` Оператор принимает основано на том факте, что оба индекса отсортированы по столбцу соединения CustomerID.  
...
план запроса для хэш-соединений дисковых таблиц.  
 ...
 В этом запросе строки из таблицы заказов получаются с помощью кластеризованного индекса. `Hash Match` Физический оператор теперь используется для `Inner Join`. Кластеризованный индекс в таблице Order не отсортирован по столбцу CustomerID и поэтому `Merge Join` потребуется оператор сортировки, который повлияет на производительность. Обратите внимание на относительную стоимость оператора `Hash Match` (75%) по сравнению с затратами оператора `Merge Join` в предыдущем примере (46%). Оптимизатором `Hash Match` оператор также в предыдущем примере, но оказалось, что `Merge Join` оператор присваивает более высокую производительность.  
...
-   Этот план содержит оператор `Hash Match`, а не `Merge Join`. Индексы в таблицах Order и Customer представляют собой хэш-индексы и, следовательно, не упорядочены. Объект `Merge Join` потребует добавления операторов сортировки, которые бы привести к снижению производительности.  
...
1.  Пользователь выполняет `CREATE PROCEDURE` инструкцию, чтобы [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].  
...
2.  Средство синтаксического анализа извлекает имя и параметры хранимой процедуры.  
     ...
     Если инструкция была подготовлена, например с помощью `sp_prep_exec`, анализатору не придется извлекать имя процедуры и параметры во время выполнения.  
