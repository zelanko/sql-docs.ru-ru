...
|[Скомпилированные в собственном коде хранимые процедуры](http://msdn.microsoft.com/library/dn133184.aspx)|Нельзя выполнить любую хранимую процедуру из хранимой процедуры, скомпилированной в собственном коде, путем копирования инструкции EXECUTE.|Да|Нет <sup>1</sup>|  
  
 <sup>1</sup> нет доступа к оптимизированной для памяти таблицы или скомпилированной хранимой процедуры из контекстного соединения (соединения из [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] при выполнении модуля CLR). Однако можно создать и открыть другое соединение, из которого можно получить доступ к оптимизированным для памяти таблицам и хранимым процедурам, скомпилированным в собственном коде. Дополнительные сведения см. в разделе [регулярного vs. Контекстные соединения](../clr-integration/data-access/context-connections-vs-regular-connections.md).  
...
## <a name="performance-and-scalability"></a>Производительность и масштабируемость  
 ...
 [!INCLUDE[tsql](../../../includes/tsql-md.md)] Выполнение  
