...
## <a name="transactions-and-error-handling"></a>Транзакции и обработка ошибок  
 ...
 Если отсутствуют активные транзакции в сеансе, то `BEGIN ATOMIC` запускает новую транзакцию. Если вне блока не возникло исключение, то транзакция будет зафиксирована в конце блока. Если блок вызывает исключение (то есть исключение не обрабатывается и не перехватывается внутри блока), то будет произведен откат этой транзакции. Для транзакций, которые охватывают один блок atomic (одна скомпилированная хранимой процедуры), вы не необходимости записывать явные `BEGIN TRANSACTION` и `COMMIT` или `ROLLBACK` инструкций.  
  
 Скомпилированные в собственном коде хранимые процедуры поддерживают `TRY`, `CATCH`, и `THROW` конструкции для обработки ошибок. `RAISERROR` не поддерживается.  
...
## <a name="session-settings"></a>Параметры сеанса  
 Параметры сеанса в блоках ATOMIC фиксируются, если выполнена компиляция хранимой процедуры. Некоторые параметры могут быть заданы с помощью `BEGIN ATOMIC` тогда как другие параметры всегда фиксируются в то же значение.  
...
|----------------------|-----------------|  
|`DATEFORMAT`|Поддерживаются все форматы даты, отличные от [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . При указании `DATEFORMAT` переопределяет формат даты по умолчанию, связанные с `LANGUAGE`.|  
