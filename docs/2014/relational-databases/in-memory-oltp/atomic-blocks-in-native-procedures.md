...
## <a name="transactions-and-error-handling"></a>Транзакции и обработка ошибок  
 ...
 Если отсутствуют активные транзакции в сеансе, то `BEGIN ATOMIC` запускает новую транзакцию. Если вне блока не возникло исключение, то транзакция будет зафиксирована в конце блока. Если блок вызывает исключение (то есть исключение не обрабатывается и не перехватывается внутри блока), то будет произведен откат этой транзакции. Для транзакций, которые охватывают один блок atomic (один скомпилированные в собственном коде хранимой процедуры), вы не необходимости записывать явные `BEGIN TRANSACTION` и `COMMIT` или `ROLLBACK` инструкции.  
  
 Скомпилированные в собственном коде хранимые процедуры поддерживают `TRY`, `CATCH`, и `THROW` создает для обработки ошибок. `RAISERROR` не поддерживается.  
...
## <a name="session-settings"></a>Параметры сеанса  
 Параметры сеанса в блоках ATOMIC фиксируются, если выполнена компиляция хранимой процедуры. Некоторые параметры могут быть заданы с `BEGIN ATOMIC` тогда как другие параметры всегда фиксируются в то же значение.  
  
 Требуются следующие параметры с `BEGIN ATOMIC`.  
  
|Обязательный параметр|Описание|  
|----------------------|-----------------|  
|`TRANSACTION ISOLATION LEVEL`|Поддерживаемые значения: `SNAPSHOT`, `REPEATABLEREAD`, и `SERIALIZABLE`.|  
...
|----------------------|-----------------|  
|`DATEFORMAT`|Поддерживаются все форматы даты, отличные от [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . Если указано, `DATEFORMAT` переопределяет формат даты по умолчанию, связанный с `LANGUAGE`.|  
