...
ms.locfileid: "62467284"
---
# <a name="atomic-blocks"></a>Блоки ATOMIC
  
  `BEGIN ATOMIC` — часть стандарта ANSI SQL. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] поддерживает блоки ATOMIC только на высшем уровне хранимых процедур, скомпилированных в собственном коде.  
...
## <a name="transactions-and-error-handling"></a>Транзакции и обработка ошибок  
 ...
 Все хранимые процедуры, скомпилированные в собственном коде, поддерживают конструкции `TRY`, `CATCH` и `THROW` для обработки ошибок. `RAISERROR`не поддерживается.  
...
|`DELAYED_DURABILITY`|Поддерживаемые значения: `OFF` и `ON`.<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]фиксации транзакций могут быть либо полностью устойчивыми, либо по умолчанию, либо отложенными устойчивыми. Дополнительные сведения см. в разделе [Управление устойчивостью транзакций](../logs/control-transaction-durability.md).|  
