...
-   Помощь с планированием и выполнением миграции в OLTP в памяти. Миграция из дисковой таблицы в оптимизированную для памяти таблицу может занять много времени. Помощник по оптимизации для памяти позволит определить несовместимые компоненты в таблице, которые необходимо удалить до перемещения таблицы в In-Memory OLTP. Помощник также позволяет понять последствия переноса таблицы в оптимизированную для памяти таблицу для приложения.  
 ...
 Сборщик данных о производительности транзакции и возможность создания отчета об анализе производительности транзакции устанавливаются при выборе **средства управления — основные** или **средства управления — Дополнительно** При установке [!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)].  
...
3.  Нажмите кнопку **Далее** для выбора базы данных, который будет собирать данные профилирования.  
...
5.  Поле, помеченное **выберите наборы сборщиков данных, следует ли включить**выберите **наборы элементов сбора производительности транзакций**. Нажмите кнопку **Далее** после завершения.  
...
6.  Поле, помеченное **выберите наборы сборщиков данных, следует ли включить**выберите **наборы элементов сбора производительности транзакций**.  
...
9. Выберите прокси.  
    ...
    2.  В **новой учетной записи-посредника** диалоговое окно, введите имя прокси-сервера, выберите учетные данные и при необходимости введите описание. Нажмите кнопку **участников**.  
  
    3.  Нажмите кнопку **добавить** и выберите **Msdb** роли.  
  
    4.  Выберите `dc_proxy` и нажмите кнопку **ОК**. Нажмите кнопку **ОК** еще раз.  
  
     После выбора правильного прокси-сервера щелкните **Далее**.  
...
### <a name="generate-reports"></a>Формирование отчета  
 Отчеты об анализе производительности транзакции можно создать, щелкнув правой кнопкой мыши базу данных хранилища данных управления и выбрав **отчеты**, затем **хранилища данных управления**, а затем **Обзор анализа производительности транзакций**.  
...
-   Раздел статистики блокировки  
     ...
     В этом разделе представлена таблица, в которой отображаются блокировки для таблицы базы данных. Дополнительные сведения о блокировках и кратковременных базы данных см. в разделе [архитектура блокировок](http://msdn.microsoft.com/library/aa224738\(v=sql.80\).aspx). Существуют следующие столбцы.  
...
-   Раздел ссылок на таблицы  
 ...
 Статистика выполнения в отчете сведения о хранимой процедуре собирается и объединяется из [sys.dm_exec_procedure_stats &#40;Transact-SQL&#41;](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-procedure-stats-transact-sql). Ссылки извлекаются из [sys.sql_expression_dependencies &#40;Transact-SQL&#41;](/sql/relational-databases/system-catalog-views/sys-sql-expression-dependencies-transact-sql).  
  
 Чтобы просмотреть подробные сведения о преобразовании хранимой процедуры, скомпилированные в собственном коде хранимой процедуре, используйте [помощник по собственной компиляции](native-compilation-advisor.md).  
