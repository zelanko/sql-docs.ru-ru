...
10. Запустите рабочую нагрузку на `T`.  
 ...
 <sup>1</sup> Примечание, `T_copy` которое сохраняется на диске в этом примере. Если имеется резервная копия `T`, то `T_copy` может быть временной или недолговечной таблицей.  
  
 <sup>2</sup> должен быть достаточный объем памяти `T_copy`для. Память не освобождается сразу же после `DROP TABLE`. Если `T_copy` оптимизирована для памяти, должно быть достаточно памяти для двух дополнительных копий `T`. Если `T_copy` является таблицей, находящейся на диске, то свободной памяти должно быть достаточно только для одной дополнительной копии `T` из-за необходимости отработки сборщика мусора после удаления старой версии `T`.  
