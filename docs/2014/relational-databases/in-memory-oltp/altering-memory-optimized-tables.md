...
## <a name="updating-the-definition-of-a-memory-optimized-table"></a>Обновление определения таблицы, оптимизированной для памяти  
 ...
 В следующей процедуре описаны шаги, необходимые для обновления таблицы. В этом примере обновление добавляет индекс. Эта процедура сохраняет имя таблицы и требует две операции копирования данных: один раз для временной таблицы и один раз в новую таблицу. Изменение bucket_count индекса, добавление или удаление столбца выполняются таким же образом.  
...
4.  Создайте копию таблицы и скопируйте данные из исходной таблицы в копию. Копия может быть создана с помощью следующих [!INCLUDE[tsql](../../includes/tsql-md.md)] <sup>1</sup>.  
    ...
    ```  
     ...
     Если имеется достаточный объем доступной памяти `T_copy` может быть оптимизированной для памяти таблицей, благодаря чему копирование данных.<sup> 2</sup>  
