...
|`EXECUTE AS`|Хранимые процедуры, скомпилированные в собственном коде, не поддерживают `EXECUTE AS CALLER`, который является контекстом выполнения по умолчанию. Поэтому необходимо указать контекст выполнения. Поддерживаются `EXECUTE AS OWNER`параметры `EXECUTE AS`, *User*и `EXECUTE AS SELF` .|  
