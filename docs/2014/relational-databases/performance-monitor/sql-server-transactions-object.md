...
|**Транзакции моментальных снимков**|Число активных в настоящий момент транзакций, которые используют уровень изоляции моментальных снимков.<br /><br /> Примечание. **Транзакций моментальных снимков** счетчик объекта реагирует, когда происходит первый доступ к данным, не при `BEGIN TRANSACTION` выполняется инструкция.|  
