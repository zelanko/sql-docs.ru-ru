...
|**Получено байт журнала/с**|Количество записей журнала, полученных вторичной репликой для базы данных в течение последней секунды.|Вторичная реплика|  
|**Журнал, оставшийся для отмены**|Объем журнала в килобайтах, оставшийся до завершения стадии отката.|Вторичная реплика|  
...
|**Транзакций записи зеркального отображения/с**|Количество транзакций, которые были записаны в базу данных-источник, а затем ожидали фиксации до тех пор, пока журнал не был отправлен базе данных-получателю, за последнюю секунду.|Первичная реплика|  
...
|**Осталось байт повтора**|Объем операций журнала (в килобайтах), которые необходимо повторить для завершения фазы повтора.|Вторичная реплика|  
|**Повторное завершение (байт/с)**|Количество записей журнала, выполненных повторно во вторичной базе данных-получателе в течение последней секунды.|Вторичная реплика|  
|**Всего записей журнала, нуждающихся в отмене**|Общий объем журнала, который должен быть обработан для отмены, в килобайтах.|Вторичная реплика|  
