...
-   Свойство статьи **@replicate_ddl** должно иметь значение `true`; **@identityrangemanagementoption** должно иметь значение `manual`; и **@status** необходимо указать этот параметр **24** имеет значение.  
