...
-   Отложенное обновление. Для обновления данных на подписчике подключение издателя и подписчика не требуется. Обновления могут быть выполнены в режиме «вне сети» работы подписчика или издателя.  
 ...
 Создание обновляемых подписок для публикаций транзакций — [создание обновляемых подписок на публикацию транзакций](../create-updatable-subscription-transactional-publication-transact-sql.md)  
...
-   Подписчики не могут обновить или вставить `text`, `ntext` или `image` значения, поскольку он не является невозможно выполнить считывание из вставленных или удаленных таблиц внутри триггеров, отслеживающих изменения репликации. Аналогично, подписчики не может обновлять или вставлять `text` или `image` значения с помощью `WRITETEXT` или `UPDATETEXT` так, как данные будут перезаписаны издателем. Вместо этого можно секционировать `text` и `image` столбцов в отдельную таблицу и изменить две таблицы в пределах транзакции.  
...
-   Для столбцов типа данных `SQL_VARIANT`: при вставке или обновлены на подписчике данных сопоставляет следующим образом агентом чтения очереди когда они копируются с подписчика в очередь:  
  
    -   `BIGINT`, `DECIMAL`, `NUMERIC`, `MONEY`, и `SMALLMONEY` сопоставляются `NUMERIC`.  
  
    -   `BINARY` и `VARBINARY` сопоставляются с `VARBINARY` данных.  
