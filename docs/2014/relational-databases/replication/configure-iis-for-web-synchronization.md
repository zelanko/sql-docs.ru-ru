...
2.  Установите [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] компоненты подключения на компьютере, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] который работает IIS с помощью мастера установки. Если планируется использовать мастер настройки веб-синхронизации, упомянутый в шаге 3, то также необходимо установить среду [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] на сервер IIS.  
...
2.  Запустите **диспетчер служб IIS**:  
    ...
    2.  В **открытой** коробке, введите, `inetmgr`а затем нажмите **OK**.  
...
3.  Запустите SelfSSL:  
    ...
    -   Чтобы указать значения для одного или нескольких параметров, нажмите кнопку **Пуск**и выберите **Выполнить**. В **открытой** коробке, введите `cmd`, а затем нажмите **OK**. Найдите каталог установки программы SelfSSL, введите `SelfSSL`, а затем задайте значения для одного или нескольких параметров. Для просмотра списка параметров введите команду `SelfSSL -?`.  
...
2.  С установочного диска [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] запустите мастер установки [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . Для получения более подробной информации об использовании этого мастера, см [&#41;&#40;. ](../../database-engine/install-windows/install-sql-server-from-the-installation-wizard-setup.md)  
...
6.  На странице **Сведения виртуального каталога** выполните следующие действия:  
    ...
    2.  Введите путь виртуального каталога в поле **Путь** . Например, если `websync1` вы ввели в `C:\Inetpub\wwwroot\websync1` поле **Alias,** введите поле **Путь.** Нажмите кнопку **Далее**.  
...
11. Если службы IIS работают на компьютере под управлением 64-разрядной версии Windows, библиотека replisapi.dll должна быть скопирована в соответствующий каталог.  
    ...
    1.  Нажмите кнопку **Пуск** и выберите команду **Выполнить**. В **открытой** коробке, введите `iisreset`, а затем нажмите **OK**.  
    ...
    3.  Нажмите кнопку **Пуск** и выберите команду **Выполнить**. В **открытой** коробке, введите `cmd`, а затем нажмите **OK**.  
...
1.  Создайте на компьютере, где работают службы IIS, каталог для файла replisapi.dll. Этот каталог можно создать в любом месте, но рекомендуется разместить его в каталоге \<*диск*>:\Inetpub. Например, создайте каталог \<*диск*>:\Inetpub\SQLReplication\\.  
...
3.  Зарегистрируйте файл replisapi.dll:  
    ...
    1.  Нажмите кнопку **Пуск** и выберите команду **Выполнить**. В **открытой** коробке, введите `cmd`, а затем нажмите **OK**.  
...
1.  На сервере IIS создайте локальную учетную запись:  
    ...
    5.  Щелкните **Создать**, а затем выберите **Закрыть**.  
...
5.  Свяжите учетную запись с этим пулом приложений:  
    ...
    3.  В диалоговом ** \<окне ApplicationPoolName> свойства,** на вкладке **Identity** нажмите **Configurable.**  
