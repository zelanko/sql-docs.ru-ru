...
4.  Нажмите кнопку **ОК**.  
 ...
 Если не удается получить сертификат от центра сертификации, можно указать сертификат для тестирования. Чтобы настроить службы IIS 6.0 для тестирования, установите сертификат с помощью программы SelfSSL. Эта программа доступна в IIS 6.0 Resource Kit. Инструменты можно загрузить в [центре загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=5135). Дополнительные сведения об использовании служб IIS версии 5.0 см. на [веб-сайте справки и поддержки Майкрософт](https://go.microsoft.com/fwlink/?LinkId=46229).  
...
2.  Загрузите и установите программу SelfSSL. По умолчанию приложение устанавливается в \<*drive*> папку: \Program Files\IIS Resources\SelfSSL. Ярлыки приложений и документов копируются в \<*drive*> : \Documents and Settings\All Settings\all users\start Menu\Programs\IIS Resources\SelfSSL.  
...
2.  С установочного диска [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] запустите мастер установки [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . Дополнительные сведения об использовании этого мастера см. [в разделе Install SQL Server 2014 мастера установки &#40;&#41;установки ](../../database-engine/install-windows/install-sql-server-from-the-installation-wizard-setup.md).  
...
6.  На странице **Сведения виртуального каталога** выполните следующие действия:  
    ...
    2.  Введите путь виртуального каталога в поле **Путь** . Например, если ввести `websync1` в поле **псевдоним** , введите `C:\Inetpub\wwwroot\websync1` в поле **путь** . Щелкните **Далее**.  
...
11. Если службы IIS работают на компьютере под управлением 64-разрядной версии Windows, библиотека replisapi.dll должна быть скопирована в соответствующий каталог.  
    ...
    1.  Нажмите кнопку **Пуск**, затем щелкните **Выполнить**. В поле **Открыть** введите `iisreset` , а затем нажмите кнопку **ОК**.  
    ...
    3.  Нажмите кнопку **Пуск**, затем щелкните **Выполнить**. В поле **Открыть** введите `cmd` , а затем нажмите кнопку **ОК**.  
...
1.  Создайте на компьютере, где работают службы IIS, каталог для файла replisapi.dll. Каталог можно создать везде, где вы хотите, но мы рекомендуем создать каталог в \<*drive*> каталоге: \инетпуб. Например, создайте каталог \<*drive*> : \инетпуб\склрепликатион \\ .  
...
1.  На сервере IIS создайте локальную учетную запись:  
    ...
    1.  Щелкните правой кнопкой мыши ярлык **Мой компьютер** и выберите пункт **Управление**.  
...
5.  Свяжите учетную запись с этим пулом приложений:  
    ...
    3.  В диалоговом окне ** \<ApplicationPoolName> Свойства** на вкладке **удостоверение** щелкните **Настраиваемый**.  
...
6.  Свяжите пул приложений с виртуальным каталогом, используемым для веб-синхронизации:  
    ...
    3.  На вкладке **виртуальный каталог** диалогового окна ** \<VirtualDirectoryName> Свойства** в раскрывающемся списке **пул приложений** выберите пул приложений, созданный на шаге 5.  
...
4.  В диалоговом окне **Подключение к \<ServerName> ** укажите имя входа и пароль, которые агент слияния будет использовать для подключения к службам IIS. Эти учетные данные указываются также в мастере создания подписки.  
