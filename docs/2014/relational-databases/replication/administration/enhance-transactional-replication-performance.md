...
-   Используйте параметр **–SubscriptionStreams** для агента распространителя.  
     ...
     Значение для этого параметра агента можно указать, используя параметр **@subscriptionstreams** процедуры [sp_addsubscription (Transact-SQL)](/sql/relational-databases/system-stored-procedures/sp-addsubscription-transact-sql).  
