...
ms.locfileid: "85016784"
---
# <a name="ltagentnamegt-agent-security"></a>Защита агента &lt;Имя_агента&gt;
  На странице ** \<AgentName> Безопасность агента** можно указать учетные записи, с которыми агент распространения (для репликации транзакций и репликацией моментальных снимков) или агент слияния (для репликации слиянием) запускаются и подключаются к компьютерам в топологии репликации. Сведения о разрешениях, требуемых агентами, и об оптимальных методах защиты репликации см. в статьях [Модель безопасности агента репликации](security/replication-agent-security-model.md) и [Рекомендации по защите репликации](security/replication-security-best-practices.md).  
...
-   Для принудительных подписок локальным соединением является соединение с распространителем, поэтому в этом поле всегда будет отображаться следующее: **impersonate ' \<Domain> \\<login \> '** или **impersonate ' \<Computer> \\<login \> '** для принудительных подписок.  
  
-   Для подписок по запросу соединения также могут быть созданы в контексте имени входа [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. В поле отображается одно из следующих: **использовать имя входа " \<Login> "**, **Олицетворять " \<Domain> \\<login \> "** или **impersonate " \<Computer> \\<login \> "**. [!INCLUDE[msCoName](../../includes/msconame-md.md)] рекомендует выполнять соединение в контексте учетной записи Windows.  
...
-   Для принудительных подписок локальным соединением является соединение с издателем и распространителем, поэтому в этом поле всегда отображается: **impersonate " \<Domain> \\<login \> "** или **impersonate " \<Computer> \\<login \> "** для принудительных подписок.  
...
-   Для подписок по запросу локальным соединением является соединение с подписчиком, поэтому в этом поле всегда отображается: **impersonate " \<Domain> \\<login \> "** или **impersonate " \<Computer> \\<login \> "** для принудительных подписок.  
