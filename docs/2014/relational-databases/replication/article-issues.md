...
|Для столбцов IDENTITY необходим параметр NOT FOR REPLICATION.|Для репликации необходимо, чтобы столбцы IDENTITY использовали параметр NOT FOR REPLICATION. Если столбец IDENTITY не использует этот параметр, команды INSERT могут неверно выполнять репликацию.|Применяется к публикациям [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] , созданным на издателях с и более ранних версий. Для всех столбцов IDENTITY необходимо указать свойство NOT FOR REPLICATION.|  
...
|Необходимы таблицы, на которые выполняются ссылки в представлениях.|[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] требует, чтобы все таблицы, на которые имеются ссылки в публикуемых представлениях и индексированных представлениях, были доступны на подписчике. Если таблицы, на которые выполняются ссылки, не публикуются как статьи в этой публикации, они должны быть созданы на подписчике вручную.|Используйте кнопку **Назад** для перехода к странице **Статьи** . Добавьте необходимые объекты.|  
