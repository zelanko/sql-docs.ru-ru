...
# <a name="replication-queue-reader-agent"></a>Агент чтения очереди репликации
  Агент чтения очереди репликации — это исполняемый файл, который считывает сообщения, [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] хранящиеся в [!INCLUDE[msCoName](../../../includes/msconame-md.md)] очереди или очереди сообщений, а затем применяет эти сообщения к издателю. Агент чтения очереди используется совместно с публикациями транзакций и публикациями моментальных снимков, которые допускают обновление посредством очередей.  
...
|**3-5**|Вставлять в журнал новые записи, включая подробные сведения, которые могут оказаться полезными при устранении неполадок.|  
 ...
 **-Имя_профиля** _agent_profile_name_  
 ...
 **-QueryTimeOut** _query_time_out_seconds_  
 Число секунд до истечения времени ожидания запроса. Значение по умолчанию — 1800 секунд.  
