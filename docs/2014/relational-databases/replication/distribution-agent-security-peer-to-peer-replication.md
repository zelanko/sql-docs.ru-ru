 ...
 **Соединение с распространителем**  
 Контекст, в котором устанавливается соединение с распространителем. Локальные соединения всегда осуществляются с использованием контекста учетной записи Windows, с которой запущен агент. Этот мастер создает принудительные подписки (локальное соединение — соединение с распространителем), поэтому в этом поле всегда будет отображаться следующее: **impersonate ' \<Domain> \\<login \> '** или **impersonate ' \<Computer> \\<login \> '**.  
