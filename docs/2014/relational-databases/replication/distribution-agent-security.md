...
-   Для принудительных подписок эта учетная запись должна как минимум быть членом предопределенной роли базы данных **db_owner** в базе данных подписки.  
 ...
 Невозможно указать **учетную запись процесса** для подписок [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssExpress](../../includes/ssexpress-md.md)]по запросу на, так как агент распространения не выполняется на [!INCLUDE[ssExpress](../../includes/ssexpress-md.md)]экземплярах.  
