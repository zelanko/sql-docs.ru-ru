...
4.  Зарегистрируйте файл replisapi.dll:  
    ...
    1.  Нажмите кнопку **Пуск**, затем щелкните **Выполнить**. В **открытой** коробке, введите `cmd`, а затем нажмите **OK**.  
...
6.  Создайте виртуальный каталог в службах IIS. Виртуальный каталог должен быть создан на сайте, созданном в шаге 4, и сопоставлен с каталогом, созданным в шаге 1. Разрешения для этого каталога должны быть максимально ограничены. Необходимо выбрать как минимум разрешения **Чтение** и **Выполнение** .  
    ...
    2.  Для **Alias,** введите `SQLReplication`.  
...
1.  В **Интернет-информационных услуг (IIS) Менеджер**, нажмите **по умолчанию веб-сайт.**  
...
1.  На компьютере, на котором запущены службы IIS, создайте локальную учетную запись:  
    ...
    1.  Открытый **менеджер сервера**. В меню «Пуск» щелкните правой кнопкой мыши **Компьютер**и выберите **Управление**.  
...
## <a name="see-also"></a>См. также:  
 [Веб-синхронизация для репликации слияний](web-synchronization-for-merge-replication.md)   
 [Настройка синхронизации веб-страниц](configure-web-synchronization.md)  
