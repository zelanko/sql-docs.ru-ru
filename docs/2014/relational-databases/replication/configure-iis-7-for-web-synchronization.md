...
2.  Создайте на компьютере, где запущены службы IIS, новый каталог для файла replisapi.dll. Каталог можно создать везде, где вы хотите, но мы рекомендуем создать каталог в \<*drive*> каталоге: \инетпуб. Например, создайте каталог \<*drive*> : \инетпуб\склрепликатион \\ .  
...
6.  Создайте виртуальный каталог в службах IIS. Виртуальный каталог должен быть создан на сайте, созданном в шаге 4, и сопоставлен с каталогом, созданным в шаге 1. Разрешения для этого каталога должны быть максимально ограничены. Необходимо выбрать как минимум разрешения **Чтение** и **Выполнение** .  
    ...
    3.  В качестве **физического пути**введите ** \<drive> : \Инетпуб\склрепликатион \\ **и нажмите кнопку **ОК**.  
...
7.  Установите в настройках служб IIS разрешение выполнять файл replisapi.dll.  
    ...
    6.  В качестве **исполняемого файла**введите ** \<drive>:\Inetpub\SQLReplication\replisapi.dll**.  
...
3.  Предоставьте учетной записи минимальные разрешения для доступа к папке, содержащей библиотеку replisapi.dll:  
    ...
    3.  В диалоговом окне **разрешения для \<foldername> ** нажмите кнопку **добавить** , чтобы добавить учетную запись, созданную на шаге 1.  
