...
-   **Строка, число и двоичной контрольной суммы**. Кроме подсчета количества строк на подписчике и на издателе, вычисляется контрольная сумма всех данных с помощью алгоритма подсчета контрольной суммы. Если количество строк не совпадает, то контрольная сумма не проверяется.  
...
## <a name="articles-in-transactional-replication"></a>Статьи в репликации транзакций 
...
### <a name="using-transact-sql-t-sql"></a>С помощью Transact-SQL (T-SQL)
...
#### <a name="single-article"></a>Одну статью 
...
#### <a name="single-subscriber"></a>Одного подписчика
...
### <a name="using-replication-monitor"></a>С помощью монитора репликации
...
## <a name="for-a-single-push-subscription-to-a-merge-publication"></a>Для отдельной принудительной подписки на публикацию слиянием 
