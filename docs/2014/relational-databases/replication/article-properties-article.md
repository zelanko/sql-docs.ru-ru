 ...
 **Создавать схемы на подписчике**  
 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] представляет схемы, определяемые с помощью инструкции CREATE SCHEMA. Схема — это владелец объекта; Он используется в составе имени, например \<Database> . \<Schema> . \<Object> . Если в базе данных имеются объекты, принадлежащие схемам, отличным от DBO, то репликация может создать эти схемы на подписчике, чтобы можно было создать опубликованные объекты.  
