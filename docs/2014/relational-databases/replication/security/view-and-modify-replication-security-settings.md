...
1.  В диалоговом окне **Свойства подписки — \<подписка>** на издателе можно внести следующие изменения:  
    ...
    -   Чтобы изменить учетную запись, под которой агент распространения выполняется и устанавливает соединения с распространителем, щелкните строку **учетная запись процесса агента** , а затем нажмите кнопку свойств (**...**) в строке. Задайте учетную запись и пароль в диалоговом окне **Безопасность агента распространителя** .  
  
    -   Чтобы изменить контекст, в котором агент распространения подключается к подписчику, щелкните строку **соединение с подписчиком** , а затем нажмите кнопку свойств (**...**) в строке. Задайте контекст в диалоговом окне **Ввод сведений о соединении** .  
...
1.  В диалоговом окне **Свойства подписки — \<подписка>** на подписчике можно внести следующие изменения:  
    ...
    -   Чтобы изменить учетную запись, под которой агент распространения выполняется и устанавливает соединения с подписчиком, щелкните строку **учетная запись процесса агента** , а затем нажмите кнопку свойств (**...**) в строке. Задайте учетную запись и пароль в диалоговом окне **Безопасность агента распространителя** .  
  
         При использовании подписок, обновляемых посредством очередей, заданный здесь контекст используется также агентом чтения очереди для подключений к подписчику.  
  
    -   Чтобы изменить контекст, в котором агент распространения подключается к распространителю, щелкните строку **соединение с распространителем** , а затем нажмите кнопку свойств (**...**) в строке. Задайте контекст в диалоговом окне **Ввод сведений о соединении** .  
...
1.  В диалоговом окне **Свойства подписки — \<подписка>** на издателе можно внести следующие изменения:  
    ...
    -   Чтобы изменить учетную запись, под которой агент слияния выполняется и устанавливает соединения с издателем и распространителем, щелкните строку **учетная запись процесса агента** , а затем нажмите кнопку свойств (**...**) в строке. Задайте учетную запись и пароль в диалоговом окне **Безопасность агента слияния** .  
  
    -   Чтобы изменить контекст, в котором агент слияния подключается к подписчику, щелкните строку **соединение с подписчиком** , а затем нажмите кнопку свойств (**...**) в строке. Задайте контекст в диалоговом окне **Ввод сведений о соединении** .  
...
1.  В диалоговом окне **Свойства подписки — \<подписка>** на подписчике можно внести следующие изменения:  
    ...
    -   Чтобы изменить учетную запись, под которой агент слияния выполняется и устанавливает соединения с подписчиком, щелкните строку **учетная запись процесса агента** , а затем нажмите кнопку свойств (**...**) в строке. Задайте учетную запись и пароль в диалоговом окне **Безопасность агента слияния** .  
...
1.  В диалоговом окне **свойства \<подписки —>подписки** на подписчике щелкните строку **соединение с издателем** , а затем нажмите кнопку свойств (**...**) в строке.  
...
2.  В диалоговом окне **Ввод сведений о соединении** выберите один из следующих параметров:  
    ...
    -   **Используйте проверку Подлинности SQL Server со следующим именем входа и паролем**. Выберите этот параметр, если вы не определили удаленный сервер или связанный сервер между подписчиком и издателем. Репликация создаст связанный сервер. Указанная учетная запись уже должна существовать на издателе.  
...
3.  Повторите шаг 2 для каждого из следующих свойств безопасности, которые подвергаются изменениям.  
    ...
    > [!IMPORTANT]  
    >  При настройке издателя с удаленным распространителем значения, передаваемые для всех аргументов, включая **distrib_job_login** и **distrib_job_password**, передаются распространителю в виде обычного текста. Прежде чем выполнять эту хранимую процедуру, необходимо зашифровать соединение между издателем и его удаленным распространителем. Дополнительные сведения см. [в разделе Enable encrypted connections to the ядро СУБД &#40;диспетчер конфигурации SQL Server&#41;](../../../database-engine/configure-windows/enable-encrypted-connections-to-the-database-engine.md).  
...
3.  Повторите шаг 2 для каждого из следующих свойств безопасности, которые подвергаются изменениям.  
    ...
    > [!IMPORTANT]  
    >  При настройке издателя для работы с удаленным распространителем все передаваемые значения свойств, включая **merge_job_login** и **merge_job_password**, передаются распространителю в виде обычного текста. Прежде чем выполнять эту хранимую процедуру, необходимо зашифровать соединение между издателем и его удаленным распространителем. Дополнительные сведения см. [в разделе Enable encrypted connections to the ядро СУБД &#40;диспетчер конфигурации SQL Server&#41;](../../../database-engine/configure-windows/enable-encrypted-connections-to-the-database-engine.md).  
...
1.  В базе данных подписки на подписчике выполните процедуру [sp_link_publication](/sql/relational-databases/system-stored-procedures/sp-link-publication-transact-sql). Укажите ** \@издателя**, ** \@публикацию**, имя базы данных публикации для ** \@publisher_db**и одно из следующих значений для ** \@security_mode**.  
    ...
    -   **0** , чтобы использовать проверку подлинности SQL Server при выполнении обновлений на издателе. При выборе этого параметра необходимо указать допустимое имя входа на издателе для ** \@имени входа** и ** \@пароля**.  
  
    -   **1** , чтобы использовать контекст безопасности пользователя, который вносит изменения на подписчике при соединении с издателем. Связанные с этим режимом безопасности ограничения см. в разделе [sp_link_publication](/sql/relational-databases/system-stored-procedures/sp-link-publication-transact-sql) .  
  
    -   **2** для использования существующего, определяемого пользователем имени входа связанного сервера, созданного с помощью [sp_addlinkedserver &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql).  
...
3.  Вызовите метод <xref:Microsoft.SqlServer.Replication.ReplicationServer.ChangeReplicationServerPasswords%2A> . Укажите следующие параметры:  
    ...
    -   *Login* — имя входа, для которого изменяются все экземпляры пароля.  
...
4.  Вызовите метод <xref:Microsoft.SqlServer.Replication.ReplicationDatabase.LinkPublicationForUpdateableSubscription%2A> , передав следующие параметры.  
    ...
    -   *PublisherSecurity* — <xref:Microsoft.SqlServer.Replication.PublisherConnectionSecurityContext> объект, указывающий тип режима безопасности, используемого немедленно обновляемым подписчиком при соединении с издателем и учетными данными входа для подключения.  
