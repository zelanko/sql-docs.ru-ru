...
## <a name="event-notifications-concepts"></a>Основные понятия уведомлений о событиях  
 ...
 Сведения о событиях доставляются [!INCLUDE[ssSB](../../includes/sssb-md.md)] службы как переменная типа `xml` , предоставляющий сведения о происходит определенное событие, о затронутом объекте базы данных [!INCLUDE[tsql](../../includes/tsql-md.md)] использованном пакете инструкций, а также другие сведения. Дополнительные сведения о схеме XML, создаваемой уведомлениями о событиях, см. в статье [EVENTDATA (Transact-SQL)](/sql/t-sql/functions/eventdata-transact-sql).  
...
|Триггеры могут выполнять управляемый код Transact-SQL или CLR.|Уведомления о событиях не могут выполнять код. Вместо этого они отправляют `xml` сообщений для службы компонента Service Broker.|  
...
|Сведения о событии триггера DDL могут быть захвачены с использованием функции EVENTDATA, которая возвращает `xml` тип данных.|Уведомления о событии отправляют `xml` сведения о событиях в службу компонента Service Broker. Данные форматируются по той же схеме, что и функция EVENTDATA.|  
