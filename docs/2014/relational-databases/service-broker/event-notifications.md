...
## <a name="event-notifications-concepts"></a>Основные понятия уведомлений о событиях  
 ...
 Сведения о событии, доставляется [!INCLUDE[ssSB](../../includes/sssb-md.md)] службы как переменная типа `xml` , предоставляющий сведения о когда происходит событие, об объекте базы данных влияет, [!INCLUDE[tsql](../../includes/tsql-md.md)] использованном пакете инструкций, а также другие сведения. Дополнительные сведения о схеме XML, создаваемой уведомлениями о событиях, см. в статье [EVENTDATA (Transact-SQL)](/sql/t-sql/functions/eventdata-transact-sql).  
...
|Сведения о событии триггера DDL могут быть захвачены с использованием функции EVENTDATA, которая возвращает `xml` тип данных.|Уведомления о событиях отправляют `xml` информацию о событиях службы компонента Service Broker. Данные форматируются по той же схеме, что и функция EVENTDATA.|  
