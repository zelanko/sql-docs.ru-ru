...
> [!TIP]  
>  **Configuration Manager Kerberos для [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] — это диагностическое средство, помогающее устранять неполадки, связанные с подключением Kerberos, к. [!INCLUDE[msCoName](../../../includes/msconame-md.md)] ** [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Дополнительные сведения см. в разделе [Диспетчер конфигурации Microsoft Kerberos для SQL Server](https://www.microsoft.com/download/details.aspx?id=39046).  
...
-   [Техническое дополнение Kerberos для Windows](https://go.microsoft.com/fwlink/?LinkId=101449)  
...
|MSSQLSvc/*FQDN*:*порт*|Сформированное поставщиком имя участника-службы для экземпляра по умолчанию, когда используется протокол TCP.<br /><br /> *порт* является номером TCP-порта.|  
|MSSQLSvc/*FQDN*:*instanceName*|Сформированное поставщиком имя участника-службы (по умолчанию) для именованного экземпляра, когда используется протокол, отличный от TCP.<br /><br /> *InstanceName* — это [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] имя экземпляра.|  
...
|*Домен имени пользователя*@**|Прямое указание учетной записи домена.<br /><br /> *Username* — это имя учетной записи пользователя Windows.<br /><br /> *Домен* — это доменное имя Windows или полное доменное имя.|  
|**$@*Домен* MachineName|Прямое указание учетной записи компьютера<br /><br /> (Если сервер, к которому вы подключаетесь, работает под учетной записью локальной системы или сетевой службы, для `ServerSPN` получения проверки подлинности Kerberos может использоваться формат*домена* *MachineName*$@.)|  
|*Кдккэй*/*MachineName*|Заданное пользователем имя участника-службы.<br /><br /> *Кдккэй* — это буквенно-цифровая строка, которая соответствует правилам для ключа KDC.|  
...
-   [Имена участников-служб &#40;&#41; в клиентских соединениях &#40;ODBC&#41;](../odbc/service-principal-names-spns-in-client-connections-odbc.md)  
  
-   [Имена участников-служб &#40;&#41; в клиентских подключениях &#40;OLE DB&#41;](../ole-db/service-principal-names-spns-in-client-connections-ole-db.md)  
