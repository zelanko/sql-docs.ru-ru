...
|*MachineName*$@*Domain*|Прямое указание учетной записи компьютера<br /><br /> (Если сервер, к которому вы подключаетесь, работает под учетной записью локальной системы или сетевой службы, для `ServerSPN` получения проверки подлинности Kerberos может использоваться формат*домена* *MachineName*$@.)|  
