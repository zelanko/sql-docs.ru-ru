...
|Приложение OLE DB определяет метод проверки подлинности, используемый при установлении соединения.|Если соединение установлено успешно, то приложение может запросить свойство соединения `SSPROP_AUTHENTICATION_METHOD` в `DBPROPSET_SQLSERVERDATASOURCEINFO` свойство задано, чтобы определить, какой метод проверки подлинности был использован. Значения будут включены, но не ограничиваются, `NTLM` и `Kerberos`.|  
...
## <a name="linked-servers-and-delegation"></a>Связанные серверы и делегирование  
 При создании связанных серверов, `@provstr` параметр [sp_addlinkedserver](/sql/relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql) может использоваться для указания сервера и резервным имена участников-служб. Такой подход имеет те же преимущества, что и при указании имен субъектов-служб в строках соединения клиента: проще и надежнее установить соединения, использующие проверку подлинности Kerberos.  
...
|*MachineName*$@*Domain*|Прямое указание учетной записи компьютера<br /><br /> (Если вы подключаетесь к сервере с локальной СИСТЕМОЙ и учетные записи СЕТЕВОЙ службы, для проверки подлинности Kerberos, `ServerSPN` может находиться в *MachineName*$@*домена* формат).|  
...
-   [Имена участника-службы &#40;имена участников-служб&#41; в клиентских соединениях &#40;ODBC&#41;](../odbc/service-principal-names-spns-in-client-connections-odbc.md)  
  
-   [Имена участника-службы &#40;имена участников-служб&#41; в клиентских соединениях &#40;OLE DB&#41;](../ole-db/service-principal-names-spns-in-client-connections-ole-db.md)  
