...
|*Домен MachineName*$@*Domain*|Прямое указание учетной записи компьютера<br /><br /> (Если сервер, к которому вы подключаетесь, работает под учетными записями LOCAL SYSTEM или NETWORK SERVICE, чтобы получить аутентификацию `ServerSPN` Kerberos, можно в формате *MachineName*$@*Domain.)*|  
