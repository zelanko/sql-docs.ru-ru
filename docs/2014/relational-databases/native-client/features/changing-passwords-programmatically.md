...
> [!NOTE]  
>  По возможности следует предлагать пользователям вводить свои учетные данные во время выполнения, избегая их сохранения. Если необходимо сохранить свои учетные данные, зашифруйте их с помощью [API-интерфейса шифрования Win32](http://go.microsoft.com/fwlink/?LinkId=64532). Дополнительные сведения об использовании паролей см. в разделе [Strong Passwords](../../security/strong-passwords.md).  
...
### <a name="ole-db-user-interface-password-expiration"></a>Пользовательский интерфейс OLE DB для истечения срока действия пароля  
 ...
 Если свойство DBPROP_INIT_PROMPT было присвоено любое другое значение, пользователь увидит **имя входа SQL Server** диалога, независимо от того, является ли истечение срока действия пароля. Пользователь может щелкнуть **параметры** кнопку и установите флажок **Смена пароля** изменить пароль.  
  
 Если пользователь нажимает кнопку ОК, и срок действия пароля, [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] пользователю будет предложено ввести и подтвердить новый пароль с помощью **изменить пароль SQL Server** диалоговое окно.  
  
#### <a name="ole-db-prompt-behavior-and-locked-accounts"></a>Поведение подсказки OLE DB и заблокированные учетные записи  
 Попытки соединения могут окончиться неудачей, если учетная запись заблокирована. В этом случае вслед за выводом из **имя входа SQL Server** диалоговом окне пользователь видит сообщение об ошибке сервера и попытка соединения отменяется. Он может также произойти после отображения **изменить пароль SQL Server** диалоговое окно, если пользователь неправильно ввел старый пароль. В этом случае будет выведено то же сообщение об ошибке, а попытка соединения отменяется.  
...
>  Свойство SSPROP_AUTH_OLD_PASSWORD не нужно сохранять, поскольку оно используется только при истечении срока действия пароля.  
 ...
 Если используется проверка подлинности Windows, указав старый пароль приводит DB_E_ERRORSOCCURRED или DB_S_ERRORSOCCURRED в зависимости от ли старый пароль был указан как REQUIRED или OPTIONAL соответственно, а значение состояния DBPROPSTATUS_ CONFLICTINGBADVALUE возвращается в *dwStatus*. Это определяется при **IDBInitialize::Initialize** вызывается.  
...
### <a name="odbc-user-interface-password-expiration"></a>Пользовательский интерфейс ODBC истечения срока действия пароля  
 ...
 Если **DriverCompletion** было присвоено другое значение, он видит **имя входа SQL Server** диалога, независимо от того, является ли истечение срока действия пароля. Пользователь может щелкнуть **параметры** кнопку и установите флажок **Смена пароля** изменить пароль.  
