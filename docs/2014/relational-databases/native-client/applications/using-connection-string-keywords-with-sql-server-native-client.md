...
|`APP`|Имя приложения, вызывающего [SQLDriverConnect](../../native-client-odbc-api/sqldriverconnect.md) (необязательно). Если указано, это значение сохраняется в столбце **master. dbo. sysprocesses** **program_name** и возвращается с помощью [sp_who](/sql/relational-databases/system-stored-procedures/sp-who-transact-sql) и функций [APP_NAME](/sql/t-sql/functions/app-name-transact-sql) .|  
...
|`WSID`|Идентификатор рабочей станции. Обычно это сетевое имя компьютера, на котором находится приложение (необязательно). Если указано, это значение сохраняется **в столбце** **master. dbo. sysprocesses** и возвращается с помощью [sp_who](/sql/relational-databases/system-stored-procedures/sp-who-transact-sql) и функции [HOST_NAME](/sql/t-sql/functions/host-name-transact-sql) .|  
 ...
 **Примечание.** Строка подключения, передаваемая через **IDataInitialize::-DataSource** , преобразуется в свойства и применяется через **интерфейс IDBProperties:: SetProperties**. Если службы компонентов обнаруживают описание свойства в **IDBProperties::GetPropertyInfo**, это свойство будет применяться в качестве изолированного. В противном случае оно будет применяться через свойство DBPROP_PROVIDERSTRING. Например, если указать строку подключения **Data Source = Server1; Server = Server2** `Data Source` будет задан как свойство, но `Server` будет передаваться в строку поставщика.  
 ...
 **Примечание** . В строке подключения свойство "старый пароль" задает SSPROP_AUTH_OLD_PASSWORD — текущий пароль (возможно, истекший срок действия), недоступный через свойство строки поставщика.  
