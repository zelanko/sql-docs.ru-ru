...
|`WSID`|Идентификатор рабочей станции. Обычно это сетевое имя компьютера, на котором находится приложение (необязательно). Если указано, это значение хранится в **master.dbo.sysprocesses** столбца **hostname** и возвращается [sp_who](/sql/relational-databases/system-stored-procedures/sp-who-transact-sql) и [HOST_NAME](/sql/t-sql/functions/host-name-transact-sql)функции.|  
  
 **Региональные параметры преобразований применяются для валюты, числовые, даты и времени типов данных. Настройка преобразования применяется только к выходных преобразованиях и становится видимым, только когда валюты, числовые, значений даты или времени преобразуются в символьные строки**.  
