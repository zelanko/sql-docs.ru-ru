...
-   Подключения MDAC поддерживает логическое значение (`true`) для **Trusted_Connection** ключевое слово. Объект [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] собственным клиентом строка подключения должна содержать `yes` или **не**.  
