...
> [!TIP]
> Эти значения дают примерное представление об определении точки, в которой необходимо переключаться между `ALTER INDEX REORGANIZE` и `ALTER INDEX REBUILD`. Однако фактические значения могут различаться в каждом конкретном случае. Важно определить наилучшее пороговое значение для используемой среды экспериментальным путем. Например, если данный индекс используется главным образом для операций сканирования, удаление фрагментации может повысить производительность этих операций. Преимущество производительности менее заметно для индексов, используемых в основном для операций поиска. Аналогичным образом, удаление фрагментации в куче (таблица без кластеризованного индекса) особенно полезна для операций просмотра некластеризованных индексов, но практически не влияет на операции поиска.
...
При определенных условиях перестроение кластеризованного индекса автоматически перестраивает любой некластеризованный индекс, ссылающийся на ключ кластеризации, если физические или логические идентификаторы, содержащиеся в записях некластеризованного индекса, необходимо изменить.

Сценарии, которые принудительно заставляют все некластеризованные индексы автоматически перестроены в таблице:

-  Создание кластеризованного индекса для таблицы
-  Удаление кластеризованного индекса, в результате чего таблица сохраняется в виде кучи
-  Изменение ключа кластеризации для включения или исключения столбцов

Сценарии, не требующие автоматического перестройки всех некластеризованных индексов в таблице:

-  Перестроение уникального кластеризованного индекса
-  Перестроение неуникального кластеризованного индекса
