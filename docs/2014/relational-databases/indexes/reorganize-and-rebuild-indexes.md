...
-   **Перед началом работы**  
     ...
     [Обнаружение фрагментации](#Fragmentation)  
...
###  <a name="Fragmentation"></a>Обнаружение фрагментации  
 ...
 Результирующий набор, возвращаемый функцией **sys. dm_db_index_physical_stats** , содержит следующие столбцы.  
...
|**avg_fragmentation_in_percent** значение|Корректирующая инструкция|  
|-----------------------------------------------|--------------------------|  
|> 5% и \< = 30%|ALTER INDEX REORGANIZE|  
|> 30%|ALTER INDEX REBUILD WITH (ONLINE = ON) <sup>1</sup>|

<sup>1</sup> перестроение индекса может выполняться в оперативном или автономном режиме. Реорганизация индекса всегда выполняется в режиме "в сети". Чтобы добиться доступности, подобной варианту с реорганизацией, следует перестраивать индексы в режиме "в сети".  
...
Индекс нельзя реорганизовать или перестроить, если файловая группа, в которой он расположен, находится в автономном режиме или предназначена только для чтения. Если указывается ключевое слово `ALL`, а один индекс или несколько находятся в файловой группе, которая размещена вне сети или предназначена только для чтения, то выполнить инструкцию не удастся.
