 ...
 **Тип индекса**  
 Указывает тип индекса. Для новых индексов указывается тип индекса, выбранного при открытии диалогового окна. Индексы могут быть: **Clustered**, **некластеризованный**, **первичный XML**, **вторичный XML**, **пространственных**, **кластеризованный ColumnStore**, или **Nonclustered Columnstore**.  
 ...
 *Ограничивающий прямоугольник* представляет собой периметр сетки верхнего уровня геометрической плоскости. Параметры ограничивающего прямоугольника существуют только в тесселяции геометрической сетки. Эти параметры недоступны, если **Схемой тесселяции** является **Географическая сетка**.  
 ...
 На панели отображаются **(*`X-min`*,*`Y-min`*)** и **(*`X-max`*,*`Y-max`*)** координаты ограничивающего прямоугольника. Значения координат по умолчанию отсутствуют. Поэтому при создании нового пространственного индекса для столбца типа `geometry` необходимо указать значения координат.  
