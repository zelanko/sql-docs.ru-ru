...
-   Укажите атрибут `mapping schema` с помощью объекта `Command` ADO.  
 ...
 `mapping-schema`Атрибут, указанный в элементе ** \< xpath-Query>** или ** \< атрибута updg: Sync>** , имеет наивысший приоритет; `Command` объект ADO имеет самый низкий приоритет.  
