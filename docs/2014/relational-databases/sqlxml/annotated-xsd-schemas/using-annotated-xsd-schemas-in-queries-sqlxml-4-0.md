 ...
 **\<sql:xpath-query>** Элемент позволяет указать запрос XPath к XML-представлению, определяемому схемой с заметками. Схема с заметками, в которой будет выполняться запрос XPath, определяется с помощью `mapping-schema` атрибута **\<sql:xpath-query>** элемента.  
...
```  
 ...
 В шаблоне также задано два запроса XPath. Каждый из **\<xpath-query>** элементов однозначно определяет схему сопоставления, указывая `mapping-schema` атрибут.  
  
 При указании встроенной схемы в шаблоне `sql:is-mapping-schema` заметка также должна быть указана в **\<xsd:schema>** элементе. Заметка `sql:is-mapping-schema` имеет логическое значение (0 = false, 1 = true). Встроенная схема с **SQL: schema-Mapping-Schema = "1"** рассматривается как встроенная схема с заметками и не возвращается в XML-документе.  
 ...
 Помимо указания `mapping-schema` атрибута для **\<sql:xpath-query>** элемента в шаблоне (при наличии запроса XPath) или для **\<updg:sync>** элемента в диаграмма обновления, можно выполнить следующие действия.  
  
-   Укажите `mapping-schema` атрибут **\<ROOT>** элемента (глобальное объявление) в шаблоне. Теперь эта схема сопоставления стала схемой по умолчанию и будет использоваться всеми узлами XPath и диаграммами обновления без явного указания заметки `mapping-schema`.  
  
-   Укажите атрибут `mapping schema` с помощью объекта `Command` ADO.  
  
 `mapping-schema`Атрибут, указанный в **\<xpath-query>** элементе или, **\<updg:sync>** имеет наивысший приоритет; объект ADO `Command` имеет самый низкий приоритет.  
