...
```  
 ...
 Если для элемента была указана заметка `sql:hide`, то элемент и его атрибуты или дочерние элементы не будут отображены в созданном XML-документе. Вот другой схемы XSD, в котором `sql:hide` указано на  **\<OD >** элемента:  
