...
-   Столбец или столбцы, представляющие связь между родительской и дочерней таблицами. Например, столбец CustomerID, который присутствует как в родительской, так и в дочерней таблицах.  
 ...
 Для указания имен таблиц и необходимых сведений о соединении для заметки `sql:relationship` должны быть определены следующие атрибуты. Эти атрибуты действительны только с  **\<SQL: Relationship >** элемент:  
 ...
 **Inverse**  
 Этот атрибут, указанный в  **\<SQL: Relationship >** используется диаграмм обновления. Дополнительные сведения см. в разделе [указания значения атрибута SQL: inverse для SQL: Relationship](specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md).  
  
 `sql:key-fields` Заметка должна быть указана в элемент, содержащий дочерний элемент, который имеет  **\<SQL: Relationship >** определена между элементом и потомком, и который не предоставляет первичный ключ таблицы, указанной в родительском элементе. Даже если схема не соответствует  **\<SQL: Relationship >**, необходимо указать `sql:key-fields` для создания соответствующей иерархии. Дополнительные сведения см. в разделе [Определение ключевых столбцов с помощью SQL: Key-поля](identifying-key-columns-using-sql-key-fields-sqlxml-4-0.md).  
