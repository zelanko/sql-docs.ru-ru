...
-   Столбец или столбцы, представляющие связь между родительской и дочерней таблицами. Например, столбец CustomerID, который присутствует как в родительской, так и в дочерней таблицах.  
 ...
 Для указания имен таблиц и необходимых сведений о соединении для заметки `sql:relationship` должны быть определены следующие атрибуты. Эти атрибуты действительны только с  **\<SQL: Relationship >** элемента:  
 ...
 **Inverse**  
 Этот атрибут, указанный в  **\<SQL: Relationship >** используется диаграммами обновления. Дополнительные сведения см. в разделе [указания sql:inverse атрибута SQL: Relationship](specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md).  
  
 `sql:key-fields` Заметка должна быть указана в элементе, который содержит дочерний элемент, который имеет  **\<SQL: Relationship >** определена между элементом и потомком, и не предоставляет первичный ключ таблицы, указанной в родительском элементе. Даже если схема не задает  **\<SQL: Relationship >**, необходимо указать `sql:key-fields` для получения правильной иерархии. Дополнительные сведения см. в разделе [Определение ключевых столбцов с помощью SQL: Key-поля](identifying-key-columns-using-sql-key-fields-sqlxml-4-0.md).  
