...
-   Создание элемента контейнера, таких как  **\<заказы >** элемент, который переносит все заказы.  
 ...
 `sql:is-constant` Заметки можно добавлять к  **\<complexType >** элемента.  
...
### <a name="a-specifying-sqlis-constant-to-add-a-container-element"></a>A. Указание sql:is-constant для добавления элемента контейнера  
 В этой схемы XSD с заметками  **\<CustomerOrders >** определен как постоянный элемент указан `sql:is-constant` атрибут со значением 1. Таким образом  **\<CustomerOrders >** не сопоставлен для любой таблицы базы данных или столбца. Этот постоянный элемент состоит из  **\<порядок >** дочерних элементов.  
