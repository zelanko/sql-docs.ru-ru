...
2.  В **диспетчер конфигурации SQL Server**, разверните **сетевая конфигурация SQL Server**, а затем нажмите кнопку **протоколы для**  _\<имя_экземпляра >_.  
...
4.  Введите доступный номер порта в поле **TCP-порт** раздела **IPAll** . В этом учебнике мы будем использовать `49172`.  
...
##  <a name="browser"></a> Подключение с помощью службы обозревателя SQL Server  
