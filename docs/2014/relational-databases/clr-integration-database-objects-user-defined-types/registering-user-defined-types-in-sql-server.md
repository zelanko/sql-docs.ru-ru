...
```  
 ...
 Следующие [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкция регистрирует сборку с использованием *< assembly_bits >* аргумент в предложении FROM. Данное значение типа `varbinary` представляет файл в виде потока байтов.  
...
#### <a name="example"></a>Пример  
 Следующая инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] создает тип `Point`. EXTERNAL NAME указывается с помощью синтаксиса имен двух частей из *AssemblyName*. *UDTName*.  
