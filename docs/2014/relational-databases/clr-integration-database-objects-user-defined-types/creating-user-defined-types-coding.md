...
### <a name="parse-validation-example"></a>Пример проверки при синтаксическом анализе  
 Чтобы обеспечить вызов `ValidatePoint` метода в `Point` классе, необходимо вызвать его из `Parse` метода и из процедур свойств, устанавливающих значения координат X и Y. В следующем фрагменте кода показано, как вызвать `ValidatePoint` метод проверки из `Parse` функции.  
...
> [!NOTE]  
>  Класс `SqlMethodAttribute` наследуется от `SqlFunctionAttribute`, поэтому класс `SqlMethodAttribute` наследует поля `FillRowMethodName` и `TableDefinition` от класса `SqlFunctionAttribute`. Это подразумевает, что существует возможность написать возвращающий табличное значение метод, который не является вариантом. Метод компилируется, а сборка развертывается, но ошибка типа возвращаемого значения `IEnumerable` вызывается во время выполнения со следующим сообщением: "метод, свойство или поле"\<имя> "в классе" класс "\<>" в сборке "\<сборка>" имеет недопустимый возвращаемый тип. "  
