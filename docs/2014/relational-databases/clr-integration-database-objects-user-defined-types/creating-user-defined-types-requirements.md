...
-   Структура или класс определяемого пользователем типа должны реализовать интерфейс `System.Data.SqlTypes.INullable` посредством создания метода `static` типа `Shared` ([!INCLUDE[msCoName](../../includes/msconame-md.md)] в `Null` Visual Basic). 
 ...
 **bool**, **Byte**, **SByte**, **Short**, **UShort**, **int**, **uint**, **Long**, **ulong**, **float**, **Double**, **склбите**, **SqlInt16**, **SqlInt32**, **SqlInt64**, **SqlDateTime**, **склсингле**, **SqlDouble**, **SqlMoney**, **SqlBoolean**  
 ...
 Типы значений, состоящие из полей приведенных выше типов, являются хорошим кандидатом для `Native` формата, `structs` например в Visual C# (или `Structures` в том случае, если они известны в Visual Basic). Например, определяемый пользователем тип, для которого указан формат сериализации `Native`, может содержать в качестве поля другой определяемый пользователем тип, для которого также указан формат сериализации `Native`. Для более сложных определяемых пользователем типов, содержащих типы данных, которые не вошли в приведенный список, нужно вместо этого задать формат сериализации `UserDefined`.  
...
-   Если определяемый пользователем тип задан в виде класса, а не структуры, нужно задать для атрибута `System.Runtime.InteropServices.StructLayoutAttribute` значение `StructLayout.LayoutKindSequential`. Этот атрибут управляет физической компоновкой полей данных. Он заставляет члены структуры располагаться в памяти в том порядке, в каком они описаны. 
 ...
 Пример определяемого пользователем типа, определяемого `Native` с помощью сериализации, см. в разделе [определение определяемых пользователем типов](creating-user-defined-types-coding.md)данных.  
...
-   Указать необязательное свойство атрибута `IsByteOrdered`. По умолчанию используется значение `false`.  
...
-   Написать программный код, реализующий методы `Read` и `Write` данного определяемого пользователем типа, реализовав интерфейс `System.Data.Sql.IBinarySerialize`.  
 ...
 Пример определяемого пользователем типа, определяемого `UserDefined` с помощью сериализации, см. в разделе Определение типа валюты в [коде определяемых пользователем типов](creating-user-defined-types-coding.md).  
...
### <a name="string-conversions"></a>Преобразование строк  
 Для поддержки преобразования определяемого пользователем типа в строку и восстановления из строки нужно реализовать в классе методы `Parse` и `ToString`. Метод `Parse` позволяет преобразовывать строку в определяемый пользователем тип. Он должен быть объявлен как `static` (или `Shared` в Visual Basic) и принимать параметр типа `System.Data.SqlTypes.SqlString`. Дополнительные сведения и пример реализации методов `Parse` и `ToString` см. в разделе [кодирование определяемых пользователем типов](creating-user-defined-types-coding.md).  
