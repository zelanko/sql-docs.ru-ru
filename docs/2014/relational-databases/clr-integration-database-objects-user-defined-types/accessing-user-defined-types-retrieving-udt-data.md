...
### <a name="example-using-getsqlbytes"></a>Пример использования функции GetSqlBytes  
 В этом примере показано, как получить **точки** данные в виде необработанных байтов в одной операции с помощью **метода GetSqlBytes** метод. В этом коде используется класс `StringBuilder` для преобразования байтового в символьное представление, применяемое для отображения в окне консоли.  
