...
-   Покрытие для огромного количества языков, разбиения по словам, включаются в [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] поставляются и включен по умолчанию.  
...
> [!NOTE]  
>  Код языка используется для всех типов данных, поддерживающих полнотекстовое индексирование (например, `char` или `nchar`). Если у вас есть порядок сортировки `char`, `varchar`, или `text` столбца с типом для языка, отличный от языка, заданного соответствующим кодом LCID все равно будет использоваться во время полнотекстового индексирования и запросов этих столбцов.  
...
-   Для содержимого в виде неформатированного текста  
     ...
     Если содержимое является неформатированным текстом, можно преобразовать его в `xml` тип данных и затем добавить языковые теги, показывающие язык, соответствующий определенному документу или раздел документа. Для этого, однако, необходимо знать до выполнения полнотекстового индексирования, какой язык будет использоваться.  
...
##  <a name="type"></a> Влияние типа столбца на полнотекстовый поиск  
 Другой фактор, который следует учитывать при выборе языка, связан со способом представления данных. Для данных, хранящихся в `varbinary(max)` столбец, специальная фильтрация не выполняется. Вместо этого текст обычно пропускается через компонент разбиения по словам «как есть».  
