...
-   Покрытие огромного количества языков, средства разбиения по словам, включаются в [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] поставляются и включен по умолчанию.  
  
 Список языков, для которых [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] включает средство разбиения по словам и парадигматические модули, в разделе [sys.fulltext_languages &#40;Transact-SQL&#41;](/sql/relational-databases/system-catalog-views/sys-fulltext-languages-transact-sql).  
...
> [!NOTE]  
>  Код языка используется для всех типов данных, поддерживающих полнотекстовое индексирование (например, `char` или `nchar`). Если у вас есть порядок сортировки `char`, `varchar`, или `text` столбца с типом для языка, отличный от языка, заданного соответствующим кодом LCID все равно будет использоваться во время полнотекстового индексирования и запросов из этих столбцов.  
...
-   Для содержимого в виде неформатированного текста  
     ...
     Если содержимое является неформатированным текстом, преобразуйте ее в `xml` тип данных, а затем добавить языковые теги, показывающие язык, соответствующий определенному документу или раздела документа. Для этого, однако, необходимо знать до выполнения полнотекстового индексирования, какой язык будет использоваться.  
...
##  <a name="type"></a> Влияние типа столбца на полнотекстовый поиск  
 Другой фактор, который следует учитывать при выборе языка, связан со способом представления данных. Для данных, которые не хранятся в `varbinary(max)` столбец, специальная фильтрация не выполняется. Вместо этого текст обычно пропускается через компонент разбиения по словам «как есть».  
