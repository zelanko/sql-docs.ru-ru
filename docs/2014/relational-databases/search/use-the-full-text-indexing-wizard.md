...
1.  В обозревателе объектов щелкните правой кнопкой мыши таблицу, для которой необходимо создать полнотекстовый индекс, укажите **Полнотекстовый индекс**и щелкните **Определить полнотекстовый индекс**.  
     ...
     **Столбец типа** доступна только в том случае, если столбец, указанный в **доступные столбцы** столбец имеет тип `varbinary(max)` или `image`.  
...
2.  Выбор параметров отслеживания изменений.  
     ...
     **Начать полное заполнение после создания индекса**  
...
3.  Выбор каталога, файловой группы индекса и списка стоп-слов.  
    ...
    |Значение|Описание|  
    |-----------|-----------------|  
    |**\<Система >**|Выберите это значение, чтобы использовать в новом полнотекстовом индексе системный список стоп-слов. Это значение установлено по умолчанию.|  
    |**\<Отключение >**|Выберите это значение, чтобы отключить списки стоп-слов в новом полнотекстовом индексе.|  
