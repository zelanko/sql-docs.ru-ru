...
|x64|*F* **=** *Number of crawl ranges* **\*** 10 **\*** 8|*M* **=** *T* **-** *F* **-** 500|  
 ...
 <sup>1</sup> Если выполняется несколько полных заполнений, вычислять требования к памяти fdhost.exe для каждого из них отдельно, как *F1*, *F2*, и т. д. Затем следует вычислить *M* как *T ***-** sigma **(***F*я**) **.  
