...
|**Перемен**|**Значение по умолчанию**|  
|------------------|-----------------------|  
|*number_of_crawl_ranges*|Количество ЦП|  
...
-   *F*, который представляет собой приблизительный объем памяти, необходимый для fdhost. exe (в МБ).  
...
|Платформа|Оценка требований к памяти fdhost. exe в МБ-*F*<sup>1</sup>|Формула для вычисления максимального объема памяти сервера-*M*<sup>2</sup>|  
|--------------|---------------------------------------------------------------------|---------------------------------------------------------------|  
|x86|_F_ **=** _число диапазонов сканирования_ **&#42;** 50|_M_ **= минимум (** _T_ **,** 2000 **) —*`F`* ** 500|  
|x64|_F_ **=** _число диапазонов сканирования_ **&#42;** 10 **&#42;** 8|_M_ **=** __ T **-** __ F **-** 500|  
  
 <sup>1</sup> если выполняется несколько полных заполнений, вычислите требования к памяти fdhost. exe отдельно, в виде *F1*, *F2*и т. д. Затем вычислить *M* как _T_**-** sigma **(**_F_i **)**.  
 ...
 **Пример. Оценка требований к памяти fdhost.exe**  
 ...
 При следующем вычислении получается оптимальное значение `max server memory` -для *M*. ** Общий объем физической памяти, доступной в этой системе в МБ-*t*, — `8192`.  
...
##  <a name="filters"></a>Устранение снижения производительности индексирования из-за фильтров  
