...
## <a name="remarks"></a>Примечания  
 ...
 Вызовите [bcp_init](bcp-init.md) перед вызовом этой или любой другой функции массового копирования. Вызов **bcp_init** задает [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] целевой таблицы для массового копирования. При вызове **bcp_init** для использования с **bcp_bind** и [bcp_sendrow](bcp-sendrow.md), **bcp_init** _szDataFile_параметр, указывающий файл данных имеет значение NULL; **bcp_init**_eDirection_ параметр имеет значение DB_IN.  
 ...
 При необходимости [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] для фиксации уже полученные строки, вызовите [bcp_batch](bcp-batch.md). Например, вызвать **bcp_batch** один раз для каждой 1 000 вставленных строк или через любой другой интервал.  
  
 Если больше нет строк для вставки, вызовите [bcp_done](bcp-done.md). Несоблюдение этого правила приведет к ошибке.  
  
 Настройки параметров управления, указанный с помощью [bcp_control](bcp-control.md), не оказывают влияния на **bcp_bind** передачу строк.  
  
 Если *pData* для столбца имеет значение NULL, так как его значение будет предоставлено вызовами [bcp_moretext](bcp-moretext.md), любой последующий столбец со *eDataType* присвоено SQLTEXT, SQLNTEXT, SQLXML, SQLUDT, SQLCHARACTER, SQLVARCHAR, SQLVARBINARY, SQLBINARY, SQLNCHAR или SQLIMAGE должен быть также связан с *pData* присваивается значение NULL, и их значения, также должен быть указан при вызове `bcp_moretext`.  
  
 Для новых типов больших значений таких как `varchar(max)`, `varbinary(max)`, или `nvarchar(max)`, можно использовать значения SQLCHARACTER, SQLVARCHAR, SQLVARBINARY, SQLBINARY и SQLNCHAR качестве индикаторов типа в *eDataType* параметра.  
...
## <a name="bcpbind-support-for-enhanced-date-and-time-features"></a>Поддержка функцией bcp_bind улучшенных возможностей даты и времени  
 Сведения о типах, используемых с *eDataType* параметров для типов даты и времени, см. в разделе [изменения массового копирования для типов усиленной даты и времени &#40;OLE DB и ODBC&#41;](../native-client-odbc-date-time/bulk-copy-changes-for-enhanced-date-and-time-types-ole-db-and-odbc.md).  
