...
## <a name="remarks"></a>Примечания  
 ...
 Вызовите [bcp_init](bcp-init.md) перед вызовом этой или любой другой функции массового копирования. Вызов **bcp_init** задает [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] целевой таблицы для массового копирования. При вызове **bcp_init** для использования с **bcp_bind** и [bcp_sendrow](bcp-sendrow.md), **bcp_init** *szDataFile*параметр, указывающий файл данных имеет значение NULL; **bcp_init *** eDirection* параметр имеет значение DB_IN.  
 ...
 При необходимости [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] зафиксировать уже полученные строки, вызовите [bcp_batch](bcp-batch.md). Например, вызов **bcp_batch** один раз каждые 1 000 вставленных строк или любой другой интервал.  
  
 Если больше нет строк для вставки, вызовите [bcp_done](bcp-done.md). Несоблюдение этого правила приведет к ошибке.  
  
 Настройки параметров управления, указанный с [bcp_control](bcp-control.md), не оказывают влияния **bcp_bind** передачу строк.  
  
 Если *pData* для столбца имеет значение NULL, так как его значение будет предоставлено вызовами [bcp_moretext](bcp-moretext.md), все последующие столбцы с *eDataType* присвоено SQLTEXT, SQLNTEXT, SQLXML, SQLUDT, SQLCHARACTER, SQLVARCHAR, SQLVARBINARY, SQLBINARY, SQLNCHAR или SQLIMAGE должен быть также связан с *pData* присваивается значение NULL, а также необходимо указать их значения путем вызова метода `bcp_moretext`.  
  
 Для новых типов больших значений таких как `varchar(max)`, `varbinary(max)`, или `nvarchar(max)`, SQLCHARACTER, SQLVARCHAR, SQLVARBINARY, SQLBINARY и SQLNCHAR можно использовать в качестве индикаторов типа в *eDataType* параметра.  
...
## <a name="bcpbind-support-for-enhanced-date-and-time-features"></a>Поддержка функцией bcp_bind улучшенных возможностей даты и времени  
 Дополнительные сведения о типах, используемых с *eDataType* параметров для типов даты и времени, в разделе [изменения массового копирования для улучшенной даты и времени &#40;OLE DB и ODBC&#41;](../native-client-odbc-date-time/bulk-copy-changes-for-enhanced-date-and-time-types-ole-db-and-odbc.md).  
