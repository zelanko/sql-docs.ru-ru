...
```  
 ...
 64-разрядные программы чтения `Installed Versions` ключа будут отображены значения, указывающие на 64-разрядной версии **SQLUserInstance** библиотеки DLL, хотя 32-разрядные программы (выполняющиеся в 64-разрядной версии Windows в режиме WOW64) будет автоматически перенаправлено `Installed Versions` ключ в компоненте `Wow6432Node` hive. Этот раздел содержит значения, указывающие на 32-разрядные версии **SQLUserInstance** DLL.  
