...
|`Terminate`|Открытый return_type Terminate ();|Этот метод завершает статистическое вычисление и возвращает результат статистической обработки. *Return_type* должен быть [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] управляемым типом данных, который является управляемым эквивалентом *return_sqltype* , указанного в `CREATE AGGREGATE` инструкции. *Return_type* также может быть определяемым пользователем типом.|  
