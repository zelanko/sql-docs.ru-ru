...
-   `SqlPipe`. `SqlPipe` Объект представляет «канал», по которому результаты передаются клиенту. Дополнительные сведения о `SqlPipe` объекта, см. в разделе [объект SqlPipe](sqlpipe-object.md).  
