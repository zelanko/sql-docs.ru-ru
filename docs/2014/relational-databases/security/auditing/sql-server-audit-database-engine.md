...
-   Чтобы спецификация аудита базы данных могла создавать записи аудита, на зеркальном сервере должен присутствовать аудит с тем же идентификатором GUID. Это может быть настроено с помощью команды CREATE AUDIT WITH GUID`=`*\<GUID из source Server Audit*>.  
...
### <a name="auditing-administrators"></a>Администраторы аудита  
 Члены `sysadmin` предопределенной роли сервера определены как **dbo** пользователя в каждой базе данных. Действия аудита администраторов выполняются как действия аудита пользователя **dbo** .  
