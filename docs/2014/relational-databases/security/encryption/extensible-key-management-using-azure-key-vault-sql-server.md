...
# <a name="extensible-key-management-using-azure-key-vault-sql-server"></a>Расширенное управление ключами с помощью хранилища ключей Azure (SQL Server)
  Соединитель [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] для [!INCLUDE[msCoName](../../../includes/msconame-md.md)] Azure Key Vault позволяет шифровать [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], чтобы использовать службу Azure Key Vault в качестве поставщика [расширенного &#40;&#41; управления ключами](extensible-key-management-ekm.md) для защиты ключей шифрования.  
  
 Разделы данной темы  
  
-   [Использование расширенного управления ключами](#Uses)  
  
-   [Шаг 1. Настройка Key Vault для использования SQL Server](#Step1)  
  
-   [Шаг 2. Установка соединитель SQL Server](#Step2)  
  
-   [Шаг 3. Настройка SQL Server для использования поставщика расширенного управления ключами для Key Vault](#Step3)  
  
-   [Пример а. прозрачное шифрование данных с помощью асимметричного ключа из Key Vault](#ExampleA)  
  
-   [Пример б. шифрование резервных копий с помощью асимметричного ключа из Key Vault](#ExampleB)  
  
-   [Пример в. шифрование на уровне столбцов с помощью асимметричного ключа из Key Vault](#ExampleC)  
...
##  <a name="Uses"></a>Использование расширенного управления ключами  
 ...
 ![SQL Server расширенного управления ключами с помощью Azure Key Vault](../../../database-engine/media/ekm-using-azure-key-vault.png "SQL Server расширенного управления ключами с помощью Azure Key Vault")  
...
2.  Настройте учетные данные [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] для входа администратора [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] . С их помощью хранилище ключей будет использоваться для настройки сценариев шифрования [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] и управления ими.  
    ...
    > [!IMPORTANT]  
    >  Аргумент **IDENTITY** `CREATE CREDENTIAL` требует указания имени хранилища ключей. **Секретный** аргумент `CREATE CREDENTIAL` требует, чтобы *\<идентификатор клиента >* (без дефисов) и *\<секрета >* передаваться вместе без пробела между ними.  
    ...
    ```  
     ...
     Пример использования переменных для `CREATE CREDENTIAL` аргументов и программного удаления дефисов из идентификатора клиента см. в разделе [CREATE CREDENTIAL &#40;Transact-SQL&#41;](/sql/t-sql/statements/create-credential-transact-sql).  
