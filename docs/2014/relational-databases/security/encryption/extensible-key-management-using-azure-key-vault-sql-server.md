...
2.  **Создание асимметричного ключа в хранилище:** асимметричного ключа в хранилище ключей используется для защиты [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] ключей шифрования. Только открытая часть асимметричного ключа покидает хранилище, частная же область никогда не экспортируется. Все криптографические операции, использующие асимметричный ключ, делегируются в хранилище ключей Azure и защищаются системой безопасности хранилища.  
     ...
     [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Соединителю асимметричные ключи RSA 2048-разрядный, а имя ключа можно использовать только символы «a-z», «A – Z», «0-9", и «-». В этом документе имя асимметричного ключа — **ContosoMasterKey**. Замените его на уникальное имя ключа.  
...
3.  **Получение субъектов-служб Azure Active Directory для SQL Server.** Когда организация регистрируется в облачной службе Майкрософт, она получает Azure Active Directory. Создание **субъекты-службы** в Azure Active Directory для [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] использовать (для проверки подлинности в Azure Active Directory) при доступе к хранилищу ключей.  
  
    -   Один **субъекта-службы** понадобится [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] администратора для доступа к хранилищу при настройке [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] использовать шифрование.  
  
    -   Другой **субъекта-службы** понадобится [!INCLUDE[ssDEnoversion](../../../includes/ssdenoversion-md.md)] доступа к хранилищу для распаковки ключей, используемых в [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] шифрования.  
...
-   **Microsoft.AzureKeyVaultService.EKM.dll**: это поставщику EKM DLL, должен быть зарегистрирован с [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] с помощью инструкции CREATE CRYPTOGRAPHIC PROVIDER.  
...
-   **Соединитель SQL Server для хранилища ключей Azure**— это служба Windows, которая позволяет поставщику EKM взаимодействовать с хранилищем ключей.  
 ...
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Установки соединителя также позволяет при необходимости скачайте примеры сценариев для [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] шифрования.  
...
###  <a name="ExampleC"></a> Пример в. шифрование на уровне столбцов с помощью асимметричного ключа из хранилища ключей  
