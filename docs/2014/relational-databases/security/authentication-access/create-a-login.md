...
3.  В диалоговом окне **Имя входа — создание** на странице **Общие** введите имя пользователя в поле **Имя для входа**. Также можно нажать кнопку **Поиск...**, чтобы открыть диалоговое окно **Выбор пользователя или группы**.  
    ...
    2.  В списке **Из этого расположения** щелкните **Расположение...**, чтобы открыть диалоговое окно **Расположение**, и выберите одно из следующих доступных расположений сервера. По окончании нажмите кнопку **ОК**.  
...
5.  Чтобы создать имя входа, которое сохраняется в базе данных [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] , выберите **Проверка подлинности SQL Server**.  
    ...
    3.  Чтобы обеспечить использование паролей в соответствии с политиками паролей, выберите параметр **Требовать использование политики паролей**. Дополнительные сведения см. в разделе [Политика паролей](../password-policy.md). Этот параметр используется по умолчанию при выборе **SQL Server проверки подлинности** .  
  
    4.  Чтобы обеспечить использование паролей в соответствии с политиками окончания срока действия паролей, выберите параметр **Задать срок окончания действия пароля**. Для включения этого флажка должен быть установлен флажок **требовать использование политики паролей** . Этот параметр используется по умолчанию при выборе **SQL Server проверки подлинности** .  
  
    5.  Чтобы пользователь мог создать новый пароль при первом входе в систему, выберите пункт **Пользователь должен сменить пароль при следующем входе**. Чтобы этот флажок был доступен, должен быть выбран параметр **требовать истечения срока действия пароля** . Этот параметр используется по умолчанию при выборе **SQL Server проверки подлинности** .  
...
9. В списке **База данных по умолчанию** выберите базу данных по умолчанию для нужного имени входа. Значение по умолчанию для этого параметра — **master** .  
...
### <a name="additional-options"></a>Дополнительные параметры  
 Диалоговое окно « **Создание имени входа** » также предлагает параметры на четырех дополнительных страницах: **роли сервера**, **Сопоставление пользователей**, **защищаемые объекты**и **состояние**.  
...
### <a name="server-roles"></a>Роли сервера  
 ...
 Члены предопределенной роли сервера « **администратора** » управляют именами входа и их свойствами. Они могут предоставлять, запрещать и отменять разрешения на уровне сервера (инструкции GRANT, DENY и REVOKE). Они также могут предоставлять, запрещать и отменять разрешения на уровне базы данных (инструкции GRANT, DENY и REVOKE). Кроме того, они могут сбрасывать пароли для имен входа [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] .  
...
3.  Если выбрать **конкретные объекты...**:  
  
    1.  В диалоговом окне **Выбор объектов** в разделе **выберите типы объектов**выберите **типы объектов..**..  
...
4.  Если выбрать **все объекты типов..**., в диалоговом окне **Выбор типов объектов** выберите один или все из следующих типов объектов: **конечные точки**, **имена входа**, **серверы**, **группы доступности**и **роли сервера**. [!INCLUDE[clickOK](../../../includes/clickok-md.md)]  
...
##  <a name="follow-up-steps-to-take-after-you-create-a-login"></a><a name="FollowUp"></a>Дальнейшие действия. шаги, выполняемые после создания имени входа  
