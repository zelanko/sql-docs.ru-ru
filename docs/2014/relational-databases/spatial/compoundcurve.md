...
1.  `CompoundCurve` Экземпляр считается правильными.  
...
```  
 ...
 Длина экземпляров `@g1` и `@g2` одинакова. На этом примере видно, `CompoundCurve` может хранить один или несколько экземпляров `LineString`.  
