...
```  
 ...
 `@g3` Показывает, что `CircularString` экземпляр может быть правильным, но не является допустимым. Следующее объявление экземпляра CircularString неверно. Это объявление вызывает исключение `System.FormatException`.  
...
### <a name="valid-instances"></a>Допустимые экземпляры  
 Является допустимым `CircularString` экземпляр должен быть пустым или иметь следующие атрибуты:  
...
-   Несмотря на то что `CircularString` экземпляров могут содержать линейные сегменты, эти линейные сегменты должны определяться тремя лежащими на одной прямой точками.  
  
 В следующем примере показаны допустимые `CircularString` экземпляров.  
...
```  
 ...
 Для определения окружности экземпляр `CircularString` должен содержать как минимум два сегмента дуги. Объект `CircularString` экземпляр не может использовать один сегмент дуги (например, (1 1, 3 1, 1 1)) для определения окружности. Для определения окружности используйте (1 1, 2 2, 3 1, 2 0, 1 1).  
...
### <a name="c-instantiating-a-geometry-instance-using-a-circularstring-with-multiple-circular-arc-segments"></a>В. Создание экземпляра Geometry с помощью экземпляра CircularString с несколькими сегментами дуги  
 В следующем примере показано, как создать `CircularString` экземпляра с более чем один сегмент дуги (окружности):  
...
```  
 ...
 Обратите внимание, что значение `CircularString` приведен близко к 2∏, то есть действительному круга углу.  
