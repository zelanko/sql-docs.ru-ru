...
ms.locfileid: "36109681"
---
# <a name="create-construct-and-query-geometry-instances"></a>Создание, конструирование и запрос экземпляров geometry
  Плоские Пространственные данные типа, `geometry`, представляет данные в Евклидовой (плоской) системе координат. В [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]этот тип реализован как тип данных среды CLR.  
  
 `geometry` Тип является стандартным и доступен в каждой базе данных. Можно создать столбцов таблицы типа `geometry` и работать с `geometry` данных так же, как вы используете других типов среды CLR.  
...
###  <a name="existing"></a> Создание нового экземпляра геометрического объекта из существующего экземпляра  
 `geometry` Тип данных содержит множество встроенных методов, которые можно использовать для создания новых `geometry` экземпляров на основе существующих.  
...
###  <a name="wkb"></a> Построение экземпляра геометрического объекта на основе входных данных в формате Well-Known Binary Input  
 WKB представляет собой двоичный формат, указанный по Open Geospatial Consortium (OGC), позволяющий `geometry` данных обмен между клиентским приложением и базой данных SQL. Следующие функции допускают создание геометрических объектов на основе входных данных формата WKB.  
...
###  <a name="gml"></a> Построение экземпляра геометрического объекта на основе входных данных в формате GML Text  
 `geometry` Тип данных предоставляет метод, который приводит к возникновению ошибки `geometry` экземпляра на основе GML, XML-представления геометрических объектов. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] поддерживает подмножество GML.  
...
##  <a name="returning"></a> Получение данных в формате Well-Known Text и Well-Known Binary из геометрического экземпляра  
 Можно использовать следующие методы для возврата либо в формате WKT или формате WKB из `geometry` экземпляр:  
...
###  <a name="valid"></a> Допустимость, тип экземпляра и сведения GeometryCollection  
 Один раз `geometry` создан, можно использовать следующие методы для определения правильного формата, получить типа экземпляра или, если это экземпляр коллекции, получить определенный `geometry` экземпляра.  
...
###  <a name="number"></a> Число точек  
 Все непустые `geometry` экземпляров состоят из *точки*. Эти точки представляют координаты X и Y на плоскости, где вычерчиваются геометрические объекты. `geometry` предоставляет многочисленные встроенные методы для создания запросов к точкам экземпляра.  
...
###  <a name="dimension"></a> Измерение  
 Непустой `geometry` экземпляр может иметь 0, 1 или 2 измерения. 0 измерений `geometries`, такие как `Point` и `MultiPoint`, не имеют длины или области. Одномерные объекты, такие как `LineString, CircularString, CompoundCurve`, и `MultiLineString`, имеют длину. Двумерные объекты, такие как `Polygon`, `CurvePolygon` и `MultiPolygon`, имеют длину и площадь. Пустые экземпляры имеют измерение -1, а экземпляр `GeometryCollection` имеет площадь, зависящую от типов его содержимого.  
 ...
 *Внутренних* из `geometry` — это пространство, занимаемое экземпляром и *внешнее* — пространство не занимаемое им.  
...
###  <a name="closure"></a> Замыкание  
 Объект *закрыт* `geometry` — это фигура, начальная и конечная точки совпадают. `Polygon` считаются замкнутыми. Экземпляры `Point` не замкнуты.  
