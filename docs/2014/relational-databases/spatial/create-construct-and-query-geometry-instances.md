...
###  <a name="number"></a> Число точек  
 Все непустые `geometry` экземпляры состоят из *точек*. Эти точки представляют координаты X и Y на плоскости, где вычерчиваются геометрические объекты. 
  `geometry` предоставляет многочисленные встроенные методы для создания запросов к точкам экземпляра.  
