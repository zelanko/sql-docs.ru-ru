...
8.  В **Выбор столбцов из**  *\<имя таблицы >* диалоговое окно, выберите столбец типа `geometry` или `geography` , установив соответствующий флажок. Остальные пространственные столбцы при этом станут недоступными для редактирования. Если необходимо выбрать другой пространственный столбец, то сначала нужно снять флажок со столбца, выбранного в данный момент. После завершения нажмите кнопку **ОК**.  
...
11. На странице **Пространственный** укажите значения, которые необходимо использовать для пространственных свойств индекса.  
     ...
     При создании индекса на `geometry` столбец типа, необходимо указать **(*`X-min`*,*`Y-min`*)** и **(*`X-max`*,*`Y-max`*)** координаты ограничивающего прямоугольника. При создании индекса для `geography` столбец типа поля ограничивающего прямоугольника становятся доступны только для чтения после указания **географическая сетка** схемы тесселяции, так как в тесселяции географической сетки ограничивающий прямоугольник не используется.  
...
2.  Выберите `geometry` или `geography` столбец для индекса.  
