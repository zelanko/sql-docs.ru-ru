...
11. На странице **Пространственный** укажите значения, которые необходимо использовать для пространственных свойств индекса.  
     ...
     При создании индекса на `geometry` столбец типа, необходимо указать **(*`X-min`*,*`Y-min`*)** и **(*`X-max`*,*`Y-max`*)** координаты ограничивающего прямоугольника. Для индекса на `geography` столбец типа ограничивающего поля становятся доступны только для чтения после того как вы укажете **географическая сетка** схемы тесселяции, так как в тесселяции географической сетки ограничивающий прямоугольник не используется.  
