...
-   **dc_proxy**  
 ...
 Пользователи, являющиеся членами `sysadmin` предопределенной роли сервера, имеют полный доступ к [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] объектам агента и представлениям сборщика данных. Однако для этого их нужно явно добавить в роли сборщика данных.  
...
### <a name="dc_admin-role"></a>Роль dc_admin  
 Пользователи, которым назначена `dc_admin` роль, имеют полный доступ администратора (создание, чтение, обновление и удаление) к конфигурации сборщика данных на экземпляре сервера. Члены этой роли могут выполнять следующие операции:  
...
-   выполнять все действия, разрешенные роли **dc_operator** .  
 ...
 `dc_admin`Роль является членом следующих ролей:  
  
-   **SQLAgentUserRole**. Эта роль необходима для создания расписаний и запуска заданий.  
  
    > [!NOTE]  
    >  Прокси-серверы, созданные для сборщика данных, должны предоставлять доступ к `dc_admin` для их создания и использовать в любых шагах задания, требующих прокси.  
  
-   **dc_operator**. Члены класса `dc_admin` наследуют разрешения, предоставленные **dc_operator**.  
  
### <a name="dc_operator-role"></a>Роль dc_operator  
...
### <a name="dc_proxy-role"></a>Роль dc_proxy  
...
-   **mdw_reader**  
 ...
 Пользователи, являющиеся членами `sysadmin` предопределенной роли сервера, имеют полный доступ к представлениям сборщика данных. Однако для этого их нужно явно добавить в роли базы данных для выполнения других операций.  
  
### <a name="mdw_admin-role"></a>Роль mdw_admin  
...
-   изменять при необходимости схему хранилища данных управления (например, добавлять новую таблицу при установке нового типа сбора);  
    ...
    > [!NOTE]  
    >  При изменении схемы пользователь также должен быть членом `dc_admin` роли для установки нового типа сборщика, так как для этого действия требуется разрешение на обновление конфигурации сборщика данных в msdb.  
  
-   запускать обслуживающие задания для хранилища данных управления, например архивацию или очистку.  
  
### <a name="mdw_writer-role"></a>Роль mdw_writer  
