...
-   **dc_proxy**  
 ...
 Пользователи, являющиеся членами `sysadmin` предопределенной роли сервера, имеют полный доступ [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] к объектам агента и представлениям сборщика данных. Однако для этого их нужно явно добавить в роли сборщика данных.  
...
-   **SQLAgentUserRole**. Эта роль необходима для создания расписаний и запуска заданий.  
    ...
    > [!NOTE]  
    >  Прокси-серверы, созданные для сборщика данных, должны предоставлять `dc_admin` доступ к для их создания и использовать в любых шагах задания, требующих прокси.  
