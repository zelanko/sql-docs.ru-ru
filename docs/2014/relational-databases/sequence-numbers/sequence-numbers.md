...
# <a name="sequence-numbers"></a>Порядковые номера
 ...
 Последовательность может быть определена с любым типом данных integer. Если тип данных не указан, по умолчанию используется последовательность `bigint`.  
