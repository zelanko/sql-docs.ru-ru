---
title: Параметры (SQL Server объекта страницы обозревателя сценариев) | Документы Microsoft
ms.custom: ''
ms.date: 03/07/2017
...
---
# <a name="options-sql-server-object-explorer-scripting-page"></a>Параметры (SQL Server объекта обозревателя сценариев страницы)
 ...
 **Изменить** и **изменить** команд может привести к результатам, которые отличаются от **сценарий \<объекта > как** для одного параметра. Команды **Правка** и **Изменить** предназначены для изменения объектов в текущей базе данных во время сеанса работы редактора запросов. **Сценарий \<объекта > как** команда предназначена для создания сценария, чтобы его можно будет использовать для создания объектов.  
 ...
 **Внести в скрипт разрешения**  
 Включает в скрипт разрешения для объектов базы данных. Значение по умолчанию — True. Дополнительные сведения см. в разделе [разрешений &#40;СУБД&#41;](../../relational-databases/security/permissions-database-engine.md).  
