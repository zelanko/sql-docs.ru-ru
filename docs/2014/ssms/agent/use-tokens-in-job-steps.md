...
|------------------|--------------------------------|---------------------------------|  
|Управляющие макросы используются|Все токены в заданиях заменяются успешно.|Токены, активированные предупреждениями, не заменяются. Эти токены, **A-DBN**, **A-SVR**, **A-ERR**, **A-SEV**, **A-MSG**, и **WMI () *`property`*)**. Другие статические токены заменяются успешно.|  
