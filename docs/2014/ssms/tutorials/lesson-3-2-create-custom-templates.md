...
11. Измените скрипт шаблона (скрипт из шага 7), заменив название продукта колонке параметра ***< * product_name**, `nvarchar(50)`, **имя*> ***, в четырех местах.  
...
4.  В **замена параметров шаблона** диалоговом для `product_name` введите **FreeWheel** (заменяя содержимое по умолчанию), а затем нажмите кнопку **ОК** для закрытия **Замена параметров шаблона** диалоговое окно и изменить скрипт в редакторе запросов.  
