...
ms.locfileid: "48099944"
---
# <a name="scale-out-deployment-native-mode-report-server"></a>Масштабное развертывание (сервер отчетов в собственном режиме)
  Используйте **конфигурации с масштабным развертыванием** странице в [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] Configuration Manager Чтобы просмотреть состояние инициализации для масштабного развертывания или включить сервер отчетов для масштабного развертывания. Термин *масштабное развертывание* означает два или более экземпляра сервера отчетов, совместно использующих общую базу данных сервера отчетов.  
...
-   Для сегментирования приложений сервера отчетов на различные компьютеры, где первый сервер используется для интерактивной обработки отчетов, а второй — для обработки отчетов по расписанию. В таком сценарии каждый экземпляр сервера обрабатывает разные типы запросов к одному и тому же содержимому сервера отчетов, хранящегося в общей базе данных.  
 ...
 Чтобы открыть эту страницу, запустите [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] Configuration Manager и выберите **конфигурации с масштабным развертыванием** в области навигации. Дополнительные сведения см. в разделе [Использование диспетчера конфигурации служб Reporting Services (собственный режим)](../../../2014/sql-server/install/reporting-services-configuration-manager-native-mode.md).  
  
## <a name="options"></a>Параметры  
 **Имя SQL Server**  
 Укажите имя [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] экземпляр, на котором размещена база данных сервера отчетов.  
