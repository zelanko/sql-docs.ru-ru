 ...
 **Данные tempdb и файлы журнала**  
 ...
 В более ранних [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]версиях база данных **tempdb** используется для хранения следующих объектов:  
...
|Кэширование метаданных временных таблиц и временных переменных|Для всех метаданных временной таблицы и переменной TEMP [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], кэшированной в кэше метаданных, для **базы данных tempdb**выделяется две дополнительные страницы.<br /><br /> Если хранимая процедура или триггер создает временную таблицу или временную переменную, то временный объект не удаляется, когда процедура или триггер завершает свое выполнение. Вместо этого временный объект усекается до одной страницы и используется заново в следующий раз, когда выполняется процедура или триггер.|  
