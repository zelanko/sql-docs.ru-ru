---
title: Параметры служб Reporting Services | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: database-engine
ms.topic: conceptual
helpviewer_keywords:
- Reporting Services, parameters
ms.assetid: e9a55dab-7c4e-4e09-97da-9f3a108e22c9
author: mashamsft
ms.author: mathoma
manager: craigg
ms.openlocfilehash: 4fe2f4f387dfbdf7a10ed34a5415f5f2b0d27353
ms.sourcegitcommit: 3026c22b7fba19059a769ea5f367c4f51efaf286
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/15/2019
ms.locfileid: "66092541"
---
# <a name="reporting-services-parameters"></a>Параметры служб Reporting Services
  Для анализа [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)], необходимо указать экземпляр [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] и предоставить сведения о том, как подключиться к этому экземпляру.  
  
## <a name="options"></a>Параметры  
 **Имя экземпляра служб отчетов**  
 Выберите из списка экземпляр служб [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)].  
  
## <a name="see-also"></a>См. также  
 [Работа с помощником по обновлению](../../../2014/sql-server/install/working-with-upgrade-advisor.md)   
 [Справочник по пользовательскому интерфейсу помощника по обновлению](../../../2014/sql-server/install/upgrade-advisor-user-interface-reference.md)  
  
  
