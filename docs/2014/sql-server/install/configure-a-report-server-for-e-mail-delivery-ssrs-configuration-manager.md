...
##  <a name="bkmk_setting_TO_field"></a>Параметры конфигурации для поля "Кому" в сообщении  
...
-   В качестве значения**SMTPServerPickupDirectory** указана папка на локальном жестком диске.  
    ...
    > [!NOTE]  
    >  Убедитесь, что не задан параметр `SMTPServer`, если используется локальный SMTP-сервер.  
...
6.  Убедитесь, что для параметра <`SendUsing`> задано значение 2. Если задано другое значение, сервер отчетов не настроен для использования удаленной службы SMTP.  
