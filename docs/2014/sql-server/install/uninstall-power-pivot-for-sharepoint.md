...
7.  Нажмите кнопку **Запуск** , чтобы выполнить все действия, которые являются допустимыми для данной задачи. Кнопка**Запуск** становится доступной только после прохождения проверки. При нажатии кнопки **запуска**, появится предупреждение о том, что действия обрабатываются в пакетном режиме: «Все параметры конфигурации, отмеченные как допустимый в средстве будут применены к ферме SharePoint. Продолжить?".  
