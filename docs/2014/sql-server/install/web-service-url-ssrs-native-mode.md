 ...
 **SSL-сертификат**  
 Связывает сертификат с указанным IP-адресом. Сертификат должен быть установлен на компьютер и настроен. В службах Reporting Services функции управления сертификатами не предусмотрены. Сертификат должен быть выдан для имени узла или имени компьютера, которые приводят к указанному IP-адресу. Например, чтобы использовать сертификат, выданный http://salesreports, указанный IP-адрес должен разрешаться на сервер с именем «салесрепортс».  
