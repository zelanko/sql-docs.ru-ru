...
## <a name="report-files"></a>Файлы отчетов  
 Мастер анализа помощника по обновлению создает отчеты в каталоге\\ [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] обновления "Мои документы" Advisor\110\Reports и создает подкаталог для каждого анализируемого сервера. Файлы отчетов — это XML-файлы, использующие определенное соглашение об именах. При запуске средства просмотра отчетов помощника по обновлению выводятся файлы отчетов, расположенные в каталоге по умолчанию. При копировании файлов отчетов в этот каталог необходимо убедиться, что для них выполняется соглашение об именах, иначе средство просмотра отчетов не сможет их автоматически отобразить.  
