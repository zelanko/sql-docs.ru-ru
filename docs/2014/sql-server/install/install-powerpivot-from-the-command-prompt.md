---
title: Установка PowerPivot из командной строки | Документы Microsoft
ms.custom: ''
ms.date: 03/07/2017
...
3.  Замените заполнитель \<domain\username > и \<StrongPassword > с действительными учетными записями пользователей и пароли.  
     ...
     **/Assysadminaccounts** параметр должен иметь значение с удостоверением пользователя, запустившего программу установки SQL Server. Для служб необходимо указать хотя бы одного системного администратора. Следует отметить, что программа установки SQL Server больше не предоставляет автоматически разрешения sysadmin членам встроенной группы «Администраторы».  
  
4.  Удалите разрывы строк.  
  
5.  Выделите всю команду и нажмите кнопку **копирования** в меню "Правка".  
  
6.  Откройте командную строку администратора. Чтобы сделать это, нажмите кнопку **запустить**, щелкните правой кнопкой мыши в командной строке, а затем выберите **Запуск от имени администратора**.  
  
7.  Перейдите в папку на диске или общую папку, которая содержит установочный носитель SQL Server.  
  
8.  Вставьте измененную команду в командную строку. Для этого щелкните значок в левом верхнем углу окна командной строки, укажите **изменить**, а затем нажмите кнопку **вставить**.  
