...
##  <a name="Geneva"></a> Шаг 2. Запуск служб на сервере  
 Для развертывания PowerPivot для SharePoint требуется, чтобы ферма включала следующие службы: Службы вычислений Excel, служба Secure Store и утверждений для службы маркеров Windows.  
...
4.  На странице **Создание нового приложения службы PowerPivot** введите имя приложения. Значение по умолчанию — PowerPivotServiceApplication @ no__t-0number >. При создании нескольких приложений службы PowerPivot описательное имя поможет другим администраторам понять, как используется это приложение.  
...
7.  В поле **имя базы данных**значение по умолчанию — PowerPivotServiceApplication1_ @ no__t-1guid >. Имя базы данных по умолчанию соответствует используемому по умолчанию имени приложения службы. Если было введено уникальное имя приложения службы, следуйте аналогичным соглашениях об именах при выборе имени базы данных, чтобы ими можно было управлять совместно.  
...
> [!IMPORTANT]  
>  Если служба безопасного хранилища включена, следует проверить, что для нее создан главный ключ. Инструкции см. в части 2. Создайте главный ключ в следующей процедуре.  
...
##### <a name="part-1-enable-secure-store-service"></a>Часть 1. Включить служба Secure Store  
...
6.  В поле **имя базы данных**введите имя базы данных приложения службы. Значение по умолчанию — Secure_Store_Service_DB_ @ no__t-0guid >. Имя, используемое по умолчанию, соответствует используемому по умолчанию имени приложения службы. Если было введено уникальное имя приложения службы, следуйте аналогичным соглашениях об именах при выборе имени базы данных, чтобы ими можно было управлять совместно.  
...
1.  Откройте созданное веб-приложение в браузере. Если используются значения по умолчанию, в URL-адресе можно указать http://\<your Computer name >.  
...
6.  В поле имя сервера введите **\<server-name > \PowerPivot**, где **\<server-name >** — это имя компьютера, на котором установлена установка PowerPivot для SharePoint.  
