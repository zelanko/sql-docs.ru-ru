...
|Значение `Databases` для элемента `SQLServer`|Содержит список анализируемых баз данных.|Один раз для каждого `SQLServer` элемент. Если этот элемент отсутствует, выполняется анализ всех баз данных на экземпляре.|  
...
|`TraceFiles`|Содержит список анализируемых файлов трассировки.|Один раз для каждого `SQLServer` элемент.|  
