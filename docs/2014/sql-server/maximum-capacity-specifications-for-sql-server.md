...
|Столбцов на ключ индекса<br /><br /> Примечание: Если таблица содержит один или несколько индексов XML, ключ кластеризации пользовательской таблицы ограничивается 15 столбцами, так как XML-столбец добавляется к ключу кластеризации первичного XML-индекса. В [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], можно включать неключевые столбцы в некластеризованный индекс, чтобы избежать ограничения максимального количества, равного 16 ключевых столбцов. Дополнительные сведения см. в статье [Создание индексов с включенными столбцами](../relational-databases/indexes/create-indexes-with-included-columns.md).|16|16|  
...
|Ссылок на таблицы внешнего ключа для таблицы<br /><br /> Обратите внимание: Несмотря на то, что таблица может содержать неограниченное количество ограничений FOREIGN KEY, рекомендуемый максимум равен 253. В зависимости от конфигурации оборудования, на котором установлен [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], определение дополнительных ограничений FOREIGN KEY может потребовать слишком много ресурсов для обработки с точки зрения оптимизатора запросов.|253|253|  
|Длина идентификатора (в символах)|128|128|  
|Экземпляров на один компьютер|50 экземпляров на изолированном сервере для всех выпусков [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)].<br /><br /> [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] поддерживает 25 экземпляров в отказоустойчивом кластере при использовании общего диска кластера в качестве места хранения для кластерной установки [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] общих файловых ресурсов поддерживает 50 экземпляров в отказоустойчивом кластере, при выборе SMB в качестве места хранения для кластерной установки. Дополнительные сведения см. в разделе [оборудованию и программному обеспечению для установки SQL Server 2014](install/hardware-and-software-requirements-for-installing-sql-server.md).|50 экземпляров на отдельном сервере.<br /><br /> 25 экземпляров в отказоустойчивом кластере при использовании общего диска кластера в качестве места хранения для кластерной установки. [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] поддерживает 50 экземпляров в отказоустойчивом кластере, если выбрать общие папки SMB в качестве места хранения для кластерной установки.|  
|Индексы на оптимизированную для памяти таблицу|Не поддерживается|8|  
|Длина строки, содержащей инструкции SQL (размер пакета)<br /><br /> Примечание: Размер сетевого пакета — размер пакетов потока табличных данных, используемый для обмена данными между приложениями и компонентом [!INCLUDE[ssDE](../includes/ssde-md.md)]. По умолчанию размер пакета равен 4 КБ, а его настройка осуществляется с помощью параметра конфигурации network packet size.|65 536 * размер сетевого пакета|65 536 * размер сетевого пакета|  
|Блокировок на соединение|Максимальное число блокировок на сервер|Максимальное число блокировок на сервер|  
|Блокировок на экземпляр [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]<br /><br /> Примечание: Это значение является к статическим блокировкам. Количество динамических блокировок ограничивается только объемом памяти.|до 2 147 483 647|Ограничено только объемом памяти|  
|Уровней вложенных хранимых процедур<br /><br /> Примечание: Если хранимая процедура обращается больше чем к 64 базам данных или более чем к двум базам в режиме чередования, вы получите ошибку.|32|32|  
...
|Таблиц на одну базу данных<br /><br /> Примечание: Объекты базы данных включают объекты, такие как таблицы, представления, хранимые процедуры, определяемые пользователем функции, триггеры, правила, значения по умолчанию и ограничения. Суммарное число всех объектов в базе данных не может превышать 2 147 483 647.|Ограничено числом объектов в базе данных|Ограничено числом объектов в базе данных|  
...
|Триггеров на таблицу<br /><br /> Примечание: Объекты базы данных включают объекты, такие как таблицы, представления, хранимые процедуры, определяемые пользователем функции, триггеры, правила, значения по умолчанию и ограничения. Суммарное число всех объектов в базе данных не может превышать 2 147 483 647.|Ограничено числом объектов в базе данных|Ограничено числом объектов в базе данных|  
