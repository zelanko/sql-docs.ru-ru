...
##  <a name="Top"></a>В этом пошаговом окне  
 ...
 [Рекомендации по проектированию кластеризованных индексов](#Clustered)  
  
 [Рекомендации по проектированию некластеризованных индексов](#Nonclustered)  
  
 [Рекомендации по проектированию уникальных индексов](#Unique)  
  
 [Рекомендации по проектированию фильтруемых индексов](#Filtered)  
...
-   Большое количество индексов в таблице влияет на производительность операторов INSERT, UPDATE, DELETE и MERGE, так как при изменении данных в таблице все индексы необходимо будет соответствующим образом настраивать. Например, если столбец используется в нескольких индексах и выполняется инструкция UPDATE, которая изменяет данные из этого столбца, каждый индекс, содержащий этот столбец, должен быть обновлен, как и столбец в базовой таблице (куча или кластеризованный индекс).  
...
-   Создавайте запросы, которые вставляют или изменяют максимально возможное число строк в одном операторе, вместо того, чтобы использовать несколько запросов для обновления тех же строк. Используя только одну инструкцию, можно воспользоваться возможностями, которые обеспечивает поддержание оптимизированного индекса.  
...
###  <a name="Sort_Order"></a>Рекомендации по проектированию порядка сортировки индексов  
...
```  
 ...
 ![План выполнения показывает, что оператор SORT не использовался](media/insertsort2.gif "План выполнения показывает, что оператор SORT не использовался")  
...
##  <a name="Clustered"></a>Рекомендации по проектированию кластеризованных индексов  
...
-   Использовать предложения ORDER BY или GROUP BY.  
     ...
     Индекс по столбцам, указанным в предложении ORDER BY или GROUP BY, может исключить потребность в сортировке данных для компонента [!INCLUDE[ssDE](../includes/ssde-md.md)] , потому что строки будут уже отсортированы. Это повышает производительность запросов.  
...
##  <a name="Nonclustered"></a>Рекомендации по проектированию некластеризованных индексов  
...
####  <a name="Included_Columns"></a>Использование включаемых столбцов для расширения некластеризованных индексов  
...
##  <a name="Unique"></a>Рекомендации по проектированию уникальных индексов  
...
##  <a name="Filtered"></a>Рекомендации по проектированию фильтруемых индексов  
...
||  
|-|  
|**Применимо к** [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] : [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]с до.|  
  
 Отфильтрованные индексы могут предоставить следующие преимущества по сравнению с индексами, построенными на всей таблице.  
  
-   **Улучшенная производительность запросов и качество плана**  
...
-   **Снижение затрат на обслуживание индекса**  
...
-   **Снижение затрат на хранение индексов**  
