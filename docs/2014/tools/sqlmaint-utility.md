...
# <a name="sqlmaint-utility"></a>sqlmaint, программа
  Программа**sqlmaint** выполняет указанный набор операций обслуживания для одной или нескольких баз данных. Программа **sqlmaint** используется для выполнения проверок DBCC, создания резервных копий базы данных и ее журнала транзакций, обновления статистики и перестроения индексов. При всех действиях по обслуживанию базы данных формируется отчет, который можно записать в указанный текстовый файл, в HTML-файл или отправить по электронной почте. **sqlmaint** выполняет планы обслуживания базы данных, созданные в предыдущих версиях [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]. Для запуска планов обслуживания [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] из командной строки используйте программу [dtexec utility](../integration-services/packages/dtexec-utility.md).  
  
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepNextAvoid](../includes/ssnotedepnextavoid-md.md)]Используйте [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] вместо этого функцию плана обслуживания. Дополнительные сведения о планах обслуживания см. в разделе [Планы обслуживания](../relational-databases/maintenance-plans/maintenance-plans.md).  
 ...
 **-Хтмлрпт** _html_file_  
 Указывает полный путь и имя файла, в котором будет сформирован HTML-отчет. Программа **sqlmaint** создает имя файла, добавляя строку формата _*ГГГГММДДЧЧмм* к имени файла, точно так же, как это делается для параметра **-RPT** .  
 ...
 **-Supportcomputedcolumn** _free_space_  
 ...
 **-BkUpDB** [ *backup_path*] |  **-BkUpLog** [ *backup_path* ]  
 Задает действие резервного копирования. **-BkUpDB** создает резервную копию всей базы данных. **-BkUpLog** создает резервную копию только журнала транзакций.  
  
 *backup_path* указывает каталог для резервной копии. *backup_path* не требуется, если также указан параметр **-UseDefDir** и переопределяется функцией **-UseDefDir** , если указаны оба значения. Резервная копия может размещаться в каталоге или по адресу ленточного устройства (например, \\\\.\TAPE0). Имя файла резервной копии базы данных создается автоматически следующим образом:  
...
-   *ГГГГММДДЧЧмм* — это время операции резервного копирования с *гггг* = year, *мм* = месяц, *дд* = день, *чч* = час и *mm* = Minute.  
 ...
 **-CrBkSubDir**  
 При резервном копировании дисков, если указан параметр *-UseDefDir*, создает вложенный каталог в каталоге [ **backup_path** ] или в каталоге резервных копий по умолчанию. Имя вложенного каталога создается на основе имени базы данных, указанной в параметре **-D**. **-Крбксубдир** предлагает простой способ размещения всех резервных копий для разных баз данных в отдельных подкаталогах без необходимости изменения параметра *backup_path* .  
  
 **-UseDefDir**  
 Для резервного копирования на диск задает создание файла резервной копии в каталоге резервных копий по умолчанию. **UseDefDir** переопределяет *backup_path* , если указаны оба значения. При установке по [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] умолчанию каталог резервного копирования по умолчанию — C:\Program Files\Microsoft SQL Server \ MSSQL10_50. мссклсервер\мсскл\баккуп.  
 ...
 *число*[**минуты**| | ,******дни**| ******** недели]|   
 Задает интервал времени, используемый для определения, является ли отчет или файл резервной копии достаточно старым для его удаления. *число* — это целое число, за которым следует единица времени (без пробела). Допустимые варианты:  
