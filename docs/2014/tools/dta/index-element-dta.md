...
|**Однократно**|Требуется один раз для каждого элемента `Create` или `Drop`, если не заданы никакие другие структуры физического проектирования с помощью элементов `Statistics` или `Heap`.|  
