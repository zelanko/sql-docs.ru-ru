...
ms.locfileid: "66110177"
---
# <a name="viewing-tuning-recommendations"></a>Просмотр рекомендаций по настройке
  Эта задача использует сеанс настройки, созданный при [настройке рабочей нагрузки](lesson-1-1-tuning-a-workload.md). После [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] настройки базы данных с помощью скрипта MyScript. SQL [!INCLUDE[tsql](../../includes/tsql-md.md)] помощник по настройке компонента выводит результаты на вкладке **рекомендации** . Следующая задача познакомит вас с вкладкой **рекомендации** графического [!INCLUDE[ssDE](../../includes/ssde-md.md)] интерфейса пользователя (GUI) помощника по настройке и поможет исследовать сведения о результатах сеанса настройки.  
...
2.  Дважды щелкните сеанс **MySession** на панели **Монитор сеансов** . [!INCLUDE[ssDE](../../includes/ssde-md.md)]Помощник по настройке компонента загружает сведения о сеансе из предыдущего сеанса настройки и отображает вкладку **рекомендации** . [!INCLUDE[ssDE](../../includes/ssde-md.md)] Обратите внимание, что помощник по настройке не выполнил **рекомендации по секциям** , так как вы приняли все параметры настройки по умолчанию и на вкладке **Параметры настройки** не выбрано **секционирование** .  
  
3.  На вкладке **Рекомендации** с помощью полосы прокрутки внизу страницы с вкладками просмотрите все столбцы в области **Рекомендации по индексам** . Каждая строка отображает объект базы данных (индекс или индексированное представление), который помощник по настройке компонента [!INCLUDE[ssDE](../../includes/ssde-md.md)] рекомендует удалить или создать. Прокрутите содержимое до крайнего правого столбца и нажмите кнопку **Определение**. [!INCLUDE[ssDE](../../includes/ssde-md.md)]Помощник по настройке компонента отображает окно **предварительного просмотра скрипта SQL** , в [!INCLUDE[tsql](../../includes/tsql-md.md)] котором можно просмотреть скрипт, который создает или удаляет объект базы данных в этой строке. Нажмите кнопку **Закрыть** , чтобы закрыть окно предварительного просмотра.  
...
7.  В меню **Действия** выберите команду **Оценить рекомендации**. 
  [!INCLUDE[ssDE](../../includes/ssde-md.md)] создаст новый сеанс настройки, в котором можно провести оценку набора исходных рекомендаций из сеанса настройки MySession.  
