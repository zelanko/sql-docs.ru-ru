...
ms.locfileid: "63149992"
---
# <a name="prepare-the-input-trace-data"></a>Подготовка входных данных трассировки
  Прежде чем можно будет запустить распределенное воспроизведение с [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] помощью функции распределенное воспроизведение, необходимо подготовить входные данные трассировки, запустив этап предварительной обработки из средства администрирования распределенного воспроизведения. На стадии предварительной обработки контроллер распределенного воспроизведения обрабатывает данные трассировки и формирует промежуточный файл:  
...
1.  **(Необязательно.) измените параметры конфигурации предварительной обработки**. Если вы хотите изменить параметры конфигурации предварительной обработки, например, следует ли фильтровать системные сеансы или настроить максимальное время простоя, необходимо изменить `<PreprocessModifiers>` элемент файла конфигурации предварительной обработки на основе XML, `DReplay.exe.preprocess.config`. При изменении файла конфигурации предварительной подготовки рекомендуется изменять его копию, а не оригинал. Чтобы изменить параметры, выполните следующие действия.  
...
2.  **Инициация этапа**предварительной обработки: для подготовки входных данных трассировки необходимо запустить средство администрирования с параметром **Предварительная обработка** . Дополнительные сведения см. в разделе [Параметр предварительной обработки (средство администрирования распределенного воспроизведения)](preprocess-option-distributed-replay-administration-tool.md).  
