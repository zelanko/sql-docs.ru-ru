 ...
 *database_name*  
 ...
 Можно также явно указать имя базы данных с помощью `d-`.  
  
 **в** *data_file* | **out *** data_file* | **queryout *** data_file* | **форматирования nul**  
 ...
 **-F** *first_row*  
 ...
 Параметр*first_row* может иметь положительное целое значение до 2^63-1. **-F *** first_row* отсчитывается от 1.  
 ...
 **-x**  
 Используется с **формат** и **-f *** format_file* параметров, создает файл формата XML не XML-файл форматирования в формате по умолчанию. Параметр **-x** не работает при импорте или экспорте данных. Без параметров, создается ошибка **формат** и **-f *** format_file*.  
...
## <a name="computed-columns-and-timestamp-columns"></a>Вычисляемые столбцы и столбцы типа timestamp  
 ...
 Вычисляемые и `timestamp` столбцы выполняются операции массового копирования из [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] в файл данных обычным образом.  
...
-   Собственные представления `float` или `real` типы данных являются допустимыми.  
