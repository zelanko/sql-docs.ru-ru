...
# <a name="profiler-utility"></a>Приложение SQL Profiler
  Служебная программа **Profiler** запускает [!INCLUDE[ssSqlProfiler](../includes/sssqlprofiler-md.md)] средство. Дополнительные аргументы, перечисленные ниже в этом разделе, позволяют управлять способом запуска приложения.  
