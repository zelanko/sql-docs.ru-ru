 ...
 **osql** отображает `money` и `smallmoney` типы данных с двумя десятичными разрядами, хотя [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] сохраняет значение изнутри с четыре десятичных разряда. Рассмотрим пример:  
