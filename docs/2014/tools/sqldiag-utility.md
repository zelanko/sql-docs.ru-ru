...
ms.locfileid: "63035423"
---
# <a name="sqldiag-utility"></a>SQLdiag, программа
  Программа **SQLdiag** — это служебная программа сбора диагностических сведений общего назначения, которая может запускаться как консольное приложение или как служба. Программа **SQLdiag** позволяет выполнять сбор журналов и файлов данных с [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] и других типов серверов, а также мониторинг серверов и устранение определенных неполадок на серверах. Программа **SQLdiag** предназначена для ускорения и упрощения сбора диагностических [!INCLUDE[msCoName](../includes/msconame-md.md)] сведений для службы поддержки пользователей.  
...
>  Вследствие изменений в программе приложения или скрипты, зависящие от аргументов ее командной строки или особенностей работы, в будущих выпусках могут работать неправильно.  
 ...
 Программа **SQLdiag** может выполнять сбор данных диагностики следующих типов:  
  
-   журналы производительности Windows;  
  
-   Журналы событий Windows  
  
-   [!INCLUDE[ssSqlProfiler](../includes/sssqlprofiler-md.md)]отладоч  
...
> [!NOTE]  
>  Программа **SQLdiag** не добавляет выходные данные в текущую папку выходных данных при запуске. Она может только перезаписать выходную папку по умолчанию (параметр 1) или переименовать папку (параметр 2), после чего производит вывод в новую выходную папку по умолчанию, имеющую имя SQLDIAG.  
 ...
 **/B** [**+**]*start_time*  
 ...
 Используйте **+** без даты (только чч: мм: СС), чтобы указать время относительно текущей даты и времени. Например, если указать **/B +02:00:00**, то программа **SQLdiag** будет ждать 2 часа перед тем, как начать сбор сведений.  
  
 Не вставляйте пробел между **+** и заданными *start_time*.  
 ...
 *SQLdiag_application_name* можно использовать для запуска или завершения работы определенного экземпляра службы **SQLdiag** .  
  
 Пример:  
  
 Программа **SQLdiag Start/a** _SQLdiag_application_name_  
 ...
 **/Q**  
 Запускает **SQLdiag** в автоматическом режиме. **/Q** подавляет все запросы, например запросы на ввод пароля.  
 ...
 **/L**  
 Запускает программу **SQLdiag** в непрерывном режиме, когда, помимо прочего, указано время запуска или время остановки с помощью аргумента **/B** или **/E** , соответственно. Программа **SQLdiag** автоматически перезапускается после того, как сбор диагностических сведений прекращается из-за запланированного отключения. Например, при использовании аргументов **/E** или **/X** .  
  
> [!NOTE]  
>  Программа **SQLdiag** игнорирует аргумент **/l** , если время начала или окончания не указано с помощью аргументов командной строки **/b** и **/e** .  
 ...
 **/X**  
 Запускает **SQLdiag** в режиме моментального снимка. Программа **SQLdiag** создает моментальный снимок всех настроенных диагностических данных, а затем автоматически завершает работу.  
  
 **Запуск** | **** | **STOP_ABORT** отмены  
 Запускает или останавливает службу **SQLdiag** . **STOP_ABORT** заставляет службу завершать работу как можно быстрее без завершения сбора данных диагностики, собираемых в данный момент.  
...
## <a name="data-collection-process"></a>Процесс сбора данных  
 ...
 Программа **SQLdiag** собирает большинство диагностических данных в параллельном режиме. Все диагностические сведения собираются с помощью соединения со средствами, такими как программа [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] **sqlcmd** или интерпретатор командной строки Windows, кроме тех случаев, когда сведения собираются из журналов производительности Windows и журналов событий. **SQLdiag** использует по одному рабочему потоку на компьютер для наблюдения за сбором диагностических данных этих других средств, часто одновременно ожидая выполнения нескольких инструментов. В процессе сбора данных программа **SQLdiag** направляет выходной поток каждого средства диагностики в выходную папку.  
 ...
 **Завершение работы программы SQLdiag в качестве службы**  
...
## <a name="automatically-starting-and-stopping-sqldiag"></a>Автоматический запуск и остановка SQLdiag  
 Чтобы автоматически запускать и прекращать сбор диагностических данных в указанное время, используйте аргументы **/b**_start_time_ и **/e**_stop_time_ с использованием 24-часовой нотации. Например, если нужно устранить неполадку, которая регулярно происходит приблизительно в 02:00:00, можно настроить **SQLdiag** таким образом, чтобы она автоматически начинала собирать диагностические данные в 01:00:00 и останавливалась в 03:00:00. Укажите аргументы **/B** и **/E** , чтобы указать время запуска и остановки. Используйте 24-часовой формат времени, чтобы указать точную дату и время запуска и остановки в формате ГГГГММДД_ЧЧ:ММ:СС. Чтобы указать время относительных запусков или останова, задайте для префикса **+** время запуска и остановки с параметром и опустите часть даты (YYYYMMDD_), как показано в следующем примере, в результате чего программа **SQLdiag** ждет 1 час перед началом сбора информации, а затем собирает сведения в течение 3 часов перед остановкой и завершением работы.  
 ...
 **Регистрация программы SQLDiag для запуска в качестве службы**  
 ...
 Чтобы изменить имя службы по умолчанию SQLDIAG, используйте параметр командной строки **/A** и укажите другое имя. Программа **SQLdiag** автоматически добавляет префикс DIAG $ в любое имя экземпляра **SQLdiag** , указанное в параметре **/a** , чтобы создать разумные имена служб.  
