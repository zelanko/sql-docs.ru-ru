...
# <a name="report-server-properties-service-tab"></a>Свойства сервера отчетов (вкладка «Службы»)
  Эта служба является [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] службой сервера отчетов. Значения свойств, выделенные светло-серым цветом, с помощью этого приложения изменить нельзя.  
