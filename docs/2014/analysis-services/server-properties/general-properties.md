...
## <a name="non-specific-category"></a>Общая категория  
 ...
 `BackupDir`  
 Строковое свойство, определяющее имя каталога, в котором хранятся файлы резервных копий по умолчанию, в случае, если путь не указан как часть команды Backup.  
...
>  `ForceCommitTimeout` применяется к командам обработки куба и операциям обратной записи.  
 ...
 Целочисленное свойство, определяющее максимальное время ожидания в секундах для бездействующего сеанса. Значение по умолчанию равно нулю (0), что означает, что сеансы никогда не превышены. Однако бездействующие сеансы по-прежнему будут удалены, если сервер находится под ограничениями ресурсов.  
 ...
 Значение этого свойства по умолчанию равно нулю (0), то есть по умолчанию используется порт 2383. Дополнительные сведения о настройке порта см. в разделе [Configure the Windows Firewall to Allow Analysis Services Access](../instances/configure-the-windows-firewall-to-allow-analysis-services-access.md).  
