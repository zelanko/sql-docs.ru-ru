---
title: Соединение семантической модели бизнес-Аналитики PowerPivot (bism) | Документы Microsoft
ms.custom: ''
ms.date: 04/19/2015
...
ms.locfileid: "36195475"
---
# <a name="powerpivot-bi-semantic-model-connection-bism"></a>Соединение семантической модели бизнес-аналитики PowerPivot (BISM)
  Соединение семантической модели бизнес-Аналитики (bism-файл) — это переносимое соединение, который подключается Excel или [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)] отчеты в [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] базы данных табличной модели или [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] экземпляра в многомерном режиме. Определение и использование BISM-файлов в определенной степени аналогично файлам подключения к данным Оffice (ODC).  
  
 Создание семантической модели бизнес-аналитики и доступ к ней через SharePoint. Создание соединения семантической модели бизнес-аналитики позволяет использовать команды быстрого запуска соединения семантической модели с библиотекой. Команды быстрого запуска открывают новую книгу Excel или параметры файла соединения. Если [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] будет установлен, вы также увидите команду, чтобы создать [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)] отчета.  
...
-   Книги PowerPivot, сохраненные в SharePoint. Базы данных PowerPivot, внедренные в книги Excel, эквивалентны табличным шаблонам баз данных, запущенным на отдельном сервере Analysis Services в табличном режиме. Если уже используется PowerPivot для Excel и PowerPivot для SharePoint, можно определить соединение семантической модели бизнес-Аналитики, указывающее на книги PowerPivot из библиотеки SharePoint и создавать [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)] отчетов с помощью существующих данных PowerPivot.  Можно использовать книги, созданные в версиях PowerPivot для Excel в SQL Server 2008 R2 или [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)].  
 ...
 **Соединение с табличными данными из Excel по сети**  
 ...
 ![Соединения из Excel с базой данных табличной модели](../media/ssas-powerpivotbismconnection-1.gif "соединения из Excel с базой данных табличной модели")  
 ...
 **Соединение с табличными данными из Power View по сети**  
 ...
 Если протокол Kerberos не настроен и первый запрос соединения не выполняется, то службы Reporting Services создают второй запрос. В этом случае клиентская библиотека подключается к службам Analysis Services с использованием удостоверения службы Reporting Services и проверки подлинности NTLM. Удостоверение пользователя Power View передается в строку соединения с помощью `effectiveusername` параметра.  
 ...
 На следующем рисунке показана последовательность, в которой для каждого соединения используется одно и то же удостоверение пользователя Windows. Последнее соединение со службами Analysis Services, соединение устанавливается с удостоверение приложения службы Reporting Services, передача удостоверения пользователя Windows с помощью `effectiveusername`.  
