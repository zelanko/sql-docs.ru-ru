...
5.  Укажите один или несколько URL-адресов, по которым передается канал.  
    ...
    2.  **URL-адрес веб-службы** является обязательным. В отсутствие базового URL-адреса это значение должно содержать http:// или https://. Если указан базовый URL-адрес, то URL-адрес веб-службы представляет часть URL-адреса, следующую за базовым адресом. Например, если полный URL-адрес имеет http://adventure-works/inventory/today.aspxзначение, то базовый URL- http://adventure-works/inventoryадрес будет иметь значение, а URL веб службы будет/Тодай.аспкс..  
