...
##  <a name="prereq"></a> Предварительные требования  
 ...
 Необходима веб-служба или служба данных, которые поставляют данные в формате Atom 1.0. [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)] И SharePoint 2010 могут предоставлять данные в этом [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] формате.  
...
##  <a name="rsreport"></a>Создание веб-канала данных из Reporting Services отчета  
...
##  <a name="dsdoc"></a>Создание веб-канала данных из сервисного документа данных  
...
5.  Укажите один или несколько URL-адресов, по которым передается канал.  
    ...
    2.  Требуется **URL-адрес Web Service** . В отсутствие базового URL-адреса это значение должно содержать http:// или https://. Если указан базовый URL-адрес, то URL-адрес веб-службы представляет часть URL-адреса, следующую за базовым адресом. Например, если полный URL-адрес имеет http://adventure-works/inventory/today.aspxзначение, то базовый URL- http://adventure-works/inventoryадрес будет иметь значение, а URL веб службы будет/Тодай.аспкс..  
