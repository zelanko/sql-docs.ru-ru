...
## <a name="how-to-check-disk-space-usage"></a>Как проверить использование места на диске  
 ...
 В кэше содержатся только данные PowerPivot. Базы данных PowerPivot хранятся в нескольких файлах, находящихся в одной родительской папке Backup. Базы данных PowerPivot предназначены для использования в книге Excel в качестве внутренних данных, поэтому имеют имена на основе идентификаторов GUID, а не описательные имена. Папка GUID в разделе ** \<сервицеаппликатионнаме>** — это родительская папка базы данных PowerPivot. По мере загрузки баз данных PowerPivot на сервер для каждой из них создаются дополнительные папки.  
...
4.  В разделе «Кэш на диске» можно указать время хранения неактивной базы данных в памяти для обслуживания новых запросов (по умолчанию 48 часов), а также время ее хранения в кэше (по умолчанию 120 часов).  
     ...
     **Сохранение неактивной базы данных в памяти** указывает время, в течение которого неактивная база данных остается в памяти для обслуживания новых запросов к данным. Активная база данных всегда остается в памяти, пока к ней поступают запросы, но после того, как она перестает быть активной, система будет хранить эту базу данных в памяти в течение определенного периода времени на случай поступления новых запросов к этим данным.  
     ...
     **Сохранение неактивной базы данных в кэше** указывает время, в течение которого неактивная база данных остается в файловой системе после ее выгрузки из памяти. С помощью этого параметра задание очистки определяет, какие файлы подлежат удалению. Все базы данных PowerPivot, остающиеся неактивными в течение 168 часов (48 часов в памяти и 120 часов в кэше), удаляются с диска заданием очистки.  
