...
5.  В поле «Самое раннее время запуска» выберите одно из следующих значений.  
    ...
    1.  **После того как рабочие часы** задают период обработки в нерабочее время, когда серверы баз данных более вероятно будут иметь актуальные данные, созданные в течение рабочего дня.  
  
    2.  **Указанное самое раннее время запуска** — это час и минута самого раннего времени суток, когда запрос на обновление данных добавляется в очередь процесса. Время задается с интервалами в 15 минут. Этот параметр относится к текущему дню, а также к будущим датам. Например, если указано значение 6:30, а текущее время — 16:30, то запрос на обновление будет добавлен к очереди в текущий день, поскольку время 16:30 следует за 6:30.  
