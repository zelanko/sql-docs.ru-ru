---
title: Просмотр модели упрощенного алгоритма Байеса | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
##  <a name="BKMK_Tabs"></a> Просмотр модели  
 ...
 При желании поэкспериментировать с помощью средства просмотра упрощенного алгоритма Байеса, использовать [мастера классификации &#40;интеллектуального анализа данных надстройки для Excel&#41; ](classify-wizard-data-mining-add-ins-for-excel.md) мастера на ленте «Интеллектуальный анализ данных», нажмите кнопку **Дополнительно** и измените алгоритм, используемый упрощенный алгоритм Байеса  
  
 В этих примерах мы использовали исходные данные в образце книги и сгруппировали столбец **Yearly Income**, в пяти группах с доход от **очень низкий** для **очень высокая**. Модель упрощенного алгоритма Байеса затем выполнила анализ факторов, связанных с каждой категорией дохода.  
  
###  <a name="bkmk_DepNet"></a> Сеть зависимостей  
 Первое окно, вы воспользуетесь находится **сеть зависимостей**. В нем наглядно отображается, какие входные данные близко связаны с выбранным результатом.  
...
1.  Сначала щелкните целевой результат, **Yearly Income**, который представлен как узел в графе.  
...
3.  После фильтрования диаграммы нажмите кнопку **Копировать представление графа**. Выберите лист Excel и нажмите клавиши Ctrl+V.  
 ...
 **Профили атрибутов** windows предоставляет это визуальное представление о том, как любые другие переменные связаны с конкретными результатами.  
 ...
 **Характеристики атрибута** полезны для глубокого анализа определенной переменной результата и влияющих факторов.  
...
2.  Нажмите кнопку **Копировать в Excel** для создания таблицы, на новом листе, содержащую списки оценок для всех атрибутов, которые связаны с выбранным целевым результатом.  
  
 [К началу](#BKMK_Tabs)  
  
###  <a name="bkmk_AttDisc"></a> Сравнение атрибутов  
 **Сравнение атрибутов** позволяет сравнивать 2 результата или один результат и все другие результаты.  
...
1.  С помощью элементов управления, **значение 1** и **значение 2**, чтобы выделить результаты, которые нужно сравнить.  
     ...
     Например, в этой модели отсутствуют некоторые полезные атрибуты в группе «низкий доход», мы выбрали самую низкую категорию дохода в первом раскрывающемся списке при выборе **все прочие Штаты** во втором раскрывающемся списке.  
