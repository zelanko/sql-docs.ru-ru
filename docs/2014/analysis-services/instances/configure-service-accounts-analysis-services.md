...
-   Выполнение compmgmt. msc | **Локальные пользователи и группы**  |  **Groups**  |  `SQLServerMSASUser$` Группы \<server-name> `$MSSQLSERVER` (для экземпляра по умолчанию).  
...
2.  Используйте **Computer Manager**  |  **локальные группы пользователей и групп**диспетчера компьютеров  |  **Groups** для проверки членства в \<servername> $ \<instancename> группе безопасности SQLServerMSASUser $.  
  
     Элемент идентификатора безопасности службы должен совпадать с идентификатором безопасности по службе из пункта 1.  
  
3.  Использование **Windows Explorer**  |  **программных файлов**проводника Windows  |  **Microsoft SQL Server** | MSASxx. MSSQLServer | **OLAP**  |  **bin** для проверки того, что свойства безопасности папки предоставлены группе безопасности на шаге 2.  
