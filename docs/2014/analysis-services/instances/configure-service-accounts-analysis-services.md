...
-   Выполнение compmgmt. msc | `SQLServerMSASUser$`**Группы** |  `$MSSQLSERVER` **Local Users and Groups** | "Локальные пользователи и группы" — имя сервера>(для экземпляра по умолчанию).\<  
...
##  <a name="windows-privileges-assigned-to-the-analysis-services-service-account"></a><a name="bkmk_winpriv"></a>Привилегии Windows, назначенные учетной записи службы Analysis Services  
...
##  <a name="file-system-permissions-assigned-to-the-analysis-services-service-account"></a><a name="bkmk_FilePermissions"></a> Разрешения файловой системы, назначенные учетной записи служб Analysis Services  
...
3.  Использование | **программных файлов** |  **проводника Windows****Microsoft SQL Server** | MSASxx. MSSQLServer | **OLAP** | **Ячейка** OLAP для проверки свойств безопасности папки предоставляется группе безопасности на шаге 2.  
...
##  <a name="granting-additional-analysis-services-permissions-for-specific-server-operations"></a><a name="bkmk_tasks"></a>Предоставление дополнительных разрешений Analysis Services для конкретных операций сервера  
