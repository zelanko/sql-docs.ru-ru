...
##  <a name="bkmk_usingTE"></a> Открытие шаблона служб Analysis Services  
 ...
 ![Обозреватель шаблонов отфильтрованных для служб Analysis Services](../media/ssas-templateexplorer.gif "обозреватель шаблонов с фильтрацией для служб Analysis Services")  
...
3.  В шаблоне замените [элемент RequestType &#40;XMLA&#41; ](../xmla/xml-elements-properties/type-element-xmla.md) элемент со следующим текстом: `<RequestType>MDSCHEMA_INPUT_DATASOURCES</RequestType>`  
...
||Значения непрерывного столбца|Демонстрирует использование инструкции SELECT DISTINCT FROM  *\<модели >* инструкции с DMX `RangeMin` и `RangeMax` функции для получения набора значений в указанном диапазоне из непрерывных столбцов в указанную модель интеллектуального анализа.|  
...
|Многомерные выражения\Выражения|Скользящее среднее-фиксированное|Демонстрирует использование многомерных Выражений `ParallelPeriod` и `CurrentMember` функций с естественно упорядоченным набором для создания вычисляемой меры, дающая скользящее среднее меры за фиксированное число периодов времени, содержащихся в иерархии в измерении времени.|  
||Скользящее среднее-переменное|Демонстрирует использование многомерных Выражений `CASE` инструкции в пределах `Avg` функцию для создания вычисляемой меры, дающая скользящее среднее меры переменного числа временных периодов, содержащихся в иерархии в измерении времени.|  
...
||Запрос ключевых показателей эффективности|Демонстрирует использование многомерных Выражений `KPIValue` и `KPIGoal` функции для получения сведений о ключевых индикаторов производительности в запросе многомерных Выражений.|  
...
|XML для аналитики или Управление|Резервное копирование|Демонстрирует использование XML для Аналитики `Backup` команду, чтобы создать резервную копию [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] файл базы данных.|  
||Отмена|Демонстрирует использование XML для Аналитики `Cancel` команду для отмены всех выполняемых операций в текущем сеансе (для пользователей, не являющихся администраторами или администраторами сервера), базы данных (для администраторов) или экземпляре (для администраторов сервера).|  
...
||DELETE|Демонстрирует использование XML для Аналитики `Delete` команду, чтобы удалить существующее [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] базы данных.|  
...
||Обработка секции|Демонстрирует использование XML для Аналитики `Batch` команды в сочетании с `Parallel` элемент и `Process` команды для полной обработки секции с использованием параллельной пакетной обработки.|  
...
||Расположение|Демонстрирует использование XML для Аналитики `Discover` метод для получения содержимого набора строк схемы DISCOVER_LOCATIONS, указав путь расположения файлов резервной копии.|  
