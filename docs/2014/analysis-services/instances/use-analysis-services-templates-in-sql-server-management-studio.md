...
# <a name="use-analysis-services-templates-in-sql-server-management-studio"></a>Use Analysis Services Templates in SQL Server Management Studio
  [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]предоставляет набор шаблонов для быстрого создания скриптов XMLA, DMX или запросов многомерных выражений, создания ключевых показателей эффективности в Кубе или табличной модели, скриптов операций резервного копирования и восстановления и выполнения многих других задач. Шаблоны расположены в **обозревателе шаблонов** в среде [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)].  
...
##  <a name="bkmk_usingTE"></a>Открытие шаблона Analysis Services  
...
##  <a name="BKMK_Building_Queries"></a>Создание и выполнение запроса многомерных выражений для табличной модели с помощью шаблона  
...
##  <a name="bkmk_backup"></a>Создание скрипта XMLA на основе шаблона  
...
##  <a name="bkmk_Ref"></a>Справочник по шаблону Analysis Services  
...
|--------------|-------------------|-----------------|  
|Расширения интеллектуального анализа данных\Содержимое модели|Запрос содержимого|Демонстрирует использование * \<>модели *расширения интеллектуального анализа данных SELECT FROM. CONTENT Инструкция для получения содержимого набора строк схемы модели интеллектуального анализа данных для указанной модели интеллектуального анализа данных.|  
||Значения непрерывного столбца|Демонстрирует использование инструкции SELECT DISTINCT из * \<модели>* расширений интеллектуального анализа данных с помощью `RangeMin` расширений `RangeMax` интеллектуального анализа данных и функций для получения набора значений в указанном диапазоне из непрерывных столбцов в указанной модели интеллектуального анализа данных.|  
||Значения дискретных столбцов|Демонстрирует использование инструкции SELECT DISTINCT из * \<модели>* расширений интеллектуального анализа данных для получения полного набора значений из дискретных столбцов в указанной модели интеллектуального анализа данных.|  
...
||Содержимое PMML|Демонстрирует использование * \<>модели *расширения интеллектуального \* анализа данных SELECT FROM. PMML-Инструкция для получения представления модели интеллектуального анализа данных на языке разметки прогнозирующих моделей (PMML) для алгоритмов, поддерживающих эту функцию.|  
...
|Расширения интеллектуального анализа данных\Прогнозирующие запросы|Основной прогноз|Демонстрирует, как объединить инструкцию SELECT FROM * \<Model>* PREDICTION JOIN расширений интеллектуального анализа данных с запросом источника данных OPENQUERY, чтобы выполнить прогнозирующий запрос к модели интеллектуального анализа, используя данные, полученные с помощью запроса, из существующего источника данных.|  
||Вложенный прогноз|Демонстрирует, как комбинировать инструкцию SELECT FROM * \<Model>* PREDICTION JOIN расширений интеллектуального анализа данных с запросами к исходным данным Shape и OPENQUERY для выполнения прогнозирующего запроса к модели интеллектуального анализа данных, используя данные, содержащие вложенные таблицы, полученные с помощью запроса, из существующего источника данных.|  
||Вложенный одноэлементный прогноз|Демонстрирует использование предложения SELECT FROM * \<Model>* естественного прогнозирующего соединения расширений интеллектуального анализа данных для выполнения прогнозирующего запроса к модели интеллектуального анализа данных с помощью одного значения, явно указанного в прогнозирующих запросах, в столбце, имя которого совпадает со столбцом в модели интеллектуального анализа данных и содержит набор значений во вложенной таблице, созданной с помощью инструкции Union, имена которых также соответствуют вложенным столбцам в модели|  
||Одноэлементный прогноз|Демонстрирует, как использовать инструкцию SELECT FROM \<Model расширений интеллектуального анализа данных> естественного прогнозирующего подключения для выполнения прогнозирующего запроса к модели интеллектуального анализа данных с помощью одного значения, явно указанного в прогнозирующих запросах, в столбце, имя которого совпадает со столбцом в модели интеллектуального анализа данных.|  
