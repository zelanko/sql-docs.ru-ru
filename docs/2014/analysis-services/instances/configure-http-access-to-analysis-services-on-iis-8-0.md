...
-   [Средств](#bkmk_overview)  
  
-   [Необходимые компоненты](#bkmk_prereq)  
  
-   [Копирование MSMDPUMP. dll в папку на веб-сервере](#bkmk_copy)  
  
-   [Создание пула приложений и виртуального каталога в службах IIS](#bkmk_appPool)  
  
-   [Настройка проверки подлинности IIS и Добавление расширения](#bkmk_auth)  
  
-   [Измените MSMDPUMP. INI-файл для задания целевого сервера](#bkmk_edit)  
 ...
 **Дополнительные настройки в IIS 8,0**  
...
-   **Безопасность** | **проверки подлинности Windows**, **Обычная проверка подлинности**и другие функции безопасности, необходимые для сценария доступа к данным.  
...
2.  Откройте**разработку приложений** **веб-сервера** | и выберите расширения **CGI** и **ISAPI**.  
 ...
 **Когда службы IIS находятся на удаленном сервере**  
...
1.  Скопируйте следующие файлы, расположенные на \<диске>: \program Files\Microsoft SQL Server\\<instance\>\OLAP\bin\isapi: MSMDPUMP. DLL, MSMDPUMP. INI и папку Resources.  
...
4.  Проверьте, чтобы в папке \inetpub\wwwroot\OLAP на веб-сервере содержалось следующее: файлы MSMDPUMP.DLL, MSMDPUMP.INI и папка Resources. Структура папки должна выглядеть следующим образом:  
    ...
    -   \<> диска: \Инетпуб\ввврут\олап\мсмдпумп.длл  
  
    -   \<> диска: \Инетпуб\ввврут\олап\мсмдпумп.ини  
  
    -   \<> диска: \Инетпуб\ввврут\олап\ресаурцес  
  
##  <a name="bkmk_appPool"></a>Шаг 2. Создание пула приложений и виртуального каталога в службах IIS  
...
##  <a name="bkmk_auth"></a>Шаг 3. Настройка проверки подлинности IIS и Добавление расширения  
 ...
 **Обычная проверка подлинности** используется при наличии удостоверений Windows, но пользовательские соединения относятся к недоверенным доменам, запрещая использование делегированных или олицетворенных подключений. Обычная проверка подлинности позволяет указать учетные данные пользователя и пароль в строке подключения. Для подключения к службам Analysis Services вместо контекста безопасности текущего пользователя применяются учетные данные из строки подключения. Службы Analysis Services поддерживают только проверку подлинности Windows, поэтому любые передаваемые им учетные данные должны представлять пользователя (или группу) Windows, который является членом домена, где размещены службы Analysis Services.  
  
 **Анонимная проверка подлинности** часто используется во время первоначального тестирования, так как ее простота настройки помогает быстро проверить подключение по протоколу HTTP к Analysis Services. С помощью всего лишь нескольких шагов можно назначить уникальную учетную запись пользователя в качестве идентификатора, предоставить этой учетной записи разрешения в службах Analysis Services, воспользоваться учетной записью для проверки возможности доступа к данным в клиентском приложении, а затем отключить анонимную проверку подлинности по завершении тестирования.  
...
7.  Щелкните правой кнопкой мыши в любом месте страницы и выберите пункт **Добавить карту скриптов**. В диалоговом окне Добавление схемы скрипта в качестве пути запроса укажите ** \*. dll** , укажите к:\инетпуб\ввврут\олап\мсмдпумп.длл в качестве исполняемого файла и в качестве имени введите **OLAP** . Сохраните все ограничения по умолчанию, связанные с этой картой скриптов.  
...
##  <a name="bkmk_edit"></a>Шаг 4. изменение MSMDPUMP. INI-файл для задания целевого сервера  
...
```  
 ...
 Если экземпляр служб Analysis Services, для которого настраивается доступ через HTTP, расположен на локальном компьютере и является экземпляром по умолчанию, нет причин для изменения этого параметра. В противном случае необходимо указать имя сервера (например, \<ServerName>ADWRKS-SRV01\</сервернаме>). Для сервера, установленного как именованный экземпляр, обязательно добавьте имя экземпляра (например, \<ServerName>ADWRKS-SRV01\Tabular\</сервернаме>).  
  
 По умолчанию экземпляр служб Analysis Services прослушивает TCP/IP-порт 2383. Если вы установили Analysis Services в качестве экземпляра по умолчанию, вам не нужно указывать порт в \<ServerName>, так как Analysis Services знает, как прослушивать порт 2383 автоматически. Тем не менее необходимо разрешить входящие соединения с этим портом в брандмауэре Windows. Дополнительные сведения см. в статье [Configure the Windows Firewall to Allow Analysis Services Access](configure-the-windows-firewall-to-allow-analysis-services-access.md).  
  
 Если вы настроили именованный или заданный по умолчанию экземпляр Analysis Services для прослушивания фиксированного порта, необходимо добавить номер порта к имени сервера (например \<, ServerName>AW-SRV01:\<55555/сервернаме>). Кроме того, необходимо разрешить входящие подключения в брандмауэре Windows к этому порту.  
...
##  <a name="bkmk_test"></a>Шаг 6. Тестирование конфигурации  
 ...
 **Проверка соединений с помощью SQL Server Management Studio**  
 ...
 **Проверка соединений с помощью Excel**  
 ...
 **Проверка соединений с помощью объектов AMO**  
...
## <a name="see-also"></a>См. также  
 ...
 [Настройка SSL в IIS 7](https://go.microsoft.com/fwlink/?LinkId=207562)  
