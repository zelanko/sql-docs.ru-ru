---
title: VERIFY a PowerPivot for SharePoint Installation | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
8.  Нажмите кнопку **приложение службы PowerPivot по умолчанию** для открытия панели управления PowerPivot для этого приложения. На первый запуск панели мониторинга требуется несколько минут.  
  
     Также можно щелкнуть пустое место рядом с **приложение службы PowerPivot по умолчанию** выберите строку и нажмите кнопку **свойства** для просмотра параметров конфигурации для этого приложения службы. Изменить конфигурацию сервера можно с помощью как параметров конфигурации, так и свойств приложения. Дополнительные сведения об этих параметрах см. в разделе [создать и настроить приложение службы PowerPivot в центре администрирования](../../power-pivot-sharepoint/create-and-configure-power-pivot-service-application-in-ca.md).  
...
8.  В поле имя сервера введите  **\<имя_сервера > \powerpivot**, где  **\<имя сервера >** имя компьютера, на котором установлен PowerPivot для SharePoint.  
  
9. Нажмите кнопку **Соединить**. Будет проверена доступность сервера служб Analysis Services.  
  
10. В обозревателе объектов щелкните **баз данных** список загруженных файлов данных PowerPivot.  
  
11. В файловой системе компьютера откройте следующую папку, чтобы определить, были ли файлы кэшированы на диск. Наличие кэшированных файлов служит еще одним подтверждением работоспособности развертывания. Чтобы просмотреть кэш файлов, перейдите в \<диск >: \Program Files\Microsoft SQL Server\MSAS11. Папка POWERPIVOT\OLAP\Backup\Sandboxes\Default приложение службы PowerPivot. Каждая кэшируемая база данных хранится в собственной папке, схема именования которых основана на идентификаторах GUID и гарантирует уникальность имен.  
