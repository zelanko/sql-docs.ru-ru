 ...
 **Изменения в работе.** Книги SQL Server 2008 R2 PowerPivot не обновляются автоматически при их использовании в SQL Server 2012 PowerPivot с пакетом обновления 1 (SP1) для SharePoint 2013. Поэтому плановые обновления данных не будут действовать для книг SQL Server 2008 R2 PowerPivot.  
...
##  <a name="bkmk_to_2012sp1_from_2012"></a> Обновление до книг Office 2013 из версий, созданных с помощью 2012 надстройки PowerPivot для Excel  
 ...
 «Операция обновления для книг, созданных в предыдущей версии PowerPivot, недоступна».  
...
1.  Обновление каждой книги вручную путем ее открытия в Excel на компьютере, где [!INCLUDE[ssSQL11](../../../includes/sssql11-md.md)] версии PowerPivot для Excel и затем опубликовать его на сервер. При открытии книги в последней версии надстройки выполняются следующие внутренние операции. Поставщик данных в строке подключения к данным книги обновляется до MSOLAP.5, обновляются также метаданные, и выполняется воссоздание связей для соответствия более новой реализации.  
  
2.  Кроме того, администратор SharePoint может включить функцию автоматического обновления для системной службы PowerPivot в ферме SharePoint для автоматического обновления [!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)] книги PowerPivot при обновлении данных по расписанию (только книги, настроить для запланированного обновления данных будут обновлены).  
...
```  
 ...
 Результатом вызова Get-PowerPivotSystemService будет список свойств вместе с соответствующими значениями. Вы увидите `WorkbookUpgradeOnDataRefresh` в списке свойств. Оно будет иметь значение **true** , если автоматическое обновление включено. Если же это свойство имеет значение **false**, перейдите на следующий шаг и включите автоматическое обновление книги.  
...
2.  На вкладке **Определение** в начале строки подключения отображается версия поставщика.  
     ...
     **Источник данных = $Embedded$** указывает, что книга книги PowerPivot, с помощью внедренной базы данных.  
