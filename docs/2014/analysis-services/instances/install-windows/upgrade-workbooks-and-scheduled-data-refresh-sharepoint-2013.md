...
1.  Обновление каждой книги вручную путем ее открытия в Excel на компьютере, где [!INCLUDE[ssSQL11](../../../includes/sssql11-md.md)] версии PowerPivot для Excel с последующей повторной публикацией на сервере. При открытии книги в последней версии надстройки выполняются следующие внутренние операции. Поставщик данных в строке подключения к данным книги обновляется до MSOLAP.5, обновляются также метаданные, и выполняется воссоздание связей для соответствия более новой реализации.  
  
2.  Кроме того, администратор SharePoint может включить функцию автоматического обновления для системной службы PowerPivot в ферме SharePoint для автоматического обновления [!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)] книги PowerPivot при обновлении данных по расписанию (только книги, которые являются настроить для расписания обновления данных будут обновлены).  
...
```  
 ...
 Результатом вызова Get-PowerPivotSystemService будет список свойств вместе с соответствующими значениями. Вы должны увидеть `WorkbookUpgradeOnDataRefresh` в списке свойств. Оно будет иметь значение **true** , если автоматическое обновление включено. Если же это свойство имеет значение **false**, перейдите на следующий шаг и включите автоматическое обновление книги.  
