...
|`Compression Level`|Если `TransportCompression` является XPRESS, можно задать используемый уровень сжатия, чтобы контролировать объем сжатия. Допустимые значения: от 0 до 9, где 0 дает наименьшее сжатие, а 9 — самое большое. Более высокое сжатие снижает производительность. Значение по умолчанию — 0.|  
...
|`Optimize Response`|Битовая маска, определяющая используемую оптимизацию ответа на запрос.<br /><br /> 0x01: по умолчанию. Использовать NormalTupleSet <br />0x02: использовать, когда срезы пусты.|  
 ...
 **Строка подключения в Excel**  
 ...
 Дополнительные сведения см. в разделе [подключения к данным, источники данных и строки подключения в службах Reporting Services](../../reporting-services/data-connections-data-sources-and-connection-strings-in-reporting-services.md) и [данные проверки подлинности для служб Excel в SharePoint Server 2013](http://go.microsoft.com/fwlink/?LinkId=296350).  
