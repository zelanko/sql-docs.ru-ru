...
|`Impersonation Level`|Указывает уровень олицетворения, который разрешен серверу при олицетворении клиента. Допустимы следующие значения.<br /><br /> **Анонимные**: клиент является анонимным на сервер. Серверному процессу не удается получить идентификационные сведения о клиенте, также клиент не может быть олицетворен.<br /><br /> **Определить**: серверный процесс может получить удостоверение клиента. Сервер может олицетворять клиента для авторизации, но не может получить доступ к системным объектам от имени клиента.<br /><br /> **Олицетворение**: это значение по умолчанию. Клиент может быть олицетворен, но только при установке соединения, а не при каждом вызове.<br /><br /> **Делегат**: серверный процесс может олицетворять контекст безопасности клиента, действуя от имени клиента. Серверный процесс также может отправлять исходящие вызовы другим серверам, действуя от имени клиента.|  
|`Integrated Security`|Идентификатор участника Windows вызывающей стороны используется для подключения к службам Analysis Services. Допустимые значения — blank (пусто), SSPI и BASIC.<br /><br /> `Integrated Security`=`SSPI` значение по умолчанию для TCP-подключений, позволяя NTLM, Kerberos или анонимной проверки подлинности. Значение по умолчанию для HTTP-соединений — blank (пустое значение).<br /><br /> При использовании `SSPI`, `ProtectionLevel` должно быть присвоено одно из следующих: `Connect`, `PktIntegrity`, `PktPrivacy`.|  
...
|`Compression Level`|Если `TransportCompression` является XPRESS, можно задать используемый уровень сжатия сколько сжатие используется. Допустимые значения: от 0 до 9, где 0 дает наименьшее сжатие, а 9 — самое большое. Более высокое сжатие снижает производительность. Значение по умолчанию — 0.|  
...
|`Optimize Response`|Битовая маска, определяющая используемую оптимизацию ответа на запрос.<br /><br /> 0x01: по умолчанию. Использовать NormalTupleSet <br />0x02: используйте, когда срезы пусты|  
 ...
 **Строка подключения в Excel**  
 ...
 Дополнительные сведения см. в разделе [подключения к данным, источники данных и строки подключения в службах Reporting Services](../../reporting-services/data-connections-data-sources-and-connection-strings-in-reporting-services.md) и [данных проверки подлинности для служб Excel в SharePoint Server 2013](http://go.microsoft.com/fwlink/?LinkId=296350).  
 ...
 **Соединения HTTP (s) с книгами PowerPivot (файлы XLSX, xlsb или xlsm)**  
  
 `Data Source=<URL>`, где «URL» — это путь SharePoint к книге PowerPivot, опубликованной в библиотеке SharePoint. Например «источник данных =http://localhost/Shared Documents/Sales.xlsx».  
  
 **Соединения по HTTP(S) с файлами соединения семантической модели бизнес-аналитики**  
  
 `Data Source=<URL>` , где «URL» — путь SharePoint к BISM-файлу. Например «источник данных =http://localhost/Shared Documents/Sales.bism».  
...
## <a name="see-also"></a>См. также  
 ...
 [Поставщики данных, используемые для соединений со службами Analysis Services](data-providers-used-for-analysis-services-connections.md)   
