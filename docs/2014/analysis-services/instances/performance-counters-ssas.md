...
###  <a name="cache"></a><a name="bkmk_Cache"></a>Мбайта  
...
###  <a name="connection"></a><a name="bkmk_Connection"></a>Соединен  
 Статистические данные, относящиеся к соединениям служб Microsoft Analysis Services.  
  
|Счетчик|Описание|  
|-------------|-----------------|  
|Текущие подключения|Текущее число установленных клиентских подключений.|  
|Число запросов в секунду|Интенсивность запросов на соединение.  Поступившие запросы.|  
|Общее количество запросов|Общее число запросов на подключение.  Поступившие запросы.|  
|Успехи/сек|Частота успешных установок подключений.|  
|Всего успехов|Всего успешных соединений.|  
|Отказы/с|Интенсивность отказов при соединениях.|  
|Всего отказов|Общее число неудачных попыток подключения.|  
|Текущие сеансы пользователей|Текущее число установленных пользовательских сеансов.|  
  
###  <a name="data-mining-model-processing"></a><a name="bkmk_DataMiningModelProcessing"></a>Обработка модели интеллектуального анализа данных  
...
###  <a name="data-mining-prediction"></a><a name="bkmk_DataMiningPrediction"></a>Прогноз интеллектуального анализа данных  
...
###  <a name="locks"></a><a name="bkmk_Locks"></a>Намерен  
...
###  <a name="mdx"></a><a name="bkmk_MDX"></a>МНОГОМЕРНЫХ выражений  
...
###  <a name="memory"></a><a name="bkmk_Memory"></a>Свободной  
...
|Занято памяти, КБ|Использование памяти процессом сервера согласно вычислениям стоимости памяти очистки.  Равно сумме значений счетчика Process\PrivateBytes и объема сопоставленных в памяти данных за вычетом памяти, сопоставленной или выделенной xVelocity в подсистеме, выполняющейся в памяти аналитики (VertiPaq) сверх предела памяти подсистемы VertiPaq.|  
|Нижний предел памяти, КБ|Нижняя граница объема памяти, из файла конфигурации.|  
|Верхний предел памяти, КБ|Верхняя граница объема памяти, из файла конфигурации.|  
|Общий кэш, КБ|Текущий объем памяти, выделенный кэшу агрегатов (КБ).|  
|Квота, КБ|Текущая квота памяти, в КБ.  Квота памяти также называется выделением или резервированием памяти.|  
|заблокировано квот|Текущее число запросов на квоту, заблокированных до высвобождения других квот памяти.|  
...
|Ограничение памяти VertiPaq (КБ)|Ограничение памяти, из файла конфигурации.|  
  
###  <a name="proactive-caching"></a><a name="bkmk_ProactiveCaching"></a>Упреждающее кэширование  
...
###  <a name="processing-aggregations"></a><a name="bkmk_ProcAggregations"></a>Обработка агрегатов  
...
###  <a name="processing-indexes"></a><a name="bkmk_ProcIndexes"></a>Обработка индексов  
...
###  <a name="processing"></a><a name="bkmk_Processing"></a>Обработки  
...
|Преобразовано строк/с|Интенсивность преобразованных в ходе обработки строк.|  
|Всего строк преобразовано|Общее число преобразованных в ходе обработки строк.|  
|Записано строк/с|Интенсивность записанных в ходе обработки строк.|  
|Всего строк записано|Общее число записанных в ходе обработки строк.|  
  
###  <a name="storage-engine-query"></a><a name="bkmk_StorageEngineQuery"></a>Запрос к подсистеме хранилища  
...
|Счетчик|Описание|  
|-------------|-----------------|  
|бездействующие потоки быстрого анализа|Число бездействующих потоков в пуле потоков быстрого анализа.|  
|занятые потоки быстрого анализа|Число занятых потоков в пуле потоков быстрого анализа.|  
...
|бездействующие потоки продолжительного анализа|Число бездействующих потоков в пуле потоков продолжительного анализа.|  
|занятые потоки продолжительного анализа|Число занятых потоков в пуле потоков продолжительного анализа.|  
...
|бездействующие потоки в пуле запросов|Число бездействующих потоков в пуле потоков запросов.|  
...
|бездействующие потоки в пуле обработки, не связанные с вводом-выводом|Количество бездействующих потоков в пуле потоков обработки, выделенных для заданий, не связанных с вводом-выводом.|  
|занятые потоки в пуле обработки, не связанные с вводом-выводом|Количество потоков, выполняющих задания, не связанные с вводом-выводом в пуле потоков обработки.|  
|Длина очереди заданий пула обработки|Количество заданий, не связанных с вводом-выводом, в очереди пула потоков обработки.|  
...
|бездействующие потоки ввода-вывода в пуле обработки заданий|Количество бездействующих потоков, выполняющих задания ввода-вывода, в пуле потоков обработки.|  
|занятые потоки ввода-вывода в пуле обработки заданий|Количество потоков, выполняющих задания ввода-вывода, в пуле потоков обработки.|  
|длина очереди заданий ввода-вывода пула обработки|Количество заданий ввода-вывода в очереди пула потоков обработки.|  
