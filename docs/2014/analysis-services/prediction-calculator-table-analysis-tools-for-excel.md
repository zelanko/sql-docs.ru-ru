---
title: Расчет прогноза (средства анализа таблиц для Excel) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36109597"
---
# <a name="prediction-calculator-table-analysis-tools-for-excel"></a>Расчет прогноза (средства анализа таблиц для Excel)
  ![Рассчет прогнозов](media/tat-predcal.gif "средство «расчет прогноза»")  
  
 **Расчет прогноза** помогает создать оценочный лист, который может использоваться для анализа новых данных и оценки параметров или риска. Например, если у вас есть исторические и демографические данные о заказчиках **расчет прогноза** средства помогут вам в две основные задачи:  
...
3.  В **расчет прогноза** диалоговое окно, для целевого объекта, выберите столбец, который необходимо создать прогноз, например поведения при покупке.  
...
6.  В **Расширенный выбор столбцов** диалоговое окно, выберите столбцы, содержащие полезную информацию. Удалите все столбцы, не имеющие отношение к анализу. Нажмите кнопку **ОК**.  
...
7.  В **параметры вывода** выберите **функциональный калькулятор** создать анализ и оценочный лист в книге Excel. Выберите **калькулятор для печати** Чтобы создать анализ, а также формируется отчет, который может быть распечатан и использован для ручной оценки.  
 ...
 **Расчет прогноза** средство использует алгоритм логистической регрессии Майкрософт, который может работать с дискретными значениями, а также дискретизированным, так и непрерывных числовых данных.  
...
-   Объект **отчет по прогнозу**, содержащий результаты анализа с интерактивными таблицами и графиками, помогающими экспериментировать с взаимодействиями и прибылями.  
  
-   Интерактивная **расчет прогноза** , помогающее создавать оценки.  
  
-   Объект **калькулятор для печати** с инструкциями и коэффициентами, используемыми при оценке.  
...
#### <a name="chart-for-viewing-maximum-profit"></a>Диаграмма для просмотра максимальной прибыли  
 ...
 Например if, в левом верхнем углу таблице ячейка для **предлагаемое пороговое значение для максимизации прибыли** показывает значение 500, с правой стороны диаграммы будет показывать 500 в качестве высшей точки на линейного графика. Значение 500 означает, что для максимизации прибыли необходимо использовать 500 первых рекомендаций модели интеллектуального анализа данных, представленных в порядке убывания вероятности.  
...
#### <a name="table-listing-scores-for-each-attribute-and-value"></a>Табличные списки оценок для каждого атрибута и значения  
 ...
 Значения в **относительное влияние** столбец, значения вероятности в процентах. Затенение ячейки служит визуальным представлением влияния этого значения на результаты.  
...
#### <a name="first-table"></a>Первая таблица  
 ...
 Например, если отчет содержит следующие значения, можно уменьшить значение «Автомобили» до 1, а затем до 0, чтобы посмотреть, как это влияет на поведение клиента при покупке. При изменении значения **автомобилей** 0 прогноз в нижней меняется на TRUE.  
...
### <a name="printable-prediction-calculator"></a>Рачет прогноза с возможностью печати  
 Третий лист, созданный средством «расчет прогноза» называется **калькулятор PrintablePrediction \<целевое состояние > из \<целевой атрибут >**. Этот оценочный лист можно напечатать, так что с его помощью можно вручную рассчитывать оценки, если нет под рукой компьютера.  
...
1.  Перейдите на вкладку под названием **печатных расчет прогноза для \<атрибут >**.  
  
2.  Выберите в меню файл Excel **предварительного просмотра перед печатью**.  
...
7.  Вычислить сумму оценок для каждого атрибута и введите это число **общее** строки.  
  
8.  Преобразуйте оценку в результат прогноза с помощью критериев, напечатанных на листе сразу после **общее** строки.  
  
## <a name="related-tools"></a>Дополнительные средства  
 Службы [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] предоставляют алгоритм логистической регрессии (Майкрософт) для использования в анализе этого типа. Если вы уже знакомы с логистической регрессией, можно легко создать модели логистической регрессии с помощью **Дополнительно** параметры клиента интеллектуального анализа данных для Excel. Дополнительные сведения см. в разделе [Advanced моделирования &#40;интеллектуального анализа данных надстройки для Excel&#41;](advanced-modeling-data-mining-add-ins-for-excel.md). Дополнительные сведения о параметрах и параметров для моделей логистической регрессии см. в разделе «Алгоритм логистической регрессии» в [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] электронной документации.  
