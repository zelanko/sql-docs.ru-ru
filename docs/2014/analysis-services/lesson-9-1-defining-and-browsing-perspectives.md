...
1.  Откройте конструктор кубов для куба "Учебник по службам [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] " и перейдите на вкладку **Перспективы** .  
     ...
     ![Вкладка «Перспективы» конструктора кубов](../../2014/tutorials/media/l9-perspectives-1.gif "вкладка «Перспективы» конструктора кубов")  
...
2.  Измените имя новой перспективы к `Reseller Sales`.  
...
2.  Измените имя новой перспективы к `Sales Summary`.  
...
4.  При выполнении команды «Анализ в Excel» выводится запрос выбора перспективы, которая будет использоваться при просмотре модели в Excel, как показано на следующем рисунке.  
     ...
     ![Объекты для перспективы Internet Sales](../../2014/tutorials/media/l9-perspectives-3.gif "объектов для перспективы Internet Sales")  
...
5.  Кроме того, можно запустить Excel из меню «Пуск» Windows, определить подключение к базе данных учебника по службам Analysis Services на узле localhost и выбрать перспективу в мастере подключения к данным, как показано на следующем рисунке.  
     ...
     ![Мастер подключения данных в Excel](../../2014/tutorials/media/l9-perspectives-3b.gif "мастер подключения данных в Excel")  
  
6.  Выберите `Internet Sales` в **перспективы** списка, а затем просмотрите меры и измерения в панели «метаданные».  
...
7.  На панели "Метаданные" разверните узел **Меры**.  
     ...
     Обратите внимание, что только `Internet Sales` отображается группа мер, вместе с **Internet GPM** и **Internet Sales Ratio to All Products** вычисляемые элементы.  
