...
-   **имя атрибута>. Samples: гистограммы в этой строке показывают последовательности элементов, которые являются репрезентативными для каждого кластера. \<** Для расширений интеллектуального анализа данных — это образцы вариантов для каждого кластера.  
  
-   имя атрибута>: гистограммы в этой строке описывают все элементы, содержащиеся в кластере, и их общее распределение. ** \< ** Щелкните гистограмму, когда отображается **Легенда интеллектуального анализа данных** , чтобы увидеть числовые значения каждого  
