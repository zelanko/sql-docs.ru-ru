---
title: Вкладка «Сравнение» (средство просмотра моделей интеллектуального анализа данных) кластера | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
|**Отдает приоритет \<кластера 1 >**|Гистограмма слева показывает вероятность, что выбранная пара "атрибут-значение" типична для кластера, выбранного в поле **Кластер 1**. Можно остановить курсор мыши над полосой, чтобы увидеть значение в процентах. Обратите внимание, что, даже если значение равно нулю, это не значит, что атрибут-значение наверняка отсутствует в кластере, просто распределение явно предпочитает один кластер другому.|  
|**Отдает приоритет \<кластера 2 >**|Гистограмма справа показывает вероятность, что выбранная пара "атрибут-значение" типична для кластера, выбранного в поле **Кластер 2**.|  
