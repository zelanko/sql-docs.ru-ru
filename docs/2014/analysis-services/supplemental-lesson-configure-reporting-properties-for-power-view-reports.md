...
-   **Скрытие таблиц и столбцов, которые не используются для конечного пользователя** . свойство **Hidden** определяет, отображаются ли таблицы и столбцы таблицы в клиенте отчетов. Скрытые элементы будут по-прежнему частью модели и доступны для запросов и вычислений.  
...
-   **Задайте требуемую группировку** . свойство " **хранить уникальные строки** " определяет, должны ли значения в столбце группироваться по значениям в другом поле, например поле идентификатора. Для столбцов, содержащих повторяющиеся значения, такие как имена клиентов (например, нескольких клиентов зовут Александр Иванов), важно выполнить группирование (по уникальным строкам) по полю **Идентификатор строки** , чтобы представить пользователю правильный результат.  
  
-   **Установка типов данных и форматов данных** . по умолчанию Power View применяет правила, основанные на типе данных столбца, чтобы определить, можно ли использовать поле в качестве меры. Так как каждая визуализация данных в Power View также может иметь правила, касающиеся того, где могут быть помещены меры и данные, отличные от мер, важно задать тип данных в модели либо переопределить значение по умолчанию, чтобы добиться результата, необходимого пользователю.  
...
3.  В диалоговом окне в списке **Набор полей по умолчанию** в разделе **Поля в таблице** нажмите клавишу CTRL и выберите следующие поля, а затем нажмите **Добавить**.  
     ...
     **Дата рождения**, **альтернативный идентификатор клиента**, **имя**, **Фамилия**.  
...
4.  Повторите эти действия, задав следующий столбец и свойства для каждой указанной таблицы. Оставьте все остальные свойства в параметрах по умолчанию.  
    ...
    |------------|--------------|-----------|  
    |Geography Id|Hidden|Истина|  
    |Дата рождения|Формат данных|Short Date|  
    ...
    |Номер дня недели|Hidden|Истина|  
    |День недели|Сортировать по столбцам|Номер дня недели|  
    |Day of Week|Hidden|Истина|  
    |День месяца|Hidden|Истина|  
    |День года|Hidden|Истина|  
    |Названия месяца|Сортировать по столбцам|Месяц|  
    |Месяц|Hidden|Истина|  
    |Месячный календарь|Hidden|Истина|  
    |Финансовый квартал|Hidden|Истина|  
    |Финансовый год|Hidden|Истина|  
    |Финансовый семестр|Hidden|Истина|  
    ...
    |------------|--------------|-----------|  
    |Geography Id|Hidden|Истина|  
    |Идентификатор территории продаж|Hidden|Истина|  
  
     **Продукт**  
  
    |Столбец|Свойство|Значение|  
    |------------|--------------|-----------|  
    |Product Id|Hidden|Истина|  
    |Product Alternate Id|Метка по умолчанию|Истина|  
    |Идентификатор подкатегории продукта|Hidden|Истина|  
    ...
    |Большое изображение|Hidden|Истина|  
    ...
    |------------|--------------|-----------|  
    |Product Id|Hidden|Истина|  
    |Customer Id|Hidden|Истина|  
    |Идентификатор продвижения|Hidden|Истина|  
    |Идентификатор валюты|Hidden|Истина|  
    |Идентификатор территории продаж|Hidden|Истина|  
    ...
    |Дата оплаты счета|Тип данных|Short Date|  
