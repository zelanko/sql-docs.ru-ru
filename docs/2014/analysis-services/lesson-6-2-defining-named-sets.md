...
4.  На **метаданных** вкладку в **средства вычисления** области, разверните **продукта**, разверните **категории**, разверните `Members`, а затем Разверните **все продукты**.  
...
1.  Щелкните правой кнопкой мыши `[Core Products]` в **Организатор скриптов** , а затем щелкните **создать именованный набор**.  
...
7.  На **метаданные** вкладку в **средства вычисления** области, разверните **количество сотрудников** в измерении «Торговый посредник» разверните `Members`и разверните **Все торговые посредники**.  
...
9. В окне «Свойства» измените `DiscretizationMethod` свойства **автоматического**и затем измените `DiscretizationBucketCount` свойства `5`. Дополнительные сведения см. в разделе [Группирование элементов атрибутов (дискретизация)](multidimensional-models/attribute-properties-group-attribute-members.md).  
...
12. На **метаданные** вкладку в **средства вычисления** области, разверните **количество сотрудников** в **Reseller** измерения, разверните `Members`, а затем разверните **все торговые посредники**.  
     ...
     Группируются элементы этой иерархии атрибута, так как свойство DiscretizationBucketCount задано значение `5` и DiscretizationMethod является свойство **автоматического**.  
...
13. В поле **Выражение** добавьте запятую в выражении для набора Exists после функции Members и перед закрывающей скобкой, а затем перетащите элемент **83 — 100** с панели **Метаданные** и поместите его после запятой.  
     ...
     На следующем рисунке показана **выражения вычисления** область для `[Large Resellers]` именованного набора.  
...
6.  На панели **Метаданные** в измерении **Товар** перетащите **Основные товары** в область фильтра.  
     ...
     ![Члены в основной продукт именованный набор](../../2014/tutorials/media/l6-named-set-04.gif "члены в основной продукт именованный набор")  
...
7.  На панели **Метаданные** разверните узел **Торговый посредник**, добавьте элемент **Крупные торговые посредники** в область фильтра.  
     ...
     ![Задает область фильтра, содержащий два именованных](../../2014/tutorials/media/l6-named-set-05.gif "задает содержащий два с именем «фильтр»")  
