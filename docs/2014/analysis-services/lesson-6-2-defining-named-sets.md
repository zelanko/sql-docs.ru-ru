...
3.  В **имя** измените имя нового именованного набора на `[Core Products]`.  
...
4.  На **метаданных** вкладке **средства вычисления** области разверните **продукта**, разверните **категории**, разверните `Members`, а затем Разверните **все продукты**.  
...
1.  Щелкните правой кнопкой мыши `[Core Products]` в **Организатор скриптов** области, а затем щелкните **создать именованный набор**.  
  
2.  В **имя** измените имя этого именованного набора на `[Large Resellers]`.  
  
3.  В **выражение** введите `Exists()`.  
...
6.  После части выражения набора введите точку, а затем добавьте функцию Members. Выражение должно иметь следующий вид:  
    ...
    ```  
     ...
     Теперь, когда вы определили первое множество для Exists выражение набора, можно приступать к добавлению второго набора для набора элементов измерения торгового посредника, который содержит наибольшее количество сотрудников.  
  
7.  На **метаданных** вкладке **средства вычисления** панели разверните **количество сотрудников** в измерении «Торговый посредник» разверните узел `Members`и затем разверните **Все торговые посредники**.  
...
9. В окне «Свойства» измените `DiscretizationMethod` свойства **автоматического**, а затем измените `DiscretizationBucketCount` свойства `5`. Дополнительные сведения см. в разделе [Группирование элементов атрибутов (дискретизация)](multidimensional-models/attribute-properties-group-attribute-members.md).  
...
12. На **метаданных** вкладке **средства вычисления** панели разверните **количество сотрудников** в **торгового посредника** измерение, разверните узел `Members`, а затем разверните **все торговые посредники**.  
     ...
     Элементы этой иерархии атрибута распределены в том случае, так как свойству DiscretizationBucketCount присвоено `5` и присваивается свойству DiscretizationMethod **автоматического**.  
...
13. В поле **Выражение** добавьте запятую в выражении для набора Exists после функции Members и перед закрывающей скобкой, а затем перетащите элемент **83 — 100** с панели **Метаданные** и поместите его после запятой.  
     ...
     На следующем рисунке показана **выражения вычисления** области для `[Large Resellers]` именованного набора.  
  
     ![Панель «Выражения вычисления» для [Крупные торговые посредники]](../../2014/tutorials/media/l6-named-set-02.gif "панель «выражения вычисления» для [Крупные торговые посредники]")  
...
5.  Разверните измерение «Товар», а затем добавьте в область строк элементы «Категория» и «Подкатегория», как показано на следующем рисунке.  
     ...
     ![Элементы атрибута «Подкатегория»](../../2014/tutorials/media/l6-named-set-03.gif "элементы атрибута «Подкатегория»")  
...
6.  На панели **Метаданные** в измерении **Товар** перетащите **Основные товары** в область фильтра.  
     ...
     ![Членами основной продукт именованного набора](../../2014/tutorials/media/l6-named-set-04.gif "членами основной продукт именованный набор")  
...
7.  На панели **Метаданные** разверните узел **Торговый посредник**, добавьте элемент **Крупные торговые посредники** в область фильтра.  
     ...
     ![Задает область фильтра, содержащий два именованных](../../2014/tutorials/media/l6-named-set-05.gif "задает панель фильтра, содержащий два именованных")  
