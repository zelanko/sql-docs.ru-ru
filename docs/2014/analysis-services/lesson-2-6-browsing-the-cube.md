...
2.  Нажмите кнопку **браузера** вкладку для отображения **все** членом `Product Key` иерархии атрибута. На третьем занятии будет определена пользовательская иерархия для измерения «Продукты», позволяющая просматривать это измерение.  
