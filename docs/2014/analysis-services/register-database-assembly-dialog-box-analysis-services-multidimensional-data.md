...
|Термин|Определение|  
|----------|----------------|  
|**Type**|Выбирает тип ссылки на сборку. Доступны следующие значения:<br /><br /> **Сборка .NET** <br />                      Ссылка указывает на сборку [!INCLUDE[msCoName](../includes/msconame-md.md)] .NET Framework.<br /><br /> **COM DLL** <br />                      Ссылка на сборку указывает на библиотеку COM.<br /><br /> <br /><br /> Примечание по ** \* безопасности \* \* ** Сборки COM могут представлять угрозу безопасности. По этой причине, а также по ряду других сборки COM в службах [!INCLUDE[ssASversion10](../includes/ssasversion10-md.md)]являются устаревшими. Поддержка сборок COM в последующих версиях может быть прекращена.|  
...
|**Значение по умолчанию**|Выберите этот режим, чтобы использовать пользовательскую учетную запись по умолчанию для служб [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]. Этот режим равноценен установке параметра **Использовать учетные данные текущего пользователя** .|  
