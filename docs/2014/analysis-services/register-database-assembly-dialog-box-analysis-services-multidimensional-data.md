...
|**Имя сборки**|Введите имя для ссылки на сборку.<br /><br /> Примечание. Изменение этого значения не меняет имя сборки, на которую выполняется ссылка, но изменяет имя, используемое экземпляром служб [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] или базой данных при обращении к ссылке на сборку.|  
