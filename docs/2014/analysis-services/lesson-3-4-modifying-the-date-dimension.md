...
2.  Ближе к нижней части **таблиц** панели, щелкните правой кнопкой мыши `Date`, а затем нажмите кнопку **создать именованное вычисление**.  
  
3.  В **Создание именованного вычисления** диалоговом `SimpleDate` в **имя столбца** поле, а затем введите или скопируйте и вставьте следующий `DATENAME` инструкции в **выражение**  поле:  
...
4.  Нажмите кнопку **ОК**, а затем разверните `Date` в **таблиц** области.  
     ...
     `SimpleDate` Именованного вычисления появится в списке столбцов в таблице дат со значком, обозначающим, что это именованное вычисление.  
...
1.  Откройте измерение Date в **конструкторе измерений** среды [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]. Для этого дважды щелкните `Date` измерение **измерения** узел **обозревателе решений**.  
...
1.  В **Структура измерения** конструктора измерений для `Date` измерение, перетащите **календарный год** атрибута из **атрибуты** на панель **Иерархий** области.  
  
2.  Перетащите **Календарное полугодие** атрибута из **атрибуты** на панель  **\<новый уровень >** ячейки в **иерархии**области, расположенных под **календарный год** уровне.  
  
3.  Перетащите **Календарный квартал** атрибута из **атрибуты** на панель  **\<новый уровень >** ячейки в **иерархии**области, расположенных под **Календарное полугодие** уровне.  
  
4.  Перетащите **English Month Name** атрибута из **атрибуты** на панель  **\<новый уровень >** ячейки в **иерархии**области, расположенных под **Календарный квартал** уровне.  
  
5.  Перетащите **Date Key** атрибута из **атрибуты** на панель  **\<новый уровень >** ячейки в **иерархий** области , расположенную под **English Month Name** уровне.  
  
6.  В **иерархий** панели, щелкните правой кнопкой мыши строку заголовка **иерархии** , выберите команду **переименование**и введите `Calendar Date`.  
  
7.  С помощью контекстного меню, в `Calendar Date` иерархии, переименовать **English Month Name** уровень `Calendar Month`, а затем переименуйте **ключ даты** уровень `Date`.  
...
1.  В **конструктор измерений** для `Date` измерение, щелкните **связи атрибутов** вкладки.  
...
3.  В **Создание именованного вычисления** диалоговом `MonthName` в **имя столбца** поле, а затем введите или скопируйте и вставьте следующую инструкцию в **выражение** поле:  
...
6.  В **Создание именованного вычисления** диалоговом `CalendarQuarterDesc` в **имя столбца** поле, а затем введите или скопируйте и вставьте следующий сценарий SQL в **выражение** поле:  
...
9. В **Создание именованного вычисления** диалоговом `CalendarSemesterDesc` в **имя столбца** поле, а затем введите или скопируйте и вставьте следующий сценарий SQL в **выражение** поле:  
...
8.  В **столбец имени** в диалоговом **исходный столбец** выберите `MonthName`, а затем нажмите кнопку **ОК**.  
...
6.  В **столбец имени** в диалоговом **исходный столбец** выберите `CalendarQuarterDesc`, а затем нажмите кнопку **ОК**.  
...
6.  В **столбец имени** в диалоговом **исходный столбец** выберите `CalendarSemesterDesc`, а затем нажмите кнопку **ОК**.  
...
2.  После получения **развертывание выполнено успешно** сообщений, нажмите кнопку **браузера** вкладке **конструктор измерений** для `Date` измерения, и на панели инструментов конструктора нажмите кнопку повторного соединения.  
...
1.  Откройте **Структура измерения** конструктора измерений для `Date` измерения, а затем выберите **Календарное полугодие** в **атрибуты** области.  
...
9. В меню **Построение** среды [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)]выберите команду **Развернуть Analysis Services Tutorial**. После успешного завершения развертывания нажмите кнопку **браузера** в конструкторе измерений для `Date` измерения.  
