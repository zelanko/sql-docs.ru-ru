...
1.  В диалоговом окне **Диспетчер секций** в разделе **Секции**щелкните только `Internet Sales 2005` что созданную секцию, а затем **скопируйте**.  
...
4.  Установите флажок в столбце **Обработка** для каждой из пяти созданных секций, а затем нажмите кнопку **ОК**.  
     ...
     Затем отображается диалоговое окно **обработка данных** , в котором отображаются сведения о процессе для каждой секции. Обратите внимание, что для каждой секции передается разное количество строк. Это вызвано тем, что каждая секция включает только строки за год, указанный в предложении WHERE инструкции SQL. Нет данных за 2010 год.  
