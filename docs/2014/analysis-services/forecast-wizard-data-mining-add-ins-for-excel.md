...
# <a name="forecast-wizard-data-mining-add-ins-for-excel"></a>Мастер прогнозов (надстройки интеллектуального анализа данных для Excel)
 ...
 С помощью мастера прогнозов можно прогнозировать значения во временных рядах. Мастер прогнозов использует алгоритм [!INCLUDE[msCoName](../includes/msconame-md.md)] временных рядов (), который является алгоритмом регрессии, предназначенным для прогнозирования непрерывных столбцов, таких как продажи продукта.  
...
3.  На странице **Прогнозирование** в поле **Метка времени**выберите столбец, содержащий уникальное числовое значение (в том числе значения даты и времени), которое можно использовать в качестве ряда вариантов. Источник данных должен быть отсортирован по этому столбцу в порядке возрастания.  
     ...
     Если в данных нет такого столбца, можно использовать параметр \<без метки времени>. Мастер добавит уникальный столбец сортировки для входных данных, поэтому необходимо убедиться, что данные отсортированы должным способом, прежде чем запускать мастер и выбирать этот параметр.  
