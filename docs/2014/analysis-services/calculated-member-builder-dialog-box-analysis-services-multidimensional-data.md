...
|Термин|Определение|  
|----------|----------------|  
|**Name**|Введите имя вычисляемого элемента.|  
|**Родительская иерархия**|Выберите иерархию, в которой должен быть создан вычисляемый элемент.|  
|**Родительский элемент**|Данный параметр включен, если выбрана родительская иерархия (отличная от измерения `Measures`), имеющая несколько уровней. Нажмите кнопку с многоточием (**...** ) кнопку, чтобы выбрать родительский элемент. Родительский элемент определяет расположение вычисляемого элемента в структуре измерения.|  
