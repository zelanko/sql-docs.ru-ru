...
5.  Теперь щелкните раскрывающийся список **Переменная заливки** и выберите переменную **Age** . После этого в текстовом поле **состояние** появится список значений.  
     ...
     Столбец «Возраст», используемый в качестве входных данных для этой модели, содержит непрерывные числовые значения, но для кластеризации алгоритм всегда дискретизирует значения. Здесь можно просмотреть ячейки или группы, созданные алгоритмом, например "очень низкий ( \<=27)" and "Very High (> = 63)".  
