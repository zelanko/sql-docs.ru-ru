...
# <a name="cell-element-mddataset-xmla"></a>Элемент Cell (MDDataSet) (XML для аналитики)
  Содержит сведения об одной ячейке, содержащейся в родительском [CellData](celldata-element-xmla.md) элемента.  
...
## <a name="calculating-cell-ordinal-values"></a>Вычисление значения порядкового номера ячейки  
 Положение ячейки на оси может быть вычислено на основании значения атрибута `CellOrdinal`. По существу, ячейки нумеруются в наборе данных, как будто набор данных *p*-мерный массив, где *p* — число осей. Адресация ячеек осуществляется по строкам.  
