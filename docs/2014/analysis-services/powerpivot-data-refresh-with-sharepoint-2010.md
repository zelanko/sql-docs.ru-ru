---
title: Обновление данных PowerPivot с SharePoint 2010 | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
##  <a name="bkmk_services"></a> Шаг 1: Включение службы Secure Store и формирование главного ключа  
...
15. Нажмите кнопку **ОК**.  
 ...
 Прежде чем станет доступным ведение журналов аудита операций службы Secure Store, которое может оказаться полезным для целей отладки, его необходимо включить. Дополнительные сведения о включении ведения журнала см. в разделе [настроить службу Secure Store (SharePoint 2010)](http://go.microsoft.com/fwlink/p/?LinkID=223294).  
  
##  <a name="bkmk_creds"></a> Шаг 2: Отключите параметры учетных данных, не требуется поддерживать  
 ...
 Вариант 1, **использовать для обновления данных учетную запись, настроенную администратором**, всегда отображается на странице определения расписания, но работает только при настройке учетной записи автоматического обновления. Дополнительные сведения о создании учетной записи см. в разделе [настройки PowerPivot данные учетной записи автоматического обновления &#40;PowerPivot для SharePoint&#41;](configure-unattended-data-refresh-account-powerpivot-sharepoint.md).  
  
 Вариант 2, **подключения со следующими учетными данными windows**, всегда отображается на странице, но работает только при включении **разрешить пользователям вводить пользовательские учетные данные Windows** параметр в службе страница конфигурации приложения. Этот параметр включен по умолчанию, но его можно отключить, если минусы его использования превышают плюсы (см. ниже).  
  
 Вариант 3, **подключиться, используя учетные данные, сохраненные в службе Secure Store**, всегда отображается на странице, но работает только владелец расписания указал действительное целевое приложение. Администратор должен предварительно создать это целевое приложение и сообщить его имя создателю расписания обновления данных. Дополнительные сведения о том, как создать целевое приложение для данных операций обновления см. в разделе [Настройка сохраненных учетных данных для обновления данных PowerPivot &#40;PowerPivot для SharePoint&#41;](configure-stored-credentials-data-refresh-powerpivot-sharepoint.md).  
...
3.  В действиях, нажмите кнопку **Настройка параметров приложения службы** чтобы открыть страницу параметров приложений службы PowerPivot  
...
##  <a name="bkmk_stored"></a> Шаг 3: Создание целевых приложений для хранения учетных данных, используемых при обновлении данных  
 ...
 Дополнительные сведения о создании целевых приложений, которые содержат сохраненные учетные данные в разделе [настройки PowerPivot данные учетной записи автоматического обновления &#40;PowerPivot для SharePoint&#41; ](configure-unattended-data-refresh-account-powerpivot-sharepoint.md) и [ Настройка сохраненных учетных данных для обновления данных PowerPivot &#40;PowerPivot для SharePoint&#41;](configure-stored-credentials-data-refresh-powerpivot-sharepoint.md).  
...
##  <a name="bkmk_scale"></a> Шаг 4: Настройка сервера для масштабируемого обновления данных  
 ...
 Дополнительные сведения см. в разделе [Настройка выделенного обновления данных или обработка Query-Only &#40;PowerPivot для SharePoint&#41;](configure-dedicated-data-refresh-query-only-processing-powerpivot-sharepoint.md).  
 ...
 **Включена PowerPivot учетной записи автоматического обновления?**  
 ...
 **Вы с помощью второй вариант учетных данных: позволяет владельцу расписания вводить имя пользователя Windows и пароль?**  
 ...
 **Вы с помощью третий вариант учетных данных: с помощью целевого приложения службы Secure Store, чтобы предоставить удостоверение пользователя для запуска заданий обновления данных?**  
 ...
 **Как проверить строку подключения, чтобы определить, можно ли переопределить учетные данные, используемые во время обновления данных**  
...
3.  Нажмите кнопку **конструктора**.  
  
4.  Нажмите кнопку **существующие соединения**.  
  
     В списке отображаются все соединения, используемые в книге **подключения к данным PowerPivot**.  
...
5.  Выберите соединение и нажмите кнопку **изменить**, а затем нажмите кнопку **Дополнительно**. Строка подключения находится в нижней части страницы.  
 ...
 Если вы видите **Persist Security Info = False, Password =\* \* \* \* \* \* \* \* \* \* \*, UserID =\<имя_входа >**, то у вас есть строку соединения, учетные данные можно переопределить. Учетные данные в строке подключения (UserID и Password) являются не учетными данными Windows, а именем входа базы данных или зарегистрированной учетной записью на целевом источнике данных.  
  
 **Переопределение учетных данных в строке подключения**  
...
###  <a name="procIntervals"></a> Установите «время» для обработки в нерабочие  
...
5.  Нажмите кнопку **ОК**.  
 ...
 Для долговременного физического хранения данных журнала для приложения службы PowerPivot используется база данных приложения службы PowerPivot. Дополнительные сведения о том, как собираются и хранятся данные об использовании см. в разделе [сбора данных об использовании PowerPivot](power-pivot-sharepoint/power-pivot-usage-data-collection.md).  
...
##  <a name="configTimerJob"></a> Изменить расписание задания таймера обновления данных PowerPivot  
 ...
 Если позволяет оборудование, можно уменьшить этот риск, указав дополнительные процессоры, которые понадобятся для параллельного запуска большего числа заданий обновления данных. Дополнительные сведения см. в разделе [Настройка выделенного обновления данных или обработка Query-Only &#40;PowerPivot для SharePoint&#41;](configure-dedicated-data-refresh-query-only-processing-powerpivot-sharepoint.md). Дополнительные сведения о обнаруженных, добавляются в очередь и обработать запросы обновления данных см. в разделе [обновления данных PowerPivot](power-pivot-sharepoint/power-pivot-data-refresh.md).  
