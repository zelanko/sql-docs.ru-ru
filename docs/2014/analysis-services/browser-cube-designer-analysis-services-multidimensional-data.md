---
title: Браузер (конструктор кубов) (службы Analysis Services — многомерные данные) | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
 ...
 **Просмотр и запросы в режиме графического конструирования**  
 ...
 При работе в режиме графического конструктора, если **Автовыполнение** (![автоматическое выполнение запроса](media/rsqdicon-autoexecute.gif "автоматическое выполнение запроса")) выбран переключатель на панели инструментов, **Браузера** выполняет запрос каждый раз при помещении объекта метаданных на панель «данные». Можно также запустить вручную запроса с помощью **выполнить запрос** (![выполните запрос](media/rsqdicon-run.gif "выполните запрос")) на панели инструментов.  
