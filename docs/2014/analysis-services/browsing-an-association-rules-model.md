 ...
 *Вероятность* представляет собой долю вариантов в наборе данных, содержащих заданное сочетание элементов. Вероятность аналогична статистическому понятию *достоверности*и дает указание как вероятность результата применения правила выдается. Можно изменить значение **Минимальная вероятность** на этой панели, чтобы отфильтровать отображаемые правила.  
 ...
 Значение для **Минимальная вероятность** изначально отображаются — это пороговое значение, которое было использовано алгоритмом, при построении модели. После завершения модели это значение понизить нельзя, но можно увеличить его, чтобы отобразить только элементы с большей вероятностью.  
...
1.  Попробуйте щелкнуть столбец заголовков - **вероятность**, **важности**, и **правило** -, чтобы узнать, как изменяются данные.  
  
2.  Используйте **правило фильтрации** параметр, чтобы ввести значения и сосредоточиться на выбранных правилах.  
  
     Например если вы хотите увидеть все правила, предсказать, какие заказчики, скорее всего, приобретут вместе с перчатками, введите «gloves» в текстовом поле и обновите панель.  
...
5.  Задайте значение для **максимальное число строк** значение 100, а затем нажмите кнопку **Копировать в Excel**.  
     ...
     Обратите внимание, что изменение этого значения не оказывает никакого влияния на объем данных в модели; а лишь определяет количество строк в отображаемом списке. Этот параметр может быть полезен при работе с очень большими моделями.  
...
1.  Нажмите кнопку **найти** кнопки и использовать **найти узел** диалоговое окно для ввода интересующего нас элемента.  
     ...
     Например, введите «gloves» и затем раскройте граф в окне, чтобы можно легко просмотреть результаты.  
  
     Узел, содержащий элемент, выделен; стрелки, указывающие на узел, представляют правило, соединяющее элементы.  
  
     Направление стрелки сообщает о направленности правила. К примеру Если кто-то перчатки, скорее всего, приобрести жилет, стрелка будет запустить из узла «glove» и завершить на узле «жилет».  
  
     Чтобы получить дополнительную статистику об этом правиле, можно щелкнуть **правила** табуляция и найдите правило с описанием «Glove — существующий» -> «Vest — существующие.»)  
...
-   **Наборы элементов:** При создании модели формируемым количеством наборов элементов также можно управлять с помощью параметра MINIMUM_PROBABILITY. Этот параметр доступен в диалоговом окне Параметры алгоритма.  
