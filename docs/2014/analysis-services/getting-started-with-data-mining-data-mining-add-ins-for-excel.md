...
## <a name="define-your-goal"></a>Определение цели  
 ...
 Например вместо цели «поиск новых заказчиков», следует сделать цель более конкретной — «определение демографии заказчиков, которые, скорее всего, купят наш продукт с вероятностью по крайней мере 65%».  
...
-   Набор данных должен содержать по крайней мере один атрибут «результат», который можно использовать для обучения и прогнозирования. Если такого атрибута нет, можно вручную обозначить некие обучающие данные или создать прокси-сервер для выходного результата с помощью других столбцов.  
     ...
     Например если требуется предсказать «наиболее подходящих потенциальных клиентов», необходимо применить бизнес-правило заранее чтобы пометить существующих клиентов, таким образом, интеллектуальному анализу данных получить из примеров необходимые знания.  
...
-   При работе со значением, которое меняется со временем и для которого требуется прогнозировать будущие тенденции, нужно определить гранулярность анализа. Необходимы ли прогнозы на месяц, на день, на год? Данные должны быть проанализированы с помощью тех же единиц, которые необходимо прогнозировать.  
     ...
     С циклами Если вы не получили хорошие результаты ежедневные данные, попробуйте другие временные срезы, или с помощью дни недели, месяцы или даже праздничные дни.  
...
-   Что такое метрики, по которому будет оцениваться успешность модели? Как вы поймете, что модель является «достаточно хорошо»?  
...
## <a name="explore-your-data-and-explore-the-model"></a>Изучение данных и модели  
 ...
 Если планируется выполнить интеллектуальный анализ данных для набора данных, большие или сложном, что не удалось проанализировать его с другими методами, рассмотрите возможность выборки или сокращении объема данных.  
...
## <a name="validate-your-model"></a>Проверка модели  
 После запуска мастера или средства алгоритм анализирует содержимое данных и определяет, существует ли статистически значимая закономерность. Если алгоритм не находит такой закономерности, вы получите сообщение об ошибке. Тем не менее даже если модель создана успешно, необходимо проверить модель, чтобы увидеть, если она подтверждает предположения. Можно использовать средства, такие как [диаграмма точности &#40;SQL Server Data Mining Add-ins&#41; ](accuracy-chart-sql-server-data-mining-add-ins.md) или [перекрестной проверки &#40;SQL Server Data Mining Add-ins&#41; ](cross-validation-sql-server-data-mining-add-ins.md) для создания статистических Показатели качества модели.  
