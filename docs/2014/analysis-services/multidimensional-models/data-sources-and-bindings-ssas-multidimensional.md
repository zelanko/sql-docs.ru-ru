...
### <a name="measuregroup-granularity-attributes"></a>Атрибуты гранулярности группы мер  
 ...
 Однако в некоторых случаях атрибуты `GranularityAttributes` могут не существовать как столбцы в таблице фактов. Например `GranularityAttributes` не существовать в виде столбцов в следующих случаях:  
...
-   Каждый столбец невложенной таблицы привязан к мере в кубе, к атрибуту в измерении этого куба (указав `CubeDimension` для устранения неоднозначности в случае ролей измерения), или к атрибуту в измерении.  
...
## <a name="out-of-line-bindings"></a>внешние привязки  
 ...
 Язык ASSL позволяет задавать либо вне строки привязки `Process` команды, если это не в пакете или в `Batch` команды. Если внешние привязки указываются в команде `Batch`, все привязки, заданные в команде `Batch`, создают новый контекст привязок, в котором выполняются все команды `Process` пакета. Новый контекст привязок включает объекты, которые обрабатываются неявно вследствие команды `Process`.  
  
 Если внешние привязки заданы в команде, они переопределяют встроенные привязки, содержащиеся в неизменяемых библиотеках DDL для указанных объектов. Эти обрабатываемые объекты могут включать имя объекта непосредственно в `Process` команды, или они могут включать другие объекты, обработка которых начинается автоматически в ходе обработки.  
  
 Внешние привязки указываются с помощью включения в команду обработки необязательного объекта коллекции `Bindings`. Необязательный `Bindings` коллекция содержит следующие элементы.  
...
### <a name="definition-of-the-out-of-line-binding-type"></a>Определение типа внешней привязки  
 В пределах внешней коллекции `Bindings` язык ASSL позволяет создавать коллекцию привязок для нескольких объектов `Binding`. Каждый объект `Binding` имеет расширенную ссылку на объект, которая напоминает простую объектную ссылку, но может ссылаться также на более мелкие объекты (например атрибуты измерения и атрибуты группы мер). Такой объект принимает плоский вид, типичный для элемента `Object` элемент в `Process` команд, кроме тех, которые \< *объект*>\<*/Object*> отсутствуют теги.  
  
 Каждый объект, для которого задана привязка определяется в XML-элементе в формате \< *объект*> ID (например, `DimensionID`). После определения объекта как можно точнее с формой \< *объект*> ID, находится элемент, для которого задана привязка, которая обычно `Source`. В общем случае Обратите внимание, — это, в котором `Source` является свойством `DataItem`, представляющего собой вариант привязки к столбцу в атрибуте. В этом случае тег `DataItem` не указывается, вместо этого нужно задать свойство `Source`, как при непосредственной привязке столбца.  
 ...
 Один элементом, допускающимся в `Binding` , не существует непосредственно в DDL является `ParentColumnID`, который используется для вложенных таблицах для интеллектуального анализа данных. В этом случае необходимо идентифицировать родительский столбец вложенной таблицы, для которого предоставляется привязка.  
