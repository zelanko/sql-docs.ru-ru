...
-   [Объекты серверов](#ServerObjects)  
  
-   [Объекты исключений AMOException](#AMO)  
  
-   [Объекты баз данных](#DatabaseObjects)  
...
### <a name="connecting-to-the-server-object"></a>Соединение с объектом Server  
 ...
 Следующий образец кода возвращает объект <xref:Microsoft.AnalysisServices.Server>, если соединение прошло успешно, либо значение `null` в случае ошибки. Ошибки во время процесса соединения обрабатываются операторами try и catch. Ошибки объектов AMO обрабатываются с помощью класса исключения <xref:Microsoft.AnalysisServices.AmoException>. В данном примере ошибка выдается пользователю в окне сообщения.  
...
### <a name="processing-a-database"></a>Обработка базы данных  
 ...
 Этому методу можно передавать параметры, но все они не являются обязательными. Если не передано ни одного параметра, то все дочерние объекты будут обработаны с параметром `ProcessDefault`. Дополнительные сведения о параметрах обработки см. в разделе <xref:Microsoft.AnalysisServices.Database>.  
...
1.  Создание объекта <xref:Microsoft.AnalysisServices.DataSourceView>.  
     ...
     Сначала проверьте, что объект <xref:Microsoft.AnalysisServices.DataSource> существует. Если значение равно `true`, удалите объект <xref:Microsoft.AnalysisServices.DataSource> и создайте его. Если объект <xref:Microsoft.AnalysisServices.DataSource> отсутствует, создайте его.  
...
```  
 ...
 В этом образце кода методы `AddTable` и `AddComputedColumn` используют метод `FillSchema` объекта `DataAdapter` для добавления объекта `DataTable` в объект `DataSet`, а также для настройки схемы таким образом, чтобы она соответствовала схеме источника данных. Расширенные свойства добавляют сведения, необходимые для настройки схемы для служб [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)].  
