...
1.  В области **Кубы** выберите куб, а затем щелкните `Read` или доступ на **чтение и запись** .  
     ...
     `Read`для большинства операций достаточно доступа. **Чтение и запись** используются только для обратной записи, но не для обработки. Обратитесь к разделу [Set Partition Writeback](set-partition-writeback.md) за дополнительной информацией об этой возможности.  
