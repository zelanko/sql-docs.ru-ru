...
|**агрегат**|По умолчанию меры суммируются вдоль каждого измерения. Однако свойство `AggregateFunction` позволяет изменить это. См. список в разделе [Use Aggregate Functions](use-aggregate-functions.md) .|  
