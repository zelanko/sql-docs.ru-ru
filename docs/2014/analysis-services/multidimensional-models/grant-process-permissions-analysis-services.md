...
3.  В области **Общие** установите `Process Database` флажок. Кроме того, `Read Definition` установите флажок, чтобы включить интерактивную обработку через один из средств SQL Server, [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]таких как.  
