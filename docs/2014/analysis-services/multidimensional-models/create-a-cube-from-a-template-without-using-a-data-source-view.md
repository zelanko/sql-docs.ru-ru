...
|**Агрегирование**|Стандартная статистическая функция для меры. Щелкните эту ячейку, чтобы задать для меры одну из стандартных статистических функций (или значение **Нет**). Значение по умолчанию при создании меры — `Sum`. В этом столбце задается свойство `AggregationFunction` объекта меры.|  
