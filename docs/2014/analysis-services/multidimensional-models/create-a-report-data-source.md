...
5.  В поле **Строка подключения**введите имя сервера служб Analysis Services, имя базы данных, имя куба и все необязательные параметры.  
    ...
    >  Если имеется более одного куба, необходимо указать имя куба.  
     ...
     (Необязательно) Кубы могут иметь метаданные и переводы данных, заданные для различных языков в модели. Чтобы просмотреть переводы (данные и метаданные), необходимо добавить в строку подключения свойство "код локали":`Data source=<servername>;initial catalog=<multidimensionaldatabasename>-ee;cube='<cubename>'; Locale Identifier=<identifier number>`  
