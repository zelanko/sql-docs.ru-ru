...
## <a name="readwritemode-usage"></a>Использование свойства ReadWriteMode  
 ...
 `ReadWriteMode` Чтобы переключить свойство базы `ReadWrite` данных `ReadOnly`между и, необходимо выполнить последовательность `Detach/Attach` команд.  
