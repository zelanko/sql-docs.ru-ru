...
## <a name="using-the-properties-keyword"></a>Использование ключевого слова PROPERTIES  
 Ключевое слово `PROPERTIES` указывает свойства элементов, которые будут использоваться для данной оси измерения. `PROPERTIES` Ключевое слово запрятано в `<axis specification>` предложение многомерных выражений [ВЫБЕРИТЕ](/sql/mdx/mdx-data-manipulation-select) инструкции:  
