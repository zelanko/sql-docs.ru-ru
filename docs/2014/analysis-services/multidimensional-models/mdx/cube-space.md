...
> [!NOTE]  
>  Обратите внимание, что `WITH` используется ключевое слово с [CurrentMember (многомерные Выражения)](/sql/mdx/current-mdx) и [Name (многомерные Выражения)](/sql/mdx/members-string-mdx) функции для создания вычисляемого элемента для использования в запросе. Дополнительные сведения см. в разделе [Базовый запрос многомерных выражений (многомерные выражения)](mdx-query-the-basic-query.md).  
