...
|**FONT_FLAGS**|Битовая маска, определяющая стиль шрифта. Например, значение 5 соответствует комбинации полужирного (`MDFF_BOLD`) и подчеркнутого (`MDFF_UNDERLINE`) со шрифтами. Значение представляет собой результат побитовой операции OR над одной или несколькими следующими константами.<br /><br /> `MDFF_BOLD` = 1<br /><br /> `MDFF_ITALIC` = 2<br /><br /> `MDFF_UNDERLINE` = 4<br /><br /> `MDFF_STRIKEOUT` = 8|  
|**FONT_NAME**|Шрифт, который будет использоваться для отображения `VALUE` или `FORMATTED_VALUE` свойства.|  
...
|`FORMAT`|То же, что `FORMAT_STRING`.|  
...
```  
 ...
 Свойства ячеек не возвращаются Многомерными запросами, возвращающими плоские наборы строк; в этом случае каждая ячейка представляется, как если бы только `FORMATTED_VALUE` свойство ячейки были возвращены.  
