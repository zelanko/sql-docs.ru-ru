...
|`ExternalAccess`|Предоставляет такой же доступ, как `Safe` параметр с дополнительными возможностями для доступа к внешним системным ресурсам. Данный сегмент разрешений не предлагает гарантий безопасности (хотя возможность обезопасить данный сценарий существует), однако он предоставляет гарантии надежности.|  
...
-   В случае отсутствия промежуточной инструкции EXECUTE AS, изменившей контекст исходного вызывающего элемента, попытка получить доступ к внутреннему ресурсу закончится ошибкой.  
 ...
 `ImpersonationMode` Свойству можно присвоить значение `ImpersonateCurrentUser` или `ImpersonateAnonymous`. Значение по умолчанию `ImpersonateCurrentUser`, запускает сборку под сетевой учетной записью входа текущего пользователя. Если `ImpersonateAnonymous` параметр используется, контекст выполнения соответствует учетной записи входа Windows IUSER_*servername* на сервере. Это учетная запись гостя из сети Интернет, которой предоставлены ограниченные права на сервере. Сборка, запущенная в таком контексте, может получить доступ только к ограниченным ресурсам на локальном сервере.  
...
## <a name="see-also"></a>См. также  
 [Настройка безопасности хранимых процедур](../multidimensional-models-extending-olap-stored-procedures/setting-security-for-stored-procedures.md)   
