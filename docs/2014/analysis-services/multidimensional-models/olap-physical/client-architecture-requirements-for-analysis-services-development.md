---
title: Требования к архитектуре клиентов для развития аналитических услуг Документы Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="client-architecture-requirements-for-analysis-services-development"></a>Требования к архитектуре клиента для разработки служб Analysis Services
 ...
 ![Логическая архитектура клиента для аналитических служб](../../../analysis-services/dev-guide/media/as-clientarch9.gif "Логическая архитектура клиента для аналитических служб")
...
## <a name="analysis-services-in-tabular-or-sharepoint-mode"></a>Службы Analysis Services в табличном режиме или режиме интеграции с SharePoint.
 ...
 [!INCLUDE[ssGeminiClient](../../../includes/ssgeminiclient-md.md)] и [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)] — единственные клиентские среды, которые поддерживаются при создании размещенных в памяти баз данных, использующих режим SharePoint или табличный режим соответственно. Встроенная база данных PowerPivot, [!INCLUDE[ssGemini](../../../includes/ssgemini-md.md)] которую вы создаете с помощью Excel и инструментов, содержится в рабочей книге Excel и сохраняется как часть файла Excel .xlsx.
...
### <a name="interfaces-for-powerpivot-client"></a>Интерфейсы для клиента PowerPivot
 [!INCLUDE[ssGemini](../../../includes/ssgemini-md.md)]взаимодействует с движком анализа памяти xVelocity (VertiPaq) в рабочей книге, используя установленные интерфейсы и языки для аналитических служб: AMO и ADOMD.NET, а также MDX и XMLA. В надстройке меры определяются с помощью языка формул, аналогичного языку формул Excel и DAX (выражения анализа данных). Выражения анализа данных внедряются в сообщения XMLA, отправляемые внутрипроцессному серверу.

### <a name="providers"></a>Поставщики
 Связь [!INCLUDE[ssGemini](../../../includes/ssgemini-md.md)] между и Excel использовать MSOLAP OLEDB провайдера (версия 11.0). В поставщике MSOLAP имеется четыре модуля (также называемых транспортами), которые можно использовать для пересылки сообщений между клиентом и сервером.

 **TCP/IP** Используется для обычных клиент-серверных соединений.

 **HTTP** Используется для подключения HTTP через службу насоса данных SSAS или по вызову в компонент Web-сервиса SharePoint PowerPivot (WS).

 **ИНПРОК** Используется для подключения к движку в процессе.

 **КАНАЛ** Зарезервировано для связи с системой PowerPivot Service на ферме SharePoint.
