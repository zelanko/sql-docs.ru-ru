---
title: Справочник по свойствам атрибута измерения | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
|`AttributeHierarchyOptimizedState`|Определяет уровень оптимизации иерархии атрибутов. По умолчанию иерархия атрибутов полностью `FullyOptimized`, то есть службы [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] создают индексы для иерархии атрибутов, чтобы повысить производительность запросов. Другое значение, `NotOptimized`, означает, что для иерархии атрибутов не создано ни одного индекса. С помощью `NotOptimized` полезно, если иерархия атрибутов используется для целей, нежели формирование запросов, так как дополнительные индексы не создаются для атрибута. Другие применения иерархии атрибутов могут помочь упорядочить другой атрибут.|  
|`AttributeHierarchyOrdered`|Определяет упорядоченность связанной иерархии атрибутов. Значение по умолчанию — `True`. Тем не менее, если иерархия атрибута не будет использоваться для выполнения запросов, то можно сократить время обработки, изменив значение этого свойства равным `False`.|  
|`AttributeHierarchyVisible`|Определяет, является ли иерархия атрибутов видимой для клиентских приложений. Значение по умолчанию — `True`. Тем не менее, если иерархия атрибута не будет использоваться для выполнения запросов, то можно сократить время обработки, изменив значение этого свойства равным `False`.|  
...
|`ValueColumn`|Определяет столбец со значением атрибута. Если `NameColumn` указан элемент атрибута, же `DataItem` значение используется в качестве значения по умолчанию для `ValueColumn` элемент. Если элемент `NameColumn` атрибута не определен и коллекция атрибутов `KeyColumns` содержит один элемент `KeyColumn`, представляющий ключевой столбец со строковым типом данных, то в качестве значений по умолчанию для элемента `DataItem` используются те же значения `ValueColumn`.|  
  
> [!NOTE]  
>  Дополнительные сведения о том, как задать значения для `KeyColumn` свойство при работе со значениями null и других проблемах целостности данных см. в разделе [обработка проблем целостности данных в службах аналитики 2005](http://go.microsoft.com/fwlink/?LinkId=81891).  
