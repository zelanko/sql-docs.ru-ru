...
##  <a name="bkmk_Design"></a>Создание моделей для использования с режимом DirectQuery  
...
-   **Ограниченное использование функций DAX:** Некоторые функции DAX нельзя использовать в режиме DirectQuery, поэтому их необходимо заменить другими функциями или создать значения с помощью производных столбцов в источнике данных. Конструктор моделей во время разработки обеспечивает проверку ошибок, возникающих при создании формул, несовместимых с режимом DirectQuery. Дополнительные сведения см. в следующих разделах: [Проверка](#bkmk_Validation).  
...
|**Свойство DirectQueryMode**|После развертывания модели предпочтительный источник данных запросов для модели DirectQuery можно изменить, задав свойство в среде [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]<br /><br /> Как и в случае с предыдущим свойством, это свойство указывает источник данных по умолчанию для модели и может принимать следующие значения.<br /><br /> **Memory**: запросы могут использовать только кэш.<br /><br /> **DirectQuerywithInMemory**: запросы используют реляционный источник данных по умолчанию, если иное не указано в строке подключения клиента.<br /><br /> **InMemorywithDirectQuery**: запросы используют кэш по умолчанию, если иное не указано в строке подключения клиента.<br /><br /> (**DirectQuery**. Запросы используют только реляционный источник данных.<br /><br /> <br /><br /> Дополнительные сведения см. в разделе [Установка или изменение предпочтительного метода подключения для DirectQuery](../set-or-change-the-preferred-connection-method-for-directquery.md).|  
...
##  <a name="bkmk_related_tasks"></a>Связанные разделы и задачи  
