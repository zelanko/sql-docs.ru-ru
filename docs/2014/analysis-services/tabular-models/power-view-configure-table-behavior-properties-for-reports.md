...
-   **Идентификатор строки** ─ указывает столбец, содержащий только уникальные значения, что позволяет использовать этот столбец в качестве внутреннего ключа группирования.

-   **Хранить уникальные строки** ─ указывает, какие столбцы содержат значения, которые должны рассматриваться как уникальные, даже если они являются повторяющимися (например, имя и фамилия сотрудника, если два или более сотрудника имеют одно и то же имя).

-   **Метка по умолчанию** ─ указывает, какой столбец предоставляет отображаемое имя для представления данных строк (например, имя сотрудника в записи о сотруднике).

-   **Образ по умолчанию** ─ указывает, какой столбец предоставляет изображение, представляющее данные строки (например, идентификатор фотографии в записи сотрудника).
 ...
 **Ранее: группирование по умолчанию на основе полей в матрице**
 ...
 **Ранее: группирование по умолчанию на основе полей в диаграмме**
 ...
 **После: группирование по идентификатору строки (идентификатор строки превращается в ось)**
