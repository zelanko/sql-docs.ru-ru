...
-   Элемент `Measure` определяет вычисления, которые могут использоваться в модели. Меру можно превратить в путем добавления набора особых атрибутов отображения, с использованием нового `KPI` элемента.  
 ...
 **Размеры:** измерение в CSDLBI представляется как сущности (таблицы) со столбцами и свойствами. Обратите внимание, что измерение, даже не входящее в перспективу, но включенное в модель, будет представлено в выходе языка CSDL, отмеченное как `Hidden`.  
