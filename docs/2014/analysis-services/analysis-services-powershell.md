---
title: Службы Analysis Services PowerShell | Документы Microsoft
ms.custom: ''
ms.date: 03/11/2017
...
<!-- ff637750.aspx above is linked to by:  (http://go.microsoft.com/fwlink/?LinkId=227613). -->
 ...
 Необходимо импортировать **SQLPS** модуль, прежде чем использовать `SQLAS` поставщиком и командлетами. Поставщик SQLAS является расширением `SQLServer` поставщика. Имеется несколько способов импорта модуля SQLPS. Дополнительные сведения см. в разделе [Импорт модуля SQLPS](../../2014/database-engine/import-the-sqlps-module.md).  
  
 Для удаленного доступа к экземпляру служб Analysis Services требуется включить удаленное администрирование и общий доступ к файлам. Дополнительные сведения см. в разделе [разрешить удаленное управление](#bkmk_remote) в этом разделе.  
...
|Табличные экземпляры и базы данных|Поддерживается локальное и удаленное администрирование.<br /><br /> Дополнительные сведения см. в разделе блога августа 2011 г. [управление табличной модели с помощью PowerShell](http://go.microsoft.com/fwlink/?linkID=227685).|  
...
```  
 ...
 При использовании обычной проверки подлинности следует всегда использовать протокол HTTPS с включенным SSL, чтобы имя пользователя и пароль передавались по зашифрованному соединению. Дополнительные сведения см. в разделе [Настройка SSL в IIS 7.0](http://go.microsoft.com/fwlink/?linkID=184299) и [Настройка обычной проверки подлинности (IIS 7)](http://go.microsoft.com/fwlink/?LinkId=230776).  
...
```  
 ...
 Теперь можно создавать резервную копию зашифрованного файла базы данных и восстанавливать ее, передавая переменную $pwd в параметр пароля. Полный пример, в котором эта возможность сочетается с другими командлетами см. в разделе [командлет Backup-ASDatabase](/sql/analysis-services/powershell/backup-asdatabase-cmdlet) и [командлета Restore-ASDatabase](/sql/analysis-services/powershell/restore-asdatabase-cmdlet).
...
-   [Загрузить анализа служб поставщика и командлетов](#bkmk_load)  
  
-   [Включить удаленное администрирование](#bkmk_remote)  
  
-   [Подключиться к объекту служб Analysis Services](#bkmk_connect)  
  
-   [Администрирование службы](#bkmk_admin)  
  
-   [Получение справки по Analysis Services PowerShell](#bkmk_help)  
  
###  <a name="bkmk_load"></a> Загрузить анализа служб поставщика и командлетов  
...
3.  На удаленном сервере, убедитесь, что запущены следующие службы: служба удаленного вызова процедур (RPC), служба поддержки NetBIOS через TCP/IP, служба инструментария управления Windows (WMI), службы удаленного управления Windows (WS-Management).  
...
6.  Дважды щелкните **брандмауэр Windows: разрешать исключения входящего удаленного администрирования**выберите **включено**, а затем нажмите кнопку **ОК**.  
  
7.  Дважды щелкните **брандмауэр Windows: Разрешить входящие файлам и принтерам исключения для совместного использования**выберите **включено**и нажмите кнопку **ОК**.  
  
8.  На локальном компьютере с установленными средствами клиента, используйте следующие командлеты для проверки удаленного администрирования, подставив фактическое имя сервера для *удаленного имя_сервера* заполнителя. Пропустите имя экземпляра, если службы Analysis Services установлены как экземпляр по умолчанию. Чтобы эта команда работала, необходимо предварительно импортировать модуль SQLPS.  
...
###  <a name="bkmk_connect"></a> Подключиться к объекту служб Analysis Services  
...
```  
 ...
 Вы увидите следующие коллекции: сборки, баз данных, роли и трассировки. Продолжая использовать `cd` и `dir`, можно просмотреть содержимое всех коллекций.  
  
 **HTTP-соединений служб Analysis Services**  
 ...
 Подключения по протоколу HTTP полезны, если сервер настроен на доступ по протоколу HTTP с помощью инструкций в этом разделе: [Настройка доступа по протоколу HTTP к службам Analysis Services в службах Internet Information Services &#40;IIS&#41; 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
...
```  
 ...
 Вы увидите следующие коллекции: сборки, баз данных, роли и трассировки. Если просмотреть содержимое этих коллекций не удается, проверьте параметры проверки подлинности в виртуальном каталоге OLAP. Убедитесь, что анонимный доступ отключен. Если используется проверка подлинности Windows, убедитесь, что у учетной записи пользователя Windows имеются административные разрешения для экземпляра служб Analysis Services.  
...
## <a name="see-also"></a>См. также  
 ...
 [Управление табличной модели с помощью PowerShell (блог)](http://go.microsoft.com/fwlink/?linkID=227685)   
