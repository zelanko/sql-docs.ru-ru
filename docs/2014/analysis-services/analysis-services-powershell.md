...
|Табличные экземпляры и базы данных|Поддерживается локальное и удаленное администрирование.<br /><br /> Дополнительные сведения см. в разделе на блог август 2011 г. [управление табличных моделей с помощью PowerShell](http://go.microsoft.com/fwlink/?linkID=227685).|  
...
6.  Дважды щелкните **брандмауэр Windows: Разрешить исключение для входящих сообщений удаленного администрирования**выберите **включено**, а затем нажмите кнопку **ОК**.  
  
7.  Дважды щелкните **брандмауэра Windows: разрешить входящих файлов и принтеров, исключения для совместного использования**выберите **включено**, а затем нажмите кнопку **ОК**.  
...
```  
 ...
 Вы должны увидеть следующие коллекции: сборок, баз данных, роли и трассировки. Продолжая использовать `cd` и `dir`, можно просмотреть содержимое всех коллекций.  
 ...
 **Подключения по протоколу HTTP к службам Analysis Services**  
 ...
 Подключения по протоколу HTTP полезны, если вы настроили свой сервер доступ по протоколу HTTP с помощью инструкций в этом разделе: [Настройка HTTP-доступа к службам Analysis Services в службах IIS &#40;IIS&#41; 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
...
```  
 ...
 Вы должны увидеть следующие коллекции: сборок, баз данных, роли и трассировки. Если просмотреть содержимое этих коллекций не удается, проверьте параметры проверки подлинности в виртуальном каталоге OLAP. Убедитесь, что анонимный доступ отключен. Если используется проверка подлинности Windows, убедитесь, что у учетной записи пользователя Windows имеются административные разрешения для экземпляра служб Analysis Services.  
...
## <a name="see-also"></a>См. также  
 ...
 [Управление табличных моделей с помощью PowerShell (блог)](http://go.microsoft.com/fwlink/?linkID=227685)   
 [Настройка HTTP-доступа к службам Analysis Services в службах IIS &#40;IIS&#41; 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
