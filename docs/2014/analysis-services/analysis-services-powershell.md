...
3.  Убедитесь, что на удаленном сервере запущены следующие службы:  Служба удаленного вызова процедур (RPC), служба поддержки NetBIOS TCP/IP, служба инструментарий управления Windows (WMI) (WMI), служба служба удаленного управления Windows (WS-Management).  
...
6.  Дважды щелкните **брандмауэр Windows: Разрешите исключение**для входящего удаленного администрирования, выберите **включено**и нажмите кнопку **ОК**.  
  
7.  Дважды щелкните **брандмауэр Windows: Разрешите исключение**для входящего общего доступа к файлам и принтерам, выберите **включено**и нажмите кнопку **ОК**.  
 ...
 **HTTP-подключения к Analysis Services**  
 ...
 HTTP-подключения полезны при настройке сервера для доступа по протоколу HTTP с помощью инструкций, приведенных в этом разделе. [Настройка HTTP-доступа к службам Analysis Services в службах Internet Information Services (IIS) 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
