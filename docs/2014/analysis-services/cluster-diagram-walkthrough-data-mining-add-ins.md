---
title: Пошаговое руководство по диаграмме (надстройки интеллектуального анализа данных) кластера | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="cluster-diagram-walkthrough-data-mining-add-ins"></a>Пошаговое руководство по диаграмме кластеров (надстройки интеллектуального анализа данных)
  После создания модели кластеризации, можно импортировать его в Visio с помощью **кластера** формирование и затем продолжить настройку и Улучшение макета. **Фигур интеллектуального анализа данных для Visio** включают следующие пользовательские элементы управления для работы с диаграммами интеллектуального анализа данных:  
  
-   создание элементов управления для диаграммы кластеров.  
  
     Эти параметры являются частью **мастера кластеров** , запускается, когда форма перемещается в рабочее пространство Visio.  
...
## <a name="build-a-cluster-diagram"></a>Создание диаграммы кластеров  
 ...
 Чтобы следовать описанным шагам, следует иметь готовую модель кластеризации. Если у вас модели, используйте [мастера кластеров &#40;интеллектуального анализа данных надстройки для Excel&#41; ](cluster-wizard-data-mining-add-ins-for-excel.md) мастера и создать модель с помощью набора данных для обучения в образце книги, используя значения по умолчанию.  
...
2.  Перетащите **кластера** на страницу.  
  
3.  На странице приветствия **мастера фигур Visio в кластере**, нажмите кнопку **Далее**.  
  
4.  На **выбрать источник данных** страница **мастера кластеров**, Выбор соединения с [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] сервер, содержащий модели интеллектуального анализа данных, которую требуется визуализировать.  
  
5.  Выберите модель соответствующие интеллектуального анализа данных и нажмите кнопку **Далее**.  
  
     Чтобы выбрать модель кластеризации, изучите описание на **свойства** области.  
  
6.  Если соединение установлено успешно, на странице **параметры диаграммы кластеров**, выберите тип диаграммы кластеров для включения в презентацию Visio:  
     ...
     **Показывать кластеры с диаграммой сравнения**  
     ...
     Можно переключиться на другой тип диаграммы после того, как мастер создаст диаграмму. Для этого щелкните кластер правой кнопкой мыши и выберите новый тип диаграммы. Теперь выберите параметр, **Показывать кластеры с диаграммой характеристик**.  
  
7.  Оставьте для параметра **количество строк в диаграмме**, как 5.  
...
8.  Щелкните **Дополнительно**.  
     ...
     **Параметры кластера** диалоговое окно находится там, где настраивается внешний вид фигуры, используемые на схеме. Можно изменить цвета, используемые на диаграмме, и фигуру, используемую для кластеров.  
  
     **Переменная заливки** управления не работает в Office 2013.  
  
     ![Щелкните "Дополнительно", чтобы выбрать цвета фигуры](media/dm13-visio-clusteroptions-advanced.gif "щелкните \"Дополнительно\", чтобы выбрать цвета фигуры")  
...
9. Нажмите кнопку **Готово** для создания диаграммы.  
     ...
     ![Сеть зависимостей](media/dm13-visiodepnet-defaultgraph.gif "сети зависимостей")  
...
1.  Перетащите ползунок **параметры кластера** элемента управления, чтобы отфильтровать слабые связи и упростить диаграмму.  
  
2.  Используйте параметр Visio **страница повторного макета** параметр, чтобы поэкспериментировать с различными макетами кластера.  
  
3.  Используйте **соединители** параметр **разработки** вкладку для изменения стиля соединителя, чтобы линии не пересекали кластеры.  
  
4.  Нажмите кнопку **надстройки** ленты, а затем отобразить один из пользовательских панелей инструментов, используемых для работы с диаграммами интеллектуального анализа данных:  
     ...
     **Описание**  
     ...
     ![Щелкните описание, чтобы получить сведения о кластере](media/dm13-visio-cluster-description-control.gif "щелкните описание, чтобы получить сведения о кластере")  
...
5.  Используйте **сдвига и масштабирования** элемента управления, в **область задач** область Visio **представление** ленты, чтобы сосредоточиться на наборе кластеров и перемещать диаграмму.  
