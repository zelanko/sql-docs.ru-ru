---
title: Поиск категорий (средства анализа таблиц для Excel) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36098767"
---
# <a name="detect-categories-table-analysis-tools-for-excel"></a>Поиск категорий (средства анализа таблиц для Excel)
  ![Определить категории кнопки на ленте](media/tat-detectcat.gif "Поиск категорий кнопка на ленте")  
  
 **Поиск категорий** средство автоматически находит строки в таблице с похожими характеристиками.  
...
6.  Средство создает новый лист с именем «Отчет по категориям», содержащий список категорий и их характеристики.  
 ...
 Дополнительные сведения об указании параметров для этого инструмента см. в разделе [обнаружить диалогового окна категории (средства анализа таблиц для Excel)](detect-categories-table-analysis-tools-for-excel.md).  
...
### <a name="category-list"></a>Список категорий  
 ...
 Модель создает временные имена для каждой категории, но можно переименовать категории, сколько пожелаете. Например, в следующем примере первая категория была переименована **низкий доход**, так как это было атрибут кластера верхнего уровня.  
...
### <a name="category-characteristics"></a>Характеристики категории  
 ...
 Заливка в столбце **относительную важность**, указывает, насколько велико, что сочетание атрибут и значение является качестве определяющего фактора. Чем длиннее полоса, тем выше вероятность, что этот атрибут является типичным для этой категории.  
  
### <a name="categories-profile-chart"></a>Диаграмма профиля категорий  
 Окончательная диаграмма **отчет по категориям** листа, **профилей категорий**, является интерактивной **Сводная диаграмма** , можно использовать для изменения порядка и скрыть поля, фильтр на основании значений и настройка внешнего вида диаграммы.  
  
 Excel 2013 теперь предоставляет **стили диаграммы** и **элементы диаграммы** управляет непосредственно в область конструктора, облегчающий Улучшение макета диаграммы.  
...
## <a name="related-tools"></a>Дополнительные средства  
 ...
 После создания модели интеллектуального анализа данных с помощью **Анализ ключевых факторов влияния** средства, можно использовать клиент интеллектуального анализа данных для Excel для обзора модели и изучения связей более подробно. Клиент интеллектуального анализа данных для Excel представляет собой отдельную надстройку, обеспечивающую более широкие возможности интеллектуального анализа данных. Сведения см. в разделе [Просмотр моделей в Excel &#40;сервера интеллектуального анализа данных SQL надстроек&#41;](browsing-models-in-excel-sql-server-data-mining-add-ins.md).  
 ...
 Дополнительные сведения об алгоритме, используемом **Поиск категорий** инструмент, см. в разделе «Алгоритм кластеризации Майкрософт» в [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] электронной документации.  
