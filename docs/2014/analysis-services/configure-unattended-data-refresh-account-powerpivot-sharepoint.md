---
title: Настройка PowerPivot (PowerPivot для SharePoint) учетной записи автоматического обновления | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
 ...
 **[!INCLUDE[applies](../includes/applies-md.md)]**  SharePoint 2010  
 ...
 [Шаг 1: Создайте целевое приложение и задайте учетные данные](#bkmk_create)  
  
 [Шаг 2: Укажите автоматической учетной записи на страницах настройки сервера PowerPivot](#bkmk_specifyUA)  
  
 [Шаг 3: Предоставьте contribute разрешения учетной записи](#bkmk_grant)  
  
 [Шаг 4: Предоставление разрешения на чтение для доступа к внешним источникам данных при обновлении данных](#bkmk_dbread)  
  
 [Шаг 5: Проверка доступности учетной записи в данных обновления страницы настройки](#bkmk_verify)  
  
 [С помощью автоматического обновления данных PowerPivot учетной записи](#bkmk_use)  
...
##  <a name="bkmk_prereq"></a> Предварительные требования  
 Служба Secure Store должна быть включена и настроена, и должен быть создан главный ключ. Инструкции о том, как это сделать в разделе [обновления данных PowerPivot в SharePoint 2010](powerpivot-data-refresh-with-sharepoint-2010.md)  
 ...
 Пользователю необходимо знать идентификатор приложения системной службы PowerPivot. Установите для этой учетной записи службы **полный доступ** учетной записи разрешения по данным автоматического обновления при создании целевого приложения на шаге 1. Эти разрешения позволяют системной службе PowerPivot получить учетные данные учетной записи автоматического обновления данных во время обновления данных. Чтобы получить сведения об учетной записи необходимые службы, откройте **Настройка учетных записей служб** в центре администрирования и выберите пул приложений служб, используемый приложением службы PowerPivot. По умолчанию это **пул приложений служб — система веб-служб SharePoint**.  
...
4.  В целевом приложении, введите **PowerPivotDataRefresh**.  
...
12. На странице «Администраторы целевого приложения» укажите идентификатор пула приложений, соответствующий приложению службы PowerPivot. Служба требует **полный доступ** разрешений, так что он может получать данные автоматической обновить сведения об учетной записи во время выполнения. Кроме того, задайте учетные записи пользователей домена Windows для любых других пользователей SharePoint, которые должны иметь права администратора для доступа к настройкам приложения.  
  
13. Нажмите кнопку **ОК**.  
  
14. Выберите целевое приложение, которое вы только что создали, щелкните стрелку вниз и выберите **задания учетных данных.**  
  
15. В **владельцы учетных данных**, введите учетную запись пользователя домена Windows, необходимо иметь разрешения на обновление учетных данных. Для действий по обновлению данных используются учетные данные и **владельцы учетных данных** имеют разрешения на изменение учетных данных.  
...
5.  В обновление данных в PowerPivot данные учетной записи автоматического обновления, введите идентификатор конечного приложения, созданный на предыдущем шаге: **PowerPivotDataRefresh**.  
  
6.  Нажмите кнопку **ОК**.  
  
###  <a name="bkmk_grant"></a> Шаг 3: Предоставьте contribute разрешения учетной записи  
...
###  <a name="bkmk_dbread"></a> Шаг 4: Предоставление разрешения на чтение для доступа к внешним источникам данных при обновлении данных  
...
2.  Убедитесь, что **использовать для обновления данных учетную запись, настроенную администратором** флажок на странице настройки обновления данных.  
...
##  <a name="bkmk_use"></a> С помощью автоматического обновления данных PowerPivot учетной записи  
...
5.  В **владельцы учетных данных**, введите учетную запись пользователя домена Windows, необходимо иметь разрешения на обновление учетных данных. Для действий по обновлению данных используются учетные данные и **владельцы учетных данных** имеют разрешения на изменение учетных данных.  
...
8.  Нажмите кнопку **ОК**.  
 ...
 Если происходит смена не только пароля, но и имени пользователя учетной записи, то, по всей видимости, потребуется выполнение дополнительных шагов настройки, таких как предоставление разрешений на чтение для внешних источников данных и разрешений SharePoint на обновление книги PowerPivot. Соответствующие инструкции, перейдите к следующему шагу в PowerPivot автоматической конфигурации обновления данных учетную запись: [Step 3: предоставление contribute разрешения учетной записи](#bkmk_grant)и выполните оставшиеся шаги, завершении при помощи проверки, Учетная запись настроена правильно.  
