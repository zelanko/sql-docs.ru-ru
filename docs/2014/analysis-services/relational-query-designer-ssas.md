---
title: Конструктор реляционных запросов (службы SSAS) | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
```  
 ...
 Нажмите кнопку **Выполнить** (**!**). Ниже приводится пример `uspGetWhereUsedProductID` параметров, для которых необходимо указать значения в **определить параметр запроса** диалоговое окно.  
...
#### <a name="example"></a>Пример  
 ...
 При вводе имени таблицы `ContactType`, это эквивалентно использованию создания инструкции SQL `SELECT * FROM ContactType`.  
