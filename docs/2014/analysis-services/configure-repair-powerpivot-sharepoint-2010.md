...
-   Общие требования для запуска средства настройки [средства настройки PowerPivot](power-pivot-sharepoint/power-pivot-configuration-tools.md).  
...
##  <a name="bkmk_using"></a> С помощью средства настройки PowerPivot  
...
-   Исключением из этого правила в средстве настройки PowerPivot для SharePoint 2010 является учетная запись службы Analysis Services. Эта учетная запись указывается во время установки, и введите пароль для этой учетной записи в **регистрация служб SQL Server Analysis Services (PowerPivot на локальном сервере)** действие. На странице сводки нет поля для этого пароля, поэтому его необходимо ввести на странице для данного действия.  
 ...
 Средство настройки PowerPivot для настройки сервера использует Windows PowerShell. Можно щелкнуть **скрипт** вкладку для просмотра скрипта Windows PowerShell.  
  
 ![Пользовательский интерфейс средства настройки](media/ssas-pctui.gif "средство настройки пользовательского интерфейса")  
...
1.  На **запустить** последовательно выберите пункты **все программы**, нажмите кнопку [!INCLUDE[ssCurrentUI](../includes/sscurrentui-md.md)], нажмите кнопку **средства настройки**, а затем нажмите кнопку **средство настройки PowerPivot** .  
...
6.  Нажмите кнопку **зарегистрировать SQL Server Analysis Services (PowerPivot) на локальном сервере**.  
...
|**Настройка экземпляра локальных служб**|Пароль учетной записи служб Analysis Services|Вводимые пользователем данные|Необходимо ввести пароль учетной записи службы Analysis Services в **регистрация служб SQL Server Analysis Services (PowerPivot) на локальном сервере** страницы.<br /><br /> Учетная запись служб была указана во время установки. Теперь необходимо ввести пароль для регистрации локального экземпляра службы в SharePoint.|  
...
||Обновить книги, чтобы получить возможность обновления данных|Вводимые пользователем данные|Обновление данных завершается ошибкой и не поддерживается для книг SQL Server 2008 R2 PowerPivot. Параметр **обновить книги, чтобы включить данные обновления** обновляет книги до версии SQL Server 2012 PowerPivot.|  
|**Создание веб-приложения по умолчанию**|Имя веб-приложения|По умолчанию (при необходимости)|Если веб-приложений не существует, то средство создаст одно веб-приложение. Веб-приложение будет настроен классический режим проверки подлинности и будет прослушивать **порт 80**. Максимальный размер файла для передачи устанавливается равным 2047 МБ, что является максимально допустимым значением в SharePoint. Больший размер файла для передачи необходим для совместимости с большими файлами PowerPivot.|  
...
|**Активация компонента PowerPivot в семействе сайтов**|URL-адрес сайта||URL-адрес семейства веб-сайтов, для которого активируются функции PowerPivot.|  
...
|**Создание учетной записи автоматического обновления для Datarefresh**|Идентификатор целевого приложения||Идентификатор приложения может быть описательным текстом.|  
...
### <a name="grant-sharepoint-permissions-to-workbook-users"></a>Предоставление разрешений SharePoint пользователям книги  
 Для публикации и просмотра книг пользователям требуются разрешения SharePoint. Не забудьте предоставить **представление** разрешений для пользователей, которым необходимо просматривать опубликованные книги и **Contribute** разрешения для пользователей, которые публикуют и обрабатывают книги. Для предоставления разрешений необходимо быть администратором семейства веб-сайтов.  
...
1.  Перейдите к документации требования к оборудованию и программному обеспечению для SharePoint 2010, [определить оборудованию и программному обеспечению (SharePoint 2010)](http://go.microsoft.com/fwlink/?LinkId=169734)  
