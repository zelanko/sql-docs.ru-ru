 ...
 *Непрерывные* числовые данные могут содержать бесконечное число дробных значений. Примером непрерывного столбца является столбец доходов. Если указать, что столбец является числовым, каждое значение в нем должно быть числовым, кроме NULL. Обратите внимание, что в Excel можно учитывать метки времени и любое другое представление даты и времени, которое можно преобразовать в тип данных [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] .  
  
 **Преобразование чисел в категориальные переменные**  
  
 Если столбец содержит числа, это не обязательно означает, что их необходимо обрабатывать как непрерывные числа. *Дискретизация* предоставляет множество преимуществ для анализа. Одно из них — уменьшение проблемной области. Кроме того, числа иногда не подходят два выражения результата.  
...
##  <a name="bkmk_OtherRequirements"></a>Другие требования к данным  
 ...
 **Ключи вложенной таблицы** используются только при получении данных из внешнего источника данных, определенного в качестве представления источника [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] данных. Дополнительные сведения о вложенных таблицах см [https://msdn.microsoft.com/library/ms175659.aspx](https://msdn.microsoft.com/library/ms175659.aspx). в следующих статьях:  
