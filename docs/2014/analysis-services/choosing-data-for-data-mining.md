...
### <a name="excel-number-formats"></a>Числовые форматы Excel  
 ...
 Один из вариантов является использование [Переразметка](relabel-sql-server-data-mining-add-ins.md) средство для изменения способа отображения и группирования чисел.  
 ...
 *Тип содержимого* — это свойство, которое применяется к столбцу для указания способа использования данных в модели. Алгоритм может использовать тип содержимого в виде инструкции или указания при анализе.  
 ...
 О типах содержимого не нужно думать, если в надстройках используются мастеры и средства. Тем не менее если вы используете [добавить модель к структуре &#40;интеллектуального анализа данных надстройки для Excel&#41; ](add-model-to-structure-data-mining-add-ins-for-excel.md) моделирование параметр, чтобы добавить новую модель к существующим данным, может возникнуть ошибка, связанная с типами содержимого.  
