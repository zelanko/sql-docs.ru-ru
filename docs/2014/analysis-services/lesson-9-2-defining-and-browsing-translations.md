...
6.  В строке для объекта **заголовка** группы мер « **Интернет-продажи** `Ventas del lnternet` » введите значение в столбце перевода « **Испанский (Испания)** » и `Ventes D'Internet` в столбце перевода « **французский (Франция)** ».  
  
7.  В строке для объекта **Caption** для `Cantidad de las Ventas del Internet` меры «продажи через Интернет — сумма продаж» введите в столбце перевода **Испанский (Испания)** и `Quantité de Ventes d'Internet` в столбце перевода **французский (Франция)** .  
