...
2.  Щелкните правой кнопкой мыши **Организатор диаграмм** области, нажмите кнопку **новую схему**и укажите `Internet Sales Order Reasons` имени созданной диаграммы.  
...
7.  В окне «Свойства» измените **FriendlyName** свойство **DimSalesReason** таблицы, к `SalesReason`, а затем измените **FriendlyName** свойство **FactInternetSalesReason** таблицу `InternetSalesReason`.  
...
9. Просмотрите типы данных для столбцов в `InternetSalesReason` таблицы.  
...
3.  В **новую группу мер** выберите `InternetSalesReason` в **выберите таблицу из представления источника данных** , а затем нажмите **ОК**.  
...
9. На странице **Выбор атрибутов измерения** атрибут **Sales Reason Key** автоматически выбран, поскольку он является ключевым. Установите флажок рядом с **Sales Reason Reason Type** атрибут, измените ее имя на `Sales Reason Type`, а затем нажмите кнопку **Далее**.  
...
12. В **атрибуты** область конструктора измерений для **Sales Reason** измерения, выберите **Sales Reason Key**и затем измените **имя**в окне свойств для `Sales Reason.`  
  
13. В **иерархий** область конструктора измерений для создания **Sales Reasons** пользовательскую иерархию, которая содержит `Sales Reason Type` уровне и **Sales Reason** уровня в Этот порядок.  
  
14. В окне свойств укажите `All Sales Reasons` как значение для **AllMemberName** иерархии Sales Reason.  
...
6.  Раскройте каждый из элементов `Sales Reason Type` уровень, чтобы просмотреть значения доллара, связанные с каждой из причин в их покупки Заказчик из штата Квинсленд [!INCLUDE[ssSampleDBCoShort](../includes/sssampledbcoshort-md.md)] продукта через Интернет.  
     ...
     ![И фильтрация данных панели конструктора кубов](../../2014/tutorials/media/l5-many-to-many-5.gif "и фильтрация данных панели конструктора кубов")  
