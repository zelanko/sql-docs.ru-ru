...
# <a name="defining-the-unknown-member-and-null-processing-properties"></a>Определение свойств Unknown Member и Null Processing
 ...
 В задачах этого раздела будут добавлены атрибуты категории и подкатегории товара в измерение Product из таблиц, связанных по схеме «снежинка», которые, в свою очередь, будут добавлены в представление источника данных [!INCLUDE[ssSampleDBCoShort](../includes/sssampledbcoshort-md.md)] DW. Затем необходимо включить свойство **UnknownMember** для измерения Product, указать `Assembly Components` в качестве значения свойства **UnknownMemberName** , связать атрибуты `Subcategory` и `Category` с атрибутом Product Name, а затем определить пользовательский элемент Обработка ошибок для ключевого атрибута элемента, связывающего таблицы по схеме «снежинка».  
...
5.  Перейдите на вкладку **браузер** , убедитесь в том, что в списке **Иерархия** выбран пункт **линии модели продукта** , а затем разверните узел `All Products`.  
...
7.  Убедитесь, что в списке **Иерархия** отображаются **линии модели продукта** , разверните `All Products`, а затем разверните узел **компоненты**.  
...
8.  В списке **Иерархия** выберите **категории продуктов** , разверните `All Products`, а затем — **компоненты**.  
 ...
 Чтобы изменить поведение, упомянутое в предыдущей задаче, необходимо включить свойство **UnknownMember** измерения Products, задать значение для свойства **UnknownMemberName** , задать свойство **NullProcessing** для `Subcategory` и  **Атрибуты имени модели** для **UnknownMember**, определите `Category` атрибут в качестве связанного атрибута `Subcategory` атрибута, а затем определите атрибут « **линейка продукта** » в качестве связанного атрибута атрибута « **имя модели** ». В результате выполнения этих действий службы [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] станут использовать значение имени неизвестного элемента для товаров, не имеющих значений в столбце **SubcategoryKey** , как будет показано в следующей задаче.  
...
4.  На диаграмме щелкните правой кнопкой мыши атрибут `Subcategory` и выберите пункт **создать связь атрибутов**.  
  
5.  В диалоговом окне **Создание связи атрибутов** **атрибут Source** имеет значение `Subcategory`. Задайте для параметра **связанный атрибут** значение `Category`. Оставьте для типа связи значение **Гибкая**.  
...
12. Установите для свойства **NullProcessing** значение **UnknownMember**.  
     ...
     Из-за этих изменений, когда [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] встречает значение NULL для атрибута `Subcategory` или атрибута **имени модели** во время обработки, значение неизвестного элемента будет заменено значением ключа, а пользовательские иерархии будут созданы работает.  
...
4.  Разверните элемент `Assembly Components` уровня `Category`, а затем разверните элемент `Assembly Components` уровня `Subcategory`.  
