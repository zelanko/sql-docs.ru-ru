...
5.  Нажмите кнопку **браузера** убедитесь, что **линии моделей товаров** выбран в **иерархии** списка, а затем разверните `All Products`.  
...
9. В **атрибуты** область, измените имя нового атрибута на `Category`.  
...
13. В **атрибуты** область, измените имя нового атрибута на `Subcategory`.  
...
17. Укажите `All Products` как значение для **AllMemberName** свойства пользовательской иерархии категорий продуктов.  
...
2.  В **свойства** измените **UnknownMember** свойства **Visible**и измените значение для **UnknownMemberName**свойства `Assembly Components`.  
...
4.  На диаграмме щелкните правой кнопкой мыши `Subcategory` атрибута, а затем выберите **новую связь атрибутов**.  
  
5.  В **Создание связи атрибутов** диалоговом **исходный атрибут** — `Subcategory`. Задать **связанный атрибут** для `Category`. Оставьте для типа связи значение **Гибкая**.  
...
12. Установите для свойства **NullProcessing** значение **UnknownMember**.  
     ...
     После внесения этих изменений при [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] встретится значение null для `Subcategory` атрибута или **имя модели** во время обработки, значение неизвестного элемента будет замещено на значение ключа и пользовательские иерархии будут созданы правильно.  
...
3.  Убедитесь, что **категории продуктов** выбран в **иерархии** списка, а затем разверните `All Products`.  
  
     Обратите внимание, что элемент Assembly Components отображается в качестве нового элемента на уровне категории.  
  
4.  Разверните `Assembly Components` членом `Category` уровня, а затем разверните `Assembly Components` членом `Subcategory` уровне.  
  
     Обратите внимание, что все компоненты сборки отображаются на уровне **Product Name** , как показано на рисунке ниже.  
  
     ![Уровню имени товара компонентами сборки](../../2014/tutorials/media/l4-assemblycomponents-1.gif "уровень название продукта, компонентами сборки")  
