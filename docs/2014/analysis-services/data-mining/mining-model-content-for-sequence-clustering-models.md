...
## <a name="understanding-the-structure-of-a-sequence-clustering-model"></a>Основные сведения о структуре модели кластеризации последовательностей  
 ...
 ![Структура модели кластеризации последовательностей](../media/modelcontent-seqclust.gif "Структура модели кластеризации последовательностей")  
 ...
 Описание переходов последовательностей и состояний с примерами см. в разделе [Microsoft Sequence Clustering Algorithm](microsoft-sequence-clustering-algorithm.md).  
 ...
 **Корневой элемент модели** Значение кратности равно количеству кластеров плюс один. Дополнительные сведения см. в разделе [Количество элементов](#bkmk_cardinality).  
  
 **Узлы кластера** Количество элементов всегда равно 1, так как каждый кластер имеет один дочерний узел, содержащий список последовательностей в кластере.  
  
 **Узлы последовательности** Количество элементов указывает число переходов, которые включены в этот кластер. Например, количество элементов узла последовательности для корневого элемента модели показывает, сколько переходов было обнаружено во всей модели.  
 ...
 NODE_PROBABILITY  
 **Корневой элемент модели** Всегда равно 0.  
  
 **Узлы кластера** Скорректированная вероятность кластера в модели. Сумма скорректированных вероятностей не составляет 1, поскольку метод кластеризации, применяемый в кластеризации последовательностей, допускает частичное включение в несколько кластеров.  
  
 **Узлы последовательности** Всегда равно 0.  
  
 **Узлы перехода** Всегда равно 0.  
 ...
 **Узлы кластера** Необработанная поддержка для кластера, то есть количество обучающих вариантов, которые вносят обращения в этот кластер.  
  
 **Узлы последовательности** Всегда равно 0.  
  
 **Узлы перехода** Процент вариантов в кластере, представляющих конкретный переход. Может быть значением, большим или равным 0. Вычисляется путем умножения исходного несущего множества для узла кластера на вероятность этого кластера.  
...
|Transitions|Нет дочерних узлов|Перечислены переходы для первого связанного состояния.<br /><br /> Несущее множество имеет скорректированное значение, равное числу вариантов, которые участвуют в каждом переходе. Значение вероятности является скорректированным и представляется в виде процентного отношения.|  
...
###  <a name="bkmk_NODEDIST"></a>Таблица NODE_DISTRIBUTION  
 ...
 В таблицу переходов всегда добавляется отдельная строка, представляющая возможные значения `Missing`. Сведения о том, что `Missing` означает значение, а также о том, как оно влияет на вычисления, см. в разделе [отсутствующие значения &#40;Analysis Services-&#41;интеллектуального анализа данных ](missing-values-analysis-services-data-mining.md).  
...
###  <a name="bkmk_cardinality"></a>Количества элементов  
