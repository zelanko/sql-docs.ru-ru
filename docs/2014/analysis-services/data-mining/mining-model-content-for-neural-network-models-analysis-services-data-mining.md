...
# <a name="mining-model-content-for-neural-network-models-analysis-services---data-mining"></a>Содержимое моделей интеллектуального анализа данных для моделей нейронных сетей (службы Analysis Services — интеллектуальный анализ данных)
  В этом разделе приведено описание содержимого модели интеллектуального анализа данных, характерного для моделей, в которых используется алгоритм нейронной сети (Майкрософт). Объяснение способов интерпретации статистики и описание структуры, общей для всех типов моделей, а также общие определения терминов, связанных с содержимым моделей интеллектуального анализа данных, см. в разделе [Mining Model Content &#40;Analysis Services - Data Mining&#41;](mining-model-content-analysis-services-data-mining.md).  
...
##  <a name="bkmk_NodeDistTable"></a>Анализ данных в таблице NODE_DISTRIBUTION  
 ...
 **Дискретный числовой атрибут:** Входной узел хранит имя атрибута и значение, которое может быть диапазоном или указанным значением. Все значения представляются выражениями, например «77.4 - 87.4» или «< 64.0» для значения атрибута [Time Per Issue]. В таблице NODE_DISTRIBUTION для каждого узла приводится только текущее значение атрибута.  
 ...
 **Дискретный атрибут:** Последние две строки таблицы NODE_DISTRIBUTION содержат коэффициент для узла в целом, а также текущее значение атрибута.  
  
 **Дискретный числовой атрибут:** Аналогично дискретным атрибутам, за исключением того, что значение атрибута является диапазоном значений.  
