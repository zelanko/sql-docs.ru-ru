...
###  <a name="BKMK_Relational"></a>Реляционные модели интеллектуального анализа данных и OLAP  
...
-   **Автоматическое определение типов данных**. мастер проверит уникальность и распределение значений столбцов, а затем порекомендует лучший тип данных и предложит тип использования для данных. Эти предложения можно отвергнуть и выбрать значения из списка.  
  
-   **Предложения для переменных**: можно щелкнуть диалоговое окно и запустить анализатор, который вычисляет корреляции по столбцам, включенным в модель, и определяет, являются ли какие-либо столбцы, вероятнее всего, прогностические факторы от атрибута результата, учитывая конфигурацию модели на данный момент. Эти предложения можно отвергнуть и ввести другие значения.  
  
-   **Выбор компонентов**. Большинство алгоритмов автоматически определяет столбцы, которые хорошо прогностические факторы, и используют их в первую очередь. В столбцах с большим количеством значений используется функция *выбор компонентов* для уменьшения количества элементов связи данных и повышения вероятности получения значимой закономерности. На выбор компонентов можно повлиять с помощью параметров модели.  
  
-   **Автоматическое фрагментирование Куба**. Если модель интеллектуального анализа данных основана на источнике данных OLAP, возможность среза модели с помощью атрибутов куба предоставляется автоматически. Это удобно, когда модели создаются на основе подмножеств данных куба.  
...
## <a name="related-content"></a>См. также  
 ...
 [Отсутствующие значения &#40;Analysis Services интеллектуального анализа данных&#41;](missing-values-analysis-services-data-mining.md)  
