...
> [!NOTE]  
>  Независимо от типа данных, эти операторы не может применяться к столбцу с типом `Discrete`, `Discretized`, или `Key`.  
...
###  <a name="bkmk_Ex7"></a> Пример 7. Сложные сочетания фильтров  
 Сценарий для этой модели напоминает сценарий из примера 4, но здесь он более сложный. Вложенную таблицу, **ProductsOnSale**, имеет условие фильтра `(OnSale)` это значит, что значение **OnSale** должно быть `true` для продукта, перечисленного в **ProductName**. Здесь **OnSale** — это столбец структуры.  
  
 Во второй части фильтра, для **ProductsNotOnSale**, повторяет этот синтаксис, но фильтрация по продуктам, для которого значение **OnSale** — `not true``(!OnSale)`.  
...
###  <a name="bkmk_Ex8"></a> Пример 8. Фильтрация по датам  
 ...
 Однако нельзя задать фильтр по столбцу дат в модели временных рядов в том случае, если столбец дат, используемый для фильтрации, также является ключевым столбцом для модели. Это происходит потому, что в модели временных рядов и модели кластеризации последовательностей, столбец дат может обрабатываться как тип `KeyTime` или `KeySequence`.  
