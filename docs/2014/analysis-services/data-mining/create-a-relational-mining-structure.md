...
## <a name="requirements"></a>Требования  
 Сначала требуется существующий источник данных. Если источник не существует, его можно настроить в конструкторе источников данных. Дополнительные сведения см. в разделе [Create a Data Source &#40;SSAS Multidimensional&#41;](../multidimensional-models/create-a-data-source-ssas-multidimensional.md).  
...
##  <a name="BKMK_Relational_Structure"></a>Общие сведения о процессе  
...
1.  **Выбор метода определения**: здесь можно выбрать тип источника данных и выбрать **реляционную базу данных или хранилище данных**.  
...
3.  **Выберите представление источника данных**: выберите представление источников данных для использования при обучении модели. Представление источника данных также может содержать данные, используемые для тестирования, и не связанные с анализом данные. Можно выбрать, какие данные будут фактически использоваться в структуре и в модели. Позднее к данным можно применить фильтры.  
  
4.  **Укажите типы таблиц**: выберите таблицу, содержащую варианты, используемые для анализа. Для некоторых наборов данных, особенно для тех, которые используются в построении моделей покупательского поведения, также можно включить связанную таблицу для использования в качестве вложенной.  
...
5.  **Укажите обучающие данные**: на этой странице выберите в качестве *таблицы вариантов*, которая является таблицей, содержащей наиболее важные данные для анализа.  
...
6.  **Укажите содержимое столбцов и типы данных**: для каждого столбца, используемого в структуре, необходимо выбрать *тип данных* и *тип содержимого*.  
...
7.  **Создание проверочного набора**. на этой странице можно указать мастеру, какой объем данных следует настроить для использования при тестировании модели. Если данные будут поддерживать несколько моделей, то разумно создать набор контрольных данных, чтобы тестировать все модели по одним и тем же данным.  
  
     Дополнительные сведения см. в разделе [Testing and Validation &#40;Data Mining&#41;](testing-and-validation-data-mining.md).  
  
8.  **Завершение работы мастера**. на этой странице вы присваиваете имя новой структуре интеллектуального анализа данных и связанной с ней модели интеллектуального анализа данных, а также сохраняете структуру и модель.  
...
##  <a name="BKMK_ChooseRelData"></a>Выбор реляционных данных  
 ...
 Если таблицы включают связи типа «один ко многим» (например, имеется несколько записей о покупках для каждого клиента, который необходимо проанализировать), можно добавить обе таблицы, а затем использовать одну таблицу в качестве таблицы вариантов, связывая данные на стороне «многие» в виде вложенная таблица.  
...
##  <a name="bkmk_Holdout"></a>Почему и как разделить данные на обучающие и проверочные наборы  
...
##  <a name="BKMK_DrillThru"></a>Почему и как включить детализацию  
