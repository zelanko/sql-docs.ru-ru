...
## <a name="making-predictions-using-the-model"></a>Создание прогнозов с помощью модели  
 ...
 Дополнительные сведения о создании прогнозирующих запросов с помощью расширений интеллектуального анализа данных см. в разделе [интерфейсы запросов интеллектуального анализа данных](data-mining-query-tools.md).  
  
 [Вернуться в начало](#bkmk_top2)  
  
###  <a name="bkmk_Query8"></a> Образец запроса 8. Прогноз результатов на основании модели кластеризации  
 Если создаваемая модель кластеризации содержит прогнозируемый атрибут, ее можно использовать для прогноза результатов. Однако модель будет обрабатывать прогнозируемый атрибут по-разному в зависимости от ли значение прогнозируемого столбца `Predict` или `PredictOnly`. Если для использования столбца задано `Predict`, значения для этого атрибута добавляются в модель кластеризации и появляются в виде атрибутов в законченной модели. Однако если для использования столбца задано значение `PredictOnly`, то его значения не используются для создания кластеров. Вместо этого по завершении режима алгоритм кластеризации создает новые значения для `PredictOnly` атрибут на основе кластеров, к которым принадлежат варианты.  
