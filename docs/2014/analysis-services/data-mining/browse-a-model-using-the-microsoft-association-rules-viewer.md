...
###  <a name="BKMK_Itemsets"></a> Наборы элементов  
 На вкладке **Наборы элементов** отображается список наборов элементов, которые по определению модели часто используются совместно. На вкладке представлена таблица со следующими столбцами: **Поддержка**, **размер**, и **набора элементов**. Дополнительные сведения о поддержке см. в разделе [Microsoft Association Algorithm](microsoft-association-algorithm.md). В столбце **Размер** отображается количество элементов в наборе. Столбец **Набор элементов** представляет фактический набор элементов, обнаруженный моделью. Формат набора элементов можно изменять с помощью списка **Показать** , позволяющего установить следующие параметры:  
