...
|Алгоритм линейной регрессии (Майкрософт)|Поддерживается.<br /><br /> Тем не менее, поскольку модель одного узла, `All`, детализация возвращает все обучающие варианты для модели. Если задан большой обучающий набор, то загрузка результатов может занять много времени.|  
