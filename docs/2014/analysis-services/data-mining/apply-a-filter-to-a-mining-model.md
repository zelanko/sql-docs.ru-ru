...
8.  Щелкните текстовое поле **Значение** и введите значение в поле.  
     ...
     Например, выберите `Income` в качестве столбца, выберите оператор "больше" (>), а затем введите `30000`.  
...
11. Выберите оператор и введите значение, как описано в шагах 7 и 8.  
     ...
     Например, выберите `Income` в качестве столбца еще раз, выберите оператор «меньше» (<), а затем введите `40000`.  
...
1.  В диалоговом окне>фильтр модели щелкните пустую строку в сетке в **столбце структура интеллектуального анализа данных**. ** \<**  
...
4.  Щелкните поле **значение** и нажмите кнопку **(...)** , чтобы создать выражение.  
     ...
     Откроется диалоговое окно " ** \<имя>фильтра** ". Это диалоговое окно позволяет задавать условия только для текущей таблицы, которая в данном случае является вложенной таблицей.  
...
1.  При необходимости добавьте дополнительные условия, объединив условия, `AND` выбрав или `OR` в поле или **/или** в левой части сетки **условий** . [!INCLUDE[clickOK](../../includes/clickok-md.md)]  
