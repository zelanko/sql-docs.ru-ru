...
|REGRESSOR|Указывает, что столбец содержит непрерывные численные значения, которые при анализе нужно обрабатывать как потенциальные независимые переменные.<br /><br /> Примечание. Пометка в качестве регрессора столбец не гарантирует, что столбец будет использоваться в качестве регрессора в окончательной модели.<br /><br /> Применяется к столбцам модели интеллектуального анализа данных.|  
