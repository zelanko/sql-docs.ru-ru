...
> [!NOTE]  
>  `Cyclical` и `Ordered` поддерживаемые типы содержимого, но алгоритм обрабатывает их как дискретные значения и специальная обработка не выполняется.  
