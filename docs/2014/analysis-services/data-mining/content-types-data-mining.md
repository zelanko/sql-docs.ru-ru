...
## <a name="discrete"></a>Дискретный  
 *Дискретный* означает, что столбец содержит конечное число значений без Continuum между значениями. Например, столбец с обозначением пола представляет собой типичный столбец для дискретного атрибута, в котором данные представляют конкретное количество категорий.  
 ...
 *Дискретизация* — это процесс помещения значений непрерывного набора данных в контейнеры, чтобы существовало ограниченное количество возможных значений. Дискретизировать можно только числовые данные.  
