...
##  <a name="bkmk_Top"></a>Основные сведения о структуре модели дерева принятия решений  
...
##  <a name="bkmk_ModelContent"></a>Содержимое модели для модели дерева принятия решений  
 ...
 **Родительский узел** Указывает число смоделированных прогнозируемых атрибутов. Для каждого прогнозируемого атрибута создается дерево.  
 ...
 **Внутренний узел** Содержит агрегированные статистические данные для конечных узлов.  
  
 **Конечный узел** Содержит поддержку и вероятность для прогнозируемых результатов с учетом всех условий в пути, ведущих к текущему конечному узлу.  
  
 **Узел регрессии** Содержит регрессионную формулу, представляющую связь между входными данными и прогнозируемым атрибутом.  
...
###  <a name="NodeRule"></a>Правило узла и маржинальное правило  
...
|||Age < 30 и Gender = Female|10|10/40 = .25|10/100 = .10|  
 ...
 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]использует вероятности для каждого узла, чтобы сравнить хранимую вероятность с предыдущей вероятностью, чтобы определить, указывает ли путь от родителя к дочернему узлу на сильное определение.  
