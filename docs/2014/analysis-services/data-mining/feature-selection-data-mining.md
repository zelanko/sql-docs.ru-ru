...
ms.locfileid: "36099915"
---
# <a name="feature-selection-data-mining"></a>Выбор компонентов (интеллектуальный анализ данных)
  *Выбор компонентов* — это термин, который обычно используется в интеллектуальном анализе данных для описания средств и методик, доступных для сокращения входные данные для управляемый размер для обработки и анализа. Выбор компонентов предполагает не только *операция сокращения количества элементов*, означающее налагающий произвольного или определенного уменьшения количество атрибутов, которые могут быть использованы при создании модели, но также и Выбор атрибутов, то есть что аналитик либо средство моделирования активно выбирает или отбрасывает атрибуты в зависимости от их полезности для анализа.  
...
#### <a name="interestingness-score"></a>Оценка интересности  
 Характеристика представляет интерес, если она предоставляет полезный фрагмент информации. Так как определение Каков полезные изменяется в зависимости от сценария, индустрии интеллектуального анализа данных разработаны различные способы измерения *интересности*. Например *нестандартные* может оказаться интересной при обнаружении выбросов, но способность различать между собой тесно связанные элементы или *распознавать веса*, возможно, больший интерес для Классификация.  
  
 Мера интересности, которая используется в SQL Server Analysis Services — *на основе энтропии*, что означает, что атрибуты со случайными распределениями имеют более высокую энтропию и менее значительный прирост информации; таким образом, меньше, такие атрибуты интерес. Энтропия, относящаяся к любому конкретному атрибуту, сравнивается с энтропией всех других атрибутов следующим образом:  
