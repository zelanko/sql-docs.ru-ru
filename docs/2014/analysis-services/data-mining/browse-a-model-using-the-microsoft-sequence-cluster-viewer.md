...
###  <a name="BKMK_Diagram"></a> Диаграмма кластеров  
 ...
 По умолчанию тень обозначает заполнение кластера. С помощью **Переменнаязаливки** и **состояние** параметры, можно выбрать, что атрибут и состояние пара заливка представляет. Чем темнее заливка, тем сильнее распределение атрибута для конкретного состояния. Распределение уменьшается по мере того, как заливка становится светлее.  
