...
|ATTRIBUTE_NAME|ATRIBUTE_VALUE|ПОДДЕРЖКА|PROBABILITY|дисперсия|VALUE_TYPE|  
|---------------------|---------------------|-------------|-----------------|--------------|-----------------|  
|Age|Missing|0|0|0|1 (отсутствует)|  
 ...
 **Узлы кластера** Обычно 000.  
  
 NODE_DESCRIPTION  
 Описание узла.  
  
 **Родительский узел** Всегда **(все)**.  
  
 **Узлы кластера** Разделенный запятыми список основных атрибутов, которые отличают кластер от других кластеров.  
 ...
 NODE_PROBABILITY  
 Вероятность, связанная с этим узлом. **Родительский узел** Всегда равно 1.  
  
 **Узлы кластера** Вероятность представляет составную вероятность атрибутов с некоторыми корректировками в зависимости от алгоритма, используемого для создания модели кластеризации.  
 ...
 **Родительский узел** См. статью введение в этот раздел.  
  
 **Узлы кластера** Представляет распределение атрибутов и значений для вариантов, включаемых в этот кластер.  
 ...
 **Примечание** . Если модель использует кластеризацию K-средних, каждый вариант может принадлежать только к одному кластеру. Но если в модели используется кластеризация по методу EM, то каждый вариант может принадлежать к различным кластерам и варианту присваивается значение взвешенного расстояния для каждого кластера, к которому он принадлежит. Поэтому для моделей EM сумма несущих множеств отдельных кластеров больше несущего множества всей модели.  
 ...
 **Родительский узел** Оценка Байеса Information критерия (БИК) для модели кластеризации.  
