 ...
 **MODEL_EXISTENCE_ONLY**  
 Указывает, что столбец должен рассматриваться как имеющий два состояния: `Missing` и `Existing`. Если значение равно `NULL`, рассматривается как недостающее. Флаг MODEL_EXISTENCE_ONLY применяется к прогнозируемому атрибуту и поддерживается большинством алгоритмов.  
  
 Фактически установка флаг MODEL_EXISTENCE_ONLY `True` меняет представление значений так, что существует только два состояния: `Missing` и `Existing`. Все неотсутствующие состояния объединяются в единый `Existing` значение.  
  
 Этот флаг моделирования, как правило, используется в атрибутах, для которых состояние `NULL` имеет неявный смысл, а явно заданное значение состояния `NOT NULL` может не быть столь важным, как факт, что этот столбец имеет некоторое значение. Например, может быть столбце [DateContractSigned] `NULL` Если контракт не подписан и `NOT NULL` Если контракт подписан. Поэтому, если модель предназначена для прогнозирования того, контракт будет подписан, можно использовать флаг MODEL_EXISTENCE_ONLY пропуска точного значения даты в `NOT NULL` вариантов и проведения различия только между случаях, когда контракт `Missing` или `Existing`.  
