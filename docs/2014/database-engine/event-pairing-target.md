...
|max_orphans||Указывает общее количество непарных событий, которые будут собраны в целевом объекте. После достижения этого предела непарные события удаляются по принципу «первым пришел, первым ушел» (FIFO). По умолчанию равно 10,000.|  
 ...
 Цель «Попарное разбиение событий» сериализует непарные события в формате XML. Этот формат не соответствует ни одной схеме. Он содержит только два типа элементов. Непарный элемент>является корнем, за которым следует один. ** \<** элемент>события для каждого непарного события, которое в данный момент отслеживается. ** \<** Элемент ** \<>событий** содержит один атрибут, который содержит имя непарного события.  
