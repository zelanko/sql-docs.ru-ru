---
title: Попарное разбиение событий | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
|max_orphans||Указывает общее количество непарных событий, которые будут собраны в целевом объекте. После достижения этого предела непарные события удаляются по принципу «первым пришел, первым ушел» (FIFO). По умолчанию равно 10,000.|  
 ...
 Цель «Попарное разбиение событий» сериализует непарные события в формате XML. Этот формат не соответствует ни одной схеме. Он содержит только два типа элементов. **\<Непарные >** элемент является корневым, за которым следует один. **\<событие >** элемент для каждому обнаруженному непарному событию. **\<Событий >** элемент содержит один атрибут, содержащий имя непарного события.  
