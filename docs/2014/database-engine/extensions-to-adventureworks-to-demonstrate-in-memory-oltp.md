---
title: Расширения AdventureWorks для демонстрации In-Memory OLTP | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
## <a name="overview"></a>Обзор  
 В этом образце показана новая [!INCLUDE[hek_2](../includes/hek-2-md.md)] функции, которая входит в состав объекта [!INCLUDE[ssSQL14](../includes/sssql14-md.md)]. Он показывает новых таблиц, оптимизированных для памяти и скомпилированных в собственном коде хранимые процедуры и может использоваться, чтобы продемонстрировать преимущества производительности [!INCLUDE[hek_2](../includes/hek-2-md.md)].  
...
-   [!INCLUDE[ssSQL14](../includes/sssql14-md.md)] RTM — выпуски Evaluation, Developer или Enterprise  
...
##  <a name="InstallingtheIn-MemoryOLTPsamplebasedonAdventureWorks"></a> Установка образца [!INCLUDE[hek_2](../includes/hek-2-md.md)], основанного на AdventureWorks  
...
1.  Скачайте архив с полной резервной копией базы данных AdventureWorks2014:  
    ...
    1.  Откройте следующий адрес: [ http://msftdbprodsamples.codeplex.com/downloads/get/880661 ](http://msftdbprodsamples.codeplex.com/downloads/get/880661).  
...
5.  Загрузите образец скрипта "[!INCLUDE[ssSQL14](../includes/sssql14-md.md)] RTM [!INCLUDE[hek_2](../includes/hek-2-md.md)] Sample.sql" из [SQL Server 2014 RTM In-Memory OLTP Sample](http://go.microsoft.com/fwlink/?LinkID=396372) в локальную папку.  
  
6.  Измените значение переменной «checkpoint_files_location» в скрипте «[!INCLUDE[ssSQL14](../includes/sssql14-md.md)] RTM [!INCLUDE[hek_2](../includes/hek-2-md.md)] Sample.sql», чтобы указать на целевое расположение для файлов контрольных точек [!INCLUDE[hek_2](../includes/hek-2-md.md)]. Файлы контрольных точек следует разместить на диске с высокой производительностью последовательного ввода-вывода.  
  
     Обновите значение переменной "database_name", чтобы указать на базу данных AdventureWorks2014.  
  
    1.  В имени пути обязательно укажите символ обратной косой черты «\»  
...
7.  Запустите образец скрипта на выполнение одним из двух способов.  
    ...
    2.  В среде Management Studio:  
        ...
        4.  Чтобы запустить скрипт, нажмите кнопку «Выполнить»  
...
-   *Параметры сортировки BIN2* — столбцы Name и ProductNumber включены в ключи индекса и таким образом параметры сортировки BIN2 в [!INCLUDE[ssSQL14](../includes/sssql14-md.md)]. Здесь же предполагается, что приложение не опирается на особенности параметров сортировки, например чувствительность к регистру.  
...
##  <a name="PerformanceMeasurementsusingtheDemoWorkload"></a> Измерение производительности с помощью демонстрационной рабочей нагрузки  
...
-   -E использование проверки подлинности Windows для подключения (по умолчанию); при использовании проверки подлинности [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] задайте параметры -U и -P, чтобы указать имя пользователя и пароль соответственно  
...
#### <a name="functional-validation-of-the-workload"></a>Функциональная проверка рабочей нагрузки  
 Чтобы проверить работу, начнем с теста образца, задействовано 10 параллельных соединений и 5 итераций со вставкой 10 * 5 \* 20 = 1000 заказов на продажу.  
