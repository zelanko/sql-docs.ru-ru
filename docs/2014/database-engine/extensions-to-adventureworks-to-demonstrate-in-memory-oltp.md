...
-   [Описание образцов таблиц и процедур](#Descriptionofthesampletablesandprocedures) — Сюда входят описания таблиц и процедур, добавленных в AdventureWorks примером [!INCLUDE[hek_2](../includes/hek-2-md.md)] образца, а также вопросы переноса некоторые из исходной таблицы AdventureWorks оптимизированные для памяти  
...
5.  Загрузите образец скрипта "[!INCLUDE[ssSQL14](../includes/sssql14-md.md)] RTM [!INCLUDE[hek_2](../includes/hek-2-md.md)] Sample.sql" из [образца SQL Server 2014 RTM In-Memory OLTP](https://go.microsoft.com/fwlink/?LinkID=396372) в локальную папку.  
  
6.  Обновите значение для переменной «checkpoint_files_location» в скрипте "[!INCLUDE[ssSQL14](../includes/sssql14-md.md)] RTM [!INCLUDE[hek_2](../includes/hek-2-md.md)] Sample.sql", чтобы указать целевое расположение для [!INCLUDE[hek_2](../includes/hek-2-md.md)] файлы контрольных точек. Файлы контрольных точек следует разместить на диске с высокой производительностью последовательного ввода-вывода.  
  
     Обновите значение переменной "database_name", чтобы указать на базу данных AdventureWorks2014.  
  
    1.  Убедитесь, что обратная косая черта "\' как часть имени пути  
...
7.  Запустите образец скрипта на выполнение одним из двух способов.  
    ...
    2.  В среде Management Studio:  
        ...
        3.  Включить режим SQLCMD, щелкнув «Запрос -> режим SQLCMD»  
  
        4.  Нажмите кнопку «Выполнить», чтобы запустить сценарий  
...
-   -E использовать проверку подлинности Windows для подключения (по умолчанию); Если вы используете [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] проверки подлинности, используйте параметры- и -P, чтобы указать имя пользователя и пароль, соответственно  
...
-   Число одновременно выполняемых транзакций: При выполнении рабочей нагрузки в одном потоке, рост производительности [!INCLUDE[hek_2](../includes/hek-2-md.md)] скорее всего будет меньше, чем в два раза. Конфликты кратковременных блокировок становятся большой проблемой только при высоком уровне параллелизма.  
  
-   Небольшое количество ядер, доступных для [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]: Это означает, что будет низкий уровень параллелизма в системе, так как может существовать только как число параллельно исполняемых транзакций ядер, доступных для SQL.  
...
-   Скорость диска с журналами: Если на диск журнала не справляется с пропускной способностью транзакций в системе, рабочая нагрузка успевать журнала операций ввода-ВЫВОДА. Несмотря на то что в [!INCLUDE[hek_2](../includes/hek-2-md.md)]ведение журнала стало более эффективным, если ввод-вывод журнала является узким местом, потенциальный прирост производительности будет ограничен.  
...
|Product_inmem|124|12352|  
 ...
 Как вы видите таблицы приведены достаточно мала. SalesOrderHeader_inmem занимает примерно 7 МБ, а SalesOrderDetail_inmem — примерно 15 МБ.  
