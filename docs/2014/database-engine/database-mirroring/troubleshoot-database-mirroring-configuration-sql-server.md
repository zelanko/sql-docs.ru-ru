...
##  <a name="NetworkAccess"></a>Доступ к сети  
...
>  Если при создании зеркальной базы данных переносятся файлы базы данных, то последующее добавление файлов к базе данных не всегда возможно без прекращения зеркального отображения.  
 ...
 Дополнительные сведения см. в статье [Подготовка зеркальной базы данных к зеркальному отображению (SQL Server)](prepare-a-mirror-database-for-mirroring-sql-server.md).  
  
##  <a name="FailedCreateFileOp"></a>Сбой операции создания файла  
...
##  <a name="StartDbm"></a>Запуск зеркального отображения с помощью Transact-SQL  
...
##  <a name="CrossDbTxns"></a>Транзакции между базами данных  
