...
> [!NOTE]  
>   Убедитесь, что выполняются все [предварительные условия для зеркального отображения базы данных](prerequisites-restrictions-and-recommendations-for-database-mirroring.md).  
...
5.  Убедитесь, что роль выбрана правильно. На каждом из экземпляров сервера выполните следующую инструкцию [!INCLUDE[tsql](../../includes/tsql-md.md)].  
    ...
    ```  
     ...
     Дополнительные сведения см. в статье [sys.database_mirroring_endpoints (Transact-SQL)](/sql/relational-databases/system-catalog-views/sys-database-mirroring-endpoints-transact-sql).  
...
##  <a name="network-access"></a><a name="NetworkAccess"></a>Доступ к сети  
...
##  <a name="mirror-database-preparation"></a><a name="MirrorDbPrep"></a>Подготовка зеркальной базы данных  
...
##  <a name="starting-mirroring-by-using-transact-sql"></a><a name="StartDbm"></a>Запуск зеркального отображения с помощью Transact-SQL  
...
##  <a name="cross-database-transactions"></a><a name="CrossDbTxns"></a>Транзакции между базами данных  
