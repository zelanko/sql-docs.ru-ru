...
#### <a name="server-attribute"></a>Атрибут Server  
 Строка подключения должна содержать `Server` атрибут, предоставляющий имя изначального участника, которое должно идентифицировать текущий экземпляр основного сервера.  
...
> [!IMPORTANT]  
>  Указание префикса протокола с `Server` атрибут (`Server=tcp:`*\<имя_сервера >*) несовместим с **сети** и указание протокола в Оба окружение будет скорее всего, приведет к ошибке. Таким образом, рекомендуется, задать в строке подключения с помощью протокола **сети** атрибута и укажите имя сервера в `Server` атрибут (`"Network=dbmssocn; Server=`*\<servername >*`"`).  
