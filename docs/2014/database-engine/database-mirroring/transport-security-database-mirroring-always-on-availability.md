...
> [!NOTE]  
>  Пример установки сеанса для зеркального отображения базы данных с помощью проверки подлинности Windows см. в разделе [Пример. Настройка зеркального отображения базы данных с помощью проверки подлинности Windows (язык Transact-SQL)](example-setting-up-database-mirroring-using-windows-authentication-transact-sql.md).  
...
|---------------------|-----------------|  
|RC4;|Указывает на то, что конечная точка должна использовать алгоритм RC4. Это значение используется по умолчанию.<br /><br /> Примечание. алгоритм RC4 является устаревшим. [!INCLUDE[ssNoteDepFutureDontUse](../../includes/ssnotedepfuturedontuse-md.md)]Рекомендуется использовать алгоритм AES.|  
...
## <a name="see-also"></a>См. также:  
 ...
 [УДАЛИТЬ КОНЕЧную точку &#40;Transact-SQL&#41;](/sql/t-sql/statements/drop-endpoint-transact-sql)   
