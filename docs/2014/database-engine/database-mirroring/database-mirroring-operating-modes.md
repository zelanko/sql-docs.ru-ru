...
###  <a name="when-is-high-performance-mode-appropriate"></a><a name="WhenUseHighPerf"></a>Когда подходит режим высокой производительности?  
...
###  <a name="high-safety-mode-without-automatic-failover"></a><a name="HighSafetyWithOutAutoFailover"></a>Режим высокой безопасности без автоматической отработки отказа  
...
###  <a name="high-safety-mode-with-automatic-failover"></a><a name="HighSafetyWithAutoFailover"></a>Режим высокого уровня безопасности с автоматическим переходом на другой ресурс  
 ...
 Если задан следящий сервер, для сеанса требуется наличие *кворума*— связь по крайней мере с двумя экземплярами сервера, которые позволяют сделать базу данных доступной. Дополнительные сведения см. в разделах [Следящий сервер зеркального отображения базы данных](database-mirroring-witness.md) и [Кворум: как следящий сервер влияет на доступность базы данных (зеркальное отображение базы данных)](quorum-how-a-witness-affects-database-availability-database-mirroring.md).  
...
####  <a name="transaction-safety"></a><a name="TxnSafety"></a>Безопасность транзакций  
...
###  <a name="viewing-the-safety-setting-and-state-of-the-witness"></a><a name="ViewWitness"></a>Просмотр параметров безопасности и состояния следящего сервера  
...
###  <a name="factors-affecting-behavior-on-loss-of-the-principal-server"></a><a name="FactorsOnLossOfPrincipal"></a>Факторы, влияющие на поведение при утрате основного сервера  
