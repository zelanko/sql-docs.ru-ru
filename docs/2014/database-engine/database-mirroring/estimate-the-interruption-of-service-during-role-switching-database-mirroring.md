...
## <a name="error-detection"></a>Определение ошибки  
 Время, в течение которого система заметит ошибку, зависит от типа ошибки; Например, сетевая ошибка Заметится почти мгновенно, а при замечении того, что сервер не отвечает, занимает 10 секунд (с временем ожидания по умолчанию).  
