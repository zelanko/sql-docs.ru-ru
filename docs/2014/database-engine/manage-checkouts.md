---
title: Управление извлечениями | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36193859"
---
# <a name="manage-checkouts"></a>Управление извлечениями
  После того как файл был добавлен к системе управления версиями, для изменения этого файла его нужно извлечь. Когда извлекается файл из управления версиями, поставщик управления версиями создает его последнюю версию на локальном диске и снимает с этого файла атрибут «Только для чтения». В некоторых случаях нужно изменить файл, не извлекая его. Дополнительные сведения об изменении файлов без их извлечения см. в разделе [изменение возвращенных файлов](../../2014/database-engine/edit-checked-in-files.md).  
  
 Можно использовать [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] для извлечения файлов вручную или автоматически. Чтобы извлечь файлы вручную, открыв решение, которое содержит файлы в [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)] среды и выбрав **извлечь** команды. Файлы будут извлекаться автоматически, если настроить среду [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)] для автоматического извлечения файлов.  
