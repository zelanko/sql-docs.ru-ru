---
title: Прослушиватели группы доступности, возможность подключения клиентов и отработка отказа приложений (SQL Server) | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
```  
 ...
 В этом примере строки подключения клиент пытается подключиться к прослушивателю группы доступности `AGListener` через порт 1433 (номер порта также можно не указывать, если прослушиватель группы доступности отслеживает порт 1433).  Строка подключения имеет `ApplicationIntent` свойство `ReadOnly`, что делает эту *строку соединения с намерением чтения*.  Без этого параметра сервер не стал бы пытаться выполнить для этого подключения маршрутизацию только для чтения.  
 ...
 Вы можете обойти маршрутизации только для чтения, не задав подключения свойству намерения приложения значение `ReadOnly` (если оно не указывается значение по умолчанию — `ReadWrite` во время входа) или путем подключения непосредственно к экземпляру SQL Server первичной реплики вместо использования имя прослушивателя группы доступности.  Маршрутизация только для чтения также не выполняется при подключении непосредственно к реплике, доступной только для чтения.  
...
>  Этим параметром рекомендуется пользоваться для соединения с прослушивателем группы доступности как с одной, так и с несколькими подсетями, а также для имени экземпляра отказоустойчивого кластера SQL Server.  При использовании этого параметра задействуется дополнительная оптимизация, даже при работе с одной подсетью.  
 ...
 `MultiSubnetFailover` Подключения параметр работает только с сетевым протоколом TCP и поддерживается только при подключении к прослушивателю группы доступности, а также для любого виртуального сетевого имени, связанного с [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)].  
