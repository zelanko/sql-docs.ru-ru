---
title: Транзакции в таблицах, оптимизированных для памяти | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
#### <a name="regular-processing"></a>Обычная обработка  
 ...
 Операции записи в таблицы на этом этапе транзакции еще невидимы другим транзакциям, за одним исключением: обновления и удаления строк являются видимыми для обновления, а операции удаления в других транзакциях, для обнаружения конфликтов записи.  
...
##### <a name="repeatable-read-validation"></a>Проверка операций чтения с возможностью повторения  
 Если уровень изоляции транзакции — REPEATABLE READ или SERIALIZABLE или таблицы доступны в режиме изоляции REPEATABLE READ или SERIALIZABLE (Дополнительные сведения см. раздел «изоляция отдельных операций» в [транзакции Уровни изоляции](../../2014/database-engine/transaction-isolation-levels.md)), система проверяет, является ли чтение повторяемым. Это означает проверку, что версии строк, считываемых транзакцией, по-прежнему представляют собой действительные версии строк во время логического конца транзакции.  
