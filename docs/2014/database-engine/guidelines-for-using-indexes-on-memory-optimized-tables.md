---
title: Рекомендации по использованию индексов в таблицах, оптимизированных для памяти | Документы Microsoft
ms.custom: ''
ms.date: 03/08/2017
...
-   Некластеризованные индексы, которые создаются для сканирования диапазона и упорядоченного сканирования.  
 ...
 Повторяющиеся ключевые значения всегда делят один и тот же хэш-контейнер.  Если хэш-индекс содержит много повторяющихся ключевых значений, итоговые длинные цепочки хэширования ухудшат производительность.   Конфликты хэша, которые происходят в любом хэш-индексе, в последующем снизит производительность в этом сценарии. По этой причине, если число уникальных ключей индекса по крайней мере 100 раз меньше, чем количество строк, можно уменьшить риск конфликтов хэша, делая число контейнеров (минимум в восемь раз число уникальных ключей индекса см. в разделе [определение Правильного числа контейнеров для хэш-индексы](../../2014/database-engine/determining-the-correct-bucket-count-for-hash-indexes.md) подробнее) или полностью, с помощью некластеризованного индекса можно устранить конфликты хэша.  
...
## <a name="creating-a-memory-optimized-index-code-samples"></a>Создание индекса, оптимизированного для памяти: примеры кода  
