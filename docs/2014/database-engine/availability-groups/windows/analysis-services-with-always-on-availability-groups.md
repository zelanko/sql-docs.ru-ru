...
##  <a name="bkmk_UseSecondary"></a> Контрольный список. Использовать вторичную реплику для операций только для чтения  
...
#### <a name="step-1-configure-access-on-an-availability-replica"></a>Шаг 1. Настройка доступа к реплике доступности  
...
#### <a name="step-1-confirm-the-data-source-connection-is-redirected-to-the-secondary-replica"></a>Шаг 1. Убедитесь, что подключения к источнику данных перенаправляется на вторичную реплику  
...
#### <a name="step-2-perform-a-planned-failover-to-test-the-configuration"></a>Шаг 2. Плановая отработка отказа для тестирования конфигурации  
...
##  <a name="bkmk_whathappens"></a> Что происходит после отработки отказа  
 ...
 При отработке отказа во время обработки в службах Analysis Services в окне выходных данных или файл журнала возникает следующая ошибка: "OLE DB error: Ошибка OLE DB или ODBC: Сбой связи; 08S01; Поставщик TPC: существующее соединение было принудительно завершено удаленным узлом. ; 08S01."  
...
##  <a name="bkmk_writeback"></a> Обратная запись, если используется база данных доступности AlwaysOn  
 ...
 Для поддержки обратной записи требуется клиентское подключение READWRITE. В Excel Если вы пытаетесь выполнить обратную запись через подключение только для чтения, произойдет следующая ошибка: "Не удалось получить данные из внешнего источника данных". "Не удалось получить данные из внешнего источника данных".  
