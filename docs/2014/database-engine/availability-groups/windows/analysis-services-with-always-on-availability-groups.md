...
##  <a name="bkmk_UseSecondary"></a> Контрольный список. Использование вторичной реплики с операциями только для чтения  
...
##  <a name="bkmk_ssasAODB"></a>Создание Analysis Services источника данных с помощью базы данных доступности AlwaysOn  
...
3.  В диспетчере подключений для свойства «Имя сервере» введите имя *прослушивателя группы доступности*, а затем выберите базу данных, доступную в группе.  
     ...
     ![Папка доступности AlwaysOn в Management Studio](../../media/ssas-alwaysoninfoinssms.png "Папка доступности AlwaysOn в Management Studio")  
...
##  <a name="bkmk_whathappens"></a> Что происходит после отработки отказа  
 ...
 Если отработка отказа происходит во время обработки, в файле журнала или выходном окне служб Analysis Services появляется следующая ошибка: "OLE DB error: OLE DB or ODBC error: Communication link failure; 08S01; TPC Provider: существующее соединение было принудительно завершено удаленным узлом. ; 08S01."  
...
##  <a name="bkmk_writeback"></a>Обратная запись при использовании базы данных доступности AlwaysOn  
...
## <a name="see-also"></a>См. также:  
 ...
 [Активные вторичные реплики: Вторичные реплики &#40;, доступные для чтения группы доступности AlwaysOn&#41;](active-secondaries-readable-secondary-replicas-always-on-availability-groups.md)   
