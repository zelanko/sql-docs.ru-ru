...
##  <a name="bkmk_UseSecondary"></a> Контрольный список: Использовать вторичную реплику только для операций чтения  
...
#### <a name="step-2-configure-read-only-routing"></a>Этап 2. настройка маршрутизации только для чтения  
...
#### <a name="step-1-confirm-the-data-source-connection-is-redirected-to-the-secondary-replica"></a>Шаг 1. проверка того, что подключение к источнику данных перенаправляется на вторичную реплику  
...
#### <a name="step-2-perform-a-planned-failover-to-test-the-configuration"></a>Этап 2. плановая отработка отказа для проверки конфигурации  
...
##  <a name="bkmk_whathappens"></a> Что происходит после отработки отказа  
 ...
 Если отработка отказа происходит во время обработки, в файле журнала или выходном окне служб Analysis Services появляется следующая ошибка: «Ошибка OLE DB: OLE DB or ODBC error: Communication link failure; 08S01; TPC Provider: существующее соединение было принудительно завершено удаленным узлом. ; 08S01."  
