...
1.  Настройка удаленного распространения. Если хранимые процедуры используются для настройки издателя, запустите `sp_adddistributor`. Укажите то же значение для *@password* , который применялся при `sp_adddistrbutor` была запущена на распространителе для настройки распространения.  
