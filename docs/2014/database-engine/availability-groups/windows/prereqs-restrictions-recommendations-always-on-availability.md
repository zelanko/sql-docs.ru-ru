...
-   **Выделенные сетевые адаптеры**.  Для наилучшей производительности используйте отдельный сетевой адаптер (сетевую карту) для [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)].  
...
-   [Windows 2008 R2 отказоустойчивые Многосайтовые кластеры](https://kiruba4u.blogspot.com/2012/03/failover-clustering-in-windows-server.html)  
...
-   **Реплики доступности должны размещаться на разных узлах одного кластера WSFC:**  Для данной группы доступности реплик доступности должны находиться в экземплярах сервера, работающих на разных узлах одного кластера WSFC. Единственное исключение состоит в том, что при переносе в другой кластер WSFC группа доступности может временно находится в двух кластерах.  
...
|![Флажок](../../media/checkboxemptycenterxtraspacetopandright.gif "Флажок")|Необходима по крайней мере одна полная резервная копия базы данных.<br /><br /> Примечание. После установки базы данных в режиме полного восстановления, для включения цепочки журналов полного восстановления требуется полное резервное копирование.|[Создание полной резервной копии базы данных (SQL Server)](../../../relational-databases/backup-restore/create-a-full-database-backup-sql-server.md)|  
