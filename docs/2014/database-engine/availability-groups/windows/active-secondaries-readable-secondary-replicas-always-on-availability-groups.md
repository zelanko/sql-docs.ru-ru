---
title: 'Активные вторичные реплики: Вторичные реплики для чтения (для групп доступности AlwaysOn) | Документы Microsoft'
ms.custom: ''
ms.date: 10/27/2017
...
-   Для транзакций между контейнерами не поддерживаются транзакции с уровнем изоляции «моментальный снимок», при которых осуществляется доступ к оптимизированным для памяти таблицам. Например,  
