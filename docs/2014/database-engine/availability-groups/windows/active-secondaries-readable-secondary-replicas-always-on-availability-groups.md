...
>  Хотя в базы данных-получатели нельзя записывать данные, их можно записывать в базы данных, предназначенные для чтения и записи на экземпляре сервера, содержащем вторичную реплику, в том числе в пользовательские и системные базы данных, например в базу данных **tempdb**.  
 ...
 [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)]также поддерживает повторную маршрутизацию запросов на подключение с намерением чтения к доступной для чтения вторичной реплике (*Маршрутизация только для чтения*). Дополнительные сведения см. в статье [Соединение с помощью прослушивателя со вторичной репликой только для чтения (маршрутизация только для чтения)](../../listeners-client-connectivity-application-failover.md#ConnectToSecondary).  
...
##  <a name="bkmk_Prerequisites"></a>Предварительные требования для группы доступности  
...
-   **Маршрутизация только для чтения**  
     ...
     *Маршрутизация только для чтения* — это возможность SQL Server маршрутизации входящих запросов на подключение с намерением чтения, которые направляются в прослушиватель группы доступности, в доступную для чтения вторичную реплику. Необходимые условия для использования маршрутизации только для чтения:  
...
####  <a name="bkmk_LatencyWithInMemOLTP"></a>Задержка данных в базах данных с таблицами, оптимизированными для памяти  
...
###  <a name="ReadOnlyWorkloadImpact"></a>Воздействие на рабочую нагрузку только для чтения  
...
-   Удалите временную статистику с помощью инструкции [DROP STATISTICS](/sql/t-sql/statements/drop-statistics-transact-sql) [!INCLUDE[tsql](../../../includes/tsql-md.md)] .  
  
-   Мониторинг статистики ведется с помощью представлений каталога **sys.stats** и **sys.stats_columns**. **sys_stats** включает столбец **is_temporary**, чтобы указать, какая именно статистика является постоянной, а какие — временными.  
...
##  <a name="bkmk_AccessInMemTables"></a>Доступ к оптимизированным для памяти таблицам во вторичной реплике  
...
-   Для транзакций между контейнерами транзакции с уровнем изоляции сеанса "Snapshot", обращающиеся к оптимизированным для памяти таблицам, не поддерживаются. Например,  
...
##  <a name="bkmk_CapacityPlanning"></a>Рекомендации по планированию ресурсов  
