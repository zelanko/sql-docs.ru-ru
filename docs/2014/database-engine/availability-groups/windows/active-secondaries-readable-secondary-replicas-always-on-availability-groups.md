...
##  <a name="prerequisites-for-the-availability-group"></a><a name="bkmk_Prerequisites"></a>Предварительные требования для группы доступности  
...
###  <a name="read-only-workload-impact"></a><a name="ReadOnlyWorkloadImpact"></a>Воздействие на рабочую нагрузку только для чтения  
...
###  <a name="indexing"></a><a name="bkmk_Indexing"></a>Индекс  
...
-   Мониторинг статистики ведется с помощью представлений каталога **sys.stats** и **sys.stats_columns**. **sys_stats** включает столбец **is_temporary**для указания того, какая статистика является постоянной, а какая — временной.  
...
-   Для транзакций между контейнерами транзакции с уровнем изоляции сеанса "Snapshot", обращающиеся к оптимизированным для памяти таблицам, не поддерживаются. Например, примененная к объекту директива  
