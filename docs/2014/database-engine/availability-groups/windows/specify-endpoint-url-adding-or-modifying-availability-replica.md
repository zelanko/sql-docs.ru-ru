...
-   *\<system-address>* Строка, однозначно идентифицирующая целевую систему компьютера. Обычно сетевой адрес представляет собой системное имя (если компьютеры входят в один домен), полное доменное имя или IP-адрес.  
...
-   *\<port>* номер порта, используемого конечной точкой зеркального отображения экземпляра сервера-участника.  
...
##  <a name="finding-the-fully-qualified-domain-name-of-a-system"></a><a name="Finding_FQDN"></a>Поиск полного доменного имени системы  
