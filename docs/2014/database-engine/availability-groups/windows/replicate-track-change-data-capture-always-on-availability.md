...
# <a name="replication-change-tracking-change-data-capture-and-alwayson-availability-groups-sql-server"></a>Репликация, отслеживание изменений, изменение данных и группы доступности AlwaysOn (SQL Server)
  ...
  [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Репликация, отслеживание измененных данных (CDC) и отслеживание изменений (CT) поддерживаются в [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)]. 
  [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] помогает обеспечивать высокий уровень доступности и дополнительные возможности восстановления баз данных.  
  
 
  
##  <a name="Overview"></a>Общие сведения о репликации на группы доступности AlwaysOn  
...
###  <a name="Changes"></a>Общие изменения в агентах репликации для поддержки группы доступности AlwaysOn  
...
###  <a name="StoredProcs"></a>Хранимые процедуры, поддерживающие AlwaysOn  
...
-   **Сбор изменений для системы отслеживания измененных данных без репликации**  
...
-   **Сбор изменений для системы отслеживания измененных данных с помощью репликации**  
...
-   **Клиентские приложения, обращающиеся к данным изменений CDC и Always On**  
...
-   **Перенаправление загрузки запроса на вторичную реплику для чтения**  
    ...
    ```  
     ...
     Если `sp_addlinkedserver` используется для создания связанного сервера для доступа к базе-получателю, *@datasrc* параметр используется для имени прослушивателя группы доступности или явного имени сервера, а *@provstr* параметр используется для указания намерения только для чтения.  
...
-   **Доступ клиента к данным изменений CDC и доменным именам входа**  
...
##  <a name="Prereqs"></a>Предварительные требования, ограничения и рекомендации по использованию репликации  
...
|**Моментальный снимок**|Да|нет|Да <sup>2</sup>|  
 ...
 <sup>2</sup> отработка отказа в базу данных реплики является процедурой, выполняемой вручную. Автоматический переход на другой ресурс не предоставляется.  
...
-   [Работа с Отслеживание изменений &#40;SQL Server&#41;](../../../relational-databases/track-changes/work-with-change-tracking-sql-server.md)  
...
## <a name="see-also"></a>См. также:  
 ...
 [Группы доступности AlwaysOn:](always-on-availability-groups-interoperability-sql-server.md) [экземпляры отказоустойчивого кластера AlwaysOn (SQL Server) (SQL Server)](../../../sql-server/failover-clusters/windows/always-on-failover-cluster-instances-sql-server.md)   
