...
###  <a name="ADQuotas"></a>Не удалось создать прослушиватель группы доступности из-за Active Directoryных квот  
...
###  <a name="MultiSubnetFailover"></a>Ключевое слово MultiSubnetFailover и связанные с ним функции  
 ...
 **MultiSubnetFailover = true не поддерживается в .NET Framework 3,5 или OLEDB**  
 ...
 **Обходные пути:** Рекомендуется выполнить одну из следующих задач.  
...
-   При отсутствии разрешения на работу с ресурсами кластера измените время ожидания соединения на 30 секунд (это означает 20 секунд ожидания TCP плюс 10-секундный буфер).  
     ...
     **Профессионалы**. Если происходит отработка отказа между подсетями, время восстановления клиента будет небольшим.  
  
     Число **недостатков**: половина клиентских подключений займет более 20 секунд  
...
-   При наличии разрешений на работу с ресурсами кластера рекомендуется задать `RegisterAllProvidersIP=0`в качестве сетевого имени прослушивателя группы доступности. Дополнительные сведения см. в подразделе "Установка значения RegisterAllProvidersIP" далее в этом разделе.  
     ...
     **Специалисты:** Вам не нужно увеличивать значение времени ожидания клиентского подключения.  
...
###  <a name="HostRecordTTL"></a>Параметр HostRecordTTL  
...
###  <a name="SampleScript"></a>Пример скрипта PowerShell для отключения RegisterAllProvidersIP и сокращения срока жизни  
...
1.  Создайте прослушиватель, используя одно из следующих средств:  
    ...
    -   **Использование диспетчер отказоустойчивости кластеров WSFC:**  
    ...
    -   **Использование Windows PowerShell для отказоустойчивых кластеров:**  
...
-   [Создание нескольких прослушивателей для одной группы доступности](https://blogs.msdn.com/b/sqlalwayson/archive/2012/02/03/how-to-create-multiple-listeners-for-same-availability-group-goden-yao.aspx)  
...
## <a name="see-also"></a>См. также:  
 ...
 [SQL Server кластеризации с несколькими подсетями &#40;SQL Server&#41;](../../../sql-server/failover-clusters/windows/sql-server-multi-subnet-clustering-sql-server.md)  
