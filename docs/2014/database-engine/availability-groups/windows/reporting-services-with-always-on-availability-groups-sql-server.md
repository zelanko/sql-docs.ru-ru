...
##  <a name="bkmk_requirements"></a>Требования к использованию Reporting Services и группы доступности AlwaysOn  
...
##  <a name="bkmk_reportdatasources"></a>Источники данных отчета и группы доступности  
...
-   **Основной режим:** Используйте диспетчер отчетов для общих источников данных и отчетов, которые уже опубликованы на сервере отчетов в собственном режиме.  
  
-   **Режим интеграции с SharePoint:** Используйте страницы конфигурации SharePoint в библиотеках документов для отчетов, которые уже опубликованы на сервере SharePoint.  
 ...
 **Рекомендации.** Во вторичных репликах, как правило, возникает задержка при получении изменений данных из первичной реплики. На задержку обновления между первичной и вторичными репликами влияют следующие факторы.  
...
##  <a name="bkmk_reportdesign"></a>Проектирование отчетов и группы доступности  
...
-   **Локальная Предварительная версия:** [!INCLUDE[ssRBDenali](../../../includes/ssrbdenali-md.md)] и [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)] использование .NET Framework 4,0 [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] и поддержка свойств строки подключения.  
  
-   **Удаленный или серверный режим просмотра:** Если после публикации отчетов на сервере отчетов или использования предварительной версии [!INCLUDE[ssRBDenali](../../../includes/ssrbdenali-md.md)]в отображается сообщение об ошибке, аналогичное приведенному ниже, это указывает на то, что предварительный просмотр отчетов на сервере отчетов не установлен на сервере отчетов, [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] а исправление .NET Framework 3,5 с пакетом обновления 1 (SP1) для не установлено.  
...
###  <a name="bkmk_differences_in_server_mode"></a>Различия между режимом SharePoint Native  
...
-   **Первичная реплика:** Настройте базы данных сервера отчетов как часть одной группы доступности и создайте первичную реплику, включающую все базы данных сервера отчетов.  
...
-   **Учетные данные сервера отчетов:** Необходимо создать соответствующие учетные данные сервера отчетов на вторичных репликах, созданных на сервере-источнике. Действия, которые для этого нужно предпринять, зависят от типа проверки подлинности, используемой в среде служб [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]: учетная запись службы Window [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)], учетная запись пользователя Windows или проверка подлинности SQL Server. Дополнительные сведения см. в разделе [Настройка соединения с базой данных сервера отчетов (диспетчер конфигурации SSRS)](../../../sql-server/install/configure-a-report-server-database-connection-ssrs-configuration-manager.md).  
...
###  <a name="bkmk_steps_to_complete_failover"></a>Действия по завершению аварийного восстановления баз данных сервера отчетов  
...
###  <a name="bkmk_failover_behavior"></a>Поведение сервера отчетов при отработке отказа  
...
## <a name="see-also"></a>См. также:  
 [Поддержка высокого уровня доступности и аварийного восстановления SQL Server Native Client](../../../relational-databases/native-client/features/sql-server-native-client-support-for-high-availability-disaster-recovery.md)   
  Начало работы [группы доступности AlwaysOn (SQL Server)](always-on-availability-groups-sql-server.md) [с группы доступности AlwaysOn &#40;SQL Server&#41;](getting-started-with-always-on-availability-groups-sql-server.md)   
