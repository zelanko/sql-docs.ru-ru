 ...
 **Проблема:** Хэш-индексам требуются значения для всех ключевых столбцов с индексами, чтобы вычислить значение хэша и найти соответствующие строки и в хэш-таблице. Поэтому если запрос включает предикаты равенства только для подмножества ключей индекса в предложении WHERE, то [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] не может использовать поиск по индексу для нахождения строк, соответствующих предикатам в предложении WHERE.  
 ...
 **Симптом:** Это приводит к снижению производительности, так как службы [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] должны выполнить полный просмотр таблицы вместо поиска по индексу, который обычно является более быстрой операцией.  
 ...
 **Решения.** Есть несколько возможных решений. Пример:  
