...
2.  Переведите базу данных как почтовую систему для система предупреждений SQL Server: щелкните правой кнопкой мыши **агента SQL Server**выберите **система предупреждений**, проверьте **Включить почтовый профиль** поле, выберите  **Компонент Database Mail** как **почтовая система**и выберите ранее созданный профиль электронной почты.  
...
-   **Свободно - A:** это обычный файл резервной копии. Резервное копирование выполнено, а также проверена доступность копии в хранилище Windows Azure.  
  
-   **Выполняется копирование — б** — это состояние для баз данных группы доступности. Если [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] обнаружит разрыв в цепочке резервного копирования журнала, сначала будет сделана попытка определить операцию копирования, вызвавшую этот разрыв. При обнаружении файла резервной копии будет сделана попытка скопировать файл в хранилище Windows Azure. Данное состояние отображается при выполнении операции копирования.  
  
-   **Скопируйте Failed — F:** подобный Copy In Progress, это конкретных баз данных группы доступности. Если процесс копирования завершается ошибкой, состояние отмечается как F.  
