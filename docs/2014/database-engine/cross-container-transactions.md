---
title: Транзакции между контейнерами | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
### <a name="isolation-semantics-for-individual-operations"></a>Семантика изоляции для отдельных операций  
 ...
 Операции чтения выполняются как часть уровни изоляции T. транзакций для операций чтения, можно называть:  
 ...
 SERIALIZABLE  
 Все гарантии REPEATABLE READ, а также Предотвращение фантомов и согласованность транзакций по отношению к всем сериализуемым операциям чтения, выполненных T. фантомных Предотвращение означает, что операция просмотра может возвращать только дополнительные строки, записанные транзакцией T, но не строки, которые были записаны другими транзакциями.  
...
```  
 ...
 Несмотря на то, что операция чтения из t1 в конце транзакции синтаксически — read committed, но фактически является сериализуемой, поскольку та же операция чтения выполнялась ранее в транзакции в режиме изоляции serializable: возможность сериализации гарантирует, что если считывание закончится выполнить в любой более позднем этапе транзакции, возвращаются те же строки.  
  
## <a name="cross-container-transactions-and-isolation-levels"></a>Транзакции между контейнерами и уровни изоляции  
 Транзакции между контейнерами можно рассматривать с двух сторон: дисковой стороны (для операций на дисковых таблицах) и стороны оптимизированной памяти (для операций с таблицами, оптимизированными для памяти). Уровни изоляции этих двух сторон могут быть различными. В действительности отдельные операции чтения с каждой стороны могут иметь различные уровни изоляции.  
...
-   Операция считывания в дисковой таблице выполняется на уровне изоляции X с использованием синтаксиса `WITH (X)`.  
 ...
 Оптимизированные для памяти сторона транзакции может достичь одного из двух уровней: Если Условие1 имеет значение true, то достигается сериализуемый уровень, если он имеет значение false, оптимизированная для памяти сторона достигает только для изоляции моментального снимка.  
