...
### <a name="isolation-semantics-for-individual-operations"></a>Семантика изоляции для отдельных операций  
 ...
 SERIALIZABLE.  
 Все гарантии ПОВТОРЯЕМости чтения и фантомных проблем, а также согласованность транзакций в отношении всех сериализуемых операций чтения, выполняемых T. фантомное исключение означает, что операция просмотра может возвращать только дополнительные строки, написанные T, но не строки, записанные другими транзакциями.  
...
```  
 ...
 Несмотря на то, что чтение из T1 в конце транзакции является зафиксированным, он фактически является сериализуемым, так как в транзакции в режиме сериализуемой изоляции была выполнена та же операция чтения: возможности сериализации гарантирует, что если чтение в любые последующие точки транзакции возвращаются те же строки.  
