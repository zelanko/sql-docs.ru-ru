 ...
 **Рекомендация**: Если требуется только гарантия уровня изоляции READ COMMITTED, используйте изоляцию SNAPSHOT с хранимыми процедурами, скомпилированными в собственном коде, а также для доступа к таблицам, оптимизированным для памяти, посредством интерпретируемых [!INCLUDE[tsql](../includes/tsql-md.md)]-инструкций.  
...
-   В транзакциях, использующих уровень изоляции READ COMMITTED, каждая инструкция видит самую последнюю версию строк в базе данных. Следовательно, последующие инструкции видят изменения состояния базы данных.  
     ...
     **Рекомендации:** Если приложению нужно опросить оптимизированную для памяти таблицу для получения последних строк, записанных в таблицу, переместите цикл опроса за пределы транзакции.  
