 ...
 **Рекомендация**: Если требуется только гарантия изоляции READ COMMITTED, используйте изоляцию моментального снимка с хранимыми процедурами, скомпилированными в собственном коде, а также для [!INCLUDE[tsql](../includes/tsql-md.md)]доступа к оптимизированным для памяти таблицам с помощью интерпретации.  
...
## <a name="locking-table-hints"></a>Табличные указания блокировки  
 Подсказки блокировки ([Табличные указания &#40;Transact-SQL&#41;](/sql/t-sql/queries/hints-transact-sql-table)), такие как HOLDLOCK и XLOCK, можно использовать с таблицами на диске [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] , чтобы иметь больше блокировок, чем требуется для указанного уровня изоляции.  
