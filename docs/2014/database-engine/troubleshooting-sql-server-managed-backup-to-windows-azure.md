...
3.  **Ошибки подключения:**  
    ...
    -   **Ошибки при подключении к учетной записи хранения:**  
         ...
         В управляемом резервном копировании SQL Server используется нижележащий компонент «Резервное копирование на URL-адрес», поэтому ошибки, связанные с подключением к хранилищу, относятся к обоим компонентам. Дополнительные сведения об устранении неполадок см. в **разделе "Устранение неполадок** " статьи рекомендации по [SQL Server резервного копирования на URL-адреса и](../relational-databases/backup-restore/sql-server-backup-to-url-best-practices-and-troubleshooting.md) инструкции по устранению неполадок.  
...
-   **Sqlservr. exe перестает отвечать на запросы или [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] перестает работать при выполнении:** Если SQL Server перестанет работать, работа агента SQL Server будет остановлена [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] , также прекращается, а события регистрируются в файле SQL Agent. out.  
  
     Если SQL Server прекращает отвечать, события записываются в канал администратора.  Пример журнала событий.  
  
     *Ошибка SQL (модуль не отвечает или получить sqlException: SqlException*   
