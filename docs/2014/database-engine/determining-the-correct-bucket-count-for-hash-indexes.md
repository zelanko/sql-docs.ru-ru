...
# <a name="determining-the-correct-bucket-count-for-hash-indexes"></a>Определение правильного числа контейнеров для хэш-индексов
 ...
 Дополнительные сведения о некластеризованных хэш-индексов, см. в разделе [хэш-индексы](hash-indexes.md) и [Using Indexes on Memory-Optimized Tables, касающиеся](../relational-databases/in-memory-oltp/memory-optimized-tables.md).  
...
## <a name="recommendations"></a>Рекомендации  
 ...
 Не всегда можно спрогнозировать количество значений, которые содержит конкретный ключ индекса. Уровень производительности считается приемлемым Если `BUCKET_COUNT` значение имеет в 5 раз фактическое число ключевых значений.  
...
-   Индекс первичного ключа (PK__SalesOrder…): 36 % контейнеров пусты, что хорошо. Кроме того, средняя длина цепочки имеет значение 1, что тоже является хорошим показателем. Нет необходимости в изменениях.  
 ...
 Дополнительные сведения об устранении неполадок с оптимизированными для памяти хэш-индексами см. в разделе [Устранение общих проблем с производительностью оптимизированных для памяти хэш-индексы](../../2014/database-engine/troubleshooting-common-performance-problems-with-memory-optimized-hash-indexes.md).  
...
### <a name="big-tables"></a>Большие таблицы  
 Для больших таблиц использование памяти может стать проблемой. Например, затраты на хэш-таблиц с 250 миллионов строк таблицы, которая имеет 4 хэш-индекса, каждая из которых число контейнеров, равное 1 миллиарду — 4 индекса * 1 миллиард контейнеров \* 8 байт = 32 гигабайта использования памяти. При выборе числа контейнеров, равного 250 миллионов, для всех индексов общий объем используемой памяти для хэш-таблиц составит 8 ГБ. Обратите внимание, что 8 байт использования памяти каждого индекса добавляет к каждой конкретной строке, что равно 8 ГБ в этом сценарии (4 индекса \* 8 байт \* 250 миллионов строк).  
