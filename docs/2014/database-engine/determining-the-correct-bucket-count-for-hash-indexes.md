---
title: Определение правильного числа контейнеров для хэш-индексов | Документы Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
# <a name="determining-the-correct-bucket-count-for-hash-indexes"></a>Определение правильного числа контейнеров для хэш-индексов
 ...
 Дополнительные сведения о некластеризованных хэш-индексах см. в разделе [хэш-индексы](hash-indexes.md) и [правила с помощью индексов в таблицах, оптимизированных для памяти](../relational-databases/in-memory-oltp/memory-optimized-tables.md).  
  
 Одна хэш-таблица выделяется для каждого хэш-индекса таблицы, оптимизированной для памяти. Размер хэш-таблицы, выделенной для индекса, указывается `BUCKET_COUNT` параметр в [CREATE TABLE &#40;Transact-SQL&#41; ](/sql/t-sql/statements/create-table-transact-sql) или [CREATE TYPE &#40;Transact-SQL&#41; ](/sql/t-sql/statements/create-type-transact-sql). Число контейнеров округляется до ближайшей степени числа 2. Например, если указано 300 000 контейнеров, то фактически число контейнеров составит 524 288.  
  
 Ссылки на статьи и видео на тему числа контейнеров см. в разделе [Как определить нужное число контейнеров для хэш-индексов (OLTP в памяти)](http://go.microsoft.com/fwlink/p/?LinkId=525853).  
...
## <a name="recommendations"></a>Рекомендации  
 ...
 Не всегда можно спрогнозировать количество значений, которые содержит конкретный ключ индекса. Производительности приемлемо, если `BUCKET_COUNT` значение в 5 раз фактическое число ключевых значений.  
 ...
 *empty_bucket_percent* указывает число пустых контейнеров в хэш-индекс.  
...
-   Индекс первичного ключа (PK__SalesOrder…): 36 % контейнеров пусты, что хорошо. Кроме того, средняя длина цепочки имеет значение 1, что тоже является хорошим показателем. Нет необходимости в изменениях.  
  
 Дополнительные сведения об устранении проблем с оптимизированными для памяти хэш-индексов см. в разделе [Устранение общих проблем с производительностью оптимизированных для памяти хэш-индексы](../../2014/database-engine/troubleshooting-common-performance-problems-with-memory-optimized-hash-indexes.md).  
...
### <a name="big-tables"></a>Большие таблицы  
 Для больших таблиц использование памяти может стать проблемой. Например, объем используемой памяти для хэш-таблицы с 250 миллионов строк таблицы, которая имеет 4 хэш-индекса, каждая из которых число контейнеров, равное 1 миллиарду составляет 4 индекса * 1 миллиард контейнеров \* 8 байт = 32 гигабайта использования памяти. При выборе числа контейнеров, равного 250 миллионов, для всех индексов общий объем используемой памяти для хэш-таблиц составит 8 ГБ. Обратите внимание, что 8 байт использования памяти каждый индекс добавляет к каждой конкретной строке, что равно 8 ГБ в этом сценарии (4 индекса \* 8 байт \* 250 миллионов строк).  
