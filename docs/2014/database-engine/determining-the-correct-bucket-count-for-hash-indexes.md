...
-   IX_Status: 50% контейнеров пусты, что является хорошим показателем. Однако среднее значение длины цепочки очень высоко (65 536). Это указывает на большое количество повторяющихся значений. Поэтому использование некластеризованного хэш-индекса в данном случае не подходит. Вместо этого следует пользоваться некластеризованным индексом.  
...
-   Индекс первичного ключа (PK__SalesOrder …): 36% контейнеров пусты, что является хорошим показателем. Кроме того, средняя длина цепочки имеет значение 1, что тоже является хорошим показателем. Нет необходимости в изменениях.  
