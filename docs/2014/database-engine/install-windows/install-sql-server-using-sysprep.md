...
11. На странице «Конфигурация экземпляра» укажите идентификатор экземпляра. Для продолжения нажмите кнопку **Далее**.  
  
     **Идентификатор экземпляра** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. идентификатор экземпляра используется для идентификации каталогов установки и разделов реестра для экземпляра. Действует как для экземпляров по умолчанию, так и для именованных экземпляров. Если подготовленный экземпляр на шаге завершения создан как экземпляр по умолчанию, то имя экземпляра переопределяется на MSSQLSERVER. Указанный идентификатор экземпляра остается без изменений.  
  
     **Корневой каталог экземпляра** — по умолчанию корневым каталогом [!INCLUDE[ssInstallPath](../../includes/ssinstallpath-md.md)]экземпляра является. Чтобы задать корневой каталог, отличный от каталога по умолчанию, введите имя каталога в поле или нажмите кнопку **Обзор** и выберите папку установки. Каталог, указанный на шаге установки, будет использован при настройке на шаге завершения.  
...
12. На странице **требования к месту на диске** вычисляется необходимое место на диске для указанных компонентов. Затем необходимое пространство сравнивается с доступным местом на диске.  
...
9. На странице «Конфигурация экземпляра» укажите имя подготовленного экземпляра. После завершения настройки [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]это имя будет присвоено экземпляру. Для продолжения нажмите кнопку **Далее**.  
  
     **Идентификатор экземпляра** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. идентификатор экземпляра используется для идентификации каталогов установки и разделов реестра для экземпляра. Действует как для экземпляров по умолчанию, так и для именованных экземпляров. Если подготовленный экземпляр на шаге завершения создан как экземпляр по умолчанию, то имя экземпляра переопределяется на MSSQLSERVER. Указанный на этапе подготовки идентификатор экземпляра остается без изменений.  
...
5.  На странице **Выбор компонентов** укажите компоненты, которые нужно удалить из указанного экземпляра [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . Для продолжения нажмите кнопку **Далее**.  
