---
title: Обновление PowerPivot для SharePoint (ru) Документы Майкрософт
ms.custom: ''
ms.date: 03/25/2019
...
-   Средства настройки PowerPivot — это одни из общих компонентов SQL Server. Все общие компоненты обновляются одновременно. Если во время обновления будут выбраны другие экземпляры SQL Server или функции, для которых требуется обновление общих компонентов, то средство настройки PowerPivot также будет обновлено. Если средство настройки PowerPivot было обновлено, а экземпляр PowerPivot не был, могут возникнуть проблемы. Для получения более подробной информации об общих функциях сервера S'L, см [&#41;&#40;. ](../../database-engine/install-windows/upgrade-sql-server-using-the-installation-wizard-setup.md)  
...
-   Если существующая установка PowerPivot является, [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)] [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)] сервисный пакет 2 (SP2) необходим для обновления. [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]  
  
-   Если существующая установка PowerPivot является, [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] сервисный пакет 1 (SP1) необходим для обновления. [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]  
...
1.  Запустите программу установки [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] на внутренних серверах, выполняющих [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] в режиме интеграции с SharePoint. Если сервер содержит несколько экземпляров [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)], обновите по крайней мере экземпляр **POWERPIVOT** . В следующем списке приведена сводка шагов мастера установки, относящихся к обновлению [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] :  
    ...
    2.  Нажмите **Обновление от сервера S'L.....**.  
  
    3.  На странице **Выбор экземпляра** выберите экземпляр с именем **POWERPIVOT** и нажмите кнопку **Далее**.  
  
    4.  Для получения дополнительной информации см [&#41;&#40;.](../../database-engine/install-windows/upgrade-sql-server-using-the-installation-wizard-setup.md)  
  
2.  Перезапустите сервер.  
  
3.  Запустите надстройку [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] для SharePoint (**spPowerPivot.msi**) на каждом сервере в ферме SharePoint 2013, чтобы установить поставщики данных. Исключением будут серверы, где вы запустили мастер установки SQL Server, который также обновляет поставщики данных. Для получения дополнительной информации смотрите [Установить или установить PowerPivot для SharePoint Add-in &#40;SharePoint 2013&#41;](https://docs.microsoft.com/analysis-services/instances/install-windows/install-or-uninstall-the-power-pivot-for-sharepoint-add-in-sharepoint-2013).  
...
4.  **Запустите средство настройки PowerPivot для SharePoint 2013** на одном из серверов приложений SharePoint для настройки фермы SharePoint на работу с обновленными файлами решения, которые установила надстройка. На этом шаге нельзя использовать центр администрирования SharePoint. Дополнительные сведения см. в следующих разделах:  
    ...
    2.  Во время запуска средство настройки проверяет состояние обновления решения фермы и решений веб-приложений PowerPivot. Если будут обнаружены старые версии этих решений, вы увидите сообщение "**Обнаружены новые версии файлов решений PowerPivot. Пожалуйста, выберите опцию обновления для обновления фермы».** Нажмите кнопку **ОК** , чтобы закрыть сообщение проверки системы.  
    ...
    7.  Нажмите кнопку **Запуск** , чтобы выполнить все действия, которые являются допустимыми для данной задачи. Кнопка**Запуск** становится доступной только после прохождения проверки. При нажатии **кнопки Run**появляется следующее предупреждение, напоминающее вам, что действия обрабатываются в пакетном режиме:**«Все настройки конфигурации, помеченные как действительные в инструменте, будут применены к ферме SharePoint. Вы хотите продолжить?**".  
  
    8.  Для продолжения нажмите кнопку **Да**.  
  
    9. Обновление решений и компонентов в ферме может занять несколько минут. В течение этого времени запросы на подключение к данным PowerPivot **не смогут выйти из строя** с ошибками, аналогичными **«Невозможности обновить данные»** или **«Произошла ошибка при выполнении запрошенного действия. Пожалуйста, повторите попытку**».. После завершения обновления сервер станет доступным, и эти ошибки больше не будут возникать.  
    ...
    -   [Наконфигурируйте или отремонтируйте PowerPivot для SharePoint 2013 &#40;Tool PowerPivot&#41;](https://docs.microsoft.com/analysis-services/power-pivot-sharepoint/configure-or-repair-power-pivot-for-sharepoint-2013)  
...
5.  **Запустите программу установки [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] Установку** на первом сервере приложений SharePoint, где запущена служба Windows **Службы SQL Server Analysis Services (PowerPivot)** , чтобы обновить экземпляр POWERPIVOT. На странице «Установка» мастера установки SQL Server выберите вариант обновления. Для получения дополнительной информации см [&#41;&#40;. ](../../database-engine/install-windows/upgrade-sql-server-using-the-installation-wizard-setup.md)  
...
7.  **Запустите инструмент настройки PowerPivot** на первом сервере приложений SharePoint, который запускает службу анализа серверов S'L (PowerPivot) для обновления решений и web-сервисов в SharePoint. На этом шаге нельзя использовать центр администрирования.  
    ...
    2.  Во время запуска средство настройки проверяет состояние обновления решения фермы и решений веб-приложений PowerPivot. Если будут обнаружены старые версии этих решений, вы увидите сообщение "Обнаружены новые версии файлов решений PowerPivot. Выберите параметр обновления для обновления фермы". Нажмите кнопку **ОК**, чтобы закрыть сообщение.  
  
    3.  Для продолжения выберите **Обновление компонентов, служб, приложений и решений**и нажмите кнопку **ОК** .  
  
    4.  Появляется следующее предупреждение: "Рабочие книги в панели управления PowerPivot собираются обновить до последней версии. Все настройки, внесенные в существующие книги, будут утеряны. Продолжить?".  
    ...
    8.  Нажмите кнопку **Запуск** , чтобы выполнить все действия, которые являются допустимыми для данной задачи. Кнопка**Запуск** становится доступной только после прохождения проверки. При нажатии **кнопки Run**появляется следующее предупреждение, напоминающее вам, что действия обрабатываются в пакетном режиме: «Все параметры конфигурации, помеченные как действительные в инструменте, будут применены к ферме SharePoint. Продолжить?".  
  
    9. Для продолжения нажмите кнопку **Да**.  
  
    10. Обновление решений и компонентов в ферме может занять несколько минут. В течение этого времени запросы на подключение к данным PowerPivot не смогут выполнить такие ошибки, как "Не удается обновить данные" или "Произошла ошибка при выполнении запрошенного действия. Повторите попытку". После завершения обновления сервер станет доступным, и эти ошибки больше не будут возникать.  
  
8.  **Повторите процесс** для каждой службы анализа серверов S'L (PowerPivot) на ферме: 1) Выполнить настройку сервера S'L 2) Выполнить инструмент powerPivot Configuration.  
...
10. **Ошибки устранения неполадок**  
    ...
    3.  Для развертывания или опровержений, связанных с решениями PowerPivot, скопируйте значение GUID для **JobId,** а затем вставьте его в следующую команду (используйте команды «Марк, копия» и «Вставить» в меню «Отметьте» Shell для копирования GUID):  
    ...
    4.  Повторите задачу в средстве настройки, нажав кнопку **Проверка** , а затем кнопку **Запуск**.  
     ...
     Сведения о других ошибках см. в журналах ULS. Для получения дополнительной информации смотрите [настройку и просмотр файлов журнала SharePoint и диагностическую &#40;PowerPivot для SharePoint&#41;](https://docs.microsoft.com/analysis-services/power-pivot-sharepoint/configure-and-view-sharepoint-and-diagnostic-logging).  
  

  
##  <a name="workbooks"></a><a name="bkmk_workbooks"></a>Книг  
...
##  <a name="verify-the-versions-of-powerpivot-components-and-services"></a><a name="bkmk_verify_versions"></a>Проверка версий компонентов и услуг PowerPivot  
...
4.  [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]файл версия должна быть 12.00. \<крупных> сборки. \<незначительные> сборки.  
...
3.  Убедитесь, что у вас есть версия 12.0. \<построить номер>.  
  

##  <a name="upgrading-multiple-powerpivot-for-sharepoint-servers-in-a-sharepoint-farm"></a><a name="geminifarm"></a>Обновление нескольких PowerPivot для серверов SharePoint в farm SharePoint  
...
##  <a name="applying-a-qfe-to-a-powerpivot-instance-in-the-farm"></a><a name="qfe"></a>Применение ЗФЗ к экземпляру PowerPivot на ферме  
...
|Проверьте функцию обновления данных в выбранных книгах, настроенных для поддержки обновления данных.|Самый простой способ проверки обновления данных заключается в изменении расписания обновления данных с помощью установки флажка **Обновлять при первой возможности** , чтобы операции обновления данных выполнялись немедленно. При помощи этого шага удастся определить успешность обновления данных для текущей книги. Повторите эти шаги для других часто используемых книг, чтобы убедиться, что обновление данных работает. Для получения дополнительной информации об обновлении данных планирования [&#41;&#40;](../../../2014/analysis-services/schedule-a-data-refresh-powerpivot-for-sharepoint.md)см.|  
|Следите за отчетами об обновлении данных на панели мониторинга PowerPivot, чтобы убедиться в отсутствии ошибок.|[Панель мониторинга управления PowerPivot и данные об использовании](https://docs.microsoft.com/analysis-services/power-pivot-sharepoint/power-pivot-management-dashboard-and-usage-data)|  
 ...
 Для получения дополнительной информации о том, как настроить настройки и функции PowerPivot, [см.](https://docs.microsoft.com/analysis-services/power-pivot-sharepoint/power-pivot-server-administration-and-configuration-in-central-administration)  
  
 Для пошаговых инструкций, которые помогут вам пройти через все задачи конфигурации после установки, см [&#41;&#40;. ](../../../2014/sql-server/install/initial-configuration-powerpivot-for-sharepoint.md)  

## <a name="see-also"></a>См. также:  
 [Особенности, поддерживаемые изданиями сервера S'L 2014](../../../2014/getting-started/features-supported-by-the-editions-of-sql-server-2014.md)   
