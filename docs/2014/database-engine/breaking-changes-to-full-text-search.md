---
title: Нарушение изменений в полнотекстовый поиск (ru) Документы Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
|-------------|--------------|---------------------|----------------------------------------|  
|[СОНГНОВТС](/sql/relational-databases/system-functions/containstable-transact-sql) С пользовательскими типами (UDT)|Полнотекстовый ключ — это определяемый пользователем тип [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], например `MyType = char(1)`.|Возвращаемый ключ имеет тип, назначенный определяемому пользователем типу.<br /><br /> В примере это будет **символ (1).**|Возвращаемый ключ имеет определяемый пользователем тип. В примере это будет **MyType**.|  
|*top_n_by_rank* параметр (заявления CONTAINSTABLE и [FREETEXTTABLE)](/sql/relational-databases/system-functions/freetexttable-transact-sql) [!INCLUDE[tsql](../includes/tsql-md.md)]|*top_n_by_rank* запросы с использованием 0 в качестве параметра.|Завершается с сообщением об ошибке, указывающим на то, что значение должно быть больше нуля.|Завершается успешно, возвращая 0 строк.|  
|СОНГОНТАИИи и **ItemCount**|Удаление строк из базовой таблицы перед передачей изменений компоненту MSSearch.|Функция CONTAINSTABLE возвращает фантомную запись. **ItemCount** не изменяется.|Функция CONTAINSTABLE не возвращает фантомных записей.|  
|**ItemCount**|Таблица содержит документы или типы столбцов, имеющие значение NULL.|В дополнение к индексированным документам в значении **ItemCount** учитываются документы, которые являются нулевыми или имеют нулевые типы.|В значении **ItemCount** учитываются только индексированные документы.|  
|Каталог **ItemCount**|Столбец BLOB с расширением NULL.|Он подсчитывается в **ItemCount** каталога|Он не учитывается в **ItemCount** каталога.|  
|**UniqueKeyCount**|Зачисляя уникальный отсчет ключей из каталога, например, две таблицы (таблица 1 и таблица2) каждый с тремя словами: word1, word2 и word3.|**UniqueKeyCount** No 9. В следующей таблице показано, как получено это значение.<br /><br /> таблица1 = 3<br /><br /> EOF для полнотекстового индекса таблицы1 = 1<br /><br /> таблица2 = 3<br /><br /> EOF для полнотекстового индекса таблицы2 = 1<br /><br /> полнотекстовый каталог = 1|Для каждой таблицы **UniqueKeyCount** — это количество различных ключевых слов No 1 (0xFF).  Это НЕ рассматривать те же слова в > 1 документ, как новый уникальный ключ.<br /><br /> Для каталога **UniqueKeyCount** — это сумма **UniqueKeyCount** каждой из таблиц каталога. Одинаковые слова, содержащиеся в различных таблицах, считаются уникальными ключами. В данном случае число уникальных ключей равно 8.|  
|**опция предварительного расчета ранга** сервера|Оптимизация производительности запросов FREETEXTTABLE.|Когда опция установлена до 1, запросы FREETEXTTABLE, указанные *с top_n_by_rank* используют данные заранее вычисляемых рангов, хранящиеся в полнотекстовых каталогах.|Не поддерживается.|  
|[sp_fulltext_pendingchanges](/sql/relational-databases/system-stored-procedures/sp-fulltext-pendingchanges-transact-sql) при обновлении ключевой колонки|Обновляется полнотекстовый ключевой столбец на одной строке в таблице, содержащей две строки, а затем выполняется хранимая процедура sp_fulltext_pendingchanges.|Отображаются обе строки.|Отображается только одна строка.|  
...
|Стоп-слова, зависящие от локали|Запросы inlocale-специфических вариантов родительского языка, таких как бельгийский французский и канадский французский.|Варианты inlocale-специфические запросы обрабатываются компонентами (выключателями слов, стеблями и стоп-словами) их родительского языка. Например, компоненты для языка «Французский (Франция)» используются для синтаксического анализа языка «Французский (Бельгия)».|Стоп-слова для каждого идентификатора локали (LCID) должны добавляться явным образом. В частности, может потребоваться указать LCID для Бельгии, Канады и Франции.|  
...
|[DATABASEPROPERTYEX](/sql/t-sql/functions/databasepropertyex-transact-sql)|Использование свойства **IsFullTextEnabled.**|Параметр **IsFullTextEnabled** показывает, включен ли полный текстовый поиск в данной базе данных.|Значение этого столбца не учитывается. Полнотекстовый поиск всегда включен для пользовательских баз данных.|  
  
## <a name="see-also"></a>См. также:  
 [Изменение поведения в полнотекстовый поиск](../relational-databases/search/full-text-search.md)   
 [Полный текст поиска](../relational-databases/search/full-text-search.md)  
