---
title: Критические изменения полнотекстового поиска | Документы Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
|-------------|--------------|---------------------|----------------------------------------|  
|[Функции CONTAINSTABLE](/sql/relational-databases/system-functions/containstable-transact-sql) с определяемых пользователем типов (UDT)|Полнотекстовый ключ — это определяемый пользователем тип [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], например `MyType = char(1)`.|Возвращаемый ключ имеет тип, назначенный определяемому пользователем типу.<br /><br /> В примере, это будет **char(1)**.|Возвращаемый ключ имеет определяемый пользователем тип. В примере, это будет **MyType**.|  
|*top_n_by_rank* параметра (из CONTAINSTABLE и [FREETEXTTABLE](/sql/relational-databases/system-functions/freetexttable-transact-sql) [!INCLUDE[tsql](../includes/tsql-md.md)] инструкций)|*top_n_by_rank* запросы, используя 0 в качестве параметра.|Завершается с сообщением об ошибке, указывающим на то, что значение должно быть больше нуля.|Завершается успешно, возвращая 0 строк.|  
...
|**ItemCount**|Таблица содержит документы или типы столбцов, имеющие значение NULL.|Кроме индексированных документов, документы, имеют значение null или, имеющий значение null типы учитываются в **ItemCount** значение.|Только число индексированных документов, учитываются в **ItemCount** значение.|  
|Каталог **ItemCount**|Столбец BLOB с расширением NULL.|Он вычисляется в **ItemCount** каталога|Не учитывается в **ItemCount** каталога.|  
|**UniqueKeyCount**|Уникальный ключ запроса подсчет из каталога, например, две таблицы (Таблица1 и Таблица2) каждый из трех слов: слово1, слово2 и слово3.|**UniqueKeyCount** = 9. В следующей таблице показано, как получено это значение.<br /><br /> таблица1 = 3<br /><br /> EOF для полнотекстового индекса таблицы1 = 1<br /><br /> таблица2 = 3<br /><br /> EOF для полнотекстового индекса таблицы2 = 1<br /><br /> полнотекстовый каталог = 1|Для каждой таблицы **UniqueKeyCount** число уникальных ключевых слов + 1 (0xFF).  Одинаковые слова в > различных документах не считаются уникальными ключами.<br /><br /> Для создания каталога **UniqueKeyCount** равно сумме **UniqueKeyCount** каждой из таблиц в каталоге. Одинаковые слова, содержащиеся в различных таблицах, считаются уникальными ключами. В данном случае число уникальных ключей равно 8.|  
|**precompute rank** параметр уровня сервера|Оптимизация производительности запросов FREETEXTTABLE.|Если параметр имеет значение 1, запросы FREETEXTTABLE, заданные с помощью *top_n_by_rank* пользуются предварительно вычисленными рангами, хранящиеся в полнотекстовых каталогах.|Не поддерживается.|  
...
|Стоп-слова, зависящие от локали|Запросы, зависящие от inlocale варианты родительского языка, например Бельгийский французский "и" французский (Канада).|Варианты определенного inlocale запросы обрабатываются компонентами (средства разбиения по словам, парадигматические модули и Стоп-слова) родительского языка. Например, компоненты для языка «Французский (Франция)» используются для синтаксического анализа языка «Французский (Бельгия)».|Стоп-слова для каждого идентификатора локали (LCID) должны добавляться явным образом. В частности, может потребоваться указать LCID для Бельгии, Канады и Франции.|  
