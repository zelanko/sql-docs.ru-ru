...
-   Частота резервного копирования журналов: Частоты резервного копирования журнала — как раз и роста объема журнала. Например, резервная копия журнала создается каждые 2 часа, если только объем журнала не превысил в течение этих двух часов 5 МБ. Это относится ко всем реализациям: локальным, облачным или гибридным.  
...
8.  **Просмотрите конфигурацию по умолчанию расширенных событий:** просмотрите параметры расширенных событий, выполнив следующую инструкцию transact-SQL на реплике [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] с которой планирует резервное копирование. Обычно это параметр предпочтительной реплики для группы доступности, к которой принадлежит база данных.  
