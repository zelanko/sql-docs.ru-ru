...
4.  На каждой реплике [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] выполните конфигурацию базы данных с помощью хранимой процедуры **Smart-admin. sp_set_db_backup** .  
     ...
     **поведение после отработки отказа. продолжит работу и сохранит резервные копии и возможность восстановления после отработки отказа. [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] ** [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] После отработки отказа никакие особые действия не требуются.  
...
8.  **Проверьте конфигурацию расширенных событий по умолчанию:**  Проверьте конфигурацию расширенных событий, выполнив следующую инструкцию Transact-SQL в реплике [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] , которая использует для планирования резервного копирования. Обычно это параметр предпочтительной реплики для группы доступности, к которой принадлежит база данных.  
...
9. **Включить и настроить уведомление о состоянии работоспособности.** [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] содержит хранимую процедуру, создающую задание агента для отправки уведомлений по электронной почте об ошибках или предупреждениях, которые могут потребовать внимания.  Чтобы получать такие уведомления, необходимо включить запуск хранимой процедуры, которая создает задание агента SQL Server. Приведенные ниже шаги описывают процесс включения и настройки уведомлений по электронной почте.  
    ...
    2.  Настройте уведомления агента SQL Server для использования компонента Database Mail. Дополнительные сведения см. [в статье настройка агент SQL Server mail для использования Database Mail](../relational-databases/database-mail/configure-sql-server-agent-mail-to-use-database-mail.md).  
...
11. **Наблюдение за состоянием работоспособности:**  Вы можете отслеживать настроенные ранее уведомления по электронной почте или активно отслеживать события в журнале. Ниже приведены примеры инструкций Transact-SQL, которые используются для просмотра событий.  
