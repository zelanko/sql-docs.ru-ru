...
|||  
|-|-|  
|**Полная**|Во время полного заполнения таблицы записи индекса создаются для всех строк.|  
|**Добавочное**|Операция добавочного заполнения обновляет полнотекстовый индекс для строк, добавленных, удаленных или измененных после или во время последнего заполнения. Для выполнения добавочного заполнения базовая таблица должна включать столбец с типом данных `timestamp`.|  
