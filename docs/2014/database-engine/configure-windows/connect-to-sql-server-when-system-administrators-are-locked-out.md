...
5.  На вкладке **Параметры запуска** в поле **укажите параметр запуска** введите `-m` , а затем нажмите кнопку `Add`. (Это дефис, затем буква «m» в нижнем регистре.)  
...
6.  Щелкните `OK`и после сообщения для перезапуска щелкните правой кнопкой мыши имя сервера и выберите пункт **перезапустить**.  
...
8.  На стартовом экране Windows 8 щелкните правой кнопкой мыши пиктограмму для [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]. В нижней части экрана выберите **Запуск от имени администратора**. (Это передаст учетные данные администратора в SSMS.)  
    ...
    1.  Подключитесь с помощью обозревателя объектов, используя проверку подлинности Windows (которая включает учетные данные администратора). Разверните **Безопасность**, затем **Имена входа**и дважды щелкните имя входа. На странице **роли сервера** выберите `sysadmin`и нажмите кнопку `OK`.  
  
    2.  Вместо соединения с помощью обозревателя объектов подключитесь с помощью окна запросов, используя проверку подлинности Windows (которая включает учетные данные администратора). (Такой способ можно подключить, только если вы не подключались с помощью обозревателя объектов.) Выполните код, как показано ниже, чтобы добавить новое имя входа для проверки подлинности Windows, `sysadmin` которое является членом предопределенной роли сервера. В следующем примере создается пользователь с именем `CONTOSO\PatK`.  
...
11. На вкладке **Параметры запуска** в поле **существующие параметры** выберите `-m` и нажмите кнопку `Remove`.  
    ...
    > [!NOTE]  
    >  В некоторых предыдущих версиях [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] нет вкладки **Параметры запуска** . В этом случае на вкладке **Дополнительно** дважды щелкните **Параметры запуска**. Параметры открываются в очень маленьком окне. `;-m` Удалите добавленный ранее, а затем нажмите кнопку `OK`.  
  
12. Щелкните правой кнопкой мыши имя сервера и выберите пункт **Перезапустить**.  
  
 Теперь вы должны иметь возможность подключения к одной из учетных записей, которая теперь является членом `sysadmin` предопределенной роли сервера.  
