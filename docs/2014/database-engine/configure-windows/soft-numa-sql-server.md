---
title: Настройка SQL Server для использования программной архитектуры NUMA (SQL Server) | Документы Microsoft
ms.custom: ''
ms.date: 07/12/2016
...
Современные процессоры имеют несколько или множество ядер на одном сокете. Каждый сокет обычно представлен одним узлом NUMA. Ядро базы данных SQL Server секционирует разные внутренние структуры и потоки служб в узлы NUMA. Процессоры с 10 и более ядрами на сокет, с помощью программного обеспечения NUMA (soft-NUMA) распределение нагрузки между узлами NUMA оборудования обычно повышает масштабируемость и производительность.   
...
Начиная с SQL Server 2014 с пакетом обновления 2, каждый раз, когда сервер ядра базы данных обнаруживает более 8 физических процессоров, при запуске, программной архитектуры NUMA создаются автоматически, если флаг трассировки 8079 включен в качестве параметра запуска. Однопотоковое Hyper процессорных ядер, не учитываются при подсчете физических процессоров. Если число физических процессоров, которые обнаружены более чем 8 на сокет, службе ядра СУБД создаст программной архитектуры NUMA, в идеале содержат по 8 ядер, но можно перейти к 5 до 9 логических процессоров на узел. Размер аппаратного узла может быть ограничен маской сходства ЦП. Количество узлов NUMA не может превышать максимальное количество поддерживаемых узлов NUMA.

Без флага трассировки по умолчанию отключен программной архитектуры NUMA. Программная архитектура NUMA можно включить с помощью флага трассировки 8079. Чтобы изменение этого параметра вступило в силу, потребуется перезапустить ядро базы данных.

На следующем рисунке показан тип сведений об архитектуре soft-NUMA, вы увидите в журнале ошибок SQL Server, когда SQL Server обнаружит аппаратные узлы NUMA с более чем 8 логических процессоров, если установлен флаг трассировки 8079.

![Soft-NUMA](./media/soft-numa-sql-server/soft-numa.PNG)

## <a name="manual-soft-numa"></a>Создание архитектуры Soft-NUMA вручную
  
Чтобы настроить [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] для использования программной архитектуры NUMA вручную, необходимо изменить реестр, чтобы добавить маску схожести настройки узла. Маска программной архитектуры NUMA может быть задана как запись реестра, имеющая двоичный тип данных, тип данных DWORD (шестнадцатеричный или десятичный) или QWORD (шестнадцатеричный или десятичный). Чтобы настроить большее количество процессоров (больше чем первые 32), используйте значения реестра QWORD или BINARY. (Значения QWORD нельзя использовать до [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)].) Для настройки программной архитектуры NUMA необходимо перезапустить компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)].  
