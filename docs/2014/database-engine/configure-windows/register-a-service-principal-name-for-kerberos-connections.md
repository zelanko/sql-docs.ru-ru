...
-   *MSSQLSvc/FQDN*: [_порт_**|**_instanceName_], где:  
 ...
 **Примечание** . Если имя субъекта-службы уже существует, его необходимо удалить, прежде чем его можно будет зарегистрировать повторно. Это можно сделать, используя команду `setspn` с параметром `-D` . В следующих примерах демонстрируется регистрация вручную нового имени участника-службы, основанного на экземпляре. Для экземпляра по умолчанию следует использовать:  
