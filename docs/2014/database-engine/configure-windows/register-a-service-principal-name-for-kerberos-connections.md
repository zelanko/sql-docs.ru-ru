...
-   *MSSQLSvc/FQDN*: [*порт***|*** instancename*], где:  
...
-   *MSSQLSvc/FQDN*:*порт***|*** MSSQLSvc/FQDN*, где:  
...
##  <a name="Auto"></a> Автоматическая регистрация имени участника-службы  
 ...
 Для других соединений, поддерживающих протокол Kerberos имя участника-службы регистрируется в формате *MSSQLSvc /\<полное доменное имя >*:*\<имя_экземпляра >* для именованного экземпляра. Форматом регистрации экземпляра по умолчанию является *MSSQLSvc/\<FQDN>*.  
...
|--------------|---------------------------|  
|Имя участника-службы сопоставляется с правильной учетной записью домена, виртуальной учетной записью, управляемой учетной записью службы или встроенной учетной записью. Например, «Local System» или «NETWORK SERVICE».<br /><br /> Примечание: Исправьте означает, что сопоставляется с учетной записью зарегистрированного имени участника-службы — это учетная запись, с которой запущена служба SQL Server.|Локальные соединения используют протокол NTLM, удаленные — протокол Kerberos.|  
|Имя участника-службы является правильной учетной записью домена, виртуальной учетной записью, управляемой учетной записью службы или встроенной учетной записью.<br /><br /> Примечание: Исправьте означает, что сопоставляется с учетной записью зарегистрированного имени участника-службы — это учетная запись, с которой запущена служба SQL Server.|Локальные соединения используют протокол NTLM, удаленные — протокол Kerberos.|  
