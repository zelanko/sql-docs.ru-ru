---
title: Хранимые процедуры CLR | Документы Microsoft
ms.custom: ''
ms.date: 04/27/2017
...
```  
 ...
 После сборки, содержащей выше CLR хранимые процедуры был построен и созданы на сервере, следующие [!INCLUDE[tsql](../../includes/tsql-md.md)] используется для создания процедуры в базе данных и задает *сумма* как ВЫХОДНОЙ параметр.  
...
```  
 ...
 Обратите внимание, что *сумма* объявляется как `int` тип данных SQL Server и что *значение* параметр, определенный в хранимой процедуре CLR указан как `SqlInt32` тип данных CLR. Если вызывающая программа выполняет хранимую процедуру CLR, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] автоматически преобразует `SqlInt32` тип данных CLR, `int` [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] тип данных.  Дополнительные сведения о какой среде CLR типы данных могут и не может быть преобразован в разделе [сопоставления данных о параметрах CLR](../../relational-databases/clr-integration-database-objects-types-net-framework/mapping-clr-parameter-data.md).  
...
> [!NOTE]  
>  Сообщения и результирующие наборы в клиентском приложении получаются по-разному. Например [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] результирующих наборов отображаются в **результаты** , а сообщения отображаются в **сообщения** области.  
