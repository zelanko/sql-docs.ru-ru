...
# <a name="invoke-sqlcmd-cmdlet"></a>Invoke-Sqlcmd, командлет
  **Invoke-Sqlcmd** — это [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] командлет, который запускает скрипты, содержащие инструкции из языков[!INCLUDE[tsql](../includes/tsql-md.md)] (и XQuery) и команды, поддерживаемые служебной программой **sqlcmd** .  
 ...
 **Invoke-Sqlcmd** можно использовать для выполнения многих сценариев, которые можно запустить с помощью программы **sqlcmd** . Но командлет **Invoke-Sqlcmd** функционирует в среде Windows PowerShell, отличной от среды командной строки, в которой работает **sqlcmd** . Поведение **Invoke-Sqlcmd** было изменено в целях обеспечения работы в среде Windows PowerShell.  
 ...
 **Командлет Invoke-Sqlcmd** не инициализирует среду **sqlcmd** или переменные скрипта, такие как склкмддбнаме или склкмдворкстатион.  
  
 **Командлет Invoke-Sqlcmd** не отображает сообщения, такие как выходные данные инструкций Print, если не указан параметр Windows PowerShell **-verbose** Common. Пример:  
...
|[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]Пароль проверки подлинности.|-P|-Password|  
