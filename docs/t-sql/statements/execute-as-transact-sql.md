...
## <a name="using-with-no-revert"></a>Применение WITH NO REVERT  
 Если инструкция EXECUTE AS содержит необязательное предложение WITH NO REVERT, то контекст выполнения для сеанса нельзя сбросить с помощью инструкции REVERT или путем выполнения другой инструкции EXECUTE AS. Контекст, заданный инструкцией, остается до удаления сеанса.   Обратите внимание, что, если включена организация пула соединений, `sp_reset_connection` завершится со сбоем, а соединение будет прервано.  В журнал событий будет внесено следующее сообщение об ошибке:
 
> Соединение было удалено, поскольку участник, открывший его, впоследствии получил новый контекст безопасности и попытался сбросить соединение в рамках своего олицетворенного контекста безопасности. Такой сценарий не поддерживается. См. раздел «Общие сведения об олицетворении» в электронной документации.
