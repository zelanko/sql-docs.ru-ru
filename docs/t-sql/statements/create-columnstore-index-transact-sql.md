...
DROP_EXISTING = [OFF] | ON  
   DROP_EXISTING = ON удаляет существующий кластеризованный индекс columnstore и создает новый индекс columnstore.  
...
ONLINE = [ON | OFF]   
   Применимо к: [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)], только некластеризованные индексы columnstore.
Значение ON указывает, что некластеризованный индекс columnstore остается в сети и доступен во время создания новой копии индекса.

   Значение OFF указывает, что индекс недоступен для использования во время создания новой копии. Поскольку это только некластеризованный индекс, базовая таблица остается доступной, а только некластеризованный индекс columnstore не используется для обработки запросов до создания нового индекса. 
...
WHERE \<filter_expression> [ AND \<filter_expression> ] Применимо к: с [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] по [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].
