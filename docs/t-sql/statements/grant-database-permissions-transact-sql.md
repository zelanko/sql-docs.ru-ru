...
ALL — этот параметр предоставляет не все возможные разрешения. Аргумент ALL эквивалентен предоставлению следующих разрешений: BACKUP DATABASE, BACKUP LOG, CREATE DATABASE, CREATE DEFAULT, CREATE FUNCTION, CREATE PROCEDURE, CREATE RULE, CREATE TABLE и CREATE VIEW.
...
### <a name="d-granting-control-permission-to-a-database-user"></a>Г. Предоставление разрешения CONTROL пользователю базы данных
 ...
 В следующем примере пользователю `CONTROL` предоставляется разрешение `AdventureWorks2012` для базы данных `Sarah`. Пользователь должен существовать в базе данных, которая должна быть настроена в качестве контекста.
