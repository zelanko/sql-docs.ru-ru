...
-   `DECLARE` — инструкции, объявляющие локальные переменные и локальные курсоры.  

-   `SELECT` — инструкции, которые содержат списки выбора с выражениями, присваивающими значения локальным переменным.  
...
-   `INSERT`, `UPDATE` и `DELETE` — инструкции, которые изменяют локальные табличные переменные.  

-   `EXECUTE` — инструкции, вызывающие расширенные хранимые процедуры.  
...
-   Обработчик запросов [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] автоматически использует преимущества предложения `ORDER` в следующих случаях.  
    ...
    -   `ORDER BY` — предложения, совместимые с предложением `ORDER`.  
  
    -   Статистические выражения, где `GROUP BY` совместим с предложением `ORDER`.  
  
    -   `DISTINCT` — статистические выражения, в которых уникальные столбцы совместимы с предложением `ORDER`.  
