...
REORGANIZE выполняется в сети.  
  
Для индексов columnstore REORGANIZE сжимает каждую разностную группу строк CLOSED в columnstore в виде сжатой группы строк.  
...
-  Обратите внимание два различных параметра MAX_DURATION. Один относится к low_priority_lock_wait, а другой — к параметру RESUMABLE=ON.
...
-  Команду DDL с параметром RESUMABLE = ON невозможно выполнить внутри явной транзакции (не может быть частью блока begin tran... commit).
...
Для возобновляемых операций перестроения индексов отключены следующие функциональные возможности.
   ...
   -    с параметром RESUMABLE = ON невозможно выполнить внутри явной транзакции (не может быть частью блока begin tran... commit).
