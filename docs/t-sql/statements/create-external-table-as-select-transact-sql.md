...
При выполнении инструкции CREATE EXTERNAL TABLE AS SELECT с записью в файлы Parquet или ORC возникнут ошибки, которые могут включать отклоненные записи, если в данных присутствуют следующие символы:
...
Чтобы использовать инструкцию CREATE EXTERNAL TABLE AS SELECT с такими символами, сначала выполните для данных инструкцию CREATE EXTERNAL TABLE AS SELECT, чтобы создать текстовые файлы с разделителями, а затем преобразуйте их в файлы Parquet или ORC с помощью внешнего инструмента.
