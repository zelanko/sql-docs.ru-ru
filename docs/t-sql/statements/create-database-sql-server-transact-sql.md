...
-   Первичная файловая группа с файлами `Spri1`_`dat` и `Spri2` \_ `dat`. Для этих файлов задана величина приращения FILEGROWTH, равная `15%`.  
...
### <a name="f-creating-a-database-snapshot"></a>Е. Создание моментального снимка базы данных.  
 В следующем примере создается моментальный снимок базы данных `sales`_`snapshot0600`. Моментальный снимок базы данных предназначен только для чтения, поэтому нельзя задать файл журнала. В соответствии с синтаксисом задается каждый файл базы данных-источника, а файловые группы не указываются.  
