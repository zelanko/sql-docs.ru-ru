...
\<backup_device> Указывает логическое или физическое устройство резервного копирования, которое будет использовано в операции резервного копирования.  
...
MIRROR TO \<backup_device> [ **,**...*n* ] Указывает набор от одного до трех устройств резервного копирования, которые будут зеркалами для устройств резервного копирования, описанных в предложении TO. В предложении MIRROR TO должен быть указан тот же тип и то же количество устройств резервного копирования, что и в предложении TO. Максимальное число предложений MIRROR TO — три.  
...
> * Используется резервное копирование на URL-адрес; в этом случае `MAXTRANSFERSIZE` по умолчанию меняется на 1 048 576 (1 МБ), меньшее значение принудительно не используется.
