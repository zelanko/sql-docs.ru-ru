...
SIZE_BASED_CLEANUP_MODE определяет, будет ли автоматически активирована очистка, когда общий объем данных приблизится к верхней границе ограничения. OFF — очистка на основе размера не будет автоматически активирована.
...
ALL записывает все запросы. ALL — значение по умолчанию. Это значение конфигурации по умолчанию для [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)].
...
Значение OFF не повышает уровень изоляции транзакции для интерпретированных операций [!INCLUDE[tsql](../../includes/tsql-md.md)] в таблицах, оптимизированных для памяти.
...
Если указано значение OFF, NULL не является значением по умолчанию.
...
Если указано значение OFF, вы можете определить состояние этого параметра, проверив значение столбца is_recursive_triggers_on в представлении каталога sys.databases. Вы также можете определить состояние с помощью проверки свойства IsRecursiveTriggersEnabled функции DATABASEPROPERTYEX.
