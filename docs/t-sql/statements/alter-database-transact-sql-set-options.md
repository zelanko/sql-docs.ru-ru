...
В случае повышения производительности [!INCLUDE[ssde_md](../../includes/ssde_md.md)] будет продолжать использовать последний известный удачный план. Если повышений производительности нет, [!INCLUDE[ssde_md](../../includes/ssde_md.md)] создаст новый план запроса. Инструкция завершится ошибкой, если хранилище запросов не включено или не находится в режиме *чтения и записи*.
...
Отключение Stretch Database не приводит к удалению удаленной базы данных. Чтобы удалить удаленную базу данных, воспользуйтесь порталом Azure.
...
После включения кэширования результирующих наборов для базы данных результаты сохраняются в кэше для всех запросов, за исключением запросов с недетерминированными функциями, такими как DateTime.Now(), и запросов, возвращающих данные со строками размером свыше 64 КБ, пока кэш не заполнится.   
...
В следующем примере включается хранилище запросов.
...
### <a name="b-enable-result-set-caching-for-a-database"></a>Б. Включение кэширования результирующих наборов для базы данных
...
### <a name="c-disable-result-set-caching-for-a-database"></a>В. Выключение кэширования результирующих наборов для базы данных
...
### <a name="e-check-for-number-of-queries-with-result-set-cache-hit-and-cache-miss"></a>Д. Проверка количества запросов результирующих наборов с попаданием в кэше и промахом кэша
...
### <a name="f-check-for-result-set-cache-hit-or-cache-miss-for-a-query"></a>Е. Проверка попадания в кэше и промаха кэша результирующих наборов для запроса
...
### <a name="g-check-for-all-queries-with-result-set-cache-hits"></a>Ж. Проверка всех запросов с попаданием в кэше результирующих наборов
...
### <a name="h-enable-read_committed_snapshot-option-for-a-database"></a>З. Включение параметра Read_Committed_Snapshot для базы данных
