...
PolyBase использует только пользовательский формат для импорта данных. Он не использует пользовательский формат для записи данных во внешний файл.
 ...
 Если DATE_FORMAT не указан или является пустой строкой, PolyBase использует следующие форматы по умолчанию:
...
-   Год, месяц и день могут иметь различные форматы и заказов. В таблице показаны только **гмд** формат. Месяц может иметь цифры 1 или 2 или 3 символов. День могут иметь 1 или 2 цифрами. Год может содержать два или четыре цифры.
...
|SmallDateTime|DATE_FORMAT = «гггг мм дд чч: мм»|В дополнение к год, месяц и день, этот формат включает 00-23 часа 00-59 минут.|  
|SmallDateTime|DATE_FORMAT = «гггг мм дд hh:mmtt»|В дополнение к год, месяц и день, этот формат включает 00 11 часов, минут 00-59, без секунд и AM, am, PM или pm.|  
...
|datetime2|DATE_FORMAT = «yyyy-MM-dd HH:mm:ss.fffffff»|В дополнение к год, месяц и день, этот формат включает 00-23 часа 00-59 минут 00-59 секунд до 7 цифр, для миллисекунд.|  
|datetime2|DATE_FORMAT = «гггг мм дд hh:mm:ss.ffffffftt»|В дополнение к год, месяц и день, этот формат включает 00 11 часов 00-59 минут 00-59 сек., 7 цифр для миллисекунд и AM, am, PM или pm.|  
|DateTimeOffset|DATE_FORMAT = «yyyy-MM-dd HH:mm:ss.fffffff zzz»|В дополнение к год, месяц и день, этот формат включает 00-23 часа 00-59 минут 00-59 секунд до 7 цифр, для миллисекунд и смещения часового пояса, которая помещается во входном файле, как `{+&#124;-}HH:ss`. Например, со времени Лос-Анджелес, без перехода на летнее экономия — 8 часов раньше времени UTC, значение -08:00 во входном файле указывает часовой пояс для Лос-Анджелес.|  
|DateTimeOffset|DATE_FORMAT = «гггг мм дд hh:mm:ss.ffffffftt zzz»|В дополнение к год, месяц и день, этот формат включает 00 11 часов 00-59 минут 00-59 секунд, 7 цифр для миллисекунд (AM, am, PM или pm) и смещение часового пояса. См. в описании в предыдущей строке.|  
...
-   Чтобы указать месяц как текст, используйте три или более символов. Месяцы 1 или 2 знаков интерпретируются как число.
...
-   Буквы «zzz» указать смещение часового пояса для текущего часового пояса в системе в формате {+ |-} HH:ss].
 ...
 USE_TYPE_DEFAULT = {TRUE | **FALSE** } указывает способ обработки отсутствующих значений в текстовых файлов с разделителями, когда PolyBase получает данные из текстового файла.
  
 Значение TRUE, если получение данных из текстового файла, хранение каждого отсутствующего значения, используя значение по умолчанию для типа данных соответствующего столбца в определении внешней таблицы. Например Замените отсутствующие значения с:  
...
-   1900-01-01, если столбец является столбцом дат.
 ...
 Хранилище FALSE все отсутствующие значения NULL. ПУСТЫЕ значения, сохраненные с помощью слово NULL в файле с разделителями, импортируются как строку «NULL».
  
   Кодирование = {'UTF8' | «UTF16}» в хранилище данных SQL Azure, PolyBase можно прочитать UTF8 и UTF16-LE кодировке с разделителями текстовых файлов. В SQL Server и PDW, PolyBase не поддерживает чтение UTF16 файлы в кодировке.
  
 DATA_COMPRESSION = *data_compression_method* определяет метод сжатия данных для внешних данных. Если DATA_COMPRESSION не указан, значение по умолчанию — несжатых данных.
...
-   СЖАТИЕ данных = «org.apache.hadoop.io.compress.SnappyCodec»
 ...
 Тип формата файлов PARQUET поддерживает методы сжатия следующее:
...
## <a name="general-remarks"></a>Общие замечания
 ...
 Данные хранятся в одном из форматов сжатые, PolyBase сначала распаковывает данные перед возвратом записей данных.
  
## <a name="limitations-and-restrictions"></a>Ограничения
  
 Разделитель строк в файлы с разделителями запятыми должны поддерживаться Hadoop LineRecordReader. То есть должен быть «\r», «\n» или «\r\n». Эти разделители не могут быть изменены пользователем.
 ...
 Из-за ограничения на количество файлов во внешней таблице рекомендуется хранить не более 30 000 файлы в корневой и вложенные папки из расположения внешнего файла. Кроме того рекомендуется оставлять число вложенные папки в корневом каталоге небольшому числу. При обращении к слишком много файлов, может возникнуть исключение нехватки памяти в виртуальной машине Java.
  ...
  При экспорте Экспорт данных в Hadoop или хранилище BLOB-объектов Azure через PolyBase, только данные, не столбец names(metadata), определенный в команду CREATE EXTERNAL TABLE.
...
### <a name="a-create-a-delimitedtext-external-file-format"></a>A. Создайте формат внешнего файла DELIMITEDTEXT  
 В этом примере создается с именем формата внешних файлов *textdelimited1* для файла с разделителями текста. Параметры, указанные для ФОРМАТА\_указывают, что поля в файле должны разделяться с помощью знака вертикальной черты "|". Текстовый файл также сжаты с использованием кодека Gzip. Если данные\_СЖАТИЯ не указано, текстовом файле без сжатия.
