...
[!INCLUDE[ssMIlimitation](../../includes/sql-db-mi-limitation.md)]
 ...
 Дополнительные сведения о сценариях восстановления [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] см. в статье [Обзор процессов восстановления (SQL Server)](../../relational-databases/backup-restore/restore-and-recovery-overview-sql-server.md).  Дополнительные сведения об аргументах см. в разделе [Аргументы инструкции RESTORE (Transact-SQL)](../../t-sql/statements/restore-statements-arguments-transact-sql.md).   При восстановлении базы данных из другого экземпляра примите во внимание сведения из раздела [Управление метаданными при обеспечении доступности базы данных на другом экземпляре сервера (SQL Server)](../../relational-databases/databases/manage-metadata-when-making-a-database-available-on-another-server.md).
  
> **ПРИМЕЧАНИЕ.** Дополнительные сведения о восстановлении из службы хранилища BLOB-объектов Microsoft Azure см. в разделе [Резервное копирование и восстановление SQL Server с помощью службы хранилища BLOB-объектов Microsoft Azure](../../relational-databases/backup-restore/sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md).  
...
## <a name="arguments"></a>Аргументы  
 Описание аргументов см. в разделе [Аргументы инструкции RESTORE (Transact-SQL)](../../t-sql/statements/restore-statements-arguments-transact-sql.md).  
...
-   Восстановление в сети  
    ...
    > **ПРИМЕЧАНИЕ.** Оперативное восстановление допускается только в выпуске [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Enterprise.  
    ...
    > **ПРИМЕЧАНИЕ.** Оперативное восстановление может использовать [отложенные транзакции](../../relational-databases/backup-restore/deferred-transactions-sql-server.md).  
...
> **ПРИМЕЧАНИЕ.** Резервная копия [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] не может быть восстановлена в более ранней версии [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] по сравнению с версией, которая использовалась для создания резервной копии.  
...
## <a name="general-remarks"></a>Общие замечания  
 ...
 Дополнительные сведения о восстановлении базы данных см. в разделе [Обзор процессов восстановления (SQL Server)](../../relational-databases/backup-restore/restore-and-recovery-overview-sql-server.md).  
...
## <a name="general-remarks---sql-database-managed-instance"></a>Общие примечания — управляемый экземпляр базы данных SQL
...
### <a name="restoring-a-database-enabled-for-vardecimal-storage"></a>Восстановление базы данных с включенным форматом хранения vardecimal  
 Резервное копирование и восстановление правильно работают с форматом хранения **vardecimal**. Дополнительные сведения о формате хранения **vardecimal** см. в разделе [sp_db_vardecimal_storage_format (Transact-SQL)](../../relational-databases/system-stored-procedures/sp-db-vardecimal-storage-format-transact-sql.md).  
...
> **ПРИМЕЧАНИЕ.** Дополнительные примеры см. в разделах руководства по восстановлению, перечисленных в статье [Обзор процессов восстановления (SQL Server)](../../relational-databases/backup-restore/restore-and-recovery-overview-sql-server.md).  
...
> **ПРИМЕЧАНИЕ.** Для базы данных, использующей модель полного восстановления или модель восстановления с неполным протоколированием, в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] в большинстве случаев необходимо создать резервную копию заключительного фрагмента журнала перед восстановлением базы данных. Дополнительные сведения см. в статье [Резервные копии заключительного фрагмента журнала (SQL Server)](../../relational-databases/backup-restore/tail-log-backups-sql-server.md).  
...
> **ПРИМЕЧАНИЕ.** Восстановление выполняется как отдельный шаг, чтобы предотвратить слишком раннее восстановление, до того как будут восстановлены все резервные копии журналов.  
...
> **ПРИМЕЧАНИЕ.** При восстановлении до моментального снимка удаляются все полнотекстовые каталоги.  
...
Полная резервная копия базы данных в `mysecondcontainer` из `Sales` будет восстановлена в локальное хранилище.  `Sales` в настоящее время не существует на сервере.
```
RESTORE DATABASE Sales
  FROM URL = 'https://mystorageaccount.blob.core.windows.net/mysecondcontainer/Sales.bak'   
...
## <a name="much-more-information"></a>Дополнительные сведения  
