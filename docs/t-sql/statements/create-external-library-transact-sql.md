...
Библиотеки добавляются в базу данных с областью, ограниченной пользователем. То есть имена библиотек считаются уникальными в контексте определенного пользователя или владельца, и имена библиотек должны быть уникальными для каждого пользователя. Например, два пользователя **RUser1** и **RUser2** могут загрузить библиотеку R `ggplot2` индивидуально и по отдельности.
...
Нельзя использовать большие двоичные объекты в качестве источника данных в выпуске SQL Server 2017.

## <a name="permissions"></a>Разрешения

Требуется разрешение `CREATE ANY EXTERNAL LIBRARY`.
...
Поскольку может быть трудно определить все зависимости при осмотре манифеста пакета, рекомендуется использовать такой пакет, как [miniCRAN](https://cran.r-project.org/web/packages/miniCRAN/index.html) или [iGraph](http://igraph.org/redirect.html), для поиска всех пакетов, необходимых для успешного завершения установки.
...
Если у вас нет возможности сохранить файлы пакета на сервере, можно также передать содержимое пакетов в переменной. В следующем примере библиотека создается путем передачи битов в виде шестнадцатеричного литерала.

```SQL
CREATE EXTERNAL LIBRARY customLibrary FROM (CONTENT = 0xabc123) WITH (LANGUAGE = 'R');
```

Здесь шестнадцатеричные значения были усечены для удобства чтения.
...
Чтобы удалить библиотеку пакета из базы данных, выполните инструкцию:
...
> [!NOTE]
> В отличие от других инструкций `DROP` в [!INCLUDE[ssnoversion](../../includes/ssnoversion.md)], эта инструкция поддерживает дополнительный параметр, который задает права пользователя. Этот параметр позволяет пользователям с ролями владельца удалять библиотеки, загруженные обычными пользователями.
