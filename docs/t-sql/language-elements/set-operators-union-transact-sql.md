...
В третьем примере с первым оператором `ALL` используется ключевое слово `UNION`, а во втором предложении `UNION` вместо ключевого слова `ALL` используются скобки. Второе предложение `UNION` обрабатывается первым, так как оно заключено в скобки. Оно возвращает только неповторяющиеся строки из таблицы, так как параметр `ALL` не используется и повторяющиеся строки исключаются. Полученные строки объединяются с результатами выполнения первой инструкции `SELECT` с помощью ключевого слова `UNION ALL`. В данном случае повторяющиеся строки двух множеств не удаляются.  
