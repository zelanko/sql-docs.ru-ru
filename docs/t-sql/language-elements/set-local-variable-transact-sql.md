 ...
 *cursor_name*  
 ...
 Определяет обновляемые столбцы в курсоре. Если OF *column_name* [**,**...*n*] указано, только перечисленные столбцы позволяют вносить изменения. Если список не предоставляется, все столбцы могут быть обновлены, если только курсор не был определен как READ_ONLY.  
