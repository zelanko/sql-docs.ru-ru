...
> [!NOTE]
> Имена некоторых системных функций Transact-SQL начинаются с двух символов *@* (@@). Хотя в предыдущих версиях [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] функции @@functions называются глобальными переменными, они не являются переменными и ведут себя по-другому. Функции, помеченные символами @@functions, являются системными, и синтаксис их использования соответствует правилам вызова функций.
