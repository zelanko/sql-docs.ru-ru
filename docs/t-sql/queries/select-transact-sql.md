...
|[GROUP BY](../../t-sql/queries/select-group-by-transact-sql.md)|[Предложение OPTION](../../t-sql/queries/option-clause-transact-sql.md)|  
 ...
 Четырехкомпонентное имя, использующее функцию OPENDATASOURCE в качестве части имени сервера, может служить в качестве исходной таблицы в любом месте инструкции SELECT, где может появляться имя таблицы. Четырехкомпонентное имя невозможно указать для [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)].  
