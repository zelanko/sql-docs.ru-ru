---
title: "OVER, предложение (Transact-SQL) | Документы Microsoft"
ms.custom: 
ms.date: 08/11/2017
 ...
 *value_expression*  
 Определяет столбец, по которому секционируется набор строк. *value_expression* может ссылаться только на столбцы, которые становятся доступными с помощью предложения FROM. *value_expression* не может ссылаться на выражения или псевдонимы в списке выбора. *value_expression* может быть выражением столбца, скалярным вложенным запросом, скалярной функции или определяемой пользователем переменной.  
...
**Область применения**: начиная с [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Применяется вместе с \<спецификация неподписанного значения > для указания количества строк или значений перед текущей строкой. Эта спецификация не допускается в предложении RANGE.  
...
**Область применения**: начиная с [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]. 
 ...
 МЕЖДУ \<граница рамки окна > AND \<граница рамки окна >  
**Область применения**: начиная с [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]. 
  
 Используется совместно с предложением ROWS или RANGE для указания нижней (начальной) или верхней (конечной) граничной точки окна. \<Граница рамки окна > определяет граничную начальную точку и \<граница рамки окна > определяет граничную конечную точку. Верхняя граница не может быть меньше нижней границы.  
...
**Область применения**: начиная с [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]. 
 ...
 \<Спецификация неподписанного значения > СЛЕДУЮЩИЕ  
 Применяется вместе с \<спецификация неподписанного значения > для обозначения числа строк или значений после текущей строки. Когда \<спецификация неподписанного значения > ниже указан в качестве начальной точки, конечная точка должна быть \<спецификация неподписанного значения > СЛЕДУЮЩИЕ. Например, ROWS BETWEEN 2 FOLLOWING AND 10 FOLLOWING определяет, что окно начинается на второй строке после текущей и заканчивается на десятой строке после текущей строки. Эта спецификация не допускается в предложении RANGE.  
...
Если указан ROWS или RANGE и \<предшествующая рамка окна > используется для \<экстента рамки окна > (короткий синтаксис), а затем эта спецификация используется для начальной точки границы рамки окна и CURRENT ROW используется для конца границ точка. Например «ROWS 5 PRECEDING» равно «ROWS BETWEEN 5 PRECEDING AND CURRENT ROW».  
...
## <a name="limitations-and-restrictions"></a>Ограничения  
 ...
 ДИАПАЗОН не может использоваться с \<спецификация неподписанного значения > PRECEDING или \<спецификация неподписанного значения > СЛЕДУЮЩИЕ.  
  
 В зависимости от ранжирования, Агрегатная или аналитическая функция, используемая с предложением OVER \<предложение ORDER BY > и/или \<ROWS и RANGE предложение > Возможно, не поддерживается.  
...
**Область применения**: начиная с [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] до [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 В следующем примере предложение ROWS для определения окна, по которому вычисляется как текущую строку и *N* строк, которые (1 строка в этом примере).  
...
### <a name="f-using-the-over-clause-with-aggregate-functions"></a>Е. Использование предложения OVER с агрегатными функциями  
 В следующих примерах показано использование предложения OVER с агрегатными функциями. В этом примере использование предложения OVER является более эффективным, чем использование вложенных запросов.  
...
```  
 ...
 В следующем примере показано использование предложения OVER с агрегатной функцией в вычисляемом значении. Обратите внимание, что статистические выражения для вычисления, `SalesOrderNumber` и процент общего заказа на продажу вычисляется для каждой строки каждого `SalesOrderNumber`.  
...
```  
 ...
 — Первый запуск этот результирующий набор:  
