---
title: "TOP (Transact-SQL) | Документы Microsoft"
ms.custom: 
ms.date: 03/16/2017
...
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]
  ...
  Ограничивает число строк, возвращаемых в результирующем наборе запроса до заданного числа или процентного значения в [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]. Если предложение TOP используется в сочетании с предложением ORDER BY, результирующий набор ограничивается до первого *N* строками отсортированного результата; в противном случае он возвращает первый *N* строк в неопределенном порядке. Это предложение позволяет указать число строк, возвращаемых инструкцией SELECT или обработанных инструкциями INSERT, UPDATE, MERGE и DELETE.  
 ...
 *expression*  
 Числовое выражение, определяющее количество возвращаемых строк. *выражение* неявно преобразуется в **float** значение, если ПРОЦЕНТ указанного; в противном случае, он преобразуется в **bigint**.  
  
 PERCENT  
 Указывает, что запрос возвращает только первый *выражение* процентов строк из результирующего набора. Дробные значения округляются до следующего целого числа.  
  
 WITH TIES  
 Используется, если требуется вернуть две или более строки, которые совместно занимают последнее место в ограниченном результирующем наборе. Должен использоваться с **ORDER BY** предложения. **WITH TIES** может привести к более строк должны быть возвращены, чем значение, указанное в *выражение*. Например если *выражение* равно 5, но 2 дополнительные строки соответствуют значениям **ORDER BY** столбцов в строке 5, результирующий набор будет содержать 7 строк.  
...
## <a name="interoperability"></a>Совместимость  
 Выражение TOP в запросе не влияет на инструкции, которые могут быть выполнены из-за срабатывания триггера. **Вставлены** и **удалены** таблицы в триггерах возвратит только строки, которые реально обработаны инструкциями INSERT, UPDATE, MERGE или DELETE. Например, если триггер INSERT сработал в результате выполнения инструкции INSERT с предложением TOP,  
...
|--------------|------------------------------|  
|[Базовый синтаксис](#BasicSyntax)|TOP • PERCENT|  
|[Включить равные значения](#tie)|WITH TIES|  
|[Ограничения строк, затронутых инструкцией DELETE, INSERT или UPDATE](#DML)|DELETE • INSERT • UPDATE|  
...
###  <a name="DML"></a>Ограничения строк, затронутых инструкцией DELETE, INSERT или UPDATE  
...
## <a name="examples-includesssdwfullincludessssdwfull-mdmd-and-includesspdwincludessspdw-mdmd"></a>Примеры: [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] и[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]  
 В следующем примере возвращается 31 верхних строк, соответствующих критериям запроса. **ORDER BY** предложение используется для обеспечения возврата 31 строки являются первыми 31 строк на основании алфавитный порядок `LastName` столбца.  
  
 С помощью **ВЕРХНЕЙ** без указания WITH ties.  
...
```  
 ...
 Результат: 31 строки возвращаются.  
  
 Использование ключевого слова TOP, указав WITH TIES.  
