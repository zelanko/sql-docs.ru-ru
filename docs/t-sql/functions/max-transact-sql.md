---
title: "MAX (Transact-SQL) | Документы Microsoft"
ms.custom: 
ms.date: 08/23/2017
 ...
 *expression*  
 Может быть константой, именем столбца или функцией, а также любым сочетанием арифметических, побитовых и строковых операторов. MAX можно использовать с **числовое**, **символ**, **uniqueidentifier**, и **datetime** столбцов, а не с **бит**  столбцов. Агрегатные функции и вложенные запросы не допускаются.  
...
### <a name="b-using-the-over-clause"></a>Б. Использование предложения OVER  
 В следующем примере функции MIN, MAX, AVG и COUNT с предложением OVER для получения статистических значений для каждого из отделов в `HumanResources.Department` в таблицу [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] базы данных.  
...
### <a name="c-using-max-with-character-data"></a>В. С помощью MAX с символьными данными   
Следующий пример возвращает имя базы данных, которая сортирует как фамилии в алфавитном порядке. В примере используется `WHERE database_id < 5`, которые следует учитывать только системных баз данных.  
```sql   
SELECT MAX(name) FROM sys.databases WHERE database_id < 5;
```
Последний системной базы данных является `tempdb`.  
