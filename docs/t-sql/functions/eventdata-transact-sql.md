...
## <a name="schemas-returned"></a>Возвращаемые схемы  
 Функция EVENTDATA возвращает значение типа **xml**. По умолчанию определение схемы для всех событий устанавливается в следующем каталоге: [!INCLUDE[ssInstallPath](../../includes/ssinstallpath-md.md)]Tools\Binn\schemas\sqlserver\2006\11\events\events.xsd.  
  
 Кроме того, схема событий опубликована на веб-странице [Схемы XML Microsoft SQL Server](http://go.microsoft.com/fwlink/?LinkID=31850).  
...
> [!NOTE]  
>  При запросе элемента `\<TSQLCommand>` с помощью представления **В виде сетки** в среде [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] символы разрыва строк в тексте команды не отображаются. Используйте вместо этого представление **В виде текста**.  
...
> [!NOTE]  
>  Для возврата данных события рекомендуется использовать метод XQuery **value()**, а не **query()**. Метод **query()** возвращает XML-данные, содержащие символы возврата каретки и переноса строки (CR/LF), экранированные амперсандом, а метод **value()** не отображает эти символы.  
