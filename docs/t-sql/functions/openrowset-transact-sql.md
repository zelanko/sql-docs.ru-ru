...
*catalog*. Имя каталога или базы данных, в которых хранится указанный объект.

*schema*. Имя схемы или владелец указанного объекта.
...
*data_file*. Полный путь к файлу данных, данные из которого копируются в целевую таблицу.
...
\<bulk_options>. Указывает аргументы для параметра BULK.
...
FIRSTROW =*first_row*. Указывает номер первой строки для загрузки. Значение по умолчанию равно 1. Значение по умолчанию — первая строка указанного файла данных. Номера строк определяются с помощью подсчета признаков конца строки. Значения аргумента FIRSTROW начинаются с 1.
...
SINGLE_BLOB. Возвращает содержимое файла *data_file* в виде набора строк с одной строкой и одним столбцом типа **varbinary(max)** .
