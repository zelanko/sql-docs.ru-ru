...
### <a name="a-using-a-simple-select-statement-with-openxml"></a>A. Использование простой инструкции SELECT с OPENXML  
 ...
 Значение аргумента *flag* устанавливается равным `1`. Это свидетельствует об **атрибутивной модели** сопоставления. Поэтому, XML-атрибуты сопоставляются столбцам набора строк. Аргумент *rowpattern*, заданный как `/ROOT/Customer`, указывает, какие узлы `<Customers>` должны обрабатываться.  
