---
title: "Функция GROUPING_ID (Transact-SQL) | Документы Microsoft"
ms.custom: 
ms.date: 03/03/2017
...
[!INCLUDE[tsql-appliesto-ss2008-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-asdb-xxxx-xxx-md.md)]
  ...
  Представляет собой функцию, которая вычисляет уровень группирования. Функция GROUPING_ID может использоваться только в инструкции SELECT \<выберите > список, HAVING, или ORDER BY предложения, если указан GROUP BY.  
...
## <a name="arguments"></a>Аргументы  
 \<column_expression >  
 — *Column_expression* в [GROUP BY](../../t-sql/queries/select-group-by-transact-sql.md) предложения.  
  
## <a name="return-type"></a>Тип возвращаемых данных  
 **int**  
  
## <a name="remarks"></a>Замечания  
 GROUPING_ID \<column_expression > должно точно соответствовать выражению в списке GROUP BY. Например, если группируются с помощью функции DATEPART (yyyy, \< *имя столбца*>), использовать функцию GROUPING_ID (DATEPART (yyyy, \< *имя столбца*>)); или если группируются по \< *имя столбца*>, использовать функцию GROUPING_ID (\<*имя столбца*>).  
  
## <a name="comparing-groupingid--to-grouping-"></a>Сравнение функций GROUPING_ID () и GROUPING ()  
 Функция GROUPING_ID (\<column_expression > [ **,**...  *n*  ]) вводит эквивалента GROUPING (\<column_expression >) на каждый столбец в своем списке столбцов в каждой строке вывода как строку единиц и нулей. Функция GROUPING_ID интерпретирует эту строку как двоичное число и выполняет возврат эквивалентного целого числа. Например, рассмотрим следующую инструкцию: `SELECT a, b, c, SUM(d),``GROUPING_ID(a,b,c)``FROM T GROUP BY <group by list>`. В следующей таблице показаны входные и выходные значения функции GROUPING_ID ().  
...
## <a name="technical-definition-of-groupingid-"></a>Техническое определение функции GROUPING_ID ()  
 Каждый аргумент функции GROUPING_ID должен быть элементом списка GROUP BY. Функция GROUPING_ID () возвращает **целое** растрового изображения, которой N самых младших битов могут быть установлены. Установленный **бит** указывает соответствующий аргумент не является столбцом группирования для указанной выходной строки. Самый младший **бит** соответствует аргументу N и n-1<sup>й</sup> младший **бит** соответствует аргументу 1.  
  
## <a name="groupingid--equivalents"></a>Функции, эквивалентные функции GROUPING_ID ()  
 Для к одиночному запросу группирования ГРУППИРОВАНИЕ (\<column_expression >) эквивалентна GROUPING_ID (\<column_expression >), и обе функции возвращают 0.  
...
### <a name="c-using-groupingid--with-rollup-and-cube-to-identify-grouping-levels"></a>В. Использование функции GROUPING_ID () с операторами ROLLUP и CUBE для обозначения уровней группирования  
 ...
 В этих примерах функция `GROUPING_ID()` используется для создания значения, обозначающего уровень группирования, в каждой строке столбца `Grouping Level`. Уровни группирования не всегда последовательный список целых чисел, которые начинаются с 1 (0 1, 2... *n*).  
