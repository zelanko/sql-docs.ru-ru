...
В следующем примере используется `DB`_`ID` для возврата идентификатора [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] базы данных в системной функции `sys.dm_db` \_ `index` \_ `operational` \_ `stats`. Эта функция принимает идентификатор базы данных в качестве первого параметра.
