---
title: "Инструкция DBCC SHOWCONTIG (Transact-SQL) | Документы Microsoft"
ms.custom: 
ms.date: 07/17/2017
...
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepFutureDontUse](../../includes/ssnotedepfuturedontuse-md.md)]Используйте [sys.dm_db_index_physical_stats](../../relational-databases/system-dynamic-management-views/sys-dm-db-index-physical-stats-transact-sql.md) вместо него.  
...
|**Переключения экстентов**|Количество раз, когда инструкция DBCC в процессе обхода страниц таблицы или индекса переходила с одного экстента на другой.|  
|**Средн. Страниц на экстент**|Количество страниц на экстент в цепочке страниц.|  
|**Плотность просмотра [лучший счетчик: фактический счетчик]**|Процентное отношение. Это отношение **Best Count** для **Actual Count**. Значение 100 указывает на отсутствие фрагментации; значение меньше 100 указывает на наличие фрагментации.<br /><br /> **Лучший счетчик** является идеальное количество изменений экстентов в случае полной непрерывности. **Фактическое количество** фактическое число изменений экстентов.|  
|**Логическая фрагментация просмотра**|Процент неупорядоченных страниц, определенный с помощью просмотра конечных страниц индекса. Это значение несущественно для куч. Из внеочередной страница является страницей, для которой следующая физическая страница, выделенная для индекса не страницы, на который указывает неупорядоченной*e* указатель в текущей конечной странице.|  
|**Проверка фрагментации экстентов**|Процент неупорядоченных экстентов при просмотре конечных страниц индекса. Это значение несущественно для куч. Неупорядоченным называется такой экстент, для которого экстент, содержащий текущую страницу индекса, не расположен физически непосредственно за экстентом, содержащим предыдущую страницу индекса.<br /><br /> Примечание: Этот номер не имеет смысла, если индекс охватывает несколько файлов.|  
|**Средн. Байт на страницу**|Среднее количество свободных байтов на просмотренных страницах. Чем больше это число, тем меньше заполнены страницы. Предпочтительны более низкие показатели, если в индексе не будет большого количества случайных вставок. На этот показатель влияет также размер строк: большой размер строки может привести к его увеличению.|  
|**Средн. Плотность страницы (полная)**|Средняя плотность страницы в процентах. Этот показатель учитывает размер строк. Поэтому он более точно отражает плотность заполнения страниц. Чем больше процентное отношение, тем лучше.|  
  
Когда *table_id* и FAST указано, инструкция DBCC SHOWCONTIG возвращает результирующий набор со только следующие столбцы.
-   **Просмотрено страниц**  
-   **Переключения экстентов**  
-   **Плотность просмотра [лучший счетчик: фактический счетчик]**  
-   **Проверка фрагментации экстентов**  
-   **Логическая фрагментация просмотра**  
...
|**Экстенты**|Количество экстентов на данном уровне индекса или во всей куче.|  
...
|**ScanDensity**|Процентное отношение. Это отношение **BestCount** для **ActualCount**. Значение 100 указывает на отсутствие фрагментации; значение меньше 100 указывает на наличие фрагментации.|  
...
|**LogicalFragmentation**|Процент неупорядоченных страниц, определенный с помощью просмотра конечных страниц индекса. Это значение несущественно для куч. Из внеочередной страница является страницей, для которой следующая физическая страница, выделенная для индекса не страницы, на который указывает неупорядоченной*e* указатель в текущей конечной странице.|  
...
Инструкция DBCC SHOWCONTIG не отображает данные с **ntext**, **текст**, и **изображение** типов данных. Это связано с тем, что текстовые индексы, в которых хранятся текстовые и графические данные, больше не существуют.
  
Кроме того, инструкция DBCC SHOWCONTIG не поддерживает некоторые новые возможности. Например:
...
-   Инструкция DBCC SHOWCONTIG не отображает сведения о хранении переполнения строки и другие новые типы данных вне строки, таких как **nvarchar(max)**, **varchar(max)**, **varbinary(max)**, и **xml**.  
-   Пространственные индексы не поддерживаются инструкцией DBCC SHOWCONTIG.  
  
Полностью поддерживает все новые возможности [sys.dm_db_index_physical_stats &#40; Transact-SQL &#41; ](../../relational-databases/system-dynamic-management-views/sys-dm-db-index-physical-stats-transact-sql.md) динамическое административное представление.
...
-   Путем сравнения значения **переключения экстентов** и **проверенных экстентов**.  
     Значение **переключения экстентов** должно быть максимально совпадали, **проверенных экстентов**. Это отношение вычисляется как **Scan Density** значение. Это значение должно быть как можно выше, и его можно улучшить, снизив уровень фрагментации индекса.  
  
    > [!NOTE]  
    >  Данный метод не работает в том случае, если индекс охватывает несколько файлов.  
  
-   Поняв **Logical Scan Fragmentation** и **Extent Scan Fragmentation** значения.  
     **Логическая фрагментация сканирования** и в меньшей степени **Extent Scan Fragmentation** значения являются наилучшими показателями уровня фрагментации таблицы. Оба значения должны быть как можно ближе к нулю, хотя значение от 0 до 10 процентов также приемлемо.  
...
Следующий пример возвращает сокращенный результирующий набор для `Product` в таблицу [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] базы данных.
