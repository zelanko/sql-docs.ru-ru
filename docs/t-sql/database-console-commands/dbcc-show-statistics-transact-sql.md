 ...
 *table_name*  
 Имя таблицы, которая содержит статистические данные для отображения. Таблица не может быть внешней таблицей.  
...
Дата обновления статистики хранится в [большой двоичный объект статистики](../../relational-databases/statistics/statistics.md#DefinitionQOStatistics) вместе с [гистограммы](#histogram) и [вектор плотностей](#density), а не в метаданных. При чтении нет данных для создания статистических данных, статистические данные большого двоичного объекта не создается, дата не доступен и *обновление* столбец имеет значение NULL. Это происходит для отфильтрованной статистики, для которого предикат не возвращает ни одной строки или новые пустые таблицы.
...
### <a name="c-display-the-contents-of-one-statistics-object"></a>В. В. Отображение содержимого одного объекта статистики  
...
Результаты показывают заголовок, вектор плотностей и часть гистограммы.
