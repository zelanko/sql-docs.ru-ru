 ...
 *имя_таблицы*  
 Имя таблицы, которая содержит статистические данные для отображения. Таблица не может быть внешней таблицы.  
  
 *Цель*  
 Имя индекса, статистики или столбца, для которого отображаются статистические данные. *целевой* заключается в квадратные скобки, одинарные кавычки, двойные кавычки или кавычки не. Если *целевой* является именем существующего индекса или статистики для таблицы или индексированного представления, статистические данные об этом адресате не возвращается. Если *целевой* является именем существующего столбца, и автоматически созданной статистики, на этот столбец существует, возвращаются сведения об этой автоматически созданной статистики. Если автоматически созданная статистика для целевого столбца отсутствует, возвращается сообщение об ошибке 2767.  
 В [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] и [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], *целевой* не может быть именем столбца.  
...
|Обновлен|Дата и время последнего обновления статистики. [STATS_DATE](../../t-sql/functions/stats-date-transact-sql.md) функция представляет собой альтернативный способ получения этих сведений.|  
...
|Сохранить образец процентов|Сохранен образец процент, используемый для обновления статистики, которые явно не указан процент выборки. Если значение равно нулю, процентное значение сохраненного образец устанавливается для этой статистики.<br /><br /> **Применяется к:** [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP1 CU4| 
...
## <a name="density-vector"></a>Вектор плотностей  
...
Чтобы просматривать объект статистики, пользователь должен быть владельцем таблицы или пользователь должен быть членом `sysadmin` предопределенной роли сервера `db_owner` предопределенной роли базы данных или `db_ddladmin` предопределенной роли базы данных.
  
[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]С пакетом обновления 1 изменены ограничения разрешений, теперь пользователи с разрешением SELECT могут использовать эту команду. Обратите внимание, что для того, чтобы разрешение SELECT было достаточным для выполнения команды, существуют следующие требования.
...
Инструкция DBCC SHOW_STATISTICS необходимо разрешение SELECT на таблицу или членство в одной из следующих:
-   предопределенная роль сервера sysadmin   
-   db_owner предопределенной роли базы данных  
-   db_ddladmin предопределенной роли базы данных  
...
Инструкция DBCC SHOW_STATISTICS показывает статистику, хранящихся в базе данных оболочки на уровне узлов элемента управления. Не содержит статистические данные, которые были автоматически созданы с [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] на вычислительных узлах.
  
Инструкция DBCC SHOW_STATISTICS не поддерживается во внешних таблицах.
...
Следующий пример отображает все статистические данные для `AK_Address_rowguid` индекс `Person.Address` в таблицу [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] базы данных.
...
### <a name="c-display-the-contents-of-one-statistics-object"></a>В. Отображение содержимого объекта один статистики  
 В следующем примере отображается содержимое Customer_LastName статистики в таблице DimCustomer.  
...
Результаты показывают, заголовок, вектор плотностей и части.
