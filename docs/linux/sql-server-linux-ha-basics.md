...
| 3260        | TCP      | iSCSI Initiator (если используется) — может быть изменен в `/etc/iscsi/iscsid.config` (RHEL), но должен совпадать с портом цели iSCSI |
...
-   Pacemaker — ядро кластеризации компонент, который выполняет действия, например координат кластеризованных компьютеров.
...
-   Забора агента — скриптов или функций, которые дали узлов и работать с ними, если они возникают проблемы.
...
Дополнительные сведения о стеке целиком, также доступны на официальном [страницы документации Pacemaker](http://clusterlabs.org/doc/) на сайте Clusterlabs.
