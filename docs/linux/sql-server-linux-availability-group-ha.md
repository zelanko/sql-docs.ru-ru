...
>[!IMPORTANT]
>Если `required_synchronized_secondaries_to_commit` является 0, отсутствует риск потери данных. Во время сбоя первичной реплики агент ресурса не вызывает изменения автоматически отработки отказа. Можно ожидать первичный для восстановления, или вручную при сбое с помощью `FORCE_FAILOVER_ALLOW_DATA_LOSS`.
