---
title: Настройка использования и сбора диагностических данных для SQL Server в Linux | Документация Майкрософт
description: Описывает способы сбора и настроен на платформе Linux использование клиента SQL Server и диагностических данных.
author: rothja
ms.author: jroth
manager: craigg
ms.date: 03/27/2019
...
ms.locfileid: "59944235"
---
# <a name="configure-usage-and-diagnostic-data-collection-for-sql-server-on-linux"></a>Настройка использования и сбора диагностических данных для SQL Server в Linux
...
## <a name="disable-usage-and-diagnostic-data-collection"></a>Отключение использования и сбора диагностических данных

Этот параметр позволяет изменять, если SQL Server отправляет использования и сбора диагностических данных в корпорацию Майкрософт или нет. По умолчанию это значение на true. Чтобы изменить значение, выполните следующие команды:

> [!IMPORTANT]
> Можно не отключить использование и сбор диагностических данных бесплатно выпусков SQL Server, Express и Developer.

### <a name="on-red-hat-suse-and-ubuntu"></a>В Red Hat, SUSE и Ubuntu

1. Запустите сценарий mssql-conf в качестве привилегированного пользователя с **задать** команду для **telemetry.customerfeedback**. Следующий пример отключает использование и сбор диагностических данных, указав **false**.
...
Чтобы отключить использование и сбор диагностических данных в docker, необходимо иметь Docker [сохранить данные](sql-server-linux-configure-docker.md). 
...
## <a name="local-audit-for-sql-server-on-linux-usage-and-diagnostic-data-collection"></a>Локальный аудит для SQL Server на использование Linux и сбор диагностических данных

Microsoft SQL Server 2017 содержит Интернет-функциями, которые можно собирать и отправлять в Майкрософт сведения о компьютере или устройстве («Стандартные сведения о компьютере»). Компонент локального аудита для сбора данных о состоянии и использование SQL Server может записывать данные, собранные службой, в указанную папку, представляющие данные (журналы), будут отправляться в корпорацию Майкрософт. Локальный аудит позволяет клиентам просмотреть все данные, которые корпорация Майкрософт собирает с помощью этой функции для обеспечения соответствия, выполнения нормативных требований или соблюдения конфиденциальности.

В SQL Server в Linux локальный Аудит настраивается на уровне экземпляра для ядра СУБД SQL Server. Другие компоненты SQL Server и средств SQL Server не имеют функций локального аудита для использования и сбора диагностических данных.
