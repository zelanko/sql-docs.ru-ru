...
Предоставлять другим пользователям доступа к SQL Server путем создания имени входа в базе данных master с помощью [CREATE LOGIN](https://msdn.microsoft.com/library/ms189751.aspx) инструкции. Например:
...
Список предопределенных ролей базы данных см. в разделе [роли уровня базы данных](https://msdn.microsoft.com/library/ms189121.aspx).

Позже, когда будете готовы настроить более точного доступа к данным (настоятельно рекомендуется), создать свои собственные роли определяемую пользователем базу данных с помощью [CREATE ROLE](https://msdn.microsoft.com/library/ms187936.aspx) инструкции. Затем назначьте конкретные детальные разрешения вы пользовательскими ролями.
...
[Динамическое маскирование данных](https://msdn.microsoft.com/library/mt130841.aspx) позволяет ограничить раскрытия конфиденциальных данных, чтобы пользователи приложения, полностью или частично, маскируя определенные столбцы. 
...
Дополнительные сведения см. в разделе [шифрование резервной копии](https://msdn.microsoft.com/library/dn449489.aspx).


## <a name="next-steps"></a>Следующие шаги

Дополнительные сведения о средствах безопасности SQL Server см. в разделе [центр обеспечения безопасности для ядра СУБД SQL Server и базы данных SQL Azure](https://msdn.microsoft.com/library/bb510589.aspx).
