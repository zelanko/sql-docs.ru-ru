...
description: "В этом разделе описаны действия стандартных параметров безопасности."
author: BYHAM
ms.author: rickbyh
manager: jhubbard
ms.date: 07/17/2017
...
Предоставлять другим пользователям доступа к SQL Server путем создания имени входа в базе данных master с помощью [CREATE LOGIN](/sql-docs/docs/t-sql/statements/create-login-transact-sql) инструкции. Например:
...
>  [!NOTE]
>  Всегда используйте надежный пароль, вместо звездочки выше.

Имена входа можно подключиться к SQL Server и иметь доступ (с ограниченными разрешениями) к базе данных master. Чтобы подключиться к базе данных пользователя, имя входа должно соответствующих удостоверений на уровне базы данных, называется пользователем базы данных. Пользователи, относящиеся к каждой из баз данных и должны создаваться отдельно в каждой базе данных, чтобы предоставить им доступ. В следующем примере переходят базы данных AdventureWorks2014 и затем использует [CREATE USER](/sql-docs/docs/t-sql/statements/create-user-transact-sql) инструкцию, чтобы создать пользователя с именем, связанный с именем входа, с именем Ларри Ларри. На то, что имя входа и пользователя связаны (сопоставляются друг с другом), они являются различными объектами. Имя входа является участника уровня сервера. Пользователь является участником уровня базы данных.
...
Теперь входа Джерри может создавать дополнительные имена входа и пользователь Джерри может создавать дополнительных пользователей.
...
Список предопределенных ролей базы данных см. в разделе [роли уровня базы данных](/sql-docs/docs/relational-databases/security/authentication-access/database-level-roles).

Позже, когда будете готовы настроить более точного доступа к данным (настоятельно рекомендуется), создать свои собственные роли определяемую пользователем базу данных с помощью [CREATE ROLE](/sql-docs/docs/t-sql/statements/create-role-transact-sql) инструкции. Затем назначьте конкретные детальные разрешения вы пользовательскими ролями.
...
Дополнительные сведения о системе разрешений см. в разделе [Приступая к работе с разрешениями Database Engine](/sql-docs/docs/relational-databases/security/authentication-access/getting-started-with-database-engine-permissions).


## <a name="configure-row-level-security"></a>Настройка безопасности на уровне строки  

[Безопасность на уровне строк](/sql-docs/docs/relational-databases/security/row-level-security) позволяет ограничить доступ к строкам в базу данных на основании пользователя, выполняющего запрос. Эта возможность полезна для сценариев, таких как обеспечивает, что клиенты имеет доступ только к собственным данным или что работники доступны только данные, которые имеют отношение к их отделу.   

Описанные ниже руководства по настройке различных два пользователя с уровня строки доступ к `Sales.SalesOrderHeader` таблицы. 
...
[Динамическое маскирование данных](/sql-docs/docs/relational-databases/security/dynamic-data-masking) позволяет ограничить раскрытия конфиденциальных данных, чтобы пользователи приложения, полностью или частично, маскируя определенные столбцы. 
...
Дополнительные сведения см. в разделе [шифрование резервной копии](/sql-docs/docs/relational-databases/backup-restore/backup-encryption).


## <a name="next-steps"></a>Следующие шаги

Дополнительные сведения о средствах безопасности SQL Server см. в разделе [центр обеспечения безопасности для ядра СУБД SQL Server и базы данных SQL Azure](/sql-docs/docs/relational-databases/security/security-center-for-sql-server-database-engine-and-azure-sql-database).
