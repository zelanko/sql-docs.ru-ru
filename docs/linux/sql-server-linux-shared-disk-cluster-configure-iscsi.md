---
title: Настройка хранилища iSCSI для экземпляра отказоустойчивого кластера — SQL Server на Linux
description: ''
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: vanto
...
12. Для системных баз данных или других объектов, хранящихся в расположении данных по умолчанию, выполните указанные ниже действия. В противном случае перейдите к шагу 13.
   ...
   *    Перейдите в режим суперпользователя. В случае успешного перехода подтверждение не выводится.

    ```bash
    sudo -i
    ```

   *    Переключитесь на пользователя mssql. В случае успешного перехода подтверждение не выводится.

    ```bash
    su mssql
    ```

   *    Создайте временный каталог для хранения данных и файлов журналов SQL Server. В случае успешного перехода подтверждение не выводится.
   ...
   *    Скопируйте данные и файлы журналов SQL Server во временный каталог. В случае успешного перехода подтверждение не выводится.
   ...
   *    Удалите файлы из существующего каталога данных SQL Server. В случае успешного перехода подтверждение не выводится.
   ...
   *    Подключите логический том iSCSI в папке данных SQL Server. В случае успешного перехода подтверждение не выводится.
   ...
   *    Измените владельца подключения на mssql. В случае успешного перехода подтверждение не выводится.

    ```bash
    chown mssql /var/opt/mssql/data
    ```

   *    Измените владельца группы подключения на mssql. В случае успешного перехода подтверждение не выводится.
   ...
   *    Скопируйте файлы из временного каталога /var/opt/mssql/data. В случае успешного перехода подтверждение не выводится.
...
13. Для объектов, отличных от системных баз данных, например пользовательских баз данных или резервных копий, выполните указанные ниже действия. Если используется только расположение по умолчанию, перейдите к шагу 14.
   ...
   *    Подключите логический том iSCSI в папке, созданной в предыдущем шаге. В случае успешного перехода подтверждение не выводится.
   ...
   *    Измените владельца созданной папки на mssql. В случае успешного перехода подтверждение не выводится.

    ```bash
    chown mssql <FolderName>
    ```

    \<FolderName> — это имя созданной папки. Пример показан ниже.

    ```bash
    chown mssql /var/opt/mssql/userdata
    ```
  
   *    Измените владельца группы папки на mssql. В случае успешного перехода подтверждение не выводится.
...
16. Измените раздел конфигурации активации файла /etc/lvm/lvm.conf. Настройте следующую строку.
    ...
    \<ListOfVGsNotUsedByPacemaker> — это список групп томов из выходных данных шага 20, которые не будут использоваться экземпляром отказоустойчивого кластера. Заключите каждый из них в кавычки и разделите запятыми. Пример показан ниже.
