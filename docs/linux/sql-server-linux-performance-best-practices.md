...
Значение параметра **vm.max_map_count** по умолчанию (65 536) может быть недостаточно большим для системы SQL Server. По этой причине измените значение **vm.max_map_count** на 262144 для развертывания SQL Server и ознакомьтесь с разделом [Рекомендуемые параметры Linux при использовании настраиваемого профиля mssql](#proposed-linux-settings-using-a-tuned-mssql-profile) для дальнейшей настройки этих параметров ядра. Максимальное значение для vm.max_map_count равно 2147483647.
