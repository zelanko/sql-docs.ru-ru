...
>[!NOTE]
>Для SQL Server 2017 развертывания на основе Linux с помощью представленных непосредственно на сервер таких дисков должен быть отформатирован XFS или EXT4. В настоящее время другие файловые системы не поддерживаются. Все изменения будут отражены здесь.
