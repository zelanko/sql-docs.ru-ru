...
1. Создание кластера. 
   ...
   >[!WARNING]
   >Связи с известной проблемой, кластеризации поставщика над этой проблемой, начиная кластера («компьютеры кластера start»), будут завершаться под ошибкой. Это так, как настроить файл журнала в /etc/corosync/corosync.conf которого создается при запуске команды установки кластера, не так. Чтобы обойти эту проблему, измените файл журнала: /var/log/corosync/corosync.log. В качестве альтернативы можно создать файл /var/log/cluster/corosync.log.
