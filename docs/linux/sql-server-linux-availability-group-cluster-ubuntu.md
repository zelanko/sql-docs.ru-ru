...
1. Создание кластера. 
   ...
   >[!WARNING]
   >Связи с известной проблемой, кластеризации поставщика над этой проблемой, начиная кластера («компьютеры кластера start») под ошибкой завершается. Это так, как настроить файл журнала в /etc/corosync/corosync.conf которого создается при запуске команды установки кластера, не так. Чтобы обойти эту проблему, измените файл журнала: /var/log/corosync/corosync.log. В качестве альтернативы можно создать файл /var/log/cluster/corosync.log.
...
>[!WARNING]
>После автоматического перехода на другой ресурс при `start-failure-is-fatal = true` диспетчера ресурсов пытается запустить ресурс. В случае неудачи при первой попытке необходимо вручную выполнить `pcs resource cleanup <resourceName>` для очистки количество сбоев ресурсов и сбросить конфигурацию.
