...
description: Сведения о кластерах групп доступности при использовании Red Hat Enterprise Linux (RHEL)
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: vanto
ms.date: 03/12/2019
...
Этот документ описывает, как создать кластер группы доступности с тремя узлами для SQL Server на Red Hat Enterprise Linux. Для обеспечения высокой доступности группе доступности в Linux требуется три узла — см. статью [Высокий уровень доступности и защита данных для конфигураций групп доступности](sql-server-linux-availability-group-ha.md). Уровень кластеризации основан на [надстройке высокой доступности](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/6/pdf/High_Availability_Add-On_Overview/Red_Hat_Enterprise_Linux-6-High_Availability_Add-On_Overview-en-US.pdf) Red Hat Enterprise Linux (RHEL), созданной на базе [Pacemaker](https://clusterlabs.org/). 
...
1. [Настройте SQL Server на узлах кластера](sql-server-linux-setup.md).
...
Поставщики кластера Pacemaker требуют, чтобы STONITH был включен, а устройство ограждения было настроено для поддерживаемой установки кластера. STONITH расшифровывается как "застрелить другой узел". Если диспетчер ресурсов кластера не может определить состояние узла или ресурса на нем, ограждение переводит кластер в известное состояние.
...
Ограждение на уровне узлов гарантирует, что узел не выполняет никакие ресурсы. Это осуществляется путем сброса узла. Pacemaker поддерживает множество разных устройств ограждения, например источник бесперебойного питания или карты интерфейса управления для серверов.
...
>[!IMPORTANT]
>Отключение STONITH выполняется только в целях тестирования. Если вы планируете использовать Pacemaker в рабочей среде, следует спланировать реализацию STONITH с учетом особенностей среды и поддерживать ее в рабочем состоянии. RHEL не предоставляет агенты ограждения для каких-либо облачных сред (включая Azure) или Hyper-V. Следовательно, поставщик кластеров не предоставляет поддержку для запуска рабочих кластеров в этих средах. Мы работаем над этой проблемы, решение будет доступно в будущих выпусках.
...
1. Ресурс виртуального IP-адреса останавливается на узле 1.
1. Ресурс виртуального IP-адреса запускается на узле 2.
