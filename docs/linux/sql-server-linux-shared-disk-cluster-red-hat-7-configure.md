...
1. Настройка файла hosts для каждого узла кластера. Необходимо включить файл узла, IP-адрес и имя каждого узла кластера. 
   ...
   Задайте имя компьютера, на каждом узле. Присвойте каждому узлу уникальное имя, которое составляет 15 символов или меньше. Задайте имя компьютера, добавив его в `/etc/hosts`. Следующий сценарий позволяет редактировать `/etc/hosts` с `vi`. 
...
1. На одном из узлов Создайте кластер.
   ...
   > RHEL HA надстройки имеет ограждения агентов для VMWare и KVM. Разграничения должна быть отключена на всех остальных низкоуровневые оболочки. Отключение разграничения агентов в производственных средах не рекомендуется. Начиная с версии-кандидата 2 промежуток времени отсутствуют разграничения агенты для Hyper-v или облачных сред. Если вы используете одну из этих конфигураций, необходимо отключить разграничения. \**Это не рекомендуется в рабочей среде!**
