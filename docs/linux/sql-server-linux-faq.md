...
1. **Определения функций SQL Server, поддерживаемых на платформе Linux?**

   Полный список поддерживаемых функций и известные проблемы, см. в разделе [заметки о выпуске](sql-server-linux-release-notes.md).
