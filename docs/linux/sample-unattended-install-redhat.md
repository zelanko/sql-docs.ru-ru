...
Этот пример скрипта Bash устанавливает SQL Server 2017 на Red Hat Enterprise Linux (RHEL) без интерактивного ввода данных. Он предоставляет примеры установки компонента database engine, средства командной строки SQL Server, агент SQL Server и выполняет шаги после установки. При необходимости можно установить компонент full-text search и создать пользователь с правами администратора.

> [!TIP]
> Если не требуется использовать сценарий автоматической установки, то самый быстрый способ установки SQL Server — следуйте [краткое руководство по Red Hat](quickstart-install-connect-red-hat.md). Другие сведения о настройке см. в разделе [руководство по установке для SQL Server в Linux](sql-server-linux-setup.md).
...
Сохранить сценарий в файл и замените значения переменных в скрипте для его настройки. Можно также задать любой из переменных скрипта как переменные среды, до тех пор, пока он удаляется из файла скрипта.
