---
title: Устранение неполадок в SQL Server в Linux
...
- Если вы не сможете подключения с помощью **localhost**, попробуйте вместо этого использовать IP-адрес 127.0.0.1. Возможно, **localhost** сопоставлены неправильно по этому адресу.
...
Существует много факторов, влияющих на производительность, включая структуры базы данных, оборудования и пиковой нагрузки. Если вам нужно повысить производительность, начните с изучения рекомендаций в этой статье, [рекомендации по производительности и рекомендации по конфигурации для SQL Server в Linux](sql-server-linux-performance-best-practices.md). Затем изучите некоторые доступные средства для устранения неполадок производительности.
