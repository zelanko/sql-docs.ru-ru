...
После завершения сценария, оператор Kubernetes создает хранилище, экземпляры SQL Server, службы подсистемы балансировки нагрузки. Вы можете отслеживать развертывание с помощью [панели мониторинга Kubernetes](http://docs.microsoft.com/azure/aks/kubernetes-dashboard).
