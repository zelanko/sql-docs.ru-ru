---
title: Настройка отказоустойчивого кластера экземпляра хранилища SMB - SQL Server в Linux | Документация Майкрософт
description: ''
author: MikeRayMSFT
ms.author: mikeray
manager: craigg
...
2.  Получите сведения о пользователе mssql.
...
4. Для системных баз данных или все данные, хранящиеся в расположении данных по умолчанию выполните следующие действия. В противном случае перейдите к шагу 5. 
   ...
   *    Подключите общий ресурс SMB в папке данных SQL Server. Вы не получите любое подтверждение при успешном выполнении. В этом примере показан синтаксис для подключения к общей папке SMB 3.0, под управлением Windows Server.
   ...
   *    Для дальнейшего тестирования создайте базу данных убедитесь, что разрешения являются нормально. В следующем примере используется Transact-SQL; можно использовать SSMS.

    ![10_testcreatedb][2] 
  
   *    Остановка SQL Server и убедитесь, что завершение работы. Если планируется добавление или тестирование другие диски, не завершить работу SQL Server до тех добавляются и протестированы.
   ...
   *    Только в том случае, если завершено, отключите общую папку. В противном случае отключение после завершения тестирования: Добавление дополнительные диски.
...
5.  Для таких вещей, кроме системных баз данных, таких как пользовательских баз данных или резервных копий выполните следующие действия. Если только с использованием расположения по умолчанию, перейдите к шагу 14.
   ...
   *    Подключите общий ресурс SMB в папке данных SQL Server. Вы не получите любое подтверждение при успешном выполнении. В этом примере показан синтаксис для подключения к общей основе Samba SMB 3.0.
...
6.  Повторите эти действия на другие узлы.
