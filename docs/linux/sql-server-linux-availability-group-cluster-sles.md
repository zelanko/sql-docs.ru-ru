...
Процедура создания группы доступности для обеспечения высокой доступности серверов Linux и отличаются отказоустойчивый кластер Windows Server. Ниже приводятся общие действия. 
...
3. Настройте диспетчер ресурсов кластера, например Pacemaker. Эти инструкции приведены в этом документе.
   ...
   >[!IMPORTANT]
   >Рабочих средах требуется агент разграничения, например STONITH для обеспечения высокой доступности. Примеры в этой статье не используют разграничения агентов. Они предназначены для тестирования и проверки только. 
...
Чтобы выполнить следующий сценарий начала до конца, необходимо трех компьютеров для развертывания трех узлов кластера. Ниже описывается способ настройки этих серверов.
...
2. Запуск сценария начальной загрузки, выполнив:
   ...
   Если принято решение продолжить скрипта автоматически создает ключи для доступа SSH и средство синхронизации Csync2 и запустить службы, необходимые для обоих. 

3. Настройка уровня связи кластеров (Corosync): 

   A. Введите сетевой адрес для привязки. По умолчанию скрипт предлагает eth0 сетевой адрес. Также можно ввести отдельный сетевой адрес, например, адрес bond0. 
...
- `Csync2` настраивается на существующих узлах. Дополнительные сведения см. в разделе Настройка Csync2 с YaST. 
...
4. Если вы еще не настроили passwordless SSH доступа между обеих машинах, вы также предложит ввести пароль учетной записи root существующего узла. 
   ...
   После входа в указанном узле скрипт копирует конфигурацию Corosync, настраивает SSH и `Csync2`и переводит текущего компьютера сети как новый узел кластера. Кроме того, она запускает службу, которая необходима для Hawk. Если вы настроили общее хранилище с `OCFS2`, он также автоматически создает каталог монтирования для `OCFS2` файловой системы. 
...
Ресурс уровня разграничения главным образом, гарантируется без повреждения данных во время сбоя путем настройки ресурса. Можно использовать разграничения уровня ресурсов, например, с DRBD (распределенных реплицируются блоков) для пометки диска на узле как устаревший, когда канал связи выходит из строя.

Узел уровня разграничения гарантирует узла не выполняет какие-либо ресурсы. Это делается путем сброса узел и его реализация Pacemaker вызывается STONITH (что означает «прокрутить другой узел в начало»). Pacemaker поддерживает разнообразных ограждения устройства, такие как источник бесперебойного питания питания или управления карт для серверов.
...
Во время инициализации кластера STONITH будет отключен, если конфигурация не обнаруживается. Его можно включить позже введите следующую команду:
...
Следующая команда создает и настраивает группы доступности для трех реплик группы доступности [ag1]. Монитор операций и увеличение времени ожидания должны быть явно указаны в SLES исходя из того, что время ожидания, сильно зависят от рабочей нагрузки и тщательно настраивается для каждого развертывания.
...
Вручную переключить группу доступности с `crm`. Не инициировать отработку отказа с помощью Transact-SQL. Дополнительные сведения см. в разделе [перехода на другой ресурс](sql-server-linux-availability-group-failover-ha.md#failover).
