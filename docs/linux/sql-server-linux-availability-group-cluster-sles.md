...
3. Чтобы настроить слой связи кластера (Corosync), выполните указанные ниже действия. 
   ...
   A. Введите сетевой адрес для привязки. По умолчанию скрипт предлагает сетевой адрес eth0. Можно ввести другой сетевой адрес, например bond0. 

   Б. Введите адрес многоадресной рассылки. Скрипт предлагает случайный адрес, который можно использовать по умолчанию. 

   в. Введите порт многоадресной рассылки. Сценарий предлагает порт 5405 по умолчанию. 

   г. Чтобы настроить `SBD ()`, введите постоянный путь к разделу блочного устройства, которое следует использовать для SBD. Путь должен быть одинаковым во всех узлах кластера. 
...
4. Главная реплика группы доступности в узле 1 понижается до подчиненной.
5. Подчиненная реплика группы доступности в узле 2 повышается до главной. 
