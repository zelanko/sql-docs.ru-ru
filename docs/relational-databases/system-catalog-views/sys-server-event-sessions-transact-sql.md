...
|event_retention_mode|**nchar (1)**|Определяет способ обработки потери события. Значение по умолчанию — S. Не допускает значения NULL. Принимает одно из следующих значений.<br /><br /> Х. Сопоставляется с event_retention_mode_desc = ALLOW_SINGLE_EVENT_LOSS<br /><br /> Н. Сопоставляется с event_retention_mode_desc = ALLOW_MULTIPLE_EVENT_LOSS<br /><br /> О. Сопоставляется с event_retention_mode_desc = NO_EVENT_LOSS|  
