...
|-----------------|---------------|-----------------|  
|**document_type**|**имеет sysname**|Расширение файла поддерживаемого типа документа.<br /><br /> Это значение можно использовать для определения фильтра, который будет использоваться во время полнотекстового индексирования столбцов типа **varbinary (max)** или **Image**.|  
...
|**Версия**|**имеет sysname**|Версия IFilter DLL.|  
|**manufacturer**|**имеет sysname**|Название производителя IFilter.<br /><br /> Примечание. поддерживаются только документы с изготовителем [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssSDS](../../includes/sssds-md.md)].|  
