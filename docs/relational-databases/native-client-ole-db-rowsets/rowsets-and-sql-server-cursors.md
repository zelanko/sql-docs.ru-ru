...
-   Серверные курсоры, которые характеризуются следующим:  
 ...
 Пользователь может запросить другой режим работы курсоров, установив определенные свойства набора строк. Если потребитель не устанавливает ни одного из этих свойств строк и не [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] устанавливает их все к значениям по умолчанию, поставщик Native Client OLE DB реализует набор строк с помощью набора результатов по умолчанию. Если какое-либо из этих свойств настроено [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] на значение, кроме значения, кроме значения по умолчанию, поставщик Native Client OLE DB реализует набор строк с помощью курсора сервера.  
...
|DBPROP_REMOVEDELETED|VARIANT_TRUE|Обновление данных [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] через набор строк невозможно. Этот набор строк поддерживает только прямую прокрутку. Относительное позиционирование строки поддерживается. Текст команды может содержать предложение ORDER BY, если это не запрещено другим свойством.|  
 ...
 Рядовые [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] наборы поставщиков услуг Native Client OLE DB, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] поддерживаемые курсором сервера, могут быть легко созданы на базовой таблице или в представлении с помощью метода **IOpenRowset::OpenRowset.** Укажите таблицу или представление по имени, передав требуемые наборы свойств набора строк в параметре *rgPropertySets*.  
...
|Свойства набора строк и модели курсора|Значение по умолчанию<br /><br /> набор по<br /><br /> set<br /><br /> (RO)|Быстрый<br /><br /> однопроходный<br /><br /> только<br /><br /> (RO)|Статические<br /><br /> (RO)|Keyset<br /><br /> управляемый<br /><br /> (RO)|  
|--------------------------------------|-------------------------------------------|--------------------------------------------|-----------------------|----------------------------------|  
...
## <a name="sql-server-cursor-block-size"></a>Размер блока курсора SQL Server  
 Когда [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] курсор поддерживает [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] строку поставщика NATIVE Client OLE DB, количество элементов в строке обрабатывает параметр массива **IRowset::GetNextRows** или **IRowsetLocate::GetRowsAt** методы определяют размер блока курсора. Строки, указанные дескрипторами в массиве, являются элементами блока курсора.  
