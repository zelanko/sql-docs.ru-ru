...
#### <a name="validation-phase-2-of-3"></a>Проверка: фаза 2 (из 3)  
  
- Фаза проверки начинается с назначения времени окончания (тем самым транзакция помечается как логически завершенная). В результате все вносимые в транзакцию изменения видны другим транзакциям, которые так или иначе зависят от этой. Транзакции будет невозможно зафиксировать до тех пор, пока успешно не завершится эта транзакция. Кроме того, транзакции с подобными зависимостями не могут возвращать наборы результатов клиенту. Благодаря этому клиент видит только данные, которые были успешно зафиксированы в базе данных.  
- Этот этап включает уровни проверку параметров repeatable read и serializable. При проведении проверки изоляции repeatable read система проверяет, были ли обновлены какие-либо из строк, прочитанных транзакцией, с момента прочтения. При проверке параметра serializable система проверяет, были ли какие-либо строки вставлены в диапазон данных, сканированный этой транзакцией. Обратите внимание, что, согласно таблице в разделе [Уровни изоляции и конфликты](#confdegreeiso30ni), проверки repeatable read и serializable могут произойти на фоне изоляции снимков, чтобы проверить консистентность ограничений уникального и внешнего ключей.  
  
#### <a name="commit-processing-phase-3-of-3"></a>Обработка фиксации: фаза 3 (из 3)  
...
- Сбои зависимости. Они возникают, если транзакция, от которой зависят ваши дальнейшие действия, не фиксируется либо количество зависимостей становится слишком большим.

Ниже перечислены условия ошибок, которые могут вызывать сбои транзакций, осуществляющих доступ к таблицам, оптимизированным в памяти.
...
\<!--  
Идентификаторы GUID для ссылок:  
016fb05e-a702-484b-bd2a-a6eabd0d76fd , ms173763.aspx , "SET TRANSACTION ISOLATION LEVEL (Transact-SQL)"  
  
ef1cc7de-63be-4fa3-a622-6d93b440e3ac , dn511014(v=sql.130,d=robot).aspx , "Введение в таблицы, оптимизированные для памяти"  
...
3ac93b28-cac7-483e-a8ab-ac44e1cc1c76 , dn449490.aspx , "Управление устойчивостью транзакций"  
  
Изображение: 'hekaton_transactions' , e9c5eb2f-c9a3-4625-8ae4-ac91447db42f  
См. также статьи об XMetaL: dn133169.aspx "Время существования транзакций"  
  
Операции с таблицами и процедурами в памяти  
{ba6f1a15-8b69-4ca6-9f44-f5e3f2962bc5} , dn479429.aspx  
Возможно, заменяет: 06075248-705e-4563-9371-b64cd609793c , dn479429.aspx , "Основные сведения о транзакциях с таблицами, оптимизированными для памяти"  
  
GeneMi, 28.03.2016 11:40:00  
-->  
