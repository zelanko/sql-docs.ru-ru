---
title: "Планирование освоения возможностей выполняющейся в памяти OLTP в SQL Server | Microsoft Docs"
ms.custom: ""
ms.date: "10/05/2016"
...
## A. Освоение возможностей выполняющейся в памяти OLTP
...
Не существует запроса, способного применить [табличное указание](Table%20Hints%20%28Transact-SQL%29.md) READPAST к какой-либо оптимизированной для обработки в памяти таблице.
...
## В. Обслуживание в рамках администрирования
...
### В.1. Сброс начального значения идентификаторов, приращение >1
...
1. [Резервное копирование журнала транзакций](Back%20Up%20a%20Transaction%20Log%20%28SQL Server%29.md).
...
- Опасно недооценивать число *контейнеров*, которое может потребоваться хэш-индексу, это может привести к длинным цепочкам в отдельных контейнерах, чего следует избегать. Подробная информация доступна в следующей статье:
...
- [Индекс columnstore](Columnstore%20Indexes%20Guide.md) может быть создан на основе дисковой таблицы и даже в форме кластеризованного индекса. Однако в таблице, оптимизированной для обработки в памяти, индекс columnstore не может быть кластеризован.
...
## Д. Ограничения, связанные со скомпилированными в машинный код процедурами
...
### Д.1. Отсутствие выражения CASE в скомпилированной в машинный код процедуре
...
### Д.2. Отсутствие выражения MERGE в скомпилированной в машинный код процедуре
...
### Д.3. Отсутствие соединений в инструкциях UPDATE и DELETE в скомпилированной в машинный код процедуре
...
#### Недопустимая инструкция T-SQL, DELETE...JOIN
...
#### Рабочее обходное решение: удаление и соединение вручную
...
### Д.4. Ограничения, связанные с планом запросов для скомпилированных в машинный код процедур
...
## Е. Проектирование приложений: транзакции и логика повторных попыток
