---
title: "Создание хранимых процедур, скомпилированных в собственном коде | Microsoft Docs"
ms.custom: ""
ms.date: "03/16/2017"
...
-   Блоки ATOMIC. Дополнительные сведения см. в статье [Atomic Blocks](../../relational-databases/in-memory-oltp/атомарные-блоки-в-собственных-процедурах.md).  
  
-   Ограничения **NOT NULL** на параметры и переменные в хранимых процедурах, скомпилированных в собственном коде. Нельзя присвоить значения **NULL** параметрам или переменным, объявленным как **NOT NULL**. Дополнительные сведения см. в разделе [DECLARE @локальная_переменная (Transact-SQL)](../../t-sql/language-elements/declare-local-variable-transact-sql.md).  
  
    -   CREATE PROCEDURE dbo.myproc (@myVarchar  varchar(32)  **not null**) ...  
  
    -   DECLARE @myVarchar  varchar(32)  **not null = "Hello"**; -- *(Должно быть инициализировано значение.)*  
  
    -   SET @myVarchar **= null**; -- *(Компилируется, но во время выполнения происходит ошибка.)*  
