...
|10|Если усечение долей секунды происходит с потерей данных, возвращается значение DBSTATUS_E_DATAOVERFLOW. Проверка переполнения при преобразовании строк возможна, только если строка соответствует синтаксису ISO. Если строка является литералом даты OLE, доли секунды округляются.<br /><br /> Для преобразования типа DBTIMESTAMP (datetime) в smalldatetime [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client усекает значение секунд вместо вызова ошибки DBSTATUS_E_DATAOVERFLOW.|  
