---
title: "Изменение данных в темпоральной таблице с системным управлением версиями | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/28/2016"
...
## Удаление данных  
 Данные в текущей таблице удаляются с помощью регулярной инструкции **DELETE** . В удаленных строках в столбец периода окончания будет подставлено время начала базовой транзакции.   
Когда **SYSTEM_VERSIONING = ON**, удалять строки напрямую из таблицы журнала нельзя.   
Чтобы удалить строки из текущей таблицы и таблицы журнала, задайте **SYSTEM_VERSIONING = OFF**. Но в этом случае система не будет вести журнал изменений.   
Когда **SYSTEM_VERSIONING = ON**, инструкции **TRUNCATE**, **SWITCH PARTITION OUT** (для текущей таблицы) и **SWITCH PARTITION IN** (для таблицы журнала) не работают.  
