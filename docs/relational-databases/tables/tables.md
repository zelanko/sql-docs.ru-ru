...
helpviewer_keywords: 
  - "таблицы [SQL Server]"
  - "табличные компоненты [SQL Server]"
...
## Типы таблиц  
 ...
 Системные таблицы  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] хранит данные, определяющие конфигурацию сервера и всех его таблиц в специальном наборе таблиц, называющихся системными. Пользователи не могут напрямую выполнить запрос или обновление системных таблиц. Данные из системных таблиц доступны через системные представления. Дополнительные сведения см. в разделе [Системные представления (Transact-SQL)](../Topic/System%20Views%20\(Transact-SQL\).md).  
...
|-----------------|-----------|  
|Описание процесса создания таблицы.|[Создание таблиц (компонент Database Engine)](../../relational-databases/tables/create-tables-database-engine.md)|  
|Описание процесса удаления таблицы.|[Удаление таблиц (компонент Database Engine)](../../relational-databases/tables/delete-tables-database-engine.md)|  
...
|Описывает процесс переименования таблицы.|[Переименование таблиц (компонент Database Engine)](../../relational-databases/tables/rename-tables-database-engine.md)|  
...
|------------------|-----------|  
|Описывает добавление столбцов в существующую таблицу.|[Добавление столбцов в таблицу (компонент Database Engine)](../../relational-databases/tables/add-columns-to-a-table-database-engine.md)|  
...
|Описывает изменение имени столбца.|[Переименование столбцов (компонент Database Engine)](../../relational-databases/tables/rename-columns-database-engine.md)|  
|Описывает копирование столбцов из одной таблицы в другую, когда копируется либо только определение столбца, либо определение и данные.|[Копирование столбцов из одной таблицы в другую (компонент Database Engine)](../../relational-databases/tables/copy-columns-from-one-table-to-another-database-engine.md)|  
...
|Описывает, как установить значение по умолчанию для столбца. Это значение используется, если другое значение не предоставлено.|[Указание значений по умолчанию для столбца](../../relational-databases/tables/specify-default-values-for-columns.md)|  
