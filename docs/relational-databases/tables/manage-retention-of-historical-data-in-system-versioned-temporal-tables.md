...
## <a name="data-retention-management-for-history-table"></a>Управление хранением данных для таблицы журнала  
 ...
 Когда вы определите срок хранения данных, следующим шагом будет разработка плана управления данными журнала, а именно определение способа и места хранения данных журнала, а также способа удаления данных после истечения их срока хранения. Доступны следующие четыре подхода к управлению данными журнала в темпоральной таблице журнала:  
...
## <a name="using-temporal-history-retention-policy-approach"></a>Используя подход политики хранения журнала
> **Примечание:** с помощью временных политики хранения журнала подход применим к [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] и SQL Server 2017 г., начиная с CTP-версии 1.3.  
...
```
Можно указать срок хранения, используя различные единицы времени: дни, НЕДЕЛИ, МЕСЯЦЫ и ГОДЫ. Если HISTORY_RETENTION_PERIOD опущен, подразумевается БЕСКОНЕЧНОГО хранения. Также можно использовать ключевое слово БЕСКОНЕЧНЫЙ явным образом.
В некоторых сценариях может потребоваться настройка хранения после создания таблицы, либо для изменения ранее настроить значение. В этом случае используйте инструкцию ALTER TABLE:
```
ALTER TABLE dbo.WebsiteUserInfo
SET (SYSTEM_VERSIONING = ON (HISTORY_RETENTION_PERIOD = 9 MONTHS));
```
Чтобы просмотреть текущее состояние политики хранения, используйте следующий запрос, объединяющий флаг включения временного хранения на уровне базы данных со сроками хранения для отдельных таблиц:
```
...
```
### <a name="how-sql-database-deletes-aged-rows"></a>Способ удаления базы данных SQL устаревших строк?
Процесс очистки зависит от макета индекс таблицы журнала. Обратите внимание, что очень важно *таблицы журнала с кластеризованным индексом (B-дерева или columnstore) могут иметь конечное хранения политика, настроенная только*. Для выполнения очистки устаревших данных для всех темпоральных таблиц с периодом хранения конечное создается в фоновом режиме. Логика очистки для кластеризованный индекс rowstore (сбалансированное дерево) удаляет устаревшие строки небольшими фрагментами (до 10 K) к минимуму нагрузку на журнал базы данных и подсистемы ввода-вывода. Несмотря на то, что используется логика очистки требуется индекс сбалансированного дерева, порядок удаления для строк, возраст которых превышает срок хранения не может быть гарантирована надежно. Таким образом *не выполняют любую зависимость на порядок очистки в приложениях*.

Задача очистки для кластеризованный индекс columnstore удаляет всю строку группы за один раз (обычно содержит 1 миллион строк каждого), который очень эффективен, особенно в том случае, когда данные журнала создается с высокой скоростью.

![Clustered columnstore хранения](../../relational-databases/tables/media/cciretention.png "Clustered columnstore хранения")

Отличное сжатие данных и делает очистки эффективного хранения кластеризованный индекс columnstore оптимален для сценариев, когда рабочая нагрузка быстро создает большой объем исторических данных. Этот шаблон является типичным для служб с интенсивными вычислениями транзакционной обработки рабочих нагрузок, использующих временных таблиц для отслеживания изменений и аудита, анализа трендов или приема данных IoT.

Проверьте [управление данными журнала в Темпоральных таблиц с политикой хранения](https://docs.microsoft.com/en-us/azure/sql-database/sql-database-temporal-tables-retention-policy) для получения дополнительных сведений.
