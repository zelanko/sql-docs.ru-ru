...
Импортировать данные из файлов Excel в SQL Server или базу данных SQL Azure можно несколькими способами.
-   Можно напрямую импортировать данные из Excel SQL с помощью мастера импорта и экспорта SQL Server, служб Integration Services (SSIS) или функции OPENROWSET. 
-   Можно сохранить данные Excel в виде текста и использовать инструкцию BULK INSERT, BCP или фабрику данных Azure. Здесь кратко описаны основные варианты и приведены ссылки на подробные инструкции.

> [!NOTE]
> В этом обзоре отсутствует полное описание таких сложных средств и служб, как службы SSIS или фабрика данных Azure. Дополнительные сведения и руководства по интересующему вас решению доступны по ссылкам ниже.

## <a name="sql-server-import-and-export-wizard"></a>мастер импорта и экспорта SQL Server

Импортируйте данные напрямую из файлов Excel, выполнив инструкции на страницах мастера. Или сохраните параметры импорта и экспорта как пакет служб SQL Server Integration Services (SSIS), который можно настраивать и повторно использовать.
...
> [!NOTE]
> Поставщик ACE (прежнее название — поставщик Jet), который подключается к файлам Excel, предназначен для интерактивного клиентского использования. Если поставщик ACE используется на сервере, особенно в автоматизированных процессах или процессах, выполняющихся параллельно, вы можете получить непредвиденные результаты.

Импортируйте данные напрямую из файлов Excel с помощью функции `OPENROWSET` или `OPENDATASOURCE`. Такая операция называется *распределенный запрос*.
...
В следующем примере кода данные импортируются из листов Excel `Customers` в новую таблицу SQL Server с помощью `OPENROWSET`.
...
Для обращения к данным Excel без импорта используйте синтаксис `SELECT ... FROM ...`.
...
Кроме того, можно настроить постоянное подключение к файлу Excel как к *связанному серверу*. В примере ниже данные импортируются из листа Excel `Data` на существующем связанном сервере `EXCELLINK` в новую таблицу SQL Server с именем `Data_ls`.
...
`BULK INSERT` — это команда, которую можно выполнить в SQL Server Management Studio. В следующем примере данные загружаются из файла `Data.csv` с разделителями-запятыми в существующую таблицу.
...
BCP — это средство SQL Server, которое запускается из командной строки. В примере ниже данные загружаются из файла `Data.csv` с разделителями-запятыми в существующую таблицу `Data_bcp`.
...
Импортируйте данные, сохраненные как текстовые файлы, выполнив инструкции на страницах мастера. Дополнительные сведения о мастере копирования см. в указанных ниже разделах.
...
Если вы уже работали с фабрикой данных Azure вы можете не запускать мастер копирования фабрики данных Azure. Для этого создайте конвейер с действием копирования из текстового файла, расположенного в месте хранения файлов, в SQL Server или базу данных SQL Azure.
