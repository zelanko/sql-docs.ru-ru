---
title: "XML-файлы форматирования (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "файлы форматирования [SQL Server], XML-файлы форматирования"
...
> [!NOTE]  
>  Для массового экспорта и импорта поддерживаются два типа файлов форматирования: *файлы форматирования, отличные от XML*, и *XML-файлы форматирования*. Они более гибкие и мощные по сравнению с файлом форматирования в формате, отличном от XML. Сведения о файлах форматирования в формате, отличном от XML, см. в разделе [Файлы форматирования, отличные от XML (SQL Server)](../../relational-databases/import-export/non-xml-format-files-sql-server.md).  
...
-   Элемент \<ROW> описывает, как создавать строки данных из файла данных, который импортируется в таблицу сервера [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
...
-   [Атрибуты элемента <COLUMN>\>](#AttrOfColumnElement) (и [значения типа Xsi:type элемента \<COLUMN> ](#XsiTypeValuesOfCOLUMN))  
...
|---------------------|-----------------|------------------------------|  
|ID **="***fieldID***"**|Задает логическое имя поля в файле данных. Идентификатор поля является ключом, используемым для обращения к полю.<br /><br /> \<FIELD ID**="***fieldID***"**/> сопоставляется \<COLUMN SOURCE**="***fieldID***"**/>|Обязательное|  
...
|Значения xsi:type элемента \<FIELD>|Обязательные XML-атрибуты<br /><br /> для типа данных|Необязательные XML-атрибуты<br /><br /> для типа данных|  
|-------------------------------|---------------------------------------------------|---------------------------------------------------|  
...
|----------------------|-----------------|------------------------------|  
|SOURCE **="***fieldID***"**|Задает идентификатор поля, сопоставляемого со столбцом.<br /><br /> \<COLUMN SOURCE**="***fieldID***"**/> сопоставляется \<FIELD ID**="***fieldID***"**/>|Обязательное|  
...
###  <a name="MapXSItype"></a> Г. Сопоставление типа данных xsi:type элемента \<FIELD> типу данных xsi:type элемента \<COLUMN>  
