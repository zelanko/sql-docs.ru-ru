...
|LENGTH **="***n***"**|Этот атрибут определяет длину для экземпляра типа данных фиксированной длины.<br /><br /> Значение атрибута *n* должно быть положительным целым числом.|Необязательный, когда не требуется значением xsi:type|  
