...
|---------------------|-----------------|------------------------------|  
|ID **="***fieldID***"**|Задает логическое имя поля в файле данных. Идентификатор поля является ключом, используемым для обращения к полю.<br /><br /> \<FIELD ID**="***fieldID***"**/> сопоставляется с \<COLUMN SOURCE**="***fieldID***"**/>|Обязательно|  
...
|Длина **=»***n***»**|Этот атрибут определяет длину для экземпляра типа данных фиксированной длины.<br /><br /> Значение *n* должно быть положительным целым числом.|Необязательный, когда не требуется значением xsi:type|  
...
|Длина **=»***n***»**|Определяет длину для экземпляра типа данных фиксированной длины. Атрибут LENGTH используется только в том случае, если значение xsi:type является строковым типом данных.<br /><br /> Значение *n* должно быть положительным целым числом.|Необязательный (доступен только в том случае, если значение xsi:type является строковым типом данных)|  
|ТОЧНОСТЬ **=»***n***»**|Указывает количество цифр в числе. Например, число 123,45 имеет точность 5.<br /><br /> Значение должно быть положительным целым числом.|Необязательный (доступен только в том случае, если значение xsi:type является переменным числовым типом данных)|  
