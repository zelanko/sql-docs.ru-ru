...
description: Узнайте о поддержке для взаимной проверки подлинности имен субъектов-служб в SQL Server Native Client.
ms.custom: ''
ms.date: 08/08/2016
ms.prod: sql
ms.reviewer: ''
...
|*MachineName* $@ *Домен*|Прямое указание учетной записи компьютера<br /><br /> (если сервер, с которым выполняется соединение, работает с учетными записями LOCAL SYSTEM или NETWORK SERVICE, то для проверки подлинности Kerberos **ServerSPN** может указываться в формате *MachineName*$@*Domain* ).|  
