...
-   Повышения Чтобы включить использование имен субъектов-служб по умолчанию, учетная [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] запись службы, в которой выполняется экземпляр, должна иметь достаточные права для обновления Active Directory в KDC.  
  
-   Удобство и прозрачность расположения: Как будут затронуты имена участников-служб приложения, если его база данных будет перемещена в другой [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] экземпляр? Это относится как к основному серверу, так и его партнеру по обеспечению отработки отказа при зеркальном отображении базы данных. Если при смене сервера потребуется замена имен участников-служб, как это повлияет на приложения? Будут ли изменения управляемыми?  
