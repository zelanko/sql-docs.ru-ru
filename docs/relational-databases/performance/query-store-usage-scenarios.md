---
title: "Сценарии использования хранилища запросов | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "04/12/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Хранилище запросов, сценарии использования"
...
-   Поддержание стабильной производительности во время обновления до [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]  
...
-   Принудительно вернуться к предыдущему плану в журнале, если тот оказался лучше. Чтобы принудительного выбрать план для запроса, нажмите кнопку **Force Plan** (Принудительный выбор плана) в разделе **Regressed Queries** (Регрессивные запросы).  
...
4.  Сравните результаты шагов 1 и 3.  
    ...
    1.  Откройте раздел **Overall Database Consumption** (Использование общей базы данных), чтобы определить влияние на всю базу данных.  
...
## Поддержание стабильной производительности во время обновления до SQL Server 2016  
 ...
 Начиная с [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] , все изменения в оптимизаторе запросов привязаны к последнему уровню `COMPATIBILITY_LEVEL`, поэтому планы изменяются не в момент обновления, а тогда, когда пользователь изменяет `COMPATIBILITY_LEVEL` на последнюю версию. В сочетании с хранилищем запросов эта возможность обеспечивает высокий уровень контроля над производительностью запросов в процессе обновления. Рекомендуемый рабочий процесс обновления показан на следующем рисунке.  
  
 ![query-store-usage-5](../../relational-databases/performance/media/query-store-usage-5.png "query-store-usage-5")  
  
1.  Обновление [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] без изменения `COMPATIBILITY_LEVEL`. Не устанавливает последнюю версию оптимизатора запросов, но предоставляет функции [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] , включая хранилище запросов.  
  
2.  Включение хранилища запросов: регистрация запросов и планов и установка базового уровня производительности с предыдущим уровнем `COMPATIBILITY_LEVEL`. Выполняйте этот этап достаточно долго для того, чтобы зафиксировать все планы и получить стабильный базовый уровень.  
  
3.  Переход на последний уровень совместимости: передайте рабочую нагрузку в оптимизатор запросов и позвольте ему заполнить возможные новые планы.  
  
4.  Использование хранилища запросов для анализа и устранения проблемы регрессии: в большинстве случаев новый оптимизатор запросов должен улучшать планы. Тем не менее хранилище запросов позволяет легко установить регрессию при выборе плана и устранить ее с помощью механизма принудительного применения планов.  
  
## Определение и улучшение нерегламентированных рабочих нагрузок  
 В некоторых случаях рабочие нагрузки не содержат доминантных запросов, которые можно настроить для повышения общей производительности приложений. Обычно такую рабочую нагрузку характеризует относительно большое количество различных запросов, каждый из которых задействует часть системных ресурсов. Поскольку эти запросы уникальны, они выполняются очень редко (обычно лишь один раз, в связи с чем называются нерегламентированными), поэтому не оказывают существенного влияния на ресурсы системы. С другой стороны, учитывая, что приложение постоянно формирует новые запросы, значительная часть системных ресурсов расходуется на компиляцию запросов, а это не оптимально. Это не идеальная ситуация для хранилища запросов еще и потому, что множество запросов и планов заполняют зарезервированное вами пространство, а значит, хранилище запросов быстрее переходит в режим доступности только для чтения. Если вы активировали **политику очистки на основе размера** ([настоятельно рекомендуется](https://msdn.microsoft.com/library/mt604821.aspx) для поддержания работоспособности хранилища запросов), фоновый процесс будет очищать структуры хранилища запросов большую часть времени и, таким образом, также задействовать значительную часть системных ресурсов.  
