...
При запуске хранилища запросов в Azure [!INCLUDE[ssSDS](../../includes/sssds-md.md)]можно использовать [Анализ производительности запросов](https://docs.microsoft.com/azure/sql-database/sql-database-query-performance) для анализа потребления ресурсов с течением времени. Хотя вы можете использовать [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] и [Azure Data Studio](https://docs.microsoft.com/sql/azure-data-studio/what-is) для получения подробного потребления ресурсов для всех запросов, таких как ЦП, память и ввод-вывод, анализ производительности запросов предоставляет быстрый и эффективный способ определения их влияния на общее использование DTU для базы данных. Дополнительные сведения см. в разделе [Анализ производительности запросов в базе данных SQL Azure](https://azure.microsoft.com/documentation/articles/sql-database-query-performance/).

В этом разделе описаны оптимальные параметры по умолчанию, разработанные для обеспечения надежной работы хранилища запросов и зависимых компонентов. По умолчанию конфигурация оптимизирована для постоянного сбора данных, т. е. для минимальной продолжительности состояний "Отключено" и "Только для чтения".
...
> [!IMPORTANT]
> Эти значения по умолчанию будут автоматически применяться на последнем этапе активации хранилища запросов для всех баз данных Azure SQL (см. важное примечание выше). После этого база данных SQL Azure не будет изменять значения конфигурации, установленные клиентами, если они отрицательно влияют на основную рабочую нагрузку или надежные операции хранилища запросов.
...
## <a name="Configure"></a>Сохранение хранилища запросов в рабочей нагрузке
...
**Максимальный размер (МБ)**: определяет ограничение для пространства данных, которое хранилище запросов использует в базе данных. Это наиболее важный параметр, который непосредственно влияет на режим работы хранилища запросов.
...
**Интервал сбора статистики**: определяет уровень гранулярности для собираемой статистики среды выполнения, выраженную в минутах. Значение по умолчанию — 60 минут. Рекомендуется использовать меньшее значение, если требуется большая степень детализации или меньшее время на обнаружение и устранение проблем. Помните, что это значение напрямую влияет на объем данных в хранилище запросов. Чтобы задать другое значение для [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]интервала сбора статистики[!INCLUDE[tsql](../../includes/tsql-md.md)], используйте ** или **:
...
**Порог устаревания запроса (в днях)**: политика очистки на основе времени, которая управляет сроком хранения статистики времени выполнения и неактивными запросами, выраженными в днях. По умолчанию для хранилища запросов настроено хранение данных в течение 30 дней, что может быть излишне долго для вашего сценария.
...
**Режим очистки на основе размера**: определяет, происходит ли автоматическая очистка данных, когда размер данных хранилища запросов приближается к пределу. Активируйте очистку на основе размера, чтобы хранилище запросов всегда работало в режиме чтения и записи и собирало последние данные.
...
**Режим записи хранилища запросов**: определяет политику записи запросов для хранилища запросов.

- **ALL**: фиксирует все запросы. Этот параметр используется по умолчанию в [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)].
- **Auto**: редкие запросы и запросы с незначительной длительностью компиляции и выполнения игнорируются. Пороговые значения для числа выполнений, длительности компиляции и времени выполнения определяются внутренним образом. Начиная с версии [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] это параметр по умолчанию.
- **Нет**: хранилище запросов прекращает запись новых запросов.
- **Custom**: предоставляет дополнительный контроль и возможность точной настройки политики сбора данных. Новые пользовательские параметры определяют действия, которые выполняются при достижении внутреннего порогового значения времени для политики записи. Это временная граница, в пределах которой происходит оценка настраиваемых условий и, если какие-либо из них соблюдаются, разрешается запись запроса в хранилище запросов.
...
|---------------|--------------|
|**Регрессионные запросы**|Выявите запросы, метрики выполнения для которых недавно регрессировали (т. е. стали хуже). <br />Используйте это представление для сопоставления наблюдаемых проблем производительности в приложении с фактическими запросами, которые необходимо улучшить или исправить.|
|**Общее использование ресурсов**|Анализируйте общее потребление ресурсов базы данных для любой из метрик выполнения.<br />Используйте это представление для определения шаблонов ресурсов (дневная и ночная рабочие нагрузки) и оптимизации общего потребления для базы данных.|
|**Топ ресурсоемких запросов**|Выберите интересующую метрику выполнения и определите запросы, которые имели максимальные значения в указанном промежутке времени. <br />Используйте это представление, чтобы сосредоточиться на наиболее важных запросах, которые оказывают самое большое воздействие на потребление ресурсов базы данных.|
...
|**Запросы с высокой вариацией**|Анализ запросов с высокой вариативностью выполнения с учетом всех доступных параметров, таких как длительность, время ЦП, ввод-вывод данных и использование памяти, в соответствующем временном интервале.<br />Используйте это представление для выявления запросов с сильно варьируемой производительностью, которые могут влиять на работу пользователей в приложениях.|
|**Статистика ожидания запросов**|Анализируйте категории ожидания, наиболее активные в базе данных, и определите, какие запросы вносят наибольший вклад в выбранную категорию ожидания.<br />Используйте это представление, чтобы проанализировать статистику ожидания и определить запросы, которые могут влиять на работу пользователей в приложениях.<br /><br />Применимо к: начиная [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] с версии v [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)]18.0 и.|
|**Отслеживание запросов**|Отслеживайте выполнение наиболее важных запросов в реальном времени. Как правило, эти представления используются, когда имеются запросы с принудительными планами и требуется убедиться в стабильной производительности запросов.|
...
- Если запрос выполнялся с несколькими планами и последний план оказался значительно хуже предыдущего, можно применить механизм принудительного использования плана. 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] попытается принудительно применить план в оптимизаторе. Если это не удастся сделать, будет порождено событие XEvent и оптимизация будет выполнена как обычно.
...
- Вы можете решить, что в запросе отсутствует индекс для оптимального выполнения. Эта информация будет отображена в плане выполнения запроса. Создайте отсутствующий индекс и проверьте производительность запросов в хранилище Усингкуери.

   ![Хранилище запросов — план отображения](../../relational-databases/performance/media/query-store-show-plan.png "query-store-show-plan")
...
## <a name="Verify"></a>Убедитесь, что хранилище запросов постоянно собирает данные запросов.
...
|------------------------|--------------|
|**все**|Тщательно анализируйте рабочую нагрузку на основе форм всех запросов и частоты их выполнения, а также других статистических данных.<br /><br /> Выявление новых запросов в рабочей нагрузке.<br /><br /> Обнаружение использования оперативных запросов с целью выявления возможностей для пользовательской или автоматической параметризации.<br /><br />Примечание. это режим записи по умолчанию в [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)].|
|**Авто**|Сосредоточьте внимание на важных и действенных запросах. Примерами могут служить запросы, которые выполняются регулярно или потребляют ресурсы в значительных объемах.<br /><br />Примечание. Начиная с [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)], этот режим используется по умолчанию.|
...
## <a name="Parameterize"></a>Старайтесь не использовать непараметризованные запросы
...
## <a name="Drop"></a>Избегайте шаблона DROP и CREATE для объектов, содержащих объекты
...
## <a name="CheckForced"></a>Регулярно проверять состояние принудительных планов
...
## <a name="Renaming"></a>Избегайте переименования баз данных для запросов с принудительными планами
...
## <a name="Recovery"></a>Использование флагов трассировки на критически важных серверах
...
> [!IMPORTANT]
> Если вы используете хранилище запросов для JIT-анализа рабочих нагрузок в [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)], запланируйте установку исправлений масштабируемости производительности ([статья базы знаний 4340759](https://support.microsoft.com/help/4340759)) как можно скорее.
...
- [Руководством по архитектуре обработки запросов](../../relational-databases/query-processing-architecture-guide.md)
