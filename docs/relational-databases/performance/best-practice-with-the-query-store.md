...
|Запросы с принудительными планами|Списки ранее принудительно планов с помощью хранилища запросов. <br />Это представление используется для быстрого доступа к все в настоящее время принудительными планами.|  
|Запросы с высокой вариативностью|Анализ запросов с помощью варианта высокий уровень выполнения по отношению к любой из доступных измерений, например использование длительность, время ЦП, ввода-ВЫВОДА и памяти в период времени.<br />Это представление используется для идентификации запросов с широко variant производительности, может повлиять на взаимодействие с пользователем для приложений.|  
...
>  Подробное описание того, как использовать [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] для выявления запросов с самым большим потреблением ресурсов и исправления тех, чьи метрики ухудшились из-за изменения плана, см. в [блогах @Azureпо хранилищу запросов в Azure](https://azure.microsoft.com/blog/query-store-a-flight-data-recorder-for-your-database/).  
 ...
 При указании запроса с недостаточной производительностью действие зависит от природы проблемы.  
...
> [!NOTE]  
> Приведенная выше интересные различные формы для планов определенный запрос с следующие значения для всех возможных состояний:<br />  
> |Фигурная|Значение|  
> |-------------------|-------------|
> |Circle|Запрос завершен (регулярное выполнение успешно завершено)|
> |Square|Отменена (инициируется клиентом прервал выполнение)|
> |Triangle|Не удалось (исключение прервано выполнение)|
> Кроме того размер фигуры отражает число выполнений запроса в течение указанного интервала времени, увеличение размера с более высоким номером выполнений.  
...
```  
 ...
 Если проблема сохранится, это означает повреждение данных, сохраненных на диске хранилище запросов.
 
 Хранилище запросов можно освободить путем выполнения **sp_query_store_consistency_check** хранимой процедуры в соответствующей базе данных.
 
 Если это не помогает, можно попытаться очистить хранилище запросов перед запросом режима чтения и записи.  
