 ...
 **Максимальный размер (МБ)** . Задает предельный объем пространства данных, который будет занимать хранилище запросов в базе данных. Это наиболее важный параметр, который непосредственно влияет на режим работы хранилища запросов.  
 ...
 Значение по умолчанию в версиях [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] (100 МБ) может быть недостаточным, если рабочая нагрузка создает большое количество различных запросов и планов или если вы хотите хранить журнал запросов за более длительный период времени. Начиная с версии [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] значение по умолчанию равно 1 ГБ. Отслеживайте текущее использование пространства и увеличивайте максимальный размер (МБ), чтобы предотвратить переход хранилища запросов в режим "только чтение". Используйте [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] или выполните следующий скрипт, чтобы получить последние сведения о размере хранилища запросов.  
...
-   **All** — записываются все запросы. Это параметр по умолчанию в [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)].  
...
-   **Custom** — разрешает дополнительное управление политикой сбора данных и ее тонкую настройку. Новые пользовательские параметры определяют действия, которые выполняются при достижении внутреннего порогового значения времени для политики записи: временная граница, в пределах которой происходит оценка настраиваемых условий и, если какие-либо из них соблюдаются, разрешается запись запроса в хранилище запросов.
...
В следующем примере для хранилища запросов устанавливаются режим записи запросов Auto и остальные рекомендованные параметры в [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]:  
...
В следующем примере для хранилища запросов устанавливаются режим записи запросов Auto и остальные рекомендованные параметры в [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] для включения статистики ожидания:  
...
В следующем примере для хранилища запросов устанавливаются режим записи запросов Auto и остальные рекомендованные параметры в [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)], а также **при необходимости** устанавливается настраиваемая политика записи с параметрами по умолчанию вместо нового режима автоматического сбора по умолчанию.  
...
```  
 ...
 В версии [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] и выше хранилище запросов можно восстановить, выполнив хранимую процедуру **sp_query_store_consistency_check** в соответствующей базе данных. В версии [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] необходимо очистить данные в хранилище запросов, как показано ниже.
...
|Auto|Сосредоточьте внимание на важных и действенных запросах; это запросы, которые выполняются регулярно или потребляют ресурсы в значительных объемах.<br /><br />**Примечание.** Начиная с версии [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] это режим записи по умолчанию.|  
...
##  <a name="Recovery"></a> Использование флагов трассировки для критически важных серверов
 
Глобальные флаги трассировки 7745 и 7752 можно использовать для повышения уровня доступности баз данных, использующих хранилища запросов. Дополнительные сведения см. в разделе [Флаги трассировки](../../t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql.md).
  
-  Флаг трассировки 7745 будет препятствовать поведению по умолчанию, при котором хранилище запросов записывает данные на диск до того, как [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] сможет завершить работу. Это означает, что будут потеряны все собранные данные хранилища запросов, которые еще не сохранены на диске. 
  
-  Флаг трассировки 7752 включает асинхронную загрузку для хранилища запросов. Это позволяет восстановить базу данных и выполнять запросы раньше, чему будет полностью восстановлено хранилище запросов. По умолчанию загрузка хранилища запросов выполняется в синхронном режиме. Этот вариант не позволяет выполнять запросы до полного восстановления хранилища запросов, но зато предотвращает потерю запросов при сборе данных.

   > [!NOTE]
   > Начиная с версии [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] эта реакция управляется подсистемой и флаг трассировки 7752 не оказывает влияния.
