---
title: "Рекомендации по хранилищу запросов | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "11/24/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Хранилище запросов, рекомендации"
...
##  <a name="a-namessmsa-use-the-latest-sql-server-management-studio"></a><a name="SSMS"></a> Используйте последний выпуск SQL Server Management Studio  
...
Скачайте последнюю версию [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] на странице [https://msdn.microsoft.com/library/mt238290.aspx](https://msdn.microsoft.com/library/mt238290.aspx)  
...
##  <a name="a-nameinsighta-use-query-performance-insight-in-azure-sql-database"></a><a name="Insight"></a> Используйте анализ производительности процессов в Базе данных SQL Azure  
...
##  <a name="a-nameconfigurea-keep-query-store-adjusted-to-your-workload"></a><a name="Configure"></a> Поддерживайте адаптированность хранилища запросов к рабочей нагрузке  
...
Параметры по умолчанию подходят для быстрого запуска, но вы должны наблюдать за поведением хранилища запросов с течением времени и соответствующим образом настраивать его конфигурацию.  
...
##  <a name="a-nameverifya-verify-query-store-is-collecting-query-data-continuously"></a><a name="Verify"></a> Проверяйте непрерывность сбора данных запросов хранилищем запросов  
...
-   Вы можете предотвратить автоматические изменения режима работы, применяя рекомендации. Если обеспечить, чтобы размер хранилища запросов всегда был меньше максимально допустимого значения, это существенно уменьшит вероятность перехода в режим "только чтение". Активируйте политику на основе размера, как описано в разделе [Настройка хранилища запросов](#Configure) , чтобы хранилище запросов автоматически очищало данные при достижении предельного размера.  
...
```  
 ...
 Если проблема сохраняется, это означает, что имеется повреждение данных хранилища запросов, сохраненных на диске. Необходимо очистить хранилище запросов перед запросом режима чтения и записи.  
...
|Автоматически|Сосредоточьте внимание на важных и действенных запросах; это запросы, которые выполняются регулярно или потребляют ресурсы в значительных объемах.|  
...
##  <a name="a-nameparameterizea-avoid-using-non-parameterized-queries"></a><a name="Parameterize"></a> Старайтесь не использовать запросы без параметров  
 Не рекомендуется использовать запросы без параметров там, где это не является абсолютной необходимостью (например, в случае динамического анализа).  Кэшированные планы не могут использоваться повторно, что заставляет оптимизатор запросов компилировать запросы для каждого уникального текста запроса.  
...
-   Параметризация запросов везде, где это возможно, например перенос запросов в хранимую процедуру.  
  
-   Использование параметра **Оптимизировать для нерегламентированной рабочей нагрузки**, если рабочая нагрузка содержит много динамических пакетов однократного использования с разными планами запроса.  
...
-   Применение параметра FORCED PARAMETERIZATION для базы данных или подмножества запросов, если количество разных планов запроса невелико.  
  
    -   Используйте структуру плана для принудительного применения параметризации только в выбранном запросе.  
  
    -   Настройте FORCED PARAMETERIZATION для базы данных, если в рабочей нагрузке существует небольшое число разных планов запроса (когда коэффициент отношения между числом разных query_hash и общим числом записей в sys.query_store_query намного меньше чем 1).  
...
##  <a name="a-namedropa-avoid-a-drop-and-create-pattern-when-maintaining-containing-objects-for-the-queries"></a><a name="Drop"></a> Старайтесь не использовать шаблоны DROP и CREATE при обслуживании содержащих объектов для запросов  
...
```  
 ...
 Полный список причин см. в разделе [sys.query_store_plan (Transact-SQL)](../../relational-databases/system-catalog-views/sys-query-store-plan-transact-sql.md). Кроме того, можно использовать XEvent **query_store_plan_forcing_failed** для отслеживания сбоев принудительного выполнения планов устранения неполадок.  
  
##  <a name="a-namerenaminga-avoid-renaming-databases-if-you-have-queries-with-forced-plans"></a><a name="Renaming"></a> Старайтесь не переименовывать базы данных при наличии запросов с принудительными планами  
