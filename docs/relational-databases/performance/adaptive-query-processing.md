...
> [!NOTE]
> Атрибуты плана обратной связи с временно предоставляемым буфером памяти отображаются в графических планах выполнения запросов в SQL Server Management Studio 17.9 и более поздних версий. 
...
Вы также можете отключить сброс данных во временно предоставляемый буфер памяти в строчном режиме для определенного запроса, назначив DISABLE_ROW_MODE_MEMORY_GRANT_FEEDBACK в качестве указания запроса USE HINT.  Пример:
