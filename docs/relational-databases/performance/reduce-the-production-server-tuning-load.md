...
description: Сведения о том, как помощник по настройке ядра СУБД полагается на оптимизатор запросов, который анализирует рабочую нагрузку и создает рекомендации по настройке в SQL Server.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
4.  Примените рекомендацию, полученную при настройке тестового сервера, на рабочем сервере.  
 ...
 ![Использование тестового сервера с помощником по настройке ядра СУБД](../../relational-databases/performance/media/testsvr.gif "Использование тестового сервера с помощником по настройке ядра СУБД")  
