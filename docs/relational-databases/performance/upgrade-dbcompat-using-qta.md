...
4.  Запрос на второй сбор данных рабочей нагрузки в целях сравнения и обнаружения регрессии.
...
Ниже показано, как QTA, по сути, меняет только последние этапы рекомендуемого рабочего процесса для обновления уровня совместимости с помощью упоминаемого выше хранилища запросов. Вместо возможности выбора между текущим неэффективным планом выполнения и последним известным удачным планом выполнения QTA представляет параметры настройки, соответствующие выбранным регрессионным запросам, для формирования нового улучшенного состояния с настроенными планами выполнения.
...
3.  В окне мастера QTA необходимо выполнить два действия для настройки сеанса.
    ...
    1.  В окне **Настройка** настройте хранилище запросов для записи эквивалента одного полного бизнес-цикла данных рабочей нагрузки с целью анализа и настройки. 
    ...
    2.  В окне **Параметры** в двух столбцах отображаются **Текущие** настройки хранилища запросов в целевой базе данных, а также **Рекомендуемые** параметры. 
        -  По умолчанию выбраны "Рекомендуемые" параметры, но если установить переключатель над столбцом "Текущие", будут приняты текущие параметры и станет доступна точная настройка текущей конфигурации хранилища запросов. 
...
3.  Отправной точкой для нового сеанса является шаг **Сбор данных**. 
    ...
    Этот шаг состоит из трех вложенных шагов.
