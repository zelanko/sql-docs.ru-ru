...
description: Сведения о том, как использовать тестовый сервер для настройки базы данных на рабочем сервере. Это важное преимущество помощника по настройке ядра СУБД.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
