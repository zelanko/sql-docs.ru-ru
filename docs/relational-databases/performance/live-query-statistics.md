...
Для заполнения страницы результатов `SHOWPLAN`Динамическая статистика запросов**требуется разрешение** уровня базы данных. Для выполнения запроса требуются все разрешения, необходимые для этого.
