...
|**Имя файла**|**nvarchar(260)**|Имя основного файла базы данных и путь к нему в операционной системе.<br /><br /> **Имя файла** является видимым для **dbcreator**, **sysadmin**, владелец базы данных с разрешениями CREATE ANY DATABASE или участникам, которым предоставлены следующие разрешения: ALTER ANY DATABASE СОЗДАНИЕ ЛЮБОЙ БАЗЫ ДАННЫХ, ПРОСМОТР ЛЮБОГО ОПРЕДЕЛЕНИЯ. Чтобы получить путь и имя файла, запросите [sys.sysfiles](../../relational-databases/system-compatibility-views/sys-sysfiles-transact-sql.md) Просмотр в режиме совместимости, или [sys.database_files](../../relational-databases/system-catalog-views/sys-database-files-transact-sql.md) представления.|  
