...
|**Имя файла**|**nvarchar(260)**|Имя основного файла базы данных и путь к нему в операционной системе.<br /><br /> **Имя файла** является видимым для **dbcreator**, **sysadmin**, владелец базы данных с разрешениями CREATE ANY DATABASE или участникам, которым предоставлены следующие разрешения: ALTER ANY БАЗЫ ДАННЫХ, СОЗДАНИЕ ЛЮБОЙ БАЗЫ ДАННЫХ, VIEW ANY DEFINITION. Чтобы получить путь и имя файла, запросите [sys.sysfiles](../../relational-databases/system-compatibility-views/sys-sysfiles-transact-sql.md) представление совместимости или [sys.database_files](../../relational-databases/system-catalog-views/sys-database-files-transact-sql.md) представления.|  
