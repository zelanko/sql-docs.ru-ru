...
|-----------------|---------------|-----------------|  
|**удостоверения**|**int**|Идентификатор таблицы, которой принадлежит данный индекс.|  
|**состояние**|**int**|Сведения о состоянии системы.<br /><br /> [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
...
|**корневой**|**двоичный (6)**|Для **indid** >= 1 **корень** является указателем на корневую страницу.<br /><br /> Не используется, если **столбец indid** = 0.<br /><br /> NULL = индекс секционирован, если **столбец indid** > 1.<br /><br /> NULL = таблица секционирована, если **столбец indid** имеет значение 0 или 1.|  
...
|**name**|**имеет sysname**|Имя индекса или статистики. Возвращает значение NULL, если **столбец indid** = 0. Измените приложение, чтобы оно выполняло поиск кучи с именем NULL.|  
|**statblob**|**Эскиз**|Статистический большой двоичный объект (BLOB).<br /><br /> Возвращает значение NULL.|  
...
-   Ничего не предпринимать. Новое значение **ровмодктр** часто помогает определить, когда следует обновлять статистику, так как это поведение практически близко к результатам предыдущих версий.  
