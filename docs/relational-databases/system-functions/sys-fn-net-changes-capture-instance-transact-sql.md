...
2.  выполнить скрипт, чтобы создать функцию-оболочку.  
 ...
 Затем к результирующему набору добавляются битовые флаги для каждого столбца, указанного в параметре @update_flag_list. Для оболочки **net changes** битовые флаги всегда будут иметь значение null, если @row_filter_option объект, используемый в вызове функции-оболочки, равен ALL или все вместе с MERGE. Если для @row_filter_option параметра задано значение "все с маской", а __CDC_OPERATION — "d" или "I", то значение флага также будет равно null. Если \__CDC_OPERATION имеет значение "UN", флаг будет установлен в 1 или 0 в зависимости от того, привело ли операция **net** Update к изменению столбца.  
