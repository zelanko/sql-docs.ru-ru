...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  Оболочки для **net изменения** функций запроса. Скрипты, необходимые для создания этих функций, создаются хранимой процедурой sys.sp_cdc_generate_wrapper_function.  
 ...
 **Datetime** значение, представляющее нижнюю конечную точку диапазона записей таблицы изменений, чтобы включить в результирующий набор.  
 ...
 Только те строки cdc. < capture_instance > _CT изменений таблицы, для которых время фиксации превышает *start_time* включаются в результирующий набор.  
...
-   @closed_high_end_point = 1  
     ...
     Только те таблицы, которые содержат значение в изменений строк в cdc. < capture_instance > _CT \_ \_$start_lsn и соответствующее фиксации которых меньше или равно времени **start_time** включаются в результирующий набор.  
  
-   @closed_high_end_point = 0  
  
     Только те таблицы, которые содержат значение в изменений строк в cdc. < capture_instance > _CT \_ \_$start_lsn и соответствующее время фиксации которых меньше, чем **start_time** включаются в результирующий набор.  
