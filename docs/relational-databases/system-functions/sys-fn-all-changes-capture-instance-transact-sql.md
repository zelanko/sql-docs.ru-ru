...
|\<столбцов из @column_list|**различать**|Столбцы, указанные в аргументе *column_list* , sp_cdc_generate_wrapper_function при вызове для создания скрипта, создающего функцию-оболочку.|  
...
2.  выполнить скрипт, чтобы создать функцию-оболочку.  
 ...
 Результирующий набор, возвращаемый функцией-оболочкой **All Changes** , возвращает __ $ start_lsn и \_\_$seqval столбцы таблицы изменений в виде СТОЛБЦОВ \__CDC_STARTLSN и \__CDC_SEQVAL соответственно. В нем следуют только те столбцы, которые были записаны в параметре *\@column_list* при создании оболочки. Если *\@column_list* имеет значение null, возвращаются все отслеживания исходных столбцов. За исходными столбцами следует столбец операции, \__CDC_OPERATION, который является столбцом из одного или двух символов, идентифицирующим операцию.  
