---
title: Время дат в версиях S'L (ODBC)
ms.custom: ''
ms.date: 12/18/2019
ms.prod: sql
...
## <a name="down-level-client-behavior"></a>Работа в клиентах низкого уровня  
 Клиентские приложения, скомпилированные с помощью собственного клиента [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] версии более ранней, чем [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)], распознают новые типы даты и времени, как столбцы nvarchar. Содержимое столбца представляет сявра в буквальном виде, описанном в разделе «Форматы данных: строки и буквы» [службы поддержки типа данных для улучшения даты и времени ODBC.](../../relational-databases/native-client-odbc-date-time/data-type-support-for-odbc-date-and-time-improvements.md) Размер столбца равен максимальной длине литерала для долей секунды, указываемых с заданной для столбца точностью.  
 ...
 Метаданные заявления, возвращенные S'LDescribeCol, S'LDescribeParam, S'GetDesccField и S'LColAttribute, будут возвращать метаданные, соответствующие типу уровня вниз во всех отношениях, включая имя типа. Примером такого нистого типа является **nvarchar**.  
...
||Time(7)|SQL_C_TIME|Неудачи - недействительное время буквальное.|OK (1)|  
|||SQL_C_TYPE_TIMESTAMP|Неудачи - недействительное время буквальное.|OK (1)|  
