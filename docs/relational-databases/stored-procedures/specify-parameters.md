...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)] Путем указания параметров процедуры вызывающие программы могут передавать значения в тело процедуры. Эти значения могут использоваться для разных целей во время исполнения процедуры. Параметры процедуры могут также возвращать значения вызывающей программе, если параметр помечен признаком OUTPUT.  
...
> [!NOTE]  
>  Если любое из значений параметров предоставляется в виде **@parameter =***value*, все последующие параметры необходимо предоставлять в том же формате. Если для значений параметров не используется формат **@parameter =***value*, все значения должны предоставляться в строгом порядке (слева направо), в котором они перечислены в инструкции CREATE PROCEDURE.  
  
> [!WARNING]  
>  Если имя любого параметра в формате **@parameter =***value* будет указано с ошибкой, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] создает ошибку и отменяет выполнение процедуры.  
...
> [!NOTE]  
>  Переменная OUTPUT должна быть определена во время создания процедуры, а также в ходе использования переменной. Имена параметра и переменной не должны совпадать. Однако тип данных и положение параметра должны соответствовать определению (если не используется формат **@listprice =***variable*).  
