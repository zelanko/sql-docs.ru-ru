...
-   Для столбцов типа **varbinary(max)** можно указать любой тип документа, в котором поддерживается полнотекстовое индексирование. Дополнительные сведения см. в разделе [Как определить, какие типы документов могут быть проиндексированы](#doctypes) в этой статье.  
...
##  <a name="BestPracticeFilegroup"></a> Рекомендация. Создайте отдельную файловую группу для полнотекстового и семантического индексов  
