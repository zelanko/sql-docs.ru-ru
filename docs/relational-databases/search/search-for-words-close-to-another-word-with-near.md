---
title: "Поиск слов близких к другим с использованием оператора NEAR | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
dev_langs: 
  - "TSQL"
helpviewer_keywords: 
  - "поиск слов [полнотекстовый поиск]"
  - "параметр NEAR [полнотекстовый поиск]"
  - "поиск фраз [полнотекстовый поиск]"
  - "поиск по сходству [полнотекстовый поиск]"
  - "полнотекстовый поиск [SQL Server], поиск с учетом расположения"
  - "полнотекстовые запросы [SQL Server], с учетом расположения"
  - "запросы [полнотекстовый поиск], с учетом расположения"
 ...
 *условие_поиска* [ ,…*n* ]  
 ...
 (*условие_поиска* [ ,…*n* ] ) [, \<максимальное_расстояние> [, \<порядок_сопоставления> ] ]  
...
> [!NOTE]  
>  Дополнительные сведения о синтаксисе <custom_proximity_term> см. в разделе [CONTAINS (Transact-SQL)](../../t-sql/queries/contains-transact-sql.md).  
...
```  
 ...
 Настраиваемое выражение с учетом расположения нельзя сочетать с универсальным выражением с учетом расположения (*выражение1* NEAR *выражение2*), производным выражением (ISABOUT…) или взвешенным выражением (FORMSOF…).  
...
##  <a name="Generic_NEAR"></a> Устаревшее универсальное выражение с учетом расположения  
  
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] Рекомендуется использовать [настраиваемое выражение с учетом расположения](#Custom_NEAR).  
...
> [!NOTE]  
>  Сведения о синтаксисе <generic_proximity_term> см. в разделе [CONTAINS (Transact-SQL)](../../t-sql/queries/contains-transact-sql.md).  
