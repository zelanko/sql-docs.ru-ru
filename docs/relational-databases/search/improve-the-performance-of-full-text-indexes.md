---
title: "Улучшение производительности полнотекстовых индексов | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Производительность [SQL Server], полнотекстовый поиск"
  - "Полнотекстовые запросы [SQL Server], производительность"
  - "сканирование [полнотекстовый поиск]"
  - "Полнотекстовые индексы [SQL Server], производительность"
  - "Полнотекстовый поиск [SQL Server], производительность"
  - "Пакеты [SQL Server], полнотекстовый поиск"
...
-   Если не хватает физической памяти, то узким местом системы может оказаться память.  
...
##  <a name="full"></a> Устранение неполадок с производительностью полного заполнения  
 Для диагностики проблем производительности следует изучить журналы полнотекстового сканирования. Сведения о журналах сканирования см. в разделе [Заполнение полнотекстовых индексов](../../relational-databases/search/populate-full-text-indexes.md).  
...
> [!NOTE]  
>  Сведения о буферах памяти, используемых полнотекстовым сканированием, см. в разделе [sys.dm_fts_memory_buffers (Transact-SQL)](../../relational-databases/system-dynamic-management-views/sys-dm-fts-memory-buffers-transact-sql.md).  
...
|--------------|-----------------------------------------------------------|-----------------------------------------------------|  
|x86|*F* **=** *Число диапазонов сканирования***\*** 50|*M* **=minimum(** *T* **,** 2000**)–***F***–** 500|  
|x64|*F* **=** *Число диапазонов сканирования* **\*** 10 **\*** 8|*M* **=** *T* **–** *F* **–** 500|  
  
 (1) Если параллельно выполняется несколько полных заполнений, то требования к памяти fdhost.exe для каждого из них следует вычислять отдельно, как *F1*, *F2* и т. д. Затем вычислить *M* как *T***–** sigma**(***F*i**)**.  
 ...
 (3) Предполагается, что значение .*ism_size* равно 8 МБ для платформ x64.  
...
##  <a name="filters"></a> Устранение неполадок со снижением производительности индексации, связанных с фильтрами  
