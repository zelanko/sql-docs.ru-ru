...
## <a name="returns"></a>Возвращает  
 Указатель на строку, заканчивающуюся нулевым символом, которая содержит имя параметра. Длина имени параметра хранится в *len*. Если отсутствует *n*-й параметр или удаленная хранимая процедура, то возвращается значение NULL, для *len* устанавливается значение –1 и передается информационное сообщение об ошибке. Если имя параметра равно NULL, для *len* устанавливается значение 0 и возвращается пустая строка, заканчивающаяся нулевым символом.  
