...
## <a name="returns"></a>Возвращает  
 Указатель на строку, заканчивающуюся нулевым символом, которая содержит имя параметра. Длина имени параметра хранится в *len*. При наличии не *n*-й параметр или удаленная хранимая процедура отсутствует, то возвращается значение NULL, *len* имеет значение -1, и отправляется информационное сообщение об ошибке. Если имя параметра равно NULL, для *len* устанавливается значение 0 и возвращается пустая строка, заканчивающаяся нулевым символом.  
