 ...
 **SQL: Limit-значение** заметка используется для указания предельного значения в столбце, который указан в **SQL: Limit-поле** заметки. **SQL: Limit-значение** заметка необязательна. Если **SQL: Limit-значение** — не указано, предполагается значение NULL.  
  
> [!NOTE]  
>  При работе с **SQL: Limit-поле** где сопоставляемый столбец SQL имеет тип **реальные**, SQLXML 4.0 выполняет преобразование **SQL: Limit-значение** , как указано в XML-схем как **nvarchar** заданное значение. Для этого необходимо, чтобы десятичные граничные значения были указаны в полном экспоненциальном представлении. Дополнительные сведения см. в приведенном ниже Примере Б.  
...
-   Addresses (CustomerID, AddressType, StreetAddress)  
 ...
 Это схема сопоставления, в котором **ShipTo** атрибут схемы сопоставляется со столбцом StreetAddress в связи Addresses. Возвращаемые для этого атрибута значения ограничиваются только адреса доставки, указав **SQL: Limit-поле** и **SQL: Limit-значение** заметок. Аналогичным образом **BillTo** атрибут схемы возвращает только адреса выставления счетов для клиента.  
...
-   OrderDetails (OrderID, ProductID, UnitPrice, Quantity, Price, Discount)  
 ...
 Это схема сопоставления, в котором **OrderID** атрибут сведений о заказе сопоставляется со столбцом OrderID в связи orders. Будут возвращены для этого атрибута значения ограничиваются только теми, которые имеют значение равно 2.0000000e-001 (0.2), указанным для **скидки** атрибута с помощью **SQL: Limit-поле** и **SQL: Limit-значение** заметок.  
