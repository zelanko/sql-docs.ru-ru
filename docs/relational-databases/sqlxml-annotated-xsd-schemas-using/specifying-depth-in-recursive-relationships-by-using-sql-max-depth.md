...
>  Чтобы создать различные глубины иерархий в результате, измените значение заметки **SQL: max-depth** в схеме и снова выполните шаблон после каждого изменения.  
 ...
 В предыдущей схеме все элементы **\<Emp >** имели один и тот же набор атрибутов (**EmployeeID**, **FirstName**и **LastName**). Следующая схема была немного изменена, чтобы возвратить дополнительный атрибут **подчиняется** для всех **\<элементов > EMP** , которые сообщают руководителю.  
...
### <a name="nonrecursive-elements"></a>Нерекурсивные элементы  
 Если Аннотация **SQL: max-depth** задана для элемента в схеме, который не вызывает рекурсию, он игнорируется. В следующей схеме элемент **\<EMP** состоит из **\<константного >** дочернего элемента, который, в свою очередь, имеет дочерний элемент **\<EMP >** .  
  
 В этой схеме Аннотация **SQL: max-depth** , указанная в элементе **\<Constant >** , игнорируется, так как отсутствует рекурсия между родительским **\<EMP >** и дочерним элементом **\<Constant >** . Но существует рекурсия между предком **\<emp >** и дочерним **\<EMP >** . Схема указывает аннотацию **SQL: max-depth** для обоих. Поэтому приоритет имеет заметку **SQL: max-depth** , указанную для предка ( **\<EMP >** в роли супервизора).  
...
## <a name="complex-types-derived-by-restriction"></a>Сложные типы, наследуемые по ограничению  
 Если имеется сложный тип, производный от **\<> ограничения**, элементы соответствующего базового сложного типа не могут указывать аннотацию **SQL: max-depth** . В этих случаях в элемент производного типа можно добавить аннотацию **SQL: max-depth** .  
 ...
 Например, следующая схема XSD создает ошибку, так как Аннотация **SQL: max-depth** задана для базового типа. Эта аннотация не поддерживается для типа, производного от **\<ограничений >** из другого типа. Чтобы устранить эту проблему, необходимо изменить схему и указать для элемента в производном типе аннотацию **SQL: max-depth** .  
...
```  
 ...
 В схеме **SQL: max-depth** задается для сложного типа **кустомербасетипе** . В схеме также указывается **\<Customer >** элемент типа **CustomerType**, который является производным от **кустомербасетипе**. Запрос XPath, указанный в такой схеме, выдаст ошибку, поскольку **SQL: max-depth** не поддерживается для элемента, определенного в базовом типе ограничения.  
