...
`[ @rowcount_only = ] type_of_check_requested` указывает тип контрольной суммы или количество строк для выполнения. *type_of_check_requested* имеет значение **smallint**и значение по умолчанию **1**.  
 ...
 Если значение **равно 0**, то необходимо выполнить количество строк и [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 7,0-совместимую контрольную сумму.  
...
`[ @column_list = ] 'column_list'` — это список столбцов, которые должны использоваться в функции CHECKSUM. Аргумент *column_list* имеет тип **nvarchar (4000)** и значение по умолчанию NULL. Включает проверку статей слияния для указания списка столбцов, в который не входят вычисляемые столбцы или столбцы с отметками времени.  
...
## <a name="return-code-values"></a>Значения кода возврата  
 ...
 Если при выполнении проверки количества строк и ожидаемого номера строки равен числу в таблице, **sp_table_validation** возвращает сообщение о том, что таблица прошла проверку количества строк. В противном случае будет выведено сообщение о том, что таблица может быть не синхронизирована, и отчет, показывающий разницу между ожидаемым и реальным числом строк.  
 ...
 **sp_table_validation** используется во всех типах репликации. **sp_table_validation** не поддерживается для издателей Oracle.  
 ...
 Значения с плавающей запятой, скорее всего, будут формировать разницу в контрольной сумме, если используется символьный режим **bcp** . это происходит, если публикация имеет подписчики, отличные от [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Это связано с минимальными и неизбежными различиями в точности при преобразовании в символьный режим и из символьного режима.  
