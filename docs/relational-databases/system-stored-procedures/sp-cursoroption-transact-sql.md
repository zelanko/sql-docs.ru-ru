...
|0x0006|ROWCOUNT|Количество строк, находящихся в результирующем наборе.<br /><br /> Примечание. Если используется асинхронное заполнение, то с момента возвращения значений процедурой sp_cursoropen параметр ROWCOUNT мог измениться. Если число строк неизвестно, возвращается значение -1.|  
