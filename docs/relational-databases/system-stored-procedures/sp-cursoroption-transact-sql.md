...
|0x0006|ROWCOUNT|Количество строк, находящихся в результирующем наборе.<br /><br /> Примечание: ROWCOUNT мог измениться с момента значение, возвращаемое процедурой sp_cursoropen параметр, если используется асинхронное заполнение. Значение -1 возвращается в том случае, если число строк неизвестно.|  
