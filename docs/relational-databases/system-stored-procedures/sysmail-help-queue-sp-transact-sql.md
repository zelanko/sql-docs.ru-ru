...
`[ @queue_type = ] 'queue_type'` Дополнительный аргумент удаляет сообщения электронной почты типа, указанного как *queue_type*. *queue_type* — **nvarchar(6)** не имеет значения по умолчанию. Допустимыми значениями являются **mail** и **состояние**.  
