...
`[ @stmttype = ] 'statement_type'`Указывает инструкцию SQL, которая запускает триггер. *statement_type* имеет тип **varchar (50)** и может использоваться для вставки, обновления, удаления, входа в [!INCLUDE[tsql](../../includes/tsql-md.md)] систему или любого события инструкции, указанного в [DDL-событиях](../../relational-databases/triggers/ddl-events.md). Группы событий задавать нельзя.  
  
 Триггер можно назначить **первым** или **последним** триггером для типа инструкции только после того, как триггер был определен как триггер для этого типа инструкции. Например, триггер **TR1** может быть назначен **первым** для инструкции INSERT в таблице **T1** , если **TR1** определен как триггер INSERT. Функция возвращает ошибку, если **TR1**, который был определен только как триггер INSERT, устанавливается в качестве **первого**или последнего триггера для инструкции UPDATE. [!INCLUDE[ssDE](../../includes/ssde-md.md)] Дополнительные сведения см. в разделе "Примечания".  
...
||  
|-|  
|Сервер применим к: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] с [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]до.|  
...
## <a name="general-trigger-considerations"></a>Общие соглашения о триггерах  
 Если инструкция ALTER TRIGGER изменяет первый или последний триггер, **первый** или **последний** атрибут, изначально заданный для триггера, удаляется, а значение заменяется на **None**. Значение порядка должно быть сброшено с помощью хранимой процедуры **sp_settriggerorder**.  
  
 Если один и тот же триггер должен быть назначен в качестве первого или последнего заказа для более чем одного типа инструкции, то для каждого типа инструкции необходимо выполнить хранимую процедуры **sp_settriggerorder** . Кроме того, триггер необходимо сначала определить для типа инструкции, прежде чем его можно будет назначить **первым** или **последним** триггером для этого типа инструкции.  
