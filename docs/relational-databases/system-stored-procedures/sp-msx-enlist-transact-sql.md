...
`[ @msx_server_name = ] 'msx_server'` Имя сервера (главного) администрирования нескольких серверов. *msx_server* — **sysname**, не имеет значения по умолчанию.  
  
`[ @location = ] 'location'` Расположение целевого сервера для добавления. *расположение* — **nvarchar(100)** , значение по умолчанию NULL.  
