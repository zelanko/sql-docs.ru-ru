...
|**репликация разрешена** параметры|Разрешена репликация транзакций или репликация слиянием.|  
...
|**Публикация**|**bit**|Значение, если для базы данных включена публикация моментальных снимков или публикации транзакций; значение **1** означает, что публикация моментального снимка или публикации транзакций включена.|  
|**мержепублиш**|**bit**|Значение, если для базы данных включена публикация слиянием. значение **1** означает, что публикация слиянием включена.|  
|**dbowner**|**bit**|Значение, если пользователь является членом предопределенной роли базы данных **db_owner** ; значение **1** указывает, что пользователь является членом этой роли.|  
|**дбреадонли**|**bit**|Имеет значение, если база данных помечена как доступная только для чтения; значение **1** означает, что база данных доступна только для чтения.|  
|**хаспубликатионс**|**bit**|Имеет значение, если база данных содержит какие-либо существующие публикации; значение **1** означает наличие существующих публикаций.|  
|**хаспуллсубскриптионс**|**bit**|Имеет значение, если база данных содержит какие-либо существующие подписки по запросу; значение **1** означает, что существуют подписки по запросу.|  
