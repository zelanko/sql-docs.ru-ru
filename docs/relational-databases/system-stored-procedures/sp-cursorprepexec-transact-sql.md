 ...
 *подготовленный обработчик*  
 Созданный идентификатор подготовленного *обработчика.* [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] *подготовленный обработчик* является обязательным и возвращает значение **int**.  
...
> [!NOTE]  
>  Используйте строку **ntext** в качестве входного значения при параметризованной инструкции *stmt* , а значение *scrollopt* PARAMETERIZED_STMT — on.  
...
> [!NOTE]  
>  Правила для указания значения stmt аналогичны правилам для sp_cursoropen, за исключением того, что строковый тип данных *stmt* должен быть **ntext**.  
...
|0x0004|**Оптимистичный параллелизм** (ранее назывался ОПТКК)|  
...
|0x80000|OPTIMISTIC_ACCEPTABLE|  
 ...
 Как ив случае [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] с скроллпт, может присвоить значение, отличное от запрошенного.  
...
|--------------------|---------------------|  
|Если AUTO_FETCH указывается с FAST_FORWARD курсорами *ROWCOUNT* , представляет число строк, помещаемых в буфер выборки.|Представляет число строк в результирующем наборе. Если указано значение AUTO_FETCH *scrollopt* , функция *ROWCOUNT* возвращает количество строк, которые были получены в буфер выборки.|  
