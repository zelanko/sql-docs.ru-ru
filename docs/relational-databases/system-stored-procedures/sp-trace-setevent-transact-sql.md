...
`[ @traceid = ] trace_id` — это идентификатор изменяемой трассировки. *trace_id* имеет **тип int**и не имеет значения по умолчанию. Пользователь использует это *trace_id* значение для выявления, изменения и управления трассировкой.  
...
|19|DTCTransaction|Отслеживает [!INCLUDE[msCoName](../../includes/msconame-md.md)] координатор распределенных транзакций (MS DTC) координированных транзакций между двумя или более базами данных.|  
...
|13|**Продолжительность**|Длительность события (в микросекундах). Этот столбец данных не заполняется событием Hash Warning.|  
 ...
 **[@on=]** *в*  
 ...
 В этой таблице показано взаимодействие между **\@ми** и **\@columnid**.  
...
-   **xp_trace_seteventclassrequired**  
 ...
 Пользователи должны выполнять **sp_trace_setevent** для каждого столбца, добавляемого для каждого события. При каждом выполнении, если **\@on** имеет значение **1**, **sp_trace_setevent** добавляет указанное событие в список событий трассировки. Если **\@on** имеет значение **0**, **sp_trace_setevent** удаляет указанное событие из списка.  
