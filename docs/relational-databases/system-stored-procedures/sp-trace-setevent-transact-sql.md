...
|3|**DatabaseID**|Идентификатор базы данных, заданный инструкцией USE *базы данных* инструкции или базы данных по умолчанию, если инструкция USE *базы данных* выдается для данного соединения.<br /><br /> Значение для базы данных можно определить с помощью функции DB_ID.|  
...
|16|**Reads**|Число логических чтений диска, выполненное сервером для данного события. Этот столбец не заполняется **блокировки: выпущена** событий.|  
...
|26|**ServerName**|Имя экземпляра [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], либо *servername* или *имя_сервера\имя_экземпляра*, которого выполняется трассировка.|  
...
|29|**NestLevel**|Уровень вложенности, на котором выполняется хранимая процедура. См. в разделе [@@NESTLEVEL &#40;Transact-SQL&#41;](../../t-sql/functions/nestlevel-transact-sql.md).|  
