...
|11,9|Не рекомендуется|Не рекомендуется|  
...
|104|Audit AddLogin Event|Происходит при добавлении [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] или удалении имени входа; для **sp_addlogin** и **sp_droplogin**.|  
...
|4|**ИД транзакции**|Назначенный системой идентификатор транзакции.|  
...
|7|**нтдомаиннаме**|Домен Windows, к которому принадлежит пользователь.|  
|8|**Имя узла**|Имя клиентского компьютера, отправившего запрос.|  
...
|12|**ИНТЕРФЕЙС**|Идентификатор процесса сервера, который [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] присвоил процессу, связанному с клиентом.|  
...
|16|**Операции чтения**|Число логических чтений диска, выполненное сервером для данного события. Этот столбец не заполняется событием **Lock: Released** .|  
|17|**Запись**|Число логических обращений к дискам на запись, выполненное сервером для данного события.|  
|18|**ЗАГРУЗКИ**|Объем времени ЦП (в миллисекундах), использованного событием.|  
...
|23|**Успех**|Успех попытки использования разрешений. Применяется для аудита.<br /><br /> **1** = успешное завершение**0** = сбой|  
...
|26|**Имя**|Имя экземпляра [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], *ServerName* или *servername\instancename*, для которого выполняется трассировка.|  
...
|33|**Справиться**|Дескриптор объекта, относящегося к событию.|  
|34|**ИмяОбъекта**|Имя объекта, к которому выполняется доступ.|  
|35|**имя_базы_данных**|Имя базы данных, указанной в инструкции USE *Database* .|  
|36|**Файлов**|Логическое имя изменяемого файла.|  
...
|11,9|**таржетусернаме**|Пользовательское имя цели некоторой операции.|  
|40|**дбусернаме**|Имя пользовательской базы данных [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] для клиента.|  
|41|**логинсид**|Идентификатор безопасности вошедшего в систему пользователя.|  
|42|**таржетлогиннаме**|Имя входа цели некоторой операции.|  
|43|**таржетлогинсид**|Идентификатор SID имени входа, которое является целью некоторой операции.|  
|44|**Разрешений**|Состояние разрешений на уровне столбцов. Используется событием Security Auditing.|  
|45|**линкедсервернаме**|Имя связанного сервера.|  
|46|**ProviderName**|Имя поставщика OLE DB.|  
|47|**Имя_метода**|Имя метода OLE DB.|  
|48|**Количеству строк**|Количество строк в пакете.|  
|49|**RequestID**|Идентификатор запроса, содержащего инструкцию.|  
|50|**ксактсекуенце**|Токен, описывающий текущую транзакцию.|  
|51|**евентсекуенце**|Порядковый номер этого события.|  
...
|54|**УСТРОЙСТВА**|Значение идентификатора GUID, зависящее от класса события, захваченного при трассировке.|  
...
|60|**Указанным параметром IsSystem**|Указывает, произошло событие в системном или в пользовательском процессе.<br /><br /> **1** = система<br /><br /> **0** = пользователь.|  
|61|**Offset**|Начальное смещение инструкции в пределах хранимой процедуры или пакета.|  
|62|**саурцедатабасеид**|Идентификатор базы данных, в которой находится источник объекта.|  
 ...
 **sp_trace_setevent** выполняет многие действия, ранее выполненные расширенными хранимыми процедурами, доступными в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]более ранних версиях служб. Используйте **sp_trace_setevent** вместо следующего:  
