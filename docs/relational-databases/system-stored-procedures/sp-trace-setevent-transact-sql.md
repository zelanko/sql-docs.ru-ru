...
`[ @traceid = ] trace_id` — это идентификатор изменяемой трассировки. *trace_id* имеет **тип int**и не имеет значения по умолчанию. Пользователь использует это значение *trace_id* для обнаружения, изменения и управления трассировкой.  
  
`[ @eventid = ] event_id` — это идентификатор события, которое необходимо включить. *event_id* имеет **тип int**и не имеет значения по умолчанию.  
...
|104|Audit AddLogin Event|Происходит при добавлении или удалении имени входа [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]; для **sp_addlogin** и **sp_droplogin**.|  
...
|110|Audit Add Member to DB Role Event|Происходит при добавлении или удалении имени входа в качестве пользователя базы данных (фиксированного или определяемого пользователем) в базе данных; для **процедур sp_addrolemember**, **sp_droprolemember**и **sp_changegroup**.|  
...
|137|Blocked Process Report|Возникает, если процесс блокирован дольше, чем указанное время. Не включает системные процессы или процессы, ожидающие ресурсы, для которых невозможно определить состояние взаимоблокировки. Используйте **хранимую процедуру sp_configure** , чтобы настроить порог и частоту создания отчетов.|  
...
|181|TM. Начало запуска транзакции|Возникает при запуске запроса BEGIN TRANSACTION.|  
...
|184|TM. Продвижение транзакции выполнено|Возникает при завершении запроса PROMOTE TRANSACTION.|  
|185|TM. Запуск фиксации транзакции|Возникает при запуске запроса COMMIT TRANSACTION.|  
|186|TM. Фиксация транзакции завершена|Возникает при завершении запроса COMMIT TRANSACTION.|  
...
|190|Progress Report: Операция с индексами в сети|Сообщает о ходе выполнения операции построения в сети индекса при работающем процессе построения.|  
|191|TM. Сохранение запуска транзакции|Возникает при запуске запроса SAVE TRANSACTION.|  
|192|TM. Сохранение транзакции завершено|Возникает при завершении запроса SAVE TRANSACTION.|  
...
`[ @columnid = ] column_id` — это идентификатор столбца, добавляемого для события. *column_id* имеет **тип int**и не имеет значения по умолчанию.  
...
|29|**NestLevel**|Уровень вложенности, на котором выполняется хранимая процедура. См. раздел [@ @no__t &#40;-1 Transact&#41;-SQL](../../t-sql/functions/nestlevel-transact-sql.md).|  
 ...
 **[@on =]** *в*  
 ...
 Если *для параметра on* задано значение **1**, а *column_id* имеет значение null, то для события задается значение ON, а все столбцы очищаются. Если *column_id* не равно null, то для этого события в столбце устанавливается значение ON.  
  
 Если параметр *On* имеет значение **0**, а *column_id* имеет значение null, то событие отключено и все столбцы очищаются. Если *column_id* не равно null, то столбец отключен.  
 ...
 **sp_trace_setevent** выполняет многие действия, ранее выполненные расширенными хранимыми процедурами, доступными в более ранних версиях [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Используйте **sp_trace_setevent** вместо следующего:  
...
-   **xp_trace_seteventclassrequired**  
 ...
 Пользователи должны выполнить **sp_trace_setevent** для каждого столбца, добавленного для каждого события. При каждом выполнении, если параметру **\@on** присвоено значение **1**, **sp_trace_setevent** добавляет указанное событие в список событий трассировки. Если параметру **\@on** присвоено значение **0**, **sp_trace_setevent** удаляет указанное событие из списка.  
