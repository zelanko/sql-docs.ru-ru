...
`[ @repeat_interval = ] 'repeat_interval_in_seconds'` указывает интервал времени, в течение которого хранимая процедура будет многократно выполняться для отправки сведений о работоспособности.  
...
|**component_type**|**sysname**|Указывает, содержит ли строка сведения для компонента уровня экземпляра [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] или для Always On группы доступности:<br /><br /> instance<br /><br /> Always On: AvailabilityGroup|  
|**component_name**|**sysname**|Указывает имя компонента или имя группы доступности:<br /><br /> система<br /><br /> ресурс<br /><br /> query_processing<br /><br /> io_subsystem<br /><br /> события<br /><br /> *\<имя группы доступности >*|  
