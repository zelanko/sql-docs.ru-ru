...
-   **query_processing**: собирает данные с точки зрения обработки запросов в рабочих потоках, задачах, подождите типы, сеансах, интенсивно нагружающих ЦП и блокирующих задачах. Эти сведения представляют рекомендации по состоянию работоспособности.  
...
-   **события**: сбор данных и поверхности через хранимую процедуру, об ошибках и важных событиях, записанных сервером, включая сведения об исключениях кольцевого события буфера о брокере памяти, не хватает памяти, монитора планировщиков буферный пул, спин-блокировках, безопасности и подключения. В качестве состояния событий всегда указывается 0.  
