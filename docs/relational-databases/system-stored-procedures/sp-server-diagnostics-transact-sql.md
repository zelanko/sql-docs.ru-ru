---
title: sp_server_diagnostics (Transact-SQL) | Документы Microsoft
ms.custom: ''
ms.date: 11/14/2017
ms.prod: sql
...
-   **система**: собирает данные с точки зрения системы о спин-блокировках, серьезных затруднениях в обработке, невыполненных задач, ошибок страниц физической памяти и ЦП. Эти сведения представляют общие рекомендации по состоянию работоспособности.  
  
-   **ресурс**: собирает данные с точки зрения ресурсов о физической и виртуальной памяти, буферных пулах, страницах, кэше и другие объекты памяти. Эти сведения представляют рекомендации по состоянию работоспособности.  
  
-   **сообщение**: сбор данных с точки зрения обработки запросов в рабочих потоках, задачах, ожидания типов, сеансах, интенсивно нагружающих ЦП и блокирующих задачах. Эти сведения представляют рекомендации по состоянию работоспособности.  
  
-   **io_subsystem**: собирает данные на операции ввода-ВЫВОДА. Помимо диагностических данных, этот компонент передает состояние удовлетворительной работоспособности или предупреждение работоспособности только для подсистемы ввода-вывода.  
  
-   **события**: сбор данных и предоставляет доступ через хранимую процедуру на ошибки и события с сервера, включая сведения об исключениях, кольца события буфера о брокере памяти, не хватает памяти, монитора планировщиков буферный пул, спин-блокировки, безопасности и подключения. В качестве состояния событий всегда указывается 0.  
