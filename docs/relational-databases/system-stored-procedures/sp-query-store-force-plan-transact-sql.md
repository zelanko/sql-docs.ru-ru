...
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]
 ...
 Если план принудительно используется для конкретного запроса, каждый раз, когда [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] встречает запрос, он пытается принудительно выполнить план в оптимизаторе запросов. Если форсирование плана завершается неудачей, запускается расширенное событие, и оптимизатор запросов подает запрос на оптимизацию обычным способом.  
...
`[ @plan_id = ] plan_id` — это идентификатор плана запроса, который необходимо принудительно применить. *plan_id* имеет тип **bigint**и не имеет значения по умолчанию.  
