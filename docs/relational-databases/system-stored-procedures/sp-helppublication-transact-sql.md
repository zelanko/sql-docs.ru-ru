...
|compress_snapshot|**bit**|Указывает, что моментальный снимок, записываемый в позицию *alt_snapshot_folder* , должен быть сжат в формате [!INCLUDE[msCoName](../../includes/msconame-md.md)] CAB. значение **0** указывает, что моментальный снимок не будет сжиматься.|  
