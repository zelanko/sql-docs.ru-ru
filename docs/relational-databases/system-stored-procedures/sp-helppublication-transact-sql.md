...
|compress_snapshot|**бит**|Указывает, что моментальный снимок, записываемый в *alt_snapshot_folder* расположение, должен быть сжат в формате CAB-файла [!INCLUDE[msCoName](../../includes/msconame-md.md)]. значение **0** указывает, что моментальный снимок не будет сжиматься.|  
