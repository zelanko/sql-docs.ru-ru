...
### <a name="c-using-the-output-parameter"></a>В. Использование параметра OUTPUT  
 В следующем примере используется `OUTPUT` параметр для хранения результирующего набора, созданного `SELECT` инструкцией в `@SQLString` параметре. Затем `SELECT` выполняются две инструкции, использующие значение `OUTPUT` параметра.  
