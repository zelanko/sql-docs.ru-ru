...
[!INCLUDE[tsql-appliesto-ss2012-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2012-asdb-xxxx-xxx-md.md)]
  ...
  Возвращает результирующий набор, содержащий метаданные о необъявленных параметрах в [!INCLUDE[tsql](../../includes/tsql-md.md)]ном пакете. Учитывает каждый параметр, который используется в **\@пакете tsql** , но не объявлен в **параметрах\@** . Возвращается результирующий набор, содержащий одну строку для каждого такого параметра со сведениями о предполагаемом типе параметра. Процедура возвращает пустой результирующий набор, если в пакете ввода **\@TSQL** отсутствуют параметры, кроме тех, которые объявлены в **параметре\@params**.  
...
-   Если входной \@tsql не является допустимым пакетом [!INCLUDE[tsql](../../includes/tsql-md.md)]. Срок действия определяется путем синтаксического анализа и анализа [!INCLUDE[tsql](../../includes/tsql-md.md)] пакета. Ошибки, вызванные пакетом во время оптимизации запроса или во время выполнения, не учитываются при определении допустимости [!INCLUDE[tsql](../../includes/tsql-md.md)] пакета.  
...
- В запрос включено создание постоянной таблицы, к которой он будет обращен.
 ...
 Если \@tsql не содержит параметров, кроме тех, которые объявлены в \@params, процедура возвращает пустой результирующий набор.  
...
-   Простое определение  
    ...
    ```  
     ...
     В качестве особого случая, если \@p является аргументом для оператора \<, >, \<= или > =, простые правила удержания не применяются. Алгоритм определения типов будет использовать общие правила определения, описанные в следующем разделе. Например, если столбец c1 имеет тип данных char(30), рассмотрим следующие два запроса:  
    ...
    ```  
     ...
     В первом случае алгоритм выведения типа выводит тип **char (30)** в качестве типа данных для \@p согласно правилам, приведенным ранее в этом разделе. Во втором случае алгоритм выведения типа выводит **varchar (8000)** в соответствии с общими правилами удержания в следующем разделе.  
...
1.  Выбирается тип данных, который создает наименьшее число неявных преобразований в E (\@p). Если определенный тип данных создает тип данных для E (\@p), отличный от TT (\@p), алгоритм выведения типа считает это дополнительным неявным преобразованием из типа данных E (\@p) в TT (\@p).  
