...
## <a name="arguments"></a>Аргументы  
 ...
 [  **@params =** ] **N "***параметры***"**  
 @params Определяет строку объявления параметров для [!INCLUDE[tsql](../../includes/tsql-md.md)] работы пакетной службы, точно так же в хранимой процедуре sp_executesql. *Параметры* может быть **nvarchar (***n***)** или **nvarchar(max)**.  
...
-   Если @params не равно NULL и содержит строку, не является синтаксически правильной строкой объявления для параметров, или если он содержит строку, объявляющий любого параметра более одного раза.  
...
-   тип данных, к которому приводится или преобразуется инструкция.  
 ...
 Если @p не содержится в одном выражении, указанном в начале шага 2, алгоритм определения типов определяет, E (@p) является самым большим скалярным выражением, которое содержит @p, и не поддерживает алгоритм определения типов вычислений целевого типа данных TT (@p) для E (@p). Например, если запрос SELECT `@p + 2` затем E (@p) = @p + 2, а не TT (@p).  
...
-   Простое определение  
     ...
     Если E (@p) = @p и TT (@p) существует, т. е. Если @p является непосредственным аргумента одному из выражений, указанных в начале шага 2, алгоритм определения типов выводит тип данных @p быть TT (@p). Например:  
    ...
    ```  
     ...
     В первом случае алгоритм определения типов определяет **char(30)** как тип данных для @p согласно правилам ранее в этом разделе. Во втором случае алгоритм определения типов определяет **varchar(8000)** в соответствии с правилами общее определение в следующем разделе.  
