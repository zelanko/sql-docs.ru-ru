...
|--------------|-----------|-----------------|  
|**allow_anonymous**|**условия**|Анонимные подписки разрешены.|  
||**IsFalse**|Анонимные подписки запрещены.|  
|**allow_partition_realignment**|**условия**|Операции удаления отправляются на подписчик для отражения результатов изменений секции путем удаления данных, которые больше не являются частью секции подписчика. Такая реакция на события используется по умолчанию.|  
||**IsFalse**|Данные из старой секции остаются на подписчике, а изменения данных, произведенные на издателе, на этот подписчик не реплицируются. Вместо этого изменения в подписчике реплицируются на издатель. Такой метод применяется для сохранения в подписке данных из старой секции, чтобы обеспечить доступ к архивным данным.|  
|**allow_pull**|**условия**|Подписки по запросу на данную публикацию разрешены.|  
||**IsFalse**|Подписки по запросу на данную публикацию не разрешены.|  
|**allow_push**|**условия**|Принудительные подписки на данную публикацию разрешены.|  
||**IsFalse**|Принудительные подписки на данную публикацию не разрешены.|  
|**allow_subscriber_initiated_snapshot**|**условия**|Подписчик может инициировать процесс получения моментального снимка.|  
||**IsFalse**|Подписчик не может инициировать процесс получения моментального снимка.|  
|**allow_subscription_copy**|**условия**|Можно копировать базы данных, подписанные на эту публикацию.|  
||**IsFalse**|Нельзя копировать базы данных, подписанные на эту публикацию.|  
|**allow_synctoalternate**|**условия**|Разрешает альтернативному участнику синхронизации выполнять синхронизацию с этим издателем.|  
||**IsFalse**|Не разрешает альтернативному участнику синхронизации выполнять синхронизацию с этим издателем.|  
|**allow_web_synchronization**|**условия**|Подписки можно синхронизировать через HTTPS.|  
||**IsFalse**|Подписки нельзя синхронизировать через HTTPS.|  
...
|**centralized_conflicts**|**условия**|Все конфликтующие записи хранятся на издателе. После изменения этого свойства необходимо выполнить повторную инициализацию существующих подписчиков.|  
||**IsFalse**|Конфликтующие записи хранятся на сервере, который был выбран в качестве проигравшего при устранении конфликта. После изменения этого свойства необходимо выполнить повторную инициализацию существующих подписчиков.|  
|**compress_snapshot**|**условия**|Моментальный снимок в альтернативной папке снимков сжимается в формате CAB. Сжатие моментального снимка в папке моментальных снимков по умолчанию невозможно. Изменение этого свойства потребует создания нового моментального снимка.|  
||**IsFalse**|По умолчанию моментальный снимок не сжимается. Изменение этого свойства потребует создания нового моментального снимка.|  
...
|**dynamic_filters**|**условия**|Публикация фильтруется на основе динамического предложения.|  
||**IsFalse**|Публикация не фильтруется динамически.|  
|**enabled_for_internet**|**условия**|Публикация через Интернет разрешена. Для передачи файлов моментальных снимков на подписчик можно использовать протокол FTP. Файлы синхронизации для публикации размещаются в каталоге «C:\Program Files\Microsoft SQL Server\MSSQL\Repldata\ftp».|  
||**IsFalse**|Публикация через Интернет не разрешена.|  
...
|**keep_partition_changes**|**условия**|Синхронизация оптимизирована и влияет только на подписчиков, чьи строки принадлежат измененным секциям. Изменение этого свойства потребует создания нового моментального снимка.|  
||**IsFalse**|Синхронизация не оптимизирована, и при изменении данных в секции проверяются секции, переданные на подписчики. Изменение этого свойства потребует создания нового моментального снимка.|  
...
|**publish_to_activedirectory**|**условия**|Этот аргумент устарел и поддерживается только для обратной совместимости скриптов. Сведения о публикации больше не могут быть добавлены в Active Directory.|  
||**IsFalse**|Удаляет сведения о публикации из службы Active Directory.|  
...
|**snapshot_in_defaultfolder**|**условия**|Файлы моментального снимка сохранены в папке для моментальных снимков по умолчанию.|  
||**IsFalse**|Файлы моментальных снимков хранятся в альтернативном расположении, заданном *alt_snapshot_folder*. Такое сочетание позволяет размещать файлы моментальных снимков одновременно и в каталоге по умолчанию, и в альтернативном каталоге.|  
|**snapshot_ready**|**условия**|Моментальный снимок доступен для публикации.|  
||**IsFalse**|Моментальный снимок для публикации не доступен.|  
|**состояние**|**Active**|Публикация находится в активном состоянии.|  
