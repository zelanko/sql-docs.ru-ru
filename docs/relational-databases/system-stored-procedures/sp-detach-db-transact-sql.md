...
> [!IMPORTANT]
>  Параметр будет удален в следующей [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]версии. **@keepfulltextindexfile** Не используйте его при работе над новыми приложениями и как можно быстрее измените приложения, в которых он в настоящее время используется.  
...
## <a name="obtaining-exclusive-access"></a>Получение монопольного доступа  
 ...
 Например, следующая `ALTER DATABASE` инструкция получает монопольный доступ [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] к базе данных после отключения всех текущих пользователей от базы данных.  
