...
`[ @server_name = ] 'server_name'`Имя сервера, к которому применяется транзакция. *originating_server* имеет тип **sysname**и не имеет значения по умолчанию.  
  
`[ @database_name = ] 'database_name'`Имя базы данных, к которой применяется транзакция. *originating_db* имеет тип **sysname**и не имеет значения по умолчанию.  
