...
> [!NOTE]  
>  Если указанное *значение* слишком велико для параметра, то в столбце **run_value** будет отражен тот [!INCLUDE[ssDE](../../includes/ssde-md.md)] факт, что по умолчанию используется динамическая память, а не недопустимый параметр.  
