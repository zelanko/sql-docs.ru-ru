...
|**allow_initialize_from_backup**|**true**|Подписчики могут инициализировать подписку на эту публикацию из резервной копии, а не из исходного моментального снимка. Это свойство нельзя изменить для не -[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] публикаций.|  
...
|**Значение NULL** (по умолчанию)||Возвращает список поддерживаемых значений для *свойства*.|  
...
Для получения списка объектов публикации в Active Directory с помощью **publish_to_active_directory** параметр [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] объект уже должен быть создан в Active Directory.  
  
## <a name="impact-of-immediate-sync"></a>Влияние свойства «immediate sync»  
 При интерпретации синхронизации все изменения в журнале отслеживаются сразу после исходного моментального снимка, даже если нет подписок. Если клиент резервного копирования, чтобы добавить новый одноранговый узел используются зарегистрированных изменений. После восстановления резервной копии одноранговый узел является синхронизированы с любыми изменениями, выполняемых после создания резервной копии. Поскольку команды отслеживаются в базе данных распространителя, логику синхронизации можно рассмотрим последнего архивированного номера LSN и использовать ее в качестве отправной точки, зная, что команда доступна, если в течение максимального срока хранения резервной копии. (Значение по умолчанию для минимального срока хранения составляет 0 часов, а максимальный срок хранения — 24 ч.)  
