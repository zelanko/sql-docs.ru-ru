...
|**GRANTOR**|**имеет sysname**|Имя пользователя базы данных, которому предоставлены разрешения на эту **table_name** в указанный **участник**. В [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]этот столбец всегда совпадает с **table_owner**. Это поле всегда возвращает значение. Кроме того, столбец GRANT может быть либо владельцем базы данных (**table_owner**), либо пользователем, которому владелец базы данных предоставил разрешение с помощью предложения WITH GRANT OPTION в инструкции GRANT.|  
|**GRANTEE**|**имеет sysname**|Имя пользователя базы данных, которому были предоставлены разрешения на этот **table_name** указанным объектом **Grant**. Это поле всегда возвращает значение.|  
