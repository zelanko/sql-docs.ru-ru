...
|--------------|------------|-----------------|  
|**allow_interactive_resolver**|**условия**|Разрешает использование для статьи интерактивного арбитра конфликтов.|  
||**IsFalse**|Запрещает использование для статьи интерактивного сопоставителя.|  
...
|**column_tracking**|**условия**|Включает трассировку уровня столбца. Применяется только к статье таблицы.<br /><br /> Примечание. Отслеживание на уровне столбцов не может использоваться при публикации таблиц с более чем 246 столбцами.|  
||**IsFalse**|Выключает трассировку уровня столбца и оставляет обнаружение конфликтов уровня строки. Применяется только к статье таблицы.|  
|**compensate_for_errors**|**условия**|Выполняется компенсация при возникновении ошибок синхронизации. Дополнительные сведения см. в разделе [sp_addmergearticle](../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md).|  
||**IsFalse**|Компенсация не выполняется. Это значение по умолчанию. Дополнительные сведения см. в разделе [sp_addmergearticle](../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md).<br /><br /> ** \* \* Важно \* !** Несмотря на то, что данные в затронутых строках могут оказаться несогласованными, как только вы устраните ошибки, можно применить изменения, и данные будут согласованы. Если исходная таблица для статьи уже опубликована в другой публикации, то значение *compensate_for_errors* должно быть одинаковым для обеих статей.|  
|**creation_script**||Путь и имя необязательного скрипта схем статей, используемого для создания статьи в базе данных подписки.|  
|**delete_tracking**|**условия**|Выполняется репликация инструкций DELETE, что является значением по умолчанию.|  
||**IsFalse**|Репликация инструкций DELETE не выполняется.<br /><br /> ** \* \* Важно \* !** Если присвоить параметру **delete_tracking** **значение false** , результаты не конвергенции будут удалены, а удаленные строки необходимо будет удалить вручную.|  
...
|**logical_record_level_conflict_detection**|**условия**|Обнаружение конфликтов производится при внесении изменений в любое место логической записи. Требует, чтобы **logical_record_level_conflict_resolution** было установлено **значение true**.|  
||**IsFalse**|Обнаружение конфликтов по умолчанию используется, как указано в **column_tracking**.|  
|**logical_record_level_conflict_resolution**|**условия**|Выигравшая логическая запись целиком перезаписывает проигравшую.|  
||**IsFalse**|Победившие строки не ограничиваются пределами логической записи.|  
...
|**published_in_tran_pub**|**условия**|Статья также опубликована в публикации транзакций.|  
||**IsFalse**|Статья не опубликована в публикации транзакций.|  
...
|**состояние**|**Active**|Скрипт начальной обработки для публикации таблицы запущен.|  
