---
title: sp_changemergearticle (Transact-SQL) | Документы Microsoft
ms.custom: ''
ms.date: 11/09/2015
ms.prod: sql
...
||**false**|Репликация инструкций DELETE не выполняется.<br /><br /> **\*\* Важные \* \***  параметр **delete_tracking** для **false** приведет к отсутствию конвергенции или удаленные строки необходимо вручную удалить.|  
|**Описание**||Описание статьи.|  
|**destination_owner**||Имя владельца объекта в базе данных подписки, если не **dbo**.|  
|**Аргумент identity_range**||**bigint** , указывающее размер диапазона при назначении новых значений идентификаторов, если аргументу статьи **identityrangemanagementoption** значение **автоматически** или **auto_identity_ диапазон** значение **true**. Применяется только к статье таблицы. Дополнительные сведения см. раздел «Репликация слиянием» [репликации столбцов идентификаторов](../../relational-databases/replication/publish/replicate-identity-columns.md).|  
...
|**logical_record_level_conflict_detection**|**true**|Обнаружение конфликтов производится при внесении изменений в любое место логической записи. Требуется **logical_record_level_conflict_resolution** присвоить **true**.|  
...
|**processing_order**||**int** указывает порядок обработки статей в публикации слиянием.|  
|**pub_identity_range**||**bigint** , указывающее размер диапазона, выделяемого подписчику с серверной подпиской, если аргументу статьи **identityrangemanagementoption** значение **автоматически** или **auto_ Аргумент identity_range** значение **true**. Этот диапазон идентификаторов резервируется переиздающему подписчику для выделения собственным подписчикам. Применяется только к статье таблицы. Дополнительные сведения см. раздел «Репликация слиянием» [репликации столбцов идентификаторов](../../relational-databases/replication/publish/replicate-identity-columns.md).|  
...
||**0x00**|Запрещает выполнение сценариев агентом моментальных снимков и использует сценарий, приведенный в **creation_script**.|  
...
|**status**|**Активный**|Скрипт начальной обработки для публикации таблицы запущен.|  
...
|**stream_blob_columns**|**true**|Используется оптимизация потока данных при репликации столбцов больших двоичных объектов. Однако при этом некоторые функции репликации слиянием, например логические записи, могут помешать использованию оптимизации потока. *stream_blob_columns* имеет значение true, если включена поддержка FILESTREAM. Это позволяет оптимально выполнять репликацию данных FILESTREAM и уменьшить использование памяти. Чтобы принудительно заставить статьи таблицы FILESTREAM не использовать потоковую передачу объектов, установите *stream_blob_columns* значение false.<br /><br /> **\*\* Важные \* \***  Включение данного режима оптимизации памяти может отрицательно повлиять на производительность агента слияния во время синхронизации. Этот параметр должен использоваться только при репликации столбцов, содержащих мегабайты данных.|  
 ...
 **1** означает, что изменения в статье слияния вызывают повторную инициализацию подписок и дает разрешение произвести повторную инициализацию подписки.  
 ...
 **sp_changemergearticle** используется в репликации слиянием.  
 ...
 Поскольку **sp_changemergearticle** используется для изменения свойств статьи, которые изначально определяются с помощью [sp_addmergearticle](../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md), посвящены [sp_addmergearticle](../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md) Дополнительные сведения об этих свойствах.  
  
 Изменение следующих свойств требует создания нового моментального снимка, и необходимо указать значение **1** для *подписки потребуют* параметр:  
...
-   **logical_record_level_conflict_resolution**  
 ...
 При задании **column_tracking** свойства, если таблица уже опубликована в других публикациях слиянием, отслеживание столбцов должно совпадать с значение используется для существующих статей, основанных на данной таблице. Этот параметр применим только для статей таблиц.  
  
 Если в нескольких публикациях содержатся статьи, на основе одной базовой таблицы, изменение **delete_tracking** свойство или **compensate_for_errors** свойства для одной статьи приводит к тому же изменение в сделать в других статьях, основанных на той же таблице.  
 ...
 При изменении значения **schema_option**, система не выполняет побитового обновления. Это означает, что при установке **schema_option** с помощью **sp_changemergearticle**существующие битовые настройки могут быть отключены. Чтобы сохранить существующие параметры, необходимо выполнить [& (побитовое и)](../../t-sql/language-elements/bitwise-and-transact-sql.md) между значением, которое устанавливается и текущее значение *schema_option*, которое можно получить, выполнив [sp_helpmergearticle](../../relational-databases/system-stored-procedures/sp-helpmergearticle-transact-sql.md).  
  
> [!CAUTION]  
>  При вы много (достичь нескольких сотен) статей в публикации и выполнение **sp_changemergearticle** для одной из статей, он может занять много времени на завершение выполнения.  
...
## <a name="permissions"></a>Разрешения  
 Только члены **sysadmin** предопределенной роли сервера или **db_owner** предопределенной роли базы данных могут выполнять **sp_changemergearticle**.  
