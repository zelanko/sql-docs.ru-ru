...
`[ @source_schema = ] 'source_schema'` — Это имя схемы, которой принадлежит исходная таблица. *source_schema* — **sysname**, не имеет значения по умолчанию и не может иметь значение NULL.  
  
`[ @source_name = ] 'source_name'` — Имя исходной таблицы, для которого необходимо включить сбор данных изменений. *source_name* — **sysname**, не имеет значения по умолчанию и не может иметь значение NULL.  
...
`[ @role_name = ] 'role_name'` — Имя роли базы данных, использовалась для доступа к данным изменений. *имя_роли* — **sysname** и должен быть указан. Если явно задано значение NULL, то шлюзовая роль не используется для ограничения доступа к информации об изменениях.  
...
`[ @capture_instance = ] 'capture_instance'` — Имя экземпляра системы отслеживания, используемое для информации об изменениях с определенным экземпляром имя объектов отслеживания. *capture_instance* — **sysname** и не может иметь значение NULL.  
...
`[ @supports_net_changes = ] supports_net_changes` Указывает, следует ли поддержку запросов суммарных изменений для данного экземпляра системы отслеживания. *supports_net_changes* — **бит** значение по умолчанию 1, если таблица имеет первичный ключ или таблица имеет уникальный индекс, который был определен с помощью @index_name параметра. В противном случае данный параметр по умолчанию принимает значение 0.  
...
`[ @captured_column_list = ] 'captured_column_list'` Определяет столбцы исходной таблицы, которые должны быть включены в таблице изменений. *captured_column_list* — **nvarchar(max)** и может иметь значение NULL. Если аргумент имеет значение NULL, то в таблицу изменений включаются все столбцы.  
...
`[ @filegroup_name = ] 'filegroup_name'` — Это файловая группа, используемая для таблицы изменений, созданной для экземпляра отслеживания. *filegroup_name* — **sysname** и может иметь значение NULL. Если указано, *filegroup_name* должен быть определен в текущей базе данных. Если значение равно NULL, то используется файловая группа, заданная по умолчанию.  
  
 Для хранения системы отслеживания информации об изменениях рекомендуется создать отдельную файловую группу.  
  
`[ @allow_partition_switch = ] 'allow_partition_switch'` Указывает, возможно ли выполнение команды SWITCH PARTITION инструкции ALTER TABLE для таблицы, включенной для отслеживания измененных данных. *allow_partition_switch* — **бит**, значение по умолчанию 1.  
