...
## <a name="arguments"></a>Аргументы  
 ...
 [  **@message_id =** ] *message_id*  
 Номер сообщения об ошибке, которое определяет предупреждение. (Обычно соответствует номеру ошибки в **sysmessages** таблицы.) *message_id* — **int**, значение по умолчанию **0**. Если *серьезность* используется для определения предупреждения, *message_id* должно быть **0** или значение NULL.  
