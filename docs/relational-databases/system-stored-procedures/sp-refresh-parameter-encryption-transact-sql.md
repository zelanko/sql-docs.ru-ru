...
Обновляет метаданные постоянного шифрования для параметров указанной не привязанной к схеме хранимой процедуры, определяемой пользователем функции, представления, DML триггера, триггер DDL уровня базы данных, или триггер DDL уровня сервера в текущей базе данных. 
...
[  **@namespace =** ] **"** < класс > **"**   
Класс указанного модуля. Когда *имя_модуля* является триггером DDL `<class>` является обязательным. Параметр `<class>` равен `nvarchar(20)`. Допустимыми входными значениями являются `DATABASE_DDL_TRIGGER` и `SERVER_DDL_TRIGGER`.    
...
* Модуль ссылается на другой модуль с метаданные устаревших параметров шифрования.  
