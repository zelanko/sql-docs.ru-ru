...
## <a name="arguments"></a>Аргументы  
 ...
 [ @article=] '*статьи*"  
 Статья, на которую подписана публикация. *статья* — **sysname**, по умолчанию all. Если значение равно ALL, то подписка добавляется ко всем статьям в данной публикации. Издателями Oracle поддерживаются только значения ALL и NULL.  
...
|Значение|Описание|  
|-----------|-----------------|  
|none|Подписчик уже имеет схему и начальные данные для опубликованных таблиц.<br /><br /> Примечание: Этот параметр рекомендуется к использованию. Вместо этого используйте значение «replication support only».|  
...
-   Попытка установить *update_mode* значение sync tran.  
