---
title: "sp_article_validation (Transact-SQL) | Документы Microsoft"
ms.custom: 
ms.date: 03/16/2017
...
## <a name="arguments"></a>Аргументы  
 ...
 Если **0**, выполнить вычисление количества строк и [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] контрольной суммы, совместимой версии 7.0.  
...
|**1**|Выполняет быстрый подсчет из **sysindexes.rows**. Подсчет строк в **sysindexes** быстрее, чем подсчет строк в фактической таблице. Тем не менее **sysindexes** обновляется, и количество строк может оказаться неточным.|  
|**2** (по умолчанию)|Выполняет условный быстрый подсчет, при котором сначала применяется быстрый метод, Если быстрый метод дает неточные результаты, переключается на полный подсчет. Если *expected_rowcount* имеет значение NULL и хранимая процедура используется для получения значения полный COUNT(*) всегда используется.|  
 ...
 [  **@subscription_level=**] *процедурой*  
 Указывает, будет ли проверка применяться к части подписчиков. *процедурой* — **бит**, значение по умолчанию **0**. Если **0**, проверка применяется для всех подписчиков. Если **1**, проверка применяется только к части подписчиков, определяемой вызовами **sp_marksubscriptionvalidation** в текущую открытую транзакцию.  
 ...
 **sp_article_validation** вызывает проверки сбор сведений о указанной статьи и отправляет запрос на проверку в журнал транзакций. Когда агент распространителя получает этот запрос, он сравнивает сведения для проверки, указанные в запросе, с таблицей подписчика. Результаты проверки отображаются в мониторе репликации и в виде предупреждений агента [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
  
## <a name="permissions"></a>Permissions  
 Только пользователи, имеющие разрешение SELECT ALL на исходную таблицу для проверяемой статьи, можно выполнить **sp_article_validation**.  
