...
2.  Выполните слияние с помощью агента слияния. Мы рекомендуем использовать **— Validate** параметр командной строки агента на каждом подписчике при запуске агента слияния. Если вы используете непрерывном режиме слияния, см. в разделе *особые рекомендации о непрерывном режиме слияния* далее в этом разделе.  
