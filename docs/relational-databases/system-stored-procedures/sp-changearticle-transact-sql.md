...
`[ @publication = ] 'publication'` — Имя публикации, которая содержит статью. *Публикация* — **sysname**, значение по умолчанию NULL.  
  
`[ @article = ] 'article'` — Имя статьи, свойство которого должен быть изменен. *статья* — **sysname**, значение по умолчанию NULL.  
  
`[ @property = ] 'property'` Это свойство статьи для изменения. *Свойство* — **nvarchar(100)** .  
  
`[ @value = ] 'value'` — Это новое значение свойства статьи. *значение* — **nvarchar(255)** .  
...
`[ @force_reinit_subscription = ]force_reinit_subscription_` Подтверждает, что действие, выполняемое данной хранимой процедурой, может сделать необходимой повторную инициализацию текущих подписок. *Этот* — **бит** значение по умолчанию **0**.  
