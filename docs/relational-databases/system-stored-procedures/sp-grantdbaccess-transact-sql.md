...
## <a name="examples"></a>Примеры  
 В следующем примере используется `CREATE USER` для добавления пользователя базы данных для имени входа `Edmonds\LolanSo` Windows в текущую базу данных. Новому пользователю присваивается имя `Lolan`. Этот метод является предпочтительным при создании пользователя базы данных.  
