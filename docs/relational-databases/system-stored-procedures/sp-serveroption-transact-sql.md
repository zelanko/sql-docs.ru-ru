...
`[ @server = ] 'server'` — Имя сервера, для которого требуется задать параметр. Аргумент*server* имеет тип **sysname**и не имеет значения по умолчанию.  
  
`[ @optname = ] 'option_name'` — Это возможность задать для указанного сервера. *option_name* — **varchar (** 35 **)**, не имеет значения по умолчанию. *option_name* может быть любым из следующих значений.  
...
`[ @optvalue = ] 'option_value'` Указывает ли *option_name* должен быть включен (**TRUE** или **на**) или отключена (**FALSE** или **off**). *option_value* — **varchar (** 10 **)**, не имеет значения по умолчанию.  
