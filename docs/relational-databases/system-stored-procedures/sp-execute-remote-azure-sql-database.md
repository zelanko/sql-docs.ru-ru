...
## <a name="arguments"></a>Аргументы  
 [ \@data_source_name =] *источник источника*  
...
Создайте учетные данные уровня базы данных в пользовательской базе данных, указав учетные данные администратора для базы данных master. Создайте внешний источник данных, указывающий на базу данных master, и укажите учетные данные области базы данных. Затем, например, в пользовательской базе данных выполняется `sp_set_firewall_rule` процедура в базе данных master. `sp_set_firewall_rule` Процедура требует 3 параметра и требует, чтобы `@name` параметр был в Юникоде.
