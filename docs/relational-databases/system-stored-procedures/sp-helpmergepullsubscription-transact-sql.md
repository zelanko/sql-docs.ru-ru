...
|**use_ftp**|**бит**|Подписка на публикацию осуществляется через Интернет, также настраиваются свойства адреса FTP. Если **0**, подписка не использует протокол FTP. Если **1**, подписка использует протокол FTP.|  
|**offload_agent**|**бит**|Указывает, может ли агент быть активирован и запущен удаленно. Если **0**, агент не может быть активирован удаленно.|  
