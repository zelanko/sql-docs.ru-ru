...
`[ @publication = ] 'publication'` — Имя публикации, которая содержит эту статью. *Публикация* — **sysname**, не имеет значения по умолчанию.  
...
`[ @change_active = ] change_active` Разрешает изменение столбцов в публикациях, на которые имеются подписки. *change_active* — **int** значение по умолчанию **0**. Если **0**, столбцы не изменяются. Если **1**, столбцы можно добавить или удалить из активных статей, которые имеются подписки.  
