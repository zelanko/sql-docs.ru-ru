...
|0x10|ABSOLUTE|Извлекает буфер *nrows* строк, начиная с *rownum* строки.<br /><br /> Примечание: При использовании значения ABSOLUTE для курсора DYNAMIC или FORWARD_ONLY курсора возвращает сообщение об ошибке, поскольку FORWARD_ONLY поддерживает прокрутку только в одном направлении.|  
...
|Для ABSOLUTE|-1 возвращает последнюю строку в наборе.<br /><br /> -2 возвращает вторую от конца строку в наборе и так далее.<br /><br /> Примечание: Если для выборки в этом случае запрашивается более одной строки, две последние строки результирующего набора возвращаются.|  
