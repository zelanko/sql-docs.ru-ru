...
`[ @article = ] 'article'`Имя статьи. Аргумент *article* имеет **%** тип **sysname**и значение по умолчанию, которое возвращает все сведения о подписке для выбранных публикаций и подписчиков. Вэтом случае для полной подписки на публикацию возвращается только одна запись.  
...
|**Тип синхронизации**|**tinyint**|Тип синхронизации подписки:<br /><br /> **1** = автоматический<br /><br /> **2** = нет|  
...
|**ИД задания распространения**|**двоичный (16)**|Идентификатор задания агента распространителя.|  
...
|**offload_server**|**sysname**|Имя сервера, используемого для удаленной активации агента. Если значение равно NULL, то используется текущий offload_server, указанный в таблице [MSdistribution_agents](../../relational-databases/system-tables/msdistribution-agents-transact-sql.md) .|  
...
## <a name="permissions"></a>Разрешения  
 Разрешения на выполнение по умолчанию принадлежат роли **public** . Пользователям всего лишь возвращаются сведения о подписках, которые они создали. Сведения обо всех подписках возвращаются членам предопределенной роли сервера **sysadmin** на издателе или членам предопределенной роли базы данных **db_owner** в базе данных публикации.  
