---
title: sp_helpsubscription (Transact-SQL) | Документы Microsoft
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
## <a name="arguments"></a>Аргументы  
 ...
 Имя связанной публикации. *Публикация* — **sysname**, значение по умолчанию **%**, которая возвращает сведения обо всех подписках для этого сервера.  
  
 [  **@article=** ] **"***статьи***"**  
 Имя статьи. *статья* — **sysname**, значение по умолчанию **%**, которая возвращает сведения обо всех подписках для выбранных публикаций и подписчиков. Если **все**, возвращаются только одна запись для полной подписки на публикацию.  
  
 [  **@subscriber=** ] **"***подписчика***"**  
 Имя подписчика, для которого возвращаются сведения о подписке. *подписчик* — **sysname**, значение по умолчанию **%**, которая возвращает сведения обо всех подписках для выбранных публикаций и статей.  
  
 [  **@destination_db=** ] **"***destination_db***"**  
 Имя целевой базы данных. *destination_db* — **sysname**, значение по умолчанию **%**.  
  
 [  **@found=** ] **"***найти***"** выходных данных  
 Флаг для указания возвращаемых строк. *найти*— **int** и является ВЫХОДНЫМ параметром, значение по умолчанию 23456.  
...
> [!NOTE]  
>  *издатель* указывать не нужно, кроме случаев, когда он издателя Oracle.  
...
|**Тип подписки**|**int**|Тип подписки:<br /><br /> **0** = принудительно отправить<br /><br /> **1** = по запросу<br /><br /> **2** = анонимная|  
|**Полная подписка**|**бит**|На все ли статьи публикации подписана данная подписка:<br /><br /> **0** = Нет<br /><br /> **1** = Да|  
...
|**loopback_detection**|**бит**|Механизм распознавания обратной связи определяет, отправляет ли агент распространителя транзакции, созданные в подписчике, обратно подписчику:<br /><br /> **0** = отправляет обратно.<br /><br /> **1** = не отправляет обратно.<br /><br /> Используется с двунаправленной репликацией транзакций. Дополнительные сведения см. в статье [Двунаправленная репликация транзакций](../../relational-databases/replication/transactional/bidirectional-transactional-replication.md).|  
|**offload_enabled**|**бит**|Указывает, было ли установлено разгрузочное выполнение агента репликации для запуска на подписчике.<br /><br /> Если **0**, агент выполняется на издателе.<br /><br /> Если **1**, агент выполняется на подписчике.|  
...
|**dts_package_location**|**int**|Расположение пакета служб DTS, если он назначен для подписки. Если пакет, а значение **0** указывает местоположение пакета на **распространителя**. Значение **1** указывает **подписчика**.|  
...
|**subscriber_password**||Фактический пароль подписчика никогда не возвращается. Результат скрывается "**\*\*\*\*\*\***» строки.|  
|**job_login**|**sysname**|Имя учетной записи Windows, под которой работает агент распространителя.|  
|**job_password**||Фактический пароль задания никогда не возвращается. Результат скрывается "**\*\*\*\*\*\***» строки.|  
 ...
 **sp_helpsubscription** используется в моментальных снимков и репликации транзакций.  
