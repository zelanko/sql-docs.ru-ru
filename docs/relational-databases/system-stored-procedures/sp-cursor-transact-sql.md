...
|0X40|ABSOLUTE|Может использоваться только совместно с UPDATE или DELETE.  ABSOLUTE используется только с курсорами KEYSET (не используется курсорами DYNAMIC, а курсоры STATIC не допускают обновления).<br /><br /> Примечание. Если значение ABSOLUTE указано для строки в ключевом наборе, которая не попала в выборку, то операция может не пройти проверку на параллелизм и возвращаемый результат не гарантируется.|  
