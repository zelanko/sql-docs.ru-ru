...
ms.locfileid: "72304939"
---
# <a name="sp_execute_external_script-transact-sql"></a>sp_execute_external_script (Transact-SQL)

[!INCLUDE[tsql-appliesto-ss2016-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2016-xxxx-xxxx-xxx-md.md)]

Выполняет скрипт, предоставленный в качестве входного аргумента для процедуры. Выполнение скриптов в [платформе расширяемости](../../advanced-analytics/concepts/extensibility-framework.md). Скрипт должен быть написан на поддерживаемом и зарегистрированном языке в ядре СУБД, имеющей по крайней мере одно расширение: [**R**](../../advanced-analytics/concepts/extension-r.md), [**Python**](../../advanced-analytics/concepts/extension-python.md)или [ **Java** (только в предварительной версии SQL Server 2019)](../../advanced-analytics/java/extension-java.md). 
...
> [!Note]
> Машинное обучение (R и Python) и расширения программирования устанавливаются в качестве надстройки для экземпляра ядра СУБД. Поддержка конкретных расширений зависит от версии SQL Server.
 ...
 **\@language** = N '*Language*'  
 Указывает язык скрипта. *язык* имеет тип **sysname**.  В зависимости от версии SQL Server допустимыми значениями являются R (SQL Server 2016 и более поздние версии), Python (SQL Server 2017 и более поздние версии) и Java (SQL Server 2019 Preview). 
...
::: moniker range=">=sql-server-ver15||>=sql-server-linux-ver15||=sqlallproducts-allversions"
`[ @input_data_1_order_by_columns = N'input_data_1_order_by_columns' ]` применяется только к SQL Server 2019 и используется для построения моделей на основе секций. Указывает имя столбца, используемого для упорядочивания результирующего набора, например по названию продукта. Тип данных переменной во внешнем скрипте зависит от языка. В случае с R входная переменная является кадром данных. В случае Python входные данные должны быть табличными.

`[ @input_data_1_partition_by_columns = N'input_data_1_partition_by_columns' ]` применяется только к SQL Server 2019 и используется для построения моделей на основе секций. Указывает имя столбца, используемого для сегментирования данных, таких как географическая область или дата. Тип данных переменной во внешнем скрипте зависит от языка. В случае с R входная переменная является кадром данных. В случае Python входные данные должны быть табличными. 
::: moniker-end
...
Используйте **sp_execute_external_script** для выполнения скриптов, написанных на поддерживаемом языке. В настоящее время Поддерживаемые языки — R для SQL Server 2016 R Services, Python и R для SQL Server 2017 Службы машинного обучения. 

По умолчанию результирующие наборы, возвращаемые этой хранимой процедурой, выводятся с неименованными столбцами. Имена столбцов, используемые в скрипте, являются локальными для среды сценариев и не отражаются в выходном результирующем наборе. Чтобы присвоить имя столбцам результирующего набора, используйте предложение `WITH RESULT SET` для [`EXECUTE`](../../t-sql/language-elements/execute-transact-sql.md).
  
 В дополнение к возврату результирующего набора можно вернуть скалярные значения в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] с помощью выходных параметров. В следующем примере показано использование ВЫХОДного параметра для возврата сериализованной модели R, которая использовалась в качестве входных данных для скрипта:  
...
### <a name="parameters-for-partition-modeling"></a>Параметры моделирования секций
 ...
 В SQL Server 2019 в настоящее время в общедоступной предварительной версии можно задать два дополнительных параметра, которые позволяют моделировать секционированные данные, где секции основаны на одном или нескольких столбцах, которые естественно сегментируют набор данных в логические секции, созданные и используемые. только во время выполнения скрипта. Столбцы, содержащие повторяющиеся значения для Age, Gender, географического региона, даты или времени, представляют собой несколько примеров, которые применяются к секционированным наборам данных.
 
 Два параметра — **input_data_1_partition_by_columns** и **input_data_1_order_by_columns**, где второй параметр используется для упорядочивания результирующего набора. Параметры передаются в качестве входных данных в `sp_execute_external_script` с помощью внешнего скрипта, который выполняется один раз для каждой секции. Дополнительные сведения и примеры см. в разделе [Tutorial: Создание моделей на основе секций @ no__t-0.
...
### <a name="streaming-execution-for-r-and-python-scripts"></a>Потоковое выполнение для скриптов R и Python  

Потоковая передача позволяет скрипту R или Python работать с большим объемом данных, чем может уместиться в памяти. Для управления числом строк, передаваемых во время потоковой передачи, укажите целочисленное значение для параметра `@r_rowsPerRead` в коллекции `@params`.  Например, при обучении модели, использующей очень широкие данные, можно изменить значение для чтения меньшего количества строк, чтобы все строки можно было отправить в один блок данных. Этот параметр также можно использовать для управления числом строк, считываемых и обрабатываемых одновременно, чтобы снизить производительность сервера. 
...
В качестве обходного решения **приведите** столбец или значение к поддерживаемому типу в [!INCLUDE[tsql](../../includes/tsql-md.md)], прежде чем отправлять его внешнему сценарию.  
...
В следующем примере создается хранимая процедура, использующая **sp_execute_external_script** для создания модели IRI и возвращающая модель в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
