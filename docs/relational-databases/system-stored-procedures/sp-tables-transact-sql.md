---
title: sp_tables (Transact-SQL) | Документы Microsoft
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
## <a name="arguments"></a>Аргументы  
 [  **@table_name=** ] **"***имя***"**  
 Таблица, используемая для возврата сведений о каталоге. *имя* — **nvarchar(384)**, значение по умолчанию NULL. Поиск совпадений по шаблону поддерживается.  
  
 [  **@table_owner=** ] **"***владельца***"**  
 Владелец таблицы, используемой для возврата сведений о каталоге. *владелец* — **nvarchar(384)**, значение по умолчанию NULL. Поиск совпадений по шаблону поддерживается. Если владелец не указан, применяются правила видимости таблиц по умолчанию базовой СУБД.  
 ...
 [ **,** [  **@table_type=** ] **«"***тип***"**, **"** типа **"»** ]  
 Список значений, разделенных запятой, в котором представлены сведения обо всех таблицах с указанными табличными типами. К ним относятся **таблицы**, **SYSTEMTABLE**, и **ПРЕДСТАВЛЕНИЕ**. *Тип* — **varchar(100)**, значение по умолчанию NULL.  
...
>  Каждый табличный тип должен быть заключен в одиночные кавычки, а весь параметр должен быть заключен в двойные кавычки. Типы таблиц должны указываться в верхнем регистре. Если настройка SET QUOTED_IDENTIFIER установлена в ON, то каждый тип таблицы должен быть заключен в двойные кавычки, а весь параметр должен быть заключен в одинарные кавычки.  
 ...
 Определяет, следует ли рассматривать символы подчеркивания (_), процента (%) и скобки ([ или ]) в качестве символов-шаблонов. Допустимые значения: 0 (сопоставление с шаблоном отключено) и 1 (сопоставление с шаблоном включено). *Шаблонов* — **бит**, значение по умолчанию 1.  
