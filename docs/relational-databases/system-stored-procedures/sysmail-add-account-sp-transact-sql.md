...
`[ @use_default_credentials = ] use_default_credentials` указывает, следует ли отправлять почту на SMTP-сервер, используя учетные данные @no__t – 1. **use_default_credentials** имеет бит и значение по умолчанию 0. Если этот аргумент равен 1, компонент Database Mail использует учетные данные компонента [!INCLUDE[ssDE](../../includes/ssde-md.md)]. Если этот параметр равен 0, Database Mail отправляет параметры **\@username** и **\@password** , если они есть, в противном случае отправляет почту без параметров **\@Username** и **\@password** .  
