...
Для [!INCLUDE[sssdw-md](../../includes/sssdw-md.md)], `sp_spacedused` необходимо указать именованные параметры (например `sp_spacedused (@objname= N'Table1');` вместо того чтобы полагаться на порядковый номер параметра. 
...
|**данные**|**varchar(18)**|Общий объем пространства, используемый данными *objname*.|  
|**index_size**|**varchar(18)**|Общий объем пространства, используемый индексами *objname*.|  
|**неиспользуемые**|**varchar(18)**|Общий объем пространства, зарезервированного для *objname* , но еще не используется.|  
