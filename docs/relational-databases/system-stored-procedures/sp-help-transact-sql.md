...
2.  Если *Name* является типом [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] данных или определяемым пользователем типом данных, **sp_help** возвращает этот результирующий набор.  
  
    |Имя столбца|Тип данных|Description|  
    |-----------------|---------------|-----------------|  
    |**Type_name**|**nvarchar (** 128 **)**|Имя типа данных.|  
    |**Storage_type**|**nvarchar (** 128 **)**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]имя типа.|  
    |**Недопустим**|**smallint**|Физическая длина типа данных (в байтах).|  
    |**прек**|**int**|Точность (общее количество знаков).|  
    |**Масштабирование**|**int**|Количество знаков справа от десятичной запятой.|  
    |**Nullable**|**varchar (** 35 **)**|Указывает, разрешены ли значения NULL: Да или нет.|  
    ...
    |**Параметры сортировки**|**имеет sysname**|Параметры сортировки для типа данных. Имеет значение NULL для несимвольных типов данных.|  
...
3.  Если *Name* — любой объект базы данных, кроме типа данных, **sp_help** возвращает этот результирующий набор, а также дополнительные результирующие наборы на основе указанного типа объекта.  
    ...
    -   Дополнительный результирующий набор, возвращаемый для объектов столбца:  
        ...
        |**Недопустим**|**int**|Длина столбца в байтах.<br /><br /> Примечание. Если тип данных столбца имеет тип больших значений (**varchar (max)**, **nvarchar (max)**, **varbinary (max)** или **XML**), то значение будет отображаться как-1.|  
        |**прек**|**char (** 5 **)**|Точность столбца.|  
        |**Масштабирование**|**char (** 5 **)**|Масштаб столбца.|  
        |**Nullable**|**varchar (** 35 **)**|Указывает, разрешены ли значения NULL в столбце: Да или нет.|  
        ...
        |**Параметры сортировки**|**имеет sysname**|Параметры сортировки столбца. Имеет значение NULL для несимвольных типов данных.|  
  
    -   Дополнительный результирующий набор, возвращаемый для столбцов идентификаторов:  
  
        |Имя столбца|Тип данных|Description|  
        |-----------------|---------------|-----------------|  
        |**Идентифицирует**|**nvarchar (** 128 **)**|Имя столбца, чей тип данных объявлен удостоверением.|  
        |**Инициализировать**|**ISNUMERIC**|Стартовое значение для столбца идентификаторов.|  
        |**Производим**|**ISNUMERIC**|Шаг прироста, который следует использовать для значений в этом столбце.|  
    ...
    -   Дополнительный результирующий набор, возвращаемый для столбцов:  
        ...
        |Имя столбца|Тип данных|Description|  
        |-----------------|---------------|-----------------|  
        |**RowGuidCol**|**имеет sysname**|Имя столбца глобального уникального идентификатора.|  
    ...
    -   Дополнительный результирующий набор, возвращаемый для хранимых процедур, функций или расширенных хранимых процедур.  
        ...
        |**Недопустим**|**smallint**|Максимальная физическая длина хранилища, в байтах.|  
        |**прек**|**int**|Точность или общее количество знаков.|  
        |**Масштабирование**|**int**|Число цифр справа от десятичной запятой.|  
...
## <a name="permissions"></a>Разрешения  
 Требуется членство в роли **Public** . Пользователь должен иметь по крайней мере одно разрешение на объект *objname*. Чтобы просмотреть ключи, значения по умолчанию или правила ограничения для столбца, необходимо обладать разрешением VIEW DEFINITION для этой таблицы.  
