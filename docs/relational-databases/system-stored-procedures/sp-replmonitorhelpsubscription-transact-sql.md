...
|**3-5**|Возвращать подписки с ошибками или предупреждениями о нарушениях пороговых показателей.|  
|**4**|Возвращает 25 самых худших подписок.|  
|**5.0**|Возвращать 50 подписок с худшей производительностью.|  
...
|-----------------|---------------|-----------------|  
|**состояние**|**int**|Проверяет состояние всех агентов репликации, связанных с публикацией, и возвращает самое высокое состояние из обнаруженных с учетом следующего порядка:<br /><br /> **6** = сбой<br /><br /> **5** = повторная попытка<br /><br /> **2** = остановлено<br /><br /> **4** = бездействие<br /><br /> **3** = выполняется<br /><br /> **1** = запущено|  
...
|**subscriber_db**|**имеет sysname**|Имя базы данных, использующейся подпиской.|  
...
|**distribution_agentname**|**имеет sysname**|Имя задания агента распространителя для подписки на публикацию транзакций.|  
|**mergeagentname**|**имеет sysname**|Имя задания агента слияния для подписки на публикацию слиянием.|  
|**mergesubscriptionfriendlyname**|**имеет sysname**|Понятное имя подписки.|  
|**mergeagentlocation**|**имеет sysname**|Имя сервера, на котором запущен агент слияния.|  
