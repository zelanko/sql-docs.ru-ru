...
|**64** (при запуске службы агента [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)])|*frequency_interval* не используется.|  
...
`[ @freq_subday_interval = ] frequency_subday_interval` количество периодов *frequency_subday_type* , которые должны быть выполнены между выполнением задания. *frequency_subday_interval* имеет **тип int**и значение по умолчанию 0.  
  
`[ @freq_relative_interval = ] frequency_relative_interval` дополнительно определяет *frequency_interval* , если *frequency_type* установлен в **32** (ежемесячное относительное значение).  
...
`[ @active_start_date = ] active_start_date` дату начала выполнения задания. *active_start_date* имеет **тип int**и не имеет значения по умолчанию. Формат даты: ГГГГMMДД. Если задан параметр *active_start_date* , дата должна быть больше или равна 19900101.  
...
-   **SQLAgentOperatorRole**  
 ...
 В следующем примере расписание задания назначается `SaturdayReports`, которое будет выполняться каждую субботу в 2:00 AM.
```sql  
