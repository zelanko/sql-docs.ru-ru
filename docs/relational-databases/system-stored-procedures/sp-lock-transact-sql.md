...
`[ @spid1 = ] 'session ID1'` — [!INCLUDE[ssDE](../../includes/ssde-md.md)] Числовой идентификатор сеанса из **sys.dm_exec_sessions** для которого пользователю требуются сведения о блокировке. *Session ID1* — **int** со значением по умолчанию NULL. Выполнение **sp_who** для получения сведений о сеансе. Если *session ID1* не указан, отображается информация обо всех блокировках.  
  
`[ @spid2 = ] 'session ID2'` Другой [!INCLUDE[ssDE](../../includes/ssde-md.md)] числовой идентификатор сеанса из **sys.dm_exec_sessions** , может иметь блокировку в то же время, как *session ID1* и о том, какие пользователю также требуются сведения. *сеанс ID2* — **int** со значением по умолчанию NULL.  
  
## <a name="return-code-values"></a>Значения кода возврата  
 0 (успешное завершение)  
  
## <a name="result-sets"></a>Результирующие наборы  
 **Sp_lock** результирующий набор содержит по одной строке для каждой блокировки, удерживаемые заданных **@spid1** и **@spid2** параметров. Если ни один из **@spid1** , ни **@spid2** указано, результат набор содержит блокировки для всех сеансов в экземпляре активный [!INCLUDE[ssDE](../../includes/ssde-md.md)].  
...
|**Если столбец indid равен**|**smallint**|Числовой идентификатор индекса, для которого удерживается блокировка.|  
...
|**Ресурс**|**nchar(32)**|Значение, определяющее блокируемый ресурс. Формат значения зависит от типа ресурса определяемого в **тип** столбца:<br /><br /> **Тип** значение: **Ресурс** значение<br /><br /> RID: Идентификатор в формате fileid:pagenumber:rid, где fileid обозначает файл, содержащий страницу, pagenumber ― страницу, содержащую строку, а rid ― заданную строку на странице. fileid соответствует **file_id** столбца в **sys.database_files** представления каталога.<br /><br /> KEY: шестнадцатеричное число, используемое компонентом [!INCLUDE[ssDE](../../includes/ssde-md.md)] для внутренних целей.<br /><br /> PAG: Число в формате fileid:pagenumber, где fileid определяет файл, содержащий страницу, а pagenumber определяет страницу.<br /><br /> EXT: Число, определяющее первую страницу в экстенте. Число в формате fileid:pagenumber.<br /><br /> TAB: Сведения не предоставляются, так как таблица уже определена в **ObjId** столбца.<br /><br /> DB: Сведения не предоставляются, так как база данных уже определена в **dbid** столбца.<br /><br /> FIL: Идентификатор файла, который соответствует **file_id** столбца в **sys.database_files** представления каталога.<br /><br /> APP: Уникальный идентификатор блокируемого ресурса приложения. В формате DbPrincipleId:\<первые два до 16 символов строки ресурса >\<значение хэша >.<br /><br /> MD: зависит от типа ресурса. Дополнительные сведения см. в описании **resource_description** столбца в [sys.dm_tran_locks &#40;Transact-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md).<br /><br /> HBT: Сведения не предоставлены. Используйте **sys.dm_tran_locks** динамическому административному представлению вместо этого.<br /><br /> AU: Сведения не предоставлены. Используйте **sys.dm_tran_locks** динамическому административному представлению вместо этого.|  
...
|**Состояние**|**nvarchar(5)**|Состояние запроса блокировки:<br /><br /> CNVRT: Блокировка, преобразованная в другом режиме, в ситуации блокирования другого процесса, удерживающего блокировку в конфликтном режиме.<br /><br /> GRANT: Блокировка получена.<br /><br /> WAIT: Блокировка занята другим процессом, удерживающим блокировку в конфликтном режиме.|  
