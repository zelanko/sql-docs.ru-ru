...
`[ @spid1 = ] 'session ID1'` — это [!INCLUDE[ssDE](../../includes/ssde-md.md)] идентификатор сеанса из **sys. dm_exec_sessions** , для которого пользователь хочет получить сведения о блокировке. *session ID1* имеет **тип int** и значение по умолчанию NULL. Выполните **sp_who** , чтобы получить сведения о ходе сеанса. Если параметр *session ID1* не указан, отображаются сведения обо всех блокировках.  
  
`[ @spid2 = ] 'session ID2'` — это еще один [!INCLUDE[ssDE](../../includes/ssde-md.md)] идентификатор сеанса из **sys. dm_exec_sessions** , который может иметь блокировку в то же время, что и *id1 сеанса* , а также сведения о том, что пользователю требуется информация. *Session id2* имеет **тип int** и значение по умолчанию NULL.  
