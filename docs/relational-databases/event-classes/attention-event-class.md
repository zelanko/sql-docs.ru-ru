...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] Класс событий **Attention** указывает, что произошло событие, требующее внимания пользователя (отмена, прерванный клиентом запрос, обрыв соединения). Операции отмены также могут появляться при истечении времени ожидания драйвером доступа к данным.  
