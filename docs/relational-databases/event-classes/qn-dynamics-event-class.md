...
|EventSubClass|**nvarchar**|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> **Clock run started**: был запущен фоновый поток в компоненте [!INCLUDE[ssDE](../../includes/ssde-md.md)], который планирует очистку таблиц параметров с истекшими сроками действия.<br /><br /> **Clock run finished**: был завершен фоновый поток в компоненте [!INCLUDE[ssDE](../../includes/ssde-md.md)], который планирует очистку таблиц параметров с истекшими сроками действия.<br /><br /> **Master cleanup task started**: определяет, когда будет запущена очистка (сборка мусора), удаляющая данные подписки на уведомления о запросах с истекшим сроком действия.<br /><br /> **Master cleanup task finished**: определяет, когда будет завершена очистка (сборка мусора), удаляющая данные подписки на уведомления о запросах с истекшим сроком действия.<br /><br /> **Master cleanup task skipped**: указывает на то, что компонентом [!INCLUDE[ssDE](../../includes/ssde-md.md)] не была выполнена очистка (сборка мусора) для удаления данных подписки на уведомления о запросах с истекшим сроком действия.|21|Да|  
