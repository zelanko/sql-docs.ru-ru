...
|EventSubClass|**nvarchar**|Тип подкласса событий, предоставляющий дополнительные сведения о каждом классе события. Данный столбец может содержать следующие значения.<br /><br /> **Подписка зарегистрирована**: указывает на то, что подписка на уведомления о запросах успешно зарегистрирована в базе данных.<br /><br /> **Повторная подписка**: указывает на то, что компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] получает запрос на подписку, полностью совпадающую с уже существующей подпиской. В таком случае компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] устанавливает значение тайм-аута для существующей подписки, совпадающее со значением тайм-аута, указанным в новом запросе на подписку.<br /><br /> **Подписка запущена**: указывает, что подписка на уведомления создает сообщение уведомления.<br /><br /> **Не удалось запустить подписку из-за ошибки брокера**: указывает, что сообщение уведомления не доставлено из-за ошибки компонента [!INCLUDE[ssSB](../../includes/sssb-md.md)].<br /><br /> **Не удалось запустить подписку, при этом ошибка брокера отсутствует**: указывает, что сообщение уведомления не доставлено, но не из-за ошибки компонента [!INCLUDE[ssSB](../../includes/sssb-md.md)].<br /><br /> **Перехвачена ошибка брокера**: указывает, что компонент [!INCLUDE[ssSB](../../includes/sssb-md.md)] передал ошибку в диалоге, который использует уведомление о запросе.<br /><br /> **Попытка удаления подписки**: указывает, что для высвобождения ресурсов компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] пытался удалить подписку с истекшим сроком действия.<br /><br /> **Не удалось удалить подписку**: указывает, что не удалось удалить подписку с истекшим сроком действия. Для высвобождения ресурсов компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] автоматически повторно запланирует удаление этой подписки.<br /><br /> **Подписка уничтожена**: указывает, что компонент [!INCLUDE[ssDE](../../includes/ssde-md.md)] успешно удалил подписку с истекшим сроком действия.|21|Да|  
