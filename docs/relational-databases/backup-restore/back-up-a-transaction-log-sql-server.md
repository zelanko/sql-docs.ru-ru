---
title: "Создание резервной копии журнала транзакций (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "02/01/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "резервные копии журналов транзакций [SQL Server], среда SQL Server Management Studio"
  - "резервные копии [SQL Server], создание"
  - "резервное копирование журналов транзакций [SQL Server], среда SQL Server Management Studio"
...
-   Инструкция BACKUP не разрешена в явных и [неявных](https://msdn.microsoft.com/library/ms187807.aspx) транзакциях.  Явными транзакциями являются транзакции, для которых явно назначаются запуск и остановка. 
  
###  <a name="Recommendations"></a> Рекомендации  
  
-   Если в базе данных используется полная модель восстановления или [модель восстановления](https://msdn.microsoft.com/library/ms189275.aspx) с неполным протоколированием, то необходимо регулярно создавать резервную копию журнала транзакций, чтобы защитить данные и предотвратить [переполнение журнала транзакций](https://msdn.microsoft.com/library/ms175495.aspx). При этом журнал усекается и поддерживает восстановление базы данных на определенный момент времени. Это нехорошо!
...
## Резервное копирование журнала транзакций с помощью среды SSMS  
...
13. Выберите параметр **Переписать носитель** , указав один из следующих вариантов:  
    ...
    -   **Создать резервную копию в существующем наборе носителей**  
         ...
         Для этого параметра выберите вариант **Добавить в существующий резервный набор данных** или **Перезаписать все существующие резервные наборы данных**. Дополнительные сведения см. в разделе [Наборы носителей, семейства носителей и резервные наборы данных (SQL Server)](../../relational-databases/backup-restore/media-sets-media-families-and-backup-sets-sql-server.md).  
