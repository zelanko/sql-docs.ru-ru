...
9.  В разделе **Соединения с сервером** установите флажок **Закрыть существующие подключения к целевой базе данных**.
    ...
    > [!NOTE]
    > Если вы не выберете этот параметр, может отобразиться следующее сообщение об ошибке: "System.Data.SqlClient.SqlError: Exclusive access could not be obtained because the database is in use. (Microsoft.SqlServer.SmoExtended)"
