...
# <a name="enable-sql-server-managed-backup-to-azure"></a>Включение управляемого резервного копирования SQL Server в Azure
 ...
 В этом руководстве используется Azure PowerShell. Перед началом работы с руководством [скачайте и установите Azure PowerShell](/powershell/azure/).  
...
Дополнительные сведения об учетных записях хранения см. в статье [Об учетных записях хранения Azure](/azure/storage/common/storage-account-create). 
...
Запишите URL-адрес контейнера и SAS для использования при создании учетных данных (CREDENTIAL) SQL. Дополнительные сведения о SAS см. в статье [Использование подписанных URL-адресов (SAS)](/azure/storage/common/storage-sas-overview).  
