...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  Поставщик [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB поддерживает две модели для потребителя, чтобы указать данные для строк параметров, оцененных по таблице: модель нажатия и модель притяжения. Образец, в котором демонстрируется применение модели по запросу, см. в разделе [Образцы программирования служб SQL Server](https://msftdpprodsamples.codeplex.com/).  
...
## <a name="push-model-loads-all-table-valued-paremeter-data-in-memory"></a>Принудительная модель (загружает все данные табличного параметра в память)  
 ...
 После [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] того, как поставщик NATIVE Client OLE DB начнет выполнение команды, значения параметров, оцениваемых таблицей, будут извлечены из этого объекта параметра и отправлены на сервер.  
