...
## <a name="example"></a>Пример  
 Прежде всего в базе данных `AdventureWorks` создается таблица для хранения событий графа взаимоблокировок. Таблица содержит два столбца: `AlertTime` столбец содержит время запуска оповещения, а столбец `DeadlockGraph` содержит XML-документ, содержащий граф взаимоблокировок.  
...
> [!NOTE]  
>  Для агент SQL Server получения событий WMI [!INCLUDE[ssSB](../../includes/sssb-md.md)] должны быть включены в **msdb** и [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)].  
...
```  
 ...
 Выполните следующий скрипт на вкладке второго запроса. Этот скрипт создает один результирующий набор, а затем блокирует, ожидая получения блокировки на `Production.Product`.  
...
```  
 ...
 Выполните следующий скрипт на вкладке первого запроса. Этот скрипт блокирует, ожидая получения блокировки на `Production.Location`. После короткого времени ожидания [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] выберет этот скрипт или скрипт в образце в качестве жертвы взаимоблокировки и завершит транзакцию.  
