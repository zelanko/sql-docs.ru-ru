...
## <a name="timestamp-maps-differently-in-client--vs-server-side-formatting"></a>Различное сопоставление типа timestamp при форматировании на стороне клиента и на стороне сервера  
 В XML на стороне сервера, форматирование, в столбце базы данных **timestamp** тип сопоставляется с типом XDR i8 (Если параметр XMLDATA указан в запросе).  
  
 В XML на стороне клиента, форматирование, в столбце базы данных **timestamp** либо сопоставляется с типом **uri** или **bin.base64** тип XDR (в зависимости от того binary base64 указан параметр в запросе). **Bin.base64** тип XDR полезно при использовании диаграммы обновления и массовой загрузки, так как этот тип преобразуется в [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] **timestamp** типа. Таким образом, операции вставки, обновления или удаления выполняются успешно.  
