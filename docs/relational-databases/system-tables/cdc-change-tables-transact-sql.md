...
|**supports_net_changes**|**бит**|Поддержка запросов сетевых изменений включается в таблице изменений.|  
|**has_drop_pending**|**бит**|Процесс отслеживания получил уведомление об удалении исходной таблицы.|  
...
|**partition_switch**|**бит**|Указывает, является ли **SWITCH PARTITION** команде **ALTER TABLE** могут быть выполнены для таблицы, в которой включено отслеживание изменений в данных. Значение 0 указывает на то, что переключение секций заблокировано. Несекционированные таблицы всегда возвращают значение 1.|  
