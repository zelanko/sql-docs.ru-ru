...
ms.locfileid: "39102212"
---
# <a name="system-base-tables"></a>Системные базовые таблицы
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]

  Системные базовые таблицы — это основные таблицы, в которых хранятся метаданные определенной базы данных. **Master** базы данных не особо, поскольку он содержит некоторые дополнительные таблицы, которые не находятся в других базах данных. Эти таблицы содержат устойчивые метаданные, областью которых является весь сервер.  
...
## <a name="system-base-table-metadata"></a>Метаданные системной базовой таблицы  
 У участника, который имеет разрешение CONTROL, ALTER или VIEW DEFINITION на базу данных можно увидеть метаданные системной базовой таблицы в **sys.objects** представления каталога. Участнику можно разрешать имена и идентификаторам системные базовые таблицы объектов с помощью встроенных функций, таких как [OBJECT_NAME](../../t-sql/functions/object-name-transact-sql.md) и [OBJECT_ID](../../t-sql/functions/object-id-transact-sql.md).  
...
> [!IMPORTANT]  
>  Доступ к системной базовой таблице с помощью приложения уровня данных предназначен только для [!INCLUDE[msCoName](../../includes/msconame-md.md)] персонала, он не является заказчикам.  
...
|**sys.sysidxstats**|Существует в каждой базе данных. Содержит по строке на каждый индекс или статистику для таблиц и индексированных представлений.<br /><br /> Примечание: Каждый индекс (кроме кучи) связан с статистикой, имеющей то же имя, что индекс.|  
...
|**sys.sysdbfiles**|Существует в каждой базе данных. Если столбец **dbid** равно нулю, строка представляет файл, относящийся к этой базе данных. В **master** базы данных, столбца **dbid** может иметь ненулевое значение. В этом случае строка представляет главный файл.|  
