...
1.  Постройте проект, выбрав **сборка** \<имя проекта > в меню **Сборка** .  
  
2.  Перед развертыванием сборки на тестовом сервере устраните все ошибки и предупреждения, полученные во время построения.  
  
3.  В меню **Сборка** выберите пункт **развернуть** . Сборка будет зарегистрирована в экземпляре [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] и базе данных, указанной при создании проекта [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] в среде Visual Studio.  
...
4.  Для файлов с исходным кодом на языке [!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual Basic:  
 ...
 Эти команды запускают визуальный C# или Visual Basic компилятор с помощью параметра **/Target** для указания сборки библиотеки DLL.  
...
1.  Процедура, функция, статистическое выражение, определяемый пользователем тип или триггер должны быть созданы в экземпляре [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Если сборка **HelloWorld** содержит метод с именем **HelloWorld** **в классе** Procedures, в запрос можно добавить следующий [!INCLUDE[tsql](../../includes/tsql-md.md)], чтобы создать процедуру с именем **Hello** в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
...
5.  На панели **Выбор типов объектов** выберите тип развертываемого объекта базы данных. Установите переключатель в положение **Далее**.  
...
## <a name="post-deployment-scripts"></a>Скрипты, выполняемые после развертывания  
 ...
 Чтобы добавить такой скрипт, необходимо добавить файл с именем postdeployscript.sql в каталог проекта Visual Studio. Например, щелкните правой кнопкой мыши проект в **Обозреватель решений** и выберите **Добавить существующий элемент**. Добавьте файл в корневой каталог проекта, а не в папку «Тестовые скрипты».  
