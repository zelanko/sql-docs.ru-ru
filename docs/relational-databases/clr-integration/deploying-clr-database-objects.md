...
4.  Для файлов с исходным кодом на языке [!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual Basic:  
 ...
 Эти команды запускают компилятор Visual C# или Visual Basic с помощью параметра **/Target** для указания сборки библиотеки DLL.  
...
1.  Процедура, функция, статистическое выражение, определяемый пользователем тип или триггер должны быть созданы в экземпляре [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Если сборка **HelloWorld** содержит метод с именем **HelloWorld** **в классе** Procedures, в запрос можно [!INCLUDE[tsql](../../includes/tsql-md.md)] добавить следующую строку для создания процедуры с именем Hello in. **** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
