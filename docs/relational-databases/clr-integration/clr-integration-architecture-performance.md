---
title: Производительность интеграции CLR (англ.) Документы Майкрософт
description: В этой статье рассматриваются варианты проектирования для интеграции сервера Microsoft S'L Server с системой .NET Framework CLR, включая процесс компиляции и производительность.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-asdbmi-xxxx-xxx-md](../../includes/appliesto-ss-asdbmi-xxxx-xxx-md.md)]
  В этой теме обсуждаются некоторые варианты [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] проектирования, которые повышают производительность интеграции с общим временем выполнения [!INCLUDE[msCoName](../../includes/msconame-md.md)] общего языка .NET Framework (CLR).  
  
## <a name="the-compilation-process"></a>Процесс компиляции  
 При компиляции выражений S'L при обнаружении [!INCLUDE[msCoName](../../includes/msconame-md.md)] ссылки на управляемую рутину генерируется заглушка промежуточного языка (MSIL). Заглушка содержит программный код для упаковки параметров процедуры из [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] в среду CLR, вызова функции и возвращения результата. Этот связующий код основан на типе параметра и его направлении (входной, выходной, передача по ссылке).  
...
## <a name="performance-considerations"></a>Вопросы производительности  
 Далее проводится краткое перечисление факторов производительности, специфичных для интеграции со средой CLR и [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Более подробную информацию можно найти в «[Использование интеграции CLR в s'L Server 2005»](https://go.microsoft.com/fwlink/?LinkId=50332)на веб-сайте MSDN. Общую информацию о производительности управляемого кода можно найти в статье[«Улучшение производительности и масштабируемости приложений .NET»](https://go.microsoft.com/fwlink/?LinkId=50333)на веб-сайте MSDN.  
...
### <a name="streaming-table-valued-functions"></a>Функции потока с табличным значением  
 ...
 STVFs управляются функциями, которые возвращают **IEnumerable** интерфейс. **IEnumerable** имеет методы навигации по набору результатов, возвращенным STVF. При вызове STVF возвращенный **IEnumerable** непосредственно связан с планом запроса. План запроса вызывает **IEnumerable** методы, когда ему нужно получить строки. Такая модель итерации позволяет провести немедленную обработку результатов сразу после получения первой строки, не ожидая заполнения всей таблицы. Она также существенно снижает затраты памяти на вызов функции.  
...
### <a name="string-data"></a>Строковые данные  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]данные о персонажах, такие как **varchar,** могут быть типа SqlString или SqlChars в управляемых функциях. Переменные типа SqlString создают в памяти экземпляр всего значения целиком. Переменные типа SqlChars обеспечивают потоковый интерфейс, который позволяет добиться более высокой производительности и масштабируемости, так как не создает в памяти экземпляра всего значения сразу. Это особенно важно для типов больших объектов (LOB). Кроме того, доступ к данным сервера XML можно получить через интерфейс потоковой передачи, возвращенный **SqlXml.CreateReader()**.  
  
### <a name="clr-vs-extended-stored-procedures"></a>Сравнение CLR и расширенных хранимых процедур  
 API-интерфейсы Microsoft.SqlServer.Server, позволяющие управляемым процедурам отсылать результирующие наборы обратно клиенту, имеют более высокую производительность, чем API-интерфейсы служб Open Data Services (ODS), используемые расширенными хранимыми процедурами. Кроме того, AIS System.Data.SqlServer поддерживают типы данных, такие как **xml,** **varchar(max)**, **nvarchar(max)** и **varbinary(max),** введенные в , в [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]то время как AI ODS не были расширены для поддержки новых типов данных.  
...
### <a name="native-serialization-for-user-defined-types"></a>Собственная сериализация для определяемых пользователем типов  
 Определяемые пользователем типы (UDT) представляют собой механизм расширения скалярной системы типов. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]реализует формат сериализации для UDTs под названием **Format.Native**. Во время компиляции исследуется структура типа, а затем создается код MSIL, настраиваемый для данного конкретного определения типа.  
  
 Собственная сериализация используется в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] по умолчанию. Сериализация, определяемая пользователем, вызывает для сериализации метод, указанный автором типа. **Формат.Родные** сериализации должны быть использованы, когда это возможно для лучшей производительности.  
