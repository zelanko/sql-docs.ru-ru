---
title: Проектирование ассембл (ru) Документы Майкрософт
description: В этой статье описываются факторы, которые следует учитывать при проектировании сборки для размещения на сервере S'L, включая упаковку, управление и ограничения на сборки.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
## <a name="managing-assembly-security"></a>Управление безопасностью сборок  
 Можно управлять доступом сборки к ресурсам, защищенным системой безопасности доступа к коду платформы .NET, когда она выполняет управляемый код. Вы делаете это, указывая один из трех наборов разрешений при создании или изменении сборки: SAFE, EXTERNAL_ACCESS или UNSAFE.  
...
### <a name="disallowed-net-framework-apis"></a>Неразрешенные API-интерфейсы платформы .NET Framework  
 Любой [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] API, аннотированный одним из запрещенных **HostProtectionAttributes,** не может быть вызван из SAFE и EXTERNAL_ACCESS сборки.  
