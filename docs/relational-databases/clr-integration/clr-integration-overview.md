---
title: Обзор интеграции CLR (ru) Документы Майкрософт
description: Хостинг CLR сервера Microsoft S'L называется интеграцией CLR. Авторство в управляемом коде может повысить производительность. Сервер S'L использует CAS для защиты управляемого кода.
ms.custom: ''
ms.date: 04/19/2017
ms.prod: sql
ms.reviewer: ''
...
> [!NOTE]  
>  Хотя многие из этих классов могут быть использованы из кода CLR в SQL Server, те из них, которые не подходят для использования на сервере (например, классы для работы с окнами), недоступны. Для получения дополнительной информации [см.](../../relational-databases/clr-integration/database-objects/supported-net-framework-libraries.md)  
 ...
 Интеграция со средой CLR позволяет улучшить производительность. Для получения [информации см.](../../relational-databases/clr-integration/clr-integration-architecture-performance.md)  
