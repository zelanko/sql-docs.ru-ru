...
-  При **реорганизации индекса** используется минимальный объем системных ресурсов. Эта операция выполняется в режиме "в сети". Это означает, что долгосрочные блокировки таблицы не удерживаются и запросы или обновления базовой таблицы могут продолжаться во время выполнения транзакции `ALTER INDEX REORGANIZE`.     
...
-  При **перестроении** старый индекс удаляется и создается новый. В зависимости от типа индекса и версии [!INCLUDE[ssDE](../../includes/ssde-md.md)] это можно сделать в режиме "в сети" или "вне сети".      
...
Индекс нельзя реорганизовать или перестроить, если файловая группа, в которой он находится, размещена **вне сети** или настроена **только для чтения**. Если указывается ключевое слово `ALL`, а один индекс или несколько находятся в файловой группе, которая размещена вне сети или предназначена только для чтения, то выполнить инструкцию не удастся.
...
В версиях, предшествующих [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)], перестроение кластеризованного индекса columnstore выполняется как операция "вне сети". [!INCLUDE[ssde_md](../../includes/ssde_md.md)] должен получить монопольную блокировку в таблице или секции на время выполнения перестроения. Данные находятся в режиме "вне сети" и недоступны во время перестроения даже при использовании `NOLOCK`, изоляции моментальных снимков с чтением зафиксированных данных (RCSI) или изоляции моментальных снимков.         
