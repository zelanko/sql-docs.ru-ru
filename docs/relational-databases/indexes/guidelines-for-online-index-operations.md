...
-   Управление, планирование и расширение окна обслуживания индекса. Можно приостановить и перезапустить операцию перестроения индекса несколько раз по размеру окна обслуживания.
- Восстановление после сбоев перестроения индекса (например, переход на другой ресурс базы данных или нехватки места на диске).
- При приостановке операции с индексами, исходный индекс и только что созданной требуется место на диске и должны быть обновлены во время операций DML.

- Возможность производить усечение усечение журналов во время перестроения индекса (эта операция не может выполняться для операции регулярного индексами в сети).
- Параметр SORT_IN_TEMPDB = ON не поддерживается

> [!IMPORTANT]
> Возобновляемые перестроения требует закрывайте длительных усечение, позволяя усечение журнала во время этой операции и более эффективного управления пространства журнала. Благодаря новой конструкции управляемых необходимые данные должны храниться в базе данных вместе с все ссылки, потребуется перезапустить возобновляемые операции.
>

Как правило является отсутствие разницы в производительности перестроение индекса в сети возобновляемые и не возобновляемые. При обновлении возобновляемые индекс, пока операция перестроения индекса приостановлено:
- В основном для чтения рабочих нагрузок незначительна влияние на производительность. 
- Для обновления ресурсоемких рабочих нагрузок то может снизиться пропускной способности (нашей тестирования показано меньше 10% снижение).

Как правило нет разницы в качества дефрагментации между перестроение индекса в сети возобновляемые и не возобновляемые.
