...
|SERVICE_BROKER_WAITFOR_MANAGER| Используется для синхронизации уровня картой экземпляров очередей ожидающий объект. Каждого кортежа идентификатор версии базы данных и идентификатор очереди базы данных имеется одна очередь. Состязание за кратковременную блокировку этого класса может возникать в случае большого количества подключений: В WAITFOR(RECEIVE) ожидания состояния; вызов WAITFOR(RECEIVE); Превышение времени ожидания WAITFOR; Получение сообщения; Фиксация или откат транзакции, содержащей WAITFOR(RECEIVE); Можно снизить количество конфликтов за счет сокращения числа потоков в состоянии ожидания WAITFOR(RECEIVE). |  
