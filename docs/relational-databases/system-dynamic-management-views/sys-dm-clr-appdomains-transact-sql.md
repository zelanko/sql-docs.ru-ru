...
|**маршрут**|**int**|Стоимость **AppDomain**. Чем выше стоимость, тем выше вероятность, что этот **AppDomain** будет выгружен при нехватке памяти. Затраты обычно зависят от объема памяти, необходимой для повторного создания этого **AppDomain**.|  
|**значений**|**int**|Значение **домена приложения**. Чем ниже значение, тем скорее вероятность, что этот **AppDomain** будет выгружен при нехватке памяти. Значение обычно зависит от того, сколько подключений или пакетов использует этот **AppDomain**.|  
