...
|status|**nvarchar (30)**|Состояние запроса. Может применяться один из перечисленных ниже типов.<br /><br /> Фоновый<br />Выполнение<br />Готово к запуску<br />В режиме ожидания<br />Suspended<br /><br /> Не допускает значение NULL.|  
|command|**nvarchar (32)**|Тип выполняемой в данный момент команды. Основные типы команд:<br /><br /> SELECT<br />INSERT<br />UPDATE<br />УДАЛИТЬ<br />BACKUP LOG<br />BACKUP DATABASE<br />DBCC<br />FOR<br /><br /> Текст запроса можно получить при помощи функции sys.dm_exec_sql_text, передав ей значение столбца sql_handle. Внутренние системные процессы устанавливают команду в соответствии с выполняемой задачей. Например:<br /><br /> LOCK MONITOR;<br />CHECKPOINTLAZY;<br />WRITER.<br /><br /> Не допускает значение NULL.|  
...
|quoted_identifier|**версий**|1 = Параметр QUOTED_IDENTIFIER для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|arithabort|**версий**|1 = Параметр ARITHABORT для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|ansi_null_dflt_on|**версий**|1 = Параметр ANSI_NULL_DFLT_ON для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|ansi_defaults|**версий**|1 = Параметр ANSI_DEFAULTS для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|ansi_warnings|**версий**|1 = Параметр ANSI_WARNINGS для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|ansi_padding|**версий**|1 = Параметр ANSI_PADDING для запроса включен (ON).<br /><br /> В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|ansi_nulls|**версий**|1 = Параметр ANSI_NULLS для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
|concat_null_yields_null|**версий**|1 = Параметр CONCAT_NULL_YIELDS_NULL для запроса включен (ON). В противном случае — 0.<br /><br /> Не допускает значение NULL.|  
...
|statement_sql_handle|**varbinary (64)**|**Применимо к**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] и более поздним версиям.<br /><br /> Описатель SQL отдельного запроса.<br /><br />Этот столбец имеет значение NULL, если хранилище запросов не включено для базы данных. |  
|statement_context_id|**bigint**|**Применимо к**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] и более поздним версиям.<br /><br /> Необязательный внешний ключ к sys. query_context_settings.<br /><br />Этот столбец имеет значение NULL, если хранилище запросов не включено для базы данных. |  
|dop |**int** |**Применимо к**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и более поздним версиям.<br /><br /> Степень параллелизма запроса. |  
|parallel_worker_count |**int** |**Применимо к**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и более поздним версиям.<br /><br /> Количество зарезервированных параллельных рабочих ролей, если это параллельный запрос.  |  
|external_script_request_id |**uniqueidentifier** |**Применимо к**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] и более поздним версиям.<br /><br /> Идентификатор запроса внешнего скрипта, связанный с текущим запросом. |  
|is_resumable |**версий** |**Применимо к**: [!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)] и более поздним версиям.<br /><br /> Указывает, является ли запрос возобновляемой операцией с индексом. |  
...
### <a name="a-finding-the-query-text-for-a-running-batch"></a>а. Поиск текста запроса для выполнения пакета
...
### <a name="b-finding-all-locks-that-a-running-batch-is-holding"></a>B. Поиск всех блокировок, которые содержит выполняемый пакет
...
Затем, чтобы найти сведения о блокировке, используйте скопированную `transaction_id` функцию с системной функцией **sys. dm_tran_locks**.  
...
### <a name="c-finding-all-currently-blocked-requests"></a>C. Поиск всех запросов, заблокированных в настоящий момент

В следующем примере производится запрос к **sys. dm_exec_requests** для поиска сведений о заблокированных запросах.  
...
### <a name="d-ordering-existing-requests-by-cpu"></a>D. Упорядочивание существующих запросов по ЦП
