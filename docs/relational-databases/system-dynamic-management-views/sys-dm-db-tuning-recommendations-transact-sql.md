...
| **—\_исполняемый\_действие** | **bit** | 1 = рекомендации, которые могут быть выполнены в базе данных с помощью [!INCLUDE[tsql_md](../../includes/tsql_md.md)] скрипта.<br />0 = рекомендация не может быть выполнена в базе данных (например: сведения о рекомендации только из цифр или возвращенного в предыдущее состояние) |
