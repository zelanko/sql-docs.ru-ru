...
> [!NOTE]  
> Чтобы вызвать это из [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] или [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], используйте имя **sys. dm_pdw_nodes_tran_locks**.  
...
## <a name="remarks"></a>Remarks  
 Состояние предоставленного запроса показывает, что блокировка ресурса была предоставлена запрашивающему объекту. Ожидающий запрос обозначает, что запрос еще не был предоставлен. **Request_status** столбец возвращает следующие типы ожидающих запросов:  
...
-   Табличные подсказки блокировки для указания уровня блокировки для отдельной ссылки таблицы в предложении FROM. Синтаксис и ограничения см. в разделе [Табличные указания &#40;Transact&#41;-SQL](../../t-sql/queries/hints-transact-sql-table.md).  
 ...
 В пределах одного сеанса ресурсу может быть предоставлено более одной блокировки. Разные сущности, выполняющиеся в одном сеансе, могут владеть блокировкой одного и того же ресурса, и эти сведения отображаются в столбцах **request_owner_type** и **request_owner_id** , возвращаемых **sys. dm_tran_locks**. Если существует несколько экземпляров одного **request_owner_type** , столбец **request_owner_id** используется для различения каждого экземпляра. Для распределенных транзакций **request_owner_type** и столбцы **request_owner_guid** будут показывать различные сведения о сущности.  
  
 Например, сеанс S1 владеет общей блокировкой в **Table1**; и транзакция T1, которая выполняется в сеансе S1, также владеет общей блокировкой в таблице **Table1**. В этом случае столбец **resource_description** , возвращаемый **sys. dm_tran_locks** , будет показывать два экземпляра одного и того же ресурса. В столбце **request_owner_type** будет показан один экземпляр сеанса, а другой — транзакция. Кроме того, столбец **resource_owner_id** будет иметь разные значения.  
...
|METADATA.XML_INDEX_QNAME|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
 ...
 В следующей таблице представлен формат **resource_description** столбца для каждого типа ресурса.  
...
### <a name="a-using-sysdm_tran_locks-with-other-tools"></a>A. Использование sys.dm_tran_locks с другими средствами  
 В следующем примере выполняется работа со сценарием, в котором операция обновления блокируется другой транзакцией. С помощью представления **sys. dm_tran_locks** и других средств предоставляются сведения о ресурсах блокировки.  
...
```  
 ...
 Следующий запрос отображает сведения о блокировке. Значение для `<dbid>` должно быть заменено **database_id** из **sys. databases**.  
