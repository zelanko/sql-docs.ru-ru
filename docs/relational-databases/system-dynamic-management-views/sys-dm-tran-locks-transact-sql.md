...
> [!NOTE]  
> Вызывать его из [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] или [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], используйте имя **sys.dm_pdw_nodes_tran_locks**.  
  
|Имя столбца|Тип данных|Описание|  
|-----------------|---------------|-----------------|  
|**resource_type**|**nvarchar(60)**|Представляет тип ресурса. Значение может быть одним из следующих: Базы данных, файла, объект, страница, ключ, ЭКСТЕНТ, RID, приложения, МЕТАДАННЫЕ, HOBT или ALLOCATION_UNIT.|  
...
|**pdw_node_id**|**int**|**Применяется к**: [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)], [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]<br /><br /> <br /><br /> Идентификатор для узла, это распределение является на.|  
...
-   Табличные подсказки блокировки для указания уровня блокировки для отдельной ссылки таблицы в предложении FROM. Синтаксис и ограничения, см. в разделе [табличные указания &#40;Transact-SQL&#41;](../../t-sql/queries/hints-transact-sql-table.md).  
 ...
 В пределах одного сеанса ресурсу может быть предоставлено более одной блокировки. Различные сущности, которые выполняются в рамках одного сеанса может каждый является владельцем блокировки на тот же ресурс, и информация отображается в **request_owner_type** и **request_owner_id** столбцы, которые являются возвращенный **sys.dm_tran_locks**. Если несколько экземпляров одной и той же **request_owner_type** существует, **request_owner_id** столбец используется для различия экземпляров. Для распределенных транзакций **request_owner_type** и **request_owner_guid** столбцы будут показаны сведения о другой сущности.  
  
 Например, сеанс S1 владеет совмещаемой блокировкой на **Table1**; и транзакция Т1, запущенная в сеансе S1, также владеет совмещаемой блокировкой на **Table1**. В этом случае **resource_description** столбец, который возвращается методом **sys.dm_tran_locks** , отображает два экземпляра одного ресурса. **Request_owner_type** столбец будет один экземпляр отображается как сеанс и второй — как транзакция. Кроме того **resource_owner_id** столбец будет содержать разные значения.  
...
|PAGE|Представляет отдельную страницу в файле данных.|Идентификатор HoBt. Это значение соответствует **sys.partitions.hobt_id**. Идентификатор HoBt не всегда доступен для ресурсов PAGE, так как Идентификатор HoBt — дополнительные данные, предоставляемые вызывающим объектом и не все вызывающие объекты способны предоставить эти данные.|  
...
-   ddl_with_wait_at_low_priority  
 ...
 Существующее событие XEvent **progress_report_online_index_operation** для индекса в сети операций было расширено путем добавления **partition_number** и **partition_id**.  
  
## <a name="examples"></a>Примеры  
  
### <a name="a-using-sysdmtranlocks-with-other-tools"></a>A. Использование sys.dm_tran_locks с другими средствами  
 В следующем примере выполняется работа со сценарием, в котором операция обновления блокируется другой транзакцией. С помощью **sys.dm_tran_locks** и другие средства, предоставляется информация об источниках блокировки.  
...
```  
 ...
 Следующий запрос отображает сведения о блокировке. Значение для `<dbid>` следует заменить **database_id** из **sys.databases**.  
