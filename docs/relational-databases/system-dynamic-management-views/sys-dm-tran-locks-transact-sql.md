---
title: sys. DM _tran_locks (Transact-SQL) | Документация Майкрософт
ms.custom: ''
ms.date: 03/30/2017
ms.prod: sql
...
> [!NOTE]  
> Чтобы вызвать его из [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] или [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], используйте имя **sys. DM _pdw_nodes_tran_locks**.  
...
|**request_owner_id**|**bigint**|Идентификатор определенного владельца запроса.<br /><br /> Если владельцем запроса является транзакция, это значение содержит идентификатор транзакции.<br /><br /> Если таблица FileTable является владельцем запроса, **request_owner_id** имеет одно из следующих значений.<br /><br /> <br /><br /> четырех Таблица FileTable заняла блокировку базы данных.<br /><br /> 3-5 Таблица FileTable потратила блокировку таблицы.<br /><br /> Другое значение: Значение представляет собой маркер файла. Это значение также отображается как **fcb_id** в динамическом административном представлении [sys. DM &#40;_filestream_non_transacted_handles Transact-&#41;SQL](../../relational-databases/system-dynamic-management-views/sys-dm-filestream-non-transacted-handles-transact-sql.md).|  
...
|**lock_owner_address**|**varbinary (8)**|Адрес внутренней структуры данных в памяти, используемый для отслеживания этого запроса. Этот столбец можно объединить со столбцом WITH **resource_address** в **sys. DM _os_waiting_tasks**.|  
...
## <a name="remarks"></a>Примечания  
 Состояние предоставленного запроса показывает, что блокировка ресурса была предоставлена запрашивающему объекту. Ожидающий запрос обозначает, что запрос еще не был предоставлен. Столбец **request_status** возвращает следующие типы ожидающих запросов:  
...
-   Табличные подсказки блокировки для указания уровня блокировки для отдельной ссылки таблицы в предложении FROM. Синтаксис и ограничения см. в разделе [Табличные указания &#40;Transact&#41;-SQL](../../t-sql/queries/hints-transact-sql-table.md).  
 ...
 В пределах одного сеанса ресурсу может быть предоставлено более одной блокировки. Разные сущности, выполняющиеся в одном сеансе, могут владеть блокировкой одного и того же ресурса, и эти сведения отображаются в столбцах **request_owner_type** и **request_owner_id** , возвращаемых **sys. DM _tran_locks**. Если существует несколько экземпляров одного **request_owner_type** , столбец **request_owner_id** используется для различения каждого экземпляра. Для распределенных транзакций в столбцах **request_owner_type** и **request_owner_guid** отображаются различные сведения о сущности.  
  
 Например, сеанс S1 владеет общей блокировкой в **Table1**; и транзакция T1, которая выполняется в сеансе S1, также владеет общей блокировкой в таблице **Table1**. В этом случае столбец **resource_description** , возвращаемый **sys. DM _tran_locks** , будет показывать два экземпляра одного и того же ресурса. В столбце **request_owner_type** будет показан один экземпляр в сеансе, а другой — как транзакция. Кроме того, столбец **resource_owner_id** будет иметь разные значения.  
...
|PAGE|Представляет отдельную страницу в файле данных.|Идентификатор HoBt. Это значение соответствует **sys. partitions. hobt_id**. Идентификатор HoBt не всегда доступен для ресурсов страницы, поскольку Идентификатор HoBt является дополнительной информацией, которая может быть предоставлена вызывающим объектом, а не все вызывающие объекты могут предоставить эти сведения.|  
...
|OBJECT|< object_id >|Идентификатор объекта, представляемого данным ресурсом. Этот объект может быть любым объектом, указанным в **sys. Objects**, а не только в таблице.|  
...
|METADATA.XML_INDEX_QNAME|object_id = O, $qname_id = Q|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
 ...
 Следующие XEvents связаны с **переключением** секций и перестроением индекса в сети. Дополнительные сведения о синтаксисе см. в разделе [ALTER &#40;Table&#41; Transact-SQL](../../t-sql/statements/alter-table-transact-sql.md) и [ALTER &#40;index&#41;Transact-SQL](../../t-sql/statements/alter-index-transact-sql.md).  
...
-   ddl_with_wait_at_low_priority  
 ...
 Существующие операции XEvent **progress_report_online_index_operation** для операций с индексами в сети были расширены путем добавления **partition_number** и **partition_id**.  
  
## <a name="examples"></a>Примеры  
  
### <a name="a-using-sysdm_tran_locks-with-other-tools"></a>A. Использование sys.dm_tran_locks с другими средствами  
 В следующем примере выполняется работа со сценарием, в котором операция обновления блокируется другой транзакцией. С помощью представления **sys. DM _tran_locks** и других средств предоставляются сведения о ресурсах блокировки.  
...
```  
 ...
 Следующий запрос отображает сведения о блокировке. Значение `<dbid>` должно быть заменено значением **database_id** из **sys. databases**.  
...
[sys. DM _tran_database_transactions &#40;, Transact-&#41;SQL](../../relational-databases/system-dynamic-management-views/sys-dm-tran-database-transactions-transact-sql.md)      
