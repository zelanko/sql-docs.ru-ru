...
|**значение ordinal_in_order_by_list**|**smallint**|положение этого столбца в списке ORDER BY. Возвращает NULL, если столбец отсутствует в списке ORDER BY или если список ORDER BY нельзя определить однозначно.|  
...
|**функция ERROR_STATE**|**int**|Содержит сообщение о состоянии. возвращенное функцией. Столбец будет содержать NULL, если не возникло ошибок.|  
|**функция ERROR_MESSAGE**|**nvarchar(4096)**|Содержит сообщение, возвращаемое этой функцией. Столбец будет содержать NULL, если не возникло ошибок.|  
