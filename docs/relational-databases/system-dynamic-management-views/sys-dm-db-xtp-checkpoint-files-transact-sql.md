...
> **ПРИМЕЧАНИЕ.** Для текущей контрольной точки, которая не закрыт, в столбце состояния s`ys.dm_db_xtp_checkpoint_files` будет UNDER CONSTRUCTION для новых файлов. Контрольная точка закрывается автоматически, при наличии достаточно разрастания журнала транзакций с момента последней контрольной точки, или если `CHECKPOINT` команды ([контрольной ТОЧКИ &#40;Transact-SQL&#41;](../../t-sql/language-elements/checkpoint-transact-sql.md)).  
  
 Оптимизированные для памяти файловой группе внутренне использует только для добавления файлов для хранения вставленных и удаленных строках для таблиц в памяти. Существует два типа файлов. Файл данных содержит вставленные строки, а разностный файл содержит ссылки на удаленные строки. 
...
|relative_file_path|**nvarchar(256)**|Путь к файлу относительно контейнера, в которой он сопоставлен.|  
|file_type|**smallint**|-1 для FREE<br /><br /> 0 для файла данных.<br /><br /> 1 для РАЗНОСТНОГО файла.<br /><br /> 2 — КОРНЕВОЙ файл<br /><br /> 3 для БОЛЬШИХ файлов данных|  
|file_type_desc|**nvarchar(60)**|FREE - все файлы, обслуживаемые как СВОБОДНУЮ доступны для выделения. Бесплатные файлы может иметь разный размер в зависимости от нужд системой. Максимальный размер составляет 1 ГБ.<br /><br /> ТАРИФНЫЙ план — файлы данных содержат строки, которые были вставлены в таблицах, оптимизированных для памяти.<br /><br /> ДЕЛЬТА - разностные файлы содержат ссылки на строки в файлах данных, которые были удалены.<br /><br /> ROOT - корневые файлы содержат системные метаданные для объектов, оптимизированных для памяти и скомпилированные в собственном коде.<br /><br /> Большие ОБЪЕМЫ данных — большие файлы данных содержат значения, вставленные в (n)varchar(max) и varbinary(max) столбцы, а также сегментов столбцов, которые являются частью индексы columnstore таблиц, оптимизированных для памяти.|  
...
|logical_row_count|**bigint**|Для данных, количество вставленных строк.<br /><br /> Разностным изменениям число строк, удалить после учета для удаления таблицы.<br /><br /> Для корневого каталога имеет значение NULL.|  
|state|**smallint**|0 — ПРЕДВАРИТЕЛЬНО СОЗДАННЫХ ПАР<br /><br /> 1 - В ПРОЦЕССЕ ПОСТРОЕНИЯ<br /><br /> 2 — ACTIVE<br /><br /> 3 — MERGE TARGET<br /><br /> 8 - ОЖИДАЮЩИЕ УСЕЧЕНИЕ ЖУРНАЛА|  
|state_desc|**nvarchar(60)**|PRECREATED — количество файлов контрольных точек, предварительно выделяются, чтобы минимизировать или устранить Ожидание на выделение новых файлов при выполнении транзакций. Этих предварительно созданных файлов может иметь разный размер в зависимости от расчетных потребностей рабочей нагрузки, но они не содержат данных. Это связано с дополнительными затратами на хранение в базах данных с файловой группой MEMORY_OPTIMIZED_DATA.<br /><br /> В процессе ПОСТРОЕНИЯ — эти файлы контрольных точек находятся в процессе создания, это означает, что они заполняются на основе записей журнала, созданных базой данных и еще не добавлены контрольной точки.<br /><br /> ACTIVE — они содержат вставленные или удаленные строки из предыдущих закрытых контрольных точек. Они включают содержимое таблиц, которое считывается в память перед применением активной части журнала транзакций после перезапуска базы данных. Мы ожидаем, что размер этих файлов контрольных точек, следует убедиться, что приблизительно 2 раза больше размера в памяти таблиц, оптимизированных для памяти, при условии, что операция слияния справляется с транзакционными рабочими нагрузками.<br /><br /> MERGE TARGET - целевого объекта для операций слияния — эти файлы контрольных точек хранения консолидированные строки данных из исходных файлов, которые были определены политикой слияния. После установки слияния MERGE TARGET переходит в состояние ACTIVE.<br /><br /> ОЖИДАЮЩИЕ усечение ЖУРНАЛА — после установки слияния, и этот CFP MERGE TARGET является частью устойчивой контрольной точки, контрольных точек файлы слияния источник переходят в это состояние. Файлы в этом состоянии необходимы для правильной работы базы данных с оптимизированной для памяти таблицы.  Например, чтобы выполнить восстановление от устойчивой контрольной точки, чтобы вернуться назад во времени.|  
|lower_bound_tsn|**bigint**|Нижняя граница транзакции в файле; значение NULL, если не в состоянии (1, 3).|  
|upper_bound_tsn|**bigint**|Верхняя граница транзакции в файле; значение NULL, если не в состоянии (1, 3).|  
...
|file_type_desc|**nvarchar(60)**|Тип файла: DATA_FILE, DELTA_FILE или значение NULL, если столбцу состояния задано значение 7.|  
...
|state|**int**|0 — ПРЕДВАРИТЕЛЬНО СОЗДАННЫХ ПАР<br /><br /> 1 - В ПРОЦЕССЕ ПОСТРОЕНИЯ<br /><br /> 2 — ACTIVE<br /><br /> 3 — MERGE TARGET<br /><br /> 4 — MERGED SOURCE<br /><br /> 5 - ТРЕБУЕТСЯ FOR BACKUP/HA<br /><br /> 6 — IN TRANSITION TO TOMBSTONE<br /><br /> 7 - ЗАХОРОНЕНИЯ|  
|state_desc|**nvarchar(60)**|PRECREATED — небольшой набор данных и разностных пар файлов, также известный как пары (Cfp) файлов контрольных точек хранятся выделяются заранее, чтобы свести к минимуму или вообще устранить Ожидание на выделение новых файлов при выполнении транзакций. Это полноценные файлы данных размером 128 МБ и разностные файлы размером 8 МБ, но без данных. Число пар файлов соответствует числу логических процессоров или планировщиков (по одному на ядро, без максимального значения), но не менее 8. Это фиксированное пространство хранения в базах данных с таблицами, оптимизированными для памяти.<br /><br /> В процессе ПОСТРОЕНИЯ — набор пар файлов контрольных точек, которые записываются вновь вставляются и возможно, удаленные строки данных с момента последней контрольной точки.<br /><br /> ACTIVE — файлы в этом состоянии содержат вставленные и удаленные строки из ближайших предыдущих закрытых контрольных точек. Эти пары файлов содержат все обаятельные вставленные и удаленные строки, которые необходимы, прежде чем будет применена активная часть журнала транзакций после перезапуска базы данных. Размер этих CFP будет приблизительно в два раза больше размера оптимизированных для памяти таблиц в памяти при условии, что операция объединения выполняется в настоящий момент с транзакционной рабочей нагрузкой.<br /><br /> MERGE TARGET — этот CFP хранит консолидированные строки данных из cfp, которые были определены политикой слияния. После установки слияния MERGE TARGET переходит в состояние ACTIVE.<br /><br /> MERGED SOURCE — после операции слияния установки, исходные Cfp помечаются как MERGED SOURCE. Обратите внимание, что средство оценки политики слияния может определять несколько слияний, а CFP может участвовать только в одной операции слияния.<br /><br /> ТРЕБУЕТСЯ FOR BACKUP/HA — после установки слияния и пару файлов целевого СЛИЯНИЕМ является частью устойчивой контрольной точки, исходные Cfp слияния переходят в это состояние. Находящиеся в этом состоянии CFP необходимы для правильности работы базы данных с оптимизированной для памяти таблицы.  Например, чтобы выполнить восстановление от устойчивой контрольной точки, чтобы вернуться назад во времени. Пару файлов можно отметить для сборки мусора после того, как точка усечения журнала выходит из диапазона транзакций.<br /><br /> IN TRANSITION TO TOMBSTONE — эти Cfp не нужны, модуль In-Memory OLTP, и они могут подпадать сборщиком мусора. Это состояние указывает, что эти CFP ожидают, чтобы фоновый поток перевел их в следующее состояние, которым является состояние TOMBSTONE.<br /><br /> TOMBSTONE — эти Cfp ожидают заберет сборщик мусора filestream. ([sp_filestream_force_garbage_collection &#40;Transact-SQL&#41;](../../relational-databases/system-stored-procedures/filestream-and-filetable-sp-filestream-force-garbage-collection.md))|  
...
## <a name="use-cases"></a>Варианты использования  
 Вы можете оценить хранилище, используемое в In-Memory OLTP, следующим образом:  
