...
> [!NOTE]  
>  Чтобы вызвать эту функцию [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] из [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]или, используйте имя **sys. dm_pdw_nodes_tran_active_snapshot_database_transactions**.  
...
-   XSN-57: так как эта транзакция не выполняется при изоляции моментального снимка, `first_snapshot_sequence_num` это `0` `is_snapshot` значение и. Аргумент `transaction_sequence_num` показывает, что данной транзакции был присвоен порядковый номер, поскольку параметр READ_COMMITTED_SNAPSHOT или ALLOW_SNAPSHOT_ISOLATION (или оба) имеют значение ON.  
