...
## <a name="remarks"></a>Remarks  
 ...
 Можно сбросить содержимое sys. dm_os_spinlock_stats, `DBCC SQLPERF` выполнив следующую команду:  
...
## <a name="spinlocks"></a>Спин-блокировки  
 Спин-блокировка — это объект облегченной синхронизации, используемый для сериализации доступа к структурам данных, которые обычно удерживаются в течение короткого периода времени. Когда поток пытается получить доступ к ресурсу, защищенному с помощью спин-блокировки, которая удерживается другим потоком, поток выполнит цикл или "Spin" и попытается снова получить доступ к ресурсу, а не сразу же, чем с кратковременной блокировкой или другим ресурсом. ожидания. Поток будет продолжать вращаться до тех пор, пока ресурс не будет доступен, или завершится цикл, после чего поток выдаст планировщик и вернется в очередь готовности. Такой подход позволяет уменьшить чрезмерное переключение контекста потока, но если состязание происходит очень высоко, может наблюдаться значительная загрузка ЦП.
   
 В следующей таблице содержатся краткие описания некоторых наиболее распространенных типов взаимоблокировок.  
  
|Тип спин|Description|  
|-----------------|-----------------|  
...
|Идентификация|Только для внутреннего применения.|
