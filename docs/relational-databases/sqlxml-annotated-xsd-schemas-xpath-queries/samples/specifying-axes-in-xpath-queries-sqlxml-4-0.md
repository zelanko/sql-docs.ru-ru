...
```  
 ...
 В запросе `child` — это ось `Customer` , а `Order` — тесты узла (эти тесты узлов имеют значение true, если Customer и Order являются ** \<элементами>** узлах, поскольку ** \<элемент>** узлом является основным узлом для **дочерней** оси). Для каждого узла, соответствующего ** \<>клиента **, узлы, соответствующие ** \<заказам>** , добавляются к результату. В результирующем наборе возвращается только ** \<>Order** .  
