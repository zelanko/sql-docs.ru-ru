...
## <a name="required-namespaces-in-the-updategram"></a>Обязательные пространства имен в диаграмме обновления  
 Ключевые слова в диаграмме обновления, такие как  **\<синхронизации >** ,  **\<перед >** , и  **\<после >** , существуют в **urn: schemas-microsoft-com: XML-диаграмма обновления** пространства имен. Префикс пространства имен может быть произвольным. В этой документации **updg** префикса обозначает **диаграмма обновления** пространства имен.  
...
### <a name="explicit-mapping"></a>Явное сопоставление  
 ...
 Если диаграмма обновления выполняет сложное обновление (например, вставку записей в несколько таблиц на основе родительско дочернее отношение, которое указано в схеме сопоставления), необходимо явно задать схему сопоставления с помощью  **Схема сопоставления** атрибута которого выполняется диаграмма обновления.  
  
 Диаграмма обновления является шаблоном, поэтому заданный в ней путь для схемы сопоставления зависит от расположения файла шаблона (является относительным). Дополнительные сведения см. в разделе [определение схемы с заметками сопоставления в диаграмме обновления &#40;SQLXML 4.0&#41;](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/updategrams/specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md).  
