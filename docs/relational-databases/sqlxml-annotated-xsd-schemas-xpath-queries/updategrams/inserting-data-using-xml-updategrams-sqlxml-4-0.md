...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]Диаграмма обновления обозначает операцию вставки, если экземпляр записи появляется в  **\<после >** блок, но отсутствует в соответствующем  **\<перед >** блока. В этом случае диаграмма обновления вставляет запись в  **\<после >** блок в базу данных.  
...
```  
 ...
 Столбец UnitPrice в таблице Order Details имеет **money** типа. Чтобы применить соответствующее преобразование типов (из **строка** тип **money** типа), необходимо добавить как часть значения символ доллара ($). Если диаграмма обновления не задана схема сопоставления, первым символом **строка** значения вычисляются. Если первый символ — символ доллара ($), применяется соответствующее преобразование.  
...
### <a name="d-using-the-at-identity-attribute-to-retrieve-the-value-that-has-been-inserted-in-the-identity-type-column"></a>Г. Использование атрибута at-identity для получения значения, вставленного в столбец типа IDENTITY  
 ...
 Сначала диаграмма обновления добавляет запись в таблицу Sales.SalesOrderHeader. В этой таблице столбец SalesOrderID является столбцом типа IDENTITY. Таким образом, при добавлении этой записи в таблицу диаграмма обновления использует **на удостоверения** атрибут для захвата присвоенное столбцу SalesOrderID значение как «x» (значение заполнителя). Затем диаграмма обновления указывает это **на удостоверения** переменной в качестве значения атрибута SalesOrderID в \<Sales.SalesOrderDetail > элемент.  
...
### <a name="g-using-the-xsinil-attribute-to-insert-null-values-in-a-column"></a>Ж. Использование атрибута xsi:nil для вставки в столбец значений NULL  
 Если вы хотите вставить значение null в соответствующий столбец в таблице, можно указать **xsi: nil** атрибут элемента в диаграмме обновления. В соответствующей схеме XSD XSD **nillable** также должен быть задан атрибут.  
...
-   **Xml** столбец не может использоваться для идентификации существующей строки. Таким образом, он не может быть включен в **updg: перед** раздела диаграммы обновления.  
  
-   Пространства имен в пределах фрагмента XML, вставляемых в **xml** столбца будут сохранены, и декларации пространства имен добавляются на верхний элемент вставленного фрагмента.  
