...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]Здесь представлен рабочий образец usingOLE DB выполняет диаграмму обновления.  
...
2.  Получите ICommandStream, интерфейс на ICommand.  
...
```  
 ...
 Шаблон включает SQL-запрос. Запрос должен иметь значение для параметра (@Title). Если значение параметра не передано, используется значение по умолчанию («Mr.»).  
