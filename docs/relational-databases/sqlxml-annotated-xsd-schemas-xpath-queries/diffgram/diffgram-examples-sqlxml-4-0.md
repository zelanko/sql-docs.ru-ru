---
title: "Примеры дельт (SQLXML 4.0) | Документы Microsoft"
ms.custom: 
ms.date: 03/04/2017
...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]Примеры в этом разделе состоят из дельты, выполнение insert, update и delete операции базы данных. При использовании этих примеров необходимо учитывать следующие моменты.  
...
```  
 ...
 В  **\<перед >** блока, имеется  **\<порядок >** элемент (**diffgr: ID = «Order1»**) и  **\< Клиент >** элемент (**diffgr: ID = «Customer1»**). Данные элементы представляют существующие записи в базе данных. **\<DataInstance >** элемент не имеет соответствующих записей (с тем же **diffgr: ID**). Это указывает на операцию удаления.  
...
```  
 ...
 В данной Дельте  **\<перед >** блок не указывается (не существующие записи базы данных определены). Существуют два экземпляра записи (определяется  **\<клиента >** и  **\<порядок >** элементов в  **\<DataInstance >** блока), которые соответствуют таблицам Cust и Ord, соответственно. Оба эти элементы задают **diffgr:** атрибут (**hasChanges = «inserted»**). Это указывает на операцию вставки. В этом DiffGram, при указании **hasChanges = «modified»**, означает, что вы хотите изменить запись, которая не существует, что приводит к ошибке.  
 ...
 **\<Перед >** блок содержит  **\<клиента >** элемент (**diffgr: ID = «Customer1»**). **\<DataInstance >** блок содержит соответствующий  **\<клиента >** элемент с таким же **идентификатор**.  **\<Клиента >** элемент в  **\<NewDataSet >** также указывает **diffgr: HasChanges = «modified»**. Он означает операции обновления, а также запись клиента в **Cust** таблица обновляется соответствующим образом. Обратите внимание, что если **diffgr:** атрибут не указан, то логика обработки дельты пропустит этот элемент и обновления не выполняются.  
...
-   В соответствии с логикой обработки дельт все элементы верхнего уровня в  **\<перед >** блокировать карту, чтобы соответствующие таблицы, как описано в схеме сопоставления.  
  
-   **\<Перед >** блок содержит  **\<порядок >** элемент (**dffgr:id = «Order1»**) и  **\<клиента >** элемент (**diffgr: ID = «Customer1»**) для которого существует соответствующий элемент в  **\<DataInstance >** блоке (с одинаковым Идентификатором). Это указывает на операцию удаления, во время которой записи удаляются из таблиц Cust и Ord.  
  
-   **\<Перед >** блок содержит  **\<клиента >** элемент (**diffgr: ID = «Customer2»**) для которого существует соответствующий  **\<Клиента >** элемент в  **\<DataInstance >** блоке (с одинаковым Идентификатором). Элемент в  **\<DataInstance >** указывает блок **diffgr: HasChanges = «modified»**. Это операция обновления, в котором для пользователя anatr, CompanyName и ContactName данные обновляются в таблице «Cust», используя значения, заданные в  **\<DataInstance >** блока.  
  
-   **\<DataInstance >** блок содержит  **\<клиента >** элемент (**diffgr: ID = «Customer3»**) и  **\<Заказ >** элемент (**diffgr: ID = «Order3»**). Эти элементы не указана **diffgr:** атрибута. Таким образом, логика обработки дельт не учитывает эти элементы.  
  
-   **\<DataInstance >** блок содержит  **\<клиента >** элемент (**diffgr: ID = «Customer4»**) и  **\<Заказ >** элемент (**diffgr: ID = «Order4»**), для которых существует соответствующий элемент в \<перед > блока. Эти элементы в  **\<DataInstance >** указать блок **diffgr: HasChanges = «inserted»**. Таким образом, в таблицы Cust и Ord добавляется новая запись.  
