...
> [!NOTE]  
>  Чтобы задать этому свойству значение FALSE, необходимо иметь **ALTER TABLE** разрешений в целевых таблицах. Дополнительные сведения см. в разделе [ALTER TABLE (Transact-SQL)](../../../t-sql/statements/alter-table-transact-sql.md).  
...
> [!NOTE]  
>  Чтобы задать этому свойству значение TRUE, необходимо иметь **ALTER TABLE** разрешений в целевых таблицах. В противном случае ему должно быть присвоено значение FALSE. Дополнительные сведения см. в разделе [ALTER TABLE (Transact-SQL)](../../../t-sql/statements/alter-table-transact-sql.md).  
...
-   Возвращает данные переполнения для любого столбца, обозначенного аннотацией [SQL: Overflow-поле](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/bulk-load-xml/annotation-interpretation-sql-overflow-field.md) в [тип данных xml](../../../t-sql/xml/xml-transact-sql.md) формат.  
 ...
 SGUseID  
 Указывает, является ли атрибут в схеме сопоставления, служащее **идентификатор** тип можно использовать при создании ограничения PRIMARY KEY при создании таблицы. Это свойство используется в том случае, когда свойство SchemaGen имеет значение TRUE. Если SGUseID имеет значение TRUE, программа SchemaGen использует атрибут, для которого **DT: Type = «id»** указан как столбец первичного ключа и добавляет ограничение PRIMARY KEY при создании таблицы.  
