---
title: Примеры массовой загрузки XML (SQLXML 4.0) | Документация Майкрософт
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
...
## <a name="a-bulk-loading-xml-in-a-table"></a>A. Массовая загрузка XML-данных в таблицу  
 Этот пример устанавливает соединение с экземпляром [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] , указанный в свойстве ConnectionString (MyServer). В примере также указывается свойство ErrorLogFile. Поэтому ошибочный вывод сохраняется в указанном файле («C:\error.log»), для которого также можно указать другое место. Обратите внимание, что метод Execute имеющий в качестве параметров файл схемы сопоставления (SampleSchema.xml) и XML-файле данных (SampleXMLData.xml). При выполнении массовой загрузки в таблице Cust, созданном в **tempdb** база данных будет содержать новые записи, в зависимости от содержимого XML-файле данных.  
...
```  
 ...
 Этот пример Массовая загрузка XML-данные на две таблицы **Cust** и **CustOrder**:  
...
```  
 ...
 Массовая загрузка XML использует первичный ключ/связи по внешнему ключу между  **\<Cust >** и  **\<CustOrder >** элементов для массовой загрузки данных в обеих таблицах .  
...
## <a name="e-generating-table-schemas-before-bulk-loading"></a>Д. Формирование схем таблиц перед массовой загрузкой  
 При необходимости массовая загрузка XML может создать таблицы, если они не существовали до начала массовой загрузки. Задав свойство SchemaGen свойство sqlxmlbulkload значение true это. Можно также запросить Массовая загрузка XML, чтобы удалить все имеющиеся таблицы и создайте их повторно, установив свойство SGDropTables значение TRUE. Следующий пример на языке VBScript показывает использование этих свойств.  
...
## <a name="f-bulk-loading-from-a-stream"></a>Е. Массовая загрузка из потока  
 Метод Execute модели объектов массовой загрузки XML принимает два параметра. Первый параметр — это файл схемы сопоставления. Второй параметр представляет XML-данные, которые предстоит загрузить в базу данных. Существует два способа передачи XML-данных методу Execute массовой загрузки XML:  
...
```  
 ...
 Скрипт затем передает XML как поток метода Execute как второй параметр. Метод Execute массовой загружает данные в таблицу «cust».  
...
### <a name="opening-a-stream-on-an-existing-file"></a>Открытие потока для существующего файла  
 Можно также открыть поток на существующий файл данных XML и передает их поток как параметр методу Execute (вместо передачи имени файла в качестве параметра).  
...
## <a name="g-bulk-loading-in-overflow-columns"></a>Ж. Массовая загрузка в столбцы переполнения  
 Если схема сопоставления задает столбец переполнения с помощью **SQL: Overflow-поле** заметки, Массовая загрузка XML скопирует все невостребованные данные из исходного документа в этот столбец.  
...
```  
 ...
 Схема определяет столбец переполнения (OverflowColumn) для таблицы Cust. В результате всех неиспользуемых XML-данных для каждого  **\<клиента >** элемент добавляется к этому столбцу.  
  
> [!NOTE]  
>  Все абстрактные элементы (элементы, для которых **абстрактный = «true»** указан) и запрещенные атрибуты (атрибутах, для которых **prohibited = «true»** указывается) рассматриваются как переполнение путем массового XML Нагрузки на них добавляется в столбец переполнения, если указано. (в противном случае они пропускаются).  
...
## <a name="h-specifying-the-file-path-for-temp-files-in-transaction-mode"></a>З. Задание пути к файлу для временных файлов в режиме транзакции  
 При выполнении массовой загрузки в режиме транзакции (то есть когда свойство Transaction имеет значение TRUE), также необходимо задать свойство TempFilePath при выполнении одного из следующих условий:  
...
-   Для хранения временных файлов, созданных в режиме транзакции, необходимо использовать другой локальный диск или папку (отличный от пути, указанного переменной среды TEMP).  
 ...
 Например, следующий код на языке VBScript выполняет массовую загрузку данных из файла SampleXMLData.xml в таблицы базы данных в режиме транзакции. Свойство TempFilePath задается Настройка пути для временных файлов, которые создаются в режиме транзакции.  
...
> [!NOTE]  
>  Путь к временному файлу должен быть общей папкой, доступной из учетной записи службы целевого экземпляра [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] и учетной записи, от которой запущено приложение массовой загрузки. Если выполняется Массовая загрузка на локальном сервере, путь к временному файлу должен быть в формате UNC (например \\\servername\sharename).  
...
4.  В текстовом редакторе или редакторе XML по своему выбору создайте файл и сохраните его под именем ValidateAndBulkload.vbs. Добавьте следующий код VBScript в этот файл. Измените строку соединения, указав соответствующие имена сервера и базы данных. Укажите соответствующий путь для файлов, указанных в качестве параметров метода Execute. Также укажите соответствующий путь для свойства каталог временных файлов.  
...
5.  Выполните код VBScript.  
     ...
     Схема должна указывать соответствующий **SQL: DataType** для **CustomerID** атрибут значение **CustomerID** указывается как идентификатор GUID, который включает фигурные скобки ({} и}), такие как:  
    ...
    ```  
     ...
     Когда **SQL: DataType** указан, определяющий тип столбца как **uniqueidentifier**, операции массовой загрузки удаляет фигурные скобки ({и}) из **CustomerID** значение Перед вставкой в столбец.  
...
## <a name="i-using-an-existing-database-connection-with-the-connectioncommand-property"></a>И. Использование существующего подключения к базе данных со свойством ConnectionCommand  
 ...
 Свойством ConnectionCommand позволяет использовать существующее соединение ADO с помощью объектом команды ADO. Это продемонстрировано в следующем примере на языке Visual Basic.  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>К. Массовая загрузка XML-данных в столбцы типа данных xml  
 Если схема сопоставления задает [тип данных xml](../../../t-sql/xml/xml-transact-sql.md) столбца с помощью **SQL: DataType = «xml»** заметки, Массовая загрузка XML можно скопировать в этот дочерние элементы XML сопоставленного поля из исходного документа столбец.  
  
 Рассмотрим следующую схему XSD, которая сопоставляет представление таблицы Production.ProductModel в образце базы данных AdventureWorks. В этой таблице поле CatalogDescription **xml** сопоставляется с типом данных  **\<Desc >** элемента с помощью **SQL: field** и **sql: Тип данных = «xml»** заметок.  
