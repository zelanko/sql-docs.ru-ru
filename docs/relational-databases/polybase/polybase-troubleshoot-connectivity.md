...
Например, для SQL Server 2016 путем по умолчанию будет "C:\\Program Files\\Microsoft SQL Server\\MSSQL13.MSSQLSERVER\\MSSQL\\Binn\\PolyBase\\Hadoop\\conf".
...
В Active Directory вы можете просмотреть имена субъектов-служб, перейдя в панель управления > "Пользователи и компьютеры Active Directory" >  *MyRealm* > *MyOrganizationalUnit*. Если для кластера Hadoop корректно настроена защита Kerber, имена субъекта-службы должны иметься для каждой из множества служб, доступных на кластере (например, nn, dn, rm, yarn, spnego и др.).
