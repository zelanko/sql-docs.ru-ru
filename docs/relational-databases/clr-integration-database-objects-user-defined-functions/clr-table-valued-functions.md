...
## <a name="implementing-table-valued-functions"></a>Реализация функций с табличным значением  
 Функции с табличным значением реализуются в виде методов класса в сборке [!INCLUDE[msCoName](../../includes/msconame-md.md)] .NET Framework. Код функции, возвращающей табличное значение, должен реализовывать интерфейс **IEnumerable** . Интерфейс **IEnumerable** определяется в .NET Framework. Типы, представляющие массивы и коллекции в .NET Framework уже реализуют интерфейс **IEnumerable** . Это облегчает написание функций с табличным значением, преобразующих коллекцию или массив в результирующий набор.  
...
### <a name="defining-a-table-valued-function-in-transact-sql"></a>Определение функции с табличным значением на языке Transact-SQL  
 Синтаксис для определения возвращающей табличное значение функции в среде CLR подобен [!INCLUDE[tsql](../../includes/tsql-md.md)] функции, возвращающей табличное значение, с добавлением предложения **внешнего имени** . Пример:  
 ...
 **Примечание** . Возвращающая табличное значение функция может выполнять доступ к данным [!INCLUDE[tsql](../../includes/tsql-md.md)] только через запрос в методе **инитмесод** , а не в методе **FillRow** . Если [!INCLUDE[tsql](../../includes/tsql-md.md)] запрос выполняется, **инитмесод** должен быть помечен свойством **SqlFunction. DataReader. Read** атрибута.  
