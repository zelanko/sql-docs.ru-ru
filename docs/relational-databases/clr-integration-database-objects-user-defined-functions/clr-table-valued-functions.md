...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]Табличная функция является определяемой пользователем функции, возвращающей таблицу.  
...
### <a name="defining-a-table-valued-function-in-transact-sql"></a>Определение функции с табличным значением на языке Transact-SQL  
 Синтаксис для определения табличной функции CLR аналогичен [!INCLUDE[tsql](../../includes/tsql-md.md)] функции, возвращающие табличные значения, с добавлением **ВНЕШНЕЕ имя** предложения. Пример:  
