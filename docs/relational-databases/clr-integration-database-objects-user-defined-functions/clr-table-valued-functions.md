...
## <a name="differences-between-transact-sql-and-clr-table-valued-functions"></a>Различия между функциями Transact-SQL и среды CLR с табличным значением  
 [!INCLUDE[tsql](../../includes/tsql-md.md)] возвращающие табличные значения функции материализуют результаты вызова функции в промежуточной таблице. По этой причине они поддерживают в результатах ограничения и уникальные индексы. Это исключительно полезно при возвращении результатов большого объема.  
