---
title: Функции CLR Таблица-Ценные функции (ru) Документы Майкрософт
description: Функция, ценная на таблицу, возвращает таблицу. В интеграции S'L Server CLR можно записывать функции, ценные для таблицы, в управляемом коде.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
 ...
 Начиная с [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)], [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] расширяет возможности возвращающих табличное значение функций, позволяя определить функцию с табличным значением на любом управляемом языке. Данные возвращаются из функции, ценимой таблицей, через объект **IEnumerable** или **IEnumeror.**  
  
> [!NOTE]  
>  Для функций, ценных на таблицу таблицы возврата, столбцы таблицы возврата не могут включать столбцы меток времени или столбцы типа данных строки не-Unicode (например, **символ,** **varchar**и **текст).** Ограничение NOT NULL не поддерживается.  
  
 Для получения дополнительной информации о функциях CLR Table-Valued, ознакомьтесь с [функциями](https://www.mssqltips.com/sqlservertip/2582/introduction-to-sql-server-clr-table-valued-functions/) MSS'LTips'.  
...
### <a name="defining-a-table-valued-function-in-transact-sql"></a>Определение функции с табличным значением на языке Transact-SQL  
 Синтаксис для определения функции, ценной на таблицу CLR, аналогичен функции, ценной на [!INCLUDE[tsql](../../includes/tsql-md.md)] стол, с добавлением оговорки EXTERNAL **NAME.** Пример:  
 ...
 **Заметка** Функция, оцениваемая таблицей, может выполнять доступ к данным только через [!INCLUDE[tsql](../../includes/tsql-md.md)] запрос в методе **InitMethod,** а не в **методе FillRow.** **InitMethod** должен быть помечен свойством **атрибута SqlFunction.DataAccess.Read,** если [!INCLUDE[tsql](../../includes/tsql-md.md)] выполняется запрос.  
