 ...
 **Примечание** . Возвращающая табличное значение функция может выполнять доступ к данным только через [!INCLUDE[tsql](../../includes/tsql-md.md)] запрос в методе **инитмесод** , а не в методе **FillRow** . Если запрос выполняется, **инитмесод** должен быть помечен свойством **SqlFunction. DataReader. Read** атрибута. [!INCLUDE[tsql](../../includes/tsql-md.md)]  
