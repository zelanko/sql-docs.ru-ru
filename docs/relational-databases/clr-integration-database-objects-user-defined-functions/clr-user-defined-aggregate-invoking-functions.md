---
title: Ссылаясь на функции агрегированных агрегированных функций CLR, определяемых пользователем (ru) Документы Майкрософт
description: В интеграции S'L Server CLR используйте Transact-S'L SELECT для того, чтобы вызывать агрегированные данные, определяемые пользователями CLR, в соответствии с правилами, применимыми к агрегированным функциям системы.
ms.custom: ''
ms.date: 01/15/2019
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "81488416"
---
# <a name="clr-user-defined-aggregate---invoking-functions"></a>Вызов определяемых пользователем агрегатных функций среды CLR
...
-   Текущий пользователь должен иметь разрешение **EXECUTE** на пользовательский агрегат.  
  
-   Определенные пользователями агрегаты должны вызываться с использованием имени из двух частей в виде *schema_name.udagg_name.*  
  
-   Тип аргумента, определяемый пользователем, должен соответствовать или быть косвенно конвертируемым *input_type* агрегата, как это определено в заявлении **CREATE AGGREGATE.**  
  
-   Тип возврата определенного пользователя агрегатдолжен должен соответствовать *return_type* в выписке **CREATE AGGREGATE.**  
...
```  
 ...
 После компиляции кода в **MyAgg.dll**вы [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] можете зарегистрировать агрегат следующим образом:  
...
>  Управляемые объекты базы данных Visual C++, такие как скалярные функции, скомпилированные с помощью параметра компиляции /clr:pure, не поддерживаются для выполнения в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
 ...
 Как и в большинстве агрегатов, основная часть логики находится в методе **накопления.** В данном месте строка, передаваемые в качестве параметра к методу **Аккотат,** присваивается к объекту **StringBuilder,** который был инициализирован в методе **Init.** Если предположить, что это не первый раз, когда метод **Аккотати** называется, запятая также прилагается к **StringBuilder** до приложения пройденного в строке. В конце вычислительных задач называется метод **«Прекращение»,** который возвращает **StringBuilder** в строку.  
...
## <a name="example-2"></a>Пример 2  
 В следующем примере показан агрегат, который имеет два параметра на методе **накопления.**  
