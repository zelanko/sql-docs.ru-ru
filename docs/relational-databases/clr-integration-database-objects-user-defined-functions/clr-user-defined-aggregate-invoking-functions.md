...
description: В SQL Server интеграции со средой CLR используйте Transact-SQL SELECT для вызова определяемых пользователем статистических функций CLR в соответствии с правилами, применяемыми к системным агрегатным функциям.
ms.custom: ''
ms.date: 01/15/2019
ms.prod: sql
ms.reviewer: ''
...
-   Тип аргумента определяемой пользователем статистической функции должен совпадать или быть неявно преобразуемым в *input_type* агрегата, как определено в инструкции **CREATE AGGREGATE** .  
  
-   Возвращаемый тип определяемой пользователем статистической функции должен соответствовать *return_type* в инструкции **CREATE AGGREGATE** .  
...
>  Управляемые объекты базы данных Visual C++, такие как скалярные функции, скомпилированные с помощью параметра компиляции /clr:pure, не поддерживаются для выполнения в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
 ...
 Как и в случае с большинством статистических выражений, основная часть логики находится в методе **accumulate** . Здесь строка, передаваемая в качестве параметра в метод **accumulate** , добавляется к объекту **StringBuilder** , инициализированному в методе **init** . Предполагая, что это не первый раз, когда вызывается метод **accumulate** , перед добавлением переданной строки также добавляется запятая к **StringBuilder** . После завершения вычислительных задач вызывается метод **Terminate** , который возвращает **StringBuilder** в виде строки.  
...
## <a name="example-2"></a>Пример 2  
 В следующем примере показано статистическое выражение с двумя параметрами метода **accumulate** .  
