...
|**Завершение**|`public return_type Terminate();`|Этот метод завершает статистическое вычисление и возвращает результат статистической обработки. *Тип_возврата* должен быть управляемым [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] данных тип, управляемый эквивалент *return_sqltype* указано в **CREATE AGGREGATE** инструкции. *Return_type* также может быть определяемого пользователем типа.|  
...
|---------------------|  
|Обновлено описание **Accumulate** метод, который теперь принимает более одного параметра.|  
