...
description: Агент слияния репликации применяет исходный моментальный снимок, хранящийся в таблицах базы данных, к подписчикам, объединяет добавочные изменения данных и согласовывает конфликты.
ms.custom: ''
ms.date: 10/29/2018
ms.prod: sql
 ...
 **-EncryptionLevel** [ **0** \| **1** \| **2** ]  
 Уровень шифрования по протоколу TLS (ранее — SSL), который используется агентом слияния при установлении соединений.  
 ...
 **-ParallelUploadDownload** [**0**\|**1**]  
 Указывает, должен ли агент слияния параллельно обрабатывать изменения, передаваемые в издатель, и изменения, скачиваемые в подписчик. Это может оказаться удобным в средах с большими объемами данных и высокой пропускной способностью сети. Однако обратите внимание на приведенное ниже предупреждение для случаев, когда параметр **ParallelUploadDownload** имеет значение **1**.
 
В ближайшем будущем этот параметр может быть упразднен. Мы рекомендуем задавать для параметра **ParallelUploadDownload** значение 0 и не использовать профиль агента слияния "межсерверный большого объема", так как параметр **ParallelUploadDownload** в нем имеет значение 1.
