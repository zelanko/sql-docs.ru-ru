---
title: "Агент чтения очереди репликации | Microsoft Docs"
ms.custom: ""
ms.date: "06/02/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "агенты [репликация SQL Server], агент чтения очереди"
  - "командная строка [репликация SQL Server]"
  - "агент чтения очереди, ссылка на параметр"
  - "агент чтения очереди, исполняемые файлы"
 ...
 **-ProfileName** *имя_профиля_агента*  
 Имя профиля агента, используемого для предоставления агенту набора значений по умолчанию. Сведения см. в разделе [профилей агентов репликации](../../../relational-databases/replication/agents/replication-agent-profiles.md).  
  
 **-QueryTimeOut** *query_time_out_seconds*  
 Время ожидания запроса в секундах. Значение по умолчанию — 1800 секунд.  
  
 **-ResolverState** [ **1**| **2**| **3**]  
 Определяет, каким образом разрешаются конфликты обновления посредством очередей. Значение **1** указывает, что конфликт разрешается в пользу издателя, то есть на издателе и на подписчике будет произведен откат участвующей в конфликте текущей транзакции из очереди, а обработка последующих транзакций из очереди будет продолжена. Значение **2** указывает, что конфликт разрешается в пользу подписчика, то есть транзакция, находящаяся в очереди, переопределяет значения на издателе. Значение **3** означает, что любой конфликт приведет к повторной инициализации подписчиков; разрешение конфликта в пользу издателя, завершается обработка последующих транзакций из очереди и подписка повторно инициализирована. Для публикаций транзакций значение по умолчанию равно **1** , а для публикаций моментальных снимков — **3** .  
