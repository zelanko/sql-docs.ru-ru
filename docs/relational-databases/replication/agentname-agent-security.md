...
-   Для принудительных подписок локальным соединением является соединение с издателем и распространителем, поэтому это поле будет отображаться всегда: **Impersonate '\<домен>\\<имя_для_входа\>'** или **Impersonate '\<компьютер>\\<имя_для_входа\>'** для принудительных подписок.  
