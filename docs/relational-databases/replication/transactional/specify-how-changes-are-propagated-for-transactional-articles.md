---
title: "Указание способа распространения изменений для статей транзакций | Microsoft Docs"
ms.custom: ""
ms.date: "03/23/2017"
...
-   Укажите, что данное действие не должно выполняться на любом подписчике. Транзакции этого типа не реплицируются.  
 ...
 Чтобы настроить метод распространения изменений данных в статьях транзакций, см. раздел [Set the Propagation Method for Data Changes to Transactional Articles](../../../relational-databases/replication/publish/set-the-propagation-method-for-data-changes-to-transactional-articles.md).  
...
-   **sp_MSdel_ \<** *tablename* **>**, которая обрабатывает удаления.  
  
  **\<***Tablename***>** используется в процедуре зависит от того, как статья была добавлена в публикацию и содержит ли база данных подписки таблицу другого владельца с тем же именем.  
...
-   Схема в подписчике, как правило, идентична схеме в издателе, однако при использовании фильтрации столбцов схема в подписчике может быть подмножеством схемы издателя. Однако при необходимости преобразования схемы при перемещении данных так, чтобы схема на подписчике не являлась подмножеством схемы на издателе, рекомендованным решением являются службы [!INCLUDE[ssISCurrent](../../../includes/ssiscurrent-md.md)] . Дополнительные сведения см. в разделе [SQL Server Integration Services](../../../integration-services/sql-server-integration-services.md).  
  
-   При внесении изменений схемы в опубликованную таблицу пользовательские процедуры должны быть созданы заново. Дополнительные сведения см. в разделе [повторное создание пользовательских процедур транзакций для отражения изменений схемы](../../../relational-databases/replication/transactional/regenerate-custom-transactional-procedures-to-reflect-schema-changes.md).  
  
-   При использовании значений больше 1 для **- SubscriptionStreams** параметр агента распространителя, необходимо убедиться, что обновления первичных ключевых столбцов выполнены успешно. Например:  
...
#### Синтаксис функции SCALL  
 Хранимые процедуры UPDATE  
 Хранимым процедурам, обрабатывающим инструкции UPDATE передаются обновленные значения только для тех столбцов, которые изменен, за ними передаются исходные значения для столбцов первичного ключа, а затем Битовая маска (**binary(n)**) параметр, указывающий на измененные столбцы. В следующем примере столбец 2 (c2) не был изменен:  
...
#### Синтаксис функции MCALL  
 Хранимые процедуры UPDATE  
 Хранимым процедурам, обрабатывающим инструкции UPDATE передаются обновленные значения для всех столбцов, определенных в статье, а также исходные значения для столбцов первичного ключа, следуют битовой маски (**binary(n)**) параметр, указывающий на измененные столбцы:  
...
> [!NOTE]  
>  При использовании функции XCALL, до значения изображения для **текста** и **изображение** столбцы должны иметь значение NULL.  
