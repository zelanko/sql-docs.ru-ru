---
title: "Репликация столбцов идентификаторов | Microsoft Docs"
ms.custom: ""
ms.date: "10/04/2016"
...
-   Нет. Этот режим рекомендуется только для обеспечения обратной совместимости с предыдущими версиями [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] и доступен только из интерфейса хранимой процедуры для публикаций транзакций.  
 ...
 Указание режима управления диапазонами идентификаторов, в разделе [управлять столбцами идентификаторов](../../../relational-databases/replication/publish/manage-identity-columns.md).  
...
## Назначение диапазонов идентификаторов  
 ...
 Если издатель исчерпывает свой диапазон идентификаторов после вставки, он может автоматически назначить новый диапазон, если вставка была выполнена членом **db_owner** фиксированной роли базы данных. Если вставка была выполнена пользователем не в этой роли, агент чтения журнала, агент слияния или пользователь, являющийся членом из **db_owner** роли необходимо запустить [sp_adjustpublisheridentityrange &#40; Transact-SQL &#41;](../../../relational-databases/system-stored-procedures/sp-adjustpublisheridentityrange-transact-sql.md). Чтобы назначение нового диапазона для публикаций транзакций происходило автоматически, должен выполняться агент чтения журнала (по умолчанию агент функционирует в непрерывном режиме).  
...
### Репликация слиянием  
 Диапазоны идентификаторов управляются издателем и распространяются на подписчики агентом слияния (в иерархии переиздания диапазоны управляются корневым издателем и переиздающими подписчиками). Значения идентификаторов назначаются из пула на издателе. При добавлении статьи со столбцом идентификаторов в публикацию в мастере создания публикаций или при помощи [sp_addmergearticle &#40; Transact-SQL &#41;](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md), при указании значений:  
  
-   **@Identity_range** параметр, который управляет размером диапазона идентификаторов, первоначально назначаемого издателю и подписчикам с клиентскими подписками.  
  
    > [!NOTE]  
    >  Для подписчиков, использующих предыдущие версии [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], этот параметр (вместо **@pub_identity_range** параметр) также управляет размером диапазона идентификаторов на переиздающих подписчиках.  
  
-    **@Pub_identity_range** параметр, который управляет размером диапазона идентификаторов для переиздания, назначается подписчикам с серверными подписками (необходим для переиздания данных). Все подписчики с серверными подписками получают диапазон для переиздания, даже если они реально не переиздают данные.  
...
## См. также:  
 ...
 [Инструкция DBCC CHECKIDENT &#40; Transact-SQL и #41;](../../../t-sql/database-console-commands/dbcc-checkident-transact-sql.md)   
 [Функция IDENT_CURRENT &#40; Transact-SQL и #41;](../../../t-sql/functions/ident-current-transact-sql.md)   
 [IDENTITY (свойство) (Transact-SQL)](../Topic/IDENTITY%20\(Property\)%20\(Transact-SQL\).md)   
 [sp_adjustpublisheridentityrange &#40; Transact-SQL и #41;](../../../relational-databases/system-stored-procedures/sp-adjustpublisheridentityrange-transact-sql.md)  
