---
title: "Настройка сервера IIS 7 для веб-синхронизации | Microsoft Docs"
ms.custom: ""
ms.date: "09/12/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Настройка сервера IIS 7 [репликация SQL Server]"
  - "Веб-синхронизация, серверы IIS 7"
...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  Процедуры, описанные в этом разделе проведет вас через процесс настройки вручную [!INCLUDE[msCoName](../../includes/msconame-md.md)] Internet Information Services (IIS) версии 7 и выше для использования с веб-синхронизации для репликации слиянием. 
  
 Настройка IIS 7 или более поздней версии — первый из трех шагов, необходимых для включения веб-синхронизации.  
  
 Общие сведения о процессе настройки в разделе [настройки веб-синхронизации](../../relational-databases/replication/configure-web-synchronization.md).  
...
1.  Установка репликации SQL Server на компьютере IIS.

2. Создайте на компьютере, где запущены службы IIS, новый каталог для файла replisapi.dll. Можно создать каталог везде, где это требуется, но рекомендуется создать каталог в разделе \<*диск*>: \Inetpub directory. Например, создайте каталог \<*диск*>: \Inetpub\SQLReplication\\.  
  
3.  Скопируйте файл replisapi.dll из каталога [!INCLUDE[ssInstallPathVar](../../includes/ssinstallpathvar-md.md)]com\ в каталог файл, созданный на шаге 1.  
...
6.  Создайте виртуальный каталог в службах IIS. Виртуальный каталог должен быть создан на сайте, созданном в шаге 4, и сопоставлен с каталогом, созданным в шаге 1. Разрешения для этого каталога должны быть максимально ограничены. Необходимо выбрать как минимум разрешения **Чтение** и **Выполнение** .  
    ...
    1.  В **диспетчера Internet Information Services (IIS)**, в **подключений** панели, щелкните правой кнопкой мыши **веб-узел по умолчанию**, и выберите **Добавить виртуальный каталог**.  
  
    2.  В поле **Псевдоним**введите **SQLReplication**.  
  
    3.  Для **физический путь**, введите **\< диск>: \Inetpub\SQLReplication\\**, а затем нажмите кнопку **ОК**.  
  
7.  Установите в настройках служб IIS разрешение выполнять файл replisapi.dll.  
  
    1.  В **диспетчера Internet Information Services (IIS)**, нажмите кнопку **веб-узел по умолчанию**.  
    ...
    5.  Из **модуль** раскрывающемся списке выберите **IsapiModule**.  
  
    6.  Для **исполняемый файл**, введите **\< диск>: \Inetpub\SQLReplication\replisapi.dll**.  
    ...
    10. Убедитесь, что файл Replisapi.dll включен в список **Включены** сопоставлений обработчика. Если он находится в **отключенных** списка, щелкните правой кнопкой мыши запись Replisapi и выберите **Изменить разрешения функции**. Установите флажок **Выполнить** , а затем нажмите кнопку **ОК**.  
...
2.  В средней области дважды щелкните **проверки подлинности**.  
...
1.  В **диспетчера Internet Information Services (IIS)**, разверните узел локального сервера и нажмите кнопку **веб-узел по умолчанию** (или на веб-сайт синхронизации, если оно отличается от веб-сайта по умолчанию).  
  
2.  В средней области дважды щелкните **Параметры SSL**.  
...
1.  В **диспетчера Internet Information Services (IIS)**, щелкните узел локального сервера и затем в центральной области дважды щелкните **сертификаты сервера**.  
  
2.  В **действия** панели, щелкните **Создание самозаверяющего сертификата**.  
  
3.  В **Создание самозаверяющего сертификата** диалоговое окно «» введите имя для сертификата и нажмите кнопку **ОК**.  
  
###### Привязка сертификата к веб-сайту  
  
1.  В **подключений** панели, щелкните **веб-узел по умолчанию** (или на веб-сайт синхронизации, если оно отличается от веб-сайта по умолчанию).  
  
2.  На панели **Действия** выберите **Привязки**и щелкните **Добавить**. Откроется диалоговое окно **Добавить привязку сайта** .  
  
3.  От **типа** раскрывающемся списке выберите **https**. Оставьте без изменений настройки для полей **IP-адрес** и **Порт**.  
  
4.  Из **SSL-сертификат** нажмите кнопку раскрывающегося списка, выберите сертификат, созданный в «Создание самозаверяющего сертификата для тестирования» **ОК**, и нажмите кнопку **Закрыть**.  
  
###### Тестирование сертификата  
  
1.  В **диспетчера Internet Information Services (IIS)**, нажмите кнопку **веб-узел по умолчанию.**  
  
2.  Из **действия** панели, щелкните **Обзор \*: 443(https)**.  
...
1.  На компьютере, на котором запущены службы IIS, создайте локальную учетную запись:  
    ...
    1.  Откройте **Диспетчер сервера**. Из меню «Пуск», щелкните правой кнопкой мыши **компьютера**, а затем нажмите кнопку **Управление**.  
...
2.  Добавьте учетную запись в группу IIS_IUSRS:  
    ...
    2.  Щелкните правой кнопкой мыши **IIS_IUSRS**, а затем нажмите кнопку **Добавить в группу**.  
  
    3.  В **Свойства IIS_IUSRS** диалоговом нажмите кнопку **Добавить**.  
  
    4.  В диалоговом окне **Выбор: Пользователи, Компьютеры или Группы** добавьте учетную запись, созданную на шаге 1.  
  
    5.  Убедитесь, что **из этого расположения** отображает имя локального компьютера (а не в домен). Если в этом поле не отображается имя локального компьютера, нажмите кнопку **Расположение...**. В диалоговом окне **Расположение...** выберите локальный компьютер, а затем нажмите кнопку **ОК**.  
  
    6.  В **Выбор: пользователи** диалоговое окно и **Свойства IIS_IUSRS** диалоговом нажмите**OK**.  
  
3.  Предоставьте учетной записи минимальные разрешения для доступа к папке, содержащей библиотеку replisapi.dll:  
  
    1.  В проводнике Windows щелкните правой кнопкой мыши папку, созданную для файла replisapi.dll и нажмите кнопку **Свойства**.  
  
    2.  На вкладке **Безопасность** нажмите **Изменить**.  
  
    3.  В **разрешения для \< имя_папки>** диалоговом нажмите кнопку **Добавить** для добавления учетной записи, созданной на шаге 1.  
  
    4.  Убедитесь, что **из этого расположения** отображает имя локального компьютера (а не в домен). Если в этом поле не отображается имя локального компьютера, нажмите кнопку **Расположение...**. В диалоговом окне **Расположение...** выберите локальный компьютер, а затем нажмите кнопку **ОК**.  
...
4.  Создайте пул приложений в **диспетчера Internet Information Services (IIS)**:  
    ...
    1.  В **диспетчера Internet Information Services (IIS)**, в **подключений** область, разверните узел локального сервера.  
  
    2.  Щелкните правой кнопкой мыши **Пулы приложений**, а затем нажмите кнопку **Добавить пул приложений**.  
  
    3.  Введите имя пула приложений, оставьте без изменений значения по умолчанию в остальных полях и нажмите кнопку **ОК**.  
  
    > [!NOTE]  
    >  Если предполагается использовать более двух параллельных клиентов синхронизации, рекомендуется создать веб-сад. Дополнительные сведения см. в разделе «Создание веб-сада» в [настройки веб-синхронизации](../../relational-databases/replication/configure-web-synchronization.md).  
  
5.  Свяжите учетную запись с этим пулом приложений:  
  
    1.  В **диспетчера Internet Information Services (IIS)**, разверните узел локального сервера и выберите команду **Пулы приложений**.  
  
    2.  Щелкните правой кнопкой мыши пул приложений, который был создан и нажмите кнопку **значение по умолчанию пула приложений**.  
...
6.  Свяжите пул приложений с веб-сайтом репликации:  
    ...
    1.  В **диспетчера Internet Information Services (IIS)**, разверните узел локального сервера и выберите команду **веб-узел по умолчанию** (или на веб-сайт синхронизации, если оно отличается от веб-сайта по умолчанию).  
    ...
    4.  С **пул приложений** раскрывающемся списке выберите пул приложений, созданный на шаге 4 и нажмите кнопку **ОК**.  
...
2.  В обозревателе Internet Explorer на подписчике подключитесь к серверу в диагностическом режиме, добавив параметр `?diag` к адресу replisapi.dll. Например: **https://server.domain.com/directory/replisapi.dll?diag**.  
    ...
    > [!NOTE]  
    >  В приведенном выше примере **server.domain.com** необходимо заменить точным **Кому выдан** имя в списке **сертификаты сервера** в диспетчере служб IIS.  
