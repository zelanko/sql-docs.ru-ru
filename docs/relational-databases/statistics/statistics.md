...
ms.topic: "article"
helpviewer_keywords: 
  - "статистическая информация [SQL Server], оптимизация запросов"
  - "производительность запросов [SQL Server], статистика"
  - "статистика для оптимизации запросов [SQL Server]"
  - "статистическая информация [SQL Server], параметры базы данных"
  - "статистика оптимизации запросов [SQL Server], о статистике оптимизации запросов"
  - "статистическая информация [SQL Server], рекомендации"
  - "статистические сведения [SQL Server]"
  - "использование статистики [SQL Server]"
  - "статистическая информация [SQL Server], индексы"
  - "статистика индекса [SQL Server]"
  - "оптимизатор запросов [SQL Server], статистика"
...
-   SQL Server (2014 и более ранних версий) использует пороговое значение в зависимости от процента измененных строк. Это значение не зависит от числа строк в таблице.  
  
-   SQL Server (начиная с версии 2016 года и при уровне совместимости 130) использует пороговое значение, которое изменяется в зависимости от числа строк в таблице. Благодаря этому изменению статистика для больших таблиц будет обновляться чаще.  
...
### Улучшение оценки количества элементов с помощью указаний запросов  
 Чтобы улучшить оценку количества элементов для локальных переменных, можно использовать указания запросов OPTIMIZE FOR и OPTIMIZE FOR UNKNOWN с параметром RECOMPILE. Дополнительные сведения см. в разделе [Указания запросов (Transact-SQL)](../Topic/Query%20Hints%20\(Transact-SQL\).md).  
