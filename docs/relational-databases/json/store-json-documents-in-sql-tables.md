...
Возможность использования *любой* функции и предложения запроса T-SQL для запросов к документам JSON является существенным преимуществом. SQL Server и база данных SQL не вводят никаких ограничений к запросам, используемым для анализа документов JSON. Вы можете просто извлекать из документа JSON значения с помощью функции `JSON_VALUE` и использовать их в запросе, как любые другие значения.

В этом состоит ключевое отличие SQL Server и базы данных SQL от классических баз данных NoSQL: в Transact-SQL вам скорее всего доступны все необходимые функции для обработки данных JSON.
