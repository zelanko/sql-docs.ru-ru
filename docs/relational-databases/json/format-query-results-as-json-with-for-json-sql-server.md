---
title: "Форматирование результатов запроса как JSON с помощью предложения FOR JSON (SQL Server) | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "01/31/2017"
...
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx_md](../../includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]
  ...
  Вы можете экспортировать данные из SQL Server в формате JSON или отформатировать результаты запроса как JSON, добавив предложение **FOR JSON** к инструкции **SELECT** .  
...
-   **Режим PATH используется с предложением FOR JSON.** При использовании режима **PATH** с предложением **FOR JSON** вы сохраняете полный контроль над форматом выходных данных JSON. Вы можете создавать объекты-оболочки и вкладывать сложные свойства друг в друга.  
  
-   **Режим AUTO используется с предложением FOR JSON.** При использовании режима **AUTO** с предложением **FOR JSON** выходные данные JSON форматируются автоматически на основе структуры инструкции SELECT.  
  
 Предложение **FOR JSON** позволяет делегировать форматирование выходных данных JSON из клиентских приложений в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Дополнительные сведения см. в разделе [Использование выходных данных FOR JSON в SQL Server и клиентских приложениях (SQL Server)](../../relational-databases/json/use-for-json-output-in-sql-server-and-in-client-apps-sql-server.md).  
  
 ![FOR JSON](../../relational-databases/json/media/jsonslides2forjson.png "FOR JSON")  
  
## Режим PATH используется с предложением FOR JSON  
 Ниже приведен пример использования режима **PATH** с предложением **FOR JSON** .

В режиме **PATH** можно использовать синтаксис с точкой — например, `'Item.Price'` — для форматирования вложенных выходных данных. В этом примере также используется параметр **ROOT** для указания именованного корневого элемента.  
...
-   Сведения о синтаксисе и использовании см. в разделе [Предложение FOR (Transact-SQL)](../Topic/FOR%20Clause%20\(Transact-SQL\).md).  
...
## Режим AUTO используется с предложением FOR JSON  
...
## Управление выходными данными JSON с помощью параметров с предложением FOR JSON  
 Управлять выходными данными предложения **FOR JSON** можно с помощью следующих параметров.  
  
 [Добавление корневого узла в выходные данные JSON с параметром ROOT (SQL Server)](../../relational-databases/json/add-a-root-node-to-json-output-with-the-root-option-sql-server.md)  
 Чтобы добавить один элемент верхнего уровня в выходные данные JSON, укажите параметр **ROOT**. Если не указать параметр **ROOT** , выходные данные JSON не будут содержать корневой элемент.  
  
 [Использование параметра INCLUDE_NULL_VALUES для включения значений Null в выходные данные JSON (SQL Server)](../../relational-databases/json/include-null-values-in-json-include-null-values-option.md)  
 Чтобы включить значения NULL в выходные данные JSON, укажите параметр **INCLUDE_NULL_VALUES**. Если не указать этот параметр, в выходные данные не будут включены свойства JSON для значений NULL в результатах запроса.  
  
 [Удаление квадратных скобок из выходных данных JSON с помощью параметра WITHOUT_ARRAY_WRAPPER (SQL Server)](../../relational-databases/json/remove square brackets from json - without_array_wrapper option.md)  
 Чтобы удалить квадратные скобки, в которые выходные данные JSON предложения **FOR JSON** заключаются по умолчанию, укажите параметр **WITHOUT_ARRAY_WRAPPER**. Этот параметр позволяет создать единый объект JSON в качестве выходных данных. Если не указать этот параметр, выходные данные JSON будут заключены в квадратные скобки.  
  
 Дополнительные сведения о том, какое содержимое отображается в выходных данных предложения **FOR JSON** , см. в следующих разделах этой статьи.  
...
3.  Как имена столбцов, так и их значения экранируются согласно синтаксису JSON.  
...
## Дополнительные сведения о предложении FOR JSON и встроенной поддержке JSON в SQL Server  
