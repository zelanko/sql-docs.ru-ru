...
-   Предикаты BLOCK явно блокируют операции записи (AFTER INSERT, AFTER UPDATE, BEFORE UPDATE, BEFORE DELETE), которые нарушают предикат.  
 ...
 Предикаты фильтров применяются при считывании данных из базовой таблицы и влияют на все операции Get: **SELECT**, **DELETE** (пользователь не может удалять отфильтрованные строки) и **UPDATE** (пользователь не может изменять отфильтрованные строки, но может изменять строки так, чтобы отфильтровать их впоследствии). Предикаты блокировки влияют на все операции записи.  
 ...
 **Злоумышленник — администратор политик безопасности.** Важно учитывать, что администратор, имеющий разрешения на создание политики безопасности для конфиденциальных столбцов и на создание или изменение встроенных функций с табличным значением, может вступить в тайный сговор с другим пользователем, имеющим в таблице разрешения SELECT, и с его помощью извлекать данные, создавая встроенные функции с табличным значением, которые будут использовать атаки по сторонним каналам с целью вывода данных. Такие атаки потребуют сговора (или предоставления избыточных разрешений злоумышленнику) и, скорее всего, нескольких итераций изменения политики (требующих разрешения на удаление предиката, чтобы разорвать привязку схем), а также изменения встроенных функций с табличным значением и многократного выполнения инструкций SELECT в целевой таблице. Рекомендуем ограничить разрешения только необходимыми и отслеживать все подозрительные действия, в частности, постоянное изменение политик и встроенных функций с табличным значением, затрагивающее безопасность на уровне строк.  
