...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-pdw_md](../../../includes/tsql-appliesto-ss2008-xxxx-xxxx-pdw-md.md)]
 ...
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] предоставляет девять предопределенных ролей сервера. Разрешения, которые даются членам предопределенных ролей сервера (за исключением **открытый**) не может быть изменено. Начиная с [!INCLUDE[ssSQL11](../../../includes/sssql11-md.md)], можно создавать пользовательские роли сервера и добавлять разрешения на уровне сервера таким пользовательским ролям.  
...
|------------------------------|-----------------|  
|**sysadmin**|Члены **sysadmin** предопределенной роли сервера могут выполнять любые действия на сервере.|  
...
|**securityadmin**|Элементы предопределенной роли сервера **securityadmin** управляют именами входа и их свойствами. Они могут `GRANT`, `DENY`, и `REVOKE` разрешения на уровне сервера. Они также могут `GRANT`, `DENY`, и `REVOKE` уровня базы данных разрешения при наличии доступа к базе данных. Кроме того, они могут сбрасывать пароли для имен входа [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] .<br /><br /> **Важно:** возможность предоставлять доступ к [!INCLUDE[ssDE](../../../includes/ssde-md.md)] и настройки разрешений пользователей позволяют администратору безопасности назначать большинство разрешений сервера. Роль **securityadmin** должна считаться эквивалентной роли **sysadmin** .|  
|**processadmin**|Члены **processadmin** предопределенной роли сервера могут завершать процессы, запущенные в экземпляре [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].|  
|**setupadmin**|Члены **setupadmin** предопределенной роли сервера можно добавлять и удалять связанные серверы с помощью [!INCLUDE[tsql](../../../includes/tsql-md.md)] инструкции. (**sysadmin** членство необходимо при использовании [!INCLUDE[ssManStudio](../../../includes/ssmanstudio-md.md)].)|  
|**bulkadmin**|Члены **bulkadmin** предопределенной роли сервера можно запустить `BULK INSERT` инструкции.|  
|**diskadmin**|**Diskadmin** предопределенной роли сервера используется для управления файлами на диске.|  
...
|**открытые**|Каждый [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] входа принадлежит **открытый** роли сервера. Если для участника на уровне сервера не были предоставлены или запрещены конкретные разрешения на защищаемый объект, он наследует разрешения роли public на этот объект. Разрешения роли public следует назначать только тому объекту, который будет доступен всем пользователям. Нельзя изменить членство в роли public.<br /><br /> **Примечание:** **открытый** реализована не так, как другие роли и разрешения могут быть предоставленные, запрещенные или отмененные из открытых предопределенных ролей сервера.|  
