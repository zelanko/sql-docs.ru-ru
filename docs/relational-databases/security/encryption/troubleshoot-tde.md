...
Если для TDE настроено использование управляемого клиентом ключа в Azure Key Vault, нужно обеспечить постоянный доступ к этому предохранителю TDE, чтобы база данных сохраняла подключение к сети.  Если логический сервер SQL Server потеряет доступ к управляемому пользователем предохранителю TDE в Azure Key Vault, база данных заблокирует все подключения и станет недоступна на портале Azure.

Если проблему доступа к ключам в основном хранилище Azure Key Vault разрешить в течение 48 часов, база данных автоматически восстановится и подключится к сети.  Это означает, что во всех сценариях с периодическими или временными сбоями в сети вмешательство пользователя не требуется и база данных автоматически подключится к сети.  В большинстве случаев для устранения проблемы с доступом к ключу в основном хранилище ключей требуется вмешательство пользователя. 

Если недоступная база данных больше не нужна, вы можете сразу удалить ее, чтобы плата не начислялась.  Другие действия с базой данных запрещено выполнять, пока не восстановится доступ к ключу в Azure Key Vault и база данных снова не подключится к сети.   Вы также не сможете изменить на сервере параметр TDE с управляемых пользователем ключей на управляемые службой ключи, если база данных, зашифрованная с помощью управляемых пользователем ключей, недоступна. Это необходимо для защиты данных от несанкционированного доступа на период отзыва разрешений для предохранителя TDE. 

Если база данных остается недоступной дольше 48 часов, она не восстановится автоматически.  Если необходимый доступ к ключу в Azure Key Vault был восстановлен, вам нужно вручную повторно проверить доступ, чтобы база данных подключилась к сети.  Для подключения базы данных к сети после 48 часов недоступности может потребоваться много времени (в зависимости от размера базы данных). Чтобы выполнить такую процедуру, в настоящее время требуется обратится в службу поддержки. После подключения базы данных к сети будут сброшены ранее настроенные значения таких параметров, как географическая связь (если было настроено географическое аварийное восстановление), журнал PITR и теги.  Поэтому рекомендуется реализовать систему отправки уведомлений с помощью [групп действий](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups), которая позволит устранять проблемы с основным хранилищем ключей в течение 48 часов. 
...
**Событие, когда начинается 48-часовой период ожидания для самостоятельного восстановления** 

Имя события: MakeDatabaseInaccessible 

Состояние: InProgress 

Описание. База данных ожидает, что пользователь попытается повторно установить доступ к ключу в Azure Key Vault в течение 48 часов.   
...
**Событие, когда проблема не была разрешена в течение 48 часов и доступ к ключу в Azure Key Vault нужно проверить вручную** 
