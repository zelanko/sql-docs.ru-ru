---
title: "Configure Always Encrypted using SQL Server Management Studio (Настройка постоянного шифрования с помощью среды SQL Server Management Studio) | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2016"
...
helpviewer_keywords: 
  - "Постоянное шифрование, настройка с помощью среды SSMS"
...
---
# Configure Always Encrypted using SQL Server Management Studio (Настройка постоянного шифрования с помощью среды SQL Server Management Studio)
...
-   отправка значений открытого текста, предназначенных для зашифрованных столбцов (например, в инструкции `INSERT` или `UPDATE` и в инструкции `SELECT` в виде связанных подстановки предложения `WHERE`).   
...
*Пример*   
Предположим, что `SSN` — это зашифрованный столбец `char(11)` в таблице `Patients`. В этом случае если функция Always Encrypted, применяемая для подключения к базе данных, включена, в окне редактора запросов включена параметризация для Always Encrypted и при этом у вас есть доступ к главному ключу, настроенному для столбца `SSN`, то приведенный ниже скрипт произведет попытку найти строку, содержащую значение `'795-73-9838'` в столбце SSN, и вернет значение столбца `LastName`.   

![always-encrypted-patients](../../../relational-databases/security/encryption/media/always-encrypted-patients.png)
 
### <a name="a-nameen-disa-enabling-and-disabling-always-encrypted-for-a-database-connection"></a><a name="en-dis"></a> Включение и отключение функции Always Encrypted, применяемой для подключения к базе данных   
...
Чтобы включить функцию Always Encrypted, применяемую для подключения к базе данных, в диалоговом окне **Соединение с сервером** на вкладке **Дополнительные свойства** укажите параметр `Column Encryption Setting=Enabled`.    
Для отключения этой функции в диалоговом окне **Соединение с сервером** на вкладке **Дополнительные свойства** укажите параметр `Column Encryption Setting=Disabled` или же просто удалите значение параметра **Column Encryption Setting** (Параметр шифрования столбца) (по умолчанию используется значение **Отключено**).   
...
### <a name="a-nameparamaparameterization-for-always-encrypted"></a><a name="param"></a>Параметризация для Always Encrypted   
...
## <a name="migrating-databases-with-encrypted-columns-using-sql-server-import-and-export-wizard"></a>Миграция баз данных с зашифрованными столбцами с помощью мастера экспорта и импорта SQL Server

По сравнению с использованием BACPAC-файлов [мастер импорта и экспорта SQL Server](Import%20and%20Export%20Data%20Sources%20Supported%20for%20SQL%20Server.md) позволяет более полно контролировать способ обработки данных, хранящихся в зашифрованных столбцах, во время переноса данных.
...
- [CREATE COLUMN MASTER KEY (Transact-SQL)](../../../t-sql/statements/create-column-master-key-transact-sql.md) (Инструкция CREATE COLUMN MASTER KEY (Transact-SQL))
- [DROP COLUMN MASTER KEY (Transact-SQL)](../../../t-sql/statements/drop-column-master-key-transact-sql.md) (Инструкция DROP COLUMN MASTER KEY (Transact-SQL))
- [CREATE COLUMN ENCRYPTION KEY (Transact-SQL)](../../../t-sql/statements/create-column-encryption-key-transact-sql.md) (Инструкция CREATE COLUMN ENCRYPTION KEY (Transact-SQL))
- [ALTER COLUMN ENCRYPTION KEY (Transact-SQL)](../../../t-sql/statements/alter-column-encryption-key-transact-sql.md) (Инструкция ALTER COLUMN ENCRYPTION KEY (Transact-SQL))
- [DROP COLUMN ENCRYPTION KEY (Transact-SQL)](../../../t-sql/statements/drop-column-encryption-key-transact-sql.md) (Инструкция DROP COLUMN ENCRYPTION KEY (Transact-SQL)) 
