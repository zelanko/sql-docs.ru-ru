...
- Настройте Azure Key Vault в одной подписке исключительно для средств защиты TDE базы данных SQL.  Все базы данных, связанные с логическим сервером, используют одно средство защиты TDE, поэтому следует сгруппировать базы данных на логическом сервере. 
