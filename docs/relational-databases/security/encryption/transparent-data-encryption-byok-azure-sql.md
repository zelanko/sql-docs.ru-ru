...
- **Смена ключей**. Смена средств защиты TDE осуществляется согласно внутренним политикам или требованиям в области соответствия. Смены ключей можно производить с помощью хранилища ключей средства защиты TDE.  
...
- **Избыточность хранилища ключей**. Так как материал ключа никогда не покидает Azure Key Vault и сервер не имеет доступа к кэшированным копиям за пределами хранилища ключей, необходимо настроить георепликацию Azure Key Vault, чтобы обеспечить доступ к материалу ключа в случае, если в одном из регионов Azure Key Vault происходит сбой.  Геореплицированные базы данных утратят доступ к материалу ключа, если они используют только одно хранилище Azure Key Vault.
...
Чтобы обеспечить высокую доступность материала ключа в Azure Key Vault, необходимо настроить георепликацию для хранилища ключей.

- **Избыточное хранилище ключей**. Каждый геореплицированный сервер имеет доступ к отдельному хранилищу ключей, который в идеале находится в том же регионе Azure. Это рекомендуемая конфигурация, так как каждый сервер имеет собственную копию средства защиты TDE для зашифрованных геореплицированных баз данных. Если один из регионов Azure сервера переходит в автономный режим, другие серверы могут продолжать обращаться к геореплицированным базам данных.  Это необходимо тщательно настроить, чтобы в случае недоступности одного хранилища ключей сервер мог получать доступ к резервной копии средства защиты TDE в другом хранилище ключей.     
...
- Узнайте, как заменить средство защиты TDE сервера в соответствии с политиками безопасности: [Смена средства защиты прозрачного шифрования данных с помощью PowerShell](transparent-data-encryption-byok-azure-sql-key-rotation.md)
