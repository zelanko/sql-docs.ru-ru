...
В сценарии географического аварийного восстановления требуется база данных-получатель с дополнительным хранилищем ключей. Поддерживается до четырех баз данных-получателей.  Образование цепочек, что есть создание базы данных-получателя для базы данных-получателя, не поддерживается.  Во время начальной настройки служба проверяет, правильно ли настроены разрешения для основного и дополнительного хранилищ ключей.  Необходимо следить за этими разрешениями и регулярно проверять их наличие.

>[!NOTE]
>При назначении удостоверений первичному серверу и серверу-получателю сначала следует назначить его серверу-получателю.
>
