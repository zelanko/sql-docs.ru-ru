---
title: Планирование аттестации службы защиты узла | Документация Майкрософт
ms.custom: ''
ms.date: 10/12/2019
ms.prod: sql
ms.reviewer: vanto
...
Компьютер с [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] не требуется присоединять к домену, но если это уже сделано, домен должен быть отличным от используемого сервером HGS.
...
Клиент SQL и [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] должны иметь возможность взаимодействовать со службой HGS по протоколу HTTP. Для поддержки зашифрованных HTTPS-соединений в службе HGS можно настроить сертификат TLS.
...
В режиме аттестации TPM необходимо включить функцию безопасной загрузки, гарантирующую загрузку законного и подписанного корпорацией Майкрософт загрузчика и исключающую перехват процесса загрузки низкоуровневой оболочки программами rootkit или bootkit.
...
| ОЗУ | 8 ГБ (минимум) |
| Сетевые карты | Рекомендуется две сетевые карты (одна для трафика кластера и одна для службы HGS) |
