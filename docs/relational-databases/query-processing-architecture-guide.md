...
[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] создает интеллектуальные динамические планы, которые позволяют эффективно использовать распределенные запросы для доступа к данным в таблицах удаленных серверов: 
...
[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] иногда создает динамические планы выполнения даже для непараметризованных запросов. Оптимизатор запросов может параметризовать запрос так, чтобы план выполнения можно было использовать повторно. Если оптимизатор запросов параметризует запрос, ссылающийся на секционированное представление, он не будет знать, находятся ли нужные строки в заданной базовой таблице. В дальнейшем ему придется использовать динамические фильтры в планах выполнения.
...
[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] использует одни и те же алгоритмы определения степени параллелизма (общего числа отдельных рабочих потоков, которые будут запущены) как для операций с индексами, так и для других запросов. Максимальная степень параллелизма для операции с индексом определяется параметром конфигурации сервера [max degree of parallelism](../database-engine/configure-windows/configure-the-max-degree-of-parallelism-server-configuration-option.md) . Значение max degree of parallelism можно переопределять для отдельных операций с индексами путем настройки параметра индекса MAXDOP в инструкциях CREATE INDEX, ALTER INDEX, DROP INDEX и ALTER TABLE.
...
[!INCLUDE[ssKatmai](../includes/ssKatmai-md.md)] обладает повышенной эффективностью обработки запросов к секционированным таблицам для множества параллельных планов, измененным способом представления параллельных и последовательных планов и улучшенными сведениями о секционировании, содержащимися в планах выполнения времени компиляции и времени выполнения. В этом разделе описываются названные улучшения, содержится справочник об интерпретации планов выполнения запросов таблиц секционирования и индексов и дополнительные сведения об улучшении производительности запросов к секционированным объектам. 
