...
The inputs and outputs of the Query Optimizer during optimization of a single `SELECT` показаны на следующей схеме. ![query_processor_io](../relational-databases/media/query-processor-io.gif)
...
Параметр `EXPAND VIEWS` option specifies that the Query Optimizer not use any view indexes for the whole query. 

Если для представления задано указание `NOEXPAND` is specified for a view, the Query Optimizer considers using any indexes defined on the view. `NOEXPAND` может иметь необязательное предложение `INDEX()` clause forces the Query Optimizer to use the specified indexes. `NOEXPAND` может быть указано только для индексированного представления и не применяется для представлений без индексов.
...
Рекомендации по настройке MAXDOP см. в этой [статье технической поддержки Майкрософт](http://support.microsoft.com/help/2806535/recommendations-and-guidelines-for-the-max-degree-of-parallelism-configuration-option-in-sql-server).
