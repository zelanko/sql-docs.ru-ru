---
title: "Управление метаданными при обеспечении доступности базы данных на другом экземпляре сервера (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/23/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "межбазовые запросы [SQL Server]"
  - "входы в систему [SQL Server], повторное создание на другом экземпляре сервера"
  - "триггеры [SQL Server], DLL"
  - "пользовательские сообщения об ошибках [SQL Server]"
  - "разрешения [SQL Server], доступ к метаданным"
  - "механизм полнотекстового поиска [SQL Server]"
  - "метаданные [SQL Server], базы данных, доступные для других экземпляров"
  - "задания [агент SQL Server], повторное создание на другом экземпляре сервера"
  - "отработка отказа [SQL Server], управление метаданными"
  - "уведомления о событиях [SQL Server], метаданные"
  - "зеркальное отображение базы данных [SQL Server], метаданные"
  - "параметры запуска [SQL Server]"
  - "восстановление [SQL Server], на другом экземпляре сервера"
  - "связанные серверы [SQL Server], метаданные"
  - "Поставщик WMI для событий сервера, метаданные"
  - "присоединение баз данных [SQL Server]"
  - "доставка журналов [SQL Server], метаданные"
  - "шифрование [SQL Server], метаданные"
  - "конфигурация сервера [SQL Server]"
  - "распределенные запросы [SQL Server], метаданные"
  - "расширенные хранимые процедуры [SQL Server], метаданные"
  - "учетные данные [SQL Server], метаданные"
...
##  <a name="user_defined_error_messages"></a> Определяемые пользователем сообщения об ошибках  
 Определяемые пользователем сообщения об ошибках настраиваются в представлении каталога [sys.messages](../Topic/sys.messages%20\(Transact-SQL\).md), которое хранится в базе данных **master**. Если приложение базы данных зависит от определяемых пользователем сообщений об ошибках и если эта база данных доступна на другом экземпляре сервера, то для добавления на целевой экземпляр сервера уже имеющихся определяемых пользователем сообщений об ошибках следует пользоваться хранимой процедурой [sp_addmessage](../../relational-databases/system-stored-procedures/sp-addmessage-transact-sql.md).  
...
> **ВАЖНО!** [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] Используйте вместо этого [интеграцию со средой CLR](../Topic/Common%20Language%20Runtime%20\(CLR\)%20Integration%20Overview.md).  
...
> **ВАЖНО!** Прежде чем добавлять расширенные хранимые процедуры на сервер и предоставлять разрешение EXECUTE на них другим пользователям, системный администратор должен тщательно проверить каждую расширенную хранимую процедуру, чтобы убедиться, что она не содержит вредоносного или злонамеренного кода.  
...
##  <a name="triggers"></a> Триггеры уровня сервера  
