...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)] Системная база данных **tempdb** — это глобальный ресурс, доступный всем пользователям, подключенным к данному экземпляру [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], в котором хранятся следующие объекты:  
...
- **Внутренние объекты**, созданные с помощью [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]. К ним относятся следующие объекты.
  ...
  > [!NOTE]
  > Каждый внутренний объект использует минимум девять страниц: одну IAM-страницу и один восьмистраничный экстент. Дополнительные сведения о страницах и экстентах см. в разделе [Страницы и экстенты](../../relational-databases/pages-and-extents-architecture-guide.md#pages-and-extents).
...
## <a name="physical-properties-of-tempdb"></a>Физические свойства базы данных tempdb  
 Следующая таблица описывает исходную конфигурацию данных и файлов журналов базы данных **tempdb**, которая основывается на значениях по умолчанию для базы данных модели. Размеры этих файлов могут немного изменяться в зависимости от выпуска [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
...
|Журнал|templog|templog.ldf|8 МБ|Автоматическое увеличение на 64 МБ до максимального размера в 2 ТБ.|  
 ...
 \* Количество файлов зависит от числа (логических) процессоров на компьютере. Как правило, если число логических процессоров меньше или равно 8, используйте то же число файлов данных, что и логические процессоры. Если число логических процессоров больше 8, используйте 8 файлов данных, а затем, если подключение сохранится, увеличивайте число файлов данных на значение, кратное 4, пока состязание не уменьшится до приемлемого уровня, или внесите изменения в рабочую нагрузку или код.
...
## <a name="optimizing-tempdb-performance"></a>Оптимизация производительности базы данных tempdb
...
- Программа установки добавляет несколько файлов данных tempdb при установке нового экземпляра. Эту задачу можно выполнить с помощью управления вводом в новом пользовательском интерфейсе раздела **Настройка компонента Database Engine** и параметра командной строки /SQLTEMPDBFILECOUNT. По умолчанию программа установки добавит число файлов данных tempdb, равное количеству логических процессоров, либо 8 файлов. Это зависит от того, какое значение меньше.  
...
## <a name="capacity-planning-for-tempdb"></a>Планирование размера базы данных tempdb
 Определение требуемого размера базы данных tempdb в рабочей среде зависит от множества факторов. Как описано выше в этом разделе, эти факторы включают текущую рабочую нагрузку и используемые возможности [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Рекомендуется проанализировать текущую рабочую нагрузку, выполнив следующие задачи в среде тестирования SQL Server:
