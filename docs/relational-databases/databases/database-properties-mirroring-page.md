---
title: "Свойства базы данных (страница &#171;Зеркальное отображение&#187;) | Microsoft Docs"
ms.custom: ""
ms.date: "08/25/2016"
...
> **ПРИМЕЧАНИЕ.** Если на экземпляре сервера существует только зеркальная база данных, то задание мониторинга удаляется.  
...
> **ВАЖНО!** Если нажать кнопку **Отработка отказа** после изменения свойств в окне **Свойства базы данных** эти изменения будут потеряны. Чтобы сохранить текущие изменения, нажмите кнопку **Нет** в запросе подтверждения и сохраните изменения, нажав кнопку **ОК** . Затем снова откройте окно свойств базы данных и нажмите кнопку **Отработка отказа**.  
  
 **Режим работы**  
 При необходимости можно изменить режим работы. Доступность определенных режимов работы зависит от того, задан ли TCP-адрес для следящего сервера. Существуют следующие параметры выбора.  
...
### Наблюдение за зеркальным отображением базы данных  
