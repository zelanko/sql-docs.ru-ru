...
description: Отслеживание экземпляра SQL Server для определения того, находятся ли показатели загрузки ЦП в нормальном диапазоне. Использование системного монитора для определения того, сколько времени ЦП тратит на выполнение потока.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
