...
description: Сведения о том, как создавать диаграммы, предупреждения, журналы и отчеты для отслеживания экземпляра SQL Server с помощью системного монитора.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
