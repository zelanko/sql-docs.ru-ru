---
title: Параметр, оцененный таблицей, Data-At-Execution (ODBC)
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  Это похоже на процедуру [All in Memory,](../../relational-databases/native-client-odbc-table-valued-parameters/sending-data-as-a-table-valued-parameter-with-all-values-in-memory-odbc.md) но использует для параметра, оцениваемого таблицей, данные по исполнению.  
  
 Для другой выборки, демонстрирующей параметры, оценивающие таблицу, [&#41;&#40;](../../relational-databases/native-client-odbc-how-to/use-table-valued-parameters-odbc.md)см.  
  
 В этом примере, когда вызывается S'LExecute или S'LExecDirect, водитель возвращается SQL_NEED_DATA. Затем приложение неоднократно вызывает S'LParamData до тех пор, пока водитель не вернет значение, не SQL_NEED_DATA. Водитель возвращает *ParameterValuePtr,* чтобы сообщить приложению, для какого параметра он запрашивает данные. Приложение вызывает S'LPutData для предоставления данных параметров до следующего звонка в S'LParamData. Для параметра, оцениваемого таблицей, вызов на S'LPutData указывает, сколько строк он подготовил для драйвера (в этом примере всегда 1). Когда все строки значения таблицы были переданы драйверу, s'LPutData вызывается, чтобы указать, что доступны 0 строк.  
  
 Можно использовать значения данных времени выполнения в строках табличного значения. Значение, возвращенное S'LParamData, информирует приложение, которое требуется водителю. Как и в случае с обычными значениями параметров, s LPutData можно назвать одним или несколько раз для символа или значения столбца бинарной таблицы. Это позволяет приложению передавать большие значения по частям.  
  
 При вызове s'LPutData для оценки таблицы *DataPtr* используется для количества доступных строк (в этом примере всегда 1). *StrLen_or_IndPtr* всегда должно быть 0. Когда все строки таблицы-значения были пройдены, s'LPutData вызывается со значением *DataPtr* 0.  
...
2.  Выполните привязку параметров. *ColumnSize* 1, что означает, что не более одного ряда передается за один раз.  
...
5.  Вызов процедуры. SLExecDirect вернется SQL_NEED_DATA, поскольку параметр, оцениваемый таблицей, является параметром данных по исполнению.  
...
6.  Ввод данных времени выполнения для параметров. Когда S'LParamData возвращает *ПараметрValuePtr* для параметра, оцениваемого таблицей, приложение должно подготовить столбцы для следующего ряда или строк и таблицы. Затем приложение вызывает S'LPutData с *DataPtr,* установленным на количество доступных строк (в данном примере, 1) и *StrLen_or_IndPtr* установлено до 0.  
...
### <a name="description"></a>Описание  
 Этот пример показывает, что вы можете использовать строку потоковой передачи, один ряд на вызов в S'LPutData, с ODBC TVP, подобно тому, как вы можете использовать BCP.exe для загрузки данных в базу данных.  
...
### <a name="description"></a>Описание  
 Этот пример показывает, что вы можете использовать строку потокового, несколько строк на вызов в S'LPutData, с ODBC TVP, подобно тому, как вы могли бы использовать BCP.exe для загрузки данных в базу данных.  
