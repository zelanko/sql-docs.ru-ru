...
```  
 ...
 Пространство имен **Microsoft. SqlServer. Server** содержит объекты, необходимые для различных атрибутов определяемого пользователем типа, а пространство имен **System. Data. SqlTypes** содержит классы, которые [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] представляют собственные типы данных, доступные для сборки. Разумеется, конкретной сборке для правильной работы могут понадобиться и другие пространства имен. Определяемый пользователем тип **Point** также использует пространство имен **System. Text** для работы со строками.  
