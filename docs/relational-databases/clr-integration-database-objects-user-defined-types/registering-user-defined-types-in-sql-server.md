...
description: Необходимо зарегистрировать определяемый пользователем тип, прежде чем устанавливать его в SQL Server. Необходимо зарегистрировать сборку и создать тип в базе данных, где вы хотите ее использовать.
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
ms.reviewer: ''
...
```  
 ...
 Следующая [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкция регистрирует сборку с помощью *<assembly_bits>* аргумент в предложении FROM. Это значение **varbinary** представляет файл в виде потока байтов.  
...
> [!NOTE]  
>  Синтаксис CREATE TYPE также используется для создания собственных [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] псевдонимов типов данных и предназначен для замены **sp_addtype** как средства создания псевдонимов типов данных. Некоторые из дополнительных аргументов в синтаксисе CREATE TYPE служат для создания определяемых пользователем типов и неприменимы для создания псевдонимов типов данных (например базового типа).  
  
 Дополнительные сведения см. в разделе [CREATE TYPE &#40;Transact-SQL&#41;](../../t-sql/statements/create-type-transact-sql.md).  
  
#### <a name="example"></a>Пример  
 Следующая [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкция создает тип **Point** . ВНЕШНее имя задается с помощью синтаксиса имен *AssemblyName*, состоящие из двух частей. *Удтнаме*.  
...
### <a name="using-alter-assembly-to-add-source-code"></a>Использование инструкции ALTER ASSEMBLY для добавления исходного кода  
 ...
 Следующая [!INCLUDE[tsql](../../includes/tsql-md.md)] инструкция ALTER ASSEMBLY добавляет исходный код класса Point.cs для определяемого пользователем типа **Point** . В результате этого текст, содержащийся в файле Point.cs, будет скопирован и сохранен в базе данных с именем PointSource.  
