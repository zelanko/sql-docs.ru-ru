...
1.  Создайте новый проект **базы данных** на узлах **Visual Basic** или **Visual C#**  Language.  
...
#### <a name="example"></a>Пример  
 Следующая инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] регистрирует сборку Point в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] базы данных **AdventureWorks** с набором разрешений Сейф. Если предложение WITH PERMISSION_SET не указано, сборка регистрируется с набором разрешений SAFE.  
...
```  
 ...
 Следующая инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] регистрирует сборку с помощью аргумента *< assembly_bits >* в предложении FROM. Это значение **varbinary** представляет файл в виде потока байтов.  
...
> [!NOTE]  
>  Синтаксис CREATE TYPE также используется для создания собственных [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] псевдонимов типов данных и предназначен для замены процедуры **sp_addtype** как средством создания псевдонимов типов данных. Некоторые из дополнительных аргументов в синтаксисе CREATE TYPE служат для создания определяемых пользователем типов и неприменимы для создания псевдонимов типов данных (например базового типа).  
...
### <a name="example"></a>Пример  
 Следующая инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] должна выполняться в следующем порядке. Сначала таблица, которая ссылается на определяемый пользователем тип **Point** , должна быть удалена, затем тип и, наконец, сборка.  
...
### <a name="using-alter-assembly-to-add-source-code"></a>Использование инструкции ALTER ASSEMBLY для добавления исходного кода  
 ...
 Следующая инструкция [!INCLUDE[tsql](../../includes/tsql-md.md)] ALTER ASSEMBLY добавляет исходный код класса Point.cs для определяемого пользователем типа **Point** . В результате этого текст, содержащийся в файле Point.cs, будет скопирован и сохранен в базе данных с именем PointSource.  
 ...
 **file_id**  
 Число, идентифицирующее каждый объект с первым объектом, связанным с заданным **assembly_id** , которому присваивается значение 1. Если с одним и тем же **assembly_id**связано несколько объектов, каждое последующее значение **file_id** увеличивается на 1.  
