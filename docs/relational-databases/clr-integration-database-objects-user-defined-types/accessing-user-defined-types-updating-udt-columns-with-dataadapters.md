---
title: Обновление столбцов UDT с помощью DataAdapters (ru) Документы Майкрософт
description: UDTs в базе данных сервера S'L поддерживаются с помощью System.Data.DataSet и System.Data.SqlClient.SqlDataAdapter для получения и изменения данных.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
  Типы, определяемые пользователями (UDT), поддерживаются с помощью **System.Data.DataSet** и **System.Data.SqlClient.SqlDataAdapter** для получения и изменения данных.  
  
## <a name="populating-a-dataset"></a>Заполнение набора данных  
 Инструкцию [!INCLUDE[tsql](../../includes/tsql-md.md)] SELECT можно использовать для выбора значений столбцов определяемых пользователем типов, чтобы заполнить набор данных с помощью адаптера обработки данных. Следующий пример предполагает, что у вас есть таблица **очков,** определяемая со следующей структурой и некоторыми выборочными данными. Следующие [!INCLUDE[tsql](../../includes/tsql-md.md)] операторы создают таблицу **очков** и вставляют несколько строк.  
...
```  
 ...
 Следующий ADO.NET фрагмент кода извлекает действительную строку соединения, создает новый **SqlDataAdapter**и заполняет **System.Data.DataTable** рядами данных из таблицы **Очков.**  
...
## <a name="updating-udt-data-in-a-dataset"></a>Обновление данных определяемых пользователем типов в наборе данных  
 Вы можете использовать два метода для обновления столбца UDT в **DataSet:**  
  
-   Предоставьте пользовательские объекты **InsertCommand,** **UpdateCommand** и **DeleteCommand** для объекта **SqlDataAdapter.**  
  
-   Используйте командостроитель **(System.Data.SqlClient.SqlCommandBuilder)** для автоматического создания команд INSERT, UPDATE и DELETE для вас. Чтобы иметь обнаружение конфликтов, добавьте **столбец метки времени** (псевдоним **rowversion)** в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] таблицу, содержащую UDT. Тип данных **метки времени** позволяет штамповать строки в таблице и гарантированно будет уникальным в базе данных. Если значение в таблице изменяется, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] автоматически обновляет 8-байтовое двоичное число для строки, затронутой изменением.  
  
 Обратите внимание, что **SqlCommandBuilder** не учитывает UDT для обнаружения конфликтов, если в основной таблице нет столбца **метки времени.** Определяемые пользователем типы могут быть несопоставимы, поэтому они не включаются в предложение WHERE, если для формирования команды используется режим сравнения с первоначальными значениями.  
  
### <a name="example"></a>Пример  
 Следующий пример требует создания второй таблицы, содержащей столбец **Point** UDT, а также **столбец метки времени.** Обе таблицы используются для иллюстрации того, как создавать пользовательские объекты команд для обновления данных и как обновлять с помощью столбца **метки времени.** Запустите следующие инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)], чтобы создать вторую таблицу и заполнить ее образцами данными.  
...
-   **UserProvidedCommands**, который демонстрирует, как поставлять **вставитьCommand**, **UpdateCommand**, и **DeleteCommand** объектов для обновления **точки** UDT в таблице **точек** (которая не содержит **столбец метки времени).**  
  
-   **CommandBuilder**, который демонстрирует, как использовать **SqlCommandBuilder** в **таблице Points_ts,** которая содержит **столбец метки времени.**  
