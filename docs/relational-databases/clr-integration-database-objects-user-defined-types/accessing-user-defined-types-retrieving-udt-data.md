---
title: Получение данных UDT (англ.) Документы Майкрософт
description: В этой статье описывается, как получить доступ к UDTs в базе данных сервера S'L.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
## <a name="requirements-for-using-udts-in-adonet"></a>Требования к использованию определяемых пользователем типов в ADO.NET  
 Чтобы создать на клиенте определяемый пользователем тип, сборка, загруженная в [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], и сборка на клиенте должны быть совместимы. Для UDT, определяемых в формате сериализации **Native,** сборки должны быть структурно совместимы. Для сборок, определяемых с форматом **UserDefined,** сборка должна быть доступна для клиента.  
 ...
 **System.Data.SqlClient.SqlDataReader** может быть использован из кода клиента для получения набора результатов, который содержит столбец UDT, который подвергается как экземпляр объекта.  
  
### <a name="example"></a>Пример  
 В этом примере показано, как использовать **основной** метод для создания нового объекта **SqlDataReader.** В этом примере кода выполняются следующие действия.  
  
1.  Основной метод создает новый объект **SqlDataReader** и извлекает значения из таблицы Очков, которая имеет столбец UDT под названием Точка.  
  
2.  Определяемый пользователем тип Point представляет координаты X и Y, определенные как целые числа.  
  
3.  UDT определяет метод **расстояния** и метод **GetDistanceFromXY.**  
...
5.  Код образца называется методами **Point.Distance** и **Point.GetDistanceFromXY.**  
...
## <a name="binding-udts-as-bytes"></a>Привязка определяемых пользователем типов в виде байт  
 Иногда может потребоваться получить необработанные данные из столбца определяемого пользователем типа. Возможно, что тип недоступен локально, либо создание экземпляра определяемого пользователем типа нежелательно. Вы можете прочитать сырые байты в массив байта, используя метод **GetBytes** **SqlDataReader.** Этот метод считывает поток байт с указанного смещения столбца в буфер в виде массива, начиная с указанного смещения. Другой вариант заключается в использовании одного из методов **GetSqlBytes** или **GetSqlBinary** и прочитать все содержимое в одной операции. В любом случае объект определяемого пользователем типа не создается, так что в клиентской сборке создание ссылки на определяемый пользователем тип не требуется.  
  
### <a name="example"></a>Пример  
 В этом примере показано, как получить данные **Point** в качестве необработанных байтов в массив байтов с помощью **SqlDataReader.** Код использует **System.Text.StringBuilder** для преобразования необработанных байтов в репрезентативное представление строки, которое будет отображаться в окне консоли.  
...
### <a name="example-using-getsqlbytes"></a>Пример использования функции GetSqlBytes  
 В этом примере показано, как получить данные **Point** в качестве необработанных байтов в одной операции с помощью метода **GetSqlBytes.** Код использует **StringBuilder** для преобразования необработанных байтов в репрезентативное представление строки, которое будет отображаться в окне консоли.  
...
## <a name="using-udts-in-query-parameters"></a>Использование определяемых пользователем типов в параметрах запроса  
 UDT s можно использовать в качестве значений параметров при настройке **SlParameter** для объекта **System.Data.SqlClient.SqlCommand.** Перечисление **объекта SqlDbType.Udt** объекта **SqlParameter** используется для указания на то, что параметр является UDT при вызове метода **добавления** в коллекцию **параметров.** Свойство объекта **SqlCommand** **UdtTypeName** используется для указания полностью квалифицированного имени UDT в базе данных с помощью синтаксиса *database.schema_name.object_name.* Несмотря на то, что использование полного имени не требуется, это удаляет неоднозначность из кода.  
  
> [!NOTE]  
>  Локальная копия сборки определяемого пользователем типа должна быть доступна в проекте клиента.  
  
### <a name="example"></a>Пример  
 Код в этом примере создает объекты **SqlCommand** и **SqlParameter** для вставки данных в столбец UDT в таблице. Код использует **slDbType.Udt** перечисление для указания типа данных, а также свойство **UdtTypeName** объекта **SqlParameter** для указания полностью квалифицированного имени UDT в базе данных.  
