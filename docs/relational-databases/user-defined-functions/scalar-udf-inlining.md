...
- Определяемая пользователем функция не может содержать ссылки на встроенные функции (такие как @@ROWCOUNT), которые могут изменять результаты при встраивании (ограничение, добавленное в Microsoft SQL Server 2019 CU2).
- Определяемая пользователем функция не может содержать агрегатные функции, передаваемые в качестве параметров в скалярную определяемую пользователем функцию (ограничение, добавленное в Microsoft SQL Server 2019 CU2).
- Определяемая пользователем функция не может ссылаться на встроенные представления, например OBJECT_ID (ограничение, добавленное в Microsoft SQL Server 2019 CU2).
-   Определяемая пользователем функция не может ссылаться на методы XML (ограничение, добавленное в Microsoft SQL Server 2019 CU4).
-   Определяемая пользователем функция не может содержать инструкцию SELECT с предложением ORDER BY без "TOP 1" (ограничение, добавленное в Microsoft SQL Server 2019 CU4).
-   Определяемая пользователем функция не может содержать запрос SELECT, выполняющий назначение, в сочетании с предложением ORDER BY, например "SELECT @x = @x + 1 FROM имя_таблицы ORDER BY имя_столбца" (ограничение, добавленное в Microsoft SQL Server 2019 CU4).
- Определяемая пользователем функция не может содержать несколько инструкций RETURN (ограничение, добавленное в SQL Server 2019 CU5).
- Определяемая пользователем функция не может вызываться из инструкции RETURN (ограничение, добавленное в SQL Server 2019 CU5).
- Определяемая пользователем функция не может ссылаться на функцию STRING_AGG (ограничение, добавленное в SQL Server 2019 CU5). 
...
> [!NOTE]
> Сведения о последних исправлениях и изменениях встраивания скалярных пользовательских функций T-SQL см. в статье базы знаний [Устранение проблем встраивания скалярных пользовательских функций в SQL Server 2019](https://support.microsoft.com/en-us/help/4538581/fix-scalar-udf-inlining-issues-in-sql-server-2019).
...
[Устранение проблем встраивания скалярных пользовательских функций в SQL Server 2019](https://support.microsoft.com/en-us/help/4538581/fix-scalar-udf-inlining-issues-in-sql-server-2019)     
